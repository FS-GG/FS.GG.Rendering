...
- [ ] T006 [P] Define the per-corpus preamble format + in-fence `open`-directive (research.md D2) as reviewed config in the repo; document the default `open` set per corpus. **Lock the format against the real fences proven in T005 before generalizing** — this is the plan's highest-risk decision (the "which opens are in scope" that only the compiler answers); do not widen it beyond what a real fence needs. **Diagnostic (measured, CORRECTED):** an initial single-project drive appeared to compile 72/77 clean, but that was a **masking artifact** — ~4 fences had parse errors (`FS0010`) that halted type-checking of the rest. After marking those `docfences:skip` (exploratory) and re-driving, the true picture emerged: **7 of ~13 skill docs fail isolated compilation** (elmish, audio, model-swap, testing, symbology, line-drawing, grids), with `FS0039 'X is not defined'` on reader-owned identifiers (`Model`, `Msg`, `update`, `view`, `AppRoot`, `roster`, `host`, `theme`), symbols defined in an EARLIER fence of the same doc (cross-fence continuity), and modules needing qualified opens (`ControlsElmish`, `Audio`, `Expect`). **Implication:** the naive "one fence = one isolated compilation unit" does NOT work for the pedagogical skill corpus — this reopens D2. Resolution options (needs a decision): per-doc concatenation, a shared product-type prelude, skip the pedagogical majority (low value), or apply fence-compilation only to self-contained corpora (scaffold sources) and keep the symbol-oracle for skills. **Second discovery:** the shipped skills are **frozen + manifest-tracked** (`scripts/FrozenMirrorVerdict.fs` Feature541, `scripts/generate-skill-manifest.fsx` Feature231) — even an invisible HTML `docfences:` comment trips both guards, so authoring directives into real skills requires a deliberate re-freeze + re-manifest and must be tied to the design decision, not done ad hoc. The exploratory skill edits were reverted; the directive mechanism is proven against a fixture (CorpusTests T007). See DriveTests (pending) and the design fork raised on this feature.
