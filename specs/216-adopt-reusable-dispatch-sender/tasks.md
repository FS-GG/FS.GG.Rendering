...
- [X] T012 [US1] **DISCLOSED-DEFERRED live evidence (Layer 3, FR-009/SC-005 — do NOT fake green)**: DONE 2026-06-28. Three honest live fires (see `readiness/live-evidence.md`): #1 old bespoke sender, #2 reusable sender + wrong secret-name assumption (`APP_ID` read empty), #3 GREEN after mapping to the real org secrets `FSGG_DISPATCH_APP_ID`/`FSGG_DISPATCH_APP_PRIVATE_KEY` (fix commit `c74e9ab`). Sender run https://github.com/FS-GG/FS.GG.Rendering/actions/runs/28330983926 ✅; receiver run https://github.com/FS-GG/FS.GG.Templates/actions/runs/28330993001 (delivered, branch pushed); pin-bump PR https://github.com/FS-GG/FS.GG.Templates/pull/21. `FS-GG/FS.GG.Rendering#10` closed; board Blocked → Done. Residual Templates-side follow-up: enable Actions PR-creation so future bumps auto-open.
