...
/// to hold inside a `Model`. Items are bucketed by `FS.GG.UI.Scene.Point` position at a fixed cell size;
...
/// Build and query a `SpatialGrid`. Reuses `FS.GG.UI.Scene.Point`/`Rect` (and the 239 `Geometry`
/// containment helpers) rather than a look-alike geometry vocabulary. Pure and deterministic
/// throughout: identical items in identical order with an identical query yield an identical result list.
