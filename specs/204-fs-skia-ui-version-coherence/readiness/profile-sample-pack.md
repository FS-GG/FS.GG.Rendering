# Profile readiness — `sample-pack` (US1 / CV-1..CV-5)

**Pinned version:** `0.1.50-preview.1` · **Generated:** `dotnet new fs-gg-ui --profile sample-pack --name Product -o /tmp/fsgg-sample-pack`

| Step | Result | Evidence |
|------|--------|----------|
| Generate | ✅ | full sample-pack product (Scene, SkiaViewer, Elmish, … + sample content) |
...
| Layout evidence (`--layout-evidence`) | ✅ `exit=0` | `status=ok proof-level=ReadableLayout hud-region=summary:0,0,640,96` (CV-3) |
...
| Governance (`dotnet test Product.Tests`) | ✅ `test_exit=0` | **Passed! 29/29** (CV-3) |
