...
- [X] T003 LIVE DONE — packed coherent set .1.63 to local feed; scaffolded a game product from the live template, built + tested it (26/26). §3.5/§3.6 confirmed in-product. §3.4 collision is context-dependent (adjacent-def repro compiles); doc present. See readiness/live-smoke.md.
...
- [~] T006 [US1] PARTIAL — no clean minimal live repro of the collision (local type def wins F# resolution); product.md guidance present + qualified form compiles. Faithful repro needs the feedback cross-file mapKey structure. — Verify against the live scaffold from T003: apply the documented remedy → the product compiles (`dotnet build`); capture the before/after in `readiness/live-smoke.md`.
...
- [X] T009 [US2] LIVE DONE — (Scene.measureText "SCORE 0" font).Width compiles in the scaffolded product; no new measurer (FR-005). — Verify the snippet compiles in the scaffold and positions text from metrics (`dotnet build` + eyeball/headless HUD-bounds evidence); record in `readiness/live-smoke.md`. Confirm no new measurer was added (FR-005).
...
- [X] T014 [US3] Consume in the product template: in `template/base/src/Product/Model.fs` change `model, []` → `model, Cmd.none` and the `subscriptions` `[]` → `Sub.none`; in `template/base/src/Product/EvidenceCommands.fs` change the command no-op `[]` → `Cmd.none`. Behaviour-preserving (FR-006). CORRECTION (live build): EvidenceCommands.fs host Init is `ViewerEffect list`, not AdapterCommand — reverted those `[]`; §3.5 scoped to Model.fs update/subscriptions only.
...
- [X] T016 [US3] LIVE DONE — fresh scaffold builds green (main+tests) with Cmd.none/Sub.none; product tests 26/26. — Confirm T010 now passes; verify the template still builds/tests (`dotnet build` + `dotnet test` on the generated product template) and the aliases resolve unambiguously in the scaffold (no `open Elmish` ambiguity). Record in `readiness/live-smoke.md`.
