...
| Redundant `internal`/`private` access modifiers in `RetainedRender.fs` | Inherited verbatim from the imported source. | Stripping them is a behavior-neutral Tier-2 cleanup touching one imported file; bundling it into this backfill would mix a documentation pass with a code edit. Scoped as a bounded follow-up (tasks.md **DF-1**), not done here. |
