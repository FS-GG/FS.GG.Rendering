...
- [X] T001 Create the fragment directory `template/fragments/vec2/src/Product/` (empty placeholder; body authored later), and the readiness dir `specs/250-collision-safe-vec2/readiness/`
- [X] T002 Establish the no-regression baseline: `dotnet fsi scripts/baseline-tests.fsx --out specs/250-collision-safe-vec2/readiness/baseline.md` (globs EVERY `*.Tests.fsproj` — solution + Package.Tests + samples — and records the full red/green set; pre-existing reds are flagged here, not discovered at merge)
