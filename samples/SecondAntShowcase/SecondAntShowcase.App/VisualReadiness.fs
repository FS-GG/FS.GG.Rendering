...
let runCapture args =
    ...
    | Some seedText, Some sizeText, Some themeText ->
        ...
        | (true, seed), Result.Ok size, Result.Ok themes, Result.Ok pages ->
            let outDir = flag "--out" args |> Option.defaultValue "artifacts/second-ant-showcase-visual-readiness"
            ...
            printfn "second-ant-showcase: visual-readiness %s, screenshots %d/%d under %s" summary.VisualReadinessStatus summary.PresentScreenshotCount summary.RequiredScreenshotCount outDir
            0
        | (false, _), _, _, _ ->
    ...
    | Some dir when Directory.Exists dir ->
        ...
        let existing =
            if File.Exists summaryPath then
                ...
                "# Feature 171 Second Ant Showcase Visual Readiness Summary" + Environment.NewLine + Environment.NewLine
        ...
        if update.SafeToWrite then
            ...
            eprintfn "second-ant-showcase: refused to update malformed visual-readiness managed section: %s" (String.concat "; " update.Diagnostics)
