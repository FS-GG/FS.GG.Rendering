...
module CoreEvidence = SecondAntShowcase.Core.Evidence
    ...
    try
        ...
        if List.isEmpty bitmaps then
            ...
            |> List.iteri (fun index bitmap ->
                ...
                let dest =
                    SKRect.Create(
                        float32 (col * tileWidth),
                        float32 (row * tileHeight),
                        float32 tileWidth,
                        float32 tileHeight
                    )
...
let reviewerStatus outDir (targets: VisualCaptureTarget list) =
    ...
    if not (File.Exists path) then
        ...
        let hasPending =
            not parsed.MissingTargetIds.IsEmpty || not parsed.PendingTargetIds.IsEmpty

        let hasBlocking =
            parsed.Classifications
            |> List.exists (fun row -> row.Severity = VisualReviewerBlocking)

        let hasMalformed =
            not parsed.DuplicateTargetIds.IsEmpty
            || not parsed.UnknownTargetIds.IsEmpty
            || not parsed.MalformedRows.IsEmpty

        if hasPending then
            "missing", hasBlocking, parsed.Classifications
        elif hasBlocking || hasMalformed then
            "critical", true, parsed.Classifications
        else
            "clear", false, parsed.Classifications
...
let captureScreenshot
    ...
    themeId
    (page: Page)
    : CoreEvidence.VisualScreenshotRecord =
...
let writeCompleteness
    outDir
    (summary: CoreEvidence.VisualReadinessSummary)
    (records: CoreEvidence.VisualScreenshotRecord list)
    =
    ...
    let degraded =
        ...
        |> List.map (fun r ->
            sprintf "- `%s` `%s`: %s" r.PageId r.ThemeId (r.DegradedReason |> Option.defaultValue r.Completeness))

    File.WriteAllText(
        Path.Combine(dir, "summary.md"),
        sprintf
            "# Completeness\n\n- status: **%s**\n- present: `%d/%d`\n"
            summary.CompletenessStatus
            summary.PresentScreenshotCount
            summary.RequiredScreenshotCount
    )

    File.WriteAllText(
        Path.Combine(dir, "missing.md"),
        "# Missing\n\nMissing screenshots are represented as degraded capture records in this run.\n"
    )

    File.WriteAllText(
        Path.Combine(dir, "degraded.md"),
        "# Degraded\n\n"
        + (if List.isEmpty degraded then
               "None\n"
           else
               String.concat Environment.NewLine degraded + Environment.NewLine)
    )

    File.WriteAllText(Path.Combine(dir, "dimensions.md"), sprintf "# Dimensions\n\nExpected size: `%s`\n" summary.Size)

let captureRecordFor
    (records: CoreEvidence.VisualScreenshotRecord list)
    ...
    let screenshot =
        ...
        |> List.tryFind (fun record ->
            record.PageId = target.Page.PageId
            && record.ThemeId = target.Theme.ThemeId
            && record.RelativePath = target.RelativePath)
...
let writeContactSheetMetadata outDir (sheets: VisualContactSheet list) =
    ...
    File.WriteAllText(
        Path.Combine(outDir, "contact-sheets.json"),
...
let buildSummary
    ...
    (records: CoreEvidence.VisualScreenshotRecord list)
    (contactSheetMetadata: VisualContactSheet list)
    reviewerStatus
    _critical
    reviewerClassifications
    : CoreEvidence.VisualReadinessSummary =
    ...
    let limitations =
        ...
        |> List.choose (fun r ->
            r.DegradedReason
            |> Option.map (fun reason -> sprintf "%s/%s: %s" r.ThemeId r.PageId reason))
    ...
    let present =
        captureRecords
        |> List.filter (fun r -> r.Status = VisualCaptureComplete)
        |> List.length

    let completeness = if present = required then "complete" else "incomplete"

    let captureAvailability =
        if report.ReadinessStatus = VisualReadinessEnvironmentLimited then
            "environment-limited"
...
let runCapture args =
    ...
    | Some seedText, Some sizeText, Some themeText ->
        match
            Int32.TryParse seedText,
            VisualConfig.parseSize sizeText,
            VisualConfig.resolveThemeList themeText,
            pageSelection (flag "--pages" args)
        with
        | (true, seed), Result.Ok size, Result.Ok themes, Result.Ok pages ->
            let outDir =
                flag "--out" args
                |> Option.defaultValue "artifacts/second-ant-showcase-visual-readiness"
                ...
                |> List.choose (fun themeId ->
                    if completeTheme themeId then
                        ...
                        let imagePaths =
                            ...
                            |> List.map (fun r ->
                                Path.Combine(outDir, r.RelativePath.Replace("/", string Path.DirectorySeparatorChar)))

                        writeContactSheet (Path.Combine(outDir, name)) imagePaths

                        Some
                            {
                                SheetId = sprintf "second-ant-showcase-%s-%s" (VisualConfig.roleName role) folder
                                RelativePath = name
                                SizeRole = Some(VisualConfig.roleName role)
                                ThemeId = Some themeId
                                TargetIds =
                                    sharedTargets
                                    |> List.filter (fun target -> target.Theme.ThemeId = themeId)
                                    |> List.map _.TargetId
                                MissingTargetIds = []
            ...
            let reviewer, critical, reviewerClassifications =
                reviewerStatus outDir sharedTargets
            ...
            printfn
                "second-ant-showcase: visual-readiness %s, screenshots %d/%d under %s"
                summary.VisualReadinessStatus
                summary.PresentScreenshotCount
                summary.RequiredScreenshotCount
                outDir
            ...
            0
        | (false, _), _, _, _ ->
            eprintfn "second-ant-showcase: --seed must be an integer (got '%s')." seedText
            2
        ...
        | _, _, _, Result.Error error ->
            ...
            2
    | _ ->
        eprintfn "second-ant-showcase: visual-readiness requires --seed <int> --size <width>x<height> --themes <list>."
        2
    ...
    | Some dir when Directory.Exists dir ->
        ...
        let existing =
            if File.Exists summaryPath then
                ...
                "# Feature 171 Second Ant Showcase Visual Readiness Summary"
                + Environment.NewLine
                + Environment.NewLine
        ...
        if update.SafeToWrite then
            ...
            eprintfn
                "second-ant-showcase: refused to update malformed visual-readiness managed section: %s"
                (String.concat "; " update.Diagnostics)

            failwith "malformed visual-readiness managed section"

        printfn "second-ant-showcase: wrote readiness summary under %s" outDir
        0
    | Some dir ->
        eprintfn "second-ant-showcase: --summarize directory does not exist: %s" dir
        2
    | None -> runCapture args

let run args =
    if hasFlag "--summarize" args then
        runSummary args
    else
        runCapture args
