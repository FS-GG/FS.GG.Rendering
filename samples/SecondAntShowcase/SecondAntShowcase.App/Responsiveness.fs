...
let private hasFlag name args = args |> List.contains name
...
let parse args =
    ...
    | Some theme ->
        if allInteractive && Option.isSome page then
            ...
            page
            |> Option.exists (fun pageId ->
                PageRegistry.all |> List.exists (fun pageSpec -> pageSpec.Id = pageId) |> not)
        then
            ...
            let scope =
                if allInteractive then
                    ...
                    Page(page |> Option.defaultValue "buttons")
...
let private durationMs (value: float) = Math.Round(value, 3)
...
let private recordJson
    ...
    let measuredProductChanged =
        measurement
        |> Option.map (fun value -> value.ProductChanged)
        |> Option.defaultValue productChanged
    ...
    let measuredWork =
        measurement
        |> Option.map (fun value -> value.FrameWorkMs)
        |> Option.defaultValue totalMs

    let measuredDirtyStatus =
        if Option.isSome measurement then
            "measured"
        else
            environmentStatus
    ...
    JsonSerializer.Serialize(
        {|
            ...
            observedVisibleResult =
                if Option.isSome measurement || measuredProductChanged then
                    action.ExpectedVisibleResult
                else
                    "not measured in live presentation"
            ...
            dirtyRegion =
                {|
                    ...
                    repaintedNodeCount =
                        measurement |> Option.map (fun value -> value.RepaintedNodeCount) |> nullableInt
...
let private writeEnvironmentLimitedOutputs
    (request: Request)
    (run: string)
    (metrics: FrameMetrics list)
    (extraLimitations: string list)
    (extraDiagnostics: string list)
    =
    ...
    let moveFrame =
        metrics |> List.tryFind (fun metric -> metric.PointerSamplesReceived > 1)

    let longFrame =
        metrics |> List.exists (fun metric -> metric.FrameCause = FrameCause.Tick)
        ...
        |> List.mapi (fun index action ->
            let coalesced =
                if action.InputKind = "pointer-move" then
                    moveFrame
                    |> Option.map (fun metric -> max 0 (metric.PointerSamplesReceived - 1))
    ...
    let requiredFamilies =
        actions |> List.map _.ControlFamily |> List.distinct |> List.sort
                    ...
                    {|
                        ...
                        displayOnlyExclusions =
                            ...
                            |> List.map (fun action ->
                                {|
                                    controlId =
                                        action.ControlIds |> List.tryHead |> Option.defaultValue action.ActionId
...
let private effectivePresentationTiming (fallbackPaint: TimeSpan, fallbackPresent: TimeSpan) (metric: FrameMetrics) =
    let paint =
        if metric.PaintDuration > TimeSpan.Zero then
            metric.PaintDuration
        else
            fallbackPaint

    let present =
        if metric.ComposeDuration > TimeSpan.Zero then
            metric.ComposeDuration
        else
            fallbackPresent
...
let private measurementsOfMetrics fallbackTiming (metrics: FrameMetrics list) =
    ...
    |> List.mapi (fun index metric ->
        let paintDuration, presentDuration =
            effectivePresentationTiming fallbackTiming metric
...
let private markdownMs value =
    ...
    | Some number ->
        durationMs number
        |> fun rounded -> rounded.ToString("0.###", CultureInfo.InvariantCulture)
    | None -> "n/a"
...
let private writeLiveOutputs (request: Request) (run: string) (live: FS.GG.UI.Controls.Elmish.LiveScriptRunResult) =
    ...
    if List.isEmpty measurements then
        ...
        let budgetRejected =
            (p95
             |> Option.exists (fun value -> value > float Evidence.responsivenessTargetP95Ms))
            || (maxValue
                |> Option.exists (fun value -> value > float Evidence.responsivenessTargetMaxMs))

        let overallReadiness =
            if List.isEmpty missingFamilies && not budgetRejected then
                "accepted"
            else
                "rejected"
        ...
        let records =
            ...
            |> List.mapi (fun index (action, measurement) ->
                ...
                recordJson
                    ...
                    (measurement
                     |> Option.map (fun value -> value.TotalMs)
                     |> Option.defaultValue 0.0)
                    (measurement
                     |> Option.map (fun value -> value.CoalescedMovementCount)
        ...
        let groupByFamily =
            ...
            |> List.map (fun ((pageId, inputKind, family), grouped) ->
                ...
                let groupMissing =
                    grouped |> List.exists (fun (_, measurement) -> Option.isNone measurement)
                ...
                {|
                    ...
                    longFrameCount =
                        grouped
                        |> List.filter (fun (_, measurement) ->
                            measurement |> Option.exists (fun value -> value.LongFrame))
                        |> List.length
        ...
        let acceptedFamilies =
            if overallReadiness = "accepted" then
                requiredFamilies
            else
                []
        ...
        let slowestInteractions =
            ...
            |> List.choose (fun (index, action, measurement) ->
                ...
                |> Option.map (fun value ->
                    let dominant =
                        if value.PresentMs >= value.PaintMs then
                            "present"
                        else
                            "paint"
        ...
        let groupMarkdown =
            ...
            |> List.map (fun group ->
                let p50Text =
                    markdownMs (
                        if group.p50Ms.HasValue then
                            Some group.p50Ms.Value
                        else
                            None
                    )
                ...
                let p95Text =
                    markdownMs (
                        if group.p95Ms.HasValue then
                            Some group.p95Ms.Value
                        else
                            None
                    )
                ...
                let maxText =
                    markdownMs (
                        if group.maxMs.HasValue then
                            Some group.maxMs.Value
                        else
                            None
                    )
    ...
    | Ok request ->
        try
            ...
            let writeSubstitute extraLimitations extraDiagnostics =
                ...
                let summaryPath =
                    writeEnvironmentLimitedOutputs request run metrics extraLimitations extraDiagnostics
                ...
                if capability.PersistentWindow then
                    ...
                    | Result.Ok live ->
                        ...
                        | Some(summaryPath, readiness) ->
                            if request.PrintJson then
                                printfn
                                    """{"runId":"%s","summaryJson":"%s","readiness":"%s"}"""
                                    run
                                    summaryPath
                                    readiness
                            else
                        ...
                        | None ->
                            let fallbackPaint, fallbackPresent =
                                FS.GG.UI.SkiaViewer.Host.GlHost.lastPresentTiming ()
                            ...
                            let finalPaintMs =
                                fallbackPaint.TotalMilliseconds.ToString("0.###", CultureInfo.InvariantCulture)

                            let finalPresentMs =
                                fallbackPresent.TotalMilliseconds.ToString("0.###", CultureInfo.InvariantCulture)

                            writeSubstitute
                                [ "presentation:missing-boundary"; "live-runner:no-measured-present-timing" ]
                                [
                    ...
                    | Result.Error failure ->
                        writeSubstitute
                            ...
                            [
                                "LIVE: GL viewer path failed before accepted responsiveness could be measured: "
                                + failure.Message
                            ]
                else
                    writeSubstitute
