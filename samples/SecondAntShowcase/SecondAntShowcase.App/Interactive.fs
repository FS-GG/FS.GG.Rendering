...
module SecondAntShowcase.App.Interactive
...
let run (mode: ThemeMode) (startPage: string): int =
    ...
    if not capability.PersistentWindow then
        printfn "second-ant-showcase: interactive mode skipped — no live window/GL host."
        ...
        | Result.Ok outcome ->
            printfn "second-ant-showcase: interactive session ended (status=%s)." outcome.Status
            0
        | Result.Error failure ->
            eprintfn "second-ant-showcase: interactive launch failed: %s" failure.Message
            1
