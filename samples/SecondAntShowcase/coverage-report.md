# Second Ant Showcase - Coverage Report

Committed control→page map (FR-003 / SC-001). The authoritative gate is the live
`coverage` subcommand (`dotnet run --project SecondAntShowcase.App -- coverage`), which reads
`FS.GG.UI.Controls.Catalog.supportedControls` and reddens on any drift. As of the feed
refreshed for feature 171, that is **96/96 controls mapped, 19 pages (13 catalog + 6
...
bijection (R2/R4). See `SecondAntShowcase.Tests/TemplateTests.fs`.
