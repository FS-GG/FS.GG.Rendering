...
let all: InteractionContract list =
    [
        contract
            "button-click"
            [ "button"; "icon-button"; "split-button"; "float-button" ]
            "click"
            "pointer-discrete"
            "activate command"
            "ButtonClicks increments"
            "status area and command counter change"
            ButtonClicked
        contract
            "toggle-switch"
            ...
            "ToggleOn/SwitchOn changes"
            "checked visual state changes"
            (ToggleChanged false)
        contract
            "text-entry"
            [ "text-box"; "text-area"; "auto-complete" ]
            "value-change"
            "key-down"
            "enter text"
            "text fields update"
            "entered text is rendered"
            (TextChanged "review text")
        contract
            "numeric-entry"
            [ "numeric-input" ]
            "value-change"
            "key-down"
            "set numeric value"
            "NumericValue changes"
            "numeric value display changes"
            (NumericChanged 64.0)
        contract
            "date-time"
            ...
            "DatePickerSelected changes"
            "selected temporal value is visible"
            (DatePickerChanged(System.DateOnly(2026, 6, 19)))
        contract
            "slider-rating"
            [ "slider"; "rate"; "progress-bar" ]
            "drag"
            "pointer-move"
            "adjust value"
            "SliderValue/RateValue/ProgressValue changes"
            "position or stars visibly change"
            (SliderChanged 0.75)
        contract
            "selection-single"
            [
            ...
            "selected label or swatch changes"
            (ComboChanged "Product")
        contract
            "selection-multi"
            [ "check-box"; "multi-select-list"; "tree-view" ]
            "select"
            "pointer-discrete"
            "change selected values"
            "selection state changes"
            "checked/selected rows change"
            (MultiChanged [ "Bold"; "Italic" ])
        contract
            "navigation"
            [ "tabs"; "menu"; "breadcrumb"; "steps"; "pagination"; "anchor"; "affix" ]
            "navigate"
            "pointer-discrete"
            "navigate within page"
            "active navigation state changes"
            "active item or page indicator changes"
            (TabChanged "Activity")
        contract
            "disclosure"
            [
            ...
            "open-close"
            "pointer-discrete"
            "open or close surface"
            "expanded/open state changes"
            "overlay, drawer, or panel visibility changes"
            (OverlayToggled true)
        contract
            "upload"
            [ "upload" ]
            ...
            "UploadValue changes"
            "selected file name changes"
            (UploadChanged "contract.pdf")
        contract
            "data-collection"
            [ "list-view"; "data-grid" ]
            "select"
            "pointer-discrete"
            "select or page collection"
            "PaginationPage changes"
            "visible row/page state changes"
            (PageChanged 2)
        contract
            "form-validation"
            [ "validation-message" ]
            ...
            "pointer-discrete"
            "submit invalid form"
            "Form.Phase becomes Invalid"
            "validation message appears"
            FormSubmitted
        contract
            "graph-custom"
            [ "graph-view"; "canvas"; "custom-control" ]
            "click"
            "pointer-discrete"
            "script pointer/keyboard action"
            "interaction event is recorded"
            "custom/graph status changes"
            ButtonClicked
    ]
...
let forControl controlId =
    all
    |> List.filter (fun contract -> contract.ControlIds |> List.contains controlId)

let coverage () =
    let contracted = all |> List.collect _.ControlIds |> List.distinct |> List.sort

    let displayOnly =
        displayOnlyReasons |> Map.toList |> List.map fst |> List.distinct |> List.sort
...
let isClean coverage =
    List.isEmpty coverage.MissingContractOrReason
