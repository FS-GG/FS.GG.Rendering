...
module SecondAntShowcase.Core.Pages
...
open SecondAntShowcase.Core.DemoState

/// Title + body grouping. Theme-independent, so the control-tree shape is identical
/// across antLight/antDark (FR-008/SC-003).
let private section (title: string) (body: Control<SecondAntShowcaseMsg>) : Control<SecondAntShowcaseMsg> =
    Border.create
        [
            Attr.width 520.0
            Attr.height 150.0
...
let private group (bodies: Control<SecondAntShowcaseMsg> list) : Control<SecondAntShowcaseMsg> =
    Wrap.create [ Wrap.children bodies ]

let private largeSection (title: string) (body: Control<SecondAntShowcaseMsg>) : Control<SecondAntShowcaseMsg> =
    Border.create
        [
            Attr.width 520.0
            Attr.height 220.0
...
// ---------------------------------------------------------------------------------
let private displayPage (s: DemoState) : Control<SecondAntShowcaseMsg> =
    let richBlock =
        RichText.block
            [
                RichText.run "Rich text with measured runs." (RichText.defaultStyle AntTheme.defaultTheme)
            ]
...
// ---------------------------------------------------------------------------------
let private cardsPage (_s: DemoState) : Control<SecondAntShowcaseMsg> =
    group
        [
            ...
            section
                ...
                (Collapse.create
                    [
                        Attr.items collapsePanels
                        Collapse.onChange (fun k -> PageMsg(CollapseToggled k))
                    ])
...
// ---------------------------------------------------------------------------------
let private buttonsPage (s: DemoState) : Control<SecondAntShowcaseMsg> =
    let toggleProps =
    ...
    let splitItems: FS.GG.UI.Controls.Typed.SplitButtonItem list =
        [ { Key = "save-as"; Label = "Save As" }; { Key = "export"; Label = "Export" } ]
    ...
    group
        [
            section
                ...
                (Button.create
                    [
                        Button.text (sprintf "Clicked %d" s.ButtonClicks)
                        Button.onClick (PageMsg ButtonClicked)
                    ])
            section
                "icon-button"
                (IconButton.create [ IconButton.icon "play"; IconButton.onClick (PageMsg ButtonClicked) ])
...
// ---------------------------------------------------------------------------------
let private inputPage (s: DemoState) : Control<SecondAntShowcaseMsg> =
    let dateProps =
    ...
    group
        [
            section
                ...
                (TextBox.create
                    [
                        TextBox.value s.TextValue
                        TextBox.onChanged (fun v -> PageMsg(TextChanged v))
                    ])
            section
                "text-area"
                (TextArea.create
                    [
                        TextArea.value s.AreaValue
                        TextArea.onChanged (fun v -> PageMsg(AreaChanged v))
                    ])
            section
                "numeric-input"
                (NumericInput.create
                    [
                        NumericInput.value s.NumericValue
                        NumericInput.onChanged (fun v -> PageMsg(NumericChanged v))
                    ])
            section
                "slider"
                (Slider.create
                    [
                        ...
                        Slider.onChanged (fun v -> PageMsg(SliderChanged v))
                    ])
            ...
            section
                ...
                (Rate.create
                    [
                        Rate.value s.RateValue
                        Rate.onChange (fun v -> PageMsg(RateChanged(float v)))
                    ])
            section
                "auto-complete"
                (AutoComplete.create
                    [
                        AutoComplete.value s.AutoCompleteValue
                        AutoComplete.onChange (fun v -> PageMsg(AutoCompleteChanged v))
                    ])
            section
                "upload"
                (Upload.create [ Upload.text "Upload"; Upload.onChange (fun v -> PageMsg(UploadChanged v)) ])
        ]

// ---------------------------------------------------------------------------------
// Page 5 — Selection & Toggles
// ---------------------------------------------------------------------------------
let private selectionPage (s: DemoState) : Control<SecondAntShowcaseMsg> =
    let listProps =
    ...
    group
        [
            section
                ...
                (CheckBox.create
                    [
                        ...
                        CheckBox.onChanged (fun v -> PageMsg(CheckChanged v))
                    ])
            section
                "radio-group"
                (RadioGroup.create
                    [
                        ...
                        RadioGroup.selected s.RadioSelected
                        RadioGroup.onChanged (fun v -> PageMsg(RadioChanged v))
                    ])
            section
                "switch"
                (Switch.create
                    [
                        Switch.checked' s.SwitchOn
                        Switch.onChanged (fun v -> PageMsg(SwitchChanged v))
                    ])
            ...
            section
                ...
                (Cascader.create
                    [
                        Attr.items cascaderItems
                        Cascader.onChange (fun v -> PageMsg(CascaderChanged v))
                    ])
        ]
...
// ---------------------------------------------------------------------------------
let private layoutPage (_s: DemoState) : Control<SecondAntShowcaseMsg> =
    let sample t = TextBlock.create [ TextBlock.text t ]
    let scrollChild = group [ sample "Row 1"; sample "Row 2"; sample "Row 3" ]

    let scrollProps =
        FS.GG.UI.Controls.Typed.ScrollViewer.defaults "demo-scroll" (Widget.ofControl scrollChild)
...
// ---------------------------------------------------------------------------------
let private navPage (s: DemoState) : Control<SecondAntShowcaseMsg> =
    let ctxProps =
            ...
            section
                ...
                (Tabs.create
                    [
                        Tabs.items tabItems
                        Tabs.selected s.Tab
                        Tabs.onChanged (fun v -> PageMsg(TabChanged v))
                    ])
            ...
            section
                ...
                (Toolbar.create
                    [
                        Toolbar.children
                            [
                                TextBlock.create [ TextBlock.text "Undo" ]
                                TextBlock.create [ TextBlock.text "Redo" ]
                            ]
                    ])
            section
                "float-button"
                (FloatButton.create [ FloatButton.text "+"; FloatButton.onClick (PageMsg ButtonClicked) ])
            ...
            section
                ...
                (Pagination.create
                    [
                        ...
                        Pagination.onChange (fun p -> PageMsg(PageChanged(int p)))
                    ])
            section
                "segmented"
                (Segmented.create
                    [
                        Attr.items segmentedOptions
                        Segmented.onChange (fun v -> PageMsg(SegmentedChanged v))
                    ])
...
// Page 8 — Overlays
// ---------------------------------------------------------------------------------
let private overlaysPage (_s: DemoState) : Control<SecondAntShowcaseMsg> =
    group
        [
...
// ---------------------------------------------------------------------------------
let private feedbackPage (s: DemoState) : Control<SecondAntShowcaseMsg> =
    group
        [
            ...
            largeSection
                "drawer"
                (Drawer.create [ Drawer.title "Filters"; Drawer.onClose (PageMsg(DrawerToggled false)) ])
            section "popover" (Popover.create [ Popover.text "More information" ])
            section
                "popconfirm"
                (Popconfirm.create
                    [
                        Popconfirm.text "Delete this item?"
                        Popconfirm.onConfirm (PageMsg ButtonClicked)
                        Popconfirm.onCancel (PageMsg ButtonClicked)
                    ])
...
// ---------------------------------------------------------------------------------
let private dataPage (_s: DemoState) : Control<SecondAntShowcaseMsg> =
    let lvProps =
...
// ---------------------------------------------------------------------------------
let private chartsStatPage (_s: DemoState) : Control<SecondAntShowcaseMsg> =
    group
        [
...
// ---------------------------------------------------------------------------------
let private chartsAdvPage (_s: DemoState) : Control<SecondAntShowcaseMsg> =
    group
        [
...
// ---------------------------------------------------------------------------------
let private graphsPage (_s: DemoState) : Control<SecondAntShowcaseMsg> =
...
let familyPages: Page list =
    [
        ...
        {
            Id = "cards-stats-media"
            Title = "Cards, Stats & Media"
            Kind = Catalog
            ControlIds =
                [
                    "image"
        ...
        {
            Id = "charts-statistical"
            Title = "Charts I — Statistical"
            Kind = Catalog
            ControlIds =
                [
                    "line-chart"
