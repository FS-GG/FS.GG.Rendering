...
let responsivenessDragContinuity actionId sampleCount visibleFeedbackSamples maxSampleGapMs delayedCatchUp =
    let classification =
        if sampleCount < 2 then "insufficient-samples"
        elif delayedCatchUp then "delayed-catch-up"
        elif visibleFeedbackSamples >= sampleCount then "continuous"
        else "failed"
...
let datePickerReferenceOverlayEvidence () : DatePickerOverlayEvidence =
    {
        ...
        BehavioralEvidenceReference = "Feature144 SecondAntShowcase date-picker reference flow"
        ReplayLog =
            [
        ...
        FocusTransitions =
            [
                None, Some "date-picker-calendar"
                Some "date-picker-calendar", Some "date-picker-trigger"
            ]
        ProductMessages =
            [
                "DatePickerOpenChanged:true"
                "DatePickerChanged:2026-06-17"
                "DatePickerOpenChanged:false"
            ]
        Diagnostics = []
        NoStaleOverlay = true
    }
...
let private toSharedReport (summary: VisualReadinessSummary) =
    ...
    let contactSheets =
        ...
        |> List.mapi (fun index path ->
            {
                SheetId = sprintf "second-ant-showcase-contact-sheet-%d" (index + 1)
                RelativePath = path
...
let toSummaryMd (r: PageEvidenceRecord) : string =
    ...
    let sb = System.Text.StringBuilder()
    let line (t: string) = sb.AppendLine(t) |> ignore
    line (sprintf "# Second Ant Showcase evidence - page `%s` (seed %d, %s)" r.PageId r.Seed r.Mode)
    line ""
...
let retainedInspectionToMarkdown (evidence: RetainedInspectionEvidenceRecord) : string =
    String.concat
        ...
        [
            "# SecondAntShowcase retained inspection"
            ""
            ...
            sprintf
                "- screenshot parity: preferred=`%d`, minimum=`%d`"
                evidence.ScreenshotPreferredTargetCount
                evidence.ScreenshotMinimumTargetCount
