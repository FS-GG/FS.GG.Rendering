...
let private pageShell
    (title: string)
    (primaryAction: string)
    (children: Control<SecondAntShowcaseMsg> list)
    : Control<SecondAntShowcaseMsg> =
    Stack.create
        [
            Stack.children
                [
...
let private rowsGrid (idPrefix: string) (label: string) (rows: string list) : Control<SecondAntShowcaseMsg> =
...
// ---------------------------------------------------------------------------------
let private workbench (_s: DemoState) : Control<SecondAntShowcaseMsg> =
    let toolbar =
        Toolbar.create
            [
                Toolbar.children
                    [
                        Button.create [ Button.text "Refresh"; Button.onClick (PageMsg ButtonClicked) ]
                        Button.create [ Button.text "New" ]
                    ]
            ]
...
// ---------------------------------------------------------------------------------
let private listPage (s: DemoState) : Control<SecondAntShowcaseMsg> =
    let toolbar =
        Toolbar.create
            [
                Toolbar.children
                    [
                        TextBox.create [ TextBox.value "Filter…" ]
                        Button.create [ Button.text "Search"; Button.onClick (PageMsg ButtonClicked) ]
                    ]
            ]
    ...
    let tags =
        Stack.create
            [
                Stack.orientation "horizontal"
                Stack.children (tagSamples |> List.map (fun t -> Tag.create [ Tag.text t ]))
            ]

    let pager =
        Navigation2.Pagination.create
            [
                Navigation2.Pagination.total paginationTotal
                Navigation2.Pagination.onChange (fun p -> PageMsg(PageChanged(int p)))
            ]
...
// ---------------------------------------------------------------------------------
let private detail (s: DemoState) : Control<SecondAntShowcaseMsg> =
    let tabs =
        Tabs.create
            [
                Tabs.items tabItems
    ...
    let related =
        Panel.create
            [
                Panel.children
                    [
                        Card.create [ Card.title "Related" ]
                        Timeline.create [ Attr.items timelineItems ]
                    ]
            ]
...
// ---------------------------------------------------------------------------------
let private form (s: DemoState) : Control<SecondAntShowcaseMsg> =
    let f = s.Form
    ...
    let fields =
        [
            section
                ...
                (TextBox.create
                    [
                        ...
                        TextBox.onChanged (fun v -> PageMsg(FormFieldChanged("Name", v)))
                    ])
            section
                "Email"
                (TextBox.create
                    [
                        TextBox.value f.Email
                        TextBox.onChanged (fun v -> PageMsg(FormFieldChanged("Email", v)))
                    ])
            ...
            section
                ...
                (Switch.create
                    [
                        Switch.checked' f.Agree
                        Switch.onChanged (fun v -> PageMsg(FormFieldChanged("Agree", (if v then "true" else "false"))))
                    ])
        ]
    ...
    let errorNodes =
        ...
        | Invalid errors ->
            errors
            |> List.map (fun (_, m) -> ValidationMessage.create [ ValidationMessage.text m ])
        | _ -> []
    ...
    let submit =
        Button.create [ Button.text "Submit"; Button.onClick (PageMsg FormSubmitted) ]
...
// ---------------------------------------------------------------------------------
let private result (_s: DemoState) : Control<SecondAntShowcaseMsg> =
    pageShell
        "Order Result"
        "View order"
        [
            ...
            section
                ...
                (Stack.create
                    [
                        ...
                        Stack.children
                            [
                                Button.create [ Button.text "View order"; Button.onClick (PageMsg ButtonClicked) ]
                                Button.create [ Button.text "Continue" ]
                            ]
                    ])
        ]
...
// ---------------------------------------------------------------------------------
let private exception' (_s: DemoState) : Control<SecondAntShowcaseMsg> =
    pageShell
        "Exception Recovery"
        "Back home"
        [
