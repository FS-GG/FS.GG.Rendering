...
let markFixed fixReference finding =
    { finding with
        Status = Fixed
        FixReference = Some fixReference
    }
...
let validate requiredTargetIds findings =
    ...
    let classified = findings |> List.collect _.TargetIds |> Set.ofList

    let malformed =
        findings
        |> List.choose (fun finding ->
            if
                System.String.IsNullOrWhiteSpace finding.FindingId
                || List.isEmpty finding.TargetIds
            then
...
let private escapeCell (text: string) =
