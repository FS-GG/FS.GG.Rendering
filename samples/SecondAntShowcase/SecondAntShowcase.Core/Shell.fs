...
/// scrolling content region, and a bottom status strip. `view : Size -> SecondAntShowcaseModel
/// -> Control<SecondAntShowcaseMsg>` is the MVU View; its tree shape is theme-independent
...
let private region
    (bounds: ShellLayout.Rect)
    (label: string)
    (child: Control<SecondAntShowcaseMsg>)
    ...
    Border.create
        [
            ...
            Attr.padding 8.0
            Border.child (Stack.create [ Stack.children [ TextBlock.create [ TextBlock.text label ]; child ] ])
        ]

let private appBar (bounds: ShellLayout.Rect) (model: SecondAntShowcaseModel) : Control<SecondAntShowcaseMsg> =
    let modeLabel =
    ...
    region
        ...
        (Stack.create
            [
                ...
                Stack.children
                    [
                        TextBlock.create [ TextBlock.text "SecondAntShowcase" ]
...
let private navRail (bounds: ShellLayout.Rect) (model: SecondAntShowcaseModel) : Control<SecondAntShowcaseMsg> =
    ...
    let templateItems =
        PageRegistry.templatePages |> List.map (navItem model.CurrentPage)
...
let private content (bounds: ShellLayout.Rect) (model: SecondAntShowcaseModel) : Control<SecondAntShowcaseMsg> =
                ...
                Panel.children
                    [
                        ...
                        TextBlock.create
                            [
                                TextBlock.text (
                                    sprintf "profile=%A · columns=%d" profile.Density profile.SectionColumns
                                )
                            ]
                        body
                    ]
    ...
    let scrollViewer =
        Control.standard
            ...
            [ Attr.child framedBody; Attr.height (bounds.Height - 88.0) ]
...
/// `update`); the App edge persists it so it can be acted upon later (`SecondAntShowcase feedback`).
let private feedbackSection (bounds: ShellLayout.Rect) (model: SecondAntShowcaseModel) : Control<SecondAntShowcaseMsg> =
    ...
    let saved =
        if List.isEmpty forPage then
            ...
            forPage
            |> List.map (fun e -> TextBlock.create [ TextBlock.text ("• " + e.Text) ])

    let summary =
        sprintf "%d page · %d total" (List.length forPage) (List.length model.Feedback)
...
let private statusStrip (bounds: ShellLayout.Rect) (model: SecondAntShowcaseModel) : Control<SecondAntShowcaseMsg> =
    let page = PageRegistry.byId model.CurrentPage
...
let view (size: Size) (model: SecondAntShowcaseModel) : Control<SecondAntShowcaseMsg> =
