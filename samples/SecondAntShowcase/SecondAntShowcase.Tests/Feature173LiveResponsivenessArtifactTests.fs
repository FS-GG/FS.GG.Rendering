...
let tests =
    testList
        "Feature173 live responsiveness artifacts"
        [
            test "summary and records use contracted paths and status fields" {
                ...
                Expect.equal
                    (record.GetProperty("visibleResponse").GetString())
                    "environment-limited"
                    "record visible response"

                Expect.equal
                    (record.GetProperty("environmentStatus").GetString())
                    "headless-substitute"
                    "record environment"

                Expect.equal
                    (record.GetProperty("acceptanceStatus").GetString())
                    "environment-limited"
