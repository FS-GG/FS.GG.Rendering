...
let degradeTests =
    testList "Degrade" [
        ...
        test "every record carries a non-empty notAuthoritativeFor (FR-012)" {
            ...
            let degradedRecord = Evidence.build "p" 1 "antLight" [ "button" ] [] (Evidence.degraded "x")
        ...
        test "degraded run.json marks provesScreenshot false with a stated reason + Ant mode" {
            let json =
                Evidence.toRunJson (Evidence.build "display-typography" 7 "antDark" [] [] (Evidence.degraded "headless: offscreen GL unavailable"))
