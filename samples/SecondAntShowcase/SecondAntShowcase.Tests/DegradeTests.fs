module SecondAntShowcase.Tests.DegradeTests

open Expecto
open SecondAntShowcase.Core

/// FR-013 / SC-005: the disclosed-degrade contract, asserted on the pure record functions
/// so the CI signal never depends on a display. (The App's evidence path routes any
/// GL/capture failure through exactly these functions and exits 0.)
[<Tests>]
let degradeTests =
    testList
        "Degrade"
        [
            ...
            test "every record carries a non-empty notAuthoritativeFor (FR-012)" {
                ...
                let provenRecord = Evidence.build "p" 1 "antLight" [ "button" ] [] proven

                let degradedRecord =
                    Evidence.build "p" 1 "antLight" [ "button" ] [] (Evidence.degraded "x")
            ...
            test "a proven record claims the non-blank-offscreen-png; a degraded one does not" {
                ...
                let provenRecord = Evidence.build "p" 1 "antLight" [] [] proven
                let degradedRecord = Evidence.build "p" 1 "antLight" [] [] (Evidence.degraded "x")
                Expect.contains provenRecord.AuthoritativeFor "non-blank-offscreen-png" "proven claims the png"

                Expect.isFalse
                    (List.contains "non-blank-offscreen-png" degradedRecord.AuthoritativeFor)
                    "degraded never claims the png"
            }

            test "degraded run.json marks provesScreenshot false with a stated reason + Ant mode" {
                let json =
                    Evidence.toRunJson (
                        Evidence.build
                            "display-typography"
                            ...
                            []
                            []
                            (Evidence.degraded "headless: offscreen GL unavailable")
                    )
                ...
                Expect.stringContains json "\"mode\": \"antDark\"" "ant mode disclosed in json"
