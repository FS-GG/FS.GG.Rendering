...
let tests =
    testList
        "Feature173 live responsiveness workflow"
        [
            ...
            test "live unavailable fails closed and requests artifact persistence" {
                ...
                let model', effects =
                    ResponsivenessWorkflow.update
                        (ResponsivenessWorkflow.LiveSessionUnavailable "no-visible-surface")
                        model
            ...
            test "artifact write failure fails the workflow" {
                ...
                let model', _ =
                    ResponsivenessWorkflow.update
                        (ResponsivenessWorkflow.ArtifactWriteFailed "summary.json denied")
                        model
            ...
            test "interpreter maps edge failures into messages" {
                ...
                let msg =
                    ResponsivenessWorkflow.interpret interpreter model ResponsivenessWorkflow.CheckLiveSession

                Expect.equal
                    msg
                    (ResponsivenessWorkflow.LiveSessionUnavailable "missing-boundary")
                    "edge failure becomes a message"
            }
        ]
