...
let tests =
    testList
        "Feature174 environment-limited evidence"
        [
            test "render lag probe names the live-boundary limitation without claiming parity" {
                ...
                let limitations =
                    Feature174RenderLagFixtures.arrayStrings (root.GetProperty("environmentLimitations"))
            ...
            test "responsiveness fail-closed summary keeps feature 174 fields non-accepted" {
                ...
                Expect.isTrue
                    (Feature173LiveResponsivenessFixtures.hasLimitationContaining "no-live-presentation-boundary" root)
                    "missing live boundary named"
            }
        ]
