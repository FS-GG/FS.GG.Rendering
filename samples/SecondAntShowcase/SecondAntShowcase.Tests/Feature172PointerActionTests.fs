...
let tests =
    testList
        "Feature172 representative pointer actions"
        [
            test "every interaction contract declares action type and input kind" {
                for contract in InteractionContracts.all do
                    ...
                    Expect.isNonEmpty
                        ...
                        (sprintf "%s has expected visible result" contract.ContractId)
            }
            ...
            test "all-interactive script is derived from interaction contracts" {
                ...
                Expect.isGreaterThanOrEqual
                    ...
                    "each family contributes pointer or activation coverage"
            ...
            test "value-changing families are represented by drag or value-change metadata" {
                let valueFamilies =
                    ...
                    |> List.filter (fun contract ->
                        contract.ControlIds
                        |> List.exists (fun id -> id = "slider" || id = "rate" || id = "progress-bar"))
                ...
                Expect.isTrue
                    (valueFamilies
                     |> List.forall (fun contract ->
                         contract.ActionType = "drag" || contract.ActionType = "value-change"))
                    "value families are not plain display actions"
            }
        ]
