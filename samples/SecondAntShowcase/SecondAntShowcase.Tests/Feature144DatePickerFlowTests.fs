module SecondAntShowcase.Tests.Feature144DatePickerFlowTests
...
let tests =
    testList
        "Feature144 SecondAntShowcase date-picker flow"
        [
            test "reference script opens selects dismisses and recovers focus through product-owned state" {
                ...
                Expect.equal
                    finalModel.PageState.DatePickerSelected
                    (Some(System.DateOnly(2026, 6, 17)))
                    "selected date is product-owned"

                Expect.isFalse finalModel.PageState.DatePickerOpen "selection leaves calendar closed"

                Expect.equal
                    finalModel.PageState.DatePickerFocused
                    (Some "date-picker-trigger")
                    "focus recovers to trigger"
            }
