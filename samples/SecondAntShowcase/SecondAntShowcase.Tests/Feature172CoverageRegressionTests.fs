...
let tests =
    testList
        "Feature172 coverage regression preservation"
        [
            ...
            test "interactive-family coverage and display-only exclusions remain explicit" {
                ...
                Expect.isEmpty
                    coverage.MissingContractOrReason
                    "every catalog control has a contract or display-only reason"

                Expect.isTrue (InteractionContracts.isClean coverage) "interaction coverage stays clean"

                Expect.equal
                    (InteractionContracts.all
                     |> List.map _.ContractId
                     |> List.distinct
                     |> List.length)
            ...
            test "display-only exclusions do not overlap interactive controls" {
                let interactive =
                    InteractionContracts.all |> List.collect _.ControlIds |> Set.ofList
                ...
                Expect.isEmpty
                    (Set.intersect interactive displayOnly)
                    "a control is not both interactive and display-only"
            }
        ]
