module SecondAntShowcase.Tests.VisualEvidenceTests
        ...
        [
            test "workflow expands page and theme matrix" {
                let model, effects =
                    init
                        1
                        VisualConfig.preferredSize
                        VisualConfig.supportedThemeIds
                        (PageRegistry.all |> List.map _.Id)
                        "out"
                ...
                Expect.isTrue
                    (model.Targets
                     |> List.forall (fun target -> target.SharedTarget.TargetId.Contains(target.PageId)))
                    "shared target ids are exposed"
            }

            test "degraded capture keeps readiness environment-limited" {
                let model, _ =
                    init 1 VisualConfig.preferredSize [ "antLight" ] [ "display-typography" ] "out"

                let model', _ =
                    update (ScreenshotCaptureDegraded("display-typography", "antLight", "no GL")) model
            ...
            test "complete screenshots without reviewer classification are blocked" {
                ...
                let captured, _ =
                    update (ScreenshotCaptureSucceeded("display-typography", "antLight")) model
            ...
            test "visual summary serializers carry canonical status fields" {
                ...
                Expect.stringContains
                    (Evidence.visualSummaryToMarkdown summary)
                    "visual readiness"
                    "markdown summary produced"
            }

            test "reviewer rubric covers every page/theme pair" {
                let rubric =
                    Evidence.reviewerDefectTemplate [ "p1"; "p2" ] [ "antLight"; "antDark" ]

                Expect.stringContains rubric "| p1 | antLight |" "p1 light row"
                Expect.stringContains rubric "| p2 | antDark |" "p2 dark row"
                Expect.stringContains rubric "targetId" "shared reviewer table includes target ids"
                Expect.stringContains rubric "pending review" "shared reviewer rows begin pending"
            }

            test "visual summary delegates generated readiness fields to shared report" {
                let screenshot: Evidence.VisualScreenshotRecord =
                    {
                        PageId = "display-typography"
                        ThemeId = "antLight"
                ...
                let summary: Evidence.VisualReadinessSummary =
                    {
                        ...
                        CaptureAvailability = "available"
                        ReviewerDefectStatus = "clear"
                        VisualReadinessStatus = VisualReadiness.statusText VisualReadinessAccepted
                        Screenshots = [ screenshot ]
                        ContactSheets = [ "contact-sheet-light.png" ]
                        Limitations = []
                    }
                ...
                let json = Evidence.visualSummaryToJson summary
                Expect.stringContains json "\"sharedVisualReadiness\"" "shared report embedded"
                Expect.stringContains json "\"readinessStatus\": \"accepted\"" "shared readiness status embedded"
            }
        ]
