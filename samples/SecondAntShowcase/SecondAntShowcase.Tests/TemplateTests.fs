...
let private isCatalogComposed (k: string) : bool =
    catalogSet.Contains k
    || catalogIds |> List.exists (fun c -> k.StartsWith(c + "-"))
    ...
    testList
        ...
        [
            for page in PageRegistry.templatePages do
                yield
                    test (sprintf "template %s is composed only of catalog control types (SC-002)" page.Id) {
                        let tree = page.View DemoState.seed

                        let nonCatalog =
                            kinds tree |> List.filter (fun k -> not (isCatalogComposed k)) |> List.distinct

                        Expect.isEmpty nonCatalog (sprintf "every node maps to a catalog id; offenders: %A" nonCatalog)
                    }

            yield
                test "form: invalid submit ⇒ Invalid with validation messages and NO success result" {
                    let invalidModel =
                        applyAll (List.take 4 Scripts.formInvalidThenValid) Host.initModel
            ...
            yield
                test "form: valid submit ⇒ Submitted with a success result node" {
            ...
            yield
                test "form: field edits return to Editing (no premature validation)" {
            ...
            yield
                test "list template: pagination interaction changes visible page state" {
            ...
            yield
                test "exception template: recovery action records a visible command state change" {
