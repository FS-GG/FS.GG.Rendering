    ...
    testList
        ...
        [
            test "accepted showcase sizes are declared" {
                ...
                Expect.equal
                    (VisualConfig.classifySize VisualConfig.preferredSize)
                    VisualConfig.Preferred
                    "preferred role"
            ...
            test "CLI aliases resolve to canonical theme ids" {
                ...
                | Ok themes -> Expect.equal (themes |> List.map snd) [ "antLight"; "antDark" ] "aliases resolve"
                ...
                for size in [ preferredSize; minimumSize ] do
                    for mode in [ Light; Dark ] do
                        ...
                        Expect.isGreaterThan
                            rendered.NodeCount
                            0
                            (sprintf "renders %A at %s" mode (VisualConfig.sizeText size))
            }
            ...
            test "shared visual readiness target parity covers preferred and minimum matrices" {
                ...
                let minimum, _ =
                    init
                        1
                        VisualConfig.minimumSize
                        VisualConfig.supportedThemeIds
                        VisualConfig.minimumRepresentativePageIds
                        "out"
                ...
                Expect.equal
                    (preferred.Targets
                     |> List.map _.SharedTarget.TargetId
                     |> List.distinct
                     |> List.length)
                    38
                    "preferred shared target ids unique"
                ...
                Expect.equal
                    (minimum.Targets
                     ...
                     |> List.length)
                    38
                    "minimum shared target ids unique"

                Expect.isTrue
                    (preferred.Targets
                     |> List.forall (fun target -> target.SharedTarget.RelativePath = target.RelativePath))
                    "workflow exposes shared-compatible relative paths"

                Expect.isTrue
                    (minimum.Targets |> List.forall (fun target -> target.SharedTarget.Required))
                    "minimum shared targets are required"
            }
        ]
