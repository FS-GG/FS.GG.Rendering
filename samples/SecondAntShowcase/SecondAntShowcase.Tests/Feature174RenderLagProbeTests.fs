...
let tests =
    testList
        "Feature174 render lag probe artifacts"
        [
            test "forced substitute writes contracted button-click artifacts" {
                ...
                Expect.equal
                    (root.GetProperty("preparationReductionPercent").ValueKind)
                    JsonValueKind.Null
                    "no synthetic reduction"

                Expect.equal
                    (root.GetProperty("firstFramePreparationReductionPercent").ValueKind)
                    JsonValueKind.Null
                    "no synthetic first-frame reduction"
                ...
                Expect.equal
                    (record.GetProperty("metadataVisitedNodeCount").GetInt32())
                    0
                    "no synthetic metadata visits"
            }
            ...
            test "page-change artifacts include phase, trace, and markdown files" {
                ...
                let markdown =
                    File.ReadAllText(Feature174RenderLagFixtures.summaryMarkdownFile outDir)
                ...
                Expect.stringContains
                    ...
                    "trace names missing live boundary"
                ...
                Expect.stringStarts
                    (Feature174RenderLagFixtures.runRoot outDir)
                    (Path.GetFullPath outDir)
                    "run root stays under out dir"
            }
        ]
