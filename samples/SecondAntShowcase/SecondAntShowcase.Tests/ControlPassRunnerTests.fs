...
module ControlPassFixtures =
    let full = lazy (ControlPassRunner.evaluate (ControlPassRunner.testConfig None))
        ...
        [
            ...
            test "every emitted record is terminal — no Unexercised/Unclassified (C-2/C-3, VR-2)" {
                for r in records do
                    ...
                    | DisplayOnly ->
                        Expect.isFalse
                            (System.String.IsNullOrWhiteSpace r.ClassificationReason)
                            (sprintf "display-only '%s' carries a reason" r.ControlId)

                        Expect.equal
                            r.FunctionalVerdict
                            NotApplicable
                            (sprintf "display-only '%s' is NotApplicable (VR-5)" r.ControlId)
                    | Interactive ->
                        let allowed =
                            [
                                FunctionalPass
                        ...
                        Expect.contains
                            allowed
                            r.FunctionalVerdict
                            (sprintf "interactive '%s' has a functional verdict in the allowed set" r.ControlId)

                        Expect.notEqual
                            r.FunctionalVerdict
                            NotApplicable
                            (sprintf "interactive '%s' is not NotApplicable (VR-5)" r.ControlId)
            }
            ...
            test "two same-seed runs yield byte-stable deterministic evidence (live-pixel surface excluded)" {
                ...
                Expect.equal
                    (recordsA |> List.map deterministicView)
                    (recordsB |> List.map deterministicView)
                    "same-seed deterministic evidence is identical across runs"
            }
            ...
            test "functional verdicts are deterministic across runs (G-4)" {
                ...
                let verdicts rs =
                    rs |> List.map (fun r -> r.ControlId, r.FunctionalVerdict)
            ...
            test "live-pixel cells never silently pass when capture is degraded/blocked (VR-8)" {
                for r in records do
                    for cell in r.VisualEvidence do
                        ...
                        | BlockedCapture ->
                            Expect.notEqual
                                cell.FidelityVerdict
                                Approved
                                (sprintf "%s: degraded/blocked capture is never silently Approved" cell.TargetId)
                        | _ -> ()
            }
                ...
                if not env.Renderable then
                    for r in records do
                        Expect.notEqual
                            r.VisualVerdict
                            VisualApproved
                            (sprintf "%s: headless visual verdict is not a silent Approved" r.ControlId)
                else
            ...
            test "functional dimension stays authoritative headless — interactive controls are exercised" {
                ...
                for r in interactive do
                    Expect.isNonEmpty
                        r.BehaviorsExercised
                        (sprintf "interactive '%s' has exercised behaviors even headless" r.ControlId)
            }

            // --- US2: matrix completeness (T017, M-1/M-2, SC-003) ----------------------------
            test "every control has rest evidence for both appearances x both sizes (FR-003)" {
                let expected =
                    Set.ofList [ AntLight, Preferred; AntLight, Minimum; AntDark, Preferred; AntDark, Minimum ]
            ...
            test "every interactive control has at least one captured interaction state (FR-004)" {
                for r in records do
                    if r.Classification = Interactive then
                        Expect.isNonEmpty
                            r.InteractionStates
                            (sprintf "interactive '%s' has interaction-state evidence" r.ControlId)
            }
                    ...
                    for d in r.DamageEvidence do
                        ...
                        | FullSurface ->
                            Expect.notEqual
                                d.Verdict
                                Pass
                                (sprintf "%s: broad/full-surface damage carries a non-Pass verdict" d.TransitionId)
                        | _ -> ()
            }
            ...
            test "each interactive control records one damage outcome per driven behavior (G-5)" {
                for r in records do
                    if r.Classification = Interactive then
                        Expect.equal
                            (List.length r.DamageEvidence)
                            (List.length r.BehaviorsExercised)
                            (sprintf "control '%s' records damage per behavior" r.ControlId)
            }
            ...
            test "transient overlays appear and dismiss through the pure model (M-5, FR-015)" {
                ...
                let roundTrips =
                    [
                        "overlay",
                        PageMsg(OverlayToggled true),
                        PageMsg(OverlayToggled false),
                        (fun (m: SecondAntShowcaseModel) -> m.PageState.OverlayOpen)
                        "dialog",
                        PageMsg(DialogToggled true),
                        PageMsg(DialogToggled false),
                        (fun m -> m.PageState.DialogOpen)
                        "drawer",
                        PageMsg(DrawerToggled true),
                        PageMsg(DrawerToggled false),
                        (fun m -> m.PageState.DrawerOpen)
                    ]
