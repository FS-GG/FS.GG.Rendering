...
let tests =
    testList
        "Feature172 responsiveness CLI contract"
        [
            ...
            test "all-interactive require-live run fails closed with exit code 4 in headless substitute mode" {
                ...
                let code =
                    SecondAntShowcase.Tests.Feature172ResponsivenessFixtures.withForcedSubstitute (fun () ->
                        Responsiveness.run
                            [
                                "--script"
                                "representative"
                ...
                Expect.equal
                    (root.GetProperty("overallReadiness").GetString())
                    "environment-limited"
                    "summary does not claim accepted readiness"

                Expect.stringContains
                    (root.GetProperty("scope").GetString())
                    "all-interactive"
                    "scope names all-interactive run"
            }
        ]
