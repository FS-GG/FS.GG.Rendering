...
let tests =
    testList
        "Feature174 responsiveness summary fields"
        [
            test "environment-limited summary exposes baseline, optimized profile, and null reductions" {
                ...
                Expect.equal
                    ...
                    JsonValueKind.Null
                    "preparation reduction is not fabricated"

                Expect.equal
                    (root.GetProperty("firstFramePreparationReductionPercent").ValueKind)
                    JsonValueKind.Null
                    "first-frame reduction is not fabricated"

                Expect.equal
                    (root.GetProperty("parityStatus").GetString())
                    "environment-limited"
                    "parity status is fail-closed"
            ...
            test "responsiveness markdown links render-lag correlation placeholders" {
                ...
                Expect.stringContains
                    ...
                    "environment-limited first-frame reduction is n/a"
