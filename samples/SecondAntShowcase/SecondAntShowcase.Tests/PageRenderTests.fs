module SecondAntShowcase.Tests.PageRenderTests

open Expecto
open FS.GG.UI.Controls
open SecondAntShowcase.Core

let private theme = AntTheme.resolve SecondAntShowcase.Core.Model.Light
    ...
    testList
        ...
        [
            for page in PageRegistry.catalogPages do
                yield
                    test (sprintf "catalog page %s renders a non-empty tree" page.Id) {
            ...
            yield
                test "every page is reachable in one direct nav selection (≤2 actions, SC-008)" {
