...
let tests =
    testList
        "Feature173 live responsiveness regressions"
        [
            ...
            test "summary preserves visual-readiness caveat vocabulary" {
                ...
                Expect.equal
                    (root.GetProperty("artifactWriteStatus").GetString())
                    "complete"
                    "artifact status remains explicit"

                Expect.isTrue
                    (Feature173LiveResponsivenessFixtures.hasLimitationContaining "headless-substitute" root)
                    "substitute caveat remains visible"
            }
        ]
