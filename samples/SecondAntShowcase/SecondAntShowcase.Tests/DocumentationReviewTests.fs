...
let documentationReviewTests =
    testList
        ...
        [
            ...
            test
                "feature 176 control-pass report exists with the required structure and a complete prioritisation table" {
                let reportPath =
                    Path.Combine(
                        repoRoot,
                        "docs",
                        "reports",
                        "2026-06-20-feature-176-second-antshowcase-control-pass-report.md"
                    )
                ...
                for row in prioritisationRows do
                    ...
                    Expect.isFalse
                        (System.String.IsNullOrWhiteSpace cells.[2])
                        (sprintf "severity is non-empty in: %s" row)

                    Expect.isFalse
                        (System.String.IsNullOrWhiteSpace cells.[cells.Length - 1])
                        (sprintf "leverage/recommendation is non-empty in: %s" row)
            }
        ]
