    ...
    testList
        ...
        [
            test "a seeded keyboard activation changes the product model (responds-proof)" {
                let metrics =
                    ControlsElmish.Perf.runScript Host.defaultHost size (Scripts.forPage "buttons")
            ...
            test "each interactive family maps its input to its own field transition" {
                ...
                // selection
                Expect.equal
                    (Model.update (PageMsg(SegmentedChanged "List")) m).PageState.SegmentedSelected
                    "List"
                    "segmented"

                Expect.equal
                    (Model.update (PageMsg(ComboChanged "Product")) m).PageState.ComboSelected
                    "Product"
                    "combo"

                Expect.equal
                    (Model.update (PageMsg(CascaderChanged "Europe / France / Paris")) m).PageState.CascaderSelected
                    "Europe / France / Paris"
                    "cascader"
                // text / numeric
                ...
                // disclosure
                Expect.equal
                    (Model.update (PageMsg(CollapseToggled "Security")) m).PageState.CollapseOpen
                    "Security"
                    "collapse"
            }
            ...
            test "the host key map routes activation keys to a button click" {
                Expect.equal
                    (Host.mapKey FS.GG.UI.KeyboardInput.Enter true)
                    (Some(PageMsg ButtonClicked))
                    "Enter activates"

                Expect.equal
                    (Host.mapKey FS.GG.UI.KeyboardInput.Space true)
                    (Some(PageMsg ButtonClicked))
                    "Space activates"
            ...
            test "display-only controls are exempt: navigation never alters interactive state" {
                let m0 =
                    { Host.initModel with
                        PageState = { DemoState.seed with ButtonClicks = 3 }
                    }

                let m1 = Model.update (NavigateTo "display-typography") m0

                Expect.equal
                    m1.PageState.ButtonClicks
                    3
                    "navigating to a display-only page changes no interactive state"
            ...
            test "interaction contracts are theme-invariant and point at known pages" {
                ...
                for contract in InteractionContracts.all do
                    ...
                    Expect.isTrue
                        (pageIds.Contains contract.PageId)
                        (sprintf "%s references a known page" contract.ContractId)
