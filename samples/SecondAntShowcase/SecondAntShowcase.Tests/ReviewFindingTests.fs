            ...
            test "unresolved gate blocks open, fixed, and reviewed blocking findings" {
                ...
                let fixedFinding =
                    ReviewFindings.create
                        "F-fixed"
                        [ "b" ]
                        ReviewFindings.Overlap
                        ReviewFindings.Warning
                        "fixed"
                        "expected"
                        "actual"
                    |> ReviewFindings.markFixed "commit"

                let reviewedBlocking =
                    ReviewFindings.create
                        "F-reviewed"
                        [ "c" ]
                        ReviewFindings.Clipping
                        ReviewFindings.Blocking
                        "reviewed"
                        "expected"
                        "actual"
                    |> ReviewFindings.markFixed "commit"
                    |> ReviewFindings.markReviewed "run"
                ...
                Expect.equal
                    unresolvedIds
                    [ "F-open"; "F-fixed"; "F-reviewed" ]
                    "only unresolved lifecycle states remain blocking"
            }

            test "validation reports malformed findings, missing target classifications, and unresolved ids" {
                let malformed =
                    ReviewFindings.create "" [] ReviewFindings.State ReviewFindings.Warning "bad" "expected" "actual"

                let openFinding =
                    ReviewFindings.create
                        "F-002"
                        [ "target-a" ]
                        ReviewFindings.AntConformance
                        ReviewFindings.Warning
                        "open"
                        "expected"
                        "actual"
                ...
                let result =
                    ReviewFindings.validate [ "target-a"; "target-b" ] [ malformed; openFinding ]

                Expect.equal result.MalformedFindingIds [ "" ] "malformed finding id reported"

                Expect.equal
                    result.MissingClassificationTargetIds
                    [ "target-b" ]
                    "missing target classification reported"
