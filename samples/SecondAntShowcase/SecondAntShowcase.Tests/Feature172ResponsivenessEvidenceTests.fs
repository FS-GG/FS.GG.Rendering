...
let tests =
    testList
        "Feature172 responsiveness evidence shape"
        [
            test "all-interactive records include sample review fields for every interactive family" {
                ...
                Expect.isTrue
                    (root.TryGetProperty("expectedVisibleResult", &property))
                    "expected visible result is written"

                Expect.equal
                    (root.GetProperty("acceptanceStatus").GetString())
                    "environment-limited"
                    "substitute record is environment-limited, not accepted"
            }
            ...
            test "summary carries budgets, coverage, and relative records path" {
                ...
                use doc =
                    SecondAntShowcase.Tests.Feature172ResponsivenessFixtures.summaryJson outDir
                ...
                Expect.equal
                    (root.GetProperty("recordsPath").GetString())
                    "records.jsonl"
                    "records path is relative to run root"
                ...
                Expect.equal
                    (coverage.GetProperty("requiredInteractiveFamilies").GetArrayLength())
                    InteractionContracts.all.Length
                    "all families required"

                Expect.equal
                    (coverage.GetProperty("acceptedInteractiveFamilies").GetArrayLength())
                    0
                    "substitute evidence accepts no families"

                Expect.equal
                    (coverage.GetProperty("blockedInteractiveFamilies").GetArrayLength())
                    InteractionContracts.all.Length
                    "all families are blocked until live evidence"

                Expect.isGreaterThan
                    (coverage.GetProperty("displayOnlyExclusions").GetArrayLength())
                    0
                    "display-only exclusions are explicit"

                Expect.equal
                    (coverage.GetProperty("missingInteractiveFamilies").GetArrayLength())
                    0
                    "families are enumerated, not missing"
            }
        ]
