...
let tests =
    testList
        "Feature173 live responsiveness budgets"
        [
            test "summary exposes p95 and max budgets" {
                ...
                Expect.equal
                    (budgets.GetProperty("inputToVisibleP95Ms").GetInt32())
                    Evidence.responsivenessTargetP95Ms
                    "p95 budget"

                Expect.equal
                    (budgets.GetProperty("inputToVisibleMaxMs").GetInt32())
                    Evidence.responsivenessTargetMaxMs
                    "max budget"
            }
            ...
            test "fail-closed run names environment-boundary as first failed budget" {
                ...
                Expect.equal
                    ...
                    "environment boundary failure is first"
            }
            ...
            test "Synthetic drag continuity helper classifies continuous and delayed feedback" {
                ...
                let continuous =
                    Evidence.responsivenessDragContinuity "slider-rating" 4 4 (Some 16.0) false

                let delayed =
                    Evidence.responsivenessDragContinuity "slider-rating" 4 2 (Some 80.0) true
