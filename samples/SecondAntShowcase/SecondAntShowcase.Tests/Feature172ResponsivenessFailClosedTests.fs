...
let tests =
    testList
        "Feature172 responsiveness fail-closed behavior"
        [
            test "require-live writes a visible limitation instead of accepting substitute evidence" {
                ...
                let code =
                    Responsiveness.run [ "--all-interactive"; "--require-live"; "--out"; outDir; "--json" ]

                use doc =
                    SecondAntShowcase.Tests.Feature172ResponsivenessFixtures.summaryJson outDir

                let limitations =
                    doc.RootElement.GetProperty("environmentLimitations").EnumerateArray()
                    |> Seq.map _.GetString()
                    |> Seq.toList
            ...
            test "records remain blocked when there is no measured presentation boundary" {
                ...
                let line =
                    SecondAntShowcase.Tests.Feature172ResponsivenessFixtures.records outDir
                    |> List.head
                ...
                Expect.equal
                    (root.GetProperty("environmentStatus").GetString())
                    "headless-substitute"
                    "environment status is explicit"

                Expect.equal
                    (root.GetProperty("visibleResponse").GetString())
                    "environment-limited"
                    "visible response is not presented-frame"
                ...
                Expect.isFalse
                    (root.GetProperty("phaseTiming").GetProperty("totalInputToVisibleMs").ValueKind =
                        JsonValueKind.Number)
                    "no live total timing is synthesized"
            }
        ]
