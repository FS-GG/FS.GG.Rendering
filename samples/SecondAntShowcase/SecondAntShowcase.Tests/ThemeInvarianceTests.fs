...
let themeInvarianceTests =
    testList
        ...
        [
            yield
                test "antLight and antDark resolve to distinct themes (visuals change)" {
                    Expect.notEqual AntTheme.antLight AntTheme.antDark "the two Ant variants differ"
                }

            for page in representative do
                yield
                    test (sprintf "page %s: tree shape + accessibility ids invariant across antLight/antDark" page.Id) {
                        ...
                        Expect.equal
                            (List.distinct shapes)
                            [ List.head shapes ]
                            "node count + bound ids identical across modes"
                    }

            yield
                test "theme switching preserves page, values, selections, overlays, and validation state" {
                    let before =
                        { Host.initModel with
                            ...
                            PageState =
                                { Host.initModel.PageState with
                                    ...
                                    DrawerOpen = true
                                    Form =
                                        { Host.initModel.PageState.Form with
                                            Phase = Invalid [ "Email", "Enter a valid email address" ]
                                        }
                                }
                        }
