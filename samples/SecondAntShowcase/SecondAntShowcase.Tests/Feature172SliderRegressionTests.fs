...
let tests =
    testList
        "Feature172 slider and value-changing regressions"
        [
            test "slider and rating messages still change product state" {
                let m = Host.initModel

                Expect.equal
                    (Model.update (PageMsg(SliderChanged 0.75)) m).PageState.SliderValue
                    0.75
                ...
                Expect.equal
                    m.PageState.ProgressValue
                    DemoState.seed.ProgressValue
                    "progress seeded display value is preserved"
            }
