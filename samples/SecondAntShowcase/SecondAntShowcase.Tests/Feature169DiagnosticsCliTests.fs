module SecondAntShowcase.Tests.Feature169DiagnosticsCliTests
...
let tests =
    testList
        "Feature169 SecondAntShowcase diagnostics CLI"
        [
            test "Synthetic diagnostics command writes JSON Markdown and compact default output" {
                let outDir =
                    Path.Combine(Path.GetTempPath(), "antshowcase-diagnostics-" + Guid.NewGuid().ToString("N"))
                ...
                try
                    ...
                    Expect.isTrue
                        (lines |> List.exists (fun line -> line.Contains("Diagnostics: accepted")))
                        "accepted status"
                finally
            ...
            test "Synthetic diagnostics command verbose output includes detailed records" {
                ...
                try
                    ...
                    Expect.isTrue
                        (verbose |> List.exists (fun line -> line.Contains("DamageScopedDecision")))
                        "verbose code"
                finally
                    if Directory.Exists outDir then
                        Directory.Delete(outDir, true)
