...
let tests =
    testList
        "Feature172 visual regression preservation"
        [
            test "opaque preferred and minimum shells still render retained inspection evidence" {
                for sizeRole, size in [ "preferred", preferredSize; "minimum", minimumSize ] do
                    for themeId, mode in [ "antLight", Light; "antDark", Dark ] do
                        ...
                        let evidence =
                            Evidence.retainedInspectionEvidence "buttons" themeId sizeRole size.Width size.Height

                        Expect.isGreaterThan shell.NodeCount 0 (sprintf "%s %s shell renders" themeId sizeRole)

                        Expect.equal
                            evidence.RetainedStatus
                            "accepted"
                            (sprintf "%s %s retained evidence accepted" themeId sizeRole)

                        Expect.isGreaterThanOrEqual
                            evidence.RepaintedNodeCount
                            0
                            "retained evidence carries repaint facts"
