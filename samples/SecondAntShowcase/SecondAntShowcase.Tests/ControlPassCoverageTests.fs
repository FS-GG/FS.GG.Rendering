        ...
        [
                ...
                for id in ControlPass.catalogControlIds () do
                    ...
                    | DisplayOnly ->
                        Expect.isFalse
                            ...
                            (sprintf "display-only '%s' has a reason" id)
                    | Interactive ->
                        Expect.isNonEmpty
                            (ControlPass.behaviorsFor id)
                            (sprintf "interactive '%s' has >=1 documented behavior" id)
            }
            ...
            test "behavior coverage: every Interactive control exercises every documented behavior (C-4/G-3, SC-002)" {
                ...
                for id in ControlPass.catalogControlIds () do
                    ...
                    | Interactive ->
                        let declared =
                            ControlPass.behaviorsFor id |> List.map (fun c -> c.ContractId) |> Set.ofList

                        let exercised =
                            ControlPass.exerciseControl seed id
                            |> List.map (fun b -> b.BehaviorId)
                            |> Set.ofList
