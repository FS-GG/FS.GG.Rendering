/// Headless deterministic evidence mode (US3). For each page it: (1) replays the seeded
...
/// (FR-011/SC-004) and always exits 0 on success.
module ControlsGallery.App.Evidence
...
let runPage (seed: int) (outDir: string) (page: GalleryPage): Evidence.PageEvidenceRecord =
    ...
    // 3. write the record.
    let record = Evidence.build page.Id seed metrics summary
