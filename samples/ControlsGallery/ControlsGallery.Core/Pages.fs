...
let private group (bodies: Control<GalleryMsg> list) : Control<GalleryMsg> = Stack.create [ Stack.children bodies ]
...
let private displayPage (s: DemoState) : Control<GalleryMsg> =
    let richBlock =
        RichText.block
            [
                RichText.run "Rich text with measured runs." (RichText.defaultStyle GalleryTheme.defaultTheme)
            ]
...
let private selectionPage (s: DemoState) : Control<GalleryMsg> =
    ...
    let swatches: FS.GG.UI.Controls.Typed.ColorSwatch list =
        [
            {
                Name = "Indigo"
                Color = GalleryTheme.indigo
            }
            {
                Name = "Teal"
                Color = GalleryTheme.teal
            }
        ]
...
let all: GalleryPage list =
    [
        ...
        }
        {
            Id = "pointer-custom"
            Index = 10
            Title = "Pointer Playground / Custom"
            Family = "Pointer Playground / Custom"
