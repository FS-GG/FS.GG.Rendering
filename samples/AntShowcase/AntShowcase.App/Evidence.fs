...
module AntShowcase.App.Evidence
    ...
    if page.Id = "text-numeric-input" then
        ...
        let overlayLines =
            [
                ...
                sprintf
                    "- diagnostics: %s"
                    (if List.isEmpty overlay.Diagnostics then
                         "none"
                     else
                         String.concat ", " overlay.Diagnostics)
                sprintf "- no stale overlay: %b" overlay.NoStaleOverlay
            ]

        File.WriteAllText(
            Path.Combine(dir, "feature144-date-picker-overlay.md"),
            String.concat Environment.NewLine overlayLines + Environment.NewLine
        )
...
let run (seed: int) (outDir: string) (pageFilter: string option) : int =
    ...
    if List.isEmpty pages then
        eprintfn "ant-showcase: no page matched '%s'." (Option.defaultValue "" pageFilter)
        2
    else
        ...
        printfn "ant-showcase: wrote %d page evidence record(s) under %s/%d" (List.length records) outDir seed
        0
