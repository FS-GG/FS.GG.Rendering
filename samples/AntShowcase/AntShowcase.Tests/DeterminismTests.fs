module AntShowcase.Tests.DeterminismTests
        ...
        [
            test "golden state outcome is byte-identical across two same-seed runs over all pages" {
                for page in PageRegistry.all do
                    ...
                    Expect.equal
                        (Evidence.goldenState m1)
                        (Evidence.goldenState m2)
                        (sprintf "page %s golden state identical" page.Id)
            }
            ...
            test "run.json text is byte-identical for identical record inputs" {
                ...
                for page in PageRegistry.all do
                    let metrics =
                        ControlsElmish.Perf.runScript Host.defaultHost size (Scripts.forPage page.Id)

                    let shot = Evidence.degraded "pure suite: no GL capture"

                    let j1 =
                        Evidence.toRunJson (Evidence.build page.Id seed "antLight" page.ControlIds metrics shot)
            ...
            test "the form's invalid-then-valid sequence is deterministic through pure update" {
                let run () =
                    List.fold (fun acc m -> Model.update m acc) Host.initModel Scripts.formInvalidThenValid
