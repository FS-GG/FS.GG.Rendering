module AntShowcase.Tests.FeedbackTests
        ...
        [
            test "submitting a non-blank draft saves a page-tagged entry and clears the draft" {
                let m0 =
                    { Host.initModel with
                        CurrentPage = "buttons"
                        FeedbackDraft = "  the button needs a loading state  "
                    }
            ...
            test "feedback accumulates newest-first and stays tagged per page" {
                ...
                Expect.equal
                    (m.Feedback |> List.map (fun e -> e.PageId, e.Text))
                    [ "overlays", "b"; "buttons", "a" ]
                    "newest-first, per-page tags"
            }

            test "feedback lines round-trip through the persistence encoding (incl. tab/newline)" {
                let entry =
                    {
                        PageId = "feedback-status"
                        Text = "line1\nline2\tcol"
                    }
