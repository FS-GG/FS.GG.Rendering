module AntShowcase.Tests.Feature143DatePickerFlowTests
...
let tests =
    testList
        "Feature143 AntShowcase date-picker reference flow"
        [
            test "scripted date-picker overlay opens selects dismisses and records evidence" {
                let opened, _ =
                    OverlayState.update (OpenRequested datePickerSurface) (OverlayState.init ())

                let selected, effects =
                    OverlayState.update
                        (SelectionCompleted(
                            "ant-date-picker-calendar",
                            "ant-date-picker-calendar:2026-06-17",
                            Some "2026-06-17"
                        ))
                        opened
                ...
                Expect.exists
                    effects
                    (function
                    | DispatchProductMessage("ant-date-picker-calendar", Some "2026-06-17") -> true
                    | _ -> false)
                    "exactly one date selection effect"
