    ...
    [
        {
            ...
            Family = "game"
            Disposition = "Adopted"
            Reason = "curated slice — grid + gravity loop + keyboard"
        }
        {
            Spec = "Snake"
            Family = "game"
            Disposition = "Adopted"
            Reason = "curated slice — grid + directional + step loop"
        }
        {
            Spec = "Pong"
            Family = "game"
            Disposition = "Adopted"
            Reason = "curated slice — continuous motion + paddle"
        }
        {
            Spec = "Asteroids"
            Family = "game"
            Disposition = "Deferred"
            Reason = "backlog — coverage already met by Tetris/Snake/Pong"
        }
        {
            Spec = "Breakout"
            Family = "game"
        ...
        }
        {
            ...
            Family = "productivity"
            Disposition = "Adopted"
            Reason = "curated slice — data grid + pointer move + inline edit"
        }
        {
            Spec = "Todo/task manager"
            Family = "productivity"
            Disposition = "Adopted"
            Reason = "curated slice — forms + validation + list + inline edit"
        }
        {
            Spec = "Calendar scheduler"
            Family = "productivity"
            Disposition = "Adopted"
            Reason = "curated slice — date grid + forms"
        }
        {
            Spec = "Contact manager"
            Family = "productivity"
            Disposition = "Deferred"
            Reason = "backlog — patterns already met by Kanban/Todo/Calendar"
        }
        {
...
let check () : CoverageBacklogResult =
    ...
    let missingInputs =
        requiredInputs |> List.filter (fun i -> not (inputUnion.Contains i))
    ...
    let badEntries =
        backlog
        |> List.filter (fun b ->
            not (b.Disposition = "Adopted" || b.Disposition = "Deferred")
            || b.Reason.Trim() = "")
    ...
    let prod =
        backlog |> List.filter (fun b -> b.Family = "productivity") |> List.length

    let countIssues =
        [
            if List.length backlog <> 22 then
                sprintf "total:%d<>22" (List.length backlog)
            if games <> 12 then
                sprintf "games:%d<>12" games
            if prod <> 10 then
                sprintf "productivity:%d<>10" prod
        ]
    ...
    let adoptedIds =
        adopted
        |> List.choose (fun b -> Map.tryFind b.Spec adoptedSpecToId)
        |> Set.ofList
    ...
    let adoptedMismatch =
        unmappedAdopted
        @ (Set.difference adoptedIds registryIds
           |> Set.toList
           |> List.map (sprintf "adopted-not-in-registry:%s"))
        @ (Set.difference registryIds adoptedIds
           |> Set.toList
           |> List.map (sprintf "registry-not-adopted:%s"))
        @ (Set.difference coverageIds registryIds
           |> Set.toList
           |> List.map (sprintf "coverage-row-not-in-registry:%s"))
        @ (Set.difference registryIds coverageIds
           |> Set.toList
           |> List.map (sprintf "registry-not-in-coverage:%s"))
...
let render () : string =
    ...
    for r in coverageRows do
        line (
            sprintf
                ...
                r.SampleId
                r.Family
                (String.concat ", " r.Inputs)
                (String.concat ", " r.Controls)
        )
    ...
    let adoptedCount =
        backlog |> List.filter (fun b -> b.Disposition = "Adopted") |> List.length

    let deferredCount =
        backlog |> List.filter (fun b -> b.Disposition = "Deferred") |> List.length

    line (
        sprintf
            "%d adopted (the curated slice) · %d deferred · %d total."
            adoptedCount
            deferredCount
            (List.length backlog)
    )
