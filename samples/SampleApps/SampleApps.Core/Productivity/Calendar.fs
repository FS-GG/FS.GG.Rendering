...
let validate (text: string) : Result<string, string list> =
    ...
    if trimmed = "" then
        Result.Error [ "entry text is required" ]
    else
        Result.Ok trimmed
...
let update (msg: Msg) (model: Model) : Model =
    ...
    | PrevDay ->
        { model with
            Selected = clamp (model.Selected - 1)
        }
    | NextDay ->
        { model with
            Selected = clamp (model.Selected + 1)
...
let view (_size: Size) (model: Model) : Control<Msg> =
    let cell (day: int) =
        ...
        let label =
            if day = model.Selected then
                sprintf "[%d]%s" day marker
            else
                sprintf " %d %s" day marker

        TextBlock.create [ TextBlock.text label ]

    let grid = Grid.create [ Grid.children [ for d in 1..days -> cell d ] ]
    ...
    let entryList =
        if List.isEmpty dayEntries then
            [ TextBlock.create [ TextBlock.text "No entries for this day." ] ]
        else
            dayEntries
    ...
    let errorLine =
        ...
        | errs ->
            [
                TextBlock.create [ TextBlock.text (sprintf "⚠ %s" (String.concat "; " errs)) ]
            ]
    ...
    Stack.create
        [
            ...
            Stack.children (
                [
                    Label.create [ Label.text (sprintf "Calendar — day %d" model.Selected) ]
                    grid
                    draftRow
                ]
                @ errorLine
...
let renderScene (_size: Size) (model: Model) : Scene =
    let committed =
        model.Entries |> Map.toList |> List.sumBy (fun (_, es) -> List.length es)
    ...
    let header =
        [
            ...
            Scene.sizedText
                (12.0, 25.0)
                (sprintf "CALENDAR   day %d   %d entries   %d rejected" model.Selected committed model.Rejected)
                15.0
                col_text
        ]
    ...
    let entries = model.Entries |> Map.tryFind model.Selected |> Option.defaultValue []
    ...
    let entryViews =
        if List.isEmpty entries then
            ...
            entries
            |> List.mapi (fun i e ->
                Scene.sizedText (12.0, listTop + 8.0 + float i * 22.0) (sprintf "- %s" e) 13.0 col_text)
    ...
    let errs =
        ...
        | e ->
            [
                Scene.sizedText (12.0, frameH - 52.0) ("! " + String.concat "; " e) 12.0 col_red
            ]

    let draftLabel =
        if model.Draft = "" then
            "LEFT/RIGHT pick a day, type an entry, ENTER adds"
        else
            model.Draft
