...
let validate (text: string) : Result<string, string list> =
    ...
    elif trimmed.Length > 40 then
        Result.Error [ "title must be 40 characters or fewer" ]
    else
        Result.Ok trimmed
        ...
        | Some e ->
            ...
            | Result.Ok title ->
                { model with
                    Items =
                        model.Items
                        |> List.map (fun i -> if i.Id = e.Id then { i with Title = title } else i)
                    Edit = None
                    Errors = []
                }
    ...
    | KeySpace ->
        ...
        | _ :: _ ->
            { model with
                Items =
                    model.Items
                    |> List.mapi (fun idx i -> if idx = 0 then { i with Done = not i.Done } else i)
            }
        | [] -> model
    | KeyEdit ->
        match model.Items with
        | first :: _ ->
            { model with
                Edit = Some { Id = first.Id; Buffer = first.Title }
            }
        | [] -> model
                            ...
                            [
                                CheckBox.create
                                    [
                                        ...
                                        CheckBox.onChanged (fun _ -> KeySpace)
                                    ]
...
let renderScene (_size: Size) (model: Model) : Scene =
    ...
    let header =
        [
            ...
            Scene.sizedText
                (12.0, 25.0)
                (sprintf "TODO   %d tasks   %d done" (List.length model.Items) doneCount)
                16.0
                col_text
        ]
    ...
    let hint =
        [
            Scene.sizedText (12.0, headerPx + 58.0) "ENTER=add   UP=edit first   SPACE=toggle first" 11.0 col_dim
        ]
    ...
    let errs =
        ...
        | e ->
            [
                Scene.sizedText (12.0, headerPx + 78.0) ("! " + String.concat "; " e) 12.0 col_red
            ]
