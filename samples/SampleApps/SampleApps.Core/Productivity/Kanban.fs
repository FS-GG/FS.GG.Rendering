...
let validate (text: string) : Result<string, string list> =
    ...
    if trimmed = "" then
        Result.Error [ "card title is required" ]
    else
        Result.Ok trimmed
...
let update (msg: Msg) (model: Model) : Model =
    ...
    | KeyChar c ->
        ...
        | Some e ->
            { model with
                Edit = Some { e with Buffer = e.Buffer + string c }
            }
        | None ->
            { model with
                Draft = model.Draft + string c
                Errors = []
            }
    | KeyBackspace ->
        match model.Edit with
        | Some e ->
            { model with
                Edit = Some { e with Buffer = dropLast e.Buffer }
            }
        | None ->
            { model with
                Draft = dropLast model.Draft
            }
    | KeyEnter ->
        match model.Edit with
        | Some e ->
            ...
            | Result.Ok title ->
                { model with
                    Columns =
                        model.Columns
                        |> List.map (List.map (fun c -> if c.Id = e.Id then { c with Title = title } else c))
                    Edit = None
                    Errors = []
                }
    ...
    | KeyEdit ->
        ...
        | Some card ->
            { model with
                Edit = Some { Id = card.Id; Buffer = card.Title }
            }
        | None -> model
    | MoveForward ->
...
let view (_size: Size) (model: Model) : Control<Msg> =
    ...
    let columnView (name: string) (cards: Card list) =
        let cardViews =
            if List.isEmpty cards then
                ...
                cards
                |> List.map (fun c -> Border.create [ Border.child (TextBlock.create [ TextBlock.text c.Title ]) ])
    ...
    let board =
        if totalCards = 0 then
            [
                TextBlock.create
                    [
                        TextBlock.text "No cards yet — type a title and press Enter to add one to Todo."
                    ]
            ]
        else
            [
    ...
    Stack.create
        [
            Stack.orientation "vertical"
            Stack.children (Label.create [ Label.text "Kanban" ] :: draftRow :: board)
        ]
...
let renderScene (_size: Size) (model: Model) : Scene =
    ...
    let columns =
        ...
        |> List.mapi (fun ci cards ->
            ...
            let colBg =
                Scene.rectangle (x, headerPx, colW, frameH - headerPx - 50.0) (Colors.rgb 23uy 31uy 47uy)

            let colTitle =
                Scene.sizedText (x + 8.0, headerPx + 22.0) (List.item ci columnNames) 14.0 col_dim
    ...
    let draftLabel =
        match model.Edit with
        | Some e -> sprintf "editing: %s" e.Buffer
        | None ->
            (if model.Draft = "" then
                 "type a card title, ENTER adds to Todo (CLICK advances)"
             else
                 model.Draft)
    ...
    let draft =
        [
            ...
            Scene.sizedText
                ...
                draftLabel
                13.0
                (if model.Edit.IsNone && model.Draft = "" then
                     col_dim
                 else
                     col_text)
        ]
...
let deriveOutcome (model: Model) : ExpectedOutcome =
    ...
    {
        Kind = "productivity"
        Values = [ "cards", string total; "done", string doneCount; "moved", string model.Moved ]
    }
...
let private clickAt: FrameInput<Msg> =
    FrameInput.Pointer(Click("board", PointerButton.Primary, 0.0, 0.0))
...
let script: FrameInput<Msg> list =
    ...
    @ [ press ArrowUp ]
    @ typeText "Z"
    @ [ press Enter ] // inline-edit the remaining card
    @ [ FrameInput.Idle ]
...
let recordAt (seedValue: int) : SampleEvidenceRecord =
    Harness.recordFor
        "kanban"
        (hostFor seedValue FS.GG.UI.Themes.Default.Theming.Light SampleTheme.indigo)
        script
        deriveOutcome
        seedValue
...
let entry: Harness.SampleEntry =
    {
        ...
        RunEvidence =
            fun seedValue outDir ->
                Harness.evidenceForScene
                    renderScene
                    "kanban"
                    (hostFor seedValue FS.GG.UI.Themes.Default.Theming.Light SampleTheme.indigo)
                    script
