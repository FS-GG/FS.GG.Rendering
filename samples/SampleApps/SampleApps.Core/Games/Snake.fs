...
let private placeFood (snake: (int * int) list) (rng: Prng) : (int * int) * Prng =
    ...
    let rec loop (r: Prng) (fuel: int) : (int * int) * Prng =
        ...
        if fuel <= 0 || not (occupied.Contains(x, y)) then
            (x, y), r2
        else
            loop r2 (fuel - 1)
...
let update (msg: Msg) (model: Model) : Model =
    ...
    | Turn dir ->
        if opposite dir model.Dir then
            model
        else
            { model with Dir = dir }
...
let private boardScene (model: Model) : Scene =
    ...
    let bodyCells =
        ...
        |> List.mapi (fun i (x, y) ->
            let color =
                if i = 0 then
                    Colors.rgb 134uy 239uy 172uy
                else
                    Colors.rgb 34uy 197uy 94uy
...
let private overlay (model: Model) : Scene list =
    ...
    if not model.Started then
        banner "SNAKE" "ENTER to start · arrows steer"
    elif model.Over then
        banner "CRASHED" "ENTER to play again"
    else
        []
...
let view (_size: Size) (model: Model) : Control<Msg> =
    let status =
        Label.create
            [
                Label.text (
                    sprintf "SNAKE   score %d   len %d   %s" model.Score (List.length model.Snake) (phaseLabel model)
                )
                Attr.width boardW
            ]
    ...
    Stack.create
        [
            ...
            Stack.children
                [
                    status
                    Harness.canvas "snake-board" boardW boardH (fun () -> boardScene model)
                ]
        ]
