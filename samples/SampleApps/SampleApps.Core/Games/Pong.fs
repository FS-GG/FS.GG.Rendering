...
let private step (model: Model) : Model =
    ...
    let leftY =
        ...
        model.LeftY + clamp -leftSpeed leftSpeed diff |> clamp 0.0 (courtH - paddleH)
...
let update (msg: Msg) (model: Model) : Model =
    ...
    | Tick dt ->
        ...
        if t >= stepIntervalMs then
            step
                { model with
                    StepTimer = t - stepIntervalMs
                }
        else
            { model with StepTimer = t }
    | Step -> step model
    | MoveLeft Up ->
        { model with
            LeftY = clamp 0.0 (courtH - paddleH) (model.LeftY - 1.0)
        }
    | MoveLeft Down ->
        { model with
            LeftY = clamp 0.0 (courtH - paddleH) (model.LeftY + 1.0)
...
let private boardScene (model: Model) : Scene =
    ...
    let midline =
        Scene.rectangle (courtPxW / 2.0 - 1.0, 0.0, 2.0, courtPxH) (Colors.rgb 51uy 65uy 85uy)

    let leftPaddle =
        Scene.rectangle (0.0, model.LeftY * sc, paddlePxW, paddleH * sc) (Colors.rgb 59uy 130uy 246uy)

    let rightPaddle =
        Scene.rectangle (courtPxW - paddlePxW, model.RightY * sc, paddlePxW, paddleH * sc) (Colors.rgb 239uy 68uy 68uy)

    let ball =
        Scene.rectangle (model.BallX * sc - 5.0, model.BallY * sc - 5.0, 10.0, 10.0) (Colors.rgb 250uy 204uy 21uy)

    Scene.group [ bg; midline; leftPaddle; rightPaddle; ball ]

let private phaseLabel (model: Model) : string =
    if not model.Started then "READY"
    elif model.Over then "MATCH OVER"
    else "rally"
...
let private overlay (model: Model) : Scene list =
    ...
    if not model.Started then
        banner "PONG" "Press ENTER to start"
    elif model.Over then
        banner "MATCH OVER" "Press ENTER to play again"
    else
        []
    ...
    Stack.create
        [
            ...
            Stack.children
                [
                    status
                    Harness.canvas "pong-court" courtPxW courtPxH (fun () -> boardScene model)
                ]
        ]
...
let private stepTick: FrameInput<Msg> =
    FrameInput.Tick(TimeSpan.FromMilliseconds stepIntervalMs)
...
let private hostFor (seedValue: int) (mode) (accent) =
    Harness.host
        (fun () -> init (seed seedValue))
        update
        view
        mapKey
        (fun _ -> None)
        tick
        (SampleTheme.resolve mode accent)
