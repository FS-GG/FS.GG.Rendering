...
let private collides (board: int[]) (p: Piece) : bool =
    pieceCells p
    |> List.exists (fun (r, c) -> c < 0 || c >= width || r >= height || (r >= 0 && board.[r * width + c] <> 0))
...
let private tryUpdate (model: Model) (candidate: Piece) : Model =
    if collides model.Board candidate then
        model
    else
        { model with Active = candidate }

/// One gravity step: drop the piece a row, or lock+clear+spawn if it cannot fall.
let private gravity (model: Model) : Model =
    let down =
        { model.Active with
            Row = model.Active.Row + 1
        }

    if collides model.Board down then
        lockAndClear model
    else
        { model with Active = down }
...
let update (msg: Msg) (model: Model) : Model =
    ...
    | Tick dt ->
        ...
        if t >= dropIntervalMs then
            gravity
                { model with
                    DropTimer = t - dropIntervalMs
                }
        else
            { model with DropTimer = t }
    | Left ->
        tryUpdate
            model
            { model.Active with
                Col = model.Active.Col - 1
            }
    | Right ->
        tryUpdate
            model
...
let private cellKind (model: Model) (occupied: Map<int * int, int>) (r: int) (c: int) : int option =
    ...
    | Some k -> Some k
    | None ->
        if model.Board.[r * width + c] <> 0 then
            Some(model.Board.[r * width + c] - 1)
        else
            None
...
let private phaseLabel (model: Model) : string =
    ...
    elif model.Over then "GAME OVER"
    else "playing"
...
let private overlay (model: Model) : Scene list =
    ...
    if not model.Started then
        banner "TETRIS" "ENTER to start"
    elif model.Over then
        banner "GAME OVER" "ENTER to play again"
    else
        []
    ...
    Stack.create
        [
            ...
            Stack.children
                [
                    status
                    Harness.canvas "tetris-board" boardW boardH (fun () -> boardScene model)
                ]
        ]
...
let private gravityTick: FrameInput<Msg> =
    FrameInput.Tick(TimeSpan.FromMilliseconds 500.0)
