        ...
        [
            test "every sample degrades-and-discloses on a no-GL path (FR-008/SC-003)" {
                for r in records () do
                    ...
                    Expect.isSome
                        r.Screenshot.UnsupportedHostReason
                        (sprintf "%s states an unsupported-host reason" r.SampleId)

                    Expect.equal
                        r.Screenshot.Fallback
                        (Some "deterministic-state-only")
                        (sprintf "%s discloses the fallback" r.SampleId)

                    Expect.equal
                        r.Screenshot.BlockedStage
                        (Some "capture")
                        (sprintf "%s names the blocked stage" r.SampleId)

                    Expect.isNone r.Screenshot.Path (sprintf "%s leaves no frame path when not proven" r.SampleId)

                    Expect.isNonEmpty
                        r.NotAuthoritativeFor
                        (sprintf "%s discloses what it is NOT authoritative for (FR-007)" r.SampleId)

                    Expect.isFalse
                        (List.contains "non-blank-offscreen-png" r.AuthoritativeFor)
                        (sprintf "%s never claims the png when degraded" r.SampleId)
