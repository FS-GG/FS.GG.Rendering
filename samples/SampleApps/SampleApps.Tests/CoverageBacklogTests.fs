...
let private committedReport =
    Path.Combine(AppContext.BaseDirectory, "coverage-backlog.md")
    ...
    testList
        ...
        [
            ...
            test "exactly 22 backlog specs: 12 game + 10 productivity, no duplicates (R-B1/B4)" {
                ...
                let games =
                    Coverage.backlog |> List.filter (fun b -> b.Family = "game") |> List.length

                let prod =
                    Coverage.backlog
                    |> List.filter (fun b -> b.Family = "productivity")
                    |> List.length
                ...
                for b in Coverage.backlog do
                    Expect.isTrue
                        (b.Disposition = "Adopted" || b.Disposition = "Deferred")
                        (sprintf "%s dispositioned" b.Spec)
            ...
            test "registry invariants: 6 entries, unique ids, 3 game + 3 productivity (sample-registry.md)" {
                ...
                let expectedIds =
                    Set.ofList [ "tetris"; "snake"; "pong"; "kanban"; "todo"; "calendar" ]
                ...
                let prod =
                    Registry.all |> List.filter (fun e -> e.Family = "productivity") |> List.length
            ...
            test "committed coverage-backlog.md matches Coverage.render () (no drift, T035/SC-005)" {
                ...
                Expect.equal
                    (File.ReadAllText committedReport)
                    (Coverage.render ())
                    "committed report == rendered report"
            }
        ]
