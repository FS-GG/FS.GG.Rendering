...
module FeatureState =
    ...
    let updateReadiness msg model =
        let model' =
            ...
            | ProofLoaded proof ->
                { model with
                    Proofs = model.Proofs @ [ proof ]
                }
            | ParityRecorded(scenarioId, verdict) ->
                { model with
                    Parity = Map.add scenarioId verdict model.Parity
                }
            | TierEvaluated(tier, verdict) ->
                { model with
                    TierVerdicts = Map.add tier verdict model.TierVerdicts
                }
    ...
    let updateFeature154 msg model =
        let model' =
            ...
            | ArtifactPublished path ->
                { model with
                    PublishedArtifacts = model.PublishedArtifacts @ [ path ]
                }
    ...
    let feature156OverallVerdict (reports: Feature156ScenarioReport list) =
        ...
        if requiredReports.Length < feature156RequiredScenarioIds.Length then
            ...
            requiredReports
            |> List.forall (fun report -> report.Verdict = Feature156Positive)
        then
            Feature156Positive
        elif
            requiredReports
            |> List.exists (fun report -> report.Verdict = Feature156EnvironmentLimited)
        then
            Feature156EnvironmentLimited
        elif
            requiredReports
            |> List.exists (fun report -> report.Verdict = Feature156Limited)
        then
            Feature156Limited
        elif
            requiredReports
            |> List.exists (fun report -> report.Verdict = Feature156Rejected)
        then
            Feature156Rejected
        elif
            requiredReports
            |> List.exists (fun report -> report.Verdict = Feature156Incomplete)
        then
            Feature156Incomplete
    ...
    let initFeature156 warmupCount measuredRepetitions : Feature156Model * Feature156Effect list =
        ...
        [ Feature156DetectHostProfile; Feature156DeclarePolicy feature156PolicyId ]
    ...
    let updateFeature156 (msg: Feature156Msg) (model: Feature156Model) : Feature156Model * Feature156Effect list =
        let model' =
            ...
            | Feature156PolicyDeclared policyId -> { model with PolicyId = Some policyId }
    ...
    let feature157ScenarioFileName (scenarioId: string) = scenarioId.Replace("/", "-") + ".md"

    let feature157OverallStatus (summary: Feature157DamageSummary) =
        let acceptedScenarioSet =
            summary.AcceptedAttempts |> List.map _.ScenarioId |> Set.ofList

        let requiredCovered =
            feature157RequiredScenarioIds |> List.forall acceptedScenarioSet.Contains

        if summary.UnsupportedHostReason.IsSome then
            Feature157DamageStatus.EnvironmentLimited
        elif
            summary.Fallbacks
            |> List.exists (fun fallback -> fallback.Reason = "parity-mismatch")
        then
    ...
    let feature157StatusFrom
        ...
        if
            diagnostics
            |> List.exists (fun item -> item.Contains("environment-limited", StringComparison.OrdinalIgnoreCase))
        then
            Feature157DamageStatus.EnvironmentLimited
    ...
    let updateFeature157 (msg: Feature157Msg) (model: Feature157Model) : Feature157Model * Feature157Effect list =
        let model' =
            ...
            | Feature157FallbackRecorded fallback ->
                { model with
                    Fallbacks = model.Fallbacks @ [ fallback ]
                }
        ...
        let status =
            feature157StatusFrom model'.Attempts model'.Fallbacks model'.Diagnostics
    ...
    let feature158ScenarioFileName (scenarioId: string) = scenarioId.Replace("/", "-") + ".md"
    ...
    let feature158StatusFromReports (reports: Feature158ScenarioReport list) (diagnostics: string list) =
        ...
        then
            ...
            let allRequiredCovered =
                requiredReports.Length = feature158RequiredScenarioIds.Length
            ...
            if allAccepted then
                ...
                reports
                |> List.exists (fun report -> report.Status = Feature158ReadinessStatus.EnvironmentLimited)
            then
                Feature158ReadinessStatus.EnvironmentLimited
            elif
                reports
                |> List.exists (fun report -> report.Status = Feature158ReadinessStatus.FallbackOnly)
            then
    ...
    let updateFeature158 (msg: Feature158Msg) (model: Feature158Model) : Feature158Model * Feature158Effect list =
        let model' =
            ...
            | Feature158PolicyDeclared policyId -> { model with PolicyId = Some policyId }
            ...
            | Feature158ProbeEvidenceRecorded evidence ->
                { model with
                    ProofProbeEvidence = model.ProofProbeEvidence @ [ evidence ]
                }
    ...
    let feature159ScenarioFileName (scenarioId: string) = scenarioId.Replace("/", "-") + ".md"
    ...
    let feature159StatusFromAttempts (attempts: Feature159Attempt list) (diagnostics: string list) =
        ...
        then
            ...
            let allRequiredCovered =
                requiredAttempts.Length = feature159RequiredScenarioIds.Length
            ...
            if allRequiredCovered && acceptedAttempts.Length >= 3 then
                ...
                |> List.exists (fun attempt ->
                    attempt.PrimaryReason = Some "parity-mismatch"
                    || attempt.PrimaryReason = Some "missing-policy")
            then
                Feature159ReadinessStatus.Rejected
            elif
                allRequiredCovered
                && attempts
                   |> List.exists (fun attempt -> attempt.PromotionDecision = "non-beneficial")
            then
    ...
    let updateFeature159 (msg: Feature159Msg) (model: Feature159Model) : Feature159Model * Feature159Effect list =
        let model' =
            ...
            | Feature159PolicyDeclared policyId -> { model with PolicyId = Some policyId }
    ...
    let feature160ScenarioFileName (scenarioId: string) = scenarioId.Replace("/", "-") + ".md"

    let feature160IterationFileName (iterationId: string) = iterationId.Replace("/", "-") + ".md"
    ...
    let feature160IterationAccepted (iteration: Feature160Iteration) =
        ...
        && iteration.ActualDuration
           <= TimeSpan.FromMinutes(float feature160MaxIterationMinutes)
        ...
        && iteration.ArtifactPaths
           |> List.exists (fun path -> path.EndsWith(".md", StringComparison.OrdinalIgnoreCase))
    ...
    let feature160FocusedThroughputStatus (summary: Feature160ThroughputSummary) =
        ...
        | None ->
            ...
            if accepted.Length >= summary.RequiredAttempts then
                ...
                summary.Iterations
                |> List.exists (fun iteration -> iteration.Status = Feature160ReadinessStatus.Rejected)
            then
                Feature160ReadinessStatus.Rejected
            elif
                summary.Iterations
                |> List.exists (fun iteration -> iteration.Status = Feature160ReadinessStatus.EnvironmentLimited)
            then
        ...
        let summary =
            {
                ...
                UnsupportedHostReason =
                    ...
                    |> List.tryFind (fun item ->
                        item.Contains("environment-limited", StringComparison.OrdinalIgnoreCase))
    ...
    let updateFeature160 (msg: Feature160Msg) (model: Feature160Model) : Feature160Model * Feature160Effect list =
        let model' =
            ...
            | Feature160LaneDeclared laneId -> { model with LaneId = Some laneId }
            | Feature160PolicyDeclared policyId -> { model with PolicyId = Some policyId }
            | Feature160BoundDeclared minutes ->
                { model with
                    DeclaredBoundMinutes = Some(max 1 minutes)
                }
            | Feature160IterationStarted iterationId ->
                { model with
                    Diagnostics = model.Diagnostics @ [ $"iteration-started={iterationId}" ]
                }
            | Feature160IterationCompleted iteration ->
                { model with
                    Iterations = model.Iterations @ [ iteration ]
                }
            ...
            | Feature160IterationTimedOut(iterationId, reason) ->
                { model with
                    Diagnostics = model.Diagnostics @ [ $"timed-out:{iterationId}:{reason}" ]
                }
            | Feature160IterationCanceled(iterationId, reason) ->
                { model with
                    Diagnostics = model.Diagnostics @ [ $"canceled:{iterationId}:{reason}" ]
                }
            | Feature160FullValidationRecorded record ->
                { model with
                    FullValidation = Some record
                }
    ...
    let feature161ValidateHostFacts (facts: Feature161HostFacts) =
        ...
        then
            ...
            || List.isEmpty facts.ArtifactLocations
        then
            Some Perf.HostFactsMissing
            ...
            facts.CpuLoadNote.Contains("non-representative", StringComparison.OrdinalIgnoreCase)
            || facts.GpuLoadNote.Contains("non-representative", StringComparison.OrdinalIgnoreCase)
        then
            Some Perf.LoadNonRepresentative
        elif
            facts.EnvironmentLimits
            |> List.exists (fun item -> item.Contains("virtual", StringComparison.OrdinalIgnoreCase))
        then
    ...
    let feature161LedgerEntryAccepted (entry: Feature161LedgerEntry) =
        ...
        && entry.PriorGates
           |> List.forall (fun gate -> gate.Status = "confirmed" || gate.Status = "accepted")
    ...
    let feature161ScopeFromEntries (entries: Feature161LedgerEntry list) =
        ...
        let blockers =
            [
                ...
                if
                    entries
                    |> List.exists (fun entry -> entry.PrimaryExclusionReason = Some Perf.NoisyTiming)
                then
                ...
                if
                    ...
                    |> List.exists (fun entry ->
                        entry.PriorGates
                        |> List.exists (fun gate -> gate.Status <> "confirmed" && gate.Status <> "accepted"))
                then
                    "prior P7 gate blocked"
                if
                    entries
                    |> List.exists (fun entry -> feature161ValidateHostFacts entry.HostFacts |> Option.isSome)
                then
    ...
    let feature161OverallStatus (summary: Feature161Summary) =
        if summary.UnsupportedHostReason.IsSome then
            ...
            summary.Entries
            |> List.exists (fun entry -> entry.Status = Feature161ReadinessStatus.Accepted)
            && summary.ClaimScope.AcceptedLaneId.IsSome
            && summary.FullValidationStatus = "passed"
        then
        ...
        then
            ...
            summary.Entries
            |> List.exists (fun entry -> entry.Status = Feature161ReadinessStatus.Rejected)
        then
    ...
    let updateFeature161 (msg: Feature161Msg) (model: Feature161Model) : Feature161Model * Feature161Effect list =
        let model' =
            ...
            | Feature161PolicyDeclared policyId -> { model with PolicyId = Some policyId }
            | Feature161HostFactsCollected facts -> { model with HostFacts = Some facts }
            ...
            | Feature161PriorGateLinked gate ->
                { model with
                    PriorGates = model.PriorGates @ [ gate ]
                }
    ...
    let feature148ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d148, directory, name)

    let feature149ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d149, directory, name)

    let feature152ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d152, directory, name)

    let feature153ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d153, directory, name)

    let feature154ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d154, directory, name)

    let feature155ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d155, directory, name)

    let feature156ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d156, directory, name)

    let feature157ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d157, directory, name)

    let feature158ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d158, directory, name)

    let feature159ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d159, directory, name)

    let feature160ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d160, directory, name)

    let feature161ArtifactPath directory name =
        Path.Combine(FeatureCatalog.FeatureDescriptor.readinessDirectory d161, directory, name)

    let feature156ScenarioFileName (scenarioId: string) = scenarioId.Replace("/", "-") + ".md"
    ...
    let feature158OverallStatus (summary: Feature158TimingSummary) =
        ...
        | None ->
            feature158StatusFromReports
                ...
                (summary.Diagnostics
                 @ [
                     if summary.Status = Feature158ReadinessStatus.EnvironmentLimited then
                         "environment-limited"
                 ])
