...
module Render3 =
    ...
    let emitFeature156TimingSummary (summary: Feature156TimingSummary) =
        ...
        let scale =
            summary.HostProfile.Scale |> Option.map string |> Option.defaultValue "unknown"
        ...
        let rows =
            if List.isEmpty summary.ScenarioReports then
                ...
                |> List.map (fun report ->
                    let fullP50 =
                        report.FullRedraw
                        |> Option.map (fun d -> feature156FormatMs d.P50Ms)
                        |> Option.defaultValue "missing"

                    let fullP95 =
                        report.FullRedraw
                        |> Option.map (fun d -> feature156FormatMs d.P95Ms)
                        |> Option.defaultValue "missing"

                    let fullP99 =
                        report.FullRedraw
                        |> Option.map (fun d -> feature156FormatMs d.P99Ms)
                        |> Option.defaultValue "missing"
                    ...
                    let artifact =
                        ...
                        |> Option.defaultValue (
                            Path.Combine("scenarios", feature156ScenarioFileName report.ScenarioId)
                        )
        ...
        let rejectionReasons =
            ...
            |> List.collect (fun report ->
                ...
                |> List.map (fun reason -> $"{report.ScenarioId}: {reason}"))
        ...
        String.concat
            ...
            [
                ...
                "## Rejection Reasons"
                ""
                if List.isEmpty rejectionReasons then
                    "- none"
                else
                    rejectionReasons |> List.map (sprintf "- %s") |> String.concat "\n"
                ""
                "## Overhead Disclosure"
                ""
                ...
                "## Diagnostics"
                ""
                if List.isEmpty summary.Diagnostics then
                    "- none"
                else
                    summary.Diagnostics |> List.map (sprintf "- %s") |> String.concat "\n"
                ""
            ]
    ...
    let emitFeature157AttemptReport (attempt: Feature157DamageAttempt) =
        let fallback = attempt.FallbackReason |> Option.defaultValue "none"

        let diagnostics =
            if List.isEmpty attempt.Diagnostics then
                "- none"
            else
                attempt.Diagnostics |> List.map (sprintf "- %s") |> String.concat "\n"
    ...
    let emitFeature157FallbackReport (fallback: Feature157Fallback) =
        let diagnostics =
            if List.isEmpty fallback.Diagnostics then
                "- none"
            else
                fallback.Diagnostics |> List.map (sprintf "- %s") |> String.concat "\n"
    ...
    let emitFeature158ScenarioReport (report: Feature158ScenarioReport) =
        let diagnostics =
            if List.isEmpty report.Diagnostics then
                ...
                report.Diagnostics |> List.map (sprintf "- %s") |> String.concat "\n"
        ...
        | reports ->
            ...
            |> List.map (fun report ->
                let artifact =
                    ...
                    |> List.tryFind (fun path -> path.EndsWith(".md", StringComparison.OrdinalIgnoreCase))
                    |> Option.defaultValue (
                        Path.Combine("scenarios", feature158ScenarioFileName report.ScenarioId).Replace('\\', '/')
                    )
    ...
    let feature158ExcludedReasons (summary: Feature158TimingSummary) =
        let excluded =
            summary.ExcludedSamples |> List.choose _.ExclusionReason |> List.countBy id
        ...
        | attempts ->
            ...
            |> List.map (fun attempt ->
                ...
                let artifact =
                    ...
                    |> List.tryFind (fun path -> path.EndsWith(".md", StringComparison.OrdinalIgnoreCase))
                    |> Option.defaultValue (
                        Path.Combine("attempts", feature159ScenarioFileName attempt.ScenarioId).Replace('\\', '/')
                    )
    ...
    let emitFeature159PromotionSummary (summary: Feature159Summary) =
        ...
        String.concat
            ...
            [
                ...
                "## Required Scenarios"
                ""
                (summary.RequiredScenarioCoverage
                 |> List.map (sprintf "- `%s`")
                 |> String.concat "\n")
                ""
                "## Diagnostics"
    ...
    let emitFeature159CounterReport (summary: Feature159Summary) =
        ...
        let acceptedPromotionArtifacts =
            summary.Attempts |> List.sumBy _.AcceptedPromotionArtifacts
        ...
        | iterations ->
            ...
            |> List.map (fun iteration ->
                ...
                let artifact =
                    ...
                    |> Option.defaultValue (
                        Path
                            .Combine("throughput", "iterations", feature160IterationFileName iteration.IterationId)
                            .Replace('\\', '/')
                    )
                ...
                let durationMinutes =
                    iteration.ActualDuration.TotalMinutes.ToString("0.###", Globalization.CultureInfo.InvariantCulture)
