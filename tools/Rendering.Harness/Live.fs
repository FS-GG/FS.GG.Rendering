...
module Live =
    ...
    let program =
        let configuration =
            FS.GG.UI.SkiaViewer.Host.Viewer.defaultConfiguration "fsgg-harness-live" { Width = 400; Height = 300 }
    ...
    let launchViewerChild () : int =
        ...
        (try
            ...
            glfw.WindowHint(
                Silk.NET.GLFW.WindowHintContextApi.ContextCreationApi,
                Silk.NET.GLFW.ContextApi.EglContextApi
            )
         with _ ->
             ())
    ...
    let runLive (facts: ProbeFacts) (selfDll: string) (outDir: string) : Evidence.Evidence =
        ...
        | Degradation.Run ->
            ...
            | None ->
                mk
                    RunStatus.Skipped
                    (Some "could not start a nested Xvfb X server for the live tier")
                    []
                    [ "summary.md" ]
            | Some xvfb ->
                    ...
                    | proc ->
                        try
                            ...
                            | None ->
                                mk
                                    RunStatus.Skipped
                                    (Some "viewer window not discovered on the nested X server within timeout")
                                    []
                                    [ "summary.md" ]
                            | Some w ->
                                ...
                                let changed =
                                    ...
                                    && (try
                                            File.ReadAllBytes before <> File.ReadAllBytes after
                                        with _ ->
                                            false)
                                ...
                                if nonBlank && changed then
                                    ...
                                    mk
                                        ...
                                        None
                                        p.AuthoritativeFor
                                        [ "window.png"; "window-after.png"; "summary.md" ]
                                elif nonBlank then
                                    ...
                                    mk
                                        RunStatus.Passed
                                        None
                                        [ "window-creation"; "visibility"; "desktop-screenshot" ]
                                        [ "window.png"; "window-after.png"; "summary.md" ]
                                else
                                    mk
                                        RunStatus.Skipped
                                        (Some "window discovered but capture was blank in the nested server")
                                        [ "window-creation"; "window-discovery" ]
                                        [ "window.png"; "summary.md" ]
                        finally
                            try
                                proc.Kill true
                            with _ ->
    ...
    let overlayVisualLimitation (facts: ProbeFacts) =
        ...
        | _, None -> Some "Feature144 overlay visual proof requires a GL renderer; current probe did not report one."
        | _ -> None
    ...
    let classifyOverlayVisualProofHost (facts: ProbeFacts) : Evidence.HostCapabilityResult =
        ...
        | _ -> common Evidence.HostCapable Evidence.CaptureAvailable "capable"
    ...
    let captureOverlayState
        (facts: ProbeFacts)
        (outDir: string)
        (runId: string)
        (state: Evidence.VisualArtifactState)
...

        {
            ...
            OverlayAboveContent =
                if state = Evidence.OpenOverlay then
                    Some(result.Status = ScreenshotOk)
                else
                    None
            TopmostHitTarget =
                if state = Evidence.OpenOverlay && result.Status = ScreenshotOk then
                    Some scenario.ExpectedTopmostHitTarget
                else
                    None
            NoStaleOverlayPixel =
                if state = Evidence.ClosedOverlay then
                    Some(result.Status = ScreenshotOk)
                else
                    None
        }
        ...
        | Evidence.OpenOverlay ->
            {
                ...
                BehavioralEvidenceReference =
                    "samples/AntShowcase/AntShowcase.Core/Evidence.fs:datePickerReferenceOverlayEvidence"
    ...
    let runOverlayVisualProof (facts: ProbeFacts) (outDir: string) : Evidence.VisualProofRun =
        ...
        let run =
            ...
            | Evidence.HostCapable ->
                ...
                let correlations =
                    [ correlationForArtifact openArtifact; correlationForArtifact closedArtifact ]
            ...
            | Evidence.HostUnsupported ->
                ...
                withoutDecision
                    Evidence.VisualProofEnvironmentLimited
                    None
                    None
                    []
                    Evidence.Environment
                    (Some limitation)
            | Evidence.HostFailed -> withoutDecision Evidence.VisualProofFailed None None [] Evidence.Environment None

        let decision = Evidence.evaluateReadinessCaveat run

        { run with
            ReadinessDecision = Some decision
        }
    ...
    let buildSwapWindow (vsync: bool) : Silk.NET.Windowing.IWindow =
        ...
        opts.API <-
            Silk.NET.Windowing.GraphicsAPI(
                ...
                Silk.NET.Windowing.APIVersion(3, 3)
            )
    ...
    let runFaithfulPerf (facts: ProbeFacts) (selfDll: string) (outDir: string) : Evidence.Evidence * float list =
        ...
        let psi =
            ProcessStartInfo("dotnet", sprintf "%s __vsyncprobe %s %g" selfDll stampFile seconds)
        ...
        let intervalsMs =
            ...
            | proc ->
                ...
                if File.Exists stampFile then
                    ...
                    |> Array.choose (fun l ->
                        match Int64.TryParse(l.Trim()) with
                        | true, v -> Some v
                        | _ -> None)
                    |> Array.pairwise
        ...
        let authoritative =
            if p.VsyncFaithfulAllowed then
                [ "frame-interval"; "vsync-faithful"; "present-cadence" ]
            else
                [ "frame-interval" ]

        let status =
            if List.isEmpty intervalsMs then
                RunStatus.Skipped
            else
                RunStatus.Passed
        ...
        let skip =
            if List.isEmpty intervalsMs then
                Some "no swap intervals captured (the GL swap loop did not present on the GPU display)"
            else
                None
