...
let feature156Performance (rest: string list) =
    if not (isFeature156 rest) then
        ...
        let policy =
            ...
            |> Option.defaultValue Compositor.Config.feature156PolicyId
        ...
        if policy <> Compositor.Config.feature156PolicyId then
            ...
            let expectedProfile =
                ...
                |> Option.defaultValue Compositor.Config.feature156AcceptedProfileId
            ...
            let failClosedReports
                (verdict: Compositor.Types.Feature156ScenarioVerdict)
                reason
                : Compositor.Types.Feature156ScenarioReport list =
                scenarioSet
                |> List.map (fun scenario ->
                    {
                        ...
                        ArtifactPaths =
                            [
                                Path
                                    .Combine("scenarios", Compositor.FeatureState.feature156ScenarioFileName scenario)
                                    .Replace('\\', '/')
                            ]
            ...
            let reports, diagnostics =
                ...
                | NoDisplay, _, _, _ ->
                    ...
                    File.WriteAllText(
                        ...
                        Compositor.Render3.emitFeature156UnsupportedHostReport reason
                    )

                    failClosedReports Compositor.Types.Feature156EnvironmentLimited reason,
                    [ reason; "accepted performance artifacts=0" ]
                | _, None, _, _ ->
                ...
                | _, _, _, false ->
                    let reason =
                        $"profile mismatch: expected={expectedProfile} actual={profile.ProfileId}"
            ...
            let summary =
                feature156SummaryFromReports runId profile warmup repetitions reports diagnostics
...
let feature158Performance (rest: string list) =
    let policy =
        flagValue "--policy" rest
        |> Option.defaultValue Compositor.Config.feature158PolicyId
    ...
    if policy <> Compositor.Config.feature158PolicyId then
        ...
        let expectedProfile =
            flagValue "--profile" rest
            |> Option.defaultValue Compositor.Config.feature158AcceptedProfileId
        ...
        let reports, proofProbeEvidence, unsupported, diagnostics =
            ...
            | true, Some reason ->
                ...
                [
                    reason
                    "explicit probe readback unavailable"
            ...
            | true, None ->
                ...
                let reports: Compositor.Types.Feature158ScenarioReport list =
                    ...
                    |> List.map (fun scenario ->
                        let samples =
                            probeSamples |> List.filter (fun sample -> sample.ScenarioId = scenario)
                        ...
                        feature158ScenarioReport
                            ...
                            [
                                Path
                                    .Combine("scenarios", Compositor.FeatureState.feature158ScenarioFileName scenario)
                                    .Replace('\\', '/')
                            ]
                            [
                                "probe-readback-included"
                                "probe-run-excluded"
                                "accepted performance samples=0"
                            ])
            ...
            | false, None ->
                let reports: Compositor.Types.Feature158ScenarioReport list =
                    ...
                    |> List.map (fun scenario ->
                        ...
                        let included =
                            samples |> List.filter (fun sample -> sample.InclusionStatus = Perf.Included)
...
let feature160ScenarioSet (rest: string list) =
    ...
    | Some scenario when Compositor.Config.feature160RequiredScenarioIds |> List.contains scenario ->
        [ scenario ], Some scenario, None
    | Some scenario when Compositor.Config.feature160ScenarioIds |> List.contains scenario ->
        [ scenario ], Some scenario, None
            ...
            [
                Path
                    .Combine(
                        "throughput",
                        "iterations",
                        Compositor.FeatureState.feature160IterationFileName iterationId
                    )
                    .Replace('\\', '/')
...
let writeFeature160ThroughputPackage out (summary: Compositor.Types.Feature160ThroughputSummary) =
    ...
    for iteration in summary.Iterations do
        File.WriteAllText(
            Path.Combine(iterationsDir, Compositor.FeatureState.feature160IterationFileName iteration.IterationId),
            Compositor.Render4.emitFeature160IterationReport iteration
        )
    ...
    |> List.iter (fun (reason, iterations) ->
        if iterations |> List.exists (fun iteration -> iteration.ExclusionReason.IsSome) then
            File.WriteAllText(
                ...
                Compositor.Render4.emitFeature160ExcludedEvidenceReport reason iterations
            ))
    ...
    let unsupportedReport =
        Compositor.Render4.emitFeature160UnsupportedHostReport (
    ...
    if
        ...
        || String.Equals(
            Path.GetFileName(out.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)),
            "unsupported",
    ...
    File.WriteAllText(
        ...
        Compositor.Render4.emitFeature160ThroughputSummaryJson summary
        + Environment.NewLine
    )
...
let feature160Performance (rest: string list) =
    let lane =
        flagValue "--lane" rest
        |> Option.defaultValue Compositor.Config.feature160FocusedLaneId

    let policy =
        flagValue "--policy" rest
        |> Option.defaultValue Compositor.Config.feature160PolicyId
    ...
    if lane <> Compositor.Config.feature160FocusedLaneId then
        ...
        | None ->
            ...
            let expectedProfile =
                flagValue "--profile" rest
                |> Option.defaultValue Compositor.Config.feature160AcceptedProfileId
                            ...
                            let reports: Compositor.Types.Feature158ScenarioReport list =
                                ...
                                |> List.map (fun scenario ->
                                    ...
                                    feature158ScenarioReport
                                        ...
                                        [
                                            Path
                                                .Combine("throughput", "iterations", attemptRunId + ".md")
                                                .Replace('\\', '/')
                                            fullCsv
                                            fullJson
                                            damageCsv
                                        ...
                                        [
                                            "measurement-policy=focused-throughput-v1"
                                            "readback-free timing policy preserved from Feature 158"
                                        ])

                            let status, reason =
                                if restrictedScenario.IsSome then
                                    Compositor.Types.Feature160ReadinessStatus.Rejected,
                                    Some Perf.ScenarioCoverageMissing
                                elif
                                    reports
            ...
            let unsupportedReason =
                ...
                |> Option.orElse (
                    if
                        iterations
                        |> List.exists (fun iteration ->
                            iteration.Status = Compositor.Types.Feature160ReadinessStatus.EnvironmentLimited)
                    then
...
let feature161HostFactsFromProbe
...

    {
        ...
        PackageVersionSet =
            "Rendering.Harness=local;FS.GG.UI.Testing=local;source-throughput="
...
let feature161EntryFromFacts
    ...
    let status =
        match reason with
        | None when laneId = Compositor.Config.feature161HostLaneId ->
            Compositor.Types.Feature161ReadinessStatus.Accepted
    ...
    let acceptedArtifacts =
        if status = Compositor.Types.Feature161ReadinessStatus.Accepted then
            1
        else
            0
...

    {
        ...
        ArtifactPaths =
            [
                Path
                    .Combine("lane-ledger", "entries", Compositor.FeatureState.feature161LedgerEntryFileName entryId)
                    .Replace('\\', '/')
                Path
                    .Combine("lane-ledger", "host-facts", Compositor.FeatureState.feature161HostFactsFileName entryId)
                    .Replace('\\', '/')
            ]
        Diagnostics =
            [
...
let writeFeature161LaneLedgerPackage out (summary: Compositor.Types.Feature161Summary) =
    ...
    for entry in summary.Entries do
        File.WriteAllText(
            Path.Combine(entriesDir, Compositor.FeatureState.feature161LedgerEntryFileName entry.EntryId),
            Compositor.Render4.emitFeature161LedgerEntry entry
        )

        File.WriteAllText(
            Path.Combine(hostFactsDir, Compositor.FeatureState.feature161HostFactsFileName entry.EntryId),
            Compositor.Render4.emitFeature161HostFacts entry.HostFacts
        )
    ...
    |> List.iter (fun (reason, entries) ->
        if entries |> List.exists (fun entry -> entry.PrimaryExclusionReason.IsSome) then
            File.WriteAllText(
                Path.Combine(excludedDir, Perf.exclusionReasonToken reason + ".md"),
                Compositor.Render4.emitFeature161ExcludedEvidenceReport reason entries
            ))

    if not (File.Exists(Path.Combine(excludedDir, "README.md"))) then
        File.WriteAllText(
            Path.Combine(excludedDir, "README.md"),
            "# Feature 161 Excluded Evidence\n\nGrouped excluded lane entries are written by primary reason.\n"
        )

    let unsupportedReport =
        Compositor.Render4.emitFeature161UnsupportedHostReport (
            summary.UnsupportedHostReason
            |> Option.defaultValue "not run in this invocation"
        )
    ...
    File.WriteAllText(
        Path.Combine(out, "summary.json"),
        Compositor.Render4.emitFeature161LaneLedgerSummaryJson summary
        + Environment.NewLine
    )
...
let feature161Performance (rest: string list) =
    ...
    let policy =
        flagValue "--policy" rest
        |> Option.defaultValue Compositor.Config.feature161PolicyId
    ...
    if lane <> "host-ledger" then
        ...
        let expectedProfile =
            flagValue "--profile" rest
            |> Option.defaultValue Compositor.Config.feature161AcceptedProfileId

        let scenarioId =
            flagValue "--scenario" rest |> Option.defaultValue "timing/host-lane-ledger"

        let runId = "feature161-" + DateTime.UtcNow.ToString("yyyyMMddHHmmss")

        let hostFacts =
            feature161HostFactsFromProbe runId scenarioId sourceThroughput facts profile
