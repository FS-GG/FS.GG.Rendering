...
module Render4 =
    let emitFeature160IterationReport (iteration: Feature160Iteration) =
        ...
        let diagnostics =
            if List.isEmpty iteration.Diagnostics then
                "- none"
            else
                iteration.Diagnostics |> List.map (sprintf "- %s") |> String.concat "\n"
        ...
        let rows =
            ...
            | xs ->
                ...
                |> List.map (fun iteration ->
                    ...
                    let durationMinutes =
                        iteration.ActualDuration.TotalMinutes.ToString(
                            "0.###",
    ...
    let emitFeature160ThroughputSummary (summary: Feature160ThroughputSummary) =
        ...
        let acceptedCount =
            summary.Iterations |> List.filter feature160IterationAccepted |> List.length

        let excludedCount =
            summary.Iterations
            |> List.filter (fun iteration -> iteration.ExclusionReason.IsSome)
            |> List.length
        ...
        let releaseStatus =
            if overallStatus = Feature160ReadinessStatus.Accepted then
                "ready"
            else
                "blocked"
    ...
    let emitFeature160FullValidationRecord record =
        ...
        | Some validation ->
            let started =
                validation.StartedAt |> Option.map string |> Option.defaultValue "unknown"

            let completed =
                validation.CompletedAt |> Option.map string |> Option.defaultValue "unknown"
            ...
            String.concat
                ...
                [
                    ...
                    "## Diagnostics"
                    ""
                    if List.isEmpty validation.Diagnostics then
                        "- none"
                    else
                        validation.Diagnostics |> List.map (sprintf "- %s") |> String.concat "\n"
                    ""
                ]
    ...
    let emitFeature161LedgerEntry (entry: Feature161LedgerEntry) =
        ...
        let diagnostics =
            if List.isEmpty entry.Diagnostics then
                "- none"
            else
                entry.Diagnostics |> List.map (sprintf "- %s") |> String.concat "\n"
        ...
        | entries ->
            ...
            |> List.map (fun entry ->
                ...
                let artifact =
                    ...
                    |> Option.defaultValue (
                        Path
                            .Combine("lane-ledger", "entries", feature161LedgerEntryFileName entry.EntryId)
                            .Replace('\\', '/')
                    )
    ...
    let emitFeature161LaneLedgerSummary (summary: Feature161Summary) =
        ...
        let acceptedCount =
            summary.Entries |> List.filter feature161LedgerEntryAccepted |> List.length

        let excludedCount =
            summary.Entries
            |> List.filter (fun entry -> entry.PrimaryExclusionReason.IsSome)
            |> List.length
        ...
        String.concat
            ...
            [
                ...
                ""
                (summary.ClaimScope.NonGeneralizedLanes
                 |> List.map (sprintf "- %s")
                 |> String.concat "\n")
                ""
                "## Remaining Blockers"
    ...
    let emitFeature161ValidationSummary (summary: Feature161Summary) =
        ...
        String.concat
            ...
            [
                ...
                "## Remaining Blockers"
                ""
                (if List.isEmpty summary.ClaimScope.RemainingBlockers then
                     "- none"
                 else
                     summary.ClaimScope.RemainingBlockers
                     |> List.map (sprintf "- %s")
                     |> String.concat "\n")
                ""
            ]
