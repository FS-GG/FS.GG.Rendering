...
module Evidence =
    ...
    let feature144DatePickerProofScenario =
        {
            ...
            ExpectedDispatchSummary =
                "DatePickerOpenChanged:true; DatePickerChanged:2026-06-17; DatePickerOpenChanged:false"
        }
    ...
    let visualArtifactRelativePath runId state =
        (Path.Combine("artifacts", safeSegment runId, sprintf "%s.png" (artifactStateToken state))).Replace('\\', '/')
    ...
    let strList (xs: string list) =
        "[" + String.Join(", ", xs |> List.map q) + "]"

    let optStr (o: string option) =
        match o with
        | Some s -> q s
        | None -> "null"

    let optNum (o: float option) =
        match o with
        | Some v -> v.ToString("0.###", CultureInfo.InvariantCulture)
        | None -> "null"

    let optInt (o: int option) =
        match o with
        | Some v -> string v
        | None -> "null"
    ...
    let toJson (evidence: Evidence) =
        ...
        line (
            sprintf
                "    \"gl\": { \"renderer\": %s, \"version\": %s, \"direct\": %s },"
                (optStr f.GlRenderer)
                (optStr f.GlVersion)
                (boolStr f.GlDirect)
        )
        ...
        line (
            sprintf
                "  \"present\": { \"swapControl\": %s, \"vblankSource\": %s },"
                (optInt f.SwapControl)
                (optStr f.VblankSource)
        )

        line (
            sprintf
                "  \"metrics\": { \"frames\": %d, \"p50Ms\": %s, \"p95Ms\": %s, \"p99Ms\": %s },"
                e.Frames
                (optNum e.P50Ms)
                (optNum e.P95Ms)
                (optNum e.P99Ms)
    ...
    let metricsCsv (frameMs: float list) =
        ...
        frameMs
        |> List.iteri (fun i ms ->
            sb.AppendLine(sprintf "%d,%s" i (ms.ToString("0.###", CultureInfo.InvariantCulture)))
            |> ignore)
    ...
    let validateOverlayVisualCorrelation
        ...
        | _ -> mark OverlayBehavior "correlation expected state disagrees with artifact state"
    ...
    let renderCorrelation (run: VisualProofRun) =
        ...
        if run.Correlations.IsEmpty then
            ...
            |> List.iter (fun correlation ->
                ...
                line (
                    sprintf "- expected overlay state: %s" (expectedOverlayStateToken correlation.ExpectedOverlayState)
                )
    ...
    let renderVisualProofRun (run: VisualProofRun) =
        ...
        line ""
        line "## Artifacts"
        let artifacts = [ run.OpenArtifact; run.ClosedArtifact ] |> List.choose id
        ...
        if artifacts.IsEmpty then
            ...
            |> List.iter (fun artifact ->
                line (
                    sprintf
                        "- %s: %s (%dx%d, %s)"
                        (artifactStateToken artifact.State)
                        artifact.Path
                        artifact.Width
                        artifact.Height
                        (pixelContentToken artifact.PixelContentValidation)
                ))
