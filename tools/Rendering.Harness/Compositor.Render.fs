...
module Render =
    ...
    let renderValidationSummary model =
        let tierRows =
            ...
            |> List.map (fun tier ->
                let verdict =
                    model.TierVerdicts
                    |> Map.tryFind tier
                    |> Option.defaultValue (Limited "not evaluated")
        ...
        let proofRows =
            ...
            | proofs ->
                proofs
                |> List.map (fun proof ->
                    $"| `{proof.ProofId}` | `{proof.ScenarioId}` | {proofVerdictToken proof.Verdict} | `{proof.HostProfile.ProfileId}` |")
                |> String.concat "\n"
        ...
        String.concat
            ...
            [
                ...
                "## Diagnostics"
                ""
                if List.isEmpty model.Diagnostics then
                    "- none"
                else
                    model.Diagnostics |> List.map (sprintf "- %s") |> String.concat "\n"
                ""
            ]
    ...
    let emitFeature148ParityReport () =
        let rows =
            ...
            |> List.map (fun scenario ->
                let verdict =
                    if scenario = "damage/parity-failure" then
                        "rejected-sample"
                    elif scenario = "damage/unsupported" then
                        "environment-limited"
                    else
                        "passed-policy"
    ...
    let emitFeature148SnapshotReport () =
        String.concat
            ...
            [
                ...
                sprintf
                    "Ready threshold: `%g%%` improvement over replay/lower-tier baseline."
                    thresholds.SnapshotImprovementPercent
                ""
            ]

    let emitFeature148TimingReport tier =
        let normalized =
            if feature148TimingTiers |> List.contains tier then
                tier
            else
                "damage"
    ...
    let emitFeature148ValidationSummary model =
        let tierRows =
            ...
            |> List.map (fun (tier, name) ->
                ...
                let verdict =
                    model.TierVerdicts |> Map.tryFind tier |> Option.defaultValue defaultVerdict
        ...
        let proofRows =
            ...
            | proofs ->
                proofs
                |> List.map (fun proof ->
                    $"| `{proof.ProofId}` | {proofVerdictToken proof.Verdict} | `{proof.HostProfile.ProfileId}` |")
                |> String.concat "\n"
    ...
    let emitFeature149TimingReport tier =
        let normalized =
            if feature149TimingTiers |> List.contains tier then
                tier
            else
                "damage"
    ...
    let emitFeature152TimingReport tier =
        let normalized =
            if feature152TimingTiers |> List.contains tier then
                tier
            else
                "damage"
    ...
    let renderValidationDoc (featureNum: int) (kind: string) (status: string) (body: string list) =
        String.concat
            "\n"
            ([ $"# Feature {featureNum} {kind}"; ""; $"Status: `{status}`"; "" ]
             @ body
             @ [ "" ])
    ...
    let renderPackageValidation (featureNum: int) (validationLines: string list) =
        ...
        | Some hook ->
            ...
            renderValidationDoc
                ...
                ([ checksHeader; ""; validationRunsBlock validationLines; ""; surfaceHeader; "" ]
                 @ surfaceBullets)
        ...
        | Some hook ->
            ...
            renderValidationDoc
                ...
                ([
                    "## Validation Runs"
                    ""
                    validationRunsBlock validationLines
                    ""
                    sectionHeader
                    ""
                 ]
