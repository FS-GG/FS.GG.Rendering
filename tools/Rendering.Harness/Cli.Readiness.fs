...
let legacyCompositorReadiness (rest: string list) =
    ...
    let out =
        ...
        | Some d -> d
        | None when isFeature155 rest ->
            FeatureCatalog.FeatureDescriptor.readinessDirectory (FeatureCatalog.descriptorById 155)
        | None when isFeature154 rest ->
            FeatureCatalog.FeatureDescriptor.readinessDirectory (FeatureCatalog.descriptorById 154)
        | None -> outDir rest
    ...
    let proofTier =
        Compositor.Config.validateProofForScissoring profile now (TimeSpan.FromHours 24.0) (Some proof)

    let damageTier =
        Compositor.Config.evaluateTier proofTier (Some Compositor.Types.ParityPassed) None

    let model0, _ = Compositor.FeatureState.initReadiness ()

    let model1, _ =
        Compositor.FeatureState.updateReadiness (Compositor.Types.ProofLoaded proof) model0

    let model2, _ =
        Compositor.FeatureState.updateReadiness
            (Compositor.Types.ParityRecorded("damage/localized-update", Compositor.Types.ParityPassed))
            model1

    let model3, _ =
        Compositor.FeatureState.updateReadiness
            (Compositor.Types.TierEvaluated(Compositor.Types.PresentProofTier, proofTier))
            model2

    let model4, _ =
        Compositor.FeatureState.updateReadiness
            (Compositor.Types.TierEvaluated(Compositor.Types.DamageScissorTier, damageTier))
            model3

    let model5, _ =
        Compositor.FeatureState.updateReadiness
            (Compositor.Types.TierEvaluated(
                Compositor.Types.SnapshotTier,
                Compositor.Types.Skipped "no capable host timing run"
            ))
            model4
    ...
    if isFeature155 rest then
        ...
        let modelFromActualProofs =
            if List.isEmpty actualProofs then
                ...
                actualProofs
                |> List.fold
                    (fun state loadedProof ->
                        Compositor.FeatureState.updateReadiness (Compositor.Types.ProofLoaded loadedProof) state
                        |> fst)
                    model0
        ...
        let model8, _ =
            Compositor.FeatureState.updateReadiness
                (Compositor.Types.TierEvaluated(
                    Compositor.Types.PlacementReuseTier,
                    Compositor.Types.Skipped "context-only without same-profile live timing"
                ))
                model7
        ...
        let model10, _ =
            Compositor.FeatureState.updateReadiness
                (Compositor.Types.SnapshotTier
                 |> fun tier ->
                     Compositor.Types.TierEvaluated(tier, Compositor.Types.Limited "no accepted performance claim"))
                model9
        ...
        IO.File.WriteAllText(
            IO.Path.Combine(out, "package-validation.md"),
            Compositor.Render2.emitFeature155PackageValidation ()
        )

        IO.File.WriteAllText(
            IO.Path.Combine(out, "regression-validation.md"),
            Compositor.Render2.emitFeature155RegressionValidation ()
        )

        IO.Directory.CreateDirectory(IO.Path.Combine(out, "parity")) |> ignore

        IO.File.WriteAllText(
            IO.Path.Combine(out, "parity", "README.md"),
        ...
        IO.File.WriteAllText(
            IO.Path.Combine(out, "timing", "timing-damage.md"),
            Compositor.Render2.emitFeature155TimingReport "damage" 5 5
        )
        ...
        let model8, _ =
            Compositor.FeatureState.updateReadiness
                (Compositor.Types.TierEvaluated(
                    Compositor.Types.SnapshotTier,
                    Compositor.Types.Limited "no same-profile capable-host timing run"
                ))
                model7
        ...
        IO.File.WriteAllText(
            IO.Path.Combine(out, "package-validation.md"),
            Compositor.Render2.emitFeature154PackageValidation ()
        )

        IO.File.WriteAllText(
            IO.Path.Combine(out, "regression-validation.md"),
            Compositor.Render2.emitFeature154RegressionValidation ()
        )
        ...
        IO.File.WriteAllText(
            IO.Path.Combine(out, "timing", "timing-damage.md"),
            Compositor.Render2.emitFeature154TimingReport "damage" 5 5
        )
        ...
        IO.File.WriteAllText(
            IO.Path.Combine(out, "package-validation.md"),
            Compositor.Render.emitFeature153PackageValidation ()
        )

        IO.File.WriteAllText(
            IO.Path.Combine(out, "regression-validation.md"),
            Compositor.Render.emitFeature153RegressionValidation ()
        )
        ...
        let model6, _ =
            Compositor.FeatureState.updateReadiness
                (Compositor.Types.TierEvaluated(Compositor.Types.PlacementReuseTier, Compositor.Types.Ready))
                model5

        let model7, _ =
            Compositor.FeatureState.updateReadiness
                (Compositor.Types.TierEvaluated(Compositor.Types.ReplayTier, Compositor.Types.Ready))
                model6
        ...
        let model8, _ =
            Compositor.FeatureState.updateReadiness
                (Compositor.Types.TierEvaluated(
                    Compositor.Types.SnapshotTier,
                    Compositor.Types.Limited "no capable-host snapshot timing run"
                ))
                model7
...
let feature156Readiness (rest: string list) =
                    ...
                    [
                        Path
                            .Combine(
                                "timing",
                                "scenarios",
                                Compositor.FeatureState.feature156ScenarioFileName scenario
                            )
                            .Replace('\\', '/')
                    ]
    ...
    let validationSummary =
        ...
        | Some verdict ->
            { summary with
                ...
                Diagnostics =
                    summary.Diagnostics
                    @ [ "readiness package reflected existing timing summary verdict" ]
            }
        | None -> summary
    ...
    for report in reports do
        let path =
            Path.Combine(scenariosDir, Compositor.FeatureState.feature156ScenarioFileName report.ScenarioId)
    ...
    if not (File.Exists unsupportedPath) then
        File.WriteAllText(
            unsupportedPath,
            Compositor.Render3.emitFeature156UnsupportedHostReport "not run in this readiness invocation"
        )

    File.WriteAllText(
        Path.Combine(out, "compatibility-ledger.md"),
        Compositor.Render3.emitFeature156CompatibilityLedger ()
    )

    File.WriteAllText(
        Path.Combine(out, "package-validation.md"),
        Compositor.Render.renderPackageValidation 156 [ "`compositor-readiness --feature 156`: package assembled." ]
    )

    File.WriteAllText(
        Path.Combine(out, "regression-validation.md"),
        Compositor.Render.renderRegressionValidation 156 [ "`compositor-readiness --feature 156`: package assembled." ]
    )

    File.WriteAllText(
        Path.Combine(out, "validation-summary.md"),
        Compositor.Render3.emitFeature156ValidationSummary validationSummary
    )
...
let ensureFeature157FsiEvidence out =
    ...
    if not (File.Exists damageLog) then
        File.WriteAllText(
            damageLog,
            "Feature157 damage authoring type-checks against SkiaViewer and Testing damage-readiness helpers.\n"
        )
    ...
    if not (File.Exists readinessLog) then
        File.WriteAllText(
            readinessLog,
            "Feature157 readiness authoring validates accepted, fallback-only, rejected, and environment-limited package shapes.\n"
        )
...
let feature157Readiness (rest: string list) =
    ...
    if not (File.Exists(Path.Combine(damageOut, "summary.md"))) then
        runCompositorDamageCmd ("--feature" :: "157" :: "--out" :: damageOut :: rest)
        |> ignore
    ...
    let reason =
        feature157ReasonFromHost facts profile Compositor.Config.feature157AcceptedProfileId
        ...
        |> Option.map (fun r ->
            ...
            |> List.map (fun scenario ->
                feature157Fallback
                    ...
                    r
                    (Path
                        .Combine("damage", "fallbacks", Compositor.FeatureState.feature157ScenarioFileName scenario)
                        .Replace('\\', '/'))))
        |> Option.defaultValue []
    ...
    File.WriteAllText(
        Path.Combine(out, "compatibility-ledger.md"),
        Compositor.Render3.emitFeature157CompatibilityLedger ()
    )

    File.WriteAllText(
        Path.Combine(out, "package-validation.md"),
        Compositor.Render.renderPackageValidation 157 [ "`compositor-readiness --feature 157`: package assembled." ]
    )

    File.WriteAllText(
        Path.Combine(out, "regression-validation.md"),
        Compositor.Render.renderRegressionValidation 157 [ "`compositor-readiness --feature 157`: package assembled." ]
    )

    File.WriteAllText(
        Path.Combine(out, "validation-summary.md"),
        Compositor.Render3.emitFeature157ValidationSummary summary
    )
...
let ensureFeature158FsiEvidence out =
    ...
    File.WriteAllText(
        performanceLog,
        "Feature158 compositor-performance authoring PASS: command policy and probe exclusion tokens are stable.\n"
    )
    ...
    File.WriteAllText(
        readinessLog,
        "Feature158 compositor-readiness authoring PASS: no Testing or SkiaViewer helper surface added.\n"
    )
...
let feature158Readiness (rest: string list) =
    ...
    let reason =
        feature158ReasonFromHost facts profile Compositor.Config.feature158AcceptedProfileId
    ...
    let unsupportedReason, scenarioReports, runId, warmup, repetitions, feature156Comparison, performanceClaim =
        ...
        | Some reason ->
            ...
            Some reason,
            reports,
            "feature158-readiness-" + DateTime.UtcNow.ToString("yyyyMMddHHmmss"),
            1,
            1,
            "contextualizes",
                            ...
                            [
                                Path
                                    .Combine(
                                        "timing",
                                        "scenarios",
                                        Compositor.FeatureState.feature158ScenarioFileName scenario
                                    )
                                    .Replace('\\', '/')
                            ]
    ...
    let summary =
        feature158SummaryFromReports
            ...
            proofProbeEvidence
            (unsupportedReason
             |> Option.orElse (timingSnapshot |> Option.bind _.UnsupportedHostReason))
            [
                "readiness package assembled"
    ...
    File.WriteAllText(
        Path.Combine(proofProbeDir, "README.md"),
        Compositor.Render3.emitFeature158ProofProbeReport summary.ProofProbeEvidence
    )

    File.WriteAllText(
        Path.Combine(out, "compatibility-ledger.md"),
        Compositor.Render3.emitFeature158CompatibilityLedger ()
    )

    File.WriteAllText(
        Path.Combine(out, "package-validation.md"),
        Compositor.Render.renderPackageValidation 158 [ "`compositor-readiness --feature 158`: package assembled." ]
    )

    File.WriteAllText(
        Path.Combine(out, "regression-validation.md"),
        Compositor.Render.renderRegressionValidation 158 [ "`compositor-readiness --feature 158`: package assembled." ]
    )

    File.WriteAllText(
        Path.Combine(out, "validation-summary.md"),
        Compositor.Render3.emitFeature158ValidationSummary summary
    )

    if not (File.Exists(Path.Combine(unsupportedDir, "README.md"))) then
        File.WriteAllText(
            Path.Combine(unsupportedDir, "README.md"),
            Compositor.Render3.emitFeature158UnsupportedHostReport (
                unsupportedReason |> Option.defaultValue "not run in this invocation"
            )
        )

    if not (File.Exists(Path.Combine(unsupportedDir, "validation.md"))) then
        File.WriteAllText(
            Path.Combine(unsupportedDir, "validation.md"),
            Compositor.Render3.emitFeature158UnsupportedHostReport (
                unsupportedReason |> Option.defaultValue "not run in this invocation"
            )
        )
    ...
    File.WriteAllText(
        Path.Combine(surfaceDir, "FS.GG.UI.Testing.txt"),
        "No Feature 158 public Testing surface drift.\n"
    )

    File.WriteAllText(
        Path.Combine(surfaceDir, "FS.GG.UI.SkiaViewer.txt"),
        "No Feature 158 public SkiaViewer surface drift.\n"
    )
...
let feature159Attempt
    runId
    (index: int)
    (scenario: string)
    (profile: Compositor.Types.HostProfile)
    : Compositor.Types.Feature159Attempt =
    let promotion, reuse, reason, net, acceptedReuse, acceptedPromotion, parity =
        ...
        | "promotion/fallback-safe" ->
            "fallback-only", "fallback-full-redraw", Some "missing-retained-content", 0, 0, 0, "passed"
...

    {
        ...
        Diagnostics =
            [
                ...
                if reuse = "content-reused-placement-updated" then
                    "old and new placement coverage recorded"
                if reuse = "content-re-recorded" then
                    "content identity changed; stale content was not reused"
                match reason with
...
let feature159EnvironmentLimitedAttempt
    runId
    (index: int)
    (scenario: string)
    (profile: Compositor.Types.HostProfile)
...
let writeFeature159PromotionPackage out (summary: Compositor.Types.Feature159Summary) =
    ...
    File.WriteAllText(
        Path.Combine(demotionsDir, "validation.md"),
        Compositor.Render3.emitFeature159PromotionSummary summary
    )

    File.WriteAllText(
        Path.Combine(fallbacksDir, "validation.md"),
        Compositor.Render3.emitFeature159PromotionSummary summary
    )

    let unsupportedReason =
        summary.UnsupportedHostReason
        |> Option.defaultValue "not run in this invocation"

    let unsupportedReport =
        Compositor.Render3.emitFeature159UnsupportedHostReport unsupportedReason
    ...
    if
        summary.UnsupportedHostReason.IsSome
        || String.Equals(outDirectoryName, "unsupported", StringComparison.OrdinalIgnoreCase)
    then
        File.WriteAllText(Path.Combine(out, "validation.md"), unsupportedReport)

let feature159Promotion (rest: string list) =
    let policy =
        flagValue "--policy" rest
        |> Option.defaultValue Compositor.Config.feature159PolicyId
    ...
    if policy <> Compositor.Config.feature159PolicyId then
        ...
        let reason =
            feature158ReasonFromHost facts profile Compositor.Config.feature159AcceptedProfileId
        ...
        let attempts =
            ...
            | Some reason ->
                ...
                |> List.mapi (fun index scenario ->
                    feature159EnvironmentLimitedAttempt runId (index + 1) scenario profile reason)
            | None ->
                [
...
let ensureFeature159FsiEvidence out =
    ...
    File.WriteAllText(
        identityLog,
        "Feature159 content/placement identity authoring PASS: split identity tokens are stable.\n"
    )
    ...
    File.WriteAllText(
        promotionLog,
        "Feature159 compositor-promotion authoring PASS: policy id and required scenarios are stable.\n"
    )
    ...
    File.WriteAllText(
        readinessLog,
        "Feature159 compositor-readiness authoring PASS: Testing readiness helper surface is stable.\n"
    )

    File.WriteAllText(
        Path.Combine(fsiDir, "FS.GG.UI.Controls.txt"),
        "No Feature 159 public Controls package surface drift; retained helpers are internal.\n"
    )

    File.WriteAllText(
        Path.Combine(fsiDir, "FS.GG.UI.SkiaViewer.txt"),
        "No Feature 159 public SkiaViewer package surface drift; replay diagnostics are internal.\n"
    )

    File.WriteAllText(
        Path.Combine(fsiDir, "FS.GG.UI.Testing.txt"),
        "Feature159Readiness helper surface added for package validation.\n"
    )
...
let feature159Readiness (rest: string list) =
    ...
    File.WriteAllText(
        Path.Combine(out, "compatibility-ledger.md"),
        Compositor.Render3.emitFeature159CompatibilityLedger ()
    )

    File.WriteAllText(
        Path.Combine(out, "package-validation.md"),
        Compositor.Render.renderPackageValidation 159 [ "`compositor-readiness --feature 159`: package assembled." ]
    )

    File.WriteAllText(
        Path.Combine(out, "regression-validation.md"),
        Compositor.Render.renderRegressionValidation 159 [ "`compositor-readiness --feature 159`: package assembled." ]
    )

    File.WriteAllText(
        Path.Combine(out, "validation-summary.md"),
        Compositor.Render3.emitFeature159ValidationSummary summary
    )
...
let ensureFeature160FsiEvidence out =
    ...
    let readinessFsi =
        Path.Combine(fsiDir, "feature160-throughput-readiness-authoring.fsx")

    let readinessLog =
        Path.Combine(fsiDir, "feature160-throughput-readiness-authoring.log")
    ...
    File.WriteAllText(
        performanceLog,
        "Feature160 compositor-performance authoring PASS: focused lane, bound, scenarios, and exclusion tokens are stable.\n"
    )
    ...
    File.WriteAllText(
        readinessLog,
        "Feature160 throughput readiness authoring PASS: helper validates accepted, blocked, rejected, fallback-only, and environment-limited packages.\n"
    )

    File.WriteAllText(
        testingSurface,
        "FS.GG.UI.Testing exposes Feature160ThroughputReadiness additive helper records and status tokens.\n"
    )

    File.WriteAllText(
        harnessSurface,
        "Rendering.Harness.Compositor exposes Feature 160 focused-lane command, MVU, iteration, full-validation, and readiness signatures.\n"
    )
...
let feature160FullValidationRecordFromFile validationPath =
    if not (File.Exists validationPath) then
        ...
        let status =
            if text.Contains("Status: `passed`", StringComparison.OrdinalIgnoreCase) then
                "passed"
            elif text.Contains("Status: `failed`", StringComparison.OrdinalIgnoreCase) then
                "failed"
            elif text.Contains("Status: `interrupted`", StringComparison.OrdinalIgnoreCase) then
                "interrupted"
            elif text.Contains("Status: `stale`", StringComparison.OrdinalIgnoreCase) then
                "stale"
            elif text.Contains("Status: `missing`", StringComparison.OrdinalIgnoreCase) then
                "missing"
            else
                "undocumented"
...
let feature160ReadinessSummaryFromCurrentHost
    (out: string)
    ...
    let reason =
        feature160ReasonFromHost facts profile Compositor.Config.feature160AcceptedProfileId
        ...
        | Some reason ->
            ...
            [
                feature160Iteration
                    ...
                    [
                        reason
                        "readiness package assembled with zero accepted performance artifacts"
                    ]
            ]
        | None -> []
    ...
    let unsupportedReason =
        reason
        |> Option.orElse (
            if List.isEmpty iterations then
                None
            else
...
let feature160PackageSample
    ...
    : Compositor.Types.Feature158TimingSample =
    {
        ...
        ArtifactPath =
            Path
                .Combine("throughput", "raw", Compositor.FeatureState.feature160ScenarioFileName scenario)
                .Replace('\\', '/')
    }
...
let feature160PackageIteration
    ...
    let included =
        if status = Compositor.Types.Feature160ReadinessStatus.Accepted then
            Compositor.Config.feature160RequiredScenarioIds
            |> List.mapi (fun sampleIndex scenario ->
                feature160PackageSample runId profile.ProfileId iterationId scenario (sampleIndex + 1))
        else
            []
...
let feature160ReadinessSummaryFromThroughputPackage
    ...
    if not (File.Exists summaryJson) then
        ...
        try
            ...
            let runId =
                jsonStringProperty root "runId" ("feature160-readiness-" + DateTime.UtcNow.ToString("yyyyMMddHHmmss"))
            ...
            let bound =
                jsonIntProperty root "declaredBoundMinutes" Compositor.Config.feature160MaxIterationMinutes

            let requiredAttempts =
                jsonIntProperty root "requiredAttempts" Compositor.Config.feature160RequiredAttempts
            ...
            let unsupported =
                if String.IsNullOrWhiteSpace unsupportedText then
                    None
                else
                    Some unsupportedText

            let performanceClaim =
                jsonStringProperty root "performanceClaim" "performance-not-accepted"
                            ...
                            | None ->
                                ...
                                Path
                                    .Combine(
                                        "throughput",
                                        "iterations",
                                        Compositor.FeatureState.feature160IterationFileName id
                                    )
                                    .Replace('\\', '/')
...
let feature160Readiness (rest: string list) =
    ...
    File.WriteAllText(
        Path.Combine(out, "compatibility-ledger.md"),
        Compositor.Render4.emitFeature160CompatibilityLedger ()
    )

    File.WriteAllText(
        Path.Combine(out, "package-validation.md"),
        Compositor.Render.renderPackageValidation
            160
            [
    ...
    File.WriteAllText(
        Path.Combine(out, "regression-validation.md"),
        Compositor.Render.renderRegressionValidation
            160
            [
                "`compositor-readiness --feature 160`: package assembled."
    ...
    File.WriteAllText(
        Path.Combine(out, "validation-summary.md"),
        Compositor.Render4.emitFeature160ValidationSummary summary
    )
...
let ensureFeature161FsiEvidence out =
    ...
    let readinessFsi =
        Path.Combine(fsiDir, "feature161-host-lane-readiness-authoring.fsx")

    let readinessLog =
        Path.Combine(fsiDir, "feature161-host-lane-readiness-authoring.log")
    ...
    File.WriteAllText(
        compositorLog,
        "Feature161 compositor host-lane authoring PASS: command, policy, required facts, and non-generalized lanes are stable.\n"
    )
    ...
    File.WriteAllText(
        readinessLog,
        "Feature161 host lane readiness authoring PASS: helper validates accepted, rejected, environment-limited, blocked, fallback-only, and missing-evidence packages.\n"
    )

    File.WriteAllText(
        testingSurface,
        "FS.GG.UI.Testing exposes Feature161HostLaneReadiness additive helper records and status tokens.\n"
    )

    File.WriteAllText(
        harnessSurface,
        "Rendering.Harness.Compositor exposes Feature 161 host facts, lane ledger, claim scope, MVU, command, and readiness signatures.\n"
    )

    File.WriteAllText(
        perfSurface,
        "Rendering.Harness.Perf exposes Feature 161 host-lane exclusion reason tokens including missing-display, indirect-rendering, software-raster, unknown-renderer, host-facts-missing, cross-lane-evidence, noisy-timing, and prior-gate-blocked.\n"
    )
...
let feature161ReadinessSummaryFromCurrentHost out fullValidationStatus =
    ...
    let hostFacts =
        feature161HostFactsFromProbe runId "timing/host-lane-ledger" sourceThroughput facts profile

    let entry =
        feature161EntryFromFacts runId hostFacts Compositor.Config.feature161AcceptedProfileId
...
let feature161Readiness (rest: string list) =
    ...
    File.WriteAllText(
        Path.Combine(out, "compatibility-ledger.md"),
        Compositor.Render4.emitFeature161CompatibilityLedger ()
    )
    ...
    File.WriteAllText(
        Path.Combine(out, "validation-summary.md"),
        Compositor.Render4.emitFeature161ValidationSummary summary
    )
