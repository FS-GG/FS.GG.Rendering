...
let feature156ScenarioStem (scenarioId: string) = scenarioId.Replace("/", "-")
...
let writeFeature156RawSamples rawPath scenarioId path runId hostProfile samples =
    ...
    let lines =
        "sample-index,scenario-id,path,run-id,host-profile-id,duration-ms"
        :: sampleLines
...
let runFeature156Path rawDir scenarioId path warmup repetitions runId hostProfileId hostFacts =
    ...
    let rawRelative =
        Path.Combine("raw", $"{feature156ScenarioStem scenarioId}-{path}.csv").Replace('\\', '/')

    let rawPath =
        Path.Combine(rawDir, $"{feature156ScenarioStem scenarioId}-{path}.csv")
...
let feature158SummaryFromReports
    ...
    let status =
        ...
        | None when
            ...
            |> List.forall (fun scenario ->
                reports
                |> List.exists (fun report ->
                    report.ScenarioId = scenario
                    && report.Status = Compositor.Types.Feature158ReadinessStatus.Accepted))
            ->
...

    {
        ...
        Feature156Comparison =
            if status = Compositor.Types.Feature158ReadinessStatus.Accepted then
                "contextualizes"
            else
                "contextualizes"
        Status = status
    ...
    if not (File.Exists path) then
        ...
        try
            ...
            let reasons =
                ...
                if
                    root.TryGetProperty("excludedReasons", &property)
                        ...
                        for item in property.EnumerateArray() do
                            if item.ValueKind = JsonValueKind.String then
                                match
                                    item.GetString()
                                    |> Option.ofObj
                                    |> Option.bind feature158ExclusionReasonFromToken
                                with
                                | Some reason -> reason
                                | None -> ()
                    ]
            ...
            Some
                {
                    RunId =
                        jsonString root "runId"
                        |> Option.defaultValue ("feature158-readiness-" + DateTime.UtcNow.ToString("yyyyMMddHHmmss"))
                    Status =
                        jsonString root "status"
                        |> Option.map feature158StatusFromToken
                        |> Option.defaultValue Compositor.Types.Feature158ReadinessStatus.FallbackOnly
                    ...
                    UnsupportedHostReason = unsupported
                    Feature156Comparison =
                        jsonString root "feature156Comparison" |> Option.defaultValue "contextualizes"
                    PerformanceClaim =
                        jsonString root "performanceClaim"
                        |> Option.defaultValue "performance-not-accepted"
...
let loadFeature158ProofProbeEvidence
    proofProbeDir
    (profile: Compositor.Types.HostProfile)
    : Compositor.Types.Feature158ProofProbeEvidence list =
...
let feature158ReadinessReportsFromSnapshot
    (snapshot: Feature158TimingSnapshot)
    (profile: Compositor.Types.HostProfile)
    proofProbeArtifacts
    =
    ...
    |> List.map (fun scenario ->
            ...
            [
                for index in 1..excludedCount ->
                    ...
                    feature158Sample
                        ...
                        0.0
                        (Path
                            .Combine("timing", "excluded", Perf.exclusionReasonToken reason + ".md")
                            .Replace('\\', '/'))
            ]

        let status =
            match snapshot.Status with
            | Compositor.Types.Feature158ReadinessStatus.Accepted when List.isEmpty included ->
        ...
        feature158ScenarioReport
            ...
            [
                Path
                    .Combine("timing", "scenarios", Compositor.FeatureState.feature158ScenarioFileName scenario)
                    .Replace('\\', '/')
            ]
            [ "readiness summary derived from timing/summary.json" ])

let writeFeature158RawSamples
    rawDir
    scenario
    pathToken
    runId
    ...
    packageVersion
    policy
    (samples: Compositor.Types.Feature158TimingSample list)
    =
    ...
    let csvLines =
        ...
        |> List.map (fun (sample: Compositor.Types.Feature158TimingSample) ->
            let reason =
                ...
                |> Option.map Perf.exclusionReasonToken
                |> Option.defaultValue ""
    ...
    let header =
        "sample-index,sample-id,scenario-id,path,run-id,host-profile-id,package-version,scenario-definition-id,duration-ms,measurement-policy,inclusion-status,exclusion-reason,artifact-path"
    ...
    File.WriteAllText(
        jsonPath,
        String.concat Environment.NewLine [ "["; jsonSamples; "]" ]
        + Environment.NewLine
    )
        ...
        [
            for index in 1..repetitions do
                ...
                let status, reason, duration, policy =
                    ...
                    | Result.Error _ ->
                        Perf.Excluded, Some Perf.UnverifiableMeasurementPolicy, Double.NaN, Perf.Unverified

                let sampleIndex = index.ToString("000")

                let sampleId =
                    $"{runId}-{feature156ScenarioStem scenario}-{pathToken}-{sampleIndex}"

                let artifact =
                    Path.Combine("raw", $"{feature156ScenarioStem scenario}-{pathToken}.csv").Replace('\\', '/')
                ...
                | Result.Error failure ->
                    { sample with
                        ArtifactPath = artifact + $"; failure={failure.Message}"
                    }
        ]

    let csvRelative, jsonRelative =
        writeFeature158RawSamples
            rawDir
            scenario
            pathToken
            runId
...
let feature158ScenarioStatus
    (included: Compositor.Types.Feature158TimingSample list)
    (excluded: Compositor.Types.Feature158TimingSample list)
    =
    if
        List.isEmpty included
        && excluded
           |> List.exists (fun sample ->
               sample.ExclusionReason = Some Perf.EnvironmentLimitedReason
               || sample.ExclusionReason = Some Perf.UnsupportedHost)
    then
        Compositor.Types.Feature158ReadinessStatus.EnvironmentLimited
    elif
        not (List.isEmpty included)
        && included
           |> List.forall (fun sample ->
               sample.MeasurementPolicy = Perf.ReadbackFree
               || sample.MeasurementPolicy = Perf.ReadbackOutsideMeasurement)
        && excluded |> List.forall (fun sample -> sample.InclusionStatus <> Perf.Included)
    then
        Compositor.Types.Feature158ReadinessStatus.Accepted
...
let writeFeature158TimingPackage out (summary: Compositor.Types.Feature158TimingSummary) =
    ...
    for report in summary.ScenarioReports do
        File.WriteAllText(
            Path.Combine(scenariosDir, Compositor.FeatureState.feature158ScenarioFileName report.ScenarioId),
            Compositor.Render3.emitFeature158ScenarioReport report
        )
    ...
    |> List.iter (fun (reason, samples) ->
        File.WriteAllText(
            Path.Combine(excludedDir, Perf.exclusionReasonToken reason + ".md"),
            Compositor.Render3.emitFeature158ExcludedSamplesReport reason samples
        ))

    if not (File.Exists(Path.Combine(excludedDir, "README.md"))) then
        File.WriteAllText(
            Path.Combine(excludedDir, "README.md"),
            "# Feature 158 Excluded Samples\n\nGrouped excluded samples are written by primary reason.\n"
        )

    let proofProbeReadme = Path.Combine(proofProbeDir, "README.md")

    if
        not (List.isEmpty summary.ProofProbeEvidence)
        || not (File.Exists proofProbeReadme)
    then
        File.WriteAllText(
            proofProbeReadme,
    ...
    | Some reason ->
        File.WriteAllText(
            ...
            Compositor.Render3.emitFeature158UnsupportedHostReport reason
        )
    ...
    | None ->
        if not (File.Exists(Path.Combine(unsupportedDir, "README.md"))) then
            File.WriteAllText(
                Path.Combine(unsupportedDir, "README.md"),
                Compositor.Render3.emitFeature158UnsupportedHostReport "not run in this invocation"
            )

    File.WriteAllText(Path.Combine(out, "summary.md"), Compositor.Render3.emitFeature158TimingSummary summary)

    File.WriteAllText(
        Path.Combine(out, "summary.json"),
        Compositor.Render3.emitFeature158TimingSummaryJson summary + Environment.NewLine
    )
...
let feature157Fallback scenario reason artifact : Compositor.Types.Feature157Fallback =
    {
        ...
        DamageValidationStatus =
            if scenario.Contains("out-of-bounds", StringComparison.Ordinal) then
                "out-of-bounds"
            elif scenario.Contains("stale", StringComparison.Ordinal) then
                "stale"
            elif scenario.Contains("incomplete", StringComparison.Ordinal) then
                "incomplete"
            elif scenario.Contains("full-frame", StringComparison.Ordinal) then
                "full-frame-invalidation"
            elif scenario.Contains("empty-visible", StringComparison.Ordinal) then
...
let feature157Attempt
    runId
    (index: int)
    scenario
    (profile: Compositor.Types.HostProfile)
    : Compositor.Types.Feature157DamageAttempt =
...
let feature157Summary
    ...
    unsupported
    diagnostics
    : Compositor.Types.Feature157DamageSummary =
    {
        RunId = runId
        HostProfile = profile
        Status = status
        ...
        UnsupportedHostReason = unsupported
        ScenarioCoverage =
            (attempts |> List.map _.ScenarioId) @ (fallbacks |> List.map _.ScenarioId)
            |> List.distinct
        PerformanceClaim = "performance-not-accepted"
        Diagnostics = diagnostics
    }
...
let writeFeature157DamagePackage out (summary: Compositor.Types.Feature157DamageSummary) =
    ...
    for attempt in summary.AcceptedAttempts do
        File.WriteAllText(
            Path.Combine(attemptsDir, $"{attempt.AttemptId}.md"),
            Compositor.Render3.emitFeature157AttemptReport attempt
        )

        File.WriteAllText(
            Path.Combine(parityDir, $"{attempt.AttemptId}.md"),
            Compositor.Render3.emitFeature157ParityReport attempt
        )
    ...
    | Some reason ->
        File.WriteAllText(
            Path.Combine(unsupportedDir, "README.md"),
            Compositor.Render3.emitFeature157UnsupportedHostReport reason
        )
    ...
    File.WriteAllText(
        Path.Combine(out, "summary.json"),
        Compositor.Render3.emitFeature157DamageSummaryJson summary + Environment.NewLine
    )
...
let runCompositorDamageCmd (rest: string list) =
    if not (isFeature157 rest) then
        ...
        if List.isEmpty scenarioSet then
            ...
            let expectedProfile =
                flagValue "--profile" rest
                |> Option.defaultValue Compositor.Config.feature157AcceptedProfileId
                ...
                | Some reason ->
                    let fallbacks =
                        ...
                        |> List.map (fun scenario ->
                            let artifact =
                                Path
                                    .Combine("fallbacks", Compositor.FeatureState.feature157ScenarioFileName scenario)
                                    .Replace('\\', '/')
                    ...
                    let status =
                        if
                            reason.Contains("missing display", StringComparison.OrdinalIgnoreCase)
                            || reason.Contains("renderer", StringComparison.OrdinalIgnoreCase)
                            || reason.Contains("direct rendering", StringComparison.OrdinalIgnoreCase)
                        then
                    ...
                    feature157Summary
                        ...
                        fallbacks
                        (Some reason)
                        [ reason; "accepted partial-redraw artifacts=0" ]
                | None ->
                    let attempts =
                        [
                    ...
                    feature157Summary
                        runId
                        profile
                        Compositor.Types.Feature157DamageStatus.Accepted
                        attempts
                        fallbacks
                        None
                        [ "damage-scoped no-clear path accepted for current same-profile host" ]
