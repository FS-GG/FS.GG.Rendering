...
let private declared =
    GameplayVisualInventory.all |> List.map GameplayVisualInventory.elementId
...
let private evidenceDigests (catalog: Catalog.Catalog) : Catalog.EvidenceDigests =
    {
        ...
        Render =
            ...
            |> List.map (fun (element, handle, state, _, bytes) ->
                String.concat "|" [ GameplayVisualInventory.elementId element; handle; state; sha256 bytes ])
            |> String.concat "\n"
            |> textBytes
            |> sha256
    }
...
let private runtimeAudit (catalog: Catalog.Catalog) =
    Catalog.audit declared catalog GameplayVisualInventory.registeredBindings observedBindings (evidenceDigests catalog)

let private finding gap element (report: Catalog.BindingReport) =
    report.Findings
    |> List.exists (fun item -> item.Gap = gap && item.Element = element)

let private mkCatalog (entries: Catalog.Entry list) : Catalog.Catalog = { Entries = entries }
...
type private RogueState = { Locked: bool; Descending: bool }

let private rogueElements = [ DoorOpen; DoorLocked; Trapdoor ]
...
let private rogueProjection state =
    ...
    let scene =
        ...
        | DoorOpen ->
            {
                Nodes = [ Rectangle((8.0, 8.0, 8.0, 32.0), color 80uy 180uy 110uy) ]
            }
        | DoorLocked ->
            {
                Nodes = [ Rectangle((8.0, 8.0, 24.0, 32.0), color 210uy 90uy 70uy) ]
            }
        | Trapdoor ->
            {
                Nodes = [ Rectangle((8.0, 24.0, 32.0, 8.0), color 150uy 100uy 60uy) ]
            }
    ...
    testList
        ...
        [
            test "the typed runtime inventory is exhaustive, readable, non-empty, and unique" {
                ...
                Expect.equal
                    (List.distinct declared)
                    declared
                    "the runtime gameplay-visual inventory must not contain duplicates"

                Expect.equal
                    GameplayVisualInventory.bindings.Length
                    declared.Length
                    "every reflected union case has an exhaustive binding"
            }
            ...
            test "representative states traverse View.view and yield non-empty runtime frames" {
                for _, projected, actual, expected, _ in runtimeFrames do
                    Expect.equal
                        actual
                        expected
                        "View.view must consume the audited projection without a parallel renderer"

                    for item in projected do
                        Expect.isNonEmpty
                            item.Scene.Nodes
                            $"{GameplayVisualInventory.elementId item.Element} must produce real Scene nodes"
            ...
            test "required states differ per element and distinct elements cannot share byte-identical evidence" {
                for binding in GameplayVisualInventory.bindings do
                    Expect.isNonEmpty
                        binding.RequiredStates
                        $"{GameplayVisualInventory.elementId binding.Element} must declare evidence states"
                    ...
                    let digests =
                        ...
                        |> List.map (fun (state, model) ->
                            ...
                            Expect.isNonEmpty
                                scene.Nodes
                                $"{GameplayVisualInventory.elementId binding.Element}/{state} must emit Scene nodes"
            ...
            test
                "Rogue2 old self-declared control stays green while typed runtime inventory names Door and Trapdoor omissions" {
                let staleStarter =
                    mkCatalog
                        [
                            {
                                Element = "Ball"
                                Visual = Catalog.Visual.Shown "scene/ball"
                            }
                            {
                                Element = "LeftPaddle"
                                Visual = Catalog.Visual.Shown "scene/paddle"
                            }
                        ]

                Expect.equal
                    (Catalog.validate staleStarter).Verdict
                    Coverage.Verdict.Covered
                    "old control remains green"

                let report =
                    Catalog.audit (rogueElements |> List.map rogueId) staleStarter [] [] syntheticEvidence

                for element in [ "DoorOpen"; "DoorLocked"; "Trapdoor" ] do
                    Expect.isTrue
                        (finding Catalog.BindingGap.Missing element report)
                        $"typed fixture names omitted {element}"

                for stale in [ "Ball"; "LeftPaddle" ] do
                    Expect.isTrue
                        (finding Catalog.BindingGap.Stale stale report)
                        $"dungeon fixture rejects stale {stale}"
            }
            ...
            test "Rogue2 open, locked/sealed, and trapdoor states are element-bound, rendered, and visually distinct" {
                let catalog =
                    ...
                    |> List.map (fun element ->
                        ({
                            ...
                            Visual = Catalog.Visual.Shown(rogueHandle element)
                        }
                        : Catalog.Entry))
                    |> mkCatalog
                ...
                Expect.equal
                    complete.Verdict
                    Catalog.BindingVerdict.Complete
                    "all typed Rogue2 states are bound and observed"
                ...
                let swapped =
                    Catalog.audit
                        ...
                        catalog
                        [
                            "DoorOpen", "scene/door-locked"
                            "DoorLocked", "scene/door-open"
                            "Trapdoor", "scene/trapdoor"
                        ]
                        observed
                        syntheticEvidence
                ...
                Expect.isTrue
                    (finding Catalog.BindingGap.Unbound "DoorOpen" swapped)
                    "swapped element/handle pairs cannot pass"
            }
