...
let assertBuildHelpBanner () =
    ...
    Expect.isFalse
        (build.Contains("writeLog \"help\"", System.StringComparison.OrdinalIgnoreCase))
        "help path is side-effect-free (writes no completion-marker log)"
...
// The bounded cross-line span tolerates Fantomas placing each argument on its own line and the
// window-behavior request argument the `--window-*` overload threads between `viewerOptions` and the
// sink. This is exactly what a sanctioned launcher upgrade must keep satisfying.
let effectHonouringLaunch =
    System.Text.RegularExpressions.Regex(
        $@"[A-Za-z_][\w.]*\s+viewerOptions\b[\s\S]{{0,500}}?\baudioSink\s+{persistentHostValue}\b"
    )
...
let assertDefaultLaunchHonoursEffects (launchText: string) =
    ...
    Expect.stringContains
        launchText
        "FS.GG.Audio.Host.Audio.play"
        "the governed launch builds the PlayAudio sink from the audio host play seam (the effect reaches a real sink)"
    ...
    testList
        ...
        [
            test "generated headless product exposes deterministic scene evidence command" {
                ...
                Expect.isFalse
                    (source.Contains("ControlsElmish"))
                    "headless profile does not require Controls Elmish adapters"
            }
            ...
            test "generated product ships the SWAP-CHECKLIST discoverability doc" { assertSwapChecklistPresent () }

            test "generated build.fsx surfaces the build-target help banner" { assertBuildHelpBanner () }
        ]
//#else
[<Tests>]
let governanceTests =
    testList
        ...
        [
            test "generated product source is split by responsibility in compile order" {
                let approotDir =
                    System.IO.Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "src", "Product")

                let project =
                    System.IO.File.ReadAllText(System.IO.Path.Combine(approotDir, "Product.fsproj"))
                ...
                |> List.iter (fun file ->
                    Expect.isTrue
                        (System.IO.File.Exists(System.IO.Path.Combine(approotDir, file)))
                        $"{file} exists in generated product source"
                ...
                let program =
                    System.IO.File.ReadAllText(System.IO.Path.Combine(approotDir, "Program.fs"))

                Expect.stringContains program "[<EntryPoint>]" "Program.fs keeps the entrypoint"

                Expect.stringContains
                    program
                    "tryRunEvidenceCommand (List.ofArray args)"
                    "Program.fs delegates explicit evidence command dispatch"

                Expect.isFalse
                    (program.Contains("let writeGeneratedEvidenceLines", StringComparison.Ordinal))
                    "Program.fs does not own report writing"
                ...
                Expect.isFalse
                    ...
                    "Program.fs does not own layout evidence implementation"
            }
            ...
            test "compile-order scan anchors to Compile Include and survives additive filenames and comments" {
                ...
                Expect.isLessThan
                    (compileIncludeIndex synthetic "Model.fs")
                    (compileIncludeIndex synthetic "View.fs")
                    "anchored scan keeps model before view despite an additive BulwarkView.fs and a `View.fs` comment appearing earlier"

                Expect.isLessThan
                    (compileIncludeIndex synthetic "View.fs")
                    (compileIncludeIndex synthetic "LayoutEvidence.fs")
                    "anchored scan keeps view before layout evidence"
                ...
                Expect.isLessThan
                    (synthetic.IndexOf("View.fs", StringComparison.Ordinal))
                    (synthetic.IndexOf("Model.fs", StringComparison.Ordinal))
                    "the naive bare-substring scan is demonstrably fooled — the Include anchor is load-bearing"
            }
            ...
            test "generated graphical app exposes bounded smoke command" {
                ...
                Expect.stringContains
                    source
                    "--launch-evidence"
                    "generated product exposes explicit launch evidence CLI"
                ...
                Expect.stringContains
                    ...
                    "generated product exposes explicit frame diagnostic smoke CLI"

                Expect.stringContains
                    source
                    "Viewer.runBounded"
                    "bounded smoke uses the public SkiaViewer bounded run entry point"

                Expect.stringContains
                    source
                    "status=unsupported"
                    "bounded smoke reports unsupported host conditions explicitly"

                Expect.stringContains
                    source
                    "diagnostic-mode={diagnosticMode}"
                    "generated smoke writes readable diagnostics mode"
                ...
                Expect.stringContains
                    ...
                    "generated smoke limits repeated frame diagnostics"
            }
            ...
            test "generated evidence commands are opt-in and not reported as ongoing interactive play" {
                ...
                let defaultBranch =
                    program.Substring(program.LastIndexOf("| None ->", StringComparison.Ordinal))

                Expect.stringContains
                    source
                    "--launch-evidence"
                    "first-frame launch evidence is exposed only by explicit CLI flag"

                Expect.stringContains
                    source
                    "--bounded-smoke"
                    "bounded evidence smoke is exposed only by explicit CLI flag"

                Expect.stringContains
                    source
                    "--bounded-smoke-frame-diagnostics"
                    "frame diagnostics are exposed only by explicit CLI flag"
                ...
                Expect.stringContains
                    source
                    "--screenshot-evidence"
                    "screenshot evidence is exposed only by explicit CLI flag"

                Expect.stringContains
                    source
                    "--pixel-readback-evidence"
                    "pixel-readback evidence is exposed only by explicit CLI flag"

                Expect.stringContains
                    source
                    "input-dispatch=not-required"
                    "bounded evidence reports that input dispatch is not an interactive-play claim"

                Expect.stringContains
                    source
                    "self-closed-for-evidence=true"
                    "bounded evidence reports self-close semantics"
                ...
                Expect.stringContains
                    ...
                    "first-frame evidence records the evidence command"
                ...
                Expect.stringContains
                    source
                    "\"--screenshot-evidence\""
                    "screenshot evidence records the evidence command"

                Expect.stringContains
                    source
                    "\"--pixel-readback-evidence\""
                    "pixel-readback evidence records the evidence command"

                Expect.stringContains
                    source
                    "Viewer.runBounded"
                    "generated evidence commands use bounded viewer evidence entry points"
                ...
                Expect.stringContains
                    ...
                    "view-image uses the SkiaViewer-owned headless CPU rasterizer"

                Expect.stringContains
                    source
                    "SceneEvidence.renderPng"
                    "view-image renders real pixels through the public Scene evidence pixel seam"

                Expect.stringContains
                    source
                    "viewImageAtSize evidencePath 1280 720"
                    "view-image retains the deterministic default logical resolution"

                Expect.stringContains
                    source
                    "width <= 0 || height <= 0"
                    "view-image accepts only positive explicit dimensions"

                Expect.stringContains
                    source
                    "requestedPixels > maxViewImagePixels"
                    "view-image rejects an unsafe CPU raster before allocation"

                Expect.stringContains
                    source
                    "diagnostic-category=resource-limit"
                    "oversized requests receive a typed resource-limit diagnostic"

                Expect.stringContains
                    source
                    "tryPngDimensions pngBytes"
                    "view-image reads actual dimensions from the PNG header"

                Expect.stringContains
                    source
                    "evidenceField \"dimensions-match\""
                    "view-image records whether requested and actual dimensions agree"
                ...
                Expect.isFalse
                    (defaultBranch.Contains("mode=persistent-evidence"))
                    "normal launch does not report bounded evidence mode"

                Expect.isFalse
                    (defaultBranch.Contains("self-closed-for-evidence=true"))
                    "normal launch does not claim evidence self-close"

                Expect.isFalse
                    (defaultBranch.Contains("input-dispatch=not-required"))
                    "normal launch does not reuse bounded evidence input-dispatch wording"

                Expect.isFalse
                    (defaultBranch.Contains("--image-evidence"))
                    "image evidence stays out of normal launch branch"

                Expect.isFalse
                    (defaultBranch.Contains("--view-image"))
                    "view-image evidence stays out of normal launch branch"

                Expect.isFalse
                    (defaultBranch.Contains("--screenshot-evidence"))
                    "screenshot evidence stays out of normal launch branch"

                Expect.isFalse
                    (defaultBranch.Contains("--pixel-readback-evidence"))
                    "pixel-readback evidence stays out of normal launch branch"
            }

            test
                "generated visual evidence commands require screenshot proof pixel fallback and unsupported diagnostics" {
                let source = approotSources [ "Program.fs"; "EvidenceCommands.fs" ]
                ...
                Expect.stringContains
                    source
                    "--screenshot-evidence"
                    "generated product exposes screenshot evidence command"

                Expect.stringContains
                    source
                    "--pixel-readback-evidence"
                    "generated product exposes pixel-readback evidence command"

                Expect.stringContains
                    source
                    "evidenceField \"evidence-kind\" \"image\""
                    "image command records image evidence kind"
                ...
                Expect.stringContains
                    ...
                    "image command records scene-rendering proof claim"

                Expect.stringContains
                    source
                    "evidenceField \"proves-desktop-visibility\" \"false\""
                    "image command records desktop-visibility proof claim"

                Expect.stringContains
                    source
                    "evidenceField \"evidence-kind\" \"screenshot\""
                    "screenshot command records screenshot evidence kind"

                Expect.stringContains
                    source
                    "Viewer.captureScreenshotEvidence"
                    "screenshot command uses the viewer screenshot evidence contract"

                Expect.stringContains
                    source
                    "deterministic-scene-evidence"
                    "unsupported screenshot command records deterministic fallback"

                Expect.stringContains
                    source
                    "evidenceField \"viewer-open-status\""
                    "screenshot command reports viewer-open status"

                Expect.stringContains
                    source
                    "evidenceField \"first-frame-status\""
                    "screenshot command reports first-frame status"

                Expect.stringContains
                    source
                    "evidenceField \"capture-availability\""
                    "screenshot command reports capture availability"

                Expect.stringContains
                    source
                    "evidenceField \"capture-source\""
                    "screenshot command reports capture source"

                Expect.isTrue
                    (Text.RegularExpressions.Regex.IsMatch(source, "evidenceField\\s+\"deterministic-fallback-kind\""))
                    "screenshot command reports deterministic fallback kind"

                Expect.stringContains
                    source
                    "evidenceField \"proves-screenshot\""
                    "screenshot command reports screenshot proof boolean"

                Expect.isFalse
                    (source.Contains("evidenceField \"capture-source\" \"pixel-readback\"", StringComparison.Ordinal))
                    "pixel readback is not relabeled as screenshot capture source"

                Expect.isFalse
                    (source.Contains(
                        "evidenceField \"capture-source\" \"deterministic-scene-render\"\n              evidenceField \"proves-screenshot\" \"true\"",
                        StringComparison.Ordinal
                    ))
                    "deterministic render is not relabeled as screenshot proof"

                Expect.stringContains
                    source
                    "evidenceField \"evidence-kind\" evidenceKind"
                    "pixel-readback command records fallback evidence kind"

                Expect.stringContains
                    source
                    "evidenceField \"fallback-reason\" fallbackReason"
                    "pixel-readback command records why screenshot proof was unavailable"

                Expect.stringContains
                    source
                    "screenshot-unavailable"
                    "pixel-readback command names screenshot unavailability"

                Expect.stringContains
                    source
                    "evidenceField \"playfield-readable\" \"true\""
                    "visual evidence proves the playfield/grid is readable"

                Expect.stringContains
                    source
                    "evidenceField \"input-or-progress-observed\" \"true\""
                    "visual evidence proves input dispatch or time progression was observed"

                Expect.stringContains
                    source
                    "evidenceField \"unsupported-host-reason\""
                    "unsupported visual evidence reports why neither visual path is available"

                Expect.stringContains
                    source
                    "evidenceField \"supported-host\" \"false\""
                    "unsupported visual evidence is explicit instead of substituting text-only metadata"
            }
            ...
            test "generated evidence commands share Testing report conventions" {
                ...
                Expect.stringContains
                    source
                    "let writeEvidenceReport"
                    "generated product defines one local report wrapper"

                Expect.stringContains
                    source
                    "generatedEvidenceStatusText"
                    "generated product shares normalized report status vocabulary"

                Expect.stringContains
                    source
                    "| GeneratedEvidenceOk -> \"ok\""
                    "generated product preserves ok status vocabulary"

                Expect.stringContains
                    source
                    "| GeneratedEvidenceUnsupported -> \"unsupported\""
                    "generated product preserves unsupported status vocabulary"

                Expect.stringContains
                    source
                    "| GeneratedEvidenceFailed -> \"failed\""
                    "generated product preserves failed status vocabulary"

                Expect.stringContains
                    source
                    "generatedEvidenceExitCode"
                    "generated product keeps report status to exit-code semantics local"

                Expect.stringContains
                    source
                    "| GeneratedEvidenceUnsupported -> 0"
                    "unsupported generated evidence remains a non-failing host fact"

                Expect.stringContains
                    source
                    "| GeneratedEvidenceFailed -> 1"
                    "failed generated evidence remains a failing command result"

                Expect.stringContains
                    source
                    "writeEvidenceReport"
                    "shared report wrapper is called by generated evidence commands"

                Expect.stringContains
                    source
                    "evidenceField \"command\" command"
                    "report wrapper preserves command field"

                Expect.stringContains
                    source
                    "evidenceField \"output\" evidencePath"
                    "report wrapper preserves output field"

                Expect.stringContains
                    source
                    "writeGeneratedEvidenceLines evidencePath true (generatedEvidenceExitCode status) lines"
                    "report wrapper creates parent directories, writes the requested output path, and preserves exit-code semantics"

                Expect.stringContains
                    source
                    "lines |> List.iter (printfn \"%s\")"
                    "report wrapper echoes report fields to stdout"

                Expect.stringContains
                    source
                    "\"--layout-evidence\""
                    "layout command reports through the shared convention"
                ...
                Expect.stringContains
                    source
                    "\"--image-evidence\""
                    "image command reports through the shared convention"

                Expect.stringContains
                    source
                    "\"--screenshot-evidence\""
                    "screenshot command reports through the shared convention"

                Expect.stringContains
                    source
                    "\"--pixel-readback-evidence\""
                    "pixel-readback command reports through the shared convention"
            }
            ...
            test "generated graphical app default executable path uses persistent host" {
                ...
                Expect.stringContains
                    ...
                    "successful default path reports accessible desktop window claim"

                Expect.stringContains
                    source
                    "window-visible=observed:true"
                    "successful default path reports observed visible window"

                Expect.stringContains
                    source
                    "accessible-window=false"
                    "unsupported default path does not claim visible accessibility"

                Expect.stringContains
                    source
                    "mode=interactive-window"
                    "unsupported default diagnostics still identify interactive mode"
            ...
            test "generated normal launch reports desktop session diagnostics without evidence fallback" {
                ...
                let defaultBranch =
                    source.Substring(source.LastIndexOf("| None ->", StringComparison.Ordinal))

                Expect.stringContains
                    defaultBranch
                    "Viewer.desktopSessionDiagnostic()"
                    "normal launch captures desktop/session diagnostics before app lifecycle debugging"

                Expect.stringContains
                    defaultBranch
                    "diagnostic-class="
                    "normal launch reports diagnostic classification"
                ...
                Expect.stringContains
                    defaultBranch
                    "display-socket-exists="
                    "normal launch reports display socket state"
                ...
                Expect.stringContains
                    ...
                    "private runtime fallback is labeled as not a full desktop session"

                Expect.isFalse
                    (defaultBranch.Contains("Viewer.runBounded"))
                    "normal launch does not silently switch to bounded evidence"

                Expect.isFalse
                    (defaultBranch.Contains("SceneEvidence.render"))
                    "normal launch does not silently switch to scene-only metadata"

                Expect.isFalse
                    (defaultBranch.Contains("--launch-evidence"))
                    "explicit evidence flag stays out of normal launch diagnostics"

                Expect.isFalse
                    (defaultBranch.Contains("--scene-evidence"))
                    "scene evidence flag stays out of normal launch diagnostics"
            }
            ...
            test
                "generated window diagnostics command derives its verdict from the real launch gate, not fabricated failures" {
                let source = approotSources [ "Program.fs"; "EvidenceCommands.fs" ]
                ...
                Expect.stringContains
                    source
                    "--window-diagnostics"
                    "generated product exposes an explicit window diagnostics command"

                Expect.stringContains
                    source
                    "diagnostic-class=environment-session"
                    "diagnostics include environment/session class"

                Expect.stringContains
                    source
                    "diagnostic-class=window-visibility"
                    "diagnostics include window visibility class"
                ...
                Expect.stringContains
                    ...
                    "diagnostics include product defect class"
                ...
                Expect.stringContains
                    source
                    "renderable-surface="
                    "diagnostics still enumerate the renderable-surface fact"

                Expect.stringContains source "input-devices=" "diagnostics still enumerate the input-device fact"

                Expect.stringContains
                    source
                    "fallback-is-full-desktop-session="
                    "diagnostics disclose fallback session status"

                // Single source of truth: the probe reads the same runtime gate the real launch consults.
                Expect.stringContains
                    evidence
                    "Viewer.runtimeCapability()"
                    "window diagnostics derive their verdict from the same gate the real launch consults"

                Expect.stringContains
                    evidence
                    "Viewer.desktopSessionDiagnostic()"
                    "window diagnostics reflect the real desktop-session determination"

                Expect.stringContains
                    evidence
                    "persistent-window-supported="
                    "window diagnostics report the host's live-window capability from that gate"
                ...
                Expect.isFalse
                    (evidence.Contains "visible=observed:false")
                    "window diagnostics no longer fabricate an observed window-invisibility (#136)"

                Expect.isFalse
                    (evidence.Contains "taskbar-only window has no accessible visible surface")
                    "window diagnostics no longer fabricate a taskbar-only visibility failure (#136)"

                Expect.isFalse
                    (evidence.Contains "app lifecycle failed after visible window diagnostics")
                    "window diagnostics no longer fabricate an app-lifecycle failure (#136)"

                Expect.isFalse
                    (evidence.Contains "product requested a zero-sized or surface-less window")
                    "window diagnostics no longer fabricate a product-defect failure (#136)"

                Expect.isFalse
                    (defaultBranch.Contains("--window-diagnostics"))
                    "normal launch does not silently switch to diagnostics mode"
            }

            test
                "generated app Synthetic exposes window behavior flags and option diagnostics without leaving interactive launch" {
                ...
                Expect.stringContains
                    source
                    "toViewerWindowBehavior windowBehavior"
                    "parsed flags become the public viewer request"

                Expect.stringContains
                    source
                    "Viewer.validateWindowLaunchBehavior viewerOptions.InitialSize"
                    "generated diagnostics use public launch behavior validation"
                ...
                Expect.stringContains
                    ...
                    "normal launch validates parsed behavior request before calling SkiaViewer"
                ...
                Expect.stringContains
                    source
                    "status=unsupported"
                    "unsupported host/backend option diagnostics are explicit"

                Expect.isFalse
                    (defaultBranch.Contains("Viewer.runBounded"))
                    "window options do not switch normal launch to bounded evidence"
            }
            ...
            test "generated graphical app exposes deterministic scene evidence command" {
                ...
                Expect.stringContains
                    source
                    "--scene-evidence"
                    "generated product exposes non-window scene evidence CLI"
                ...
                Expect.stringContains
                    ...
                    "scene evidence remains separate from live viewer startup"

                Expect.stringContains
                    source
                    "readiness/headless-scene-evidence.txt"
                    "scene evidence writes a stable readiness path"
            }
            ...
            test "generated evidence graph command runs the in-process engine" {
                ...
                Expect.stringContains
                    build
                    "runGeneratedEvidence \"EvidenceGraph\""
                    "build delegates the graph command to the engine runner"

                Expect.stringContains
                    build
                    "runGeneratedEvidence \"EvidenceAudit\""
                    "build delegates the audit command to the engine runner"

                Expect.stringContains
                    build
                    "GeneratedRunner"
                    "build invokes the engine's generated-evidence runner by reflection"

                Expect.stringContains
                    build
                    "Assembly.LoadFrom"
                    "build binds the property-resolved engine assembly at runtime"

                Expect.stringContains
                    build
                    "FsGgUiVersion"
                    "build resolves the engine from the single-source version property"
                ...
                Expect.isFalse
                    (build.Contains("| \"EvidenceGraph\"\n    | \"EvidenceAudit\" -> writeLog target"))
                    "evidence commands are not completion-only logs"
            }
            ...
            test "generated evidence graph and audit do not shell the decommissioned scripts" {
                ...
                [
                    ...
                    "python3"
                    "ProcessStartInfo(\"bash\""
                ]
                |> List.iter (fun forbidden ->
                    Expect.isFalse
                        (build.Contains(forbidden, StringComparison.Ordinal))
                        $"generated evidence workflow excludes the decommissioned {forbidden}")

                Expect.isFalse
                    (build.Contains("chmod", StringComparison.OrdinalIgnoreCase))
                    "generated evidence workflow does not repair executable mode"
            }
            ...
            test "generated Verify redirected output is clean text" {
                ...
                Expect.stringContains
                    ...
                    "generated Verify writes text logs through the checked text writer"
                ...
                |> List.iter (fun forbidden ->
                    Expect.isFalse
                        (build.Contains(forbidden, StringComparison.OrdinalIgnoreCase))
                        $"generated Verify excludes binary log writer {forbidden}")
            }
