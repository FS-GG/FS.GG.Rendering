...
let private withFixture action =
    let root =
        Path.Combine(Path.GetTempPath(), "fs-gg-audio-" + Guid.NewGuid().ToString("N"))
...
let audioCueResolutionTests =
    testList
        "audio cue resolution readiness (#1210)"
        [
            test "fresh and missing-one fixtures remain red independently of requested cues" {
                withFixture (fun root ->
                    Expect.isFalse
                        (AppRoot.AudioCues.audioContentReadyAt root)
                        "fresh scaffold content is explicitly incomplete"
                    ...
                    Expect.isFalse
                        (AppRoot.AudioCues.audioContentReadyAt root)
                        "one valid asset cannot hide every other missing declared cue")
            }
            test "malformed and complete fixtures are classified by resolver readiness" {
                withFixture (fun root ->
                    ...
                    let malformed =
                        AppRoot.AudioCues.resolutionEvidenceAt root
                        |> List.find (fun finding -> finding.CueId = damaged)

                    Expect.equal
                        malformed.Problem
                        (Some "malformed WAV")
                        "malformed asset is named rather than silently resolving"

                    Expect.isFalse
                        (AppRoot.AudioCues.audioContentReadyAt root)
                        "corrupting a complete map returns it to red")
            }
            test "placeholder option is deterministic and carries a reviewable digest" {
                ...
                Expect.equal
                    (AppRoot.AudioCues.placeholderSha256 first)
                    (AppRoot.AudioCues.placeholderSha256 second)
            ...
            test "artifact output readiness probe" {
                match
                    optionalEnvironment "FSGG_AUDIO_FIXTURE_PROBE_ROOT", optionalEnvironment "FSGG_AUDIO_FIXTURE_EXPECT"
                with
                | Some root, Some "complete" ->
                    ...
                    for id in AppRoot.AudioCues.declaredCueIds do
                        let expected =
                            AppRoot.AudioCues.deterministicPlaceholderWave id
                            |> AppRoot.AudioCues.placeholderSha256

                        let (FS.GG.Audio.Core.SoundId raw) = id

                        let actual =
                            File.ReadAllBytes(Path.Combine(root, raw + ".wav"))
                            |> AppRoot.AudioCues.placeholderSha256
                ...
                | Some root, Some expectedProblem ->
                    ...
                    Expect.isTrue
                        (findings |> List.exists (fun finding -> finding.Problem = Some expectedProblem))
                        $"actual artifact reports {expectedProblem}"
                | _ -> ()
            }
        ]
//#endif
