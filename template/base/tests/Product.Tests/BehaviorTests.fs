...
let windowOptionOverlayTests =
    testList
        "window option overlay"
        [
            test "safe default and every single option overlay preserve unspecified fields" {
                let baseline: AppRoot.WindowOptions.WindowBehaviorSettings =
                    AppRoot.WindowOptions.parseWindowBehavior []
                ...
                for args, changed in cases do
                    let overlay: AppRoot.WindowOptions.WindowBehaviorSettings =
                        AppRoot.WindowOptions.parseWindowBehavior args

                    Expect.isTrue (changed overlay) "the supplied flag changes its own field"

                    if args.Head <> "--window-resize" then
                        Expect.equal overlay.Resize baseline.Resize "unrelated resize remains default"

                    if args.Head <> "--window-maximize" then
                        Expect.equal overlay.Maximize baseline.Maximize "unrelated maximize remains default"

                    if args.Head <> "--window-startup" then
                        Expect.equal overlay.Startup baseline.Startup "omitted startup remains safe default"

                    if args.Head <> "--window-position" then
                        Expect.equal overlay.Position baseline.Position "unrelated position remains default"

                    if args.Head <> "--window-backend" then
                        Expect.equal overlay.Backend baseline.Backend "unrelated backend remains default"
            }
        ]
//#endif
                ...
                let translated =
                    Translate(
                        (5.0, 5.0),
                        {
                            Nodes =
                                [
                                    Text((0.0, 0.0), "hello", black)
                                    SizedText((0.0, 0.0), "sized", 12.0, black)
                                ]
                        }
                    )
                    ...
                    CachedSubtree
                        {
                            ...
                            Scene =
                                {
                                    Nodes = [ Text((1.0, 1.0), "cached", black) ]
                                }
                        }
    ...
    testList
        ...
        [
            test "generated headless product exposes scene contract" {
                let scene: FS.GG.UI.Scene.Scene =
                    {
                        Nodes = [ AppRoot.Program.view initialModel ]
                    }
            ...
            test "generated headless layout evidence is readable" {
                let report =
                    AppRoot.Program.layoutEvidenceForSize { Width = 640; Height = 480 } initialModel
...
let behaviorTests =
    testList
        "product-behavior"
        [
            test "generated product test suite is wired" { Expect.equal 1 1 "product tests run" }
            ...
            test "game default view renders the playfield, ball, paddles and score as a scene" {
                ...
                Expect.isTrue
                    (List.length nodes >= 5)
                    "game view draws the playfield, ball, two paddles and a score HUD"
                ...
                let missed =
                    { model0 with
                        Ball =
                            {
                                Pos = vec2 18.0 10.0
                                Velocity = vec2 -8.0 model0.Ball.Velocity.Vy
                            }
                        LeftPaddleY = 300.0
                    }
            ...
            test
                "generated game host boots the shell main menu and cues the initial state through the same seam (#991/#1000/#458)" {
            ...
            test
                "interactive shell host proves default geometry, retained menu/rebind clicks, and key down -> ticks -> up (#1012)" {
                let host = AppRoot.Program.interactiveHost
                ...
                Expect.equal
                    defaultWindow.StartupState
                    ViewerWindowStartupState.Normal
                    "default launch explicitly uses the shell's Windowed behavior"
                ...
                Expect.equal
                    startProof.Verdict
                    FS.GG.UI.Controls.Elmish.Responsive
                    "retained Start click visibly enters play"

                Expect.equal
                    playing.Shell.Screen
                    AppRoot.GameShell.Playing
                    "the native retained pointer route dispatches Start"

                let raw key isDown = host.MapKey key isDown |> Option.get

                let tickMessage () =
                    host.Tick(TimeSpan.FromMilliseconds 16.0) |> Option.get
                ...
                Expect.isTrue
                    (afterTick1.Play.LeftPaddleY < keyDown.Play.LeftPaddleY)
                    "held W advances on the first fixed tick"

                Expect.isTrue
                    (afterTick2.Play.LeftPaddleY < afterTick1.Play.LeftPaddleY)
                    "held W remains active across a second fixed tick"

                Expect.equal
                    afterReleaseTick.Play.LeftPaddleY
                    keyUp.Play.LeftPaddleY
                    "key-up clears the held control before the next tick"
                ...
                Expect.equal
                    configProof.Verdict
                    FS.GG.UI.Controls.Elmish.Responsive
                    "retained Config click visibly opens settings"
                ...
                Expect.equal
                    rebindProof.Verdict
                    FS.GG.UI.Controls.Elmish.Responsive
                    "retained rebind-row click visibly enters capture"
                ...
                Expect.exists
                    viewerEffects
                    (function
                    | ApplyLogicalCanvas size -> size = resolution1080
                    | _ -> false)
                ...
                for mode in
                    ...
                    let modeViewerEffects =
                        modeEffects |> List.collect AppRoot.EvidenceCommands.viewerEffectsForShellEffect

                    Expect.exists
                        modeViewerEffects
                        (function
                        | ApplyWindowOptions _ -> true
                        | _ -> false)
                ...
                let frame1080 =
                    FS.GG.UI.Controls.Control.renderTree host.Theme resolution1080 (host.View resolution1080 model1080)
                ...
                let configBounds =
                    (layout1080.Bounds |> List.find (fun b -> b.NodeId = "config")).Bounds
                ...
                let routedX, routedY =
                    LogicalCanvas.toLogicalPoint resolution1080 surface physicalX physicalY
                    ...
                    { AppRoot.Program.interactiveHost with
                        ...
                        Diagnostics =
                            { Viewer.defaultDiagnostics with
                                Categories =
                                    Set.add ViewerDiagnosticCategory.Window Viewer.defaultDiagnostics.Categories
            ...
            test "generated game host PLAYS THROUGH: a pressed key reaches gameplay, not just a message" {
                ...
                let played, _ =
                    GeneratedAppHost.runKeyScriptToModel host [ downKey "w"; downKey "w" ]
            ...
            test "generated game host SOURCES the intents a player must reach (handled-but-unwired guard)" {
                ...
                let reachable =
                    GeneratedAppHost.reachableMessages host probe (Some(TimeSpan.FromMilliseconds 16.0))
            ...
            test "generated game host boundary keeps app commands separate from viewer effects" {
                ...
                let hosted, appCommands, viewerEffects =
                    AppRoot.Program.interpretAtHostBoundary (Tick oneStep) model0
                ...
                Expect.exists
                    ...
                    | RenderScene _ -> true
                    | _ -> false)
                    "host boundary emits a render effect separately"
            }

            test "game layout evidence re-points HUD onto the score strip and gameplay onto the playfield" {
                let report =
                    AppRoot.Program.layoutEvidenceForSize { Width = 640; Height = 480 } AppRoot.Program.initialModel
            ...
            test "game active item (the ball) stays inside the playfield region" {
                ...
                let ticked =
                    List.replicate 30 (Tick oneStep)
                    |> List.fold (fun m msg -> fst (AppRoot.Program.update msg m)) model0
                ...
                Expect.isTrue
                    (AppRoot.Program.boundsInside region.Bounds bounds.Bounds)
                    "the ball stays inside the playfield region"
                ...
                Expect.isTrue
                    (AppRoot.Program.collisionUsesGameplayRegion size ticked)
                    "collision policy is region based"
            }
            ...
            test "generated default game dispatches input, advances over time, and keeps evidence flags opt-in" {
                ...
                let source =
                    System.IO.File.ReadAllText(
                        System.IO.Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "src", "Product", "Program.fs")
                    )
                ...
                Expect.isFalse
                    ...
                    "launch evidence flag stays out of normal launch branch"

                Expect.isFalse
                    (defaultBranch.Contains("--bounded-smoke"))
                    "bounded smoke flag stays out of normal launch branch"

                Expect.isFalse
                    (defaultBranch.Contains("self-closed-for-evidence=true"))
                    "normal launch does not report evidence self-close"
            }
            ...
            test "window diagnostics verdict matches the real runtime gate (#136 / epic #134)" {
                let dir =
                    System.IO.Path.Combine(
                        ...
                        "window-diagnostics-" + System.Guid.NewGuid().ToString("N")
                    )
                ...
                Expect.stringContains
                    output
                    ("persistent-window-supported=" + supportedText)
                    "probe reports the real runtime-capability gate verbatim, not a fabricated verdict"

                Expect.stringContains
                    output
                    "diagnostic-class=window-visibility"
                    "probe still enumerates the window-visibility class"

                Expect.isFalse
                    (output.Contains "visible=observed:false")
                    "probe never fabricates an observed window-invisibility"

                Expect.isFalse
                    (output.Contains "status=failed")
                    "probe never reports a failed status it did not observe"

                Expect.isFalse
                    (output.Contains "taskbar-only" && output.Contains "status=ok")
                    "taskbar-only is never reported ok"

                if capability.PersistentWindow then
                    Expect.isFalse
                        (output.Contains "status=unsupported")
                        "a window-capable host is never told a live window is impossible"
                else
                    Expect.stringContains
                        output
                        "status=unsupported"
                        "an unsupported host is reported unsupported (matching the real launch), not failed"
            }
            ...
            test "view-image probe renders requested logical sizes and rejects invalid dimensions (#901/#1030)" {
                ...
                for width, height in [ 320, 200; 901, 507 ] do
                    let dir =
                        System.IO.Path.Combine(
                            System.IO.Path.GetTempPath(),
                            "view-image-" + System.Guid.NewGuid().ToString("N")
                        )
                    ...
                    Expect.stringContains
                        ...
                        "records the requested logical size"
                    ...
                    if metadata.Contains "status=ok" then
                        ...
                        Expect.stringContains
                            ...
                            "metadata records the PNG-header size"

                        Expect.stringContains
                            metadata
                            "dimensions-match=True"
                            "metadata proves requested and actual dimensions agree"

                        Expect.stringContains
                            metadata
                            "renders-full-view=true"
                            "the probe renders the full product view"
                    else
                        Expect.stringContains
                            metadata
                            "status=unsupported"
                            "an unsupported host reports unsupported (never fabricated pixels)"

                let invalidPath =
                    System.IO.Path.Combine(
                        System.IO.Path.GetTempPath(),
                        "view-image-invalid-" + System.Guid.NewGuid().ToString("N") + ".png"
                    )

                let invalid =
                    AppRoot.EvidenceCommands.tryRunEvidenceCommand [ "--view-image"; invalidPath; "0"; "720" ]

                Expect.equal invalid (Some 1) "non-positive explicit dimensions return a nonzero command result"

                Expect.isFalse
                    (System.IO.File.Exists invalidPath)
                    "invalid dimensions never fall back to a default-size image"

                let oversizedPath =
                    System.IO.Path.Combine(
                        System.IO.Path.GetTempPath(),
                        "view-image-oversized-" + System.Guid.NewGuid().ToString("N") + ".png"
                    )
            ...
            // host launch needed.
            test
                "play-input boundary is surfaced at the input-wiring site and accurate to the host contract (#139/#1000)" {
                let readAppRootFile parts =
                    System.IO.File.ReadAllText(
                        System.IO.Path.Combine(Array.append [| __SOURCE_DIRECTORY__; ".."; ".." |] parts)
                    )
                ...
                Expect.stringContains
                    modelSource
                    "HOST INPUT BOUNDARY"
                    "Model.fs surfaces the input-wiring boundary at the input-wiring site"

                Expect.stringContains
                    (modelSource.ToLowerInvariant())
                    "keyboard-only"
                    "the boundary states the play-input mapping (paddleForKey) is keyboard-only"

                Expect.stringContains
                    modelSource
                    "runInteractiveApp"
                    "the boundary names the pointer-aware interactive host the turnkey default now launches on"

                Expect.stringContains
                    modelSource
                    "MapPointer"
                    "the boundary names the pointer seam the interactive host adds"

                // A2 — accurate: the emitted ViewerKey has NO mouse/pointer case (keyboard keys only).
                let keyboardFsi =
                    readAppRootFile [| "docs"; "api-surface"; "KeyboardInput"; "KeyboardInput.fsi" |]
                ...
                Expect.isFalse
                    (viewerKeyBlock.Contains "Pointer")
                    "ViewerKey has no pointer case — the note's core claim"
                ...
                let viewerFsi =
                    readAppRootFile [| "docs"; "api-surface"; "SkiaViewer"; "SkiaViewer.fsi" |]
                ...
                Expect.isFalse
                    (generatedHostBlock.Contains "MapPointer")
                    "the default host has NO pointer seam (keyboard-only) — the boundary is real"

                let interactiveHostBlock = typeBlock viewerFsi "InteractiveViewerHost"

                Expect.stringContains
                    interactiveHostBlock
                    "MapPointer:"
                    "the pointer-aware interactive host is where the mouse seam actually lives"
                // the note's author-facing signpost names must exist in the shipped contract (accuracy of FR-002)
                Expect.stringContains
                    viewerFsi
                    "InteractiveAppHost"
                    "the note's named pointer-aware host type is real in the shipped surface"

                Expect.stringContains
                    viewerFsi
                    "runInteractiveApp"
                    "the note's named interactive-host entry point is real in the shipped surface"
            }
        ]
//#else
open FS.GG.UI.Controls
        ...
        [
            ...
            test "product-owned controls example is wired" {
                ...
                Expect.isGreaterThan
                    (Control.count view)
                    7
                    "product example owns form, rich text, chart, graph, and DataGrid controls"
            }

            test "product-owned form chart and DataGrid controls are constructible" {
                let textBox =
                    TextBox.create [ TextBox.value initialModel.Name; TextBox.onChanged NameChanged ]

                let lineChart = LineChart.create [ LineChart.series initialModel.Revenue ]

                let dataGrid =
                    DataGrid.create initialModel.GridColumns [ DataGrid.rows initialModel.GridRows ]
            ...
            test "default view renders real controls through the framework render path" {
                ...
                Expect.isGreaterThan
                    (List.length nodes)
                    (Control.count (controlsExampleView initialModel))
                    "renderTree paints nested controls (more nodes than the control count)"

                Expect.stringContains
                    text
                    "Product controls"
                    "the rendered scene shows the example TextBlock's real text"
            ...
            test "pointer click on the Save control routes its bound message (SC-003)" {
                ...
                let state1, downMsgs =
                    ControlsElmish.routeInteractivePointer
                        host
                        (Pointer.init ())
                        size
                        model0
                        (pointer ViewerPointerPhaseKind.Pressed cx cy)
                ...
                let _state2, upMsgs =
                    ControlsElmish.routeInteractivePointer
                        ...
                        size
                        model0
                        (pointer ViewerPointerPhaseKind.Released cx cy)
                ...
                Expect.contains
                    routed
                    SaveRequested
                    "press+release on the Save control dispatches its bound SaveRequested message"
            ...
            test "generated graphical app navigates pages through viewer key events" {
                ...
                Expect.exists
                    browse.InputDiagnostics
                    (fun item -> item.Flow = "home-open" && item.RawKey = Some "Enter")
                    "diagnostic names the viewer input flow"
            }
            ...
            test "pure generated app transitions expose model message and effect behavior" {
                let started, startEffects =
                    AppRoot.Program.update (ViewerInput(Enter, true)) initialModel

                let interacted, interactionEffects =
                    AppRoot.Program.update (ViewerInput(ArrowLeft, true)) started
            ...
            test "generated host cues a real transition through the audio seam" {
                ...
                let expected =
                    AppRoot.AudioCues.forTransition
                        SaveRequested
                        initialModel
                        (fst (AppRoot.Program.update SaveRequested initialModel))

                Expect.isNonEmpty
                    expected
                    "the Controls starter cues `SaveRequested` — an empty cue would make this test vacuous"
                ...
                Expect.equal
                    cued
                    expected
                    "Update lifts the transition's cues onto ViewerEffect.PlayAudio for the audio sink"
            }
            ...
            test "generated host boundary keeps app commands separate from viewer effects" {
                ...
                let hosted, observedAppCommands, viewerEffects =
                    AppRoot.Program.interpretAtHostBoundary SaveRequested initialModel

                let hostUpdated, hostViewerEffects =
                    AppRoot.Program.generatedHost.Update SaveRequested initialModel
                ...
                Expect.exists
                    appCommands
                    (function
                    | DispatchHostCommand "save:Product" -> true
                    | _ -> false)
                    "pure update emits an app command"

                Expect.equal observedAppCommands appCommands "host boundary exposes app commands before interpretation"

                Expect.exists
                    (observedAppCommands |> List.map AppRoot.Program.appCommandName)
                    ((=) "app-command:dispatch-host-command:save:Product")
                    "app command category is named separately"
                ...
                Expect.exists
                    ...
                    | RenderScene _ -> true
                    | _ -> false)
                    "host boundary emits viewer render effect separately"
                ...
                Expect.isNonEmpty
                    (renderedScenes hostViewerEffects)
                    "generated host returns render effects to SkiaViewer"
            ...
            test "generated layout evidence separates summary and content regions at default and constrained sizes" {
                let defaultReport =
                    AppRoot.Program.layoutEvidenceForSize { Width = 1280; Height = 720 } initialModel
                ...
                |> List.iter (fun report ->
                    ...
                    Expect.equal
                        ...
                        "generated layout evidence reports the measurement mode"

                    Expect.isEmpty
                        report.UnsupportedReasons
                        "readable generated layout does not use unsupported-host classification")
            }

            test "generated layout validation fails broken summary and content layouts" {
                let summaryOverlap =
                    AppRoot.Program.layoutEvidenceForSize { Width = 480; Height = 480 } initialModel
                ...
                Expect.equal
                    contentResult.FailureClass
                    (Some OverlappingLayoutBounds)
                    "summary/content overlap is classified"
            }
            ...
            test "generated content policies use the content region for the active item and bounds" {
                ...
                let bounds =
                    AppRoot.Program.activeGameplayBoundsForSize { Width = 640; Height = 480 } ticked

                Expect.isTrue
                    (AppRoot.Program.boundsInside region.Bounds bounds.Bounds)
                    "active item remains inside the content region"

                Expect.isTrue
                    (AppRoot.Program.movementUsesGameplayRegion { Width = 640; Height = 480 } ticked)
                    "movement policy is region based"

                Expect.isTrue
                    (AppRoot.Program.spawnUsesGameplayRegion { Width = 640; Height = 480 } initialModel)
                    "spawn policy is region based"

                Expect.isTrue
                    (AppRoot.Program.collisionUsesGameplayRegion { Width = 640; Height = 480 } ticked)
                    "collision policy is region based"
            }
            ...
            test "generated default app dispatches input, advances over time, and keeps evidence flags opt-in" {
                ...
                let compactDefaultBranch =
                    System.Text.RegularExpressions.Regex.Replace(defaultBranch, @"\s+", " ")
                ...
                Expect.stringContains
                    ...
                    "controls-family normal launch uses the pointer-aware persistent host with the parsed window behavior and #429/#436 audio sink"
                //#else
                Expect.stringContains
                    compactDefaultBranch
                    ...
                    "game/sample-pack normal launch uses the keyboard-only persistent host with the parsed window behavior and #245 audio sink"
                //#endif
                Expect.isFalse
                    (defaultBranch.Contains("--launch-evidence"))
                    "launch evidence flag stays out of normal launch branch"
