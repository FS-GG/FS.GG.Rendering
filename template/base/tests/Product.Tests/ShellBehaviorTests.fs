...
let private displayEffects effects =
    ...
    |> List.choose (function
        | DisplayChanged d -> Some d
        | _ -> None)

let private keymapEffects effects =
    effects
    |> List.choose (function
        | KeymapChanged k -> Some k
        | _ -> None)
...
let private atSettingsFromMenu () =
    fst (update OpenSettings (init testConfig))
...
let shellRouterTests =
    testList
        "game-shell router + menu (#991/#1002)"
        [
            test "init parks on the main menu with the game's keymap and display" {
                ...
                Expect.equal
                    (Keymap.resolve m.Keymap wKey)
                    (Some jump)
                    "the shell starts with the game's default bindings"
            }
...
let shellEscapeTests =
    testList
        "game-shell Esc routing (#991/#1002)"
        [
            ...
            test "Esc cancels an in-flight rebind before it routes any screen" {
                ...
                Expect.equal
                    cancelled.Screen
                    Settings
                    "and does NOT also back out of settings — the capture ate the Esc"
...
let shellRebindTests =
    testList
        "game-shell rebind capture (#991/#1002)"
        [
            test "rebind round-trip: ArmRebind then CaptureKey rebinds the command and emits KeymapChanged" {
                ...
                Expect.equal
                    (Keymap.resolve armed.Keymap qKey)
                    None
                    "the key to be captured is unbound before the capture (so the rebind has teeth)"
                ...
                Expect.equal
                    (Keymap.resolve captured.Keymap qKey)
                    (Some jump)
                    "the captured key now resolves to the rebound command (an upsert-by-key onto the keymap)"

                Expect.equal
                    (Keymap.resolve captured.Keymap wKey)
                    None
                    "command replacement removes the selected command's old binding"

                Expect.equal
                    (Keymap.resolve captured.Keymap fKey)
                    (Some fire)
                    "an unrelated binding is left untouched by the rebind"

                match keymapEffects capFx with
                | [ k ] ->
                    Expect.equal
                        (Keymap.resolve k qKey)
                        (Some jump)
                        "the emitted KeymapChanged carries the rebound keymap for the host to persist"
            ...
            test "capturing another action's key displaces it but keeps its catalog row, and reset restores defaults" {
                ...
                Expect.equal
                    (Keymap.resolve displaced.Keymap fKey)
                    (Some jump)
                    "the intended command owns the captured key"
...
let shellKeyRoutingTests =
    ...
    testList
        "game-shell routeKeyDown (#991/#1002)"
        [
            ...
            test "chrome screens never feed the keymap — a bound key is inert off the play screen" {
                Expect.equal
                    (routeKeyDown toGame wKey (init testConfig))
                    NoInput
                    "a bound key at the main menu resolves to nothing"

                Expect.equal
                    (routeKeyDown toGame wKey (atPaused ()))
                    NoInput
                    "a bound key on the pause overlay resolves to nothing"
            }
            ...
            test "a capture in flight swallows the very next key as CaptureKey (even a live-play binding)" {
                ...
                Expect.equal
                    (routeKeyDown toGame fKey armed)
                    (ShellMsg(CaptureKey fKey))
                    "the next key is handed to the capture, not resolved as a command"

                Expect.equal
                    (routeKeyDown toGame menuKey armed)
                    (ShellMsg(CaptureKey menuKey))
                    "even the menu key is captured (the reducer turns that into a cancel)"
            }
            ...
            test "routeKeyEvent preserves both gameplay edges on one normalized seam" {
                ...
                Expect.equal
                    (routeKeyEvent toGame wKey true playing)
                    (GameEdge(jump, true))
                    "key-down begins the resolved gameplay control"

                Expect.equal
                    (routeKeyEvent toGame wKey false playing)
                    (GameEdge(jump, false))
                    "key-up ends the same resolved gameplay control"

                Expect.equal
                    (routeKeyEvent toGame menuKey false playing)
                    NoKeyEvent
                    "shell chrome never reacts to key-up"
            }
        ]
...
let shellDisplayTests =
    testList
        "game-shell display mapping (#991/#1002)"
        [
            test
                "SetResolution / SetDisplayMode update the display and emit DisplayChanged; logicalSize follows the resolution" {
                let m0 = init testConfig
                ...
                Expect.equal
                    (displayEffects resFx)
                    [ resized.Display ]
                    "the change emits a DisplayChanged carrying the new display for the host to re-apply"

                Expect.equal
                    (logicalSize resized.Display)
                    res1080
                    "logicalSize is the chosen resolution (#246 letterbox source)"
            ...
            test "windowBehavior maps each display mode to its window startup state and resize policy" {
                let behaviourOf mode =
                    windowBehavior { Resolution = res720; Mode = mode }

                let windowed = behaviourOf Windowed

                Expect.equal
                    windowed.StartupState
                    ViewerWindowStartupState.Normal
                    "Windowed starts as a normal, resizable window"
                ...
                Expect.equal
                    borderless.StartupState
                    ViewerWindowStartupState.WindowedFullscreen
                    "Borderless is a work-area windowed-fullscreen"
...
let shellPersistenceTests =
    testList
        "game-shell keymap persistence (#991/#1002)"
        [
            test "encodeKeymap / decodeKeymap round-trips a rebound keymap onto a fresh model" {
                ...
                Expect.equal
                    (Keymap.resolve restored.Keymap qKey)
                    (Some jump)
                    "the rebound key survives the encode/decode round-trip"

                Expect.equal
                    (Keymap.resolve restored.Keymap fKey)
                    (Some fire)
                    "and the other bindings come back intact after a restore"
            }
            ...
            test "decodeKeymap degrades to the current keymap on a corrupt blob (total, never throws)" {
                ...
                Expect.equal
                    (Keymap.resolve kept.Keymap wKey)
                    (Some jump)
                    "a corrupt save is dropped and the current bindings are kept, not thrown away"
            }
