...
// Mirrored from FS-GG/FS.GG.Game @ 0.4.0 (src/Game.Core/Physics.fsi); regenerate when $(FsGgGameVersion) moves.
...
/// they settle. `interpolate` blends the double buffer into presentation-only poses on the shortest arc.
///
...
///
/// **Arcade resolution stays the first-class default.** `Resolution.pushOut`/`slide`/`knockback` remain
/// what most games use, and `Physics` is opt-in. Neither module knows the other.
///
/// Every type here is scoped to `Physics` rather than promoted alongside `Point`/`Rect`/`Contact` — the
/// rule `Grids.Edge` and `Visibility.Segment` already follow. It also keeps names as collision-prone as
/// `Static`, `Dynamic`, `Shape` and `Config` out of a namespace games `open` wholesale.
...
module Physics =
    ...
    /// input assumption `Geometry.polygonContact` makes.
    ///
    /// A degenerate shape — a non-positive or non-finite radius or half-extent, or a ring `Geometry` would
    /// reject as malformed (fewer than 3 vertices, a non-finite coordinate, or zero area) — is a
    ...
    /// Public contract type exposed by the FS.GG.Game.Core package.
    /// The contact-response coefficients of a body. `Restitution` is the bounciness `e`; `Friction` is the
    /// Coulomb coefficient `μ`. Both are read by the solver, not by the broad phase.
    ///
    /// A contact combines BOTH bodies' coefficients: restitution as the **maximum** of the two (a floor is
    /// normally `0`, and taking the minimum would make every ball dead on every floor), and friction as the
    /// geometric mean `sqrt(μa·μb)` (so one frictionless body slides on anything).
    ...
    /// `SleepTicks`, a non-positive `SleepLinearSq`, or a non-positive `SleepAngular`. Warm starting has
    /// none — it changes how fast the solver converges, not what it converges to, and is always on.
    type Config =
        {
    ...
    /// broad-phase index. **Two worlds equal in presentation may differ in this state** — so compare worlds
    /// with `checksum`, never structurally.
    type World

    /// Public contract type exposed by the FS.GG.Game.Core package.
    /// The presentation pose of one body: where to draw it (`Position` — the body origin) and how it is
    /// turned (`Rotation`, radians). It is the projection a renderer consumes, deliberately NOT a `World`:
    /// it carries no velocity, no inverse mass, and none of the sleep or warm-start cache — none of which
    /// is drawn or blended. `interpolate` produces an array of these, one per body.
    ...
    /// reports pairs in terms of them. `position` is the body's origin; `shape` is expressed relative to it.
    ///
    /// A degenerate `shape` or a non-finite `position` is accepted and indexed (so later indices do not
    /// shift), but the body collides with nothing — see `Shape`.
    ///
    /// Cost is O(body count): the world's arrays are copied. This is the *build* path, not the step path.
    /// Adding many bodies this way is O(N²), a copy per body; reach for `addBodies` when you have a batch.
    val addBody:
        kind: BodyKind -> shape: Shape -> material: Material -> position: Point -> world: World ->
            struct (int * World)

    /// Public contract function exposed by the FS.GG.Game.Core package.
    /// Append a *batch* of bodies in one pass and return `struct(indices, world')`, where `indices.[k]` is
    /// the identity of the `k`-th body of `bodies` — exactly the indices the same bodies added one at a
    /// time through `addBody` would receive.
    ///
    ...
    /// `addBody` calls are O(N²). `addBodies bodies` and folding `addBody` over the same `bodies` produce
    /// worlds with identical `checksum`s and identical `pairs`.
    ///
    /// An empty `bodies` returns `struct([||], world)` — the world unchanged.
    val addBodies:
        bodies: seq<BodyKind * Shape * Material * Point> -> world: World -> struct (int[] * World)
    ...
    /// physics step, so the order is part of this contract rather than an accident of the container.
    ///
    /// A pair is returned exactly when all three hold: **at least one body is `Dynamic`** (no other pair can
    /// ever resolve); **both bodies are collidable** (no degenerate shape, no non-finite position or
    /// rotation); and **their world-space AABBs overlap on positive area**, decided by `Geometry.intersects`
    /// and so inheriting its **strict-edge convention: two boxes that merely touch are not a pair.**
    ///
    ...
    /// allowed. Pure, total and deterministic. Never throws.
    ...
    /// `A` and `B` are `a` and `b` — the body indices — and `Normal` is the unit vector pointing from `a`
    /// toward `b`, whichever order they are given in. `Depth` is the penetration along it, and `Points`
    /// holds `PointCount` contact points: up to 2 for a polygon pair, always 1 where a circle is involved.
    ///
    /// The **strict-edge convention** is `Geometry`'s throughout: a touch is not a contact. This agrees with
    /// `pairs`, so a pair the broad phase refuses can never have had a contact to lose.
    ///
    /// `FeatureId` identifies the contacting features and is stable across ticks for an unmoving pair — it
    /// is what `step`'s warm-start cache is keyed on. **Opaque: compare it, do not decode it.**
    ///
    /// Pure, total and deterministic. An out-of-range or equal index, a degenerate shape, a non-finite
    /// position or rotation, and two exactly coincident circle centres all yield `ValueNone` rather than
    /// throwing or inventing a normal.
    ...
    /// `Loop.advance dt Physics.step frameTime state` typechecks with no adapter. Reads `dt` and the world,
    /// and nothing else — never a clock, never `alpha`.
    ///
    ...
    /// CONSECUTIVE ticks stops. A sleeping body is **immovable, not absent**: it has infinite mass while it
    /// sleeps, exactly as a `Static` body does, and things pile up on it rather than through it. A sleeper
    /// wakes when a body that is **actually moving** touches it. The consequence to know: a wake travels at
    /// most one body per tick, and only through bodies that visibly move — a sleeper that takes a load
    /// without shifting does not pass it on.
    ///
    /// **Determinism.** Manifolds are sorted by `(A, B, FeatureId)` before solving; iteration counts are
    /// fixed `int`s; every divide and `sqrt` is guarded. Identical worlds stepped by identical `dt`
    /// sequences produce identical worlds, and so identical `checksum`s.
    ///
    /// Total: a non-finite or non-positive `dt` returns the world unchanged, as does an empty world. Never
    /// throws.
    val step: world: World -> dt: float -> World

    /// Public contract function exposed by the FS.GG.Game.Core package.
    /// Blend the two double-buffered worlds `Loop.advance` keeps — `previous` and `current` — into the poses
    /// to draw this frame, at `Loop.alpha`'s fraction `alpha`. Returns one `Transform` per body of
    /// `current`, in index order.
    ///
    /// **Presentation only.** Only `Position` and `Rotation` interpolate. Velocity and the solver's
    /// cross-tick cache are NOT blended: blending a warm-start cache is meaningless, and two worlds equal in
    /// pose may hold different caches.
    ///
    /// **Shortest arc.** `Rotation` interpolates the short way around the circle. A body turning through `π`
    /// — `previous ≈ +3.13`, `current ≈ -3.13` — crosses `+π`; it does not unwind the long way through `0`.
    /// The naive `a0 + (a1 - a0)·alpha` gets this wrong and spins a turret backwards at 60 fps.
    ///
    ...
    /// `previous` is returned at its `current` transform. Total: never throws.
    val interpolate: alpha: float -> previous: World -> current: World -> Transform[]

    /// Public contract function exposed by the FS.GG.Game.Core package.
    /// A stable 64-bit hash of every body's position, velocity, rotation and angular velocity — the desync
    /// tripwire. Compare it across a replayed input stream, or against a golden value in a test, to catch
    /// the tick on which two runs diverged.
    ///
    ...
    ///
    /// Stable across runs, processes and runtimes: it is FNV-1a over the IEEE-754 bits, not `GetHashCode`.
    /// `-0.0` hashes as `0.0`, and every NaN hashes alike. Float arithmetic is byte-deterministic on a fixed
    /// compiler and ISA; a cross-platform lockstep guarantee needs fixed-point, which is a later, ADR'd
    /// decision.
