...
let private evaluateProvenance workload =
    let validate (receipt: JourneyReceipt) =
        ...
        [
            ...
            if
                not (String.Equals(string (JourneyReceipt.origin receipt), expectedOrigin, StringComparison.Ordinal))
            then
...
let private declarationPattern =
    Regex(@"Authorship\s*=\s*(?:Placeholder\s+""[^""]*""|Authored\s+""[^""]*"")", RegexOptions.CultureInvariant)

let private debtPattern =
    Regex(@"BlockingDebt\s*=\s*(?:None|Some\s+""[^""]*"")", RegexOptions.CultureInvariant)
...
let private workloadSourceFingerprint id =
    ...
    if not (File.Exists sourcePath) then
        ...
        let finish =
            source.IndexOf(endMarker, max 0 (start + beginMarker.Length), StringComparison.Ordinal)
...
let private canonicalFloat (builder: StringBuilder) (value: float) =
    if Double.IsNaN value then
        builder.Append "nan" |> ignore
    elif Double.IsPositiveInfinity value then
        builder.Append "inf" |> ignore
    elif Double.IsNegativeInfinity value then
        builder.Append "-inf" |> ignore
    else
        builder.Append(value.ToString("R", CultureInfo.InvariantCulture)) |> ignore
...
let private keyValuePairDefinition =
...
and private canonicalWriteStructured (builder: StringBuilder) (value: obj) : unit =
    ...
    if valueType.IsEnum then
        builder.Append(valueType.FullName).Append('.').Append(Enum.GetName(valueType, value))
        |> ignore
    elif
        valueType.IsGenericType
        && valueType.GetGenericTypeDefinition() = keyValuePairDefinition
    then
...
let private structuralFingerprint value =
    try
        ...
        builder.ToString() |> normalizeFingerprintText |> Option.map sha256Text
    with _ ->
        None
...
let private messageFramesFingerprint workload =
    try
        ...
        [
            for frame in 0 .. max workload.WarmupFrames workload.SampleFrames - 1 -> frame, workload.MessageAt frame
        ]
...
let private workloadDefinitionFingerprint workload =
    match
        workloadSourceFingerprint workload.Id, initialStateFingerprint workload, messageFramesFingerprint workload
    with
...
let definitionDigest workload =
    ...
    let structuralBudgets =
        String.concat "," performanceIntentSeed.StructuralCostBudgets
        ...
        observedScale <-
            ...
            |> List.fold
                (fun scales id ->
                    ...
                    scales
                    |> Map.change id (fun previous -> Some(max count (previous |> Option.defaultValue 0))))
                observedScale
...
let private costDriverProblems (results: WorkloadResult list) =
    let workloadById =
        expectedWorkloads
        |> List.map (fun workload -> workload.Id, workload)
        |> Map.ofList

    let resultById =
        results |> List.map (fun result -> result.Workload.Id, result) |> Map.ofList

    let driverById =
        performanceCostDrivers
        |> List.map (fun driver -> driver.Id, driver)
        |> Map.ofList

    let duplicateDriverIds = performanceCostDrivers |> List.map _.Id |> duplicateValues

    let inventoryVisuals =
        performanceCostDrivers |> List.choose _.VisualElement |> List.sort
            ...
            | RequiredIn workloadIds ->
                ...
                for workloadId in workloadIds do
                    ...
                    | Some workload, Some result ->
                        ...
                        let observed =
                            result.ObservedScale |> Map.tryFind driver.Id |> Option.defaultValue 0
...
let private criticInputDigest (results: WorkloadResult list) coverageProblems =
    let intent =
        performanceIntentDeclaration.WorkloadDefinitionDigests |> String.concat ","
...
let private declarationProblems () =
    ...
    let duplicateBindings =
        performanceIntentDeclaration.WorkloadDefinitionDigests |> duplicateValues
...
let writePerformanceIntentDeclaration (path: string) =
    ...
    let optional name value =
        value
        |> Option.map (fun actual -> $"  {name}: {yamlScalar actual}")
        |> Option.toList
...
let private writeIntentJson (json: Utf8JsonWriter) =
    ...
    intent.DeferralIssue
    |> Option.iter (fun value -> json.WriteString("deferralIssue", value))
    ...
    intent.Rationale
    |> Option.iter (fun value -> json.WriteString("rationale", value))
...
let writeExpectedWorkloadEvidence (path: string) =
    ...
    for result in results do
        ...
        result.ObservedScale
        |> Map.iter (fun name value -> json.WriteNumber(name, value))
...
let writePerformanceCriticRequest (path: string) =
    ...
    let evidenceDigest =
        SHA256.HashData evidenceBytes |> Convert.ToHexString |> _.ToLowerInvariant()

    use evidence = JsonDocument.Parse evidenceBytes

    let inputDigest =
        evidence.RootElement.GetProperty("critic").GetProperty("inputDigest").GetString()
