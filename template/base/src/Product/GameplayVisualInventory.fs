...
let private scored =
    { initialModel with
        LeftScore = initialModel.LeftScore + 1
    }
...
let registeredBindings =
    bindings |> List.map (fun item -> elementId item.Element, item.Handle)

let representativeModels = [ initialModel; stepped; movedLeft; scored ]
