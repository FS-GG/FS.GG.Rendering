...
type Model = { Name: string; RenderCount: int }

type Msg =
    | Rendered
    | NoOp

let initialModel = { Name = "Product"; RenderCount = 0 }

let update msg model =
    match msg with
    | Rendered ->
        { model with
            RenderCount = model.RenderCount + 1
        },
        []
    | NoOp -> model, []
...
let movePaddle side direction model =
    ...
    | LeftSide ->
        { model with
            LeftPaddleY = clampPaddle model (model.LeftPaddleY + delta)
        }
    | RightSide ->
        { model with
            RightPaddleY = clampPaddle model (model.RightPaddleY + delta)
        }
...
let stepSim model =
    ...
    let velocityY, clampedY =
        if next.Vy < ballRadius then
            -ball.Velocity.Vy, ballRadius
        elif next.Vy > model.Playfield.Vy - ballRadius then
            -ball.Velocity.Vy, model.Playfield.Vy - ballRadius
        else
            ball.Velocity.Vy, next.Vy

    let withinLeftPaddle =
        clampedY >= model.LeftPaddleY
        && clampedY <= model.LeftPaddleY + model.PaddleHeight

    let withinRightPaddle =
        clampedY >= model.RightPaddleY
        && clampedY <= model.RightPaddleY + model.PaddleHeight
...
let private advanceSim dtSeconds model =
    let struct (steps, accumulator) =
        FixedStep.drain simInterval dtSeconds model.SimAccumulator
...
let richIntro =
    ...
    { RichText.block
          [
              RichText.run "Product-owned " accent
              RichText.run "Controls guidance" baseStyle
          ] with
        MaxWidth = Some 360.0
        Clip = true
    }
...
let transitionViewerInput raw direction key isDown model =
    if not isDown then
        ...
        let nextPage, interactions, contentColumn, contentRow, flow, expected =
            ...
            | Home, Letter 'S' ->
                Settings, model.Interactions, model.ContentColumn, model.ContentRow, "settings-open", "settings"
            | Settings, Enter ->
                Browse, model.Interactions, model.ContentColumn, model.ContentRow, "settings-apply", "browse"
            | Settings, Escape
            | Settings, Backspace ->
                Home, model.Interactions, model.ContentColumn, model.ContentRow, "settings-back", "home"
            | Browse, Enter ->
                Detail, model.Interactions, model.ContentColumn, model.ContentRow, "open-detail", "detail"
            | Browse, ArrowLeft ->
                Browse,
                model.Interactions + 1,
                max 0 (model.ContentColumn - 1),
                model.ContentRow,
                "content-move",
                "browse"
            | Browse, ArrowRight ->
            ...
            | Browse, ArrowUp ->
                ...
                model.Interactions + 1,
                model.ContentColumn,
                max 0 (model.ContentRow - 1),
                "content-move",
                "browse"
            | Detail, Escape
            | Detail, Backspace ->
                Browse, model.Interactions, model.ContentColumn, model.ContentRow, "detail-back", "browse"
