...
- `src/<ProductDir>/Visibility.fs` — a 2D-visibility pass (broad-phase cull over `SpatialGrid`,
...
The geometry vocabulary still reuses the framework primitives (`Point`/`Rect`/`SpatialGrid`) — the
fragment adds only the ray-segment intersection and the angular sweep that do not belong in a frozen
package.
