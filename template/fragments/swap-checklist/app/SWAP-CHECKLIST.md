...
This is your **model-swap to-do list**. The generated product ships a minimal
controls/navigation starter plus a durable governance spine. When you swap in your own UI you
rewrite only the replaceable parts; the durable parts keep compiling and keep their
source/evidence scans green across the swap. Read
[`docs/scaffold-map.md`](docs/scaffold-map.md) for the durable-vs-replaceable rationale — this
file is the precise symbol-level checklist that projects it, so you don't have to rediscover
the re-points from compiler errors.

> Paths use `<ProductDir>` = `src/<ProjectName>` (your generated tree names it after the
> project, e.g. `src/Invoice1`). The module name stays `Product.*`.

## 1. Rewrite wholesale (replaceable — these define/call the starter model directly)

- [ ] `<ProductDir>/Model.fs` — the starter `Model`/`Msg`/`update` (pages, content cursor,
      controls state; `transitionViewerInput`, `dispatchViewerKey`, `pageName`). Replace with
      your own model.
- [ ] `<ProductDir>/View.fs` — the starter `view` (`Model -> SceneNode`) reading `model.Name`,
      `model.CanSave`, `model.Revenue`, `model.GridColumns`, `model.GridRows`, `model.RichIntro`.
...
- [ ] `contentLayout` — derives the content-grid geometry the active item sits in
...
- [ ] `hudTextBounds` — reads `model.ItemCount`, `model.Step`, `model.NextLabel`,
      `model.Page` (via `pageName`) for the HUD text
