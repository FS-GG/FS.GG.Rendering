...
> `.template.config/template.json` sources nothing from `template/fragments/skiaviewer/`, so no generated
> product ever receives this file. The guidance a product actually gets is
> [`template/product-skills/fs-gg-skiaviewer/SKILL.md`](../../../template/product-skills/fs-gg-skiaviewer/SKILL.md) — the capability's `supplied-by`.
> Recorded as `materializes: none` on the `skiaviewer` row of `template/capabilities.yml`, and held there
