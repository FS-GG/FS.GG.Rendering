...
let mapUnitB (s: Stats) : Token =
    { mapUnitA s with
        R = 16.0 + 14.0 * (min 1.0 (s.Dps / 120.0))
        Threat = 0.0
    }
...
let place (m: Stats -> Token) (u: Placed) : Scene =
    ...
    let cx, cy =
        float col * cellSize + cellSize / 2.0, float row * cellSize + cellSize / 2.0
...
let tile (col, row) =
    Scene.filledRectangle
        {
            X = float col * cellSize
            Y = float row * cellSize
            Width = cellSize
            Height = cellSize
        }
        {
...
for (name, m) in [ "A-threat-on-width", mapUnitA; "B-threat-on-size", mapUnitB ] do
    let png =
        Render.toPng size (frameScene m) (sprintf "./work/symbol-design/round-01/%s" name)

    let report =
        Legibility.scoreIn Grammar.Token (frameRoster |> List.map (fun u -> m u.Stats))
