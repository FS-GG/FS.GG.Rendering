    ...
    if findingMatches.Count = 0 then
        ...
        for index in 0 .. findingMatches.Count - 1 do
            ...
            if findingNumber <> expectedNumber then
                errors.Add(sprintf "findings: expected §4.%s, got §4.%s" expectedNumber findingNumber)
            ...
            for field in requiredFindingFields do
                let fieldPattern = @"(?m)^- \*\*" + Regex.Escape field + @":\*\*\s+\S.*$"
            ...
            let fieldValue name =
                let matched =
                    Regex.Match(chunk, @"(?m)^- \*\*" + Regex.Escape name + @":\*\*\s+(.+?)\s*$")
            ...
            then
                errors.Add(sprintf "findings: §4.%s Expected and Observed must describe a delta" findingNumber)

            let kindPattern =
                @"(?m)^- \*\*Kind:\*\*\s+("
                + String.concat "|" (List.map Regex.Escape kinds)
                + @")\s*$"

            if not (Regex.IsMatch(chunk, kindPattern)) then
                errors.Add(sprintf "findings: §4.%s Kind must be one of %s" findingNumber (String.concat ", " kinds))

    let coverage = sectionText text requiredSections.[11] "\u0000"

    let rowPattern =
        Regex(@"(?m)^\|\s*([^|]+?)\s*\|\s*(exercised|partial|not-exercised)\s*\|")
...
let changedPathsFromNameStatus (text: string) =
    ...
    |> Array.collect (fun line ->
        ...
        if fields.Length < 2 then
            ...
            fields.[0].StartsWith("R", StringComparison.Ordinal)
            || fields.[0].StartsWith("C", StringComparison.Ordinal)
        then
            if fields.Length >= 3 then
                [| normalizeWorkspacePath fields.[1]; normalizeWorkspacePath fields.[2] |]
            else
                [||]
        else
...
let findInvalidatedAuditBindingsIn (index: AuditIndex) (changedPaths: string list) =
            ...
            try
                ...
                if obj.ReferenceEquals(parsed, null) then
                    ...
                    if audit.auditSchema <> 1 then
                        ...
                        errors.Add(
                            sprintf
                                "invalidation: malformed audit %s: reportSha256 must be 64 lowercase hex characters"
                                relativeAudit
                        )
                        ...
                        for finding in audit.findings do
                            if obj.ReferenceEquals(box finding, null) then
                                errors.Add(
                                    sprintf
                                        "invalidation: malformed audit %s: findings must not contain null entries"
                                        relativeAudit
                                )
                            elif String.IsNullOrWhiteSpace(normalizedJsonString finding.id) then
                                errors.Add(
                                    sprintf "invalidation: malformed audit %s: finding id is required" relativeAudit
                                )
                            elif isNull (box finding.checkedEvidence) then
                                errors.Add(
                                    sprintf
                                        "invalidation: malformed audit %s: %s has no checkedEvidence"
                                        relativeAudit
                                        (normalizedJsonString finding.id)
                                )
                            else
                                for evidence in finding.checkedEvidence do
                                    if obj.ReferenceEquals(box evidence, null) then
                                        errors.Add(
                                            sprintf
                                                "invalidation: malformed audit %s: %s has null checkedEvidence"
                                                relativeAudit
                                                (normalizedJsonString finding.id)
                                        )
                                    else
                                        ...
                                        if
                                            ...
                                            || not (List.contains result evidenceResults)
                                        then
                                            errors.Add(
                                                sprintf
                                                    "invalidation: malformed audit %s: %s has an invalid evidence result"
                                                    relativeAudit
                                                    (normalizedJsonString finding.id)
                                            )
                                            ...
                                            errors.Add(
                                                sprintf
                                                    "invalidation: malformed audit %s: %s file evidence needs sha256"
                                                    relativeAudit
                                                    (normalizedJsonString finding.id)
                                            )
                                        elif
                                            digest
                                            |> Option.exists (fun value -> not (Regex.IsMatch(value, "^[0-9a-f]{64}$")))
                                        then
                                            errors.Add(
                                                sprintf
                                                    "invalidation: malformed audit %s: %s evidence sha256 must be 64 lowercase hex characters"
                                                    relativeAudit
                                                    (normalizedJsonString finding.id)
                                            )
                                            ...
                                            if String.IsNullOrWhiteSpace path then
                                                errors.Add(
                                                    sprintf
                                                        "invalidation: malformed audit %s: %s has an empty file locator"
                                                        relativeAudit
                                                        (normalizedJsonString finding.id)
                                                )
                                            else
...

    {
        ...
        invalidated =
            invalidated
            |> Seq.sortBy (fun item ->
                item.audit, item.report, item.findingId, item.path, item.locator, item.priorSha256)
            |> List.ofSeq
        dispositions = []
        bindings =
            bindings
...
let private sha256EvidenceBytes (bytes: byte[]) =
    try
        bytes |> UTF8Encoding(false, true).GetString |> sha256Text |> Ok
    with ex ->
        Error(sprintf "replacement evidence is not valid UTF-8 text: %s" ex.Message)
    ...
    if element.ValueKind <> JsonValueKind.Object then
        ...
        let duplicateErrors =
            ...
            |> List.choose (fun (name, count) ->
                if count > 1 then
                    Some(sprintf "%s contains duplicate property '%s'" label name)
                else
                    None)
        ...
        let missing =
            ...
            |> Seq.map (sprintf "%s is missing property '%s'" label)

        let extra =
            Set.difference actual expected
            |> Seq.map (sprintf "%s contains unknown property '%s'" label)
...
let private requiredJsonString (label: string) (propertyName: string) (element: JsonElement) =
    ...
    | true, property when property.ValueKind = JsonValueKind.String ->
        ...
        if String.IsNullOrWhiteSpace value then
            Error(sprintf "%s.%s must not be empty" label propertyName)
        else
            Ok value
    ...
    try
        ...
        | true, value when value.ValueKind = JsonValueKind.Array ->
            for index, entry in value.EnumerateArray() |> Seq.indexed do
                ...
                let fields =
                    expected |> List.map (fun name -> name, requiredJsonString label name entry)
                ...
                if fields |> List.forall (snd >> Result.isOk) then
                    let field name =
                        ...
                        |> function
                            | Ok value -> value
                            | Error _ -> failwith "validated above"
...
let private exactWorkspacePathError label (path: string) =
    ...
    if path <> path.Trim() then
        Some(sprintf "%s must not have surrounding whitespace" label)
    elif String.IsNullOrWhiteSpace normalized then
        Some(sprintf "%s must not be empty" label)
    elif Path.IsPathRooted path || path.StartsWith("/", StringComparison.Ordinal) then
        Some(sprintf "%s must be workspace-relative" label)
    elif path.Contains('\\') then
        Some(sprintf "%s must use '/' separators" label)
    elif path.Contains("//", StringComparison.Ordinal) then
        Some(sprintf "%s must use canonical single '/' separators" label)
    elif path |> Seq.exists Char.IsControl then
        Some(sprintf "%s must not contain control characters" label)
    elif path.IndexOfAny([| '*'; '?'; '['; ']' |]) >= 0 then
        Some(sprintf "%s must be exact; wildcard paths are overbroad" label)
    elif segments |> Array.exists (fun segment -> segment = "." || segment = "..") then
        Some(sprintf "%s must not contain '.' or '..' segments" label)
    elif normalized.EndsWith("/", StringComparison.Ordinal) then
        Some(sprintf "%s must name a file, not a directory" label)
    else
        None
...
let private explicitlyNamesWorkspacePath (locator: string) (path: string) =
    Regex.IsMatch(locator, @"(^|[\s""'=:(,])" + Regex.Escape(path) + @"($|[\s""',;)])", RegexOptions.CultureInvariant)
...
let private applyAuditBindingExceptions (candidate: CandidateEvidenceSubject) (result: AuditInvalidationCheck) =
    ...
    | Error detail ->
        { result with
            errors =
                (sprintf "invalidation: could not read exception ledger from %s: %s" candidate.subject detail)
                :: result.errors
                |> List.sort
        }
    | Ok None -> result
    | Ok(Some ledgerBytes) ->
        ...
        |> List.iter (fun (id, count) ->
            if count > 1 then
                errors.Add(sprintf "exception ledger contains duplicate id '%s'" id))
        ...
        |> List.iter (fun ((audit, findingId, locator, path, prior), count) ->
            if count > 1 then
                errors.Add(
                    sprintf
                        "exception ledger contains duplicate binding %s %s %s %s %s"
                        audit
                        findingId
                        locator
                        path
        ...
        for entry in entries do
            ...
            if not (Regex.IsMatch(entry.priorSha256, digestPattern)) then
                errors.Add(
                    sprintf "exception ledger entry '%s' priorSha256 must be 64 lowercase hex characters" entry.id
                )

            if not (Regex.IsMatch(entry.replacementSha256, digestPattern)) then
                errors.Add(
                    sprintf "exception ledger entry '%s' replacementSha256 must be 64 lowercase hex characters" entry.id
                )
            ...
            if not (entry.evidenceLocator.StartsWith("command:", StringComparison.Ordinal)) then
                ...
                errors.Add(
                    sprintf
                        "exception ledger entry '%s' evidenceLocator contains private or host-specific material"
                        entry.id
                )
            elif not (explicitlyNamesWorkspacePath entry.evidenceLocator entry.replacementPath) then
                errors.Add(
                    sprintf
                        "exception ledger entry '%s' evidenceLocator must explicitly inspect %s"
                        entry.id
            ...
            if not (knownBindings.Contains(exceptionBindingKey entry)) then
                errors.Add(
                    sprintf
                        "exception ledger entry '%s' is unused: no immutable audit binding matches it exactly"
                        entry.id
                )
            elif bindingCounts.[exceptionBindingKey entry] <> 1 then
                errors.Add(
                    sprintf
                        "exception ledger entry '%s' is overbroad: it matches multiple immutable audit bindings"
                        entry.id
                )

            match
                exactWorkspacePathError
                    (sprintf "exception ledger entry '%s' replacementPath" entry.id)
                    entry.replacementPath
            with
            | Some _ -> ()
            | None ->
                ...
                | Error detail ->
                    errors.Add(
                        sprintf "exception ledger entry '%s' replacement evidence is unreadable: %s" entry.id detail
                    )
                | Ok None ->
                    errors.Add(
                        sprintf
                            "exception ledger entry '%s' replacement evidence is missing: %s"
                            entry.id
                            entry.replacementPath
                    )
                | Ok(Some bytes) ->
                    ...
                    | Ok actual when actual <> entry.replacementSha256 ->
                        errors.Add(
                            sprintf
                                "exception ledger entry '%s' replacement digest is stale: expected %s, got %s"
                                entry.id
        ...
        if errors.Count > 0 then
            { result with
                errors = (result.errors @ List.ofSeq errors) |> List.sort
            }
        else
            let byBinding =
                entries
                |> List.map (fun entry -> exceptionBindingKey entry, entry)
                |> Map.ofList

            let applied, remaining =
                result.invalidated |> List.partition (auditBindingKey >> byBinding.ContainsKey)
            ...
            let dispositions: AppliedAuditBindingException list =
                ...
                |> List.map (fun binding ->
                    ...
                    disposition)
                |> List.sortBy (fun (disposition: AppliedAuditBindingException) ->
                    disposition.audit, disposition.findingId, disposition.path, disposition.id)
...
let private workspaceRelative (workspaceRoot: string) (resolved: string) =
    ...
    Path.GetRelativePath(canonicalRoot, resolved).Replace(Path.DirectorySeparatorChar, '/')
...
let private readGitBlob (workspaceRoot: string) (reference: string) (relative: string) =
    ...
    | Ok listing ->
        ...
        let parsed =
            if records.Length <> 1 then
                ...
                if separator <= 0 then
                    ...
                    let header =
                        records.[0].Substring(0, separator).Split(' ', StringSplitOptions.RemoveEmptyEntries)
        ...
        match parsed with
        | Error detail -> Error detail
        | Ok(mode, objectType) when objectType <> "blob" || (mode <> "100644" && mode <> "100755") ->
    ...
    let failed detail =
        {
            ...
            errors =
                [
                    sprintf "invalidation: could not read the audit index at %s: %s" reference detail
                ]
        }
...
let changedPathsBetween (workspaceRoot: string) (baseRef: string) (headRef: string) =
    ...
    | Ok diff when diff.exitCode <> 0 ->
        Error(sprintf "invalidation: could not read commit path changes: %s" diff.stderr)
...
let private resolveReportHead (workspaceRoot: string) (reportText: string) =
    ...
    | Ok _ ->
        ...
        | Some commit ->
            ...
            | Error detail ->
                Error(sprintf "cannot resolve report commit '%s' (%s); %s" commit detail boundedFileEvidenceGuidance)

let private committedFileText (workspaceRoot: string) (head: string) (relative: string) =
    let absent classification =
        Error(
            sprintf
                "file evidence is %s at report head: file:%s; %s"
                classification
                relative
                boundedFileEvidenceGuidance
        )
    ...
    | Error detail ->
        Error(sprintf "cannot inspect Git tree for file:%s (%s); %s" relative detail boundedFileEvidenceGuidance)
    ...
    | Ok tree when String.IsNullOrEmpty tree.stdout ->
        ...
        | Error detail ->
            Error(sprintf "cannot classify file:%s in Git (%s); %s" relative detail boundedFileEvidenceGuidance)
        | Ok _ ->
            match runGit workspaceRoot [ "ls-files"; "--error-unmatch"; "--"; relative ] with
            | Ok tracked when tracked.exitCode = 0 -> absent "absent"
            | Ok tracked when tracked.exitCode = 1 && File.Exists(Path.Combine(workspaceRoot, relative)) ->
                absent "untracked"
            | Ok tracked when tracked.exitCode = 1 -> absent "absent"
            | Ok tracked ->
                Error(
                    sprintf
                        "cannot classify file:%s in Git (%s); %s"
                        relative
                        tracked.stderr
                        boundedFileEvidenceGuidance
                )
    ...
    | Ok tree ->
        ...
        if
            fields.Length < 3
            || not (fields.[0].StartsWith("100", StringComparison.Ordinal))
        then
            Error(
                sprintf
                    "file evidence is not a regular committed file at report head: file:%s; %s"
                    relative
                    boundedFileEvidenceGuidance
            )
        else
            ...
            | Ok content ->
                Error(
                    sprintf
                        "cannot read committed file evidence file:%s (%s); %s"
                        relative
                        content.stderr
                        boundedFileEvidenceGuidance
                )
...
let private validateActionabilityAuditDetailedWithGitTree
    ...
    | Some audit ->
        ...
        let expectedReport =
            Path.GetRelativePath(workspaceRoot, Path.GetFullPath reportPath).Replace(Path.DirectorySeparatorChar, '/')
        ...
        then
            errors.Add "audit: criticMode must be fresh-context-subagent or separated-critic-pass"
        ...
        for id, kind, declaredEvidence in expectedFindings do
            let matches =
                audits |> List.filter (fun finding -> normalizedJsonString finding.id = id)
            ...
            if List.isEmpty matches then
                ...
                if status = "incomplete" || status = "unsupported" then
                    errors.Add(sprintf "actionability: %s remains %s and cannot be handed off as actionable" id status)

                let missingFacts =
                    if isNull (box finding.missingFacts) then
                        []
                    else
                        finding.missingFacts

                if
                    (status = "actionable" || status = "positive-pattern")
                    && not (List.isEmpty missingFacts)
                then
                    errors.Add(sprintf "audit: %s cannot be %s while missing facts are recorded" id status)
                ...
                let checkedLocators =
                    checks
                    |> List.map (fun check -> normalizedJsonString check.locator)
                    |> Set.ofList

                for locator in Set.difference declaredEvidence checkedLocators do
                    errors.Add(sprintf "audit: %s report evidence has no matching check: %s" id locator)

                for locator in Set.difference checkedLocators declaredEvidence do
                    errors.Add(sprintf "audit: %s checked evidence is not declared by the report: %s" id locator)

                for check in checks do
                    let locator = normalizedJsonString check.locator
                    let result = normalizedJsonString check.result

                    let digest = check.sha256 |> Option.map normalizedJsonString
                    ...
                    if String.IsNullOrWhiteSpace locator then
                        ...
                        errors.Add(sprintf "audit: %s evidence locator exposes an absolute path or secret material" id)
                    ...
                    if not (List.contains result evidenceResults) then
                        errors.Add(sprintf "audit: %s evidence has unknown result '%s'" id result)

                    if (status = "actionable" || status = "positive-pattern") && result <> "verified" then
                        errors.Add(sprintf "audit: %s cannot be %s with evidence result '%s'" id status result)

                    if locator.StartsWith("file:", StringComparison.Ordinal) then
                        match resolveEvidencePath workspaceRoot locator with
                        | None ->
                            errors.Add(sprintf "audit: %s evidence locator must be a workspace-relative file: path" id)
                        | Some path ->
...
let validateActionabilityAudit (workspaceRoot: string) (reportPath: string) (reportText: string) (auditText: string) =
...
let appendZeroEventActivation root cycle exercisedPhases evidence reasonNoEventQualified =
    ...
    if File.Exists eventPath || Directory.Exists eventPath then
        invalidArg
            "cycle"
            (sprintf "cycle '%s' already has checkpoint event state; a zero-event receipt would contradict it" cycle)
                ...
                if String.IsNullOrWhiteSpace line then
                    ...
                    try
                        ...
                        let readProperty (name: string) =
                            ...
                            | _ ->
                                errors.Add(sprintf "checkpoints: line %d is missing %s" (index + 1) name)
                        ...
                        for name, value in values do
                            if String.IsNullOrWhiteSpace value then
                                errors.Add(sprintf "checkpoints: line %d has empty %s" (index + 1) name)

                        let valueOf name =
                            values |> List.find (fst >> (=) name) |> snd
                        ...
                        if not (List.contains surface surfaces) then
                            errors.Add(sprintf "checkpoints: line %d has unknown surface '%s'" (index + 1) surface)

                        if not (List.contains kind kinds) then
                            errors.Add(sprintf "checkpoints: line %d has unknown kind '%s'" (index + 1) kind)
                    with ex ->
                        errors.Add(sprintf "checkpoints: line %d is invalid JSON: %s" (index + 1) ex.Message)

            if lineCount = 0 then
                errors.Add("checkpoints: event file contains no events; record a zero-event activation receipt")
...
let validateZeroEventActivationFile (workspaceRoot: string) (expectedCycle: string) (path: string) =
    if Directory.Exists path then
        ...
        try
            ...
            if not (isInside canonicalRoot canonicalPath) then
                raise (InvalidDataException("zero-event activation receipt resolves outside the workspace"))
            ...
            for property in properties do
                if not (Set.contains property.Name allowedProperties) then
                    errors.Add(sprintf "checkpoints: activation receipt contains unknown property '%s'" property.Name)

            for name, count in
                properties
                |> List.countBy (fun property -> property.Name)
                |> List.filter (fun (_, count) -> count > 1) do
                errors.Add(sprintf "checkpoints: activation receipt contains duplicate property '%s'" name)
                    ...
                    [
                        for item in value.EnumerateArray() do
                            if item.ValueKind = JsonValueKind.String then
                                ...
                                errors.Add(sprintf "checkpoints: activation receipt %s must contain only strings" name)
                    ]
                | _ ->
                    errors.Add(sprintf "checkpoints: activation receipt is missing %s" name)
                    []
            ...
            if cycle <> expectedCycle then
                errors.Add(sprintf "checkpoints: activation receipt cycle must be '%s', got '%s'" expectedCycle cycle)
            ...
            for name, values in [ "exercisedPhases", phases; "evidence", evidence ] do
                ...
                if values |> List.exists String.IsNullOrWhiteSpace then
                    errors.Add(sprintf "checkpoints: activation receipt %s must not contain empty values" name)
            ...
            if evidence |> List.exists containsPrivateLocatorMaterial then
                errors.Add("checkpoints: activation receipt evidence exposes an absolute path or secret material")
            ...
            List.ofSeq errors
        with
        | :? JsonException as ex -> [ sprintf "checkpoints: activation receipt is malformed JSON: %s" ex.Message ]
...
let validateCheckpointState (root: string) (cycle: string) =
    ...
    if errors.Count > 0 then
        ...
        | true, true ->
            [
                sprintf "checkpoints: cycle '%s' has both checkpoint events and a zero-event activation receipt" cycle
            ]
