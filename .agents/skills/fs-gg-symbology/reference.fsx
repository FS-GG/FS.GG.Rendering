...
// replace the four FS.GG DLL lines with their `#r "nuget: FS.GG.UI.*"` equivalents and DROP the three
// Skia lines — the packages carry SkiaSharp transitively. See the product twin under
// template/product-skills/fs-gg-symbology/.
//
// In-tree, the Skia refs are NOT optional: fsi resolves a bare `#r` DLL against the file system, not
// against the project's deps.json, so nothing pulls SkiaSharp in behind `FS.GG.UI.SkiaViewer.dll` and
// `Render.toPng` fails at the rasterise step. Keep the versions in step with
// Directory.Packages.local.props. The NativeAssets line is Linux-only — on Windows/macOS the
// SkiaSharp package carries its own natives.
#r "nuget: SkiaSharp, 4.150.0"
#r "nuget: SkiaSharp.HarfBuzz, 4.150.0"
#r "nuget: SkiaSharp.NativeAssets.Linux, 4.150.0"
