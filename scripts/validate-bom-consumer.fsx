...
let nuspecDeps () =
    ...
    let allExact =
        deps
        |> List.forall (fun (_, v) -> v.StartsWith "[" && v.EndsWith "]" && not (v.Contains ","))
...
let liveProof () : LiveResult =
    let tmp =
        Path.Combine(Path.GetTempPath(), "bom207-" + Guid.NewGuid().ToString("N").Substring(0, 8))
    ...
    if pc <> 0 then
        failwithf "coherent pack failed:\n%s" po
    ...
    let restoreSet () =
        run repoRoot "dotnet" [ "nuget"; "locals"; "global-packages"; "--clear" ]
        |> ignore

        if Directory.Exists gpf then
    ...
    let scenLow =
        repo (sprintf "../../../.local/share/nuget-local/FS.GG.UI.Scene.%s.nupkg" yLow)

    let realLow =
        Path.Combine(
            Environment.GetFolderPath Environment.SpecialFolder.UserProfile,
            ".local/share/nuget-local",
            sprintf "FS.GG.UI.Scene.%s.nupkg" yLow
        )

    if File.Exists realLow then
        File.Copy(realLow, Path.Combine(feed, Path.GetFileName realLow), true)
    ...
    let mismatch (y: string) =
        ...
        File.WriteAllText(
            ...
            consumerProj (
                sprintf
                    "<PackageReference Include=\"FS.GG.UI\" Version=\"%s\" /><PackageReference Include=\"FS.GG.UI.Scene\" Version=\"%s\" />"
                    v
                    y
            )
        )
        ...
        let ec, out =
            run mdir "dotnet" [ "restore"; "Consumer.fsproj"; "-p:WarningsAsErrors=NU1605%3BNU1608" ]
...
// ---- report -----------------------------------------------------------------------------------
let reportPath =
    repo "specs/207-ui-bom-metapackage/readiness/bom-consumer-validation.md"
...
line (
    sprintf
        "- parity: %s (nuspec dependency-id set == discovered packable FS.GG.UI.* set; single [$version$] token; exact-bracket form)"
        (if parityOk then "pass" else "fail")
)
...
if live then
    ...
    let membersResolved =
        r.Resolved
        |> List.filter (fun (id, _) -> id.StartsWith "FS.GG.UI." && id <> "FS.GG.UI")
    ...
    line (
        sprintf
            "- reproducibility: %s (two cache-cleared clean restores produced %s resolved sets)"
            (if r.ReproIdentical then "identical" else "DIVERGED")
            (if r.ReproIdentical then "identical" else "different")
    )
    ...
    line (
        sprintf
            "  - downgrade Y<V (FS.GG.UI.Scene 0.1.50-preview.1): code=%s loud-under-warnaserror-exit=%d (nonzero => blocked); default=warning(mixed-graph)"
            dgCode
            dgEc
    )
    ...
    line (
        sprintf
            "  - upgrade   Y>V (FS.GG.UI.Scene 0.1.52-preview.1): code=%s loud-under-warnaserror-exit=%d (nonzero => blocked); default=warning(mixed-graph)"
            upCode
            upEc
    )
    ...
    let overall =
        parityOk
        && atV.Length = membersResolved.Length
        && membersResolved.Length = members.Count
        && r.CleanBuild
        && r.ReproIdentical
        && forcedPass
