...
let sourcePath =
    Path.Combine(repoRoot, "src", "Themes.Default", "design-tokens.tokens.json")
...
let private parseColor (hex: string) =
    ...
    if h.Length <> 8 then
        fail (sprintf "bad color value '%s'" hex)
...
let private emitLeaf (indent: string) (name: string) (leaf: JsonElement) (sb: StringBuilder) =
    ...
    | "dimension"
    | "number" -> line "float" (fmtFloat (v.GetDouble()))
    | "shadow" -> line "string" (sprintf "\"%s\"" (v.GetString()))
    | "fontFamily" ->
        let expr =
            if v.ValueKind = JsonValueKind.Null then
                "None"
            else
                sprintf "Some \"%s\"" (v.GetString())
...
let rec private emitNode (indent: string) (key: string) (node: JsonElement) (sb: StringBuilder) =
    if isLeaf node then
        ...
        if not any then
            sb.AppendLine(sprintf "%s    ()" indent) |> ignore
...
let rec private checkModeParity (key: string) (node: JsonElement) =
    if node.ValueKind = JsonValueKind.Object && not (isLeaf node) then
        ...
        if node.TryGetProperty("light", &light) || node.TryGetProperty("dark", &dark) then
            ...
            let keysOf (e: JsonElement) =
                e.EnumerateObject() |> Seq.map (fun p -> p.Name) |> Set.ofSeq
...
let rec private emitSig (indent: string) (key: string) (node: JsonElement) (sb: StringBuilder) =
    if isLeaf node then
        sb.AppendLine(sprintf "%sval %s : %s" indent (letName key) (leafType node))
        |> ignore
    elif node.ValueKind = JsonValueKind.Object then
...
let generate () =
    ...
    sb.AppendLine(sprintf "// GENERATED — do not edit. Source: %s" sourceRel)
    |> ignore

    sb.AppendLine("// Regenerate via: dotnet fsi scripts/generate-design-tokens.fsx")
    |> ignore

    sb.AppendLine("//") |> ignore

    sb.AppendLine("// Feature 126 (Workstream F, F1): the Ant-derived token taxonomy (seed/map/alias/component +")
    |> ignore

    sb.AppendLine("// spacing/density/type/elevation). INTERNAL and additive — no .fsi, no public-surface delta;")
    |> ignore

    sb.AppendLine("// nothing reads these yet (the F4 resolver / D2 themes consume them later).")
    |> ignore
    ...
    if not emitted then
        fail "no taxonomy groups found in source (expected seed/map/alias/…)"
...
let generateFlat () =
    ...
    let groupOf (key: string) =
        ...
        if not (root.TryGetProperty(key, &node)) then
            fail (sprintf "flat group '%s' missing from source" key)
    ...
    if keysOf light <> keysOf dark then
        fail "flat light/dark key sets differ (mode parity)"
...
let generateFsi () =
    ...
    sb.AppendLine("// Feature 130 (Workstream F, F5): the PUBLIC signature for the Ant-derived token taxonomy —")
    |> ignore

    sb.AppendLine("// the deliberate, baseline-gated promotion F1 deferred. Generated in lock-step with the paired")
    |> ignore

    sb.AppendLine("// DesignTokensExt.fs; currency is enforced by this generator's --check over BOTH files.")
    |> ignore
    ...
    sb.AppendLine("/// The Ant-derived design-token taxonomy: seed -> map -> alias -> component layers plus spacing,")
    |> ignore

    sb.AppendLine("/// density, type scale, and elevation. Generated from the DTCG source; values are byte-identical")
    |> ignore
...
let formatGenerated (fileName: string) (content: string) =
    let tempDir =
        Path.Combine(Path.GetTempPath(), $"fsgg-design-tokens-{Guid.NewGuid():N}")
    ...
    try
        ...
        if formatCode <> 0 then
            fail $"Fantomas could not normalize generated design tokens.\n{formatOutput}"
...
if restoreCode <> 0 then
    fail $"restoring the repository-pinned Fantomas failed; cannot normalize generated design tokens.\n{restoreOutput}"

let generated = generate () |> formatGenerated "DesignTokensExt.fs"
let generatedFsi = generateFsi () |> formatGenerated "DesignTokensExt.fsi"
let generatedFlat = generateFlat () |> formatGenerated "DesignTokens.fs"

let fsiOutputPath =
    Path.Combine(repoRoot, "src", "DesignSystem", "DesignTokensExt.fsi")

let flatOutputPath =
    Path.Combine(repoRoot, "src", "DesignSystem", "DesignTokens.fs")
...
if check then
    ...
    let drift =
        [
            flatOutputPath, generatedFlat
            outputPath, generated
            fsiOutputPath, generatedFsi
        ]
