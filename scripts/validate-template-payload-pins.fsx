...
let live =
    Environment.GetEnvironmentVariable "FS_GG_RUN_TEMPLATE_PAYLOAD_RESTORE" = "1"
...
let stalenessSweep =
    Environment.GetEnvironmentVariable "FS_GG_TEMPLATE_PIN_STALENESS_SWEEP" = "1"
...
let bumpedInCommitUnderTest (rel: string) (element: string) =
    ...
    if ec <> 0 then
        raise (
            GuardError(
                sprintf
                    "git diff HEAD~1 HEAD -- %s failed — the RELEASE-PENDING waiver cannot be evaluated without full history (fetch-depth: 0); fail closed rather than red-for-the-wrong-reason"
                    rel
            )
        )
...
module SemVer =
    ...
    let wellFormed (s: string) =
        Regex.IsMatch(s.Trim(), @"^\d+\.\d+\.\d+(-[0-9A-Za-z.\-]+)?$")

/// Fail closed if the comparator ever regresses — every staleness verdict below rides on it.
let semverSelfCheck () =
    if not (SemVer.lt "0.1.9" "0.1.10") then
        raise (GuardError "comparator regressed: 0.1.9 < 0.1.10")
    ...
    if not (SemVer.lt "0.2.0" "0.4.0") then
        raise (GuardError "comparator regressed: 0.2.0 < 0.4.0")
...
let gateHolds (condition: string) (profile: string) =
    ...
    if not (shape.IsMatch condition) then
        raise (
            GuardError(
                sprintf
                    "unevaluable `<!--#if -->` gate in %s: `%s` — this guard understands only `profile == \"a\" || profile == \"b\"`; teach it the new shape rather than letting a profile restore the wrong pin set"
                    propsRel
                    condition
            )
        )
...
let readInputs () : Inputs =
        ...
        |> List.map (fun (axis, _) ->
            ...
            if ms.Count <> 1 then
                raise (
                    GuardError(
                        sprintf
                            "%s declares <%s> %d times — expected exactly 1; the axis is the single source of that component's pin"
                            propsRel
                            axis
                            ms.Count
                    )
    ...
    let pkgVersion =
        Regex(@"<PackageVersion\s+Include=""(?<id>[^""]+)""\s+Version=""(?<ver>[^""]+)""")
    ...
    let matched =
        ...
        |> Array.filter (fun (_, m) ->
            m.Success
            && m.Groups.["id"].Value.StartsWith("FS.GG.", StringComparison.Ordinal))
        |> Array.toList
    ...
    if matched.IsEmpty then
        raise (
            GuardError(
                sprintf
                    "%s declares zero FS.GG.* PackageVersion pins — nothing to check is not the same as checked-and-fine (#266)"
                    propsRel
            )
        )
            ...
            yield
                {
                    ...
                    Fix =
                        sprintf
                            "derive %s through its component's axis — a bare literal is invisible to an axis bump and to this guard's staleness rule"
                            id
                }
        ...
        let versions =
            if resp.StatusCode = Net.HttpStatusCode.NotFound then
                ...
                raise (
                    GuardError(
                        sprintf
                            "nuget.org returned %d for %s — the feed could not answer whether %s exists; fail closed"
                            (int resp.StatusCode)
                            url
                            packageId
                    )
                ...
                if vs.IsEmpty then
                    raise (
                        GuardError(
                            sprintf "nuget.org returned 200 but listed zero versions for %s — fail closed" packageId
                        )
                    )
...
let feedNewest (previewChannel: bool) (packageId: string) =
    ...
    | [] ->
        raise (
            GuardError(
                sprintf
                    "%s has no %s version on nuget.org — the staleness rule has nothing to compare against; fail closed"
                    packageId
                    (if previewChannel then "published" else "stable")
            )
        )
...
let analyseGraph (dir: string) (i: Inputs) (pins: Pin list) (profile: string) : ProfileGraph * Failure list =
    ...
    if not (File.Exists assets) then
        raise (
            GuardError(
                sprintf
                    "restore of the %s payload reported success but wrote no project.assets.json — cannot read the resolved graph"
                    profile
            )
        )
    ...
    if resolved.IsEmpty then
        raise (
            GuardError(
                sprintf
                    "the %s payload restored zero FS.GG.* packages — a graph with nothing in it cannot prove the pins; fail closed"
                    profile
            )
        )
                        ...
                        yield
                            {
                                ...
                                Location =
                                    sprintf
                                        "profile=%s (%s)"
                                        profile
                                        (if direct then "pinned directly" else "transitive")
                                Expected = "a stable template scaffolds a stable graph"
                                Actual = sprintf "%s %s" id ver
                                Fix =
                                    if direct then
                                        sprintf "re-pin %s onto a stable version in %s" id propsRel
                                    else
                                        sprintf
                                            "%s is pulled in transitively — bump the direct pin whose graph reaches it, or wait for its stable release"
                                            id
                            }
                    ...
                    yield
                        {
                            ...
                            Actual = "absent"
                            Fix =
                                "a pinned package that is not in the resolved graph means the restore did not do what the props file says"
                        }
        ]

    {
        Profile = profile
        Pinned = pins |> List.map (fun p -> p.Id, p.Version)
        Resolved = resolved
    },
    failures
...
let restoreProfile (tmpRoot: string) (gpf: string) (i: Inputs) (profile: string) : ProfileGraph * Failure list =
    ...
    if pins.IsEmpty then
        raise (GuardError(sprintf "profile %s matched zero pins — refusing to report a vacuous pass" profile))
    ...
    if ec <> 0 then
        if diagnosed.IsEmpty then
            ...
            raise (
                GuardError(
                    sprintf
                        "restore of the %s payload failed with no NuGet pin diagnostic — tooling/network, not the pins:\n%s"
                        profile
                        out
                )
            )
                ...
                for d in diagnosed ->
                    {
                        ...
                        Fix =
                            sprintf
                                "correct the pin — %s means the requested version was substituted or does not exist; a scaffolded product would restore a graph you never declared"
                                d
                    }
            ]
...
let uiPinAheadOfTags (i: Inputs) =
    ...
    if ec <> 0 then
        raise (GuardError "git tag --list fs-gg-ui/v* failed")
    ...
    versions
    |> Array.forall (fun tagged -> SemVer.cmp (SemVer.parse pin) (SemVer.parse tagged) > 0)
                        ...
                        yield
                            {
                                ...
                                Fix =
                                    if axis = uiAxis then
                                        sprintf
                                            "publish %s@%s, or re-pin $(%s) onto a version the feed carries. NOTE: this is NOT waived as RELEASE-PENDING, so this commit did not bump $(%s) — the pin is stale or typo'd, not mid-release"
                                            id
                                            version
                                            axis
                                            axis
                                        ...
                                        sprintf
                                            "publish %s@%s from its OWN repo, or re-pin $(%s) onto a version the feed carries — bumping this axis here publishes nothing"
                                            id
                                            version
                                            axis
                            }
                    ...
                    yield
                        {
                            ...
                            Expected =
                                sprintf
                                    "%s (newest %s on nuget.org)"
                                    newest
                                    (if i.PreviewChannel then "version" else "stable version")
                            Actual = version
                            Fix =
                                sprintf
                                    "bump $(%s) to %s — a scaffolded product otherwise starts life on a stale component. This is NOT a one-line diff: follow the reconciliation routine in docs/ci/cadence-map.md §4b (\"Bumping a component axis\"), which lists every gate the bump moves, in order"
                                    axis
                                    newest
                        }
    ]
...
let printReleasePending (i: Inputs) (pending: string list) =
    ...
    printfn
        "RELEASE-PENDING: $(%s)=%s is ahead of its snapshot tags, and the %d FS.GG.UI.* package(s) it pins are not on nuget.org yet."
        uiAxis
        version
        pending.Length

    printfn
        "  That is the release window, not drift: merging this is what cuts fs-gg-ui/v%s and publishes them (release-tags.yml -> release.yml)."
        version
    ...
    printfn
        "    - the resolved graph of all %d scaffold profiles — the TRANSITIVE half of `prerelease-in-scaffolded-graph`, and `pin-resolved-elsewhere`"
        profiles.Length
    ...
    writeStepSummary
        ...
        [
            sprintf
                "`$(%s)` is **%s**, ahead of its snapshot tags, and its FS.GG.UI.* packages are not published yet. That is the repairable release window, so `pin-not-published` / `pin-does-not-resolve` are **waived on that axis alone**."
                uiAxis
                version
            ""
            sprintf "- **awaiting publish @ %s:** %s" version (String.concat ", " pending)
            sprintf
                "- **not checked:** the resolved graph of all %d scaffold profiles — the transitive half of `prerelease-in-scaffolded-graph`, and `pin-resolved-elsewhere`. Skipped, not passed."
                profiles.Length
...
let main () =
    ...
    let axisLine =
        i.AxisVersions
        |> Map.toList
        |> List.map (fun (a, v) -> sprintf "$(%s)=%s" a v)
        |> String.concat " · "
    ...
    if stalenessSweep then
            ...
            if stale.IsEmpty then
                ...
                writeStepSummary
                    ...
                    [
                        ...
                        sprintf
                            "- every feed-owned axis equals newest %s on nuget.org."
                            (if i.PreviewChannel then
                                 "published version"
                             else
                                 "stable version")
        ...
        if not structural.IsEmpty then
            ...
            eprintfn
                "template payload pins: DRIFT — %d structural failure(s); restore proof not attempted"
                structural.Length

            1
        else
            let tmpRoot =
                Path.Combine(Path.GetTempPath(), "tpp241-" + Guid.NewGuid().ToString("N").Substring(0, 8))
            ...
            try
                ...
                if waiveUi then
                    ...
                    if stillChecked.IsEmpty then
                        ...
                        printfn
                            "template payload pins: RELEASE-PENDING — structural core OK, Game/Audio/Contracts pins all published, no directly pinned prerelease; restore proof deferred to the publish, staleness deferred to the sweep (#1102)."
                        ...
                        eprintfn
                            "template payload pins: DRIFT — %d failure(s) the release window does not excuse (RELEASE-PENDING waives only the publication of $(%s))"
                            stillChecked.Length
                            uiAxis
                    ...
                    let graphs, restoreFailures =
                        profiles |> List.map (restoreProfile tmpRoot gpf i) |> List.unzip

                    let failures = feed @ List.concat restoreFailures

                    let allResolved =
                        graphs |> List.collect (fun g -> g.Resolved) |> List.distinct |> List.sort

                    let prereleases = allResolved |> List.filter (snd >> SemVer.isPrerelease)

                    for g in graphs do
                        printfn
                            "  %-14s %d pinned · %d resolved FS.GG.* (transitive incl.)"
                            g.Profile
                            g.Pinned.Length
                            g.Resolved.Length
                    ...
                    printfn
                        ...
                        profiles.Length
                        allResolved.Length
                        (if prereleases.IsEmpty then
                             "NONE"
                         else
                             prereleases
                             |> List.map (fun (id, v) -> sprintf "%s %s" id v)
                             |> String.concat ", ")
                    ...
                    if failures.IsEmpty then
                        ...
                        printfn
                            "template payload pins: COHERENT (structural + restore-grounded). Staleness NOT judged here — `pin-lags-feed` is the scheduled sweep's (#1102)."
                        ...
                        writeStepSummary
                            ...
                            [
                                ...
                                sprintf
                                    "- distinct `FS.GG.*` resolved (transitive incl.): **%d** · prerelease: **NONE**"
                                    allResolved.Length
