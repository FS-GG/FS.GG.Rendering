...
#
# TWO READINGS OF ONE RULE WAS THE ACTUAL DEFECT. While Rendering carried foreign bodies, the retired
# mirror guard and this gate each maintained their own mirror set. #722 made this gate verify the set
# that narrows ITS OWN reference policy directly against the registry and published manifest.
#
# #1147 removed the foreign bodies and retired the other guard. `MIRRORED_SKILLS` is empty in production;
# the machinery below remains a dormant, independently tested capability for a future declared mirror.
# It must not depend on, or claim corroboration from, a checker that no longer exists.
#
...
# is the registry's to answer. Here the second half is `is_published`, read from the manifest.
#
...
# decisive: `owner:` changes at a migration, not continuously as § 2's link state does; and while the
# list is stale this gate is fail-OPEN, so merging past it is the thing § 4 would actually be protecting.
# Production carries no mirrors after #965/#1147, so this path is dormant unless a future manifest again
# publishes a foreign-owned body.
mirror_mode=checked
mirror_skip_reason=""
if ((!gh_ready)); then
  mirror_mode=skipped
