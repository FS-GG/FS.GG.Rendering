...
let private verifyVerdictCore () =
    ...
    assertTrue
        (match effLower.TryGetProperty "fileRename" with
         | true, _ -> false
         | _ -> true)
    ...
    for rel in [ "template/base/README.md"; "template/base/docs/product.md" ] do
        ...
        assertTrue
            (not (text.Contains "dotnet fsi load-product.fsx"))
            (sprintf "%s still documents the stale lowercase loader" rel)
    ...
    printfn
        "verdict-core OK: productName param present (text, default \"\"); sourceName removed; effectiveName coalesce(productNameTrimmed ?? name) drives Product (replaces+fileRename); projectSlug.source=effectiveName"
...
let private treePrintNormalised (tokens: string list) (root: string) =
    ...
    let normalise (s: string) =
        tokens
        |> List.fold
            (fun (acc: string) (t: string) -> acc.Replace(t, "NAME").Replace(t.ToLowerInvariant(), "name"))
            s
...
let private namedAs (dir: string) (name: string) =
    let hasPath =
        Directory.Exists(Path.Combine(dir, "src", name))
        && File.Exists(Path.Combine(dir, sprintf "%s.slnx" name))

    let noLeftoverProduct =
        Directory.EnumerateFileSystemEntries(dir, "*Product*", SearchOption.AllDirectories)
        |> Seq.isEmpty
...
let private reinstallLocalTemplate () =
    let code, out, err =
        runProc repoRoot "dotnet" [ "new"; "install"; repoRoot; "--force" ]

    if code <> 0 then
        failwithf "dotnet new install <repoRoot> --force failed (exit %d):\n%s\n%s" code out err
...
let private runLive () =
    ...
    if g1code <> 0 then
        failwithf "G1 FAIL: --productName Acme (no -n) exited %d (was exit 127):\n%s" g1code g1log

    let g1 = "instantiated=ok no-exit-127=ok"

    if not (namedAs acme "Acme") then
        failwithf "G2 FAIL: tree not named Acme (paths/leftover Product):\n%s" acme

    let loader = Path.Combine(acme, "load-Acme.fsx")

    if not (File.Exists loader) then
        failwithf "G2 FAIL: scoped loader filename missing: %s" loader

    if File.Exists(Path.Combine(acme, "load-product.fsx")) then
        failwith "G2 FAIL: stale lowercase loader filename was emitted"
    ...
    if bcode <> 0 then
        failwithf
            "SC-002 FAIL: README build command exited %d:\n%s"
            bcode
            (buildLog.Substring(max 0 (buildLog.Length - 2000)))

    if warnCount > 0 then
        failwithf "SC-002 FAIL: README build command had %d warning(s)" warnCount

    let fsiCode, fsiOut, fsiErr = runProc acme "dotnet" [ "fsi"; "load-Acme.fsx" ]

    if fsiCode <> 0 then
        failwithf "SC-002 FAIL: dotnet fsi load-Acme.fsx exited %d:\n%s\n%s" fsiCode fsiOut fsiErr
    ...
    if g3code <> 0 then
        failwithf "G3 FAIL: precedence scaffold exited %d:\n%s" g3code g3log

    if not (namedAs g3dir "Acme") then
        failwithf "G3 FAIL: productName did not win over -n (expected Acme)"
    ...
    if not (List.isEmpty convDiffs) then
        failwithf
            "SC-004 FAIL: --productName Acme differs from -n Acme (%d):\n%s"
            (List.length convDiffs)
            (String.concat "\n" convDiffs)
...
let private main () =
    ...
    let liveGate =
        Environment.GetEnvironmentVariable "FS_GG_RUN_PRODUCTNAME_VALIDATION" = "1"
