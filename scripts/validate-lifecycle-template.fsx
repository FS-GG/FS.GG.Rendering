...
let private templateDoc () =
    JsonDocument.Parse(File.ReadAllText templateJsonPath)
...
let private enumerateLifecycleChoices () =
    ...
    let choices =
        doc.RootElement.GetProperty("symbols").GetProperty("lifecycle").GetProperty("choices")
        |> fun arr -> [ for c in arr.EnumerateArray() -> c.GetProperty("choice").GetString() ]

    if List.isEmpty choices then
        failwith "lifecycle has no choices"
...
let private sourceRows (arrayElement: JsonElement) =
    ...
    let strs (s: JsonElement) (prop: string) =
        ...
        | true, arr ->
            [
                for e in arr.EnumerateArray() -> e.GetString() |> Option.ofObj |> Option.defaultValue ""
            ]
        | _ -> []
...
let private ruleMessage (row: SourceRow) (ruleId: string) =
    ...
    | "framework/target-agents-skills-only" ->
        sprintf
            "product-skill source %s -> %s: product skills emit to .agents/skills/ ONLY (the standalone materialize step / orchestrator fan-out own the other roots, ADR-0014)"
            s
            t
    | "framework/no-spec-kit-clause" ->
        sprintf
            "framework product-skill source %s -> %s must NOT carry `%s` (it follows the profile, not the lifecycle)"
            s
            t
            SPEC_KIT_COND
    | "framework/profile-predicate" ->
    ...
    | "manifest/ungated" -> sprintf "skill-manifest source %s -> %s must be UNGATED (ships in every lifecycle)" s t
    ...
    | "workspace/speckit-blanket-target" ->
        sprintf
            "repo-root .agents/skills/ source must target .agents/skills/ only, found %s (Feature 230 blanket twin resurrected?)"
            t
    | "workspace/speckit-blanket-include" ->
        sprintf
            ".agents/skills/ source must include ONLY speckit-*/** (no dev-surface vendoring, ADR-0014/F3); found include=%A"
            row.Includes
    | "workspace/materialize-target" ->
        sprintf "materialize source %s must target .specify/scripts/fs-gg/, found %s" s t
    | "product/no-spec-kit-clause" -> sprintf "ungated product source %s -> %s must NOT carry `%s`" s t SPEC_KIT_COND
...
let private verifyGatedSources () =
    ...
    assertTrue
        (frameworkChecked = 17)
        (sprintf
            "expected exactly 17 framework product-skill sources (.agents/skills/ provider surface; ADR-0063 (FS.GG.Rendering#965) retired the 4 game-owned copies game-core/audio/persistence/model-swap, now owner-sourced from FS.GG.Game.Skills; incl. fs-gg-project + fs-gg-collision + fs-gg-visibility + fs-gg-grids + fs-gg-line-drawing + fs-gg-symbol-design + fs-gg-samples (re-gated profile-only under #939) + fs-gg-game-shell (#991), no twins), checked %d"
            frameworkChecked)

    assertTrue
        (capabilityChecked = 1)
        (sprintf
            "expected exactly 1 capability-scope skill source (fs-gg-feedback-report — ungated since #434), checked %d"
            capabilityChecked)

    assertTrue
        (manifestChecked = 1)
        (sprintf "expected exactly 1 ungated skill-manifest source, checked %d" manifestChecked)

    assertTrue
        (materializeChecked = 1)
        (sprintf
            "expected exactly 1 spec-kit-gated materialize source (template/lifecycle/), checked %d"
            materializeChecked)

    assertTrue
        (speckitNarrowChecked = 1)
        (sprintf "expected exactly 1 narrowed tracked .claude/skills/ source, checked %d" speckitNarrowChecked)
...
let private manifestSkills () =
    ...
    assertTrue
        (File.Exists path)
        (sprintf "skill-manifest missing at %s — the supply chain has no declaration to check" path)
    ...
    let field (s: JsonElement) name =
        ...
        | _ ->
            failwithf
                "VERDICT-CORE FAIL: skill-manifest entry is missing a string `%s` (regenerate via scripts/generate-skill-manifest.fsx)"
                name
    ...
    [
        for s in doc.RootElement.GetProperty("skills").EnumerateArray() ->
            ...
            {
                Id = field s "id"
                SuppliedBy = field s "supplied-by"
...
let private targetedSkillId (target: string) =
    ...
    if not (t.StartsWith ".agents/skills/") then
        None
    else
...
let private skillsRootPrefix (target: string) =
    ...
    if segs.Length <= skillsRoot.Length && segs = List.truncate segs.Length skillsRoot then
        Some(List.skip segs.Length skillsRoot)
    else
        None
...
let private rowMaterializesSkillDirs (row: SourceRow) (prefix: string list) =
    ...
    if not (Directory.Exists sourceDir) then
        ...
        |> Seq.filter (fun d ->
            ...
            let excluded =
                row.Excludes |> List.exists (fun p -> excludeCovers (globSegments p) segs)
...
let private verifySkillSupplyChain () =
    ...
    let suppliedBy id =
        ...
        |> Option.map (fun s -> s.SuppliedBy)

    // S1: each skill-directory row is the manifest's named supplier.
    let skillRows =
        rows
        |> List.choose (fun r -> targetedSkillId r.Target |> Option.map (fun id -> id, r))
    ...
    for id, row in skillRows do
        ...
        | None ->
            assertTrue
                false
                (sprintf
                    "template source %s -> .agents/skills/%s/ fills a skill the manifest does not declare (add it to template/skill-manifest/, or stop emitting it)"
                    row.Source
                    id)
        | Some declared ->
            assertTrue
                (declared = row.Source.Replace('\\', '/'))
                (sprintf
                    "skill-manifest says %s is supplied-by %s, but template.json fills .agents/skills/%s/ from %s (#303: supplied-by must name the real source)"
                    id
                    declared
                    id
                    row.Source)
    ...
    for skill in skills do
        let suppliers =
            skillRows
            |> List.filter (fun (id, _) -> id = skill.Id)
            |> List.map (fun (_, r) -> r.Source)

        match skill.DeliveryOnly, suppliers with
        | true, [] -> ()
        | true, _ :: _ ->
            assertTrue false (sprintf "delivery-only skill %s must not also be emitted by this template" skill.Id)
        | false, [ _ ] -> ()
        | false, [] ->
            assertTrue
                false
                (sprintf
                    "no template source targets .agents/skills/%s/, so the manifest declares a skill the template never emits (#303)"
                    skill.Id)
        | false, many ->
            assertTrue
                false
                (sprintf
                    "skill %s is filled by %d template sources (%s) — exactly one may target .agents/skills/%s/ (#303)"
                    skill.Id
                    many.Length
                    (String.concat ", " many)
                    skill.Id)
    ...
    let rootRows =
        rows
        |> List.choose (fun r -> skillsRootPrefix r.Target |> Option.map (fun p -> r, p))
        ...
        for id, copiedFrom in rowMaterializesSkillDirs row prefix do
            ...
            | Some declared ->
                assertTrue
                    false
                    (sprintf
                        "template source %s -> %s materializes .agents/skills/%s/ from %s, shadowing its declared supplier %s (#303: the repo's own wrappers `#r` src/*/bin/Debug DLLs that exist in no product; narrow this row's `include`/`exclude`)"
                        row.Source
                        row.Target
                        id
                        copiedFrom
...
let private verifyBaseDocsNeutral () =
    ...
    let baseUngatedExcludes =
        ...
        |> Seq.filter (fun s ->
            (match s.TryGetProperty "source" with
             | true, v -> v.GetString() = "template/base/"
             | _ -> false)
            && (match s.TryGetProperty "condition" with
                | true, c -> String.IsNullOrWhiteSpace(c.GetString())
                | _ -> true))
        |> Seq.collect (fun s ->
    ...
    assertTrue
        (baseUngatedExcludes.Contains "CLAUDE.md")
        "template/base/ source must exclude CLAUDE.md (gated agent-context)"
...
let private verifyVerdictCore () =
    ...
    assertTrue
        (values = [ "spec-kit"; "sdd"; "typed-sdd"; "none" ])
        "lifecycle choices must remain spec-kit, sdd, typed-sdd, none in declaration order"
    ...
    printfn
        "verdict-core OK: covered-values %s; %d lifecycle-workspace sources carry `%s`; %d framework product-skill sources profile-gated & lifecycle-independent; %d product sources clean; %d manifest skills each filled by their declared supplier, %d root-targeting source(s) shadow none; directive agent-context docs lifecycle-safe"
...
let private installTemplateUnderTest (tmpRoot: string) =
    ...
    if code <> 0 then
        failwithf
            "could not install the working-tree template from %s (exit %d) — the live loop has no subject to audit:\n%s\n%s"
            repoRoot
            code
            out
            err
...
let private scaffold (tmpRoot: string) (profile: string) (extra: string list) (outSubdir: string) =
    ...
    if proc.HasExited && proc.ExitCode <> 0 && not (treeComplete ()) then
        failwithf
            "dotnet new failed for profile=%s %A (exit %d):\n%s\n%s"
            profile
            extra
            proc.ExitCode
            outTask.Result
...
let private scaffoldExpectFail (tmpRoot: string) (outSubdir: string) (extra: string list) =
    ...
    let args =
        [ "new"; "fs-gg-ui"; "--name"; productName; "--profile"; "app"; "-o"; outDir ]
        @ extra
...
type private ProfileVerdict =
    {
        ...
        DanglingRoutes: int
    } // 231: fs-gg-* skill-body path refs unresolvable in the product (must be 0)
...
let private emittedSkillId (target: string) =
    let m =
        Regex.Match(target.Replace('\\', '/'), @"^\.agents/skills/(fs-gg-[a-z0-9-]+)/?$")
...
let private verifyFrameworkSkillRoster () =
    ...
    for KeyValue(profile, expected) in expectedFrameworkSkills do
        ...
        assertTrue
            ...
            (sprintf
                ...
                profile
                (if Set.isEmpty extra then
                     "(none)"
                 else
                     extra |> Set.toList |> String.concat ", "))
    ...
    let newHoles =
        Set.difference (Set.difference offered (Set.ofList profiles)) (Set.ofList [ "game" ])
...
let private runMaterialize (dir: string) (expectFail: bool) =
    let script =
        Path.Combine(dir, ".specify", "scripts", "fs-gg", "materialize-skill-roots.fsx")

    if not (File.Exists script) then
        failwithf "%s: materialize script missing at %s" dir script
...
let private assertRootsByteIdentical (dir: string) (label: string) =
    ...
    for root in [ ".claude" ] do
        ...
        if agents <> other then
            failwithf
                "%s: %s/skills file set differs from .agents/skills: only-in-agents=%A only-in-%s=%A"
                label
                root
                (Set.difference agents other)
                root
                (Set.difference other agents)

        for rel in agents do
            let a =
                File.ReadAllBytes(Path.Combine(dir, ".agents", "skills", rel.Replace('/', Path.DirectorySeparatorChar)))

            let b =
                File.ReadAllBytes(Path.Combine(dir, root, "skills", rel.Replace('/', Path.DirectorySeparatorChar)))

            if a <> b then
                failwithf "%s: %s/skills/%s bytes diverge from .agents copy" label root rel

    if Directory.Exists(Path.Combine(dir, ".codex")) then
        failwithf
            "%s: .codex/ exists — .codex/skills is retired (ADR-0067 §5 / .github#1636); its presence resurrects the root #1121 removed"
            label
                ...
                for file in Directory.EnumerateFiles(skillDir, "*", SearchOption.AllDirectories) do
                    ...
                    for token in backticked do
                        ...
                        if not isPlaceholder && (looksRooted || isRelativeEscape) then
                            ...
                            let resolved =
                                if isRelativeEscape then
                                    Path.GetFullPath(
                                        Path.Combine(
                                            Path.GetDirectoryName file,
                                            cleaned.Replace('/', Path.DirectorySeparatorChar)
                                        )
                                    )
                                else
                                    Path.Combine(dir, cleaned.Replace('/', Path.DirectorySeparatorChar))
...
let private workspaceAbsent (dir: string) =
    ...
    && (Directory.Exists dir
        && (Directory.EnumerateDirectories(dir, "speckit-*", SearchOption.AllDirectories)
            |> Seq.isEmpty))
...
let private claudeProductSkillCount (dir: string) =
    orchestratorRootProductSkillCount dir ".claude"
...
let private codexRootAbsent (dir: string) =
    not (Directory.Exists(Path.Combine(dir, ".codex")))
...
let private assertNoWrapperDirs (dir: string) (profile: string) (specKit: bool) =
    ...
    let expected =
        let baseSet =
            ...
            | None ->
                failwithf
                    "profile %s is in `profiles` but has no entry in `expectedFrameworkSkills` — add its expected fs-gg-* set (see the note there)"
                    profile
    ...
    if not (Set.isEmpty extras) then
        failwithf
            "%s/%s: unexpected fs-gg-* skill dirs vendored (dev-surface wrappers, audit F3): %A"
            profile
            (if specKit then "spec-kit" else "sdd|none")
            extras
    ...
    if not (Set.isEmpty missing) then
        failwithf
            "%s/%s: expected fs-gg-* skills missing: %A"
            profile
            (if specKit then "spec-kit" else "sdd|none")
            missing
...
let private validateProfileLive (tmpRoot: string) (profile: string) =
    ...
    let sdd =
        scaffold tmpRoot profile [ "--lifecycle"; "sdd" ] (sprintf "%s-sdd" profile)

    let typedSdd =
        scaffold tmpRoot profile [ "--lifecycle"; "typed-sdd" ] (sprintf "%s-typed-sdd" profile)

    let specKit =
        scaffold tmpRoot profile [ "--lifecycle"; "spec-kit" ] (sprintf "%s-speckit" profile)

    let none_ =
        scaffold tmpRoot profile [ "--lifecycle"; "none" ] (sprintf "%s-none" profile)
    ...
    if not (hasGuardSentinel sdd) then
        failwithf
            "%s/sdd: guard sentinel %s absent (ADR-0056 §D2 fail-closed guard missing)"
            profile
            lifecycleGuardSentinelRel
    ...
    if hasGuardSentinel specKit then
        failwithf
            "%s/spec-kit: guard sentinel %s present (must be sdd-only, ADR-0056 §D2)"
            profile
            lifecycleGuardSentinelRel

    // ---- sdd lane ----
    if not (workspaceAbsent sdd) then
        failwithf "%s/sdd: lifecycle workspace not fully absent" profile

    if not (productPresent sdd) then
        failwithf "%s/sdd: product missing" profile
    // FR-001 (positive): the framework `fs-gg-*` product skills ARE present under sdd.
    let sddSkills = frameworkSkillCount sdd

    if sddSkills < 1 then
        failwithf "%s/sdd: no framework fs-gg-* skills present (FR-001 broken)" profile
    ...
    if not (manifestPresent sdd) then
        failwithf "%s/sdd: .agents/skills/skill-manifest.json missing" profile
    ...
    if sddClaudeProduct <> 0 then
        failwithf
            "%s/sdd: %d fs-gg-* product skills leaked into .claude/skills/ (providerWroteSddTree, #47)"
            profile
            sddClaudeProduct

    if not sddCodexAbsent then
        failwithf "%s/sdd: .codex/ exists — .codex/skills is retired (ADR-0067 §5 / .github#1636, #1121)" profile
    // FR-006: the full-registry catalog is NOT emitted under sdd (it would dangle).
    if not (catalogAbsent sdd) then
        failwithf "%s/sdd: docs/skillist-reference.md emitted (would dangle, FR-006 broken)" profile

    // ---- typed-sdd lane ----
    if not (workspaceAbsent typedSdd) then
        failwithf "%s/typed-sdd: lifecycle workspace not fully absent" profile

    if not (productPresent typedSdd) then
        failwithf "%s/typed-sdd: product missing" profile

    let typedSddSkills = frameworkSkillCount typedSdd

    if typedSddSkills < 1 then
        failwithf "%s/typed-sdd: no framework fs-gg-* skills present" profile

    if not (manifestPresent typedSdd) then
        failwithf "%s/typed-sdd: .agents/skills/skill-manifest.json missing" profile
    ...
    if claudeProductSkillCount typedSdd <> 0 then
        failwithf "%s/typed-sdd: product skills leaked into .claude/skills/" profile

    if not (codexRootAbsent typedSdd) then
        failwithf "%s/typed-sdd: .codex/ exists" profile

    if not (catalogAbsent typedSdd) then
        failwithf "%s/typed-sdd: docs/skillist-reference.md emitted" profile
    ...
    if not (typedGuard.Contains "typed-sdd") then
        failwithf "%s/typed-sdd: sentinel does not preserve typed-sdd intent" profile

    if typedGuard = sddGuard then
        failwithf "%s/typed-sdd: sentinel is byte-identical to sdd; typed intent was aliased" profile

    let withoutGuard dir =
        treeFingerprint dir
        |> List.filter (fun (rel, _) -> rel <> lifecycleGuardSentinelRel)
    ...
    if not (Set.isEmpty nonGatedAdded) then
        failwithf
            "%s/spec-kit: adds NON-gated files vs the default (FR-009 broken): %s"
            profile
            (String.concat ", " (Set.toList nonGatedAdded))

    if removedBySpecKit <> Set.singleton lifecycleGuardSentinelRel then
        failwithf
            "%s/spec-kit: differs from the default in files other than the guard sentinel (expected only %s removed, got removed=%A)"
            profile
            lifecycleGuardSentinelRel
            (Set.toList removedBySpecKit)

    // ---- none lane ----
    if not (workspaceAbsent none_) then
        failwithf "%s/none: lifecycle workspace not fully absent" profile

    if not (productPresent none_) then
        failwithf "%s/none: product missing" profile

    let noneSkills = frameworkSkillCount none_

    if noneSkills < 1 then
        failwithf "%s/none: no framework fs-gg-* skills present (FR-001 broken)" profile

    if not (manifestPresent none_) then
        failwithf "%s/none: .agents/skills/skill-manifest.json missing" profile
    ...
    if noneClaudeProduct <> 0 then
        failwithf "%s/none: %d fs-gg-* product skills leaked into .claude/skills/ (#47)" profile noneClaudeProduct

    if not noneCodexAbsent then
        failwithf "%s/none: .codex/ exists — .codex/skills is retired (ADR-0067 §5 / .github#1636, #1121)" profile

    if not (catalogAbsent none_) then
        failwithf "%s/none: docs/skillist-reference.md emitted (would dangle, FR-006 broken)" profile
    // none carries NO guard sentinel — the intent-keyed distinction from sdd (ADR-0056 §D2).
    if hasGuardSentinel none_ then
        failwithf
            "%s/none: guard sentinel %s present (must be sdd-only, ADR-0056 §D2)"
            profile
            lifecycleGuardSentinelRel
...
let private validateCompositionMatrix (tmpRoot: string) (values: string list) =
    ...
    for lc in values do
        for p in profiles do
            let dir =
                scaffold tmpRoot p [ "--lifecycle"; lc; "--designSystem"; "ant" ] (sprintf "mtx-%s-%s-ant" lc p)
...
let private validateEnforceRedCase (tmpRoot: string) =
    ...
    if code = 0 then
        failwith "enforce-red-case: corrupted fs-gg-scene body did NOT fail --enforce"
...
let private validateUnknownRejected (tmpRoot: string) =
    ...
    if code = 0 then
        failwith "unknown --lifecycle value was accepted (should fail fast)"

    if treeExists then
        failwith "unknown --lifecycle value produced an output tree (should be none)"

    "rejected"

// ---- report rendering -------------------------------------------------------------------------

let private renderReport
    (values: string list)
    (provenance: string)
    (verdicts: ProfileVerdict list)
    (matrixCount: int)
    (unknown: string)
    (enforceRedCase: string)
    =
    ...
    line (
        sprintf
            "composition-matrix: %d/16 generate; ant-overlay-present=ok; feedback-gated-under-non-speckit=ok"
            matrixCount
    )
    ...
    for v in verdicts do
        line (
            sprintf
                "sdd/%s: claude-product-skills=%d codex-root-absent=%b"
                v.Profile
                v.SddClaudeProductSkills
                v.SddCodexRootAbsent
        )
    ...
    for v in verdicts do
        line (
            sprintf
                "none/%s: claude-product-skills=%d codex-root-absent=%b"
                v.Profile
                v.NoneClaudeProductSkills
                v.NoneCodexRootAbsent
        )
...
let private runValidation () =
    ...
    let liveGate =
        Environment.GetEnvironmentVariable "FS_GG_RUN_LIFECYCLE_VALIDATION" = "1"

    if emitReport && not liveGate then
        let report =
            renderReport values verdictCoreProvenance (synthVerdicts ()) 16 "rejected" "ok"
