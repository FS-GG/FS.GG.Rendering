...
> **Platform vs. workspace.** FS-GG is a **platform** — a family of repositories, of which
> this UI framework is one **component**. The current component/repository inventory lives in
> the org [architecture doc](https://github.com/FS-GG/.github/blob/main/docs/architecture.md)
> (linked, not restated here). What you scaffold *with* the platform is a **workspace**: a
> generated repo with a runnable app, the `.fsgg/` lifecycle, skills, and optional governance.
> See the [platform vocabulary (ADR-0020)](https://github.com/FS-GG/.github/blob/main/docs/adr/0020-platform-workspace-component-vocabulary.md).
...
Published as `FS.GG.UI.*` packages on `net10.0` — 16 libraries plus the `FS.GG.UI` BOM metapackage (current framework version `0.27.0`).
