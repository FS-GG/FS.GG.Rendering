...
framework version `0.10.0`). Every release **dual-publishes** the byte-identical
...
1. **Public feed** — restore straight from nuget.org (no source configuration needed):
   ```sh
   dotnet add package FS.GG.UI.SkiaViewer --version 0.10.0
   dotnet add package FS.GG.UI.Controls   --version 0.10.0
   ```
