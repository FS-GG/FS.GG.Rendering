...
The libraries are published as `FS.GG.UI.*` packages targeting **`net10.0`**. Every
release **dual-publishes** the byte-identical
...
The `FS.GG.UI.*` libraries plus the `FS.GG.UI` BOM metapackage (see [module map](product/module-map.md)
