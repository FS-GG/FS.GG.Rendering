...
| Default theme | `Themes.Default` | Themes | The default Light/Dark `Theme` value module, `Theming` mode/accent derivation, and the DTCG token source. | import-from-source | Owned assembly `FS.GG.UI.Themes.Default` (feature 125); depends only on `DesignSystem`. |
| Ant Design theme | `Themes.AntDesign` | Themes | The Ant Design `Theme` values (`AntTheme.antLight`/`antDark`) and the `AntIntentPolicy` (`StyleResolver.IntentPolicy`) that drive Ant's visual language over the existing + net-new generic controls. | feature 132 | Owned assembly `FS.GG.UI.Themes.AntDesign` (feature 132/D2.1, decision [0006](./decisions/0006-antdesign-theme-and-new-controls.md)); depends only on `DesignSystem` — no control fork. Further concrete themes (Fluent, Material-inspired) remain future work. |
