...
| Design-system primitives | `DesignSystem` | DesignSystem | Token model, theme records, density, typography, radii, color roles, visual-state rules, the pure `Style.resolve` resolver. | import-from-source | Owned assembly `FS.GG.UI.DesignSystem` (feature 125, decision [0003](./decisions/0003-designsystem-namespace-relocation.md)); depends only on `Scene`. |
