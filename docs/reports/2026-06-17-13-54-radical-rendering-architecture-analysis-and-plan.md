...
| [x] | Feature 160 | P7 | Performance validation throughput | `specs/160-performance-validation-throughput/readiness/`; focused throughput accepted on `probe-08a47c01`; full validation passed on final retry; performance claim remains `performance-not-accepted` pending Feature 161. |
| [ ] | Feature 161 | P7 | Host performance lane ledger | Open P7 performance follow-up. |
...
- [x] **Feature 160 — performance validation throughput.** Splits long-running broad suites from
   ...
   performance claim remains `performance-not-accepted` pending Feature 161 host-lane scoping.
- [ ] **Feature 161 — host performance lane ledger.** Record host-specific GPU/display facts for each
...
timing lane is not noisy and Feature 161 scopes the result to a known lane. If any part is missing,
noisy, cross-profile, or non-beneficial, the report must say
