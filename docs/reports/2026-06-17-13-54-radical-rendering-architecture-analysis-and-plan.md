...
| **Status** | Current as of local validation on 2026-06-18: P0-P6 are implemented and landed. P7 is implemented through Feature 149 and remains environment-limited for live partial-redraw acceptance. Feature 150 implements the first P8/R3b intrinsic-layout slice and was squash-merged to `main` as `acad00d`. Feature 151 completes the remaining P8 acceptance package on `151-complete-p8-layout`: representative layout and ScrollViewer corpus coverage, measured/intrinsic dependency identity evidence, stale-rejection classification, full/incremental parity, broad regression classification, compatibility ledger, readiness summary, and package validation. Focused and broad Feature151 validation passed, `dotnet test FS.GG.Rendering.slnx --no-restore` passed, source packages pack at `0.1.13-preview.1`, and the template package packs at `0.1.7-preview.1`. P8 is accepted for the current public Feature150 protocol; P7 live partial-redraw proof remains environment-limited and no compositor performance claim is accepted. |
...
Feature151 validation passed for restore, solution build, focused Feature151 filters, broad regression filters, public surface checks after stable baseline sync, full solution tests, source package pack at `0.1.13-preview.1`, and template package pack at `0.1.7-preview.1`.
