...
| **Status** | Current as of local validation on 2026-06-18 08:08 CEST: P0-P6 are implemented and landed. P7 is implemented through Feature 149 and remains environment-limited for live partial-redraw acceptance. Feature 150 implements the first P8/R3b intrinsic-layout slice and was squash-merged to `main` as `acad00d`: 37/58 Spec Kit tasks are complete, focused Feature150 tests pass across Layout, Controls, Controls.Elmish, Testing, and Package, `dotnet build FS.GG.Rendering.slnx --no-restore` passes, public surface baselines are refreshed with additive Feature150 deltas, source packages pack at `0.1.13-preview.1`, and the template package packs at `0.1.7-preview.1`. Final P8 acceptance remains open for the full representative corpus, evaluator-internal measured/intrinsic cache reuse, broad regression evidence, and full solution test. |
...
| **P8 - Radical layout** | First bounded intrinsic-layout slice implemented as Feature 150 and squash-merged to `main` as `acad00d`. Layout now exposes constraints-down/sizes-up and intrinsic query/result contracts, ScrollViewer extent readback uses `Layout.contentExtent` instead of descendant bounds walking, Controls.Elmish exposes deterministic layout work metrics, Testing exposes layout readiness helpers, and readiness artifacts disclose the remaining full P8 scope. | `specs/150-intrinsic-layout-protocol/tasks.md` (37/58 complete); `specs/150-intrinsic-layout-protocol/readiness/validation-summary.md`; focused Feature150 tests passed for Layout.Tests, Controls.Tests, Elmish.Tests, Testing.Tests, and Package.Tests; `dotnet build FS.GG.Rendering.slnx --no-restore` passed; `dotnet fsi scripts/refresh-surface-baselines.fsx` passed; source packages pack at `0.1.13-preview.1`; template package packs at `0.1.7-preview.1`. Full representative corpus, evaluator-internal cache reuse, broad regression evidence, and full solution test remain open. |
