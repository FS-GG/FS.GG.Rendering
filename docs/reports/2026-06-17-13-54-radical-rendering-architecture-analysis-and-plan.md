...
| [ ] | Feature 158 | P7 | Separate proof readback from timing | Open P7 performance follow-up. |
...
**Remaining P7 caveats after Feature 157.** Correctness acceptance is scoped to the current stable
...
claimed compositor speedup yet. Feature157 records the no-clear damage-scissored readiness slice, but a future performance closeout still needs readback-free timing separation, net-positive reuse/promotion counters, and a host lane ledger. Restoring a root `fake.sh` wrapper would remove the separate package-target tooling
...
- [ ] **Feature 158 — separate proof readback from timing.** Keep screenshot/readback in the proof
...
Feature157 damage-scissored readiness slice is accepted for profile `probe-08a47c01`. A
performance claim is still accepted only when the same-profile timing lane is not noisy,
Feature 158 proves the measurement path is not readback-dominated, Feature 159 records
net-positive reuse/promotion counters, and Feature 161 scopes the result to a known lane. If any
part is missing, noisy, cross-profile, or non-beneficial, the report must say
