...
| **Status** | Current as of local validation on 2026-06-18 08:08 CEST: P0-P6 are implemented and landed. P7 is implemented through Feature 149 and remains environment-limited for live partial-redraw acceptance; source packages are at `0.1.12-preview.1` and the template package is at `0.1.6-preview.1`. Feature 150 implements the first P8/R3b intrinsic-layout slice on branch `150-intrinsic-layout-protocol`: 37/58 Spec Kit tasks are complete, focused Feature150 tests pass across Layout, Controls, Controls.Elmish, Testing, and Package, `dotnet build FS.GG.Rendering.slnx --no-restore` passes, and public surface baselines are refreshed with additive Feature150 deltas. Final P8 acceptance remains open for the full representative corpus, evaluator-internal measured/intrinsic cache reuse, broad regression evidence, and final package pack validation. |
...
| **P7 - Compositor** | Implemented through the current evidence/readiness scope but still environment-limited for live acceptance. Feature 147 is landed and provides proof/scissor/policy contracts plus deterministic readiness artifacts. Feature 148 adds the focused P7 evidence/readiness layer. Feature 149 completes the Spec Kit task checklist and is squash-merged as `a9a1ef1`, with first-class `--feature 149` CLI routing, corpus/readiness inventories, package FSI coverage, consumer readiness helper tests, focused Controls/SkiaViewer/Elmish/Harness/Package/Testing tests, generated readiness artifacts, and a final P7 validation package. The live framebuffer sentinel/damage readback interpreter still reports environment-limited, so damage-scoped redraw remains fallback-gated and no performance claim is accepted. | `specs/147-compositor-damage-redraw/readiness/validation-summary.md`; `specs/148-compositor-live-integration/tasks.md` (61/76 complete); `specs/149-complete-compositor-p7/tasks.md` (68/68 complete); `specs/149-complete-compositor-p7/readiness/validation-summary.md`; focused Feature149 tests passed for Rendering.Harness.Tests, Package.Tests, Testing.Tests, Controls.Tests, Elmish.Tests, and SkiaViewer.Tests; Feature149 harness commands generated live proof/parity/reuse/snapshot/timing/readiness artifacts; `dotnet build FS.GG.Rendering.slnx --no-restore` passed; source packages bumped to `0.1.12-preview.1`; template package bumped to `0.1.6-preview.1`. |
| **P8 - Radical layout** | First bounded intrinsic-layout slice implemented as Feature 150 on branch `150-intrinsic-layout-protocol`. Layout now exposes constraints-down/sizes-up and intrinsic query/result contracts, ScrollViewer extent readback uses `Layout.contentExtent` instead of descendant bounds walking, Controls.Elmish exposes deterministic layout work metrics, Testing exposes layout readiness helpers, and readiness artifacts disclose the remaining full P8 scope. | `specs/150-intrinsic-layout-protocol/tasks.md` (37/58 complete); `specs/150-intrinsic-layout-protocol/readiness/validation-summary.md`; focused Feature150 tests passed for Layout.Tests, Controls.Tests, Elmish.Tests, Testing.Tests, and Package.Tests; `dotnet build FS.GG.Rendering.slnx --no-restore` passed; `dotnet fsi scripts/refresh-surface-baselines.fsx` passed. Full representative corpus, evaluator-internal cache reuse, broad regression evidence, and pack validation remain open. |
...
Feature 150 current status: Feature 150 implements the first P8 intrinsic-layout slice on `150-intrinsic-layout-protocol`. It adds explicit Layout constraints, measurement, child placement, intrinsic query/result, content extent, and cache-entry records; replaces ScrollViewer descendant-bound extent readback with `Layout.contentExtent`; adds Controls.Elmish layout metrics and Testing layout-readiness helpers; refreshes public surface baselines; and records readiness evidence under `specs/150-intrinsic-layout-protocol/readiness/`. Validation passed for focused Feature150 filters across Layout.Tests, Controls.Tests, Elmish.Tests, Testing.Tests, and Package.Tests plus `dotnet build FS.GG.Rendering.slnx --no-restore`. The task ledger is 37/58 complete because the full representative layout/ScrollViewer corpus, evaluator-internal measured/intrinsic cache reuse, retained rendering regression sweep, full solution test, and package pack validation remain open.
...
| **P7 — Compositor** | R6 (present-path proof, promotion, scissor, key split, texture) | Needs R2 modifiers; pure perf, gated by probes | Implemented through the Feature149 evidence/readiness package: Feature 147 landed as squash merge `85b0b7e`, package bump `2247cb3` to `0.1.10-preview.1`; Feature 148 landed as squash merge `7d708c4`; Feature 149 landed as squash merge `a9a1ef1`, with 68/68 tasks checked, focused Feature149 filters and solution build passing, generated readiness artifacts, source packages bumped to `0.1.12-preview.1`, and template package bumped to `0.1.6-preview.1`. Current readiness remains `environment-limited` because live sentinel/damage readback has not produced three accepted capable-host artifacts; partial redraw and performance claims remain fallback-gated. |
| **P8 — Radical layout** | R3b (intrinsic protocol) | Needs R1b; removes the scrollViewport descendant-walk smell | First slice implemented as Feature 150: constraints/intrinsics/content extent contracts, ScrollViewer intrinsic extent readback, focused parity/cache/readiness tests, solution build, and surface refresh pass. Full P8 exit criteria remain open for representative corpus breadth, evaluator-internal cache reuse, broad regression evidence, full solution test, and package pack validation. |
...
*End of report. This document chooses the radical options and tracks their delivery status. P0-P6 are implemented and landed through Feature 146 plus package version `0.1.9-preview.1`; P7 has Feature 147's deterministic readiness slice landed on `main` plus package version `0.1.10-preview.1`, Feature 148's focused evidence/readiness layer squash-merged as `7d708c4`, and Feature 149 squash-merged as `a9a1ef1` with 68/68 tasks checked, focused Feature149 validation passing, generated readiness artifacts, solution build passing, source packages bumped to `0.1.12-preview.1`, and template package bumped to `0.1.6-preview.1`. P7 remains environment-limited for live partial-redraw acceptance until three fresh capable-host sentinel/damage readback proofs pass. P8 is now started through Feature 150's first intrinsic-layout slice, with 37/58 tasks complete and final broad acceptance still open.*
