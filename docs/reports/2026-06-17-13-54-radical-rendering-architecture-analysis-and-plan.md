...
| **Repo state** | Originally analyzed at branch `main` @ `8f75594`; implementation/package status audited through Feature 160 readiness acceptance on `160-performance-validation-throughput` after Feature 159 squash merge `e576257` and the post-merge package bump to source packages `0.1.22-preview.1` plus template package `0.1.14-preview.1` |
...
| **Status** | Current as of Feature 160 implementation status on 2026-06-18: P0-P6 are implemented and landed. P7 is implemented through Feature 149 and extended by Feature 152, Feature 153, Feature 154, local Feature 155, Feature 156, Feature 157, Feature 158, Feature 159, and Feature 160. Feature 152 added explicit three-run live proof-set acceptance, consumer compositor-readiness helpers, Feature152 harness routing, focused test coverage, readiness evidence, and package/surface validation; it was squash-merged to `main` as `8ea61c4`, then packable source projects were bumped to `0.1.15-preview.1` and the template package to `0.1.9-preview.1` in `61d1ce8`. Feature 153 implements the proof-interpreter slice: selected-attempt proof-set identity, pure live-host readiness classification, Feature153 harness routing/readiness renderers, focused tests, FSI transcript coverage, surface-baseline refresh, and durable readiness evidence under `specs/153-compositor-proof-interpreter/readiness/`; it was squash-merged to `main` as `d7c539c`, and the follow-up package bump raises source packages to `0.1.16-preview.1` and the template package to `0.1.10-preview.1`. Feature 154 implements the current proof-acceptance closeout: exact proof-set acceptance, same-profile parity, timing-decision evidence, final readiness/compatibility/package/regression artifacts, FSI transcript coverage, and focused tests across SkiaViewer, Rendering.Harness, Controls, Elmish, Testing, and Package; it was squash-merged to `main` as `2c9af24` and pushed to `origin/main`. The local feature branch was deleted, no stale remote feature branch was present, and the post-merge source package bump raises packages to `0.1.17-preview.1`. Focused Feature154 filters pass, broad `dotnet test FS.GG.Rendering.slnx --no-restore` passes locally, and unsupported-host quickstart exits in approximately 0.6s with zero accepted partial-redraw artifacts. Root `./fake.sh` remains absent, so Fake wrapper package targets are recorded as tooling-limited here. Feature 155 accepts P7 live partial-redraw correctness for current stable host profile `probe-08a47c01` with three fresh sentinel/damage attempts and same-profile parity. Feature 156 was squash-merged as `3dc6530` and adds same-profile full-redraw versus damage-scoped timing evidence on accepted profile `probe-08a47c01`; the live run is `noisy`, unsupported hosts remain `environment-limited` with zero accepted performance artifacts, the shipped compositor performance claim remains `performance-not-accepted`, and source packages are bumped to `0.1.19-preview.1` in `276b226`. Feature 157 was squash-merged as `de6033c`, pushed to `origin/main`, followed by package bump `b1e5888`, and had its local feature branch deleted with no stale remote feature branch present; it implements the no-clear damage-scissored decision/readiness slice on the same accepted profile with five required scenarios, fail-closed fallback evidence, unsupported-host validation in 10s, FSI/package coverage, and readiness under `specs/157-no-clear-damage-scissor/readiness/`; focused Feature157 filters pass, the Feature155/Feature156/Feature157 focused harness regression passes, broad solution validation was interrupted after several minutes without output, and the shipped performance claim remains `performance-not-accepted`. Feature 158 was squash-merged as `77e8a3c`, pushed to `origin/main`, followed by package bump `ae6574f` to source packages `0.1.21-preview.1` and template package `0.1.13-preview.1`, and accepts measurement separation on profile `probe-08a47c01`: the readback-free timing run records 50 accepted samples across five scenarios, explicit proof/readback probe evidence is preserved and excluded as `probe-run-excluded`, unsupported hosts fail closed, focused Feature158 and P7 harness filters pass, broad `dotnet test FS.GG.Rendering.slnx --no-restore` passes, and the shipped compositor performance claim remains `performance-not-accepted`. Feature 159 was squash-merged as `e576257` and adds accepted layer promotion/content-placement split counters on profile `probe-08a47c01`; focused Feature159 filters, Feature155/157/158/159 preservation, broad solution validation, and unsupported-host validation pass, with readiness under `specs/159-layer-promotion-keys/readiness/`. The post-merge package bump raises source packages to `0.1.22-preview.1` and the template package to `0.1.14-preview.1`. Feature 160 accepts bounded performance validation throughput on profile `probe-08a47c01`: focused throughput is accepted with three same-profile iterations under the 10 minute bound, full solution validation passes on final retry, readiness is under `specs/160-performance-validation-throughput/readiness/`, and the shipped compositor performance claim remains `performance-not-accepted`. Feature 161 accepts host-lane scoping for X11 `:1` direct OpenGL AMD/Mesa on `probe-08a47c01` with readiness under `specs/161-host-performance-lane-ledger/readiness/`. Feature 150 implements the first P8/R3b intrinsic-layout slice and was squash-merged to `main` as `acad00d`. Feature 151 completes the remaining P8 acceptance package and was squash-merged to `main` as `6f9d606`; P8 is accepted for the current public Feature150 protocol. |
...
### Current status update (2026-06-18 17:43 CEST)

This report began as a plan from `main` @ `8f75594`. The repository has since shipped P0-P3, pushed package version `0.1.3-preview.1`, implemented Feature 142 / P4 on `142-harfbuzz-text-shaping`, implemented the Feature 143 / P5 pure overlay coordinator on `143-interaction-overlay-state`, implemented Feature 144 / P5 host/widget overlay integration on `144-overlay-host-widget-integration`, implemented and merged Feature 145 / P5 overlay visual proof, bumped packages to `0.1.8-preview.1`, implemented Feature 146 / P6 render-anywhere, squash-merged it to `main` as `c0f16ce`, bumped packages to `0.1.9-preview.1` in `d62b026`, implemented Feature 152 / P7 live-proof closeout as squash merge `8ea61c4` followed by package bump `61d1ce8`, implemented Feature 153 / P7 proof-interpreter as squash merge `d7c539c`, implemented Feature 154 / P7 proof-acceptance closeout as squash merge `2c9af24` followed by source package bump `0.1.17-preview.1`, implemented Feature 156 / P7 same-profile timing evidence as squash merge `3dc6530` followed by source package bump `276b226` to `0.1.19-preview.1`, implemented Feature 157 / P7 no-clear damage-scissored readiness as squash merge `de6033c`, pushed it to `origin/main`, bumped source packages to `0.1.20-preview.1` and the template package to `0.1.12-preview.1` in `b1e5888`, deleted the local Feature 157 branch with no stale remote feature branch present, implemented Feature 158 measurement separation with readiness under `specs/158-separate-proof-timing/readiness/`, squash-merged it to `main` as `77e8a3c`, pushed `main`, deleted the local Feature 158 branch with no stale remote feature branch present, bumped source packages to `0.1.21-preview.1` plus the template package to `0.1.13-preview.1` in `ae6574f`, implemented Feature 159 layer promotion/content-placement key split with readiness under `specs/159-layer-promotion-keys/readiness/`, squash-merged it to `main` as `e576257`, bumped source packages to `0.1.22-preview.1` plus the template package to `0.1.14-preview.1`, and implemented Feature 160 performance validation throughput with accepted focused throughput plus passing full validation under `specs/160-performance-validation-throughput/readiness/`:

### Feature completion checklist

This table is the feature-level tracker. `Done` means the feature slice has its implementation
and readiness evidence recorded; unchecked rows are still open implementation work.

| Done | Feature | Phase | Scope | Evidence / next step |
|---|---|---|---|---|
| [x] | Feature 138 | P0 | Layout attributes and Elmish metrics fix | Squash merge `5ae3dad`; package bump `c1318ee`. |
| [x] | Feature 139 | P1 | Shared current-node assembly seam | Squash merge `f92621d`; package bump `143342f`. |
| [x] | Feature 140 | P2 | Internal modifier/layer IR foundation and glyph-run proof spike | Squash merge `ac2b560`; package bump `41fb05c`. |
| [x] | Feature 141 | P3 | Retained renderer unification | `specs/141-retained-renderer-unification/readiness.md`. |
| [x] | Feature 142 | P4 | HarfBuzz text shaping slice | `specs/142-harfbuzz-text-shaping/readiness/validation-log.md`. |
| [x] | Feature 143 | P5 | Pure overlay coordinator | `specs/143-interaction-overlay-state/readiness/validation-log.md`. |
| [x] | Feature 144 | P5 | Host/widget overlay integration | `specs/144-overlay-host-widget-integration/readiness/README.md`. |
| [x] | Feature 145 | P5 | Overlay visual proof closeout | Squash merge `b632d93`; package bump `512c6b0`. |
| [x] | Feature 146 | P6 | Render-anywhere protocol | Squash merge `c0f16ce`; package bump `d62b026`. |
| [x] | Feature 147 | P7 | Deterministic compositor readiness slice | Squash merge `85b0b7e`; package bump `2247cb3`. |
| [x] | Feature 148 | P7 | Focused compositor evidence/readiness layer | Squash merge `7d708c4`; readiness under `specs/148-compositor-live-integration/readiness/`. |
| [x] | Feature 149 | P7 | Complete P7 evidence/readiness Spec Kit package | Squash merge `a9a1ef1`; `specs/149-complete-compositor-p7/readiness/validation-summary.md`. |
| [x] | Feature 150 | P8 | Intrinsic layout protocol substrate | Squash merge `acad00d`; completed by Feature 151 for final P8 acceptance. |
| [x] | Feature 151 | P8 | Final P8 acceptance package | Squash merge `6f9d606`; `specs/151-complete-p8-layout/readiness/validation-summary.md`. |
| [x] | Feature 152 | P7 | Explicit live proof-set acceptance closeout | Squash merge `8ea61c4`; package bump `61d1ce8`. |
| [x] | Feature 153 | P7 | Proof-interpreter slice | Squash merge `d7c539c`; `specs/153-compositor-proof-interpreter/readiness/validation-summary.md`. |
| [x] | Feature 154 | P7 | Proof-acceptance closeout package | Squash merge `2c9af24`; source packages bumped to `0.1.17-preview.1`. |
| [x] | Feature 155 | P7 | Current-host P7 correctness closeout | `specs/155-native-proof-capture/readiness/validation-summary.md`; local closeout evidence recorded. |
| [x] | Feature 156 | P7 | Same-profile timing evidence | Squash merge `3dc6530`; package bump `276b226`; `specs/156-same-profile-timing/readiness/validation-summary.md`; live same-profile timing verdict `noisy`; performance claim remains `performance-not-accepted`. |
| [x] | Feature 157 | P7 | No-clear damage-scissored render path | Squash merge `de6033c`; package bump `b1e5888`; `specs/157-no-clear-damage-scissor/readiness/`; focused Feature157 filters passed; broad solution validation interrupted after partial output; performance claim remains `performance-not-accepted`. |
| [x] | Feature 158 | P7 | Separate proof readback from timing | Squash merge `77e8a3c`; package bump `ae6574f`; `specs/158-separate-proof-timing/readiness/`; 50 accepted readback-free timing samples; proof/readback probe excluded as `probe-run-excluded`; performance claim remains `performance-not-accepted`. |
| [x] | Feature 159 | P7 | Layer promotion and content/transform key split | `specs/159-layer-promotion-keys/readiness/`; squash merge `e576257`; source packages `0.1.22-preview.1`; template package `0.1.14-preview.1`; accepted net-positive promotion/reuse counters; unsupported hosts fail closed; performance claim remains `performance-not-accepted`. |
| [x] | Feature 160 | P7 | Performance validation throughput | `specs/160-performance-validation-throughput/readiness/`; focused throughput accepted on `probe-08a47c01`; full validation passed on final retry; performance claim remains `performance-not-accepted` until every timing/reuse/throughput/lane gate is accepted for the named lane. |
| [x] | Feature 161 | P7 | Host performance lane ledger | `specs/161-host-performance-lane-ledger/readiness/`; host-lane facts accepted for X11 `:1` direct OpenGL AMD/Mesa profile `probe-08a47c01`; non-generalized lanes listed; full validation passed; shipped performance claim remains `performance-not-accepted`. |
...
| **P7 - Compositor** | Implemented through the current evidence/readiness scope, with current-host correctness accepted by Feature 155, same-profile timing evidence recorded by Feature 156, Feature 157 no-clear damage-scissored readiness landed, Feature 158 measurement separation landed, and Feature 160 performance validation throughput is accepted. Feature 147 is landed and provides proof/scissor/policy contracts plus deterministic readiness artifacts. Feature 148 adds the focused P7 evidence/readiness layer. Feature 149 completes the Spec Kit task checklist and is squash-merged as `a9a1ef1`, with first-class `--feature 149` CLI routing, corpus/readiness inventories, package FSI coverage, consumer compositor-readiness helper tests, focused Controls/SkiaViewer/Elmish/Harness/Package/Testing tests, generated readiness artifacts, and a final P7 validation package. Feature 152 adds the explicit three-run accepted proof-set contract, Feature152 harness routing, compositor readiness helpers, FSI/package coverage, readiness summary, compatibility ledger, and focused regression validation. Feature 153 adds the proof interpreter/readiness slice for exact selected attempts, live-host classification, Feature153 harness routes, FSI transcript coverage, focused validation, and readiness evidence. Feature 154 adds exact proof-acceptance, same-profile parity, timing-decision, final readiness, compatibility, package, regression, and transcript coverage; it is squash-merged as `2c9af24`, pushed to `origin/main`, and source packages are bumped to `0.1.17-preview.1`. Feature 155 produces three accepted fresh sentinel/damage proof attempts on stable host profile `probe-08a47c01`, loads them through `compositor-readiness --feature 155`, records same-profile parity accepted, and keeps unsupported hosts fail-closed. Feature 156 is squash-merged as `3dc6530`, measures full-redraw versus damage-scoped paths on the same accepted host profile with five scenarios and five samples per path, and is followed by package bump `276b226` to `0.1.19-preview.1`; the result is `noisy`, unsupported-host timing remains `environment-limited` with zero accepted performance artifacts, and the shipped performance claim remains `performance-not-accepted`. Feature 157 is squash-merged as `de6033c`, followed by package bump `b1e5888`, and adds proof-gated no-clear damage-scoped decision helpers, `compositor-damage --feature 157`, `compositor-readiness --feature 157`, accepted same-profile damage evidence for five scenarios, fail-closed fallback evidence, unsupported-host validation in 10s, public surface baselines, FSI transcript coverage, and package/regression readiness; broad solution validation was interrupted after several minutes without output, so no broad-suite pass is claimed for Feature 157. Feature 158 is squash-merged as `77e8a3c`, followed by package bump `ae6574f`, and adds `compositor-performance --feature 158`, `--probe-readback`, and `compositor-readiness --feature 158`; accepted timing samples use `readback-free-timing-v1`, explicit proof/readback probe artifacts stay inspectable but excluded, unsupported hosts remain `environment-limited`, focused and broad validation pass, and the shipped performance claim remains `performance-not-accepted`. Feature 160 adds `compositor-performance --feature 160 --lane focused` and `compositor-readiness --feature 160`, accepts three same-profile focused iterations under the 10 minute bound on `probe-08a47c01`, keeps full solution validation as the release gate, records FSI/package/regression evidence. Feature 161 adds `compositor-performance --feature 161 --lane host-ledger` and `compositor-readiness --feature 161`, accepts the X11 `:1` direct OpenGL AMD/Mesa lane facts for `probe-08a47c01`, lists non-generalized lanes, and keeps the shipped performance claim `performance-not-accepted`. | `specs/147-compositor-damage-redraw/readiness/validation-summary.md`; `specs/148-compositor-live-integration/tasks.md` (61/76 complete); `specs/149-complete-compositor-p7/tasks.md` (68/68 complete); `specs/149-complete-compositor-p7/readiness/validation-summary.md`; `specs/152-compositor-live-proof/tasks.md` (66/66 complete); `specs/152-compositor-live-proof/readiness/validation-summary.md`; `specs/153-compositor-proof-interpreter/tasks.md` (68/68 complete); `specs/153-compositor-proof-interpreter/readiness/validation-summary.md`; `specs/154-compositor-proof-acceptance/tasks.md` (70/70 complete); `specs/154-compositor-proof-acceptance/readiness/validation-summary.md`; `specs/155-native-proof-capture/readiness/validation-summary.md`; `specs/156-same-profile-timing/tasks.md` (70/70 complete); `specs/156-same-profile-timing/readiness/validation-summary.md`; `specs/157-no-clear-damage-scissor/tasks.md` (75/75 complete); `specs/157-no-clear-damage-scissor/readiness/validation-summary.md`; `specs/158-separate-proof-timing/tasks.md` (70/70 complete); `specs/158-separate-proof-timing/readiness/validation-summary.md`; `specs/160-performance-validation-throughput/tasks.md` (57/57 complete); `specs/160-performance-validation-throughput/readiness/validation-summary.md`; focused Feature156 filters passed for Rendering.Harness, SkiaViewer, and Testing; focused Feature157 filters passed for SkiaViewer, Rendering.Harness, Testing, and Package; focused Feature158 filters passed for Rendering.Harness and Package; Feature156 and Feature158 broad solution validation passed; Feature157 broad solution validation was interrupted after several minutes without output; Feature157 unsupported-host quickstart passed in 10s with zero accepted partial-redraw artifacts; Feature158 unsupported-host quickstart passed with zero accepted proof artifacts and zero accepted performance artifacts; focused Feature160 filters passed for Rendering.Harness, Testing, and Package; Feature160 broad solution validation passed on final retry after one host-windowing abort was isolated to SkiaViewer and passed standalone; root `fake.sh` remains absent, so Fake wrapper package targets are recorded as a tooling limitation. |
...
**Remaining P7 caveats after Feature 158.** Correctness acceptance is scoped to the current stable
...
Feature156 records representative same-profile timing with the predeclared
`same-profile-live-threshold-v2` policy, but all five measured scenarios are `noisy`, so there is no
claimed compositor speedup yet. Feature157 records the no-clear damage-scissored readiness slice.
Feature158 removes forced proof/readback from the accepted timing path and keeps explicit readback
probe artifacts excluded from performance acceptance, but a future performance closeout still needs
net-positive reuse/promotion counters and a host lane ledger. Restoring a root `fake.sh` wrapper would remove the separate package-target tooling
...
- [x] **Feature 156 — same-profile timing evidence.** Adds `compositor-performance --feature 156`
   and `compositor-readiness --feature 156` for full-redraw versus damage-scoped timing on accepted
   profile `probe-08a47c01`. The readiness package records five representative scenarios, warmup
   count `3`, measured repetitions `5`, p50/p95/p99, noise band, confidence decision, raw samples,
   unsupported-host evidence, FSI transcripts, and package/surface validation. The live result is
   `noisy`, so it is recorded as evidence but not as an accepted performance claim.
- [x] **Feature 157 — no-clear damage-scissored render path.** Implements the proof-gated
   no-clear damage-scissored decision/readiness slice for accepted profile `probe-08a47c01`.
   Readiness records five required scenarios, fail-closed fallback evidence, unsupported-host
   validation in 10s with zero accepted partial-redraw artifacts, FSI/package evidence, and
   `performance-not-accepted`. Broad solution validation was interrupted after several minutes
   without output, so no broad-suite pass is claimed for this run.
- [x] **Feature 158 — separate proof readback from timing.** Adds `compositor-performance --feature 158`,
   `compositor-performance --feature 158 --probe-readback`, and `compositor-readiness --feature 158`.
   The accepted timing run uses policy `readback-free-timing-v1`, stable host profile
   `probe-08a47c01`, five required scenarios, warmup `3`, measured repetitions `5`, and records
   `50` accepted timing samples with proof/readback outside the measured interval. The explicit
   probe path still writes readback evidence, labels it `probe-readback-included`, and excludes it
   as `probe-run-excluded`. Unsupported hosts remain `environment-limited`; no Testing or
   SkiaViewer public helper surface was added. Focused Feature158 filters and broad
   `dotnet test FS.GG.Rendering.slnx --no-restore` pass. The shipped performance claim remains
   `performance-not-accepted`.
- [x] **Feature 159 — layer promotion and content/transform key split.** Promotes stable expensive
   subtrees to retained layers and splits cache identity into content fingerprint plus
   placement/transform keys. Moving or scrolling content reuses recorded content when only
   placement changes; churning, ambiguous, unsupported, or parity-failing boundaries fail closed
   with zero accepted artifacts. Readiness is recorded under `specs/159-layer-promotion-keys/readiness/`.
- [x] **Feature 160 — performance validation throughput.** Splits long-running broad suites from
   targeted performance filters. Full solution validation remains the release gate, while
   `compositor-performance --feature 160 --lane focused` records bounded same-profile timing
   iterations and `compositor-readiness --feature 160` publishes the reviewer package under
   `specs/160-performance-validation-throughput/readiness/`. Focused throughput is accepted on
   profile `probe-08a47c01` with three iterations under the declared 10 minute bound, required
   Feature 158 scenario coverage preserved, full validation passed on final retry, and the shipped
   performance claim remains `performance-not-accepted` until all timing, reuse, throughput, and
   lane gates are accepted for the named lane.
- [x] **Feature 161 — host performance lane ledger.** Records host-specific GPU/display facts for
   each timing run: display server, renderer, direct rendering, refresh rate, driver string,
   package versions, CPU/GPU load notes, and known environment limits. The accepted lane is X11
   `:1` with direct OpenGL on AMD Radeon/Mesa for profile `probe-08a47c01`; readiness records the
   accepted ledger, non-generalized lanes, package/regression evidence, and full validation under
   `specs/161-host-performance-lane-ledger/readiness/`. Results from that lane are not generalized
   to Wayland, indirect GL, missing display, software raster, virtualized, unknown-renderer, stale
   package, or cross-profile timing lanes.

**P7 performance acceptance rule.** Correctness remains accepted from Feature 155 and the
Feature157 damage-scissored readiness slice is accepted for profile `probe-08a47c01`; Feature158
proves the accepted measurement path is not proof/readback dominated; Feature 160 keeps timing
validation throughput bounded. A performance claim is still accepted only when the same-profile
timing lane is not noisy, Feature 159 reuse/promotion counters are net-positive, Feature 160
throughput is accepted, and Feature 161 scopes the result to the known host lane. The current
Feature 161 package scopes the lane but does not by itself accept the shipped performance claim, so
the report must still say `performance-not-accepted` while keeping the safe full-redraw fallback.
...
| **P7 — Compositor** | R6 (present-path proof, promotion, scissor, key split, texture) | Needs R2 modifiers; pure perf, gated by probes | Implemented through the Feature149 evidence/readiness package plus Feature152, Feature153, Feature154, Feature155 live-proof closeout, Feature156 same-profile timing, Feature157 no-clear damage-scissored readiness, Feature158 measurement separation, and Feature159 layer promotion/content-placement key split: Feature 147 landed as squash merge `85b0b7e`, package bump `2247cb3` to `0.1.10-preview.1`; Feature 148 landed as squash merge `7d708c4`; Feature 149 landed as squash merge `a9a1ef1`; Feature 152 landed as squash merge `8ea61c4`, followed by package bump `61d1ce8` to source packages `0.1.15-preview.1` and template package `0.1.9-preview.1`; Feature 153 landed as squash merge `d7c539c`, followed by source package bump to `0.1.16-preview.1` and template package bump to `0.1.10-preview.1`; Feature 154 landed as squash merge `2c9af24`, with exact proof acceptance, same-profile parity, timing decision, final readiness, compatibility, package, regression, and transcript coverage, followed by source package bump to `0.1.17-preview.1`; Feature 155 accepts current-host P7 correctness with three fresh sentinel/damage proof attempts and same-profile parity on `probe-08a47c01`; Feature 156 landed as squash merge `3dc6530`, records same-profile timing as `noisy` with `performance-not-accepted`, and is followed by source package bump `276b226` to `0.1.19-preview.1`; Feature 157 landed as squash merge `de6033c`, followed by package bump `b1e5888`, and records accepted no-clear damage-scissored readiness for five same-profile scenarios, fail-closed fallback evidence, unsupported-host validation in 10s, and no broad-suite pass claim; Feature158 landed as squash merge `77e8a3c`, followed by package bump `ae6574f`, and records 50 accepted readback-free timing samples, one excluded proof/readback probe artifact, no public helper surface drift, focused validation passing, broad solution validation passing, and `performance-not-accepted`. Feature159 records accepted net-positive promotion/reuse counters under `specs/159-layer-promotion-keys/readiness/`, is squash-merged as `e576257`, bumps source packages to `0.1.22-preview.1` and the template package to `0.1.14-preview.1`, adds the package-visible `Feature159Readiness` helper surface, passes focused and broad validation, and keeps `performance-not-accepted`. Unsupported hosts still fail closed. |
