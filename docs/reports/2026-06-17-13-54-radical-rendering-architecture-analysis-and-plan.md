...
| **Repo state** | Originally analyzed at branch `main` @ `8f75594`; implementation/package status updated through Feature 144 host/widget overlay integration on branch `144-overlay-host-widget-integration` |
...
| **Status** | Current through Feature 144: P0, P1, P2, P3, and P4 are implemented; P5 now includes the Feature 143 pure overlay coordinator plus Feature 144 transient widget metadata, host/runtime dispatch seams, AntShowcase reference date-picker flow, deterministic overlay corpus evidence, and unsupported-host visual-proof disclosure. |
...
### Current status update (2026-06-17 21:34 CEST)

This report began as a plan from `main` @ `8f75594`. The repository has since shipped P0-P3, pushed package version `0.1.3-preview.1`, implemented Feature 142 / P4 on `142-harfbuzz-text-shaping`, implemented the Feature 143 / P5 pure overlay coordinator on `143-interaction-overlay-state`, and implemented Feature 144 / P5 host/widget overlay integration on `144-overlay-host-widget-integration`:
...
| **P5 - Interaction** | Advanced through Feature 144. Feature 143 introduced the pure overlay coordinator; Feature 144 adds transient metadata for eight widget categories, pointer/focus/runtime/Elmish routing seams, product-visible open/focus/selection dispatch, AntShowcase product-owned date-picker evidence, deterministic overlay corpus parity, and explicit unsupported-host visual-proof disclosure. | `specs/143-interaction-overlay-state/readiness/feature143-readiness.md`; `specs/144-overlay-host-widget-integration/readiness/README.md`; Feature 144 restore/build passed; focused Controls/Elmish/KeyboardInput/Rendering.Harness/AntShowcase filters passed; surface baseline refreshed with additive `FS.GG.UI.Controls` entries. |
...
**Feature 143/144 status (2026-06-17).** P5 has advanced across two slices. Feature 143 added the pure overlay coordinator in `src/Controls/OverlayState.fsi` / `.fs`: finite supported surface kinds, dismissal policy, focus scope, modal trap mode, topmost hit decisions, replay logs, diagnostics, and ordered effects. Feature 144 adds product-owned transient widget metadata, metadata collection/validation/translation, pointer/focus overlay routing helpers, runtime dispatch records, Controls.Elmish overlay effect interpretation, eight-category widget lowering, AntShowcase reference date-picker evidence, and deterministic Rendering.Harness overlay corpus evidence. Focused Feature143/144 tests pass across Controls, Elmish, KeyboardInput, Rendering.Harness, and AntShowcase. Real offscreen GL visual proof remains environment-gated and is recorded as an unsupported-host limitation when no display/GL path is available.
...
| **P5 — Interaction** | R4 (overlay state) | Needs R2 portals + R3 anchoring | Advanced through Feature 144: pure overlay coordinator, transient widget metadata, host/runtime dispatch seams, AntShowcase reference date-picker flow, deterministic corpus parity, and unsupported-host visual disclosure landed |
...
*End of report. This document chooses the radical options and tracks their delivery status. P0-P4 are implemented, and P5 has advanced through Feature 144 host/widget overlay integration; recommended next action is running real offscreen visual proof on a display/GL-capable host and then continuing the later render-anywhere/compositor workstreams.*
