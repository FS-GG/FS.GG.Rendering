...
| **Repo state** | Originally analyzed at branch `main` @ `8f75594`; implementation/package status audited through Feature 159 squash merge `e576257` and the post-merge package bump to source packages `0.1.22-preview.1` plus template package `0.1.14-preview.1` |
...
| [ ] | Feature 160 | P7 | Performance validation throughput | Open P7 performance follow-up. |
...
- [ ] **Feature 160 — performance validation throughput.** Split long-running broad suites from
...
proves the accepted measurement path is not proof/readback dominated. A performance claim is still
accepted only when the same-profile timing lane is not noisy, Feature 160 keeps timing validation
throughput bounded, and Feature 161 scopes the result to a known lane. If any part is missing,
