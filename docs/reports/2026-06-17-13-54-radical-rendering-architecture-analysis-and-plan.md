...
| **Status** | Current as of local validation on 2026-06-18 07:09 CEST: P0-P6 are implemented and landed; Feature 147 landed the first P7 deterministic compositor-readiness slice on `main` and package version `0.1.10-preview.1`; Feature 148 has a focused evidence/readiness slice squash-merged to `main` as `7d708c4`, with 61/76 tasks complete, focused tests, solution build, surface refresh, and pack validation passing. Feature 149 was squash-merged to `main` as `a9a1ef1` with all 68 tasks checked, first-class `--feature 149` harness/readiness routing, focused Feature149 filters across Harness/Package/Testing/Controls/Elmish/SkiaViewer passing, generated readiness artifacts, and `dotnet build FS.GG.Rendering.slnx --no-restore` passing. Source packages are bumped to `0.1.12-preview.1`; the template package is bumped to `0.1.6-preview.1`. Live proof remains environment-limited and does not enable partial redraw. P8 remains unimplemented. |
