...
| **Repo state** | Originally analyzed at branch `main` @ `8f75594`; implementation/package status audited through Feature 157 squash merge `de6033c` and the post-merge package bump `b1e5888`; source packages are `0.1.20-preview.1` and the template package is `0.1.12-preview.1` |
...
| [x] | Feature 158 | P7 | Separate proof readback from timing | Local readiness accepted; `specs/158-separate-proof-timing/readiness/`; 50 accepted readback-free timing samples; proof/readback probe excluded as `probe-run-excluded`; performance claim remains `performance-not-accepted`. |
