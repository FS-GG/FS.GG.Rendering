...
| **Repo state** | Originally analyzed at branch `main` @ `8f75594`; implementation/package status audited through Feature 156 squash merge `3dc6530` and the post-merge source package bump `276b226` to `0.1.19-preview.1` |
...
| [ ] | Feature 157 | P7 | No-clear damage-scissored render path | Next open P7 performance implementation after Feature 156. |
...
**Remaining P7 caveats after Feature 156.** Correctness acceptance is scoped to the current stable
...
claimed compositor speedup yet. A future performance closeout still needs the real no-clear
damage-scissored path, readback-free timing separation, net-positive reuse/promotion counters, and a
host lane ledger. Restoring a root `fake.sh` wrapper would remove the separate package-target tooling
...
- [ ] **Feature 157 — no-clear damage-scissored render path.** Implement the real retained-surface
