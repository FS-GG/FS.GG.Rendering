...
| **Repo state** | Originally analyzed at branch `main` @ `8f75594`; implementation/package status audited through Feature 158 squash merge `77e8a3c` and the post-merge package bump `ae6574f`; source packages are `0.1.21-preview.1` and the template package is `0.1.13-preview.1` |
...
| [x] | Feature 159 | P7 | Layer promotion and content/transform key split | `specs/159-layer-promotion-keys/readiness/`; accepted net-positive promotion/reuse counters; unsupported hosts fail closed; performance claim remains `performance-not-accepted`. |
