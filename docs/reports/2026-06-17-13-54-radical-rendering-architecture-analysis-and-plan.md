...
| **Status** | Current as of local validation on 2026-06-18 06:08 CEST: P0-P6 are implemented and landed; Feature 147 landed the first P7 deterministic compositor-readiness slice on `main` and package version `0.1.10-preview.1`; Feature 148 has a focused evidence/readiness slice on branch `148-compositor-live-integration` with 59/76 tasks complete, focused tests and solution build passing, and live proof still environment-limited. P7 remains partial; P8 remains unimplemented. |
...
### Current status update (2026-06-18 06:08 CEST)
...
| **P7 - Compositor** | Partially implemented. Feature 147 is landed and provides proof/scissor/policy contracts plus deterministic readiness artifacts. Feature 148 adds live-proof/parity/reuse/snapshot/timing/readiness evidence routes, exact corpus inventory, FSI transcript coverage, focused Controls/SkiaViewer/Elmish/Harness/Package tests, generated readiness artifacts, and documentation updates. Live framebuffer sentinel/damage readback, no-clear damage-scoped renderer integration, snapshot composition, Perf/Evidence module timing implementation, and pack validation remain open. | `specs/147-compositor-damage-redraw/readiness/validation-summary.md`; `specs/148-compositor-live-integration/tasks.md` (59/76 complete); `specs/148-compositor-live-integration/readiness/validation-summary.md`; focused Feature148 tests passed for Rendering.Harness.Tests, Package.Tests, Controls.Tests, SkiaViewer.Tests, and Elmish.Tests; `dotnet build FS.GG.Rendering.slnx --no-restore` passed. |
...
Feature 148 current branch status: `148-compositor-live-integration` has the evidence/readiness portion of P7 implemented and validated locally. It adds Feature148 harness constants and commands (`compositor-live-proof`, `compositor-parity --feature 148`, `compositor-reuse`, `compositor-snapshots`, `compositor-timing`, `compositor-readiness --feature 148`), focused Feature148 tests across Harness/Package/Controls/SkiaViewer/Elmish, readiness artifacts under `specs/148-compositor-live-integration/readiness/`, and updated Controls/SkiaViewer docs. Validation passed for all focused Feature148 filters and `dotnet build FS.GG.Rendering.slnx --no-restore`. The live proof artifact remains `environment-limited`; it does not claim partial-redraw acceptance.
...
**Feature 148 status (2026-06-18).** The branch `148-compositor-live-integration` implements the
next evidence/readiness layer for P7 without claiming the native live-renderer work as complete.
...
artifacts; and Controls/SkiaViewer documentation updates. Current status is 59/76 tasks complete.
Validation passed for all focused Feature148 test filters and `dotnet build FS.GG.Rendering.slnx
--no-restore`. The open tasks are deliberately not marked complete: live `CompositorProof`/OpenGL
...
formatters, surface-baseline refresh/verification, and pack validation. The generated live proof is
`environment-limited`, so damage scissoring remains fallback-gated.
...
| **P7 — Compositor** | R6 (present-path proof, promotion, scissor, key split, texture) | Needs R2 modifiers; pure perf, gated by probes | Partially implemented. Feature 147 is landed: squash merge `85b0b7e`, package bump `2247cb3` to `0.1.10-preview.1`. Feature 148 branch adds the evidence/readiness layer with 59/76 tasks complete and focused tests/build passing. Live sentinel readback, no-clear damage-scoped renderer integration, snapshot composition, real timing probes, pack validation, and final surface-baseline verification remain open |
...
*End of report. This document chooses the radical options and tracks their delivery status. P0-P6 are implemented and landed through Feature 146 plus package version `0.1.9-preview.1`; P7 has Feature 147's deterministic readiness slice landed on `main` plus package version `0.1.10-preview.1`, and Feature 148 now adds a focused evidence/readiness layer on branch `148-compositor-live-integration` with 59/76 tasks complete. P7 remains performance-limited until live proof, no-clear renderer integration, snapshot composition, real timing probes, surface/pack validation, and the remaining package-surface work are complete; P8 intrinsic-layout work remains unimplemented.*
