...
module CompositorProof =
    ...
    let readiness active now maxAge proof =
        match proof with
    ...
    let private proofFromObservations now profile observations diagnostics =
    ...
    let private failedProof now profile cause diagnostics =
        { ProofId = proofId profile now
          HostProfile = profile
          ScenarioId = scenarioId
    ...
    let update now outputPath msg model =
        match msg with
