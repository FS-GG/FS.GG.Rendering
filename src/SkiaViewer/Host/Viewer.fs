...
module Viewer =
    ...
    let withEventMapping mapper program = { program with EventMapper = mapper }

    let withEffectMapping mapper program = { program with EffectMapper = mapper }
    ...
    let validate configuration =
        ...
        elif configuration.TargetFrameRate |> Option.exists (fun frameRate -> frameRate <= 0) then
