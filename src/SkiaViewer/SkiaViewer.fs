...
module Viewer =
    ...
    let defaultResponsivenessBudget = ViewerRuntime.defaultResponsivenessBudget
    let defaultResponsivenessOptions = ViewerRuntime.defaultResponsivenessOptions
    let responsivenessInputKindToken kind = ViewerRuntime.responsivenessInputKindToken kind
    let responsivenessVisibleResponseToken response = ViewerRuntime.responsivenessVisibleResponseToken response
    let responsivenessEnvironmentStatusToken status = ViewerRuntime.responsivenessEnvironmentStatusToken status
    let responsivenessReadinessToken readiness = ViewerRuntime.responsivenessReadinessToken readiness
    ...
    let createResponsivenessRunId a0 = ViewerRuntime.createResponsivenessRunId a0
    let latencyRecordToJsonLine latency = ViewerRuntime.latencyRecordToJsonLine latency
    let summarizeResponsivenessRecords runId scope recordsPath startedUtc completedUtc budget records = ViewerRuntime.summarizeResponsivenessRecords runId scope recordsPath startedUtc completedUtc budget records
    let responsivenessSummaryToJson summary = ViewerRuntime.responsivenessSummaryToJson summary
    let responsivenessSummaryToMarkdown summary = ViewerRuntime.responsivenessSummaryToMarkdown summary
    let writeResponsivenessRun outputRoot summary records = ViewerRuntime.writeResponsivenessRun outputRoot summary records
    let defaultWindowBehavior = ViewerRuntime.defaultWindowBehavior
    let validateWindowBehavior request = ViewerRuntime.validateWindowBehavior request
    let validateWindowLaunchBehavior initialSize request = ViewerRuntime.validateWindowLaunchBehavior initialSize request
    let classifyWindowState diagnostic = ViewerRuntime.classifyWindowState diagnostic
    ...
    let productDefectDiagnostic phase message = ViewerRuntime.productDefectDiagnostic phase message
    let tryProductStep report phase step = ViewerRuntime.tryProductStep report phase step
    let failureFromDiagnostic diagnostic = ViewerRuntime.failureFromDiagnostic diagnostic
    let classifyWindowObservation outcome inputs = ViewerRuntime.classifyWindowObservation outcome inputs
