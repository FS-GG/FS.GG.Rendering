...
module internal HostCapability =
    ...
    let desktopSessionDiagnostic () =
        ...
        let runtimeDirectoryExists = runtimeDirectory |> Option.exists IO.Directory.Exists
        ...
        let displaySocket =
            ...
            | Some runtimeDir, Some wayland, _ -> Some(IO.Path.Combine(runtimeDir, wayland))
            | _, _, Some display ->
                let number = display.TrimStart(':').Split('.').[0]
                Some($"/tmp/.X11-unix/X{number}")
            | _ -> None

        let displaySocketExists = displaySocket |> Option.exists IO.File.Exists
        ...
        let blockedReason =
            if not (OperatingSystem.IsLinux()) then
                ...
                Some
                    "XDG_RUNTIME_DIR does not exist; interactive Linux launch is blocked before app lifecycle debugging."
            elif displayVariable.IsNone then
                Some
                    "DISPLAY or WAYLAND_DISPLAY is missing; interactive Linux launch is blocked before app lifecycle debugging."
