...
module CommandInput =
    ...
    let private keyId = function
        | InputKeyIdentity.LogicalKey value -> "logical:" + value
        | InputKeyIdentity.PhysicalCode value -> "physical:" + value

    let private stepId (identity, modifiers) = keyId identity + ":" + string (modifierBits modifiers)
    ...
    let private gestureDiagnostics location gesture =
        ...
        | InputGesture.KeyChord(identity, _) ->
            ...
            if isBlank value then errors.Add(diagnostic "empty-key-identity" location "Key identity must be non-empty.")
            if value.Contains "+" then errors.Add(diagnostic "collapsed-modifiers" location "Modifiers must use typed fields instead of a collapsed key string.")
        | InputGesture.KeySequence values ->
            if List.isEmpty values || values.Length > 4 then
                errors.Add(diagnostic "invalid-sequence-length" location "Key sequences must contain one to four steps.")
            values
            |> List.iteri (fun index (identity, _) ->
                let stepLocation = location + "/steps/" + string index
                let value = keyValue identity
                if isBlank value then errors.Add(diagnostic "empty-key-identity" stepLocation "Key identity must be non-empty.")
                if value.Contains "+" then errors.Add(diagnostic "collapsed-modifiers" stepLocation "Modifiers must use typed fields instead of a collapsed key string."))
        | InputGesture.Pointer value
        | InputGesture.Touch value
        | InputGesture.Gamepad value ->
            if isBlank value then errors.Add(diagnostic "empty-device-control" location "Device control identity must be non-empty.")
        List.ofSeq errors

    let private hasDiscoverableAlternative command =
        command.Alternatives
        |> List.exists (function CommandAlternative.Palette | CommandAlternative.Menu -> true | _ -> false)
    ...
    let private isPrefix left right =
        ...
        | Some prefix, Some sequence when prefix.Length < sequence.Length ->
            (prefix, sequence |> List.take prefix.Length)
            ||> List.forall2 (=)
        | _ -> false

    let compile (catalog: InputCatalog) (profile: InputProfile) =
        let contextsById = catalog.Contexts |> List.map (fun value -> value.Id, value) |> Map.ofList
        let commandsById = catalog.Commands |> List.map (fun value -> value.Id, value) |> Map.ofList
        let errors = ResizeArray<InputProfileDiagnostic>()
        ...
        if isBlank profile.Id then errors.Add(diagnostic "empty-profile-id" "/id" "Profile identity must be non-empty.")
        if catalog.Contexts.Length > 64 then errors.Add(diagnostic "context-budget-exceeded" "/catalog/contexts" "A catalog may contain at most 64 contexts.")
        if catalog.Commands.Length > 512 then errors.Add(diagnostic "command-budget-exceeded" "/catalog/commands" "A catalog may contain at most 512 commands.")
        if profile.Defaults.Length > 2048 then errors.Add(diagnostic "binding-budget-exceeded" "/defaults" "A profile may contain at most 2,048 default bindings.")
        if profile.Overrides.Length > 2048 then errors.Add(diagnostic "override-budget-exceeded" "/overrides" "A profile may contain at most 2,048 overrides.")
        ...
        |> List.iteri (fun index context ->
            ...
            if isBlank context.Id then errors.Add(diagnostic "empty-context-id" (location + "/id") "Context identity must be non-empty.")
            context.Overlaps
            |> List.iter (fun related ->
                if not (contextsById.ContainsKey related) then
                    errors.Add(diagnostic "unknown-overlap-context" (location + "/overlaps") (sprintf "Unknown context '%s'." related))))

        catalog.Commands
        |> List.iteri (fun index command ->
            let location = "/catalog/commands/" + string index
            if isBlank command.Id then errors.Add(diagnostic "empty-command-id" (location + "/id") "Command identity must be non-empty.")
            if isBlank command.Label then errors.Add(diagnostic "empty-command-label" (location + "/label") "Command label must be non-empty.")
            if List.isEmpty command.Contexts then errors.Add(diagnostic "missing-command-context" (location + "/contexts") "Command must name at least one context.")
            command.Contexts
            |> List.iter (fun context ->
                if not (contextsById.ContainsKey context) then
                    errors.Add(diagnostic "unknown-command-context" (location + "/contexts") (sprintf "Unknown context '%s'." context)))
            match command.Argument with
            | CommandArgumentPolicy.RequiresArgument schema when isBlank schema ->
                errors.Add(diagnostic "empty-argument-schema" (location + "/argument") "Required argument schema must be non-empty.")
            | _ -> ())
        ...
        let validateBinding location binding =
            ...
            | None -> errors.Add(diagnostic "unknown-binding-command" (location + "/command") (sprintf "Unknown command '%s'." binding.Command))
            | Some command when not (command.Contexts |> List.contains binding.Context) ->
                errors.Add(diagnostic "command-context-mismatch" (location + "/context") (sprintf "Command '%s' is unavailable in context '%s'." binding.Command binding.Context))
            | _ -> ()
            if not (contextsById.ContainsKey binding.Context) then
                errors.Add(diagnostic "unknown-binding-context" (location + "/context") (sprintf "Unknown context '%s'." binding.Context))
            if catalog.ReservedGestures |> List.exists ((=) binding.Gesture) then
                errors.Add(diagnostic "reserved-gesture" (location + "/gesture") (sprintf "Gesture '%s' is reserved by the host." (gestureId binding.Gesture)))
            match binding.Gesture, commandsById.TryFind binding.Command with
            | InputGesture.KeySequence values, Some command when values.Length > 1 && not (hasDiscoverableAlternative command) ->
                errors.Add(diagnostic "sequence-needs-alternative" (location + "/gesture") "A multi-step sequence requires a palette or menu alternative.")
            | _ -> ()

        profile.Defaults |> List.iteri (fun index binding -> validateBinding ("/defaults/" + string index) binding)
        profile.Overrides
        |> List.iteri (fun index value ->
            ...
            | InputBindingOverride.ReplaceCommand(command, bindings) ->
                if not (commandsById.ContainsKey command) then
                    errors.Add(diagnostic "unknown-override-command" (location + "/command") (sprintf "Unknown command '%s'." command))
                if bindings |> List.exists (fun binding -> binding.Command <> command) then
                    errors.Add(diagnostic "replacement-command-mismatch" location "Every replacement binding must name the replaced command.")
                bindings |> List.iteri (fun bindingIndex binding -> validateBinding (location + "/bindings/" + string bindingIndex) binding)
        ...
        let effective =
            ...
            |> List.fold (fun bindings change ->
                ...
                | InputBindingOverride.UnbindCommand command -> bindings |> List.filter (fun value -> value.Command <> command)) profile.Defaults

        let contextsOverlap (left: InputContextDescriptor) (right: InputContextDescriptor) =
            left.Id = right.Id || List.contains right.Id left.Overlaps || List.contains left.Id right.Overlaps
        ...
        for leftIndex, left in List.indexed effective do
            for rightIndex, right in List.indexed effective do
                if leftIndex < rightIndex && left.Gesture = right.Gesture then
                    ...
                    | Some leftContext, Some rightContext when leftContext.Priority = rightContext.Priority && contextsOverlap leftContext rightContext ->
                        errors.Add(diagnostic "ambiguous-gesture" "/effective" (sprintf "Gesture '%s' ambiguously binds '%s' and '%s' at priority %d." (gestureId left.Gesture) left.Command right.Command leftContext.Priority))
                    | _ -> ()
                if not catalog.AllowTerminalPrefixes && leftIndex <> rightIndex && isPrefix left.Gesture right.Gesture then
                    match contextsById.TryFind left.Context, contextsById.TryFind right.Context with
                    | Some leftContext, Some rightContext when contextsOverlap leftContext rightContext ->
                        errors.Add(diagnostic "terminal-prefix-conflict" "/effective" (sprintf "Gesture '%s' is a terminal prefix of '%s'." (gestureId left.Gesture) (gestureId right.Gesture)))
                    | _ -> ()

        if errors.Count > 0 then Error(List.ofSeq errors)
        else Ok { Id = profile.Id; Bindings = effective }
...
module InputProfileCodec =
    ...
    let private encodeString (value: string) = Encoding.UTF8.GetBytes value |> Convert.ToBase64String
    let private decodeString (value: string) : string =
        Convert.FromBase64String value |> fun bytes -> Encoding.UTF8.GetString(bytes: byte[])
    ...
    let private encodeKey = function
    ...
    let private encodeGesture = function
    ...
    let private encodeBinding prefix binding =
        String.concat "\t" [ prefix; encodeString binding.Context; encodeString binding.Command; encodeGesture binding.Gesture ]
    ...
    let encode (profile: InputProfile) =
        ...
        profile.Defaults |> List.iter (fun binding -> lines.Add(encodeBinding "default" binding))
        profile.Overrides
        |> List.iter (function
            | InputBindingOverride.ReplaceCommand(command, bindings) ->
                lines.Add("replace\t" + encodeString command + "\t" + string bindings.Length)
                bindings |> List.iter (fun binding -> lines.Add(encodeBinding "binding" binding))
    ...
    let private parseStep (value: string) =
        ...
        | [| kind; encoded; rawBits |] ->
            ...
            if bits < 0 || bits > 31 then raise (FormatException "modifier bits must be between 0 and 31")
    ...
    let decode (bytes: byte[]) =
        try
            if bytes.Length > 1048576 then raise (FormatException "profile envelope exceeds 1 MiB")
            let rows =
            ...
            if rows.Length = 0 || rows[0] <> formatId + "\t" + string formatVersion then
                Error [ diagnostic "unsupported-envelope" "/header" "Unsupported input profile envelope." ]
            else
                ...
                while index < rows.Length && not ended do
                    ...
                    match fields with
                    | [ "schema"; encoded ] -> schema <- Some(decodeString encoded); index <- index + 1
                    | [ "id"; encoded ] -> profileId <- Some(decodeString encoded); index <- index + 1
                    | "default" :: bindingFields -> defaults.Add(parseBinding bindingFields); index <- index + 1
                    | "alias" :: bindingFields -> overrides.Add(InputBindingOverride.AddAlias(parseBinding bindingFields)); index <- index + 1
                    | [ "unbind"; encoded ] -> overrides.Add(InputBindingOverride.UnbindCommand(decodeString encoded)); index <- index + 1
                    | [ "replace"; encoded; rawCount ] ->
                        let count = Int32.Parse rawCount
                        if count < 0 || index + count >= rows.Length then raise (FormatException "replacement count exceeds envelope")
                        let bindings =
                ...
                | None, _, _ -> Error [ diagnostic "missing-profile-schema" "/schema" "Profile schema row is required." ]
        ...
        with error -> Error [ diagnostic "malformed-envelope" "/" error.Message ]
