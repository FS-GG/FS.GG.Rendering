...
module ControlInspection =
    ...
    let private controlId path (control: Control<'msg>) = control.Key |> Option.defaultValue path
                    ...
                    Diagnostics =
                        [
                            if fitStatus = VisualInspectionFitStatus.Overflow then
                                "text bounds exceed owner bounds"
                        ]
                }
                : VisualTextInspection
            )
            ...
            Some(
                {
                    ...
                    Required = true
                    Diagnostics = [ "owner bounds unavailable" ]
                }
                : VisualTextInspection
            )
        | _ -> None
    ...
    let private inspectCore
        ...
        let rec walk (path: string) (parentId: string option) (z: int) (current: Control<'msg>) =
            ...
            let coverage: VisualPaintCoverage =
                {
                    ...
                    Reason = if bounds.IsSome then None else Some "bounds unavailable"
                }
            ...
            let clip: VisualClipFact =
                {
                    ...
                    Reason =
                        ...
                        | VisualInspectionClipStatus.Intentional ->
                            Some "container or scroll clipping is part of Control.renderTree composition"
                        | _ -> None
            ...
            let nodes =
                node :: (childResults |> List.collect (fun (nodes, _, _, _, _) -> nodes))

            let regions =
                region :: (childResults |> List.collect (fun (_, regions, _, _, _) -> regions))
            ...
            let paintCoverage =
                coverage
                :: (childResults |> List.collect (fun (_, _, _, paintCoverage, _) -> paintCoverage))

            let clipFacts =
                clip
                :: (childResults |> List.collect (fun (_, _, _, _, clipFacts) -> clipFacts))
    ...
    let private flattenControl (render: ControlRenderResult<'msg>) (control: Control<'msg>) =
        ...
        walk "0" None control |> List.map (fun row -> row.NodeId, row) |> Map.ofList
    ...
    let private retainedNodeFacts prior current identities expectedAffectedRegionIds =
        ...
        |> List.map (fun id ->
...

            {
                ...
                Kind =
                    owner
                    |> Option.map (fun row -> row.Control.Kind)
    ...
    let private artifactFromRetained
        ...
        let nodes =
            retainedNodeFacts priorMap currentMap identities transition.ExpectedAffectedRegionIds
        ...
        let repaintedCount =
            workReduction
            |> Option.map _.RepaintedNodeCount
            |> Option.defaultValue (nodes |> List.filter _.Repainted |> List.length)

        let shiftedCount =
            workReduction
            |> Option.map _.ShiftedNodeCount
            |> Option.defaultValue (nodes |> List.filter _.Shifted |> List.length)

        let unaffectedCount =
            nodes
            |> List.filter (fun node ->
                node.Status = RetainedNodeStatus.Reused
                || node.Status = RetainedNodeStatus.Unaffected)
            |> List.length
        ...
        let frameTransition: RetainedFrameTransition =
            {
                TransitionId = transition.TransitionId
                PriorFrameId =
                    transition.PriorControl
                    |> Option.map (fun _ -> transition.TransitionId + ":prior")
        ...
        let artifact: RetainedInspectionArtifact =
            {
                ArtifactId =
                    $"{request.Scope.ScopeId}:{transition.TransitionId}:{request.OutputSize.Width}x{request.OutputSize.Height}:{request.Presentation}"
    ...
    let inspectRetained (request: RetainedControlInspectionRequest<'msg>) =
        ...
        | None ->
            let init =
                RetainedRender.init request.Theme request.OutputSize request.Transition.CurrentControl
        ...
        | Some prior ->
            ...
            let step =
                RetainedRender.step request.Theme request.OutputSize init.Retained request.Transition.CurrentControl
