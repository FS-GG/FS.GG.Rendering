...
type CollectionEffect = VisibleRangeChanged of VisibleRange
...
module Collections =
    ...
    let withRange model scrollOffset itemCount =
        let range =
            visibleRange model.RowHeight model.ViewportHeight scrollOffset itemCount model.Overscan
    ...
    let update msg model =
        ...
        | SelectKey key ->
            { model with
                SelectedKeys = Set.singleton key
            },
            []
        | ToggleKey key ->
            let selected =
