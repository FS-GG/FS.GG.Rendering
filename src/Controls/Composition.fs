...
module internal Composition =
    ...
    let private table name =
        classificationTable |> List.find (fun (n, _) -> n = name) |> snd
    ...
    let private combine left right =
        ...
        | Offset(ax, ay), Offset(bx, by) ->
            Some
                { right with
                    Effect = Offset(ax + bx, ay + by)
                }
        | Opacity a, Opacity b -> Some { right with Effect = Opacity(a * b) }
        | LocalZOrder _, LocalZOrder _ -> Some right
        | LayerHint _, LayerHint _ -> Some right
        | CacheBoundary _, CacheBoundary _ -> Some right
        | _ -> None
    ...
    let retainedReuseEvidence (chain: ModifierChain) : RetainedReuseEvidence =
        let invalidations =
            chain.NormalizedEffects |> List.map (fun entry -> classify entry.Effect)
            ...
            | TextRun run ->
                TextRun
                    { run with
                        Paint =
                            { run.Paint with
                                Opacity = run.Paint.Opacity * opacity
                            }
                    }
            | ClipNode(clip, scene) ->
                ClipNode(
                    clip,
                    { scene with
                        Nodes = mapSceneNodes opacity scene.Nodes
                    }
                )
            | Group scenes ->
                Group(
                    ...
                    |> List.map (fun s ->
                        { s with
                            Nodes = mapSceneNodes opacity s.Nodes
                        })
                )
            ...
            | PictureNode picture ->
                PictureNode
                    { picture with
                        Scene =
                            { picture.Scene with
                                Nodes = mapSceneNodes opacity picture.Scene.Nodes
                            }
                    }
            | Translate(offset, scene) ->
                Translate(
                    offset,
                    { scene with
                        Nodes = mapSceneNodes opacity scene.Nodes
            ...
            | CachedSubtree boundary ->
                CachedSubtree
                    { boundary with
                        Scene =
                            { boundary.Scene with
                                Nodes = mapSceneNodes opacity boundary.Scene.Nodes
                            }
                    }
            | Image _
            | RegionNode _
    ...
    let hitOrder contributions = contributions |> paintOrder |> List.rev
    ...
    let composeLayers (hosts: LayerHost list) (inFlow: OrderedContribution list) (portals: Portal list) =
                    ...
                    if Option.isNone p.AnchorId || Option.isNone p.AnchorBounds then
                        {
                            ...
                            Message =
                                sprintf "Portal targeting '%s' lacks anchor identity or bounds evidence." p.TargetLayer
                            TargetLayer = Some p.TargetLayer
                            AnchorId = p.AnchorId
                        }
                ])
        ...
        let portalContributions =
            portals
            |> List.filter (fun (p: Portal) ->
                Set.contains p.TargetLayer hostIds
                && Option.isSome p.AnchorId
                && Option.isSome p.AnchorBounds)
            |> List.map (fun (p: Portal) -> { p.Content with Layer = p.TargetLayer })

        let contributions =
            inFlow @ portalContributions |> List.groupBy _.Layer |> Map.ofList

        let layerPaint =
            allHosts
            |> List.collect (fun host -> contributions |> Map.tryFind host.Id |> Option.defaultValue [] |> paintOrder)

        let layerHit =
            allHosts
            |> List.rev
            |> List.collect (fun host -> contributions |> Map.tryFind host.Id |> Option.defaultValue [] |> hitOrder)
