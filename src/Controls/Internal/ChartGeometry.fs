...
module internal ChartGeometry =
        ...
        | _ ->
            ...
            |> List.mapi (fun i p ->
...

                {
                    ...
                    Y = box.Y + box.Height - fy * box.Height
                })
    ...
    let withPoints
        ...
        (caption: string)
        (pts: ChartPoint list)
        (body: ChartPoint list -> Scene list)
        : Scene list =
        match pts with
    ...
    let lineGeom theme (box: Rect) (pts: ChartPoint list) : Scene list =
        ...
        | (head :: _) as ps ->
            ...
            let areaCmds =
                Path.moveTo head.X baseY :: (ps |> List.map (fun p -> Path.lineTo p.X p.Y))
                @ [ Path.lineTo (List.last ps).X baseY; Path.close ]

            let area =
                Scene.path (Path.create Winding areaCmds) (Paint.withOpacity 0.22 (Paint.fill theme.Accent))

            let lineCmds =
                Path.moveTo head.X head.Y
                :: (List.tail ps |> List.map (fun p -> Path.lineTo p.X p.Y))

            let stroke =
                Scene.path (Path.create Winding lineCmds) (Paint.stroke theme.Accent 3.0)

            let dots = ps |> List.map (fun p -> Scene.circle p 3.5 theme.Accent)
            area :: stroke :: dots

    let barGeom theme (box: Rect) (pts: ChartPoint list) : Scene list =
        withPoints theme box "(no data)" pts
        <| fun pts ->
    ...
    let axes theme (box: Rect) : Scene list =
        [
            Scene.line
                ...
                (Paint.stroke theme.Foreground 1.5)
            Scene.line
                { X = box.X; Y = box.Y + box.Height }
                {
                    X = box.X + box.Width
            ...
            let plot: Rect =
                {
                    X = box.X + 6.0
                    Y = box.Y + 4.0
                    Width = box.Width - 12.0
                    Height = box.Height - 12.0
                }
    ...
    let areaGeom theme (box: Rect) (pts: ChartPoint list) : Scene list =
        ...
        | (head :: _) as ps ->
            ...
            let area =
                Scene.path (Path.create Winding areaCmds) (Paint.withOpacity 0.38 (Paint.fill theme.Accent))
            ...
            let stroke =
                Scene.path (Path.create Winding lineCmds) (Paint.stroke theme.Accent 2.5)
            ...
            |> List.mapi (fun i (_, h) ->
                ...
                [
                    ...
                    Scene.rectangleWithPaint
                        ...
                        (Paint.stroke theme.Background 1.0)
                ])
            |> List.collect id
            ...
            |> List.mapi (fun i (_, h) ->
                ...
                [
                    ...
                    Scene.rectangleWithPaint
                        ...
                        (Paint.fill (chartColorAt theme i))
                    Scene.line
                        { X = cx - half; Y = median }
                        { X = cx + half; Y = median }
                        (Paint.stroke theme.Foreground 2.0)
                ])
            |> List.collect id
    ...
    let heatmapGeom theme (box: Rect) (pts: ChartPoint list) : Scene list =
        ...
        <| fun pts ->
            ...
            |> List.mapi (fun i p ->
                ...
                Scene.rectangle
                    (box.X + float c * cw, box.Y + float r * ch, cw - 2.0, ch - 2.0)
                    (lerpColor theme.Muted theme.Accent t))
    ...
    let radarGeom theme (box: Rect) (pts: ChartPoint list) : Scene list =
        ...
        | _ ->
            ...
            let angle i =
                float i / float n * 2.0 * System.Math.PI - System.Math.PI / 2.0
            ...
            let polyCmds =
                Path.moveTo head.X head.Y
                :: (List.tail verts |> List.map (fun v -> Path.lineTo v.X v.Y))
                @ [ Path.close ]

            let poly =
                Scene.path (Path.create Winding polyCmds) (Paint.withOpacity 0.35 (Paint.fill theme.Accent))
    ...
    let gaugeGeom theme (box: Rect) (value: float) : Scene list =
        ...
        let needle =
            Scene.line
                ...
                (Paint.stroke theme.Foreground 2.5)
                ...
                [
                    for (lx, ly, lh) in left do
                        for (rx, ry, rh) in right do
                            let cmds =
                                [ Path.moveTo (lx + nodeW) (ly + lh / 2.0); Path.lineTo rx (ry + rh / 2.0) ]

                            yield
                                Scene.path
                                    (Path.create Winding cmds)
                                    (Paint.withOpacity 0.25 (Paint.stroke theme.Accent 3.0))
                ]

            let leftNodes =
                left
                |> List.mapi (fun i (x, y, h) -> Scene.rectangle (x, y, nodeW, h) (chartColorAt theme i))

            let rightNodes =
                right
                |> List.mapi (fun i (x, y, h) -> Scene.rectangle (x, y, nodeW, h) (chartColorAt theme (i + leftN)))
    ...
    let chordGeom theme (box: Rect) (pts: ChartPoint list) : Scene list =
        ...
        | _ ->
            ...
            let chords =
                [
                    for i in 0 .. n - 1 do
                        ...
                        yield
                            Scene.path
                                (Path.create Winding [ Path.moveTo a.X a.Y; Path.lineTo b.X b.Y ])
                                (Paint.withOpacity 0.3 (Paint.stroke theme.Accent 2.0))
                ]
            ...
            let nodes =
                positions |> List.mapi (fun i p -> Scene.circle p 6.0 (chartColorAt theme i))
        ...
        <| fun pts ->
            ...
            [
                for i in 0 .. pts.Length - 1 do
                    ...
                    yield
                        Scene.rectangle (x, box.Y, max 1.0 (w - 2.0), box.Height) (lerpColor theme.Muted theme.Accent t)
