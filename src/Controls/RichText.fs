...
module RichText =
    ...
    let measure (block: RichTextBlock) =
        let maxFont =
            block.Runs |> List.map (fun run -> run.Style.FontSize) |> List.fold max 0.0

        let charCount = block.Runs |> List.sumBy (fun run -> run.Text.Length)

        let width =
            float charCount * max 1.0 maxFont * 0.55
            |> fun value -> block.MaxWidth |> Option.map (min value) |> Option.defaultValue value

        let effectDiagnostics =
            block.Effects
            |> List.map (fun effect -> Diagnostics.unsupportedEnvironment "rich-text" effect)
