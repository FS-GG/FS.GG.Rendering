...
module SvgWorkspaceCommands =
    let private modifiersText (modifiers: InputModifiers) =
        [ if modifiers.Ctrl then "Control"
          if modifiers.Meta then "Meta"
          if modifiers.Alt then "Alt"
          if modifiers.Shift then "Shift"
          if modifiers.AltGraph then "AltGraph" ]

    let private keyText = function
    ...
    let project (catalog: InputCatalog) (profile: EffectiveInputProfile) activeContexts isAvailable =
        ...
        |> List.map (fun command ->
            ...
            { Command = command.Id
              ...
              PointerLabels = alternatives |> List.choose (function CommandAlternative.Pointer label -> Some label | _ -> None) })

    let paletteRows rows = rows |> List.filter (fun row -> row.Available && row.InPalette)
    let helpRows rows = rows |> List.filter _.Available
    let pointerRows rows = rows |> List.filter (fun row -> row.Available && not row.PointerLabels.IsEmpty)
    ...
    let previewRebind catalog profile command context gesture =
        ...
        |> Result.bind (fun current ->
            let displaced =
                current.Bindings
                |> List.filter (fun binding -> binding.Context = context && binding.Gesture = gesture && binding.Command <> command)
                |> List.map _.Command
                |> List.distinct
                |> List.sort
            let preserveDisplaced =
                ...
                |> List.map (fun displacedCommand ->
                    let remaining = current.Bindings |> List.filter (fun binding -> binding.Command = displacedCommand && not (binding.Context = context && binding.Gesture = gesture))
                    if remaining.IsEmpty then InputBindingOverride.UnbindCommand displacedCommand
                    else InputBindingOverride.ReplaceCommand(displacedCommand, remaining))
            let replacement = { Gesture = gesture; Command = command; Context = context }
            let candidate =
                { profile with Overrides = profile.Overrides @ preserveDisplaced @ [ InputBindingOverride.ReplaceCommand(command, [ replacement ]) ] }
