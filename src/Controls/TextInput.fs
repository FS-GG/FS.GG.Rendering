...
module TextInput =
    ...
    let insertAt model value =
        ...
        let prefix =
            model.DraftText.Substring(0, Numeric.clamp 0 model.DraftText.Length startIndex)

        let suffix =
            model.DraftText.Substring(Numeric.clamp 0 model.DraftText.Length endIndex)
    ...
    let update msg model =
        ...
        | Cancel ->
            { model with
                DraftText = model.CommittedText
                CaretIndex = model.CommittedText.Length
                Selection = None
                Composition = None
            },
            []
        | CompositionStarted value -> { model with Composition = Some value }, []
        | CompositionCommitted value ->
            { insertAt model value with
                Composition = None
            },
            []
