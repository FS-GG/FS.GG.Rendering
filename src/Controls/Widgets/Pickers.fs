...
module DatePicker =
    ...
    let view (props: DatePickerProps<'msg>) : Widget<'msg> =
        ...
        let dayStops =
            ...
            | Some date ->
                [
                    for day in 1 .. DateTime.DaysInMonth(date.Year, date.Month) -> sprintf "day-%d" day
                ]
            | None -> []
        ...
        let overlay =
            FS.GG.UI.Controls.Overlay.create [ FS.GG.UI.Controls.Overlay.child calendar; Attr.selected props.IsOpen ]
...
module TimePicker =
    ...
    let view (props: TimePickerProps<'msg>) : Widget<'msg> =
        ...
        let separator = FS.GG.UI.Controls.Label.create [ FS.GG.UI.Controls.Label.text ":" ]
...
module ColorPicker =
    ...
    let view (props: ColorPickerProps<'msg>) : Widget<'msg> =
        ...
        let swatchStops =
            props.Swatches |> List.map (fun swatch -> sprintf "swatch-%s" swatch.Name)
