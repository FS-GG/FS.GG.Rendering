...
module Graph =
    ...
    let directedLayout (graph: GraphDefinition) =
        ...
        let nodeWidth =
            min 120.0 (max 42.0 (graph.Config.Bounds.Width / float (layerCount * 2)))
        ...
        |> List.collect (fun (layer, nodes) ->
            ...
            |> List.mapi (fun index node ->
                let x =
                    graph.Config.Bounds.X
                    + ((float layer + 0.5) / float layerCount) * graph.Config.Bounds.Width

                let y =
                    graph.Config.Bounds.Y
                    + ((float index + 0.5) / float count) * graph.Config.Bounds.Height
    ...
    let undirectedLayout (graph: GraphDefinition) =
        ...
        let radius =
            max 24.0 (min graph.Config.Bounds.Width graph.Config.Bounds.Height / 2.0 - 36.0)
    ...
    let nodeLookup (layout: GraphLayoutResult) =
        layout.Nodes |> List.map (fun item -> item.Node.Id, item) |> Map.ofList
        ...
        |> Result.map (fun result ->
            ...
            let edgeScenes =
                ...
                |> List.mapi (fun index edge ->
                        ...
                        let label =
                            ...
                            |> Option.orElse (
                                ...
                                |> Option.map (fun value ->
                                    value.ToString("G4", Globalization.CultureInfo.InvariantCulture))
                            )
                        ...
                        Scene.group
                            [
                                Scene.line
                                    sourceCenter
                                    targetCenter
                                    (Paint.stroke (Colors.rgba 150uy 170uy 195uy 220uy) 1.5)
                                match label with
                                | Some text ->
                    ...
                    | _ ->
                        Scene.text
                            (graph.Config.Bounds.X, graph.Config.Bounds.Y + float index * 16.0)
                            "invalid edge"
                            (Colors.rgba 220uy 64uy 52uy 255uy))
                    ...
                    Scene.group
                        [
                            Scene.rectangleWithPaint
                                (rect item.Bounds)
                                (Paint.fill (
                                    item.Node.Style |> Option.defaultValue (Colors.rgba 64uy 128uy 220uy 230uy)
                                ))
    ...
    let directed (graph: GraphDefinition) =
        render
            { graph with
                Config = { graph.Config with Kind = Directed }
            }

    let undirected (graph: GraphDefinition) =
        render
            { graph with
                Config = { graph.Config with Kind = Undirected }
            }
