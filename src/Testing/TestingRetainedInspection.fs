...
module RetainedInspectionValidation =
    ...
    let private transitionId (artifact: RetainedInspectionArtifact) =
        artifact.Transition
        |> Option.map _.TransitionId
        |> Option.defaultValue artifact.ArtifactId
    ...
    let private retainedIdentityStable
        (artifact: RetainedInspectionArtifact)
        (previous: RetainedInspectionArtifact option)
        =
        match previous with
        | None -> []
        | Some before ->
    ...
    let private dirtyRegionUnioned (artifact: RetainedInspectionArtifact) =
        ...
        | Some damage ->
            let expected =
                RetainedInspection.dirtyUnionArea damage.FrameBounds damage.DirtyRectangles

            let frameArea =
                max 0.0 (damage.FrameBounds.Width * damage.FrameBounds.Height) |> int
    ...
    let private emptyDamageExplicit (artifact: RetainedInspectionArtifact) =
        match artifact.Damage with
        | Some damage when
            damage.DirtyRectangles.IsEmpty
            && damage.VisibleDirtyArea = 0
            && damage.DamageStatus <> DamageInspectionStatus.Empty
            && damage.DamageStatus <> DamageInspectionStatus.NotInspected
            ->
            [
                finding
                    "empty-damage-explicit"
        ...
        | Some damage ->
            let expected =
                Set.ofList (
                    expectedRegions
                    @ (artifact.Transition
                       |> Option.map _.ExpectedAffectedRegionIds
                       |> Option.defaultValue [])
                )
            ...
            [
                ...
                if damage.DamageStatus = DamageInspectionStatus.Broad then
                    let dirtyPercentage =
                        damage.DirtyPercentage.ToString("0.###", System.Globalization.CultureInfo.InvariantCulture)
                        + "%"
    ...
    let private screenshotReadinessCountsPreserved (artifact: RetainedInspectionArtifact) =
        if artifact.Scope.ScopeId.Contains("antshowcase", StringComparison.OrdinalIgnoreCase) then
            ...
            if
                joined.Contains("preferred=38", StringComparison.OrdinalIgnoreCase)
                && joined.Contains("minimum=12", StringComparison.OrdinalIgnoreCase)
            then
                []
            else
                [
    ...
    let private findingsForRule
        (check: RetainedInspectionValidationCheck)
        (rule: RetainedInspectionRule)
        : DamageLocalityFinding list =
        ...
        | unknown when rule.Required ->
            [
                finding
                    unknown
                    VisualInspectionSeverity.Unsupported
                    (transitionId check.Artifact)
                    []
                    []
                    $"rule `{unknown}` is not implemented"
                    "implemented retained inspection rule"
                    "unknown rule"
            ]
        | _ -> []
    ...
    let validateCheck (check: RetainedInspectionValidationCheck) : RetainedInspectionValidationResult =
        ...
        let floorDiagnostics =
            if
                ...
                && check.Artifact.ReadinessStatus = RetainedInspectionStatus.Accepted
            then
                [
        ...
        let knobs
            : SharedTesting.InspectionValidationKnobs<
                  DamageLocalityFinding,
                  IntentionalDamageException,
                  RetainedInspectionStatus,
                ...
                AcceptFinding =
                    fun f ex ->
                        { f with
                            ...
                            Diagnostics =
                                f.Diagnostics
                                @ [ $"accepted by retained inspection exception `{ex.ExceptionId}`: {ex.Reason}" ]
                        }
                ...
                DeriveStatus =
                    fun has ->
                        if not invalidExceptions.IsEmpty || has VisualInspectionSeverity.Blocking then
                            ...
                            | RetainedInspectionStatus.Accepted when not hasInspectionEvidence ->
                                RetainedInspectionStatus.ReviewRequired
                            | other -> other
                MkResult =
    ...
    let aggregate
...

        {
            ...
            InspectedScopes =
                artifacts
                |> List.filter (fun a ->
                    a.ReadinessStatus <> RetainedInspectionStatus.NotInspected
                    && a.ReadinessStatus <> RetainedInspectionStatus.NotRun)
            ...
            RelatedVisualEvidence =
                (relatedVisualEvidence @ (artifacts |> List.collect _.RelatedVisualEvidence))
                |> List.distinct
                |> List.sort
    ...
    let renderSummary (summary: RetainedInspectionSummary) =
        ...
        if not summary.DirtyAreaSummaries.IsEmpty then
            ...
            for transitionId, percentage, regions in summary.DirtyAreaSummaries do
                let percentageText =
                    percentage.ToString("0.###", System.Globalization.CultureInfo.InvariantCulture)
