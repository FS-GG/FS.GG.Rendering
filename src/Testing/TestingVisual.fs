...
module VisualCaptureMatrix =
    ...
    let private normalRelativePath (relativePath: string) = relativePath.Replace('\\', '/').Trim()
    ...
    let expand
        ...
        let sizeDuplicates =
            sizes
            |> List.map (fun size -> $"{size.Role}:{size.Width}x{size.Height}")
            |> duplicateValues
        ...
        let orderedSizes =
            sizes
            |> List.sortBy (fun size -> size.Order, size.Role, size.Width, size.Height)
...
module VisualCompleteness =
    ...
    let private normalizeRelativePath (relativePath: string) = relativePath.Replace('\\', '/').Trim()
    ...
    let degraded (target: VisualCaptureTarget) reason =
        if String.IsNullOrWhiteSpace reason then
            record
                ...
                None
                (Some "missing degraded reason")
                [ "degraded capture requires a non-empty reason" ]
        else
    ...
    let private validateOne evidenceRoot (target: VisualCaptureTarget) =
        ...
        if not (File.Exists path) then
            ...
            record
                ...
                [ $"missing screenshot: {target.RelativePath}" ]
        else
            ...
            if byteCount = 0L then
                ...
                record
                    ...
                    (Some artifact)
                    (Some "zero-byte artifact")
                    [ $"zero-byte screenshot: {target.RelativePath}" ]
            else
                try
                    ...
                    if isNull bitmap then
                        ...
                        record
                            ...
                            VisualCaptureUndecodable
                            (Some artifact)
                            (Some "undecodable PNG")
                            [ $"undecodable screenshot: {target.RelativePath}" ]
                    else
                        let artifact =
                            {
                                RelativePath = target.RelativePath
                ...
                with ex ->
                    ...
                    record
                        ...
                        VisualCaptureUndecodable
                        (Some artifact)
                        (Some "artifact decode failed")
                        [ $"undecodable screenshot: {target.RelativePath}: {ex.Message}" ]

    let private staleDiagnostics evidenceRoot (targets: VisualCaptureTarget list) =
        if Directory.Exists evidenceRoot then
            let targetPaths =
                targets
                |> List.map (fun target -> normalizeRelativePath target.RelativePath)
                |> Set.ofList
            ...
            |> Seq.choose (fun path ->
                ...
                if targetPaths.Contains relative then
                    None
                else
                    Some $"stale artifact outside target matrix: {relative}")
            |> Seq.toList
        else
            []
...
module VisualReviewerClassifications =
    ...
    let private sizeText (size: VisualSize) =
        $"{size.Role}:{size.Width}x{size.Height}"
    ...
    let parse (markdown: string) (targets: VisualCaptureTarget list) =
        ...
        for cells in rows do
            ...
            | targetId :: _pageId :: _themeId :: _size :: severityText :: defectClass :: impact :: reviewer :: timestamp :: notesParts ->
                ...
                | Ok severity ->
                    ...
                    if
                        severity = VisualReviewerPending
                        || impact.Equals("pending review", StringComparison.OrdinalIgnoreCase)
                        || notes.Contains("pending review", StringComparison.OrdinalIgnoreCase)
                    then
        ...
        let missingIds =
            targetIds |> List.filter (fun targetId -> seen.Contains targetId |> not)
...
module VisualReadiness =
    ...
    let private countByText (textOf: 'a -> string) (values: 'a list) =
        values |> List.countBy textOf |> List.sortBy fst
    ...
    let evaluate
        ...
        let captureByTarget =
            captures
            |> List.map (fun capture -> capture.Target.TargetId, capture)
            |> Map.ofList

        let missingCaptureIds =
            requiredTargets
            |> List.choose (fun target ->
                if captureByTarget.ContainsKey target.TargetId then
                    None
                else
        ...
        let degradedIds =
            ...
            |> List.choose (fun target ->
                ...
                |> Option.bind (fun capture ->
                    if
                        capture.Status = VisualCaptureDegraded
                        && not (acceptedExceptionIds.Contains target.TargetId)
                    then
                        Some target.TargetId
                    else
                        None))
        ...
        let missingReviewIds =
            ...
            |> List.choose (fun target ->
                ...
                | Some reviews when reviews |> List.exists (fun review -> review.Severity = VisualReviewerPending) ->
                    Some target.TargetId
                | Some _ -> None)
        ...
        let blockingReviewIds =
            ...
            |> List.choose (fun review ->
                if
                    review.Severity = VisualReviewerBlocking
                    && requiredTargetIds.Contains review.TargetId
                then
        ...
        let status =
            if not missingCaptureIds.IsEmpty then
                ...
                not captureBlocks.IsEmpty
                || not duplicateReviewIds.IsEmpty
                || not blockingReviewIds.IsEmpty
            then
...

        {
            ...
            CaptureStatusCounts =
                captures
                |> countByText (fun capture -> VisualCompleteness.statusText capture.Status)
            ReviewerStatusCounts =
                reviewerClassifications
                |> countByText (fun review -> VisualReviewerClassifications.severityText review.Severity)
...
module internal SharedTesting =
    ...
    let validateCheck
        ...
        let has severity =
            findings |> List.exists (fun f -> knobs.SeverityOf f = severity)
    ...
    let renderSummary (report: VisualReadinessReport) =
        ...
        if not problemCaptures.IsEmpty then
            ...
            for capture in problemCaptures do
                let reason =
                    capture.Reason
                    |> Option.defaultValue (VisualCompleteness.statusText capture.Status)
...
module VisualInspectionValidation =
    ...
    let private requiredRegionPresent (artifact: VisualInspectionArtifact) requiredRegionIds =
        ...
        let requiredIds =
            (requiredRegionIds
             @ (artifact.Regions |> List.filter _.Required |> List.map _.RegionId))
            |> List.distinct
    ...
    let private requiredRegionPainted (artifact: VisualInspectionArtifact) =
        let coverageByTarget =
            artifact.PaintCoverage |> List.groupBy _.TargetId |> Map.ofList

        [
            for region in artifact.Regions do
                if region.Required then
                    let coverage =
                        coverageByTarget |> Map.tryFind region.RegionId |> Option.defaultValue []
        ...
        [
            for firstIndex, first in artifact.Regions |> List.indexed do
                for second in artifact.Regions |> List.skip (firstIndex + 1) do
                    ...
                    | Some a, Some b when
                        not (isOverlayRole first.Role)
                        && not (isOverlayRole second.Role)
                        && intersects a b
                        && not (contains a b)
                        && not (contains b a)
                        ->
    ...
    let private identityStable (artifact: VisualInspectionArtifact) (previous: VisualInspectionArtifact option) =
        ...
        | Some previousArtifact ->
            let previousIds =
                previousArtifact.Nodes
                |> List.filter (fun n -> not n.Dynamic)
                |> List.map _.NodeId
                |> Set.ofList
    ...
    let private visualOrderStable (artifact: VisualInspectionArtifact) (previous: VisualInspectionArtifact option) =
        ...
        | Some previousArtifact ->
            let orderOf (a: VisualInspectionArtifact) =
                a.Nodes
                |> List.filter (fun n -> not n.Dynamic)
                |> List.sortBy (fun n -> n.ZOrder, n.NodeId)
                |> List.map _.NodeId
        ...
        | unknown when rule.Required ->
            [
                finding
                    ...
                    []
                    []
                    $"rule `{unknown}` is not implemented"
    ...
    let validateCheck (check: VisualInspectionValidationCheck) : VisualInspectionValidationResult =
        ...
        let initialFindings =
            ruleFindings |> List.append check.Artifact.Findings |> List.sortBy _.FindingId
        ...
        let floorDiagnostics =
            if
                not hasInspectionEvidence
                && check.Artifact.ReadinessStatus = VisualInspectionStatus.Accepted
            then
                [
        ...
        let knobs
            : SharedTesting.InspectionValidationKnobs<
                  VisualInspectionFinding,
                  VisualInspectionException,
                ...
                AcceptFinding =
                    fun f ex ->
                        { f with
                            ...
                            Diagnostics =
                                f.Diagnostics
                                @ [ $"accepted by visual inspection exception `{ex.ExceptionId}`: {ex.Reason}" ]
                        }
...
module VisualInspectionReadiness =
    ...
    let aggregate
        ...
        (caveats: string list)
        =
        let resultByArtifact =
            results |> List.map (fun result -> result.ArtifactId, result) |> Map.ofList
...

        {
            ...
            InspectedScopes =
                artifacts
                |> List.filter (fun a ->
                    a.ReadinessStatus <> VisualInspectionStatus.NotInspected
                    && a.ReadinessStatus <> VisualInspectionStatus.NotRun)
            ...
            FindingCounts =
                findings
                |> List.map (fun finding -> VisualInspection.severityText finding.Severity)
                |> countBy
            BlockingFindings =
                findings
                |> List.filter (fun finding -> finding.Severity = VisualInspectionSeverity.Blocking)
    ...
    let renderSummary (summary: VisualInspectionSummary) =
        ...
        if not summary.BlockingFindings.IsEmpty then
            ...
            for finding in summary.BlockingFindings do
                let affected =
                    String.concat ", " (finding.AffectedRegionIds @ finding.AffectedNodeIds)
