...
module GeneratedProductAssertions =
    ...
    let validateDefaultInteractiveLaunch (source: string) =
        let defaultBranch =
            ...
            if index >= 0 then source.Substring(index) else source
        ...
        let diagnostics =
            [
                ...
                if
                    contains "control-count"
                    || contains "count controls"
                    || contains "print metadata"
                then
        ...
        let diagnostics =
            [
                ...
                for failureClass in check.RequiredFailureClasses do
                    if
                        not (
                            contains $"diagnostic-class={failureClass}"
                            || contains $"failure-class={failureClass}"
                        )
                    then
                ...
                for fact in check.RequiredNativeFacts do
                    if
                        not (
                            contains $"{fact}=observed:true"
                            || contains $"{fact}=observed:false"
                            || contains $"{fact}=unsupported"
                            || contains $"{fact}=unavailable"
                        )
                    then
                        $"missing observable-vs-unsupported native fact: {fact}"
                if
                    contains "private runtime fallback"
                    && not (contains "fallback-full-desktop-session=false")
                then
...
module LocalConsumerPackages =
    ...
    let classifyDrift (expected: LocalConsumerPackage list) (actual: LocalConsumerPackage list) =
        ...
        |> List.choose (fun package ->
            let actualPackage =
                actual
                |> List.tryFind (fun candidate -> candidate.PackageId = package.PackageId)
...
module GeneratedConsumerValidation =
    ...
    let verifyPackageResolution check =
        let drift =
            LocalConsumerPackages.classifyDrift check.RequestedPackages check.ResolvedPackages

        let nu1603 =
            check.RestoreWarnings
            |> List.filter (fun warning -> warning.Contains("NU1603", StringComparison.OrdinalIgnoreCase))

        let missingSources = check.PackageSources |> List.isEmpty
        ...
        let failureReason =
            if not (List.isEmpty nu1603) then Some "NU1603"
            elif not (List.isEmpty drift) then Some "version-mismatch"
            elif missingSources then Some "missing-package-sources"
            else None
    ...
    let selectVisualEvidence request =
        if request.ScreenshotAvailable then
            ...
            let reason =
                ...
                |> Option.defaultValue "no screenshot or pixel-readback path available"
    ...
    let buildValidationContractOutput check =
        ...
        let failureClass =
            if not check.PackageResolution.ExactMatch then
                check.PackageResolution.FailureReason
                |> Option.defaultValue "package-verification"
            elif not check.GeneratedTests.Authoritative then
                check.GeneratedTests.NonAuthoritativeReason
                |> Option.defaultValue "generated-test-execution"
                ...
                check.ImageEvidence.FailureReason
                |> Option.defaultValue "visual-evidence-validation"
            else
                "none"
        ...
        let output =
            [
                ...
                $"generated-tests-ran={(check.GeneratedTests.Authoritative
                                        && check.GeneratedTests.NonAuthoritativeReason.IsNone)
                                           .ToString()
                                           .ToLowerInvariant()}"
    ...
    let validate (check: GeneratedLayoutValidationCheck) =
        ...
        let diagnostics =
            [
                ...
                if
                    classified.ProofLevel = UnsupportedLayoutInspection
                    && classified.UnsupportedReasons.IsEmpty
                then
        ...
        let failureClass =
            if diagnostics.IsEmpty then
                ...
                classified.ProofLevel = DeterministicRenderOnly
                && classified.RenderEvidence.IsSome
            then
...
module HostWarningClassification =
    let classify (check: HostWarningClassificationCheck) =
        ...
        let known =
            not warningLines.IsEmpty && warningLines |> List.forall lineHasKnownMarker

        let layoutAccepted =
            check.LayoutReadable = Some true
            || check.ExplicitlyUnsupportedWithoutReadabilityClaim
...
module PersistentLaunchArtifactValidation =
    ...
    let validate (check: PersistentLaunchArtifactCheck) =
        ...
        let missing =
            requiredFields |> List.filter (fun field -> not (fields.ContainsKey field))
...
module RuntimeDiagnosticReadiness =
    let validate (check: RuntimeDiagnosticReadinessCheck) : RuntimeDiagnosticReadinessResult =
        let status =
            FS.GG.UI.Diagnostics.RuntimeDiagnostics.readinessStatusToken check.Summary.Status

        let accepted =
            match check.RequiredStatus with
            | Some required -> check.Summary.Status = required
            | None when check.RequireAccepted ->
                check.Summary.Status = FS.GG.UI.Diagnostics.ReadinessDiagnosticStatus.Accepted
            | None ->
                check.Summary.Status <> FS.GG.UI.Diagnostics.ReadinessDiagnosticStatus.Blocked
                && check.Summary.Status
                   <> FS.GG.UI.Diagnostics.ReadinessDiagnosticStatus.ReviewRequired

        let diagnostics =
            [
                if
                    check.RequireAccepted
                    && check.Summary.Status <> FS.GG.UI.Diagnostics.ReadinessDiagnosticStatus.Accepted
                then
                ...
                | Some required when check.Summary.Status <> required ->
                    let requiredToken =
                        FS.GG.UI.Diagnostics.RuntimeDiagnostics.readinessStatusToken required
...
module DefaultTextGlyphEvidence =
    ...
    let regionBounds (bitmap: SKBitmap) (region: Rect option) =
        ...
        | Some bounds ->
            ...
            let maxX =
                Math.Clamp(int (Math.Ceiling(bounds.X + bounds.Width)), x + 1, bitmap.Width)

            let maxY =
                Math.Clamp(int (Math.Ceiling(bounds.Y + bounds.Height)), y + 1, bitmap.Height)
    ...
    let validate (check: DefaultTextGlyphEvidenceCheck) =
        ...
        let insideReadiness =
            screenshotFullPath.StartsWith(
                normalizedReadiness.TrimEnd(IO.Path.DirectorySeparatorChar)
                    ...
                    if Object.ReferenceEquals(bitmap, null) then
                        ...
                        let expectedDiagnostics =
                            [
                                ...
                                | Some width when bitmap.Width <> width ->
                                    $"screenshot width {bitmap.Width} does not match expected {width}"
                                | _ -> ()
                                match check.ExpectedHeight with
                                | Some height when bitmap.Height <> height ->
                                    $"screenshot height {bitmap.Height} does not match expected {height}"
                                | _ -> ()
                            ]
                        ...
                        [
                            ...
                            if
                                placeholderMetric > 0.55
                                || (foregroundRatio <= 0.25 && transitionRatio < 0.025 && foreground > 0)
                            then
        ...
        let failureClass =
            if accepted then
                ...
                artifactDiagnostics
                |> List.exists (fun item -> item.Contains("missing") || item.Contains("decodable"))
            then
                ...
                artifactDiagnostics
                |> List.exists (fun item -> item.Contains("placeholder") || item.Contains("tofu"))
            then
...
module EvidenceReports =
    ...
    let field name value = { Name = name; Value = value }
    ...
    let build (request: EvidenceReportRequest) =
        let standardFields =
            [ field "status" (statusText request.Status); field "command" request.Command ]
    ...
    let validate (report: EvidenceReport) =
        ...
        let required =
            ...
            @ (if report.Status = EvidenceUnsupported then
        ...
        let diagnostics =
            [
                if
                    report.Lines
                    <> (report.Fields |> List.map (fun item -> $"{item.Name}={item.Value}"))
                then
    ...
    let validateScreenshotArtifact (check: ScreenshotArtifactValidationCheck) =
        ...
        let expectedMatches =
            ...
            | Some(width, height), Some expectedWidth, Some expectedHeight ->
...

        {
            ...
            FailureClass =
                if diagnostics.IsEmpty then
                    ...
                    Some "invalid-screenshot-artifact"
            Diagnostics = diagnostics
        }
    ...
    let validateScreenshotEvidence (check: ScreenshotEvidenceReportCheck) =
        ...
        let normalizedKind =
            check.EvidenceKind |> Option.map (fun value -> value.Trim().ToLowerInvariant())

        let normalizedSource =
            check.CaptureSource |> Option.map (fun value -> value.Trim().ToLowerInvariant())

        let normalizedPixelValidation =
            check.PixelContentValidation
        ...
        let diagnostics =
            [
                ...
                | "ok" ->
                    ...
                    if
                        normalizedSource <> Some "live-viewer-window"
                        && normalizedSource <> Some "offscreen-scene-raster"
                    then
                    ...
                    if
                        normalizedPixelValidation <> Some "non-blank"
                        && normalizedPixelValidation <> Some "pixel-content-non-blank"
                    then
                ...
                | "unsupported" ->
                    if
                        check.ScreenshotPath.IsSome
                        || (check.ArtifactPath |> Option.exists (fun value -> value <> "none"))
                    then
    ...
    let validate (report: LayoutReadinessReport) : LayoutReadinessValidationResult =
        ...
        let missingEvidence =
            ...
            |> List.choose (fun evidence ->
                if
                    evidence.Required
                    && (evidence.Path.IsNone || evidence.Status = LayoutReadinessMissingEvidence)
                then
                    Some evidence.Name
                else
                    None)
