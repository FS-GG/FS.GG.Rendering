...
module Feature159Readiness =
    ...
    let private feature159Config
        : ReadinessValidatorConfig<
              Feature159ReadinessCheck,
              Intermediates,
              Feature159ReadinessStatus,
              Feature159ReadinessValidationResult
           > =
        {
                ...
                fun check ->
...

                    {
                        ...
                        FallbackOnly =
                            ...
                            && requiredScenarioResults
                               |> List.forall (fun scenario -> scenario.Status = Feature159FallbackOnly)
                        EnvironmentLimited =
                            requiredScenarioResults
                            |> List.exists (fun scenario -> scenario.Status = Feature159EnvironmentLimited)
                        NonBeneficial =
                            requiredScenarioResults.Length = check.RequiredScenarioIds.Length
                            && requiredScenarioResults
                               |> List.exists (fun scenario -> scenario.Status = Feature159NonBeneficial)
                            && requiredScenarioResults
                               |> List.forall (fun scenario ->
                                   scenario.Status = Feature159Accepted
                                   || scenario.Status = Feature159NonBeneficial)
                ...
                fun check inter ->
                    [
                        ...
                        if
                            ...
                            && not inter.FallbackOnly
                            && not inter.EnvironmentLimited
                        then
                            "accepted Feature 159 readiness requires at least three accepted attempts"
                        if
                            inter.RequiredScenarioResults
                            |> List.exists (fun scenario ->
                                scenario.Status = Feature159Accepted && not scenario.ParityPassed)
                        then
                            "accepted Feature 159 scenarios require passing parity"
                        if
                            check.Scenarios
                            |> List.exists (fun scenario ->
                                scenario.Status = Feature159Accepted && scenario.CounterNetSavedWork <= 0)
                        then
            ...
            DeriveStatus =
                fun check inter diagnostics ->
                    ...
                    then
                        ...
                        inter.RequiredScenarioResults
                        |> List.exists (fun scenario -> scenario.Status = Feature159Rejected)
                    then
                        Feature159Rejected
                        ...
                        && check.AcceptedAttemptCount >= 3
                        && inter.RequiredScenarioResults
                           |> List.forall (fun scenario -> scenario.Status = Feature159Accepted)
                    then
                        Feature159Accepted
                    else
                        Feature159FallbackOnly
...
module Feature160ThroughputReadiness =
    ...
    let private feature160Config
        : ReadinessValidatorConfig<
              Feature160ThroughputReadinessCheck,
              Intermediates,
              Feature160ThroughputReadinessStatus,
              Feature160ThroughputReadinessValidationResult
           > =
        {
            ComputeIntermediates =
                fun check ->
...

                    {
                        ...
                        InvalidWarmup =
                            requiredScenarioResults
                            |> List.choose (fun scenario ->
                                if scenario.WarmupCount <> 3 then
                                    Some scenario.ScenarioId
                                else
                                    None)
                        InvalidRepetitions =
                            requiredScenarioResults
                            |> List.choose (fun scenario ->
                                if scenario.MeasuredRepetitions <> 5 then
                                    Some scenario.ScenarioId
                                else
                                    None)
                        InvalidSamplePolicy =
                            requiredScenarioResults
                            |> List.choose (fun scenario ->
                                if samplePolicyAccepted scenario.SamplePolicy then
                                    None
                                else
                                    Some scenario.ScenarioId)
                        UnsupportedArtifactViolation =
            ...
            MiddleDiagnostics =
                fun check inter ->
                    [
                        ...
                        if
                            inter.FullValidationBlocked
                            && check.AcceptedIterationCount >= inter.RequiredIterations
                            && inter.MissingScenarios.IsEmpty
                        then
            ...
            DeriveStatus =
                fun check inter diagnostics ->
                    if
                        check.UnsupportedHostStatus = Feature160EnvironmentLimited
                        && check.AcceptedIterationCount = 0
                    then
                        ...
                        || check.PerformanceClaim <> "performance-not-accepted"
                    then
                        Feature160Rejected
                    elif
                        check.AcceptedIterationCount >= inter.RequiredIterations
                    ...
                    then
                        ...
                        diagnostics.IsEmpty
                        && check.AcceptedIterationCount >= inter.RequiredIterations
                        && fullValidationPassed check.FullValidationStatus
                    then
                        Feature160Accepted
...
module Feature161HostLaneReadiness =
    ...
    let requiredFacts (facts: Feature161HostFactEvidence option) =
        ...
        | Some facts ->
            [
                if blank facts.LaneId then
                    "lane-id"
                if blank facts.DisplayServer then
                    "display-server"
                if blank facts.DisplayIdentity then
                    "display-identity"
                if blank facts.RendererIdentity then
                    "renderer-identity"
                if facts.DirectRendering.IsNone then
                    "direct-rendering"
                if blank facts.RefreshStatus then
                    "refresh-status"
                if blank facts.DriverIdentity then
                    "driver-identity"
                if blank facts.PackageVersionSet then
                    "package-version-set"
                if blank facts.CpuLoadNote then
                    "cpu-load-note"
                if blank facts.GpuLoadNote then
                    "gpu-load-note"
                if blank facts.HostProfile then
                    "host-profile"
                if blank facts.RunIdentity then
                    "run-identity"
                if blank facts.ScenarioIdentity then
                    "scenario-identity"
                if blank facts.TimingPolicyIdentity then
                    "timing-policy-identity"
                if List.isEmpty facts.ArtifactPaths then
                    "artifact-paths"
            ]
    ...
    let unsupportedFacts (facts: Feature161HostFactEvidence option) =
        ...
        | Some facts ->
            [
                if facts.DisplayServer = "missing-display" || blank facts.DisplayIdentity then
                    "missing-display"
                if facts.DirectRendering = Some false then
                    "indirect-rendering"
                if
                    facts.RendererIdentity.Contains("llvmpipe", StringComparison.OrdinalIgnoreCase)
                    || facts.RendererIdentity.Contains("software", StringComparison.OrdinalIgnoreCase)
                then
                    "software-raster"
                if facts.RendererIdentity = "unknown" then
                    "unknown-renderer"
                if
    ...
    let private feature161Config
        : ReadinessValidatorConfig<
              Feature161HostLaneReadinessCheck,
              Intermediates,
              Feature161HostLaneReadinessStatus,
              Feature161HostLaneReadinessValidationResult
           > =
        {
            ComputeIntermediates =
                fun check ->
            ...
            MiddleDiagnostics =
                fun check inter ->
                    [
                        ...
                        if
                            check.AcceptedLaneScopedPerformanceArtifacts < 1
                            && check.UnsupportedHostStatus <> Feature161EnvironmentLimited
                        then
            ...
            DeriveStatus =
                fun check inter diagnostics ->
                    ...
                    then
                        ...
                        || check.ClaimScope.AcceptedLaneId.IsNone
                        || check.ClaimScope.PerformanceClaim <> "performance-not-accepted"
                    then
                        Feature161Rejected
