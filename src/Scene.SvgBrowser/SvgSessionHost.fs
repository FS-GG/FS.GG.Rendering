...
module SvgSessionPolicy =
    let private validate config =
        if
            Double.IsNaN config.SuspensionMilliseconds
            || Double.IsInfinity config.SuspensionMilliseconds
            || config.SuspensionMilliseconds <= 0.0
        then
    ...
    let private demand state =
        if state.ProjectionPending then
            ...
            { state with ProjectionPending = true }, [ SvgSessionPolicyEffect.RequestProjection state.Generation ]

    let private invalidate nextStatus recovery state =
        if state.Generation = UInt64.MaxValue then
            invalidOp "SVG session generation space is exhausted."
        ...
        if recovery then
            ...
            @ [
                ...
                SvgSessionPolicyEffect.RequestRecovery next.Generation
            ]
        else
            effects
    ...
    let update config observation state =
        ...
        if state.Status = SvgSessionStatus.Disposed then
            ...
            | SvgSessionPolicyObservation.Frame timestamp when Double.IsNaN timestamp || Double.IsInfinity timestamp ->
                state, []
            | SvgSessionPolicyObservation.Frame timestamp ->
                match state.LastTimestampMilliseconds with
                | None ->
                    { state with
                        LastTimestampMilliseconds = Some timestamp
                    },
                    []
                ...
                | Some previous ->
                    ...
                    if elapsed >= config.SuspensionMilliseconds then
                        invalidate SvgSessionStatus.Recovering true state
                    else
                        let microseconds =
                            uint64 (Math.Round(elapsed * 1000.0, MidpointRounding.AwayFromZero))
            ...
            | SvgSessionPolicyObservation.CompleteProjection(generation, revision) when generation <> state.Generation ->
                state,
                [
                    SvgSessionPolicyEffect.ProjectionRejected(state.Generation, generation, revision)
                ]
            | SvgSessionPolicyObservation.CompleteProjection(_, revision) ->
                ...
                | _ when not state.ProjectionPending ->
                    state,
                    [
                        SvgSessionPolicyEffect.ProjectionRejected(state.Generation, state.Generation, revision)
                    ]
                | _ ->
                    let accepted =
                        { state with
                    ...
                    if accepted.ProjectionPending then
                        accepted, effects @ [ SvgSessionPolicyEffect.RequestProjection accepted.Generation ]
                    else
                        accepted, effects
            ...
            | SvgSessionPolicyObservation.Resume when state.Status <> SvgSessionStatus.Running ->
                ...
                [ SvgSessionPolicyEffect.ResumeAuthority ]
            ...
            | SvgSessionPolicyObservation.Dispose ->
                { state with
                    Status = SvgSessionStatus.Disposed
                    LastTimestampMilliseconds = None
                    ProjectionPending = false
                    ProjectionQueued = false
                },
                [
                    SvgSessionPolicyEffect.CancelGeneration state.Generation
                    SvgSessionPolicyEffect.Disposed
                ]
...
type SvgSessionHost<'projection>(callbacks: SvgSessionCallbacks<'projection>, config: SvgSessionPolicyConfig) =
    ...
    let interpret effects =
        for effect in effects do
            ...
            | SvgSessionPolicyEffect.ApplyProjection revision ->
                ...
                | _ -> ()
            | SvgSessionPolicyEffect.CancelGeneration generation ->
                pendingPayload <- None
                callbacks.CancelGeneration generation
    ...
    let rec schedule () =
        if state.Status = SvgSessionStatus.Running && frame.IsNone then
            frame <-
                Some(
                    SessionDom.requestFrame (fun timestamp ->
                        frame <- None
    ...
    member _.Replace() =
        cancelScheduled ()
        apply SvgSessionPolicyObservation.Replace
        schedule ()

    member _.DemandProjection() =
        apply SvgSessionPolicyObservation.DemandProjection
