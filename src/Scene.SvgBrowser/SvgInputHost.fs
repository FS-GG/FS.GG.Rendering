...
type SvgInputHost
                    ...
                    let handle =
                        window.setTimeout (
                            (fun () ->
                                ...
                                apply (CommandResolverObservation.DeadlineElapsed(token, availableCommands ()))
                                |> ignore),
                            options.SequenceTimeoutMilliseconds
                        )
    ...
    let accepted eventId effects =
        ...
        |> List.exists (function
            | CommandResolverEffect.PreventDefault id when id = eventId -> true
            | _ -> false)
    ...
    let keyboardGesture (event: KeyboardEvent) =
        ...
        let expectedIndex =
            state.PendingSequence |> Option.map _.Steps.Length |> Option.defaultValue 0

        let uses gesture (binding: InputBinding) =
            if not (active.Contains binding.Context && available.Contains binding.Command) then
                false
            else
                match binding.Gesture, gesture with
                | candidate, expected when candidate = expected -> true
                | InputGesture.KeySequence steps, InputGesture.KeyChord(identity, modifiers) when
                    expectedIndex < steps.Length
                    ->
                    steps[expectedIndex] = (identity, modifiers)
                | _ -> false

        if state.Profile.Bindings |> List.exists (uses logical) then
            logical
        elif state.Profile.Bindings |> List.exists (uses physical) then
            physical
        else
            logical
    ...
    let keyDown (event: Event) =
        ...
        if not native then
            ...
            let effects =
                if
                    keyboard.key = "Escape"
                    && (state.CaptureRestoreFocus.IsSome
                        || not state.ModalStack.IsEmpty
                        || state.PendingSequence.IsSome)
                then
                    ...
                    dispatch
                        source
                        gesture
                        InputGesturePhase.Pressed
                        (InputDom.repeat keyboard)
                        false
                        (catalog.ReservedGestures |> List.contains gesture)
                        (InputDom.composing keyboard || state.IsComposing)
                        eventId
    ...
    let pointerGesture (pointer: PointerEvent) =
        ...
        if InputDom.pointerType pointer = "touch" then
    ...
    let releasePointer (event: Event) =
        ...
        | true, gesture ->
            ...
            let effects =
                dispatch source gesture InputGesturePhase.Released false false false false eventId

            if InputDom.hasPointerCapture root pointerId then
                InputDom.releasePointer root pointerId
        ...
        if not disposed then
            let current =
                ...
                |> List.map (fun pad ->
                    ...
                    (pad.Buttons
                     |> List.indexed
                     |> List.choose (fun (index, pressed) -> if pressed then Some index else None)
                     |> Set.ofList))
                |> Map.ofList
            ...
            for KeyValue(source, previous) in pads do
                ...
                | Some now ->
                    for index in Set.difference previous now do
                        dispatch
                            source
                            (InputGesture.Gamepad $"button-{index}")
                            InputGesturePhase.Released
                            false
                            false
                            false
    ...
    do
        if
            options.SequenceTimeoutMilliseconds < 1
            || options.SequenceTimeoutMilliseconds > 60000
        then
        ...
        listen (root :> EventTarget) "compositionstart" (fun _ ->
            apply CommandResolverObservation.CompositionStarted |> ignore)

        listen (root :> EventTarget) "compositionend" (fun _ ->
            apply CommandResolverObservation.CompositionEnded |> ignore)
        ...
        listen (root :> EventTarget) "focusout" (fun event ->
            ...
            if isNull related || not (InputDom.contains related root) then
                neutralize CommandResolverObservation.FocusLost)
        ...
        listen (document :> EventTarget) "visibilitychange" (fun _ ->
            if InputDom.documentHidden () then
                neutralize CommandResolverObservation.FocusLost)
...
module SvgInputHost =
    let browserGamepads () =
        InputDom.gamepads ()
        |> Array.map (fun (source, buttons) ->
            {
                Source = source
                Buttons = Array.toList buttons
            })
        |> Array.toList
