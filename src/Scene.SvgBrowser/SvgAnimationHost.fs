...
type SvgAnimationPolicyState = private {
      AuthorityRevision: uint64
...
module SvgAnimationPolicy =
    ...
    let private nextRevision state =
        if state.PresentationRevision = UInt64.MaxValue then invalidOp "SVG animation presentation revision space is exhausted."
        state.PresentationRevision + 1UL

    let private appendSchedule state effects =
        if state.Status = SvgAnimationStatus.Running && not state.Active.IsEmpty then effects @ [ SvgAnimationEffect.ScheduleFrame ]
        else effects @ [ SvgAnimationEffect.CancelFrame ]
    ...
    let private applyAt elapsed cueMode request state =
        ...
        { state with PresentationRevision = revision }, sample, effects
    ...
    let update config observation state =
        ...
        if state.Status = SvgAnimationStatus.Disposed then
            ...
            | SvgAnimationObservation.Start request when request.AuthorityRevision <> state.AuthorityRevision ->
                state, [ SvgAnimationEffect.EffectRefused(SvgAnimationRefusal.StaleAuthorityRevision(state.AuthorityRevision, request.AuthorityRevision)) ]
            | SvgAnimationObservation.Start request when state.Active.ContainsKey request.Id ->
                state, [ SvgAnimationEffect.EffectRefused(SvgAnimationRefusal.DuplicateEffectId request.Id) ]
            | SvgAnimationObservation.Start request ->
                let decorativeCount =
                    state.Active
                    |> Map.toSeq
                    |> Seq.sumBy (fun (_, active) -> match active.Request.Importance with Decorative _ -> 1 | Essential -> 0)
                match request.Importance, state.MotionPreference with
                | Decorative _, _ when decorativeCount >= config.MaxDecorativeEffects ->
                    state, [ SvgAnimationEffect.EffectRefused(SvgAnimationRefusal.DecorativeLimitReached config.MaxDecorativeEffects) ]
                | Decorative behavior, SvgMotionPreference.Reduced ->
                    let elapsed = match behavior with SvgReducedMotionBehavior.Settle -> completionElapsed request | SvgReducedMotionBehavior.Substitute -> TimeSpan.Zero
                    let sampled, _, effects = applyAt elapsed ClipCueMode.Seek request state
                    sampled, effects @ [ SvgAnimationEffect.EffectCompleted request.Id; SvgAnimationEffect.CancelFrame ]
                | _ ->
                    let active = { Request = request; Elapsed = TimeSpan.Zero }
                    let accepted = { state with Active = state.Active.Add(request.Id, active) }
                    let sampled, _, effects = applyAt TimeSpan.Zero ClipCueMode.Seek request accepted
                    sampled, appendSchedule sampled effects
            | SvgAnimationObservation.Frame elapsed when state.Status <> SvgAnimationStatus.Running || elapsed <= TimeSpan.Zero -> state, []
            | SvgAnimationObservation.Frame elapsed ->
                ...
                for KeyValue(id, active) in state.Active do
                    ...
                    let sampled, sample, sampleEffects = applyAt nextElapsed (ClipCueMode.LiveAdvance active.Elapsed) active.Request current
                    current <- sampled
            ...
            | SvgAnimationObservation.Pause when state.Status = SvgAnimationStatus.Running ->
                { state with Status = SvgAnimationStatus.Paused }, [ SvgAnimationEffect.CancelFrame ]
            | SvgAnimationObservation.Pause -> state, []
            | SvgAnimationObservation.Resume when state.Status = SvgAnimationStatus.Paused ->
                let resumed = { state with Status = SvgAnimationStatus.Running }
            ...
            | SvgAnimationObservation.Seek(id, elapsed) ->
                ...
                let nextActive =
                    if sample.IsComplete then sampled.Active.Remove id
                    else sampled.Active.Add(id, { active with Elapsed = bounded })
                let accepted = { sampled with Active = nextActive }
                let completed = if sample.IsComplete then effects @ [ SvgAnimationEffect.EffectCompleted id ] else effects
            ...
            | SvgAnimationObservation.ReplaceAuthority revision when revision <= state.AuthorityRevision ->
                state, [ SvgAnimationEffect.EffectRefused(SvgAnimationRefusal.StaleAuthorityRevision(state.AuthorityRevision, revision)) ]
            | SvgAnimationObservation.ReplaceAuthority revision ->
                { state with AuthorityRevision = revision; Active = Map.empty }, [ SvgAnimationEffect.CancelFrame ]
            | SvgAnimationObservation.SetMotionPreference preference when preference = state.MotionPreference -> state, []
            | SvgAnimationObservation.SetMotionPreference SvgMotionPreference.Full ->
                let accepted = { state with MotionPreference = SvgMotionPreference.Full }
                accepted, appendSchedule accepted []
            | SvgAnimationObservation.SetMotionPreference SvgMotionPreference.Reduced ->
                let mutable current = { state with MotionPreference = SvgMotionPreference.Reduced }
                let mutable nextActive = state.Active
                let effects = ResizeArray<SvgAnimationEffect>()
                for KeyValue(id, active) in state.Active do
                    ...
                    | Essential -> ()
                    | Decorative behavior ->
                        let elapsed = match behavior with SvgReducedMotionBehavior.Settle -> completionElapsed active.Request | SvgReducedMotionBehavior.Substitute -> TimeSpan.Zero
                        let sampled, _, sampleEffects = applyAt elapsed ClipCueMode.Seek active.Request current
                        current <- sampled
                        sampleEffects |> List.iter effects.Add
                        effects.Add(SvgAnimationEffect.EffectCompleted id)
                        nextActive <- nextActive.Remove id
            ...
            | SvgAnimationObservation.Dispose ->
                { state with Status = SvgAnimationStatus.Disposed; Active = Map.empty },
...
type SvgAnimationHost(callbacks: SvgAnimationCallbacks, config: SvgAnimationPolicyConfig, authorityRevision: uint64) =
    ...
    let initialMotion = if AnimationDom.prefersReducedMotion() then SvgMotionPreference.Reduced else SvgMotionPreference.Full
    let mutable state = SvgAnimationPolicy.initialize config authorityRevision initialMotion
    ...
    let rec scheduleFrame () =
        ...
        if status = SvgAnimationStatus.Running && activeCount > 0 && frame.IsNone then
            frame <- Some(AnimationDom.requestFrame (fun timestamp ->
                frame <- None
                let delta = lastTimestamp |> Option.map (fun previous -> TimeSpan.FromMilliseconds(max 0.0 (timestamp - previous))) |> Option.defaultValue TimeSpan.Zero
    ...
    and interpret effects =
        for effect in effects do
            match effect with
            | SvgAnimationEffect.ApplySample(id, authority, revision, sample) -> callbacks.ApplySample id authority revision sample
    ...
    do
        listen (document :> EventTarget) "visibilitychange" (fun _ ->
            if AnimationDom.hidden() then apply SvgAnimationObservation.Pause
            else apply SvgAnimationObservation.Resume)
        listen (window :> EventTarget) "blur" (fun _ -> apply SvgAnimationObservation.Pause)

    member _.Start request = apply (SvgAnimationObservation.Start request)
    member _.Pause() = apply SvgAnimationObservation.Pause
    member _.Resume() = apply SvgAnimationObservation.Resume
    member _.Seek(effectId, elapsed) = apply (SvgAnimationObservation.Seek(effectId, elapsed))
    member _.ReplaceAuthority revision = apply (SvgAnimationObservation.ReplaceAuthority revision)
    member _.SetMotionPreference preference = apply (SvgAnimationObservation.SetMotionPreference preference)
    member _.Observe() =
        let authority, presentation, status, motion, activeCount = SvgAnimationPolicy.observe state
        { AuthorityRevision = authority
          PresentationRevision = presentation
          Status = status
          MotionPreference = motion
          ActiveEffectCount = activeCount
          OwnedListenerCount = if status = SvgAnimationStatus.Disposed then 0 else listeners.Count
