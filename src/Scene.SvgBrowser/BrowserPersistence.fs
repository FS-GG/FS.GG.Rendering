...
type BrowserStoredValue = { Key: BrowserStorageKey; SchemaVersion: int; PayloadHash: string; Payload: string }
type BrowserArchiveMember = { Path: string; Hash: string; Content: string }
type BrowserArchive = { Members: BrowserArchiveMember list }

[<RequireQualifiedAccess>]
type BrowserArchiveIssue = EmptyArchive | InvalidPath of string | DuplicatePath of string | MissingMember of string | UnexpectedMember of string | HashMismatch of path: string * expected: string * actual: string
...
module BrowserArchive =
    ...
    let validate hashContent requiredPaths archive =
        ...
        if ordered.IsEmpty then issues.Add BrowserArchiveIssue.EmptyArchive
        let seen=HashSet<string>()
        for member' in ordered do
            if not(validPath member'.Path) then issues.Add(BrowserArchiveIssue.InvalidPath member'.Path)
            if not(seen.Add member'.Path) then issues.Add(BrowserArchiveIssue.DuplicatePath member'.Path)
            let actual=hashContent member'.Content
            if actual<>member'.Hash then issues.Add(BrowserArchiveIssue.HashMismatch(member'.Path,member'.Hash,actual))
        let actualPaths=seen|>Set.ofSeq
        let required=requiredPaths|>Set.ofList
        for path in Set.difference required actualPaths do issues.Add(BrowserArchiveIssue.MissingMember path)
        for path in Set.difference actualPaths required do issues.Add(BrowserArchiveIssue.UnexpectedMember path)
        if issues.Count=0 then Ok {Members=ordered} else Error(List.ofSeq issues)

type BrowserPersistenceConfig = { DatabaseName: string; MaxPayloadCharacters: int; MaxArchiveCharacters: int }
[<RequireQualifiedAccess>]
type BrowserPersistenceFailure = InvalidRequest of string | QuotaExceeded | DatabaseError of string | StaleGeneration | Disposed
[<RequireQualifiedAccess>]
type BrowserPersistenceEvent = Ready | Persisted of BrowserStorageOperationId | Loaded of BrowserStorageKey * BrowserStoredValue option | Exported of string | Imported of BrowserStorageOperationId | Failed of BrowserStorageOperationId option * BrowserPersistenceFailure | Disposed
type BrowserPersistenceObservation = { IsReady: bool; Generation: uint64; PendingRequestCount: int; IsDisposed: bool }
...
module private PersistenceDom =
    ...
    let put(_graph:obj)(_key:string)(_family:string)(_slot:string)(_schema:int)(_hash:string)(_payload:string)(_ok:unit->unit)(_fail:string->unit):unit=jsNative
    ...
    let importArchive(_graph:obj)(_json:string)(_max:int)(_ok:unit->unit)(_fail:string->unit):unit=jsNative
...
type BrowserPersistenceHost(config:BrowserPersistenceConfig,onEvent:BrowserPersistenceEvent->unit)=
    ...
    let storageKeyText (value: BrowserStorageKey) = familyId value.Family + ":" + value.Slot
    let validSlot (value: string) =
        not (String.IsNullOrWhiteSpace value)
        && value <> "."
        && value <> ".."
        && not (value.Contains("/", StringComparison.Ordinal))
    let token (id: BrowserStorageOperationId) = string id.Generation + ":" + string id.Operation
    ...
    do
        if String.IsNullOrWhiteSpace config.DatabaseName then invalidArg (nameof config) "database name is required"
        if config.MaxPayloadCharacters<1 then invalidArg (nameof config) "payload limit must be positive"
        if config.MaxArchiveCharacters<1 then invalidArg (nameof config) "archive limit must be positive"
    ...
    member _.Persist(operation,value)=
        if disposed then onEvent(BrowserPersistenceEvent.Failed(Some operation,BrowserPersistenceFailure.Disposed))
        elif operation.Generation<generation then onEvent(BrowserPersistenceEvent.Failed(Some operation,BrowserPersistenceFailure.StaleGeneration))
        elif not (validSlot value.Key.Slot) || value.SchemaVersion<1 || String.IsNullOrWhiteSpace value.PayloadHash then onEvent(BrowserPersistenceEvent.Failed(Some operation,BrowserPersistenceFailure.InvalidRequest "invalid stored value"))
        elif value.Payload.Length>config.MaxPayloadCharacters then onEvent(BrowserPersistenceEvent.Failed(Some operation,BrowserPersistenceFailure.QuotaExceeded))
        else
            ...
            if pending.Add t then
                PersistenceDom.put graph (storageKeyText value.Key) (familyId value.Key.Family) value.Key.Slot value.SchemaVersion value.PayloadHash value.Payload
                    (fun () ->
            ...
            else onEvent(BrowserPersistenceEvent.Failed(Some operation,BrowserPersistenceFailure.InvalidRequest "duplicate operation"))
    member _.Load(key)=
        if disposed then onEvent(BrowserPersistenceEvent.Failed(None,BrowserPersistenceFailure.Disposed))
        elif not (validSlot key.Slot) then onEvent(BrowserPersistenceEvent.Failed(None,BrowserPersistenceFailure.InvalidRequest "invalid slot"))
        else
            PersistenceDom.get
                graph
                (storageKeyText key)
    ...
    member _.ImportArchive(operation,archiveJson)=
        if disposed then onEvent(BrowserPersistenceEvent.Failed(Some operation,BrowserPersistenceFailure.Disposed))
            ...
            if pending.Add t then
                ...
                onEvent(BrowserPersistenceEvent.Failed(Some operation,BrowserPersistenceFailure.InvalidRequest "duplicate operation"))
    member _.Observe()={IsReady=ready;Generation=generation;PendingRequestCount=pending.Count;IsDisposed=disposed}
