...
/// The pure identity-label / rich-text LAYOUT engine, extracted out of `Symbology` (F-CORE-1). `internal`:
/// the grammar code in `Symbology` threads the Token-derived geometry in and consumes the fitted nodes, so
/// this module is an implementation detail, not part of the public symbology surface. Byte-identical to the
/// pre-extraction private helpers — only these four entry points cross the module boundary.
module internal LabelLayout =

    /// The label-motion rest phase (`0.0`): every `LabelMotion` kind is the identity transform here, and the
    /// grammars also use it as the static (non-animated) symbol phase. Single source so the two never drift.
    val restPhase: float

    /// Measured stacking line-height of the base (default-weight) label font at `baseSize` (FR-003).
    val lineHeightOf: baseSize: float -> float

    /// Route a resolved `LabelText option` through the zero-drift fit/wrap/cap layout for a grammar's
    /// label region, returning the fitted, centred `glyphRunProof` nodes (`[]` for no drawable label).
    val labelDispatch:
        centerX: float ->
        baselineY: float ->
        regionWidth: float ->
    ...
    /// Apply a bound `LabelMotion` to the already-fitted static label nodes as a pure function of the
    /// normalised phase; `ph = restPhase` returns the static nodes verbatim (FR-007).
    val motionLabelNodes:
        kind: LabelMotion ->
        ph: float ->
        centerX: float ->
        baselineY: float ->
