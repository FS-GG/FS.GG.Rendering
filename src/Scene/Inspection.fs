...
module VisualInspection =
    ...
    let artifactDiagnostics (artifact: VisualInspectionArtifact) =
        ...
        [
            ...
            for node in artifact.Nodes do
                ...
                | Some parent when not (Set.contains parent parentIds) ->
                    $"node {node.NodeId} references missing parent {parent}"
                | _ -> ()
    ...
    let normalizeArtifact (artifact: VisualInspectionArtifact) =
        { artifact with
            ...
            UnsupportedFacts =
                artifact.UnsupportedFacts
                |> List.sortBy (fun fact -> fact.Fact, defaultArg fact.OwnerId "")
        }
...
module RetainedInspection =
    ...
    let private clipped frame rects =
        rects |> List.choose (clipRect frame) |> List.distinct
    ...
    let dirtyUnionArea frameBounds dirtyRectangles =
        ...
        | _ ->
            ...
            let covered x1 x2 y1 y2 =
                rects
                |> List.exists (fun r -> x1 >= r.X && x2 <= r.X + r.Width && y1 >= r.Y && y2 <= r.Y + r.Height)
...
module SceneInspection =
    ...
    let private transformPoint matrix (point: Point) =
        ...
        if not (finite w) || abs w < 1e-12 then
            None
        else
    ...
    let private union results =
        match
            ...
            |> List.tryPick (function
                | SceneDrawableBounds.Unknown reason -> Some reason
                | _ -> None)
        with
        ...
        | None ->
            match
                ...
                |> List.choose (function
                    | SceneDrawableBounds.Known value -> Some value
                    | _ -> None)
            with
            | [] -> SceneDrawableBounds.NoDrawableContent
            | values ->
    ...
    let private expandByPaint paint rect =
        ...
        let finitePaint =
            [ strokeExtent; pathEffectExtent; maskExtent ] |> List.forall finite
    ...
    let inspect viewport scene =
        ...
        and walkNode parentPath path matrix clip node =
            ...
            let children, localBounds =
                    ...
                    let nested =
                        ...
                        |> List.mapi (fun sceneIndex child ->
                            ...
                            |> List.mapi (fun nodeIndex childNode ->
                                walkNode
                                    (Some path)
                                    ($"{path}/group/{sceneIndex}/nodes/{nodeIndex}")
                                    matrix
                                    clip
                                    childNode))
                        |> List.collect id
                ...
                | Arc(bounds, _, _, paint) -> [], expandByPaint paint bounds
                ...
                | ClipNode(clipShape, childScene) ->
                    ...
                    nested, nested |> List.map snd |> union
                | RegionNode(region, paint) -> [], region.Bounds |> List.map (expandByPaint paint) |> union
