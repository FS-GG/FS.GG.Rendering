...
module internal Shaping =
        ...
        let runPayload =
            ...
            |> List.map (fun r ->
                ...
                sprintf
                    ...
                    length
                    r.SourceText
                    (r.ResolvedFont |> Option.defaultValue "")
                    r.Direction
                    r.Script
                    r.FallbackDecision
                    r.Advance
    ...
    let buildGlyphRun (text: string) (font: FontSpec) : GlyphRunData =
        ...
        let diagnostics =
            ...
            |> Seq.choose (fun ch ->
                if Char.IsSurrogate ch then
                    Some(
                        sprintf
                            "glyph-run-proof: unsupported surrogate code unit U+%04X deferred to full shaping"
                            (int ch)
                    )
                else
                    None)
        ...
        let shapedGlyphs = glyphs |> List.map shapedGlyphOfGlyphRun
...

        {
            ...
            Fingerprint =
                glyphRunFingerprintOf
                    pureFallbackProvider
                    [ run ]
                    PureFallbackMode
                    text
                    font
    ...
    let buildFallbackShapedText (text: string) (font: FontSpec) : ShapedTextResult =
        ...
        let glyphs = data.Glyphs |> List.map shapedGlyphOfGlyphRun
        ...
        { result with
            Fingerprint = shapedTextFingerprintOf result
        }
    ...
    let measureText (text: string) (font: FontSpec) = measureTextHeuristic text font
    ...
    let mutable private realTextMeasurer: (string -> FontSpec -> TextMetrics) option =
        None
