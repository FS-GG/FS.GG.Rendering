...
type SpatialWorkingSetIndex<'value> =
    private | SpatialWorkingSetIndex of
        float *
        SpatialEntry<'value> list *
        Map<int64 * int64, int list> *
        Map<string, int>
...
module SpatialWorkingSet =
    ...
    let private chunkCount size bounds =
        let axis start length =
            floor ((start + length) / size) - floor (start / size) + 1.0
    ...
    let create chunkSize entries =
        let duplicates =
            ...
            |> List.choose (fun (id, values) ->
                if values.Length > 1 && not (String.IsNullOrWhiteSpace id) then
                    Some id
                else
                    None)
            |> List.sort
        ...
        let issues =
            [
                if not (finite chunkSize) || chunkSize <= 0.0 then
                    SpatialWorkingSetIssue.InvalidChunkSize chunkSize
                for index, entry in List.indexed entries do
                    if String.IsNullOrWhiteSpace entry.Id then
                        SpatialWorkingSetIssue.MissingId index

                    if
                        not (validRect entry.Bounds)
                        || (finite chunkSize
                            && chunkSize > 0.0
                            && not (validChunkAddress chunkSize entry.Bounds))
                    then
                        SpatialWorkingSetIssue.InvalidBounds entry.Id
                    elif
                        finite chunkSize
                        && chunkSize > 0.0
                        && chunkCount chunkSize entry.Bounds > maxEntryChunks
                    then
                        SpatialWorkingSetIssue.EntryChunkLimitExceeded entry.Id
                for id in duplicates do
                    SpatialWorkingSetIssue.DuplicateId id
            ]
        ...
        if not issues.IsEmpty then
                    ...
                    (fun state (index, entry) ->
                        seq {
                            for x in chunkRange chunkSize entry.Bounds.X entry.Bounds.Width do
                                for y in chunkRange chunkSize entry.Bounds.Y entry.Bounds.Height do
                                    yield x, y
                        }
                        |> Seq.fold
                            (fun current key ->
                                let values = Map.tryFind key current |> Option.defaultValue []
                                Map.add key (values @ [ index ]) current)
                            state)
                    Map.empty
            ...
            let identities =
                entries
                |> List.indexed
                |> List.map (fun (index, entry) -> entry.Id, index)
                |> Map.ofList
    ...
    let query query (SpatialWorkingSetIndex(chunkSize, entries, buckets, identities)) =
        let issues =
            [
                if not (validRect query.Viewport) then
                    SpatialWorkingSetIssue.InvalidViewport
                if not (finite query.Overscan) || query.Overscan < 0.0 then
                    SpatialWorkingSetIssue.InvalidOverscan query.Overscan
            ]
        ...
        if not issues.IsEmpty then
            ...
            if not (validRect expanded) || not (validChunkAddress chunkSize expanded) then
                ...
                query.MaxVisitedChunks <= 0
                || chunkCount chunkSize expanded > float query.MaxVisitedChunks
            then
                ...
                let chunks =
                    [
                        for x in chunkRange chunkSize expanded.X expanded.Width do
                            for y in chunkRange chunkSize expanded.Y expanded.Height do
                                yield x, y
                    ]

                let candidateIndices =
                    chunks
                    |> List.collect (fun key -> Map.tryFind key buckets |> Option.defaultValue [])
                    |> Set.ofList

                let pinnedIndices =
                    query.PinnedIds |> Seq.choose (fun id -> Map.tryFind id identities) |> Set.ofSeq
                ...
                let selected =
                    ...
                    |> List.choose (fun index ->
                        ...
                        if Set.contains index pinnedIndices || intersects entry.Bounds expanded then
                            Some entry
                        else
                            None)
