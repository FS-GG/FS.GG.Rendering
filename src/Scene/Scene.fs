...
module Colors =
    ...
    let rgb red green blue = rgba red green blue 255uy
...
module Paint =
    ...
    let withOpacity opacity paint = { paint with Opacity = opacity }
    ...
    let withStrokeCap cap paint =
        { paint with
            Stroke = Some { ensureStroke paint with Cap = cap }
        }

    let withStrokeJoin join paint =
        { paint with
            Stroke = Some { ensureStroke paint with Join = join }
        }
...
module Scene =
    ...
    let group scenes = { Nodes = [ Group scenes ] }

    let rectangle bounds fill = { Nodes = [ Rectangle(bounds, fill) ] }
    ...
    let ellipse bounds paint = { Nodes = [ Ellipse(bounds, paint) ] }
    ...
    let path path paint = { Nodes = [ Path(path, paint) ] }

    let points points paint = { Nodes = [ Points(points, paint) ] }
    ...
    let textRun run = { Nodes = [ TextRun run ] }
    ...
    let buildFallbackShapedText (text: string) (font: FontSpec) : ShapedTextResult =
        Text.Shaping.buildFallbackShapedText text font

    let shapedTextFingerprint (result: ShapedTextResult) : string =
        Text.Shaping.shapedTextFingerprint result
    ...
    let glyphRunDataFromShapedText (result: ShapedTextResult) : GlyphRunData =
        Text.Shaping.glyphRunDataFromShapedText result
    ...
    let setRealTextMeasurer (measurer: (string -> FontSpec -> TextMetrics) option) : unit =
        Text.Shaping.setRealTextMeasurer measurer

    let textMeasurementVersionBucket () : string =
        Text.Shaping.textMeasurementVersionBucket ()

    let setTextMeasurementVersionBucket (bucket: string) : unit =
        Text.Shaping.setTextMeasurementVersionBucket bucket

    let measureTextResolved (text: string) (font: FontSpec) : TextMetrics =
        Text.Shaping.measureTextResolved text font

    let image bounds source = { Nodes = [ Image(bounds, source) ] }

    let clipped clip scene = { Nodes = [ ClipNode(clip, scene) ] }
    ...
    let picture picture = { Nodes = [ PictureNode picture ] }

    let chart values = { Nodes = [ Chart values ] }
    ...
    let rec diagnostics scene =
        ...
        let paintDiagnostics paint =
            [
                ...
                | Dash([], _) ->
                    diagnostic DiagnosticSeverity.Warning "Dash path effect has no intervals." (Some "path-effect")
                | Discrete(segmentLength, _) when segmentLength <= 0.0 ->
                    diagnostic
                        DiagnosticSeverity.Warning
                        "Discrete path effect requires a positive segment length."
                        (Some "path-effect")
                | Corner radius when radius < 0.0 ->
                    diagnostic
                        DiagnosticSeverity.Warning
                        "Corner path effect requires a non-negative radius."
                        (Some "path-effect")
                | _ -> ()
            ]
            ...
            | Image(_, source) when String.IsNullOrWhiteSpace source ->
                [
                    diagnostic
                        ...
                        "Invalid image resource declaration."
                        (Some "Image source path is empty.")
                ]
    ...
    let renderReadbackEvidence (size: Size) scene =
        let capabilities = describe scene |> List.map string |> List.distinct |> List.sort
