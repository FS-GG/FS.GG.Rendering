...
module SvgAffine =
    ...
    let skewXDegrees angle =
        { identity with
            C = Math.Tan(angle * Math.PI / 180.0)
        }

    let skewYDegrees angle =
        { identity with
            B = Math.Tan(angle * Math.PI / 180.0)
        }
...
module SvgDocument =
    ...
    let private issue code path message =
        {
            Code = code
            Location = path
            Message = message
    ...
    let private refsOfElement (element: SvgElement) =
        [
            ...
            | Some presentation ->
                let sourceRef =
                    function
                    | SvgPaintSource.Definition id -> [ id ]
                    | _ -> []
                ...
                yield!
                    ...
                    |> Option.toList
                    |> List.collect (fun stroke -> sourceRef stroke.Source)
            | None -> ()
    ...
    let validate serializedByteCount (limits: SvgDocumentLimits) (document: SvgDocument) =
        ...
        let add code path message =
            issues <- issue code path message :: issues

        let definitions: Map<string, SvgDefinition> =
            document.Definitions |> List.map (fun value -> value.Id, value) |> Map.ofList
        ...
        let requireDefinition expected path reference =
            ...
            | Some definition when definitionKind definition.Content <> expected ->
                add
                    ...
                    path
                    $"expected {expected} definition but {reference} is {definitionKind definition.Content}"
            | Some _ -> ()

        if document.Schema <> schema then
            add "unknown-schema" "/schema" $"expected {schema}"

        if String.IsNullOrWhiteSpace document.Id then
            add "blank-document-id" "/id" "document id must not be blank"

        if
            not (validRect document.ViewBox)
            || document.ViewBox.Width <= 0.0
            || document.ViewBox.Height <= 0.0
        then
        ...
        let duplicateIds kind path ids =
            ...
            |> List.iter (fun (id, count) ->
                if count > 1 then
                    add "duplicate-id" path $"{kind} id is duplicated: {id}")
        ...
        let rec validateElement path (element: SvgElement) =
            ...
            element.SemanticId
            |> Option.iter (fun id -> allSemanticIds <- id :: allSemanticIds)

            if String.IsNullOrWhiteSpace element.Id then
                add "blank-element-id" (path + "/id") "element id must not be blank"
            ...
            if not (SvgAffine.isFinite element.Transform) then
                add "non-finite-transform" (path + "/transform") "transform must be finite"
            ...
            | Some presentation ->
                let requirePaint =
                    function
                    | SvgPaintSource.Definition reference -> requireDefinition "gradient" path reference
                    | _ -> ()

                presentation.FillSource |> Option.iter requirePaint

                presentation.StrokeStyle
                |> Option.iter (fun stroke ->
                    requirePaint stroke.Source

                    if
                        not (finite stroke.Width && finite stroke.Miter && finite stroke.DashOffset)
                        || stroke.Width < 0.0
                        || stroke.Miter < 0.0
                        || stroke.Dash |> List.exists (fun value -> not (finite value) || value <= 0.0)
                    then
                        add
                            "invalid-stroke-presentation"
                            (path + "/presentation/stroke")
                            "stroke width, miter, dash and dash offset must be finite and non-negative; dash entries must be positive")

                if
                    not (finite presentation.OverallOpacity)
                    || presentation.OverallOpacity < 0.0
                    || presentation.OverallOpacity > 1.0
                then
                    add
                        "invalid-presentation-opacity"
                        (path + "/presentation/opacity")
            ...
            | SvgElementContent.Group children ->
                children
                |> List.iteri (fun index child -> validateElement (childPath path index) child)
            ...
            | SvgElementContent.SymbolInstance(reference, viewport) ->
                ...
                |> Option.iter (fun value ->
                    if not (validRect value) || value.Width < 0.0 || value.Height < 0.0 then
                        add
                            "invalid-symbol-viewport"
                            (path + "/viewport")
                            "symbol viewport must be finite and non-negative")
        ...
        let rec validateDefinition path (definition: SvgDefinition) =
            if String.IsNullOrWhiteSpace definition.Id then
                add "blank-definition-id" (path + "/id") "definition id must not be blank"

            match definition.Content with
            | SvgDefinitionContent.Symbol(Some viewBox, _) when
                not (validRect viewBox) || viewBox.Width <= 0.0 || viewBox.Height <= 0.0
                ->
                add
                    "invalid-symbol-view-box"
                    (path + "/viewBox")
            ...
            | SvgDefinitionContent.Clip(_, shapes) ->
                shapes
                |> List.iteri (fun index shape ->
                    match shape with
                    | SvgClipShape.Rectangle rect when not (validRect rect) || rect.Width < 0.0 || rect.Height < 0.0 ->
                        add
                            "invalid-clip-rectangle"
                            $"{path}/shapes/{index}"
                            "clip rectangle must be finite and non-negative"
                    | SvgClipShape.Path pathSpec when not (validPath pathSpec) ->
                        add
                            "invalid-clip-path"
                            $"{path}/shapes/{index}"
                            "clip path geometry must be finite with positive arc bounds"
                    | SvgClipShape.Intersection references ->
                        if references.IsEmpty then
                            add
                                "empty-clip-intersection"
                                $"{path}/shapes/{index}"
                                "clip intersection must reference at least one clip"

                        references |> List.iter (requireDefinition "clip" path)
                    | _ -> ())
            | SvgDefinitionContent.Mask(_, region, _, children) ->
                if not (validRect region) || region.Width < 0.0 || region.Height < 0.0 then
                    add "invalid-mask-region" (path + "/region") "mask region must be finite and non-negative"

                children
                |> List.iteri (fun index child -> validateElement (childPath path index) child)
            | SvgDefinitionContent.Gradient gradient ->
                if not (SvgAffine.isFinite gradient.Transform) then
                    add "non-finite-transform" (path + "/transform") "gradient transform must be finite"
                ...
                | SvgGradientGeometry.Radial(center, radius, focal) when
                    not (
                        validPoint center
                        && finite radius
                        && radius > 0.0
                        && (focal |> Option.forall validPoint)
                    )
                    ->
                    add
                        "invalid-gradient-geometry"
                        (path + "/geometry")
                        "radial gradient center/focal must be finite and radius must be positive"
                | _ -> ()

                gradient.InheritFrom |> Option.iter (requireDefinition "gradient" path)

                gradient.Stops
                |> List.iteri (fun index stop ->
                    if not (finite stop.Offset) || stop.Offset < 0.0 || stop.Offset > 1.0 then
                        add "invalid-gradient-stop" $"{path}/stops/{index}" "offset must be between zero and one"

                    if
                        not (finite stop.StopOpacity)
                        || stop.StopOpacity < 0.0
                        || stop.StopOpacity > 1.0
                    then
                        add "invalid-gradient-stop" $"{path}/stops/{index}" "opacity must be between zero and one")

                if gradient.Stops <> (gradient.Stops |> List.sortBy _.Offset) then
                    add "unordered-gradient-stops" (path + "/stops") "gradient stops must be ordered"
            | SvgDefinitionContent.Font font ->
                if
                    String.IsNullOrWhiteSpace font.Family
                    || String.IsNullOrWhiteSpace font.Source
                    || String.IsNullOrWhiteSpace font.Sha256
                    || String.IsNullOrWhiteSpace font.License
                then
                    add "invalid-font-reference" path "font family, source, sha256 and license are required"

                let safeFamily =
                    font.Family
                    |> Seq.forall (fun value -> Char.IsLetterOrDigit value || value = ' ' || value = '_' || value = '-')

                if not safeFamily then
                    add
                        "invalid-font-family"
                        (path + "/family")
                        "font family may contain only letters, digits, spaces, underscores and hyphens"
                ...
                if
                    source.StartsWith("/")
                    || source.Contains(":")
                    || (source.Split('/') |> Array.exists ((=) ".."))
                    || not safeSource
                    || not (source.EndsWith(".woff2", StringComparison.OrdinalIgnoreCase))
                then
        ...
        document.Children
        |> List.iteri (fun index child -> validateElement (childPath "" index) child)

        document.Definitions
        |> List.iteri (fun index definition -> validateDefinition $"/definitions/{index}" definition)
        ...
        let definitionRefs (definition: SvgDefinition) =
            let rec fromElements (elements: SvgElement list) =
                ...
                |> List.collect (fun (element: SvgElement) ->
                    ...
                    @ (match element.Content with
                       | SvgElementContent.Group children -> fromElements children
                       | _ -> []))
            ...
            | SvgDefinitionContent.Clip(_, shapes) ->
                ...
                |> List.collect (function
                    | SvgClipShape.Intersection ids -> ids
                    | _ -> [])
        ...
        let rec visit stack depth id =
            if depth > limits.MaxReferenceDepth then
                add "reference-depth" "/definitions" $"reference depth exceeds {limits.MaxReferenceDepth}: {id}"
                ...
                |> Option.iter (fun definition ->
                    definitionRefs definition |> List.iter (visit (id :: stack) (depth + 1)))
        ...
        if directNodes > limits.MaxNodes then
            add "node-limit" "/children" $"document exceeds {limits.MaxNodes} nodes"

        if pathSegments > limits.MaxPathSegments then
            add "path-segment-limit" "/children" $"document exceeds {limits.MaxPathSegments} path segments"
        ...
        let rec expanded stack depth (element: SvgElement) =
            if depth > limits.MaxReferenceDepth then
                ...
                | SvgElementContent.SymbolInstance(id, _) when not (List.contains id stack) ->
                    ...
                    | Some {
                               Content = SvgDefinitionContent.Symbol(_, children)
                           } -> 1 + (children |> List.sumBy (expanded (id :: stack) (depth + 1)))
                    | _ -> 1
                | _ -> 1

        let expandedNodes = document.Children |> List.sumBy (expanded [] 1)

        if expandedNodes > limits.MaxExpandedSymbolNodes then
            add "expanded-symbol-limit" "/children" $"expanded symbols exceed {limits.MaxExpandedSymbolNodes} nodes"
    ...
    type private TokenWriter() =
        ...
        member _.Token(value: string) =
            output.Append(value.Length).Append(':').Append(value) |> ignore

        member this.Int(value: int) =
    ...
    type private TokenReader(serialized: string) =
        ...
        member _.Token() =
            ...
            if separator < offset then
                failwith "missing token length separator"

            let lengthText = serialized.Substring(offset, separator - offset)
            let mutable length = 0

            if
                not (Int32.TryParse(lengthText, NumberStyles.None, CultureInfo.InvariantCulture, &length))
                || length < 0
            then
            ...
            if start + length > serialized.Length then
                failwith "truncated token"
        ...
        member this.Int() =
            ...
            if
                not (Int32.TryParse(this.Token(), NumberStyles.AllowLeadingSign, CultureInfo.InvariantCulture, &value))
            then
                failwith "invalid integer token"
        ...
        member this.Float() =
            ...
            if not (Double.TryParse(this.Token(), NumberStyles.Float, CultureInfo.InvariantCulture, &value)) then
                failwith "invalid number token"
        ...
        member this.Option(read: unit -> 'a) =
        ...
        member this.List(read: unit -> 'a) =
            ...
            if
                count < 0
                || count >
                    defaultLimits.MaxPathSegments
                    + defaultLimits.MaxNodes
                    + defaultLimits.MaxDefinitions
            then
    ...
    let private writeColor (writer: TokenWriter) (value: Color) =
        writer.Int(int value.Red)
        writer.Int(int value.Green)
        writer.Int(int value.Blue)
        writer.Int(int value.Alpha)
    ...
    let private readColor (reader: TokenReader) =
        let channel () =
            ...
            if value < 0 || value > 255 then
                failwith "color channel is outside byte range"
    ...
    let private writePoint (writer: TokenWriter) (value: Point) =
        writer.Float value.X
        writer.Float value.Y

    let private readPoint (reader: TokenReader) =
        {
            X = reader.Float()
            Y = reader.Float()
        }

    let private writeRect (writer: TokenWriter) (value: Rect) =
        writer.Float value.X
        writer.Float value.Y
        writer.Float value.Width
        writer.Float value.Height

    let private readRect (reader: TokenReader) =
        {
            X = reader.Float()
            Y = reader.Float()
            Width = reader.Float()
    ...
    let private writeAffine (writer: TokenWriter) (value: SvgAffine) =
        ...
        writer.Float value.C
        writer.Float value.D
        writer.Float value.E
        writer.Float value.F
    ...
    let private writeCap (writer: TokenWriter) =
        function
        | StrokeCap.Butt -> writer.Token "butt"
        | StrokeCap.Round -> writer.Token "round"
        | StrokeCap.Square -> writer.Token "square"

    let private readCap (reader: TokenReader) =
        match reader.Token() with
        | "butt" -> StrokeCap.Butt
        | "round" -> StrokeCap.Round
        | "square" -> StrokeCap.Square
        | _ -> failwith "unknown stroke cap"

    let private writeJoin (writer: TokenWriter) =
        function
        | StrokeJoin.Miter -> writer.Token "miter"
        | StrokeJoin.RoundJoin -> writer.Token "round"
        | StrokeJoin.Bevel -> writer.Token "bevel"

    let private readJoin (reader: TokenReader) =
        match reader.Token() with
        | "miter" -> StrokeJoin.Miter
        | "round" -> StrokeJoin.RoundJoin
        | "bevel" -> StrokeJoin.Bevel
        | _ -> failwith "unknown stroke join"

    let private writeStroke (writer: TokenWriter) (value: Stroke) =
        writer.Float value.Width
        writeCap writer value.Cap
        writeJoin writer value.Join
        writer.Float value.Miter

    let private readStroke (reader: TokenReader) =
        {
            Width = reader.Float()
            Cap = readCap reader
            Join = readJoin reader
            Miter = reader.Float()
        }

    let private writeShader (writer: TokenWriter) =
        function
        | Shader.SolidColor color ->
            writer.Token "solid"
            writeColor writer color
        | Shader.LinearGradient(startPoint, finish, colors) ->
            writer.Token "linear"
            writePoint writer startPoint
            writePoint writer finish
            writer.List(writeColor writer, colors)
    ...
    let private writePaint (writer: TokenWriter) (value: Paint) =
        ...
        | PathEffect.Dash(intervals, phase) ->
            writer.Token "dash"
            writer.List(writer.Float, intervals)
            writer.Float phase
    ...
    let private readPaint (reader: TokenReader) =
        ...
        if reader.Token() <> "src-over" then
            failwith "unsupported blend mode"
    ...
    let private writeFillType (writer: TokenWriter) =
        function
        | PathFillType.Winding -> writer.Token "nonzero"
        | PathFillType.EvenOdd -> writer.Token "evenodd"

    let private readFillType (reader: TokenReader) =
        match reader.Token() with
        | "nonzero" -> PathFillType.Winding
        | "evenodd" -> PathFillType.EvenOdd
        | _ -> failwith "unknown fill rule"
        ...
        writer.List(
            (fun command ->
                ...
                | PathCommand.CubicTo(first, second, point) ->
                    writer.Token "C"
                    writePoint writer first
                    writePoint writer second
                    writePoint writer point
                | PathCommand.ArcTo(bounds, startAngle, sweepAngle) ->
                    writer.Token "A"
                    writeRect writer bounds
                    writer.Float startAngle
                    writer.Float sweepAngle
                | PathCommand.Close -> writer.Token "Z"),
            value.Commands
        )
    ...
    let rec private writeScene (writer: TokenWriter) (scene: Scene) =
        writer.List(writeNode writer, scene.Nodes)

    and private writeNode (writer: TokenWriter) =
        function
        | SceneNode.Empty -> writer.Token "empty"
        | SceneNode.Group scenes ->
            writer.Token "group"
            writer.List(writeScene writer, scenes)
        ...
        | SceneNode.FilledEllipse(bounds, color) ->
            writer.Token "filled-ellipse"
            writeRect writer bounds
            writeColor writer color
        | SceneNode.Ellipse(bounds, paint) ->
            writer.Token "ellipse"
            writeRect writer bounds
            writePaint writer paint
        | SceneNode.Line(first, second, paint) ->
            writer.Token "line"
            writePoint writer first
            writePoint writer second
            writePaint writer paint
        ...
        | SceneNode.Text((x, y), text, color) ->
            writer.Token "text"
            writePoint writer { X = x; Y = y }
            writer.Token text
            writeColor writer color
        | SceneNode.TextRun run ->
            writer.Token "text-run"
            writer.Token run.Text
            writePoint writer run.Position
            writer.Option(writer.Token, run.Font.Family)
            writer.Float run.Font.Size
            writer.Option(writer.Int, run.Font.Weight)
        ...
        | SceneNode.ColorSpaceNode(ColorSpace.Srgb, child) ->
            writer.Token "srgb"
            writeScene writer child
        | SceneNode.Translate((x, y), child) ->
            writer.Token "translate"
            writer.Float x
            writer.Float y
            writeScene writer child
        | SceneNode.SizedText((x, y), text, size, color) ->
            writer.Token "sized-text"
            writePoint writer { X = x; Y = y }
            writer.Token text
            writer.Float size
            writeColor writer color
        | SceneNode.CachedSubtree cached ->
            writer.Token "cache"
            writer.Token(string cached.CacheId)
            writer.Token(string cached.Fingerprint)
            writeScene writer cached.Scene
        | _ -> failwith "unsupported Scene node reached serializer"

    let rec private readScene (reader: TokenReader) =
        {
            Nodes = reader.List(fun () -> readNode reader)
        }
    ...
    and private readNode (reader: TokenReader) =
        ...
        | "rect" ->
            let value = readRect reader in
            SceneNode.Rectangle((value.X, value.Y, value.Width, value.Height), readColor reader)
        ...
        | "text-run" ->
            ...
            let font =
                {
                    Family = reader.Option(reader.Token)
                    Size = reader.Float()
                    Weight = reader.Option(reader.Int)
                }
        ...
        | "clip" ->
            let clip =
                match reader.Token() with
                | "rect" -> Clip.RectClip(readRect reader)
                | "path" -> Clip.PathClip(readPath reader)
                | _ -> failwith "unknown clip"
        ...
        | "sized-text" ->
            let point = readPoint reader in
            SceneNode.SizedText((point.X, point.Y), reader.Token(), reader.Float(), readColor reader)
        | "cache" ->
            SceneNode.CachedSubtree
                {
                    CacheId = UInt64.Parse(reader.Token(), CultureInfo.InvariantCulture)
                    Fingerprint = UInt64.Parse(reader.Token(), CultureInfo.InvariantCulture)
                    Scene = readScene reader
                }
        | _ -> failwith "unknown Scene node"

    let private writePaintSource (writer: TokenWriter) =
        function
        | SvgPaintSource.Solid color ->
            writer.Token "solid"
            writeColor writer color
        | SvgPaintSource.Definition id ->
            writer.Token "definition"
            writer.Token id

    let private readPaintSource (reader: TokenReader) =
        match reader.Token() with
        | "solid" -> SvgPaintSource.Solid(readColor reader)
        | "definition" -> SvgPaintSource.Definition(reader.Token())
        | _ -> failwith "unknown paint source"

    let private writeUnits (writer: TokenWriter) =
        function
        | SvgCoordinateUnits.UserSpaceOnUse -> writer.Token "user"
        | SvgCoordinateUnits.ObjectBoundingBox -> writer.Token "bbox"

    let private readUnits (reader: TokenReader) =
        match reader.Token() with
        | "user" -> SvgCoordinateUnits.UserSpaceOnUse
        | "bbox" -> SvgCoordinateUnits.ObjectBoundingBox
        | _ -> failwith "unknown coordinate units"
    ...
    let private writePresentation (writer: TokenWriter) (value: SvgPresentation) =
        ...
        writer.Option(
            (fun (stroke: SvgStrokePresentation) ->
                writePaintSource writer stroke.Source
                writer.Float stroke.Width
                writeCap writer stroke.Cap
                writeJoin writer stroke.Join
                writer.Float stroke.Miter
                writer.List(writer.Float, stroke.Dash)
                writer.Float stroke.DashOffset),
            value.StrokeStyle
        )

        writer.Float value.OverallOpacity
        writeFillType writer value.FillRule
    ...
    let private readPresentation (reader: TokenReader) =
        ...
        let stroke =
            reader.Option(fun () ->
                {
                    Source = readPaintSource reader
                    Width = reader.Float()
                    Cap = readCap reader
                    Join = readJoin reader
                    Miter = reader.Float()
                    Dash = reader.List(reader.Float)
                    DashOffset = reader.Float()
                })

        {
            FillSource = fill
            StrokeStyle = stroke
            OverallOpacity = reader.Float()
            FillRule = readFillType reader
        }

    let rec private writeElement (writer: TokenWriter) (value: SvgElement) =
        writer.Token value.Id
        writer.Option(writer.Token, value.SemanticId)
        writer.Bool value.Visible
        writeAffine writer value.Transform
        writer.Option(writer.Token, value.ClipId)
        writer.Option(writer.Token, value.MaskId)
        writer.Option(writePresentation writer, value.Presentation)
        ...
        | SvgElementContent.Group children ->
            writer.Token "group"
            writer.List(writeElement writer, children)
        | SvgElementContent.SymbolInstance(id, viewport) ->
            writer.Token "symbol"
            writer.Token id
            writer.Option(writeRect writer, viewport)
    ...
    let private writeDefinition (writer: TokenWriter) (value: SvgDefinition) =
        ...
        | SvgDefinitionContent.Symbol(viewBox, children) ->
            writer.Token "symbol"
            writer.Option(writeRect writer, viewBox)
            writer.List(writeElement writer, children)
        | SvgDefinitionContent.Clip(units, shapes) ->
            ...
            writer.List(
                (fun shape ->
                    match shape with
                    | SvgClipShape.Rectangle rect ->
                        writer.Token "rect"
                        writeRect writer rect
                    | SvgClipShape.Path path ->
                        writer.Token "path"
                        writePath writer path
                    | SvgClipShape.Intersection ids ->
                        writer.Token "intersection"
                        writer.List(writer.Token, ids)),
                shapes
            )
        | SvgDefinitionContent.Mask(units, region, kind, children) ->
            writer.Token "mask"
            writeUnits writer units
            writeRect writer region

            writer.Token(
                match kind with
                | SvgMaskKind.Alpha -> "alpha"
                | SvgMaskKind.Luminance -> "luminance"
            )
        ...
        | SvgDefinitionContent.Gradient gradient ->
            ...
            match gradient.Geometry with
            | SvgGradientGeometry.Linear(first, second) ->
                writer.Token "linear"
                writePoint writer first
                writePoint writer second
            | SvgGradientGeometry.Radial(center, radius, focal) ->
                writer.Token "radial"
                writePoint writer center
                writer.Float radius
                writer.Option(writePoint writer, focal)

            writeUnits writer gradient.Units
            writeAffine writer gradient.Transform

            writer.Token(
                match gradient.Spread with
                | SvgSpreadMethod.Pad -> "pad"
                | SvgSpreadMethod.Repeat -> "repeat"
                | SvgSpreadMethod.Reflect -> "reflect"
            )

            writer.List(
                (fun stop ->
                    writer.Float stop.Offset
                    writeColor writer stop.Color
                    writer.Float stop.StopOpacity),
                gradient.Stops
            )
        ...
        | SvgDefinitionContent.Font font ->
            writer.Token "font"
            writer.Token font.Family
            writer.Token font.Source
            writer.Token font.Sha256
            writer.Token font.License
    ...
    let private readDefinition (reader: TokenReader) =
        ...
        let content =
            match reader.Token() with
            | "symbol" ->
                SvgDefinitionContent.Symbol(
                    reader.Option(fun () -> readRect reader),
                    reader.List(fun () -> readElement reader)
                )
            | "clip" ->
                ...
                let shapes =
                    reader.List(fun () ->
                        match reader.Token() with
                        | "rect" -> SvgClipShape.Rectangle(readRect reader)
                        | "path" -> SvgClipShape.Path(readPath reader)
                        | "intersection" -> SvgClipShape.Intersection(reader.List(reader.Token))
                        | _ -> failwith "unknown clip shape")
            ...
            | "mask" ->
                ...
                let kind =
                    match reader.Token() with
                    | "alpha" -> SvgMaskKind.Alpha
                    | "luminance" -> SvgMaskKind.Luminance
                    | _ -> failwith "unknown mask kind"

                SvgDefinitionContent.Mask(units, region, kind, reader.List(fun () -> readElement reader))
            | "gradient" ->
                let geometry =
                    match reader.Token() with
                    | "linear" -> SvgGradientGeometry.Linear(readPoint reader, readPoint reader)
                    | "radial" ->
                        SvgGradientGeometry.Radial(
                            readPoint reader,
                            reader.Float(),
                            reader.Option(fun () -> readPoint reader)
                        )
                    | _ -> failwith "unknown gradient geometry"
                ...
                SvgDefinitionContent.Gradient
                    {
                        ...
                        Spread = spread
                        Stops = stops
                        InheritFrom = reader.Option(reader.Token)
                    }
            | "font" ->
    ...
    let serialize (document: SvgDocument) =
        ...
        if not issues.IsEmpty then
            ...
            try
                ...
                writer.Token document.Schema
                writer.Token document.Id
                writeRect writer document.ViewBox
                writer.List(writeDefinition writer, document.Definitions)
                writer.List(writeElement writer, document.Children)
                ...
                let byteCount =
                    ...
                    while index < serialized.Length do
                        ...
                        if code <= 0x7f then
                            ...
                            && int serialized[index + 1] >= 0xdc00
                            && int serialized[index + 1] <= 0xdfff
                        then
                            count <- count + 4
                            index <- index + 1
                        else
                ...
                if sizeIssues.IsEmpty then
                    Ok serialized
                else
                    Error sizeIssues
            with error ->
                Error [ issue "unsupported-serialization" "/" error.Message ]
            ...
            Error
                [
                    issue
                        "document-byte-limit"
                        "/"
                        $"serialized document exceeds {defaultLimits.MaxSerializedBytes} bytes"
                ]
        else
            try
                let reader = TokenReader(serialized)

                if reader.Token() <> wireHeader then
                    failwith "unknown typed document wire header"

                let document =
                    {
                        Schema = reader.Token()
                        Id = reader.Token()
                        ViewBox = readRect reader
                        Definitions = reader.List(fun () -> readDefinition reader)
                        Children = reader.List(fun () -> readElement reader)
                    }

                if not reader.AtEnd then
                    failwith "trailing tokens after document"

                let issues = validate (utf8ByteCount serialized) defaultLimits document
                if issues.IsEmpty then Ok document else Error issues
            with error ->
                Error [ issue "invalid-serialization" "/" error.Message ]
    ...
    let private svgColor (value: Color) =
    ...
    let private pathData (path: PathSpec) =
        ...
        let append (command: string) =
            output.Append(command).Append(' ') |> ignore
        ...
        let closeEnough (first: Point) (second: Point) =
        ...
        let appendArc (bounds: Rect) startAngle sweepAngle =
            ...
            | Some value when not (closeEnough value startPoint) ->
                append $"L {svgNumber startPoint.X} {svgNumber startPoint.Y}"
            | _ -> ()
            ...
            for index in 1..pieces do
                ...
                append
                    $"A {svgNumber (abs bounds.Width / 2.0)} {svgNumber (abs bounds.Height / 2.0)} 0 {largeArc} {sweep} {svgNumber finish.X} {svgNumber finish.Y}"
        ...
        for command in path.Commands do
            match command with
            | PathCommand.MoveTo point ->
                append $"M {svgNumber point.X} {svgNumber point.Y}"
                current <- Some point
            | PathCommand.LineTo point ->
                append $"L {svgNumber point.X} {svgNumber point.Y}"
                current <- Some point
            | PathCommand.QuadTo(control, point) ->
                append $"Q {svgNumber control.X} {svgNumber control.Y} {svgNumber point.X} {svgNumber point.Y}"
                current <- Some point
            | PathCommand.CubicTo(first, second, point) ->
                append
                    $"C {svgNumber first.X} {svgNumber first.Y} {svgNumber second.X} {svgNumber second.Y} {svgNumber point.X} {svgNumber point.Y}"
    ...
    let exportSvg (mountNamespace: string) (document: SvgDocument) =
        let namespaceIssues =
            if String.IsNullOrWhiteSpace mountNamespace then
                [
                    issue "blank-mount-namespace" "/mountNamespace" "mount namespace must not be blank"
                ]
            else
                []
        ...
        if not issues.IsEmpty then
            ...
            try
                ...
                let beginTag (output: StringBuilder) (name: string) =
                    output.Append('<').Append(name) |> ignore

                let endOpen (output: StringBuilder) = output.Append('>') |> ignore

                let closeTag (output: StringBuilder) (name: string) =
                    output.Append("</").Append(name).Append('>') |> ignore

                let emptyTag (output: StringBuilder) = output.Append("/>") |> ignore

                let units =
                    function
                    | SvgCoordinateUnits.UserSpaceOnUse -> "userSpaceOnUse"
                    | SvgCoordinateUnits.ObjectBoundingBox -> "objectBoundingBox"
                ...
                let appendPaintSource output attribute opacityAttribute =
                    function
                    | SvgPaintSource.Solid color ->
                        appendAttribute output attribute (svgColor color)
                        appendAttribute output opacityAttribute (svgOpacity color)
                ...
                let appendPresentation output (presentation: SvgPresentation) =
                    ...
                    | Some stroke ->
                        ...
                        if not stroke.Dash.IsEmpty then
                            appendAttribute
                                output
                                "stroke-dasharray"
                                (stroke.Dash |> List.map svgNumber |> String.concat " ")

                        if stroke.DashOffset <> 0.0 then
                            appendAttribute output "stroke-dashoffset" (svgNumber stroke.DashOffset)
                    ...
                    appendAttribute
                        output
                        "fill-rule"
                        (match presentation.FillRule with
                ...
                let appendStops output stops =
                    ...
                    |> List.iter (fun stop ->
                        ...
                        appendAttribute
                            output
                            "stop-opacity"
                            (svgNumber (stop.StopOpacity * float stop.Color.Alpha / 255.0))
                ...
                let appendGeneratedGradient (paint: Paint) =
                    ...
                    | Some(Shader.LinearGradient(first, second, colors)) ->
                        ...
                        beginTag definitions "linearGradient"
                        appendAttribute definitions "id" (id local)
                        appendAttribute definitions "gradientUnits" "userSpaceOnUse"
                        appendAttribute definitions "color-interpolation" "sRGB"
                        appendAttribute definitions "x1" (svgNumber first.X)
                        appendAttribute definitions "y1" (svgNumber first.Y)
                        appendAttribute definitions "x2" (svgNumber second.X)
                        appendAttribute definitions "y2" (svgNumber second.Y)
                        endOpen definitions
                        let count = colors.Length

                        colors
                        |> List.iteri (fun index color ->
                    ...
                    | Some(Shader.RadialGradient(center, radius, colors)) ->
                        generatedGradient <- generatedGradient + 1
                        let local = $"generated-gradient-{generatedGradient}"
                        beginTag definitions "radialGradient"
                        appendAttribute definitions "id" (id local)
                        appendAttribute definitions "gradientUnits" "userSpaceOnUse"
                        appendAttribute definitions "color-interpolation" "sRGB"
                        appendAttribute definitions "cx" (svgNumber center.X)
                        appendAttribute definitions "cy" (svgNumber center.Y)
                        appendAttribute definitions "r" (svgNumber radius)
                        endOpen definitions
                        let count = colors.Length
                ...
                let appendPaint output isLine (paint: Paint) =
                    ...
                    | Some stroke ->
                        appendAttribute output "fill" "none"
                        setSource "stroke" "stroke-opacity"
                        appendAttribute output "stroke-width" (svgNumber stroke.Width)
                    ...
                    | None when isLine ->
                        ...
                        setSource "stroke" "stroke-opacity"
                    | None ->
                        setSource "fill" "fill-opacity"
                        appendAttribute output "stroke" "none"

                    match paint.PathEffect with
                    | PathEffect.Dash(intervals, phase) ->
                        appendAttribute output "stroke-dasharray" (intervals |> List.map svgNumber |> String.concat " ")
                        appendAttribute output "stroke-dashoffset" (svgNumber phase)
                    | _ -> ()
                ...
                let rec appendScene output prefix presentation (scene: Scene) =
                    scene.Nodes
                    |> List.iteri (fun index node -> appendNode output $"{prefix}-{index}" presentation node)

                and appendNode output prefix presentation node =
                    let paintAttrs isLine paint =
                        match presentation with
                        | Some value -> appendPresentation output value
                        | None -> appendPaint output isLine paint

                    match node with
                    | SceneNode.Empty -> ()
                    | SceneNode.Group scenes ->
                        beginTag output "g"
                        appendAttribute output "data-fsgg-node" prefix
                        endOpen output

                        scenes
                        |> List.iteri (fun index scene -> appendScene output $"{prefix}-g{index}" presentation scene)

                        closeTag output "g"
                    | SceneNode.Rectangle((x, y, width, height), color) ->
                        beginTag output "rect"
                        appendAttribute output "data-fsgg-node" prefix
                        appendAttribute output "x" (svgNumber x)
                        appendAttribute output "y" (svgNumber y)
                        appendAttribute output "width" (svgNumber width)
                        appendAttribute output "height" (svgNumber height)

                        (match presentation with
                         | Some value -> appendPresentation output value
                         | None -> appendPaintSource output "fill" "fill-opacity" (SvgPaintSource.Solid color))
                    ...
                    | SceneNode.PaintedRectangle(rect, paint) ->
                        ...
                        paintAttrs false paint
                        emptyTag output
                    | SceneNode.Circle(center, radius, color) ->
                        beginTag output "circle"
                        appendAttribute output "data-fsgg-node" prefix
                        appendAttribute output "cx" (svgNumber center.X)
                        appendAttribute output "cy" (svgNumber center.Y)
                        appendAttribute output "r" (svgNumber radius)
                    ...
                    | SceneNode.FilledEllipse(rect, color) ->
                        beginTag output "ellipse"
                        appendAttribute output "data-fsgg-node" prefix
                        appendAttribute output "cx" (svgNumber (rect.X + rect.Width / 2.0))
                        appendAttribute output "cy" (svgNumber (rect.Y + rect.Height / 2.0))
                        appendAttribute output "rx" (svgNumber (abs rect.Width / 2.0))
                        appendAttribute output "ry" (svgNumber (abs rect.Height / 2.0))
                    ...
                    | SceneNode.Ellipse(rect, paint) ->
                        ...
                        emptyTag output
                    | SceneNode.Line(first, second, paint) ->
                        beginTag output "line"
                        appendAttribute output "data-fsgg-node" prefix
                        appendAttribute output "x1" (svgNumber first.X)
                        appendAttribute output "y1" (svgNumber first.Y)
                        appendAttribute output "x2" (svgNumber second.X)
                        appendAttribute output "y2" (svgNumber second.Y)
                        paintAttrs true paint
                        emptyTag output
                    | SceneNode.Path(path, paint) ->
                        beginTag output "path"
                        appendAttribute output "data-fsgg-node" prefix
                        appendAttribute output "d" (pathData path)
                        // An element presentation owns the effective fill rule. Emitting the path's
                        // fallback as well creates duplicate XML attributes, which a real SVG/XML
                        // parser must reject even if an HTML parser happens to recover from them.
                    ...
                    | SceneNode.Arc(bounds, startAngle, sweepAngle, paint) ->
                        let path =
                            {
                                Commands = [ PathCommand.ArcTo(bounds, startAngle, sweepAngle) ]
                        ...
                        emptyTag output
                    | SceneNode.Text((x, y), text, color) ->
                        beginTag output "text"
                        appendAttribute output "data-fsgg-node" prefix
                        appendAttribute output "x" (svgNumber x)
                        appendAttribute output "y" (svgNumber y)
                        appendTextLayout output
                        ...
                        endOpen output
                        output.Append(xmlEscape text) |> ignore
                        closeTag output "text"
                    | SceneNode.TextRun run ->
                        beginTag output "text"
                        appendAttribute output "data-fsgg-node" prefix
                        appendAttribute output "x" (svgNumber run.Position.X)
                        appendAttribute output "y" (svgNumber run.Position.Y)
                        appendAttribute output "font-size" (svgNumber run.Font.Size)
                        appendTextLayout output
                        run.Font.Family |> Option.iter (appendAttribute output "font-family")
                        run.Font.Weight |> Option.iter (string >> appendAttribute output "font-weight")
                        paintAttrs false run.Paint
                        endOpen output
                        output.Append(xmlEscape run.Text) |> ignore
                        closeTag output "text"
                    | SceneNode.ClipNode(clip, child) ->
                        let clipLocal = $"generated-clip-{prefix}"
                        beginTag definitions "clipPath"
                        appendAttribute definitions "id" (id clipLocal)
                        appendAttribute definitions "clipPathUnits" "userSpaceOnUse"
                        endOpen definitions

                        match clip with
                        | Clip.RectClip rect ->
                            beginTag definitions "rect"
                            appendAttribute definitions "x" (svgNumber rect.X)
                            appendAttribute definitions "y" (svgNumber rect.Y)
                            appendAttribute definitions "width" (svgNumber rect.Width)
                            appendAttribute definitions "height" (svgNumber rect.Height)
                            emptyTag definitions
                        | Clip.PathClip path ->
                            beginTag definitions "path"
                            appendAttribute definitions "d" (pathData path)
                        ...
                        beginTag output "g"
                        appendAttribute output "clip-path" (url clipLocal)
                        endOpen output
                        appendScene output $"{prefix}-clip" presentation child
                        closeTag output "g"
                    | SceneNode.ColorSpaceNode(ColorSpace.Srgb, child) ->
                        appendScene output $"{prefix}-srgb" presentation child
                    | SceneNode.Translate((x, y), child) ->
                        beginTag output "g"
                        appendAttribute output "transform" $"translate({svgNumber x} {svgNumber y})"
                        endOpen output
                        appendScene output $"{prefix}-translate" presentation child
                        closeTag output "g"
                    | SceneNode.SizedText((x, y), text, size, color) ->
                        beginTag output "text"
                        appendAttribute output "data-fsgg-node" prefix
                        appendAttribute output "x" (svgNumber x)
                        appendAttribute output "y" (svgNumber y)
                        appendAttribute output "font-size" (svgNumber size)
                        appendTextLayout output
                ...
                let rec clipChain visited reference =
                    if Set.contains reference visited then
                        ...
                        match
                            document.Definitions
                            |> List.tryFind (fun definition -> definition.Id = reference)
                        with
                        | Some {
                                   Content = SvgDefinitionContent.Clip(_, shapes)
                               } ->
                            ...
                            let inherited =
                                ...
                                |> List.collect (function
                                    | SvgClipShape.Intersection references ->
                                        references |> List.collect (clipChain next)
                                    | _ -> [])

                            let hasLocalGeometry =
                                shapes
                                |> List.exists (function
                                    | SvgClipShape.Rectangle _
                                    | SvgClipShape.Path _ -> true
                                    | _ -> false)
                ...
                let rec appendElement output (element: SvgElement) =
                    ...
                    beginTag output "g"
                    appendAttribute output "id" (id element.Id)
                    appendAttribute output "data-fsgg-element-id" element.Id

                    element.SemanticId
                    |> Option.iter (appendAttribute output "data-fsgg-semantic-id")

                    appendAttribute output "transform" (svgMatrix element.Transform)

                    if not element.Visible then
                        appendAttribute output "display" "none"
                    ...
                    let clipCount =
                        ...
                        | Some clip ->
                            ...
                            |> List.iter (fun reference ->
                                beginTag output "g"
                                appendAttribute output "clip-path" (url reference)
                                endOpen output)
                    ...
                    | SvgElementContent.SymbolInstance(reference, viewport) ->
                        ...
                        let appendUse () =
                            beginTag output "use"
                            appendAttribute output "href" $"#{id reference}"
                            appendAttribute output "pointer-events" "all"
                        ...
                        | Some mask ->
                            beginTag output "g"
                            appendAttribute output "mask" (url mask)
                            endOpen output
                            appendUse ()
                            closeTag output "g"
                        | None -> appendUse ()

                    for _ in 1..clipCount do
                        closeTag output "g"
                ...
                let appendExplicitDefinition (definition: SvgDefinition) =
                    match definition.Content with
                    | SvgDefinitionContent.Symbol(viewBox, children) ->
                        beginTag definitions "symbol"
                        appendAttribute definitions "id" (id definition.Id)
                        ...
                        |> Option.iter (fun rect ->
                            appendAttribute
                                definitions
                                "viewBox"
                                $"{svgNumber rect.X} {svgNumber rect.Y} {svgNumber rect.Width} {svgNumber rect.Height}")

                        endOpen definitions
                        children |> List.iter (appendElement definitions)
                        closeTag definitions "symbol"
                    | SvgDefinitionContent.Clip(coordinateUnits, shapes) ->
                        beginTag definitions "clipPath"
                        appendAttribute definitions "id" (id definition.Id)
                        appendAttribute definitions "clipPathUnits" (units coordinateUnits)
                        endOpen definitions
                        ...
                        closeTag definitions "clipPath"
                    | SvgDefinitionContent.Mask(coordinateUnits, region, kind, children) ->
                        beginTag definitions "mask"
                        appendAttribute definitions "id" (id definition.Id)
                        appendAttribute definitions "maskUnits" (units coordinateUnits)
                        appendAttribute definitions "x" (svgNumber region.X)
                        appendAttribute definitions "y" (svgNumber region.Y)
                        appendAttribute definitions "width" (svgNumber region.Width)
                        appendAttribute definitions "height" (svgNumber region.Height)

                        appendAttribute
                            definitions
                            "style"
                            (match kind with
                             | SvgMaskKind.Alpha -> "mask-type:alpha"
                             | SvgMaskKind.Luminance -> "mask-type:luminance")

                        endOpen definitions
                        children |> List.iter (appendElement definitions)
                        closeTag definitions "mask"
                    | SvgDefinitionContent.Gradient gradient ->
                        let name =
                            match gradient.Geometry with
                            | SvgGradientGeometry.Linear _ -> "linearGradient"
                            | SvgGradientGeometry.Radial _ -> "radialGradient"

                        beginTag definitions name
                        appendAttribute definitions "id" (id definition.Id)
                        appendAttribute definitions "gradientUnits" (units gradient.Units)
                        appendAttribute definitions "gradientTransform" (svgMatrix gradient.Transform)
                        appendAttribute definitions "spreadMethod" (spread gradient.Spread)
                        appendAttribute definitions "color-interpolation" "sRGB"

                        gradient.InheritFrom
                        |> Option.iter (fun reference -> appendAttribute definitions "href" $"#{id reference}")
                        ...
                        | SvgGradientGeometry.Radial(center, radius, focal) ->
                            ...
                            |> Option.iter (fun point ->
                                appendAttribute definitions "fx" (svgNumber point.X)
                                appendAttribute definitions "fy" (svgNumber point.Y))

                        endOpen definitions
                        appendStops definitions gradient.Stops
                        closeTag definitions name
                    | SvgDefinitionContent.Font font ->
                        beginTag definitions "style"
                        appendAttribute definitions "data-fsgg-font-id" definition.Id
                        endOpen definitions

                        definitions
                            .Append("@font-face{font-family:&quot;")
                            .Append(xmlEscape font.Family)
                            .Append("&quot;;src:url(&quot;")
                            .Append(xmlEscape font.Source)
                            .Append("&quot;) format(&quot;woff2&quot;)}")
                        |> ignore
                ...
                let output = StringBuilder()
                beginTag output "svg"
                appendAttribute output "xmlns" "http://www.w3.org/2000/svg"
                appendAttribute output "id" (id "root")
                appendAttribute output "data-fsgg-document-id" document.Id
                appendAttribute output "data-fsgg-mount-namespace" mountNamespace

                appendAttribute
                    output
                    "viewBox"
                    $"{svgNumber document.ViewBox.X} {svgNumber document.ViewBox.Y} {svgNumber document.ViewBox.Width} {svgNumber document.ViewBox.Height}"

                endOpen output
                beginTag output "defs"
                endOpen output
                output.Append(definitions) |> ignore
                closeTag output "defs"
                output.Append(body) |> ignore
                closeTag output "svg"
                Ok(output.ToString())
            with error ->
                Error [ issue "unsupported-export" "/" error.Message ]
...
module SvgDocumentInteraction =
    let private validCamera camera =
        ...
        && match SvgAffine.tryInverse camera with
           | Ok _ -> true
           | Error _ -> false
    ...
    let rec private visibleSemanticIdsFromElement inheritedVisible (element: SvgElement) =
        ...
        [
            if visible then
                yield! element.SemanticId |> Option.toList
            match element.Content with
            | SvgElementContent.Group children ->
    ...
    let private visibleSemanticIds document =
        document.Children |> List.collect (visibleSemanticIdsFromElement true)
    ...
    let private moveFocus direction state =
        ...
        | _ ->
            let current =
                state.FocusedSemanticId
                |> Option.bind (fun id -> candidates |> List.tryFindIndex ((=) id))
            ...
            accept
                { state with
                    FocusedSemanticId = Some candidates[index]
                }
    ...
    let update message state =
        ...
        | SvgDocumentInteractionMessage.ReplaceDocument(expected, candidateRevision, candidate) ->
            requireRevision expected state (fun () ->
                if candidateRevision <= state.Revision then
                    ...
                    | Ok document ->
                        accept (
                            withDocument candidateRevision document (state.Document :: state.UndoDocuments) [] state
                        ))
        | SvgDocumentInteractionMessage.SelectSemantic(expected, semanticId) ->
            requireRevision expected state (fun () ->
                if retainedIdentity state.Document semanticId then
                    accept
                        { state with
                            SelectedSemanticId = Some semanticId
                            FocusedSemanticId = Some semanticId
                        }
                else
        ...
        | SvgDocumentInteractionMessage.FocusNext expected ->
            requireRevision expected state (fun () -> moveFocus 1 state)
        | SvgDocumentInteractionMessage.FocusPrevious expected ->
            requireRevision expected state (fun () -> moveFocus -1 state)
        | SvgDocumentInteractionMessage.SetCamera(expected, camera) ->
            requireRevision expected state (fun () ->
                if validCamera camera then
                    accept { state with Camera = camera }
                else
                    reject SvgDocumentInteractionError.InvalidCamera state)
        ...
        | SvgDocumentInteractionMessage.ReleasePointer(expected, pointerId) ->
            requireRevision expected state (fun () ->
                if state.CapturedPointerId = Some pointerId then
                    ...
                    reject (SvgDocumentInteractionError.PointerNotCaptured pointerId) state)
                ...
                | previous :: remaining ->
                    accept (
                        withDocument
                            ...
                            previous
                            remaining
                            (state.Document :: state.RedoDocuments)
                            state
                    ))
        | SvgDocumentInteractionMessage.Redo expected ->
            requireRevision expected state (fun () ->
                ...
                | next :: remaining ->
                    accept (
                        withDocument (state.Revision + 1) next (state.Document :: state.UndoDocuments) remaining state
                    ))
