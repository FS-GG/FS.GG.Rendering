        ...
        if request.OutputSize.Width <= 0 || request.OutputSize.Height <= 0 then
            ...
            Result.Error
                {
                    ...
                    Message =
                        $"Scene evidence renderer mode '{request.RendererMode}' is not available for non-window deterministic evidence."
                }
        else
            ...
            | Metadata ->
                ...
                let value =
                    match request.Format with
                    | Metadata ->
                        $"size={request.OutputSize.Width}x{request.OutputSize.Height};capabilities={readback.CapabilityCount};hash={readback.DeterministicHash}"
...
module LayoutEvidence =
    ...
    let classify (report: LayoutEvidenceReport) =
        ...
        if
            not report.UnsupportedReasons.IsEmpty
            || report.MeasurementMode = UnsupportedTextBounds
        then
            { report with
                ProofLevel = UnsupportedLayoutInspection
                OverlapStatus =
                    if overlaps.IsEmpty then
                        report.OverlapStatus
                    else
                        LayoutOverlaps overlaps
                Diagnostics =
                    report.Diagnostics
                    @ (report.UnsupportedReasons
                       |> List.map (fun reason -> $"{reason.Fact}: {reason.Reason}"))
