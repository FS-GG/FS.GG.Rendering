...
module private ArtCommon =
    ...
    let rec mapElement id (mapper: SvgElement -> SvgElement) (element: SvgElement) : SvgElement =
        let content =
            ...
            | SvgElementContent.Group children -> SvgElementContent.Group(List.map (mapElement id mapper) children)
            | value -> value
    ...
    let contains id (document: SvgDocument) =
        let rec find (element: SvgElement) =
            element.Id = id
            || match element.Content with
               | SvgElementContent.Group children -> List.exists find children
               | _ -> false

        List.exists find document.Children

    let transformIds (ids: string list) affine (document: SvgDocument) =
        if ids.IsEmpty || not (SvgAffine.isFinite affine) then
            Error(SvgArtError.InvalidSelection "a finite transform and at least one element are required")
        else
            ...
            | None ->
                ...
                let rec map (element: SvgElement) : SvgElement =
                    ...
                    if selected.Contains value.Id then
                        { value with
                            Transform = SvgAffine.compose affine value.Transform
                        }
                    else
                        value
                ...
                validate
                    { document with
                        Children = List.map map document.Children
                    }
    ...
    let pathBounds (path: PathSpec) =
        let points =
            ...
            |> List.collect (function
                ...
                | PathCommand.QuadTo(c, p) -> [ c; p ]
                | PathCommand.CubicTo(a, b, p) -> [ a; b; p ]
                | PathCommand.ArcTo(r, _, _) ->
                    [
                        { X = r.X; Y = r.Y }
                        {
                            X = r.X + r.Width
        ...
        | first :: rest ->
            let minX, maxX, minY, maxY =
                rest
                |> List.fold
                    (fun (a, b, c, d) p -> min a p.X, max b p.X, min c p.Y, max d p.Y)
    ...
    let rec bounds (element: SvgElement) : Rect option =
        let local =
            ...
            | SvgElementContent.SceneLeaf {
                                              Nodes = [ SceneNode.PaintedRectangle(rect, _) ]
                                          }
            | SvgElementContent.SceneLeaf {
                                              Nodes = [ SceneNode.Ellipse(rect, _) ]
                                          } -> Some rect
        ...
        |> Option.map (fun r ->
            { r with
                X = r.X + element.Transform.E
                Y = r.Y + element.Transform.F
            })
...
module SvgArt =
    ...
    let snapPoint (state: SvgArtState) (point: Point) =
        if not (pointFinite point) || not (finite state.GridSize) || state.GridSize < 0.01 then
            Error(
                SvgArtError.InvalidInput
                    [
                        issue
                            "invalid-grid"
                            "/tool/gridSize"
                            "grid size and point must be finite; grid size must be at least 0.01"
                    ]
            )
        elif not state.SnapToGrid then
            Ok point
        else
            let snap v =
                (if v >= 0.0 then
                     Math.Floor(v / state.GridSize + 0.5)
                 else
                     Math.Ceiling(v / state.GridSize - 0.5))
                * state.GridSize in

            Ok { X = snap point.X; Y = snap point.Y }

    let guides state (points: Point list) =
        points
        |> List.map (snapPoint state)
        |> List.fold
            (fun acc value ->
                match acc, value with
                | Ok values, Ok p -> Ok({ Axis = "x"; Position = p.X } :: { Axis = "y"; Position = p.Y } :: values)
                | Error e, _
                | _, Error e -> Error e)
            (Ok [])
        |> Result.map (List.distinct >> List.sortBy (fun g -> g.Axis, g.Position))

    let create elementId primitive presentation (document: SvgDocument) =
        if String.IsNullOrWhiteSpace elementId || contains elementId document then
            Error(SvgArtError.InvalidSelection "element identity must be nonblank and unique")
        else
            let content =
                match primitive with
                | SvgArtPrimitive.Rectangle rect ->
                    SvgElementContent.SceneLeaf
                        {
                            Nodes = [ SceneNode.PaintedRectangle(rect, defaultPaint) ]
                        }
                | SvgArtPrimitive.Ellipse rect ->
                    SvgElementContent.SceneLeaf
                        {
                            Nodes = [ SceneNode.Ellipse(rect, defaultPaint) ]
                        }
                | SvgArtPrimitive.Polygon points ->
                    SvgElementContent.SceneLeaf
                        {
                            Nodes =
                                [
                                    SceneNode.Path(
                                        {
                                            Commands =
                                                (points
                                                 |> List.mapi (fun i p ->
                                                     if i = 0 then PathCommand.MoveTo p else PathCommand.LineTo p))
                                                @ [ PathCommand.Close ]
                                            FillType = presentation.FillRule
                                        },
                                        defaultPaint
                                    )
                                ]
                        }
                | SvgArtPrimitive.Path path ->
                    SvgElementContent.SceneLeaf
                        {
                            Nodes = [ SceneNode.Path(path, defaultPaint) ]
                        }

            let element: SvgElement =
                {
                    Id = elementId
                    SemanticId = Some elementId
                    Visible = true
            ...
            validate
                { document with
                    Children = document.Children @ [ element ]
                }

    let replacePath elementId path (document: SvgDocument) =
        if not (contains elementId document) then
            Error(SvgArtError.MissingElement elementId)
        else
            document.Children
            |> List.map (
                mapElement elementId (fun e ->
                    match e.Content with
                    | SvgElementContent.SceneLeaf { Nodes = [ SceneNode.Path(_, paint) ] } ->
                        { e with
                            Content =
                                SvgElementContent.SceneLeaf
                                    {
                                        Nodes = [ SceneNode.Path(path, paint) ]
                                    }
                        }
                    | _ -> e)
            )
            |> fun children -> validate { document with Children = children }

    let insertPathPoint elementId index point (document: SvgDocument) =
        if index < 0 || not (pointFinite point) then
            Error(SvgArtError.InvalidSelection "path index and point must be valid")
        else
            let edit (path: PathSpec) =
                if index > path.Commands.Length then
                    None
                else
                    Some
                        { path with
                            Commands = List.insertAt index (PathCommand.LineTo point) path.Commands
                        }
                    ...
                    | Some value ->
                        ...
                        { element with
                            Content =
                                SvgElementContent.SceneLeaf
                                    {
                                        Nodes = [ SceneNode.Path(value, paint) ]
                                    }
                        }
                    | None -> element
                | _ -> element
            ...
            let result =
                { document with
                    Children = List.map (mapElement elementId mapper) document.Children
                }
    ...
    let removePathPoint elementId index (document: SvgDocument) =
        if index < 0 then
            Error(SvgArtError.InvalidSelection "path index must be non-negative")
        else
            let mutable changed = false
            ...
            let mapper (element: SvgElement) =
                ...
                | SvgElementContent.SceneLeaf {
                                                  Nodes = [ SceneNode.Path(path, paint) ]
                                              } when index < path.Commands.Length ->
                    changed <- true
                            ...
                            SvgElementContent.SceneLeaf
                                {
                                    Nodes =
                                        [
                                            SceneNode.Path(
                                                { path with
                                                    Commands = List.removeAt index path.Commands
                                                },
                                                paint
                                            )
                                        ]
    ...
    let translate ids x y document =
        transformIds ids (SvgAffine.translate x y) document

    let rotate ids degrees document =
        transformIds ids (SvgAffine.rotateDegrees degrees) document

    let scale ids x y document =
        transformIds ids (SvgAffine.scale x y) document

    let group groupId (ids: string list) (document: SvgDocument) =
        if String.IsNullOrWhiteSpace groupId || contains groupId document || ids.IsEmpty then
            Error(SvgArtError.InvalidSelection "group identity and selection are invalid")
        else
            let selected = Set.ofList ids

            let children =
                document.Children |> List.filter (fun (e: SvgElement) -> selected.Contains e.Id)

            if children.Length <> selected.Count then
                Error(SvgArtError.InvalidSelection "grouping currently requires distinct top-level elements")
            else
                let group: SvgElement =
                    {
                        Id = groupId
                        SemanticId = Some groupId
                        Visible = true
                        Transform = SvgAffine.identity
                        ClipId = None
                        MaskId = None
                ...
                document.Children
                |> List.filter (fun e -> not (selected.Contains e.Id))
                |> List.insertAt first group
                |> fun value -> validate { document with Children = value }
    ...
    let ungroup groupId document =
        ...
        | Some index ->
            ...
            | SvgElementContent.Group children ->
                let value =
                    document.Children |> List.removeAt index |> List.insertManyAt index children
    ...
    let align alignment ids document =
        ...
        let found =
            document.Children
            |> List.filter (fun (e: SvgElement) -> selected.Contains e.Id)
            |> List.choose (fun e -> bounds e |> Option.map (fun b -> e.Id, b))

        if found.Length <> selected.Count || found.Length < 2 then
            Error(SvgArtError.InvalidSelection "alignment requires two bounded top-level elements")
        else
            let target =
                match alignment with
                | SvgArtAlignment.Left -> found |> List.minBy (fun (_, b) -> b.X) |> snd |> fun b -> b.X
                | SvgArtAlignment.Right ->
                    found
                    |> List.maxBy (fun (_, b) -> b.X + b.Width)
                    |> snd
                    |> fun b -> b.X + b.Width
                | SvgArtAlignment.Top -> found |> List.minBy (fun (_, b) -> b.Y) |> snd |> fun b -> b.Y
                | SvgArtAlignment.Bottom ->
                    found
                    |> List.maxBy (fun (_, b) -> b.Y + b.Height)
                    |> snd
                    |> fun b -> b.Y + b.Height
            ...
            let children =
                ...
                |> List.map (fun element ->
                    ...
                    | Some delta ->
                        match alignment with
                        | SvgArtAlignment.Left
                        | SvgArtAlignment.Right
                        | SvgArtAlignment.HorizontalCenter ->
                            { element with
                                Transform = SvgAffine.compose (SvgAffine.translate delta 0.0) element.Transform
                            }
                        | _ ->
                            { element with
                                Transform = SvgAffine.compose (SvgAffine.translate 0.0 delta) element.Transform
                            })
    ...
    let reorder id order document =
        ...
        | Some index ->
            ...
            validate
                { document with
                    Children = List.insertAt target item rest
                }

    let setPresentation ids presentation document =
        let selected = Set.ofList ids

        if ids.IsEmpty || ids |> List.exists (fun id -> not (contains id document)) then
            Error(SvgArtError.InvalidSelection "presentation selection is empty or missing")
        else
            let rec map (e: SvgElement) : SvgElement =
                let content =
                    match e.Content with
                    | SvgElementContent.Group children -> SvgElementContent.Group(List.map map children)
                    | v -> v in

                let value = { e with Content = content } in

                if selected.Contains e.Id then
                    { value with
                        Presentation = Some presentation
                    }
                else
    ...
    let putGradient id gradient document =
        if String.IsNullOrWhiteSpace id then
            Error(SvgArtError.InvalidSelection "gradient identity must not be blank")
        else
            let value: SvgDefinition =
                {
                    Id = id
            ...
            validate
                { document with
                    Definitions = value :: (document.Definitions |> List.filter (fun d -> d.Id <> id))
                }
...
module SvgGeometry =
    ...
    let flatten deviation (path: PathSpec) =
        if
            ...
            |> List.exists (function
                | PathCommand.ArcTo _ -> true
                | _ -> false)
        then
            Error "arc contours require an explicit path conversion before Boolean geometry"
        else
            ...
            let rec quad depth p0 c p1 =
                if depth >= maximumSubdivisionDepth || distanceLine c p0 p1 <= deviation then
                    output.Add p1
                else
                    let a = midpoint p0 c
                    let b = midpoint c p1
            ...
            let rec cubic depth p0 c1 c2 p1 =
                if
                    ...
                    || max (distanceLine c1 p0 p1) (distanceLine c2 p0 p1) <= deviation
                then
                    output.Add p1
                else
            ...
            for command in path.Commands do
                ...
                | PathCommand.MoveTo p, _ ->
                    output.Add p
                    current <- Some p
                    start <- Some p
                | PathCommand.LineTo p, Some _ ->
                    output.Add p
                    current <- Some p
                | PathCommand.QuadTo(c, p), Some p0 ->
                    quad 0 p0 c p
                    current <- Some p
                | PathCommand.CubicTo(a, b, p), Some p0 ->
                    cubic 0 p0 a b p
                    current <- Some p
                | PathCommand.Close, Some _ ->
                    ...
                    start
                    |> Option.iter (fun p ->
                        if output[output.Count - 1] <> p then
                            output.Add p)

                    current <- start
                | _ -> ()

            if not closed then Error "open contours are unsupported"
            elif output.Count < 4 then Error "degenerate contour"
            else Ok(List.ofSeq output)
    ...
    let encode (request: SvgGeometryRequest) =
        let points (contours: Point list list) =
            contours
            |> List.map (fun ps ->
                "["
                + (ps
                   |> List.map (fun p -> "[" + num p.X + "," + num p.Y + "]")
                   |> String.concat ",")
                + "]")
            |> String.concat ","
        ...
        then
            Error(
                SvgArtError.InvalidInput
                    [
                        issue
                            "invalid-geometry-request"
                            "/geometry"
                            "operation identity, revision and deviation (0.01–1.0) are required"
                    ]
            )
        elif (subjects @ clips) |> List.exists (fun p -> p.FillType <> PathFillType.Winding) then
            Error(
                SvgArtError.InvalidInput
                    [
                        issue "unsupported-fill-rule" "/geometry" "polygon-clipping interchange requires winding fill"
                    ]
            )
        else
            ...
            | None ->
                ...
                if values |> List.exists selfIntersects then
                    Error(
                        SvgArtError.InvalidInput
                            [
                                issue
                                    ...
                                    "/geometry"
                                    "self-intersecting contours are unsupported"
                            ]
                    )
                elif values.Length > 128 || count > 10000 then
                    Error(
                        SvgArtError.InvalidInput
                            [
                                issue "geometry-budget" "/geometry" "input exceeds 128 contours or 10,000 vertices"
                            ]
                    )
                else
                    ...
                    let contourSource (contours: Point list list) =
                        contours
                        |> List.map (fun contour ->
                            contour
                            |> List.map (fun (point: Point) -> num point.X + "," + num point.Y)
                            |> String.concat ";")
                        |> String.concat "|"
                    ...
                    let source =
                        ...
                        + num deviation
                        + "#"
                        + contourSource subjectValues
                        + "#"
                        + contourSource clipValues
                    ...
                    if utf8ByteCount encoded > 1048576 then
                        Error(
                            SvgArtError.InvalidInput
                                [ issue "geometry-byte-limit" "/geometry" "encoded request exceeds 1 MiB" ]
                        )
                    else
                        Ok
                            {
                                ...
                                InputVertexCount = count
                            }

    let transaction result prepared document =
        if
            result.OperationId <> prepared.Request.OperationId
            || result.AcceptedRevision <> prepared.Request.AcceptedRevision
            || result.InputContentHash <> prepared.Request.InputContentHash
        then
            Error(
                SvgArtError.InvalidInput
                    [
                        issue
                            "stale-geometry-result"
                            "/geometry"
                            "worker result identity does not match the accepted request"
                    ]
            )
        elif
            result.Contours.Length > 128
            || (result.Contours |> List.sumBy List.length) > 20000
        then
            Error(
                SvgArtError.InvalidInput
                    [
                        issue "geometry-result-budget" "/geometry" "result exceeds 128 contours or 20,000 vertices"
                    ]
            )
        elif
            result.Contours
            |> List.exists (fun contour ->
                contour.Length < 4
                || contour.Head <> contour[contour.Length - 1]
                || (contour |> List.exists (pointFinite >> not)))
        then
            Error(
                SvgArtError.InvalidInput
                    [
                        issue
                            "invalid-geometry-result"
                            "/geometry"
                            "worker returned an open, degenerate or nonfinite contour"
                    ]
            )
        else
            let commands =
                ...
                |> List.collect (fun c ->
                    c
                    |> List.mapi (fun i p -> if i = 0 then PathCommand.MoveTo p else PathCommand.LineTo p)
                    |> fun cs -> cs @ [ PathCommand.Close ])
