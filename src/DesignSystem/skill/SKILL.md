...
- `src/DesignSystem/DesignTokens.fs` (the flat primitive module) — regenerated by the **same**
  `dotnet fsi scripts/generate-design-tokens.fsx` as above (it writes all three files), and its
  `--check` covers this one too. Currency is enforced by `Feature126TokenTaxonomyTests`, which shells
  out to that `--check` and fails on drift.
