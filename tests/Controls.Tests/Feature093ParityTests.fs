...
let private frozenCheckboxGeom (theme: Theme) (on: bool) (label: string) : Scene list =
    ...
    let fill =
        if on then
            [ Scene.rectangle (bx, by, s, s) theme.Accent ]
        else
            [ Scene.rectangleWithPaint boxRect (Paint.stroke theme.Foreground 2.0) ]
        ...
        if on then
            [
                Scene.line
                    { X = bx + 6.0; Y = by + 15.0 }
                    { X = bx + 12.0; Y = by + 21.0 }
                    (Paint.stroke theme.Background 3.0)
                Scene.line
                    { X = bx + 12.0; Y = by + 21.0 }
                    { X = bx + 23.0; Y = by + 7.0 }
                    (Paint.stroke theme.Background 3.0)
    ...
    let text =
        [ mkText theme (bx + s + theme.SpaceSm) (cy + 5.0) 13.0 theme.Foreground label ]
...
let private parityBaselines () : (string * string) list =
    [
        for (tname, theme) in themes do
            ...
            sprintf "check-box-checked.%s.normal.scene.txt" tname,
            sprintf "%A" (frozenCheckboxGeom theme true "Enabled")
    ]

[<Tests>]
let feature093ParityTests =
    testList
        "Feature 093 migration parity (SC-003/SC-007)"
        [
            ...
            test "T020 — the pre-refactor procedural baselines are committed goldens the frozen oracle still matches" {
                ...
                for (name, content) in parityBaselines () do
                    ...
                    Expect.equal
                        ...
                        (sprintf "frozen-oracle %s matches its committed golden" name)
            }
            ...
            test "Button no-class paint is structurally-Scene-equal to the procedural baseline, both themes (SC-003)" {
                for (tname, theme) in themes do
                    ...
                    Expect.equal
                        actual
                        (frozenButtonGeom theme "Save")
                        (sprintf "button.%s no-class render matches the procedural baseline" tname)
            }
            ...
            test "CheckBox (unchecked) no-class paint matches the procedural baseline, both themes (SC-003)" {
                for (tname, theme) in themes do
                    ...
                    Expect.equal
                        actual
                        (frozenCheckboxGeom theme false "Enabled")
                        (sprintf "check-box.%s unchecked render matches baseline" tname)
            }
            ...
            test "CheckBox (checked) no-class paint matches the procedural baseline, both themes (SC-003)" {
                for (tname, theme) in themes do
                    ...
                    Expect.equal
                        actual
                        (frozenCheckboxGeom theme true "Enabled")
                        (sprintf "check-box.%s checked render matches baseline" tname)
            }
            ...
            test "an unmigrated kind ignores attached style classes — no render delta (SC-007)" {
                ...
                let classed =
                    ProgressBar.create [ ProgressBar.value 0.5; Attr.styleClasses [ Variant StyleVariant.Danger ] ]
            ...
            test "a MIGRATED kind DOES respond to an attached class (US1 vertical slice / additive proof)" {
                ...
                let danger =
                    Button.create [ Button.text "Save"; Attr.styleClasses [ Variant StyleVariant.Danger ] ]
