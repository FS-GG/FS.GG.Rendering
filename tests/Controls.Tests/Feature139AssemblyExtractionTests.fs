                        ...
                        [
                            ...
                            Stack.children
                                [
                                    ...
                                    raw
                                        "stack"
                                        [ Attr.width 260.0; Attr.height 36.0 ]
                                        [ TextBlock.create [ TextBlock.text "wide clipped child" ] ]
                                    Overlay.create
                                        [ Overlay.child (TextBlock.create [ TextBlock.text "floating overlay" ]) ]
...
let private assertRetainedMatches (name: string) (tree: Control<Msg>) =
    ...
    Expect.equal
        (visibleShape inited.Render)
        (visibleShape full)
        (name + ": retained init visible scene equals immediate renderTree")

    Expect.equal
        (visibleShape warm.Render)
        (visibleShape full)
        (name + ": warm retained visible scene equals immediate renderTree")
...
let directAssemblyContract =
    testList
        "Feature139 direct current-node assembly contract"
        [
            test "leaf and box-less nodes compose flat" {
                ...
                let boxless =
                    ControlInternals.assembleCurrentNode plain None own [ assembly child [] ]

                Expect.equal boxless.InFlowScene (own @ child) "a box-less node composes own and child scenes flat"

                Expect.isEmpty
                    (rectClipRects (Scene.group boxless.InFlowScene))
                    "box-less composition introduces no clip"
            }
            ...
            test "children are clipped to a node box when a box and child scene are present" {
                ...
                let result =
                    ControlInternals.assembleCurrentNode plain (Some box) own [ assembly child [] ]
            ...
            test "overlay nodes promote composed own and child in-flow paint to the overlay group" {
                ...
                let result =
                    ControlInternals.assembleCurrentNode overlay (Some box) own [ assembly child deeperOverlay ]

                Expect.isEmpty result.InFlowScene "an overlay node contributes no in-flow scene"

                Expect.equal
                    result.OverlayScene
                    (ControlInternals.composeContainerScene (Some box) own child @ deeperOverlay)
                    "the overlay contribution preserves composed subtree before deeper overlays"
            }
        ]
...
let parityAndCompatibility =
    testList
        "Feature139 immediate-retained parity and compatibility"
        [
            test "nested clipping, offsets, cache boundaries, overlays, and warm reuse stay byte-identical" {
                ...
                Expect.equal
                    (visibleShape inited.Render)
                    (visibleShape full)
                    "retained init matches immediate renderTree"

                Expect.equal
                    (visibleShape warm.Render)
                    (visibleShape full)
                    "warm retained step matches immediate renderTree through transparent cache wrappers"

                Expect.isNonEmpty (rectClipRects full.Scene) "the fixture exercises container clipping"

                Expect.equal
                    warm.WorkReduction.RecomputedNodeCount
                    0
                    "an idle warm step does not repaint an unchanged tree"

                Expect.equal
                    warm.WorkReduction.RemeasuredNodeCount
                    0
                    "an idle warm step does not remeasure an unchanged tree"

                Expect.isLessThan
                    warm.WorkReduction.RecomputedNodeCount
                    warm.WorkReduction.BaselineNodeCount
                    "warm reuse avoids a full-tree paint pass"

                Expect.isTrue
                    (warm.WorkReduction.PictureCacheHits > 0)
                    "the fixture exercises retained picture cache boundaries"
            }
            ...
            test "empty, overlay-free, overlay, clipped, bounds, diagnostics, events, and bound ids remain compatible" {
                ...
                assertRetainedMatches
                    "overlay-free"
                    (Stack.create
                        [
                            Stack.children
                                [
                                    TextBlock.create [ TextBlock.text "plain" ]
                                    Button.create [ Button.text "Go"; Button.onClick Clicked ]
                                ]
                        ])
...
let ownershipAndArchitecture =
    testList
        "Feature139 assembly ownership evidence"
        [
            test "retained rendering does not reimplement container clipping plus overlay splitting" {
                ...
                Expect.isFalse
                    (retained.Contains("composeContainerScene"))
                    "RetainedRender must not call composeContainerScene directly"

                Expect.isFalse
                    (retained.Contains("isOverlayNode"))
                    "RetainedRender must not branch on overlay nodes directly"

                Expect.isFalse
                    (retained.Contains("composeRetainedScenes"))
                    "the old retained-local assembly owner must stay removed"

                Expect.isTrue
                    (retained.Contains("assembleRetainedNode"))
                    "retained build sites route through the local adapter to assembleCurrentNode"

                Expect.isTrue
                    (retained.Contains("ControlInternals.assembleCurrentNode"))
                    "retained emit/build code calls the shared assembly seam"
            }
            ...
            test "the shared owner and scope fence are documented for later phases" {
                ...
                Expect.isTrue
                    (control.Contains("let assembleCurrentNode"))
                    "ControlInternals owns the current-node assembly seam"
                ...
                for term in forbidden do
                    Expect.isTrue
                        ...
                        (sprintf "quickstart records the later-phase exclusion: %s" term)
            }
        ]
