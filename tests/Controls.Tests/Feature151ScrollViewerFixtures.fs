...
let row index =
    TextBlock.create [ Attr.height 20.0; TextBlock.text (sprintf "row %02d" index) ]

let rows count = [ for index in 1..count -> row index ]

let textBlock text width height =
    TextBlock.create [ Attr.width width; Attr.height height; TextBlock.text text ]

let stack children =
    Stack.create [ Stack.children children ]

let scrollViewer id children =
    Control.create "scroll-viewer" [ Attr.children [ stack children ] ]
    |> Control.withKey id
        ...
        [
            Stack.children
                [
                    ...
                    Overlay.create
                        [
                            Overlay.child (TextBlock.create [ TextBlock.text "layer" ])
                            Attr.selected true
                        ]
...
let cases =
    [
        ...
        }
        {
            ...
            Control =
                scrollViewer
                    "sv-text-natural-size"
                    [ textBlock "natural text" 180.0 28.0; textBlock "second line" 180.0 28.0 ]
            ExpectedVerticalOverflow = false
            DynamicControl = None
        }
        {
