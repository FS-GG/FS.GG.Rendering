...
let tests =
    testList
        "Feature150ScrollViewerExtent"
        [
            test "overflowing content reports intrinsic content height and vertical max offset" {
                let rendered =
                    Feature150ScrollFixtures.render (
                        Feature150ScrollFixtures.scrollViewer "sv" (Feature150ScrollFixtures.rows 20)
                    )
            ...
            test "small content normalizes extent to viewport and reports no unnecessary overflow" {
                let small =
                    TextBlock.create [ Attr.width 40.0; Attr.height 20.0; TextBlock.text "small" ]

                let rendered =
                    Feature150ScrollFixtures.render (
                        Control.create "scroll-viewer" [ Attr.children [ small ] ]
                        |> Control.withKey "sv"
                    )
