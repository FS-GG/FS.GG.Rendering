...
let private validDispositions =
    set [ "existing"; "net-new"; "composition"; "not-applicable" ]
...
let private forbiddenDependency =
    Regex("antv|/g2|/g6|/l7|react|d3-|charting", RegexOptions.IgnoreCase)

[<Tests>]
let feature133ChartCoverageMatrixTests =
    testList
        "Feature 133 chart coverage-matrix honesty check (SC-001/SC-002) + no-dep guard (SC-006)"
        [
            ...
            test "every row has a valid, non-blank disposition (H2, H6)" {
                for r in parseRows () do
                    Expect.isTrue
                        ...
                        (sprintf
                            "%s disposition '%s' is one of existing/net-new/composition/not-applicable"
                            r.Chart
                            r.Disposition)
            ...
            test "every covered row references only live Catalog control ids (H3, SC-002)" {
                for r in
                    parseRows ()
                    |> List.filter (fun r -> coveredDispositions.Contains r.Disposition) do
                    ...
                    Expect.isNonEmpty
                        r.RepoControls
                        (sprintf "%s (%s) names at least one repo control" r.Chart r.Disposition)
            }
                ...
                for r in
                    ...
                    for tok in r.TokenEntries do
                        Expect.isTrue
                            ...
                            (sprintf
                                "%s row references token entry '%s' (must exist in the DesignSystem public surface)"
                                r.Chart
                                tok)

                    Expect.isNonEmpty
                        r.TokenEntries
                        (sprintf "%s (%s) names at least one token entry" r.Chart r.Disposition)
            }

            test "composition and not-applicable rows carry a non-empty rationale (H5)" {
                for r in
                    parseRows ()
                    |> List.filter (fun r -> r.Disposition = "composition" || r.Disposition = "not-applicable") do
                    Expect.isTrue
            ...
            test "the foot summary totals reconcile with the pinned snapshot size; every bucket used" {
                ...
                Expect.equal
                    (List.length rows)
                    (List.length pinnedCharts)
                    "matrix totals reconcile with the snapshot list size"
            ...
            test "no AntV/React/JS charting or geospatial dependency leaks into the chart sources (US5, FR-008/SC-006)" {
                for f in chartSourceFiles do
                    ...
                    Expect.isFalse
                        m.Success
                        (sprintf
                            "%s adds no charting/geospatial dependency (matched '%s')"
                            (Path.GetFileName f)
