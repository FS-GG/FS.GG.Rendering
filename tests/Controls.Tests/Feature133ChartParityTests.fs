...
let feature133ChartParityTests =
    testList
        "Feature 133 chart parity (FR-002/FR-006/FR-007, SC-004/SC-007)"
        [
            ...
            test "existing charts' Default render is deterministic and theme-identity-independent (FR-002/SC-004)" {
                ...
                for (id, c) in existingCharts do
                    ...
                    Expect.equal
                        ...
                        (paint defaultLight c)
                        (sprintf "%s Default paint is deterministic" id)
            ...
            test
                "at least one resolved visual property diverges between Default and AntDesign across the chart tree (FR-006)" {
                let divergent =
                    fullSample
                ...
                Expect.isTrue
                    (List.contains "line-chart" divergent)
                    "line-chart (brand-accent series) diverges under AntDesign"
