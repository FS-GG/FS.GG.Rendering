...
let tests =
    testList
        "Feature 113 memo seam (US1, FR-001/FR-004/FR-005, C1–C3)"
        [
            ...
            test "a stable dependency HITS: reuses the stored subtree instance, thunk NOT run (C1)" {
                ...
                Expect.isTrue
                    (System.Object.ReferenceEquals(firstSubtree, secondSubtree))
                    "the hit returns the SAME subtree instance (reference-reused)"
            }
            ...
            test "the cache is per ControlId: a never-seen id misses even with an equal dependency" {
                let _, cache1, _ =
                    RetainedRender.memoize "gridA" ("a" :> obj) (fun () -> scene 1.0) Map.empty

                let _, _, outcome =
                    RetainedRender.memoize "gridB" ("a" :> obj) (fun () -> scene 2.0) cache1
