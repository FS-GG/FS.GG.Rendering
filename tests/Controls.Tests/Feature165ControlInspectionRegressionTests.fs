    ...
    testList
        ...
        [
            test "inspection does not change renderTree scene bounds diagnostics event bindings bound ids or node count" {
                let tree: Control<Msg> =
                    Stack.create
                        [
                            Stack.children
                                [
                                    Button.create [ Button.text "Save"; Button.onClick Clicked ]
                ...
                Expect.equal
                    (after.EventBindings |> List.map (fun b -> b.ControlId, b.EventKind))
                    (before.EventBindings |> List.map (fun b -> b.ControlId, b.EventKind))
                    "event binding keys unchanged"
