    ...
    testList
        ...
        [
            test "inspect emits stable ids final bounds ownership ordering text and clip facts from renderTree" {
                let tree: Control<Msg> =
                    Stack.create
                        [
                            Stack.children
                                [
                                    TextBlock.create [ TextBlock.text "Inspection title" ]
                ...
                Expect.exists
                    artifact.Nodes
                    (fun n -> n.NodeId = "0" && n.Kind = VisualInspectionNodeKind.Root)
                    "root node id"

                Expect.exists
                    artifact.Nodes
                    (fun n -> n.NodeId = "title" && n.OwnerId = Some "title")
                    "authored key is owner"

                Expect.exists artifact.Nodes (fun n -> n.NodeId = "save" && n.Bounds.IsSome) "final bounds are present"

                Expect.equal
                    (artifact.Nodes |> List.map _.ZOrder)
                    (artifact.Nodes |> List.map _.ZOrder |> List.sort)
                    "visual order is deterministic"

                Expect.exists
                    artifact.TextRuns
                    (fun t -> t.OwnerNodeId = "title" && t.Text = "Inspection title")
                    "text facts are emitted"

                Expect.exists
                    artifact.ClipFacts
                    (fun c -> c.NodeId = "0" && c.ClipStatus = VisualInspectionClipStatus.Intentional)
                    "container clip fact"
            ...
            test "long text gets a deterministic fit classification against owner bounds" {
                let tree: Control<Msg> =
                    TextBlock.create
                        [
                            Attr.width 80.0
                            TextBlock.text "A very very very long deterministic inspection label"
                        ]
                    |> Control.withKey "title"
                ...
                Expect.equal
                    text.MeasurementMode
                    VisualInspectionMeasurementMode.Approximate
                    "adapter discloses approximate text measurement"
