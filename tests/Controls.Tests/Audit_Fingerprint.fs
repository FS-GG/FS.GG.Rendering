...
let tests =
    testList
        "Audit: Scene fingerprint determinism + collision probe (FR-007)"
        [

            // ---- T004 scaffold sanity ----
            test "Audit: Fingerprint scaffold reachability — hashScene + counters (T004)" {
                let h =
                    RetainedRender.hashScene (sceneOf [ Rectangle((0.0, 0.0, 10.0, 10.0), blue) ])
                ...
                let s =
                    RetainedRender.step theme size (RetainedRender.init theme size view).Retained view
            ...
            test "Audit: identical scenes hash identically + deterministic across calls (FR-007)" {
                let a =
                    sceneOf
                        [
                            Rectangle((0.0, 0.0, 10.0, 10.0), blue)
                            SceneNode.Text((1.0, 2.0), "hello", red)
                        ]
                ...
                Expect.equal
                    (RetainedRender.hashScene a)
                    (RetainedRender.hashScene b)
                    "structurally-equal scenes ⇒ equal fingerprint (the notEqual probes are meaningful)"
            ...
            test "Audit: Feature141 assembly-result fingerprints are owner-produced and deterministic" {
                ...
                let childAssembly = ControlInternals.assembleCurrentNode control (Some box) child []

                let assembledA =
                    ControlInternals.assembleCurrentNode control (Some box) own [ childAssembly ]

                let assembledB =
                    ControlInternals.assembleCurrentNode control (Some box) own [ childAssembly ]

                Expect.equal
                    assembledA.Fingerprint
                    assembledB.Fingerprint
                    "equivalent owner assemblies produce stable fingerprints"

                Expect.equal
                    assembledA.InFlowFingerprint
                    assembledB.InFlowFingerprint
                    "equivalent owner assemblies produce stable in-flow fingerprints"

                Expect.equal
                    assembledA.OverlayFingerprint
                    assembledB.OverlayFingerprint
                    "equivalent owner assemblies produce stable overlay fingerprints"

                Expect.equal
                    (RetainedRender.hashScene assembledA.InFlowScene)
                    (ControlInternals.hashScene assembledA.InFlowScene)
                    "RetainedRender.hashScene aliases the owner-side fingerprint"

                Expect.equal assembledA.ChildContributions.Length 1 "assembly metadata records child contribution count"

                Expect.equal
                    assembledA.ChildContributions.Head.InFlowFingerprint
                    childAssembly.InFlowFingerprint
                    "child contribution stores child in-flow fingerprint"

                Expect.equal
                    assembledA.ChildContributions.Head.OverlayFingerprint
                    childAssembly.OverlayFingerprint
                    "child contribution stores child overlay fingerprint"
            }
            ...
            test "Audit: COLLISION PROBE — any single render-affecting change flips the fingerprint (FR-007)" {
                ...
                Expect.notEqual
                    h0
                    (RetainedRender.hashScene (sceneOf [ Rectangle((0.0, 0.0, 11.0, 10.0), blue) ]))
                    "geometry change flips the fingerprint"
                // color
                Expect.notEqual
                    h0
                    (RetainedRender.hashScene (sceneOf [ Rectangle((0.0, 0.0, 10.0, 10.0), red) ]))
                    "color change flips the fingerprint"
                // text content
                let t0 = RetainedRender.hashScene (sceneOf [ SceneNode.Text((0.0, 0.0), "a", red) ])

                Expect.notEqual
                    t0
                    (RetainedRender.hashScene (sceneOf [ SceneNode.Text((0.0, 0.0), "b", red) ]))
                    "text change flips the fingerprint"
                // opacity (alpha)
                let cFade = { blue with Alpha = 128uy }

                Expect.notEqual
                    h0
                    (RetainedRender.hashScene (sceneOf [ Rectangle((0.0, 0.0, 10.0, 10.0), cFade) ]))
                    "opacity change flips the fingerprint"
                // transform (translate)
                let inner =
                    {
                        Nodes = [ Rectangle((0.0, 0.0, 10.0, 10.0), blue) ]
                    }
            ...
            test "Audit: COLLISION PROBE (FsCheck) — distinct rectangle widths never collide over >=500 cases (FR-007)" {
                ...
                let noCollision (a: float, b: float) =
                    (a <> b)
                    ==> lazy
                        (RetainedRender.hashScene (sceneOf [ Rectangle((0.0, 0.0, a, 10.0), blue) ])
