...
let feature232IdSchemeTests =
    testList
        "Feature232 unify control-id schemes onto Key ?? path"
        [
            ...
            test "US1 T007: two unkeyed same-kind focusable siblings get DISTINCT path stop ids" {
                ...
                Expect.equal
                    stopIds
                    [ "0.0"; "0.1" ]
                    "unkeyed same-kind siblings key by their distinct positional paths (was both \"button\")"
            ...
            test "US1 T007: each focus-stop id equals the unified id boundIdsOf mints for that node" {
                ...
                let stopIds =
                    (Focus.order tree).Stops |> List.map (fun s -> s.Control) |> Set.ofList
                ...
                Expect.equal
                    stopIds
                    boundIds
                    "Focus.order and eventBindings/boundIds agree on the unified Key ?? path id"
            }
            ...
            test "US2 T013: hover keyed by path stamps ONLY the matching unkeyed sibling" {
                ...
                let hovered =
                    { emptyModel with
                        HoveredControl = Some "0.1"
                    }

                let bridged = ControlRuntime.applyRuntimeVisualState hovered tree

                Expect.equal
                    (stateOf bridged.Children.[1])
                    Hover
                    "the pointer-resolved (path \"0.1\") unkeyed sibling is stamped Hover"

                Expect.equal
                    (stateOf bridged.Children.[0])
                    Normal
                    "its unkeyed same-kind sibling stays Normal (no smear)"

                Expect.equal
                    (dump bridged.Children.[0])
                    (dump (btn ()))
                    "the un-hovered sibling is byte-identical to its un-bridged form"
            }
            ...
            test "US2 T013: a Normal-and-unset tree is byte-identical through the bridge (at rest)" {
                ...
                Expect.equal
                    (dump (ControlRuntime.applyRuntimeVisualState emptyModel tree))
                    (dump tree)
                    "at rest the bridge adds no attribute anywhere"
            ...
            test "US2 T015: applyScrollOffsets shifts an UNKEYED scroll-viewer via its path id" {
                let content =
                    Stack.create
                        [
                            ...
                            Stack.children [ Button.create [ Button.text "row" ] ]
                        ]
                ...
                let tree: Control<int> =
                    Stack.create
                        [
                            Stack.children
                                [
                                    Control.create "scroll-viewer" [ Attr.children [ content ]; Attr.height 40.0 ]
                                ]
                        ]

                let scroll =
                    ScrollState.empty
                    |> ScrollState.withExtent 400.0 40.0
                    |> ScrollState.applyScrollDelta 25.0

                let model =
                    { emptyModel with
                        ScrollOffsets = Map.ofList [ "0.0", scroll ]
                    } // "0.0" = the scroll-viewer's path
                ...
                Expect.notEqual
                    (dump stamped)
                    (dump tree)
                    "the unkeyed scroll-viewer received its path-keyed scroll offset"
            }
            ...
            test "US3 T021: DatePicker keys its trigger with the declared triggerId (overlay anchor resolves)" {
                ...
                Expect.isTrue
                    (anyNode (fun c -> c.Key = Some "d-trigger") dp)
                    "a real lowered control carries the declared trigger id \"d-trigger\""
            }
            ...
            test "US3 T021: SplitButton keys its trigger with the declared triggerId" {
                ...
                Expect.isTrue
                    (anyNode (fun c -> c.Key = Some "s-trigger") sb)
                    "a real lowered control carries the declared trigger id \"s-trigger\""
            }
