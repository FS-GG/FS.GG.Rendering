...
let tests =
    testList
        "Feature143 overlay state contract"
        [
            ...
            test "open emits ordered open focus and consume effects" {
                ...
                Expect.equal
                    effects
                    [
                        RequestOpenStateChange("menu", true)
                        RequestFocus(Some "menu-item-1")
                        ConsumeInput
                    ]
                    "effect order is stable"
            }

            test "invalid messages become diagnostics instead of exceptions" {
                let state, effects =
                    OverlayState.update
                        (DismissRequested(Some "missing", DismissalReason.Escape))
                        (OverlayState.init ())

                Expect.isEmpty state.OpenSurfaces "invalid dismiss does not create state"

                Expect.exists
                    (OverlayState.diagnostics state)
                    (fun d -> d.Code = InvalidOverlayMessage)
                    "invalid message diagnostic"

                Expect.exists
                    effects
                    (function
                    | ReportOverlayDiagnostic d when d.Code = InvalidOverlayMessage -> true
                    | _ -> false)
