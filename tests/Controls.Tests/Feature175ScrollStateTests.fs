    ...
    testList
        ...
        [
            ...
            test "thumb height derives from the viewport/content ratio" {
                ...
                Expect.floatClose
                    Accuracy.high
                    (ScrollState.thumbHeight s)
                    (400.0 * 400.0 / 1000.0)
                    "thumb height = viewport * viewport/content"
            }
            ...
            test "thumb position tracks the offset monotonically across the track" {
                ...
                let atBottom =
                    ScrollState.thumbPosition
                        track
                        { s with
                            Offset = ScrollState.maxOffset s
                        }
...
let offsetAwareHitTestTests =
    let row key =
        Button.create [ Button.text key; Attr.width 120.0; Attr.height 40.0 ]
        |> Control.withKey key
    ...
    let scrollViewer (offset: float) =
        ...
        Control.create
            ...
            [
                ...
                if offset > 0.0 then
                    yield Attr.create AttrKeys.ScrollOffset Layout (FloatValue offset)
            ]
        |> Control.withKey "sv"
            ...
            test "scrolling by delta makes screen point y resolve to the content at y+delta" {
                ...
                for y in [ 30.0; 50.0; 70.0 ] do
                    ...
                    Expect.equal
                        scrolled
                        unscrolledLower
                        (sprintf "at y=%g, scrolled hit == unscrolled hit at y+delta" y)
            }
    ...
    testList
        ...
        [
            ...
            test "Home/End emit a large signed delta (clamped to top/bottom downstream)" {
                ...
                Expect.equal
                    (ScrollState.applyScrollDelta endd s).Offset
                    (ScrollState.maxOffset s)
                    "End clamps to bottom"
            }
