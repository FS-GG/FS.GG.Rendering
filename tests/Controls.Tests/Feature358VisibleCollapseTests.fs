...
let private boundOf (result: ControlRenderResult<unit>) (id: string) : Rect option =
    result.Bounds
    |> List.tryPick (fun (nodeId, rect) -> if nodeId = id then Some rect else None)

let private boundIds (result: ControlRenderResult<unit>) : string list = result.Bounds |> List.map fst
    ...
    testList
        ...
        [
            ...
            test "a hidden leaf contributes NO bounds and NO scene; its visible sibling is unaffected" {
                let tree = stackOf [ leaf "a" [] "ALPHA"; leaf "b" [ Attr.visible false ] "SECRET" ]
            ...
            test "a hidden control reserves NO layout space — a following sibling does not shift" {
                let hiddenLeading =
                    stackOf [ leaf "h" [ Attr.visible false ] "SECRET"; leaf "a" [] "ALPHA" ]

                let removedLeading = stackOf [ leaf "a" [] "ALPHA" ]

                let visibleLeading = stackOf [ leaf "h" [] "HEADER"; leaf "a" [] "ALPHA" ]
                ...
                Expect.floatClose
                    ...
                    yHidden
                    yRemoved
                    "a hidden leading sibling reserves no space: 'a' sits where it would with the sibling removed entirely"

                Expect.isTrue
                    (yVisible > yHidden + 0.5)
                    "sanity: a VISIBLE leading sibling DOES push 'a' down, so the test above is load-bearing"
            }
            ...
            test "a hidden container hides its WHOLE subtree — no bounds, no scene for its descendants" {
                let tree =
                    stackOf
                        [
                            leaf "a" [] "ALPHA"
                            Stack.create [ Attr.visible false; Stack.children [ leaf "deep" [] "BURIED" ] ]
                            |> Control.withKey "box"
                        ]
            ...
            test "the retained path agrees with renderTree for a hidden control (init parity)" {
                ...
                Expect.equal
                    inited.Render.Bounds
                    full.Bounds
                    "retained init bounds equal renderTree bounds (hidden 'b' absent from both)"

                Expect.equal
                    (sceneTexts inited.Render)
                    (sceneTexts full)
                    "retained init scene text equals renderTree (neither paints 'SECRET')"

                Expect.isFalse
                    (textPainted inited.Render "SECRET")
                    "the retained path paints nothing for the hidden control"
            }
            ...
            test "toggling visible re-measures incrementally (the AttrCategory.Layout channel)" {
                let visibleTree =
                    stackOf [ leaf "a" [] "ALPHA"; leaf "b" [ Attr.visible true ] "BETA" ]

                let hiddenTree =
                    stackOf [ leaf "a" [] "ALPHA"; leaf "b" [ Attr.visible false ] "BETA" ]
                ...
                Expect.equal
                    toHidden.Render.Bounds
                    (render hiddenTree).Bounds
                    "stepping visible->hidden collapses 'b' (matches a full render of the hidden tree)"
                ...
                Expect.equal
                    toVisible.Render.Bounds
                    (render visibleTree).Bounds
                    "stepping hidden->visible restores 'b' (matches a full render of the visible tree)"
