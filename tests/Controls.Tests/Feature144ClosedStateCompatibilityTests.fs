...
let tests =
    testList
        "Feature144 closed-state compatibility"
        [
            test "closed metadata does not create open overlay state" {
                let closed =
                    metadata TransientSurfaceKind.DatePickerCalendar "closed-calendar" 60 false true false
            ...
            test "closed metadata validates without current-frame anchor bounds" {
                let closed =
                    metadata TransientSurfaceKind.ColorPickerPalette "closed-palette" 70 false true false
            ...
            test "reset closes all product-owned surfaces through explicit requests" {
                let opened, _ =
                    openOne (metadata TransientSurfaceKind.DialogModal "modal-reset" 100 true true true)
                ...
                Expect.exists
                    effects
                    (function
                    | RequestOpenStateChange("modal-reset", false) -> true
                    | _ -> false)
                    "close is product-visible"
            }
        ]
