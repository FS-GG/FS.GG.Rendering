...
let private fileNameOf (p: string) =
    match Path.GetFileName p with
    | null -> p
    | n -> n

let private dirOf (p: string) =
...
let private allRefDocs =
    patternDocs
    @ templateDocs
    @ (Option.toList readmeDoc)
    @ (Option.toList skillDoc)

let private allowedPrefixes =
    set [ "control"; "token"; "resolver"; "policy"; "doc"; "part" ]
...
let private flatRefs =
    ...
    |> List.collect (fun d ->
        ...
        |> List.choose (fun line ->
            ...
            if idx > 0 then
                Some(d, line.Substring(0, idx), line.Substring(idx + 1))
            else
                None))
...
let private controlIds =
    Catalog.supportedControls |> List.map (fun c -> c.Id) |> Set.ofList
...
let private tokenNames: Set<string> =
    ...
    let rec walk (t: Type) (prefix: string) =
        for p in t.GetProperties staticFlags do
            acc.Add(prefix + p.Name) |> ignore

        for f in t.GetFields staticFlags do
            acc.Add(prefix + f.Name) |> ignore

        for n in t.GetNestedTypes BindingFlags.Public do
    ...
    for rootName in [ "DesignTokensExt"; "DesignTokens" ] do
        ...
        | null -> ()
        | root ->
            for n in root.GetNestedTypes BindingFlags.Public do
                walk n (n.Name + ".")
...
let private templateNames =
    set [ "workbench"; "list"; "detail"; "form"; "result"; "exception" ]
    ...
    testList
        ...
        [
            test "Family_coverage_is_bijective" {
                ...
                let dups =
                    families |> List.countBy id |> List.filter (fun (_, n) -> n > 1) |> List.map fst
            ...
            test "All_resolver_refs_resolve" {
                for (d, _, v) in refsOf "resolver" do
                    Expect.isTrue
                        (resolverMembers.Contains v)
                        (sprintf "%s: resolver ref '%s' is not a public StyleResolver member" d.Name v)
            }

            test "All_token_refs_resolve" {
                for (d, _, v) in refsOf "token" do
                    Expect.isTrue
                        (tokenNames.Contains v)
                        (sprintf "%s: token ref '%s' does not resolve on a public DesignSystem token type" d.Name v)
            }
            ...
            test "All_policy_refs_resolve" {
                for (d, _, v) in refsOf "policy" do
                    Expect.isTrue
                        (policyResolves v)
                        (sprintf "%s: policy ref '%s' is not accepted by ColorPolicy.byName" d.Name v)
            }
                ...
                for d in patternDocs do
                    ...
                    for required in [ "control"; "token"; "resolver"; "policy" ] do
                        Expect.isTrue
                            ...
                            (sprintf "%s: pattern doc needs >=1 '%s:' ref" d.Name required)
            }
                ...
                for d in patternDocs do
                    let parts =
                        ...
                        |> List.choose (fun line ->
                            ...
                            if idx > 0 && line.Substring(0, idx) = "part" then
                                Some(line.Substring(idx + 1))
                            else
                                None)

                    Expect.isNonEmpty
                        parts
                        (sprintf "%s: pattern doc needs >=1 'part:<Component>/<partName>' ref (FR-011)" d.Name)
                    ...
                    for value in parts do
                        ...
                        Expect.equal
                            segs.Length
                            2
                            (sprintf "%s: part ref '%s' must be <Component>/<partName> (exactly one '/')" d.Name value)

                        if segs.Length = 2 then
                            Expect.isTrue
                                (segs.[0].Length > 0)
                                (sprintf "%s: part ref '%s' has empty Ant component" d.Name value)

                            Expect.isTrue
                                (segs.[1].Length > 0)
                                (sprintf "%s: part ref '%s' has empty part name" d.Name value)
                    ...
                    for required in [ "control"; "token"; "resolver" ] do
                        Expect.isTrue
                            (ps.Contains required)
                            (sprintf "%s: a doc with part: refs must also carry a '%s:' ref" d.Name required)
            }
            ...
            test "Pattern_docs_state_design_language_only" {
                for d in patternDocs do
                    Expect.stringContains
                        d.Text
                        designLanguageMarker
                        (sprintf "%s: missing the design-language-only assertion line" d.Name)
            }
            ...
            test "Skill_is_advisory_and_reminds_layering" {
                ...
                | Some d ->
                    Expect.equal
                        (Map.tryFind "name" d.Front)
                        (Some "fs-gg-ant-design")
                        "skill front-matter name must be fs-gg-ant-design"
                    ...
                    for phrase in gatePhrases do
                        Expect.isFalse
                            ...
                            (sprintf "skill must stay advisory — found gating phrase '%s'" phrase)
            }
                    ...
                    for line in d.RefLines do
                        ...
                        if idx > 0 then
                            ...
                            Expect.isTrue
                                ...
                                (sprintf "%s: unknown ref prefix in '%s'" d.Name line)

                            Expect.notEqual (line.[idx - 1]) ' ' (sprintf "%s: space before ':' in '%s'" d.Name line)

                            Expect.isTrue
                                (value.Length > 0 && value.[0] <> ' ')
                                (sprintf "%s: empty value or space after ':' in '%s'" d.Name line)
            }
        ]
