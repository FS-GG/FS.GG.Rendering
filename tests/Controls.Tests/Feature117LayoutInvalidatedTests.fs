...
let private rinit (c: Control<'msg>) =
...
let tests =
    testList
        "Feature 117 layout-invalidated count + style-only zero-work (US2/US3, FR-006/007/008)"
        [
            ...
            test "a style-only frame: zero invalidated, zero re-measured (FR-006/FR-007, SC-003)" {
                ...
                Expect.equal
                    s.WorkReduction.LayoutInvalidatedNodeCount
                    0
                    "a style-only change invalidates no layout node"

                Expect.equal s.WorkReduction.RemeasuredNodeCount 0 "a style-only change re-measures no node"
            }

            test
                "a style-only / visual-state-only frame over warm text produces ZERO text-cache misses (FR-007, SC-003)" {
                ...
                Expect.equal
                    s.WorkReduction.TextMeasureCacheMisses
                    0
                    "style-only over unchanged text: ZERO text-cache misses (all served warm)"

                Expect.isTrue
                    (s.WorkReduction.TextMeasureCacheHits > 0)
                    "the unchanged text is served from the warm cache (hits)"
            }
            ...
            test "a geometry frame: bounded invalidated <= re-measured, both > 0 (FR-006, SC-006)" {
                ...
                Expect.isTrue
                    (invalidated <= remeasured)
                    (sprintf
                        "invalidated %d <= re-measured %d (propagation expands the pre-pinning set)"
                        invalidated
                        remeasured)
                ...
                Expect.isTrue
                    (invalidated <= total)
                    (sprintf "invalidated %d is bounded by the total node count %d" invalidated total)
            }
