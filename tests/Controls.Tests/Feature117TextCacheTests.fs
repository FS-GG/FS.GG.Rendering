...
let tests =
    testList
        ...
        [
            ...
            test
                "a cold key is a miss; the identical key is a hit with byte-identical metrics (FR-001/SC-002→SC-001/FR-004)" {
            ...
            test "perturbing EXACTLY ONE keyed input independently forces a miss (FR-002)" {
                ...
                let differs (text: string) (f: FontSpec) (why: string) =
                    ...
                    Expect.equal
                        ...
                        (sprintf "the %s miss returns the correct fresh metrics" why)
            ...
            test "empty and whitespace text cache without error and stay byte-identical (edge case)" {
                for t in [ ""; "   "; "\t" ] do
                    ...
                    Expect.equal
                        m
                        (Scene.measureText t font)
                        "whitespace/empty measure is byte-identical to the un-cached value"
            ...
            test "a fitted-caption's distinct candidate sizes are distinct keys (edge case)" {
                ...
                let cacheAfterSweep =
                    ...
                    |> List.fold
                        (fun c s ->
                            let _, c', hit =
                                RetainedRender.measureTextCached c true caption { font with Size = s }
                ...
                for s in sizes do
                    let _, _, hit =
                        RetainedRender.measureTextCached cacheAfterSweep true caption { font with Size = s }
            ...
            test "the always-miss oracle yields a byte-identical SCENE and layout (cache-on ≡ cache-off, FR-004/SC-004)" {
                ...
                Expect.equal
                    off.WorkReduction.RemeasuredNodeCount
                    on.WorkReduction.RemeasuredNodeCount
                    "remeasure count is unaffected by the cache"
                ...
                Expect.equal
                    on.Render.Scene
                    (Control.renderTree theme size v1).Scene
                    "cache-on scene equals a fresh full rebuild"
            }
            ...
            test "a cold retained frame with repeated text reports no same-frame duplicate hits (Feature138)" {
                ...
                Expect.equal
                    cold.WorkReduction.TextMeasureCacheHits
                    0
                    "cold same-frame duplicate reuse is not a metric hit"

                Expect.isTrue
                    (cold.WorkReduction.TextMeasureCacheMisses > 0)
                    "the first cold measurements are still recorded as misses"

                let warm = RetainedRender.step theme size cold.Retained (repeatedTextToggle 6 2)

                Expect.isTrue
                    (warm.WorkReduction.TextMeasureCacheHits > 0)
                    "the next frame reports prior-frame reuse as hits"
