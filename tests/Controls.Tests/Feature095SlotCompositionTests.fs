...
let private frozenButtonGeom (t: Theme) (label: string) : Scene list =
    ...
    // #384: the base font tracks `theme.FontSize` (was a frozen 15.0).
    ...
    [
        ...
        mkText t (box.X + t.SpaceMd) (by + h / 2.0 + 5.0) t.FontSize t.Background label
    ]
    ...
    testList
        ...
        [
            test "filling Button.Leading injects the fill into the lowered Children and consumes the carrier" {
                ...
                Expect.equal
                    (button.Children |> List.map (fun c -> c.Key))
                    [ Some "ico" ]
                    "the leading fill is the lowered Button's child"
            ...
            test "filling Leading AND Trailing lands in two DISTINCT regions, ordered, no collision/swap" {
                let button =
                    Button.view
                        { Button.defaults with
                            Text = "Go"
                            Leading = Some(label "L" "lead")
                            Trailing = Some(label "T" "trail")
                        }
                    |> Widget.toControl
                ...
                Expect.equal
                    (button.Children |> List.map (fun c -> c.Key))
                    [ Some "L"; Some "T" ]
                    "leading precedes trailing (distinct ordered regions)"
            }
            ...
            test "Panel.Header lands BEFORE content, Panel.Footer AFTER (composite container regions)" {
                ...
                Expect.equal
                    (panel.Children |> List.map (fun c -> c.Key))
                    [ Some "H"; Some "B"; Some "F" ]
                    "header, then content, then footer"
            }
            ...
            test "slotFor resolves a present name and distinguishes ABSENT from EMPTY (edge case)" {
                ...
                Expect.isSome
                    (ControlInternals.slotFor "leading" attrs)
                    "a present name resolves even when its content is empty"

                Expect.isNone
                    (ControlInternals.slotFor "trailing" attrs)
                    "an absent name resolves to None (unfilled => default)"
            }
        ]
...
module private Gen095 =
    let private kinds =
        [ "button", [ "leading"; "trailing" ]; "panel", [ "header"; "footer" ] ]
    ...
    let control: Gen<Control<int>> =
        gen {
            ...
            let baseChildren =
                if kind = "panel" then
                    [ Widget.toControl (label "body" "content") ]
                else
                    []

            let attrs =
                [
                    if not (List.isEmpty fills) then
                        ControlInternals.slotFill fills
                ]
        ...
        [
            test "the slot carrier is internal plumbing — no public SlotName / Attr.slot escape hatch" {
                ...
                Expect.all
                    button.Children
                    (fun c -> c.Kind <> "")
                    "every slot fill is a concrete Control value, not a deferred binding"
    ...
    testList
        ...
        [
            test "an unfilled Button carries no slot attribute and no peripheral children" {
                let unfilled =
                    Button.view { Button.defaults with Text = "Save" } |> Widget.toControl

                Expect.isFalse
                    (unfilled.Attributes |> List.exists (fun a -> a.Name = "slot"))
                    "no slot attribute when unfilled"
            ...
            test "an unfilled Button's render is structurally-Scene-equal to the pre-slot baseline, both themes" {
                ...
                for (tname, t) in themes do
                    Expect.equal
                        ...
                        (sprintf
                            "button.%s unfilled render == the frozen pre-slot oracle (label position invariant)"
                            tname)
            }
            ...
            test "an unfilled Panel lowers identically to the legacy no-slot Panel" {
                ...
                let legacy =
                    FS.GG.UI.Controls.Panel.create [ FS.GG.UI.Controls.Panel.children [ Widget.toControl body ] ]
            ...
            test "a non-slotted kind (CheckBox) is unchanged — exposing slots is scoped (SC-007)" {
                ...
                Expect.isFalse
                    (cb.Attributes |> List.exists (fun a -> a.Name = "slot"))
                    "a non-slotted kind carries no slot attribute"
                ...
                let dangerFill =
                    Button.view
                        { Button.defaults with
                            Id = Some "c"
                            Text = "Tag"
                            Classes = [ Variant StyleVariant.Danger ]
                        }
...
let retainedIdentity =
    let rec findByKey key (node: RetainedNode<'msg>) : RetainedNode<'msg> option =
        if node.Control.Key = Some key then
            Some node
        else
        ...
        [
            test "a keyed slotted control keeps its RetainedId when a sibling is inserted above its host" {
                ...
                Expect.equal
                    id1
                    id0
                    "SC-004: the slotted 'field' keeps its retained identity through the live path across the shift"
                // and the wired frame is byte-identical to a full rebuild (no parallel slot path)
                Expect.equal
                    s1.Render.Scene
                    (Control.renderTree theme size frame1).Scene
                    "the wired frame == a full rebuild (slotted content is a first-class sub-tree)"
            }
        ]
...
module private Evidence =
    let readinessRoot =
        Path.GetFullPath(
            Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "specs", "095-lookless-slot-composition", "readiness")
        )
        ...
        [
            test "capture pre-slot parity baselines (T016) + slot-placement proof (T014)" {
                ...
                let parityHolds =
                    themes
                    |> List.forall (fun (_, t) ->
                        ControlInternals.faithfulContent t box unfilled = frozenButtonGeom t "Save")
