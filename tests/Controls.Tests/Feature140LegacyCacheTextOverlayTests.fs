...
let private content =
    [ Scene.textAt { X = 0.0; Y = 12.0 } "cached text" Colors.white ]
        ...
        [
            test "cache node form wraps content in a transparent CachedSubtree boundary" {
                ...
                | [ { Nodes = [ CachedSubtree boundary ] } ] ->
                    ...
                    Expect.contains
                        (Scene.describe boundary.Scene)
                        TextElement
                        "wrapped scene remains transparent to describe"
            ...
            test "text node form stays on the content layer while glyph-run proof is opt-in" {
                ...
                Expect.isFalse
                    (Scene.describe scene |> List.contains GlyphRunElement)
                    "glyph-run proof is not introduced unless explicitly authored"
            }
            ...
            test "overlay node form routes layer evidence without changing text output" {
                ...
                Expect.contains
                    (overlayChain.NormalizedEffects |> List.map _.Effect)
                    (Composition.LayerHint "overlay")
                    "overlay routing records layer intent"
