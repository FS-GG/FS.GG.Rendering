    ...
    testList
        ...
        [
            test "portal layers paint bottom-to-top and hit top-to-bottom from one stream" {
                let hosts =
                    [ Composition.layerHost "popup" 10 true; Composition.layerHost "modal" 20 true ]
                ...
                Expect.equal
                    (composed.Paint |> List.map _.Id)
                    [ "content"; "popup"; "modal" ]
                    "paint follows host order bottom-to-top"

                Expect.equal
                    (composed.Hit |> List.map _.Id)
                    [ "modal"; "popup"; "content" ]
                    "hit follows reverse layer paint order"
            ...
            test "empty layers are equivalent to no portal layers" {
                let onlyContent =
                    Composition.composeLayers
                        [ Composition.layerHost "popup" 10 true ]
                        [ contribution "content" "content" 0 ]
                        []
