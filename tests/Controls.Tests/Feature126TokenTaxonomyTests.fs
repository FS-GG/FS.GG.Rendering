        ...
        [
            ...
            test "every taxonomy layer is nameable and resolves to its expected value (SC-001)" {
                ...
                Expect.equal
                    ...
                    (Colors.rgba 31uy 41uy 55uy 255uy)
                    "alias.light.text.default"

                Expect.equal
                    DesignTokensExt.Alias.Dark.textDefault
                    (Colors.rgba 241uy 245uy 249uy 255uy)
                    "alias.dark.text.default"
                // component
                Expect.equal
            ...
            test "seed functional colours align to the existing Light primitives (FR-011)" {
                Expect.equal
                    ...
                    DesignTokens.Light.success
                    "seed.colorSuccess == DesignTokens.Light.success"

                Expect.equal
                    DesignTokensExt.Seed.colorWarning
                    DesignTokens.Light.warning
                    "seed.colorWarning == DesignTokens.Light.warning"

                Expect.equal
                    DesignTokensExt.Seed.colorTextBase
                    DesignTokens.Light.foreground
                    "seed.colorTextBase == DesignTokens.Light.foreground"
            }
            ...
            test
                "the generated token modules are up to date with the DTCG source — generator --check passes (FR-010, P9)" {
                let psi =
                    ProcessStartInfo("dotnet", "fsi scripts/generate-design-tokens.fsx --check")
