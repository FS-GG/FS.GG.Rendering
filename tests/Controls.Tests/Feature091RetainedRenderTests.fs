        ...
        [
            ...
            test "a keyed control whose position SHIFTED is still matched and carries its identity" {
                ...
                Expect.equal
                    ...
                    (idOfKey "editor" r0)
                    "editor identity survives the positional shift"
            }
            ...
            test "a control whose Kind changed is Replaced with a FRESH identity (no false identity)" {
                ...
                Expect.notEqual
                    (idOfKey "editor" s.Retained)
                    (idOfKey "editor" r0)
                    "SC-001 negative: a Kind change mints a new identity"
            }
        ]
        ...
        [
            test "wired path: focus + an in-flight clock keyed by RetainedId survive an unrelated update" {
                ...
                let r0' =
                    { r0 with
                        StateByIdentity =
                            r0.StateByIdentity |> Map.add editorId { Animation = Some clock0; Text = None }
                    }
                ...
                | Some st ->
                    ...
                    let advanced =
                        RetainedRender.advance (TimeSpan.FromMilliseconds 250.0) st.Animation.Value
    ...
    testList
        ...
        [
            test "a localized leaf change recomputes only the changed subtree (RecomputedNodeCount < N)" {
                ...
                let leaves prefix n =
                    [ for i in 1..n -> leaf (prefix + string i) ("v" + string i) ]
                ...
                Expect.isLessThanOrEqual
                    ...
                    w.ChangedSubtreeBound
                    "recomputed <= changed-subtree bound"

                Expect.isLessThan
                    w.ChangedSubtreeBound
                    w.BaselineNodeCount
                    "SC-003: changed subtree is strictly smaller than N"
...
module private Gen091 =
    ...
    let private genKind: Gen<ControlKind> =
        Gen.elements [ "text-block"; "button"; "stack" ]
...
let us4 =
    testList
        ...
        [
            ...
            test
                "Synthetic: a duplicate-keyed sibling list surfaces KeyCollision on the wired path and stays total (SC-006)" {
                ...
                collisions
                |> List.iter (fun d ->
                    Expect.equal d.Severity ControlDiagnosticSeverity.Warning "KeyCollision is a Warning")
            }
        ]
...
module private Evidence =
    ...
    let readinessRoot =
        Path.GetFullPath(
            Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "specs", "091-wire-reconciler-render-path", "readiness")
        )
    ...
    let hashOf (scene: Scene) : string =
        (Scene.renderReadbackEvidence size scene).DeterministicHash
    ...
    testList
        ...
        [
            test "capture retained-parity + work-reduction artifacts (SC-003/SC-004)" {
                ...
                let leaves n =
                    [ for i in 1..n -> leaf ("n" + string i) ("v" + string i) ]
                ...
                File.WriteAllText(
                    ...
                    String.concat
                        ...
                        [
                            ...
                            sprintf
                                "invariant=RecomputedNodeCount(%d) <= ChangedSubtreeBound(%d) < BaselineNodeCount(%d)"
                                s.WorkReduction.RecomputedNodeCount
                                s.WorkReduction.ChangedSubtreeBound
                                s.WorkReduction.BaselineNodeCount
                            "authoritative-test=Feature091RetainedRenderTests/091 US3 partial update + golden parity"
                            ""
                        ]
                )
                ...
                Expect.isLessThan
                    s.WorkReduction.ChangedSubtreeBound
                    s.WorkReduction.BaselineNodeCount
                    "work bounded by changed subtree"
            }
            ...
            test "capture survives-proof artifacts (SC-002)" {
                ...
                let clockAdvanced =
                    ...
                    | Some st when st.Animation.IsSome ->
                        (RetainedRender.advance (TimeSpan.FromMilliseconds 250.0) st.Animation.Value).Elapsed >
                            clock0.Elapsed
                    | _ -> false

                // baseline = rebuild every frame (init); the id is not stable across the shift.
                let baselineFails = idOfKey "editor" (rinit theme size next) <> Some editorId
