...
let tests =
    testList
        "Audit_TextCache: Text-measure cache parity + key-completeness + effectiveness (FR-004/008/009)"
        [
            ...
            test "Audit: TextCache scaffold reachability — TextCacheEnabled + counters (T004)" {
                ...
                Expect.isTrue
                    (s.WorkReduction.TextMeasureCacheHits >= 0
                     && s.WorkReduction.TextMeasureCacheMisses >= 0)
                    "text-cache counters reachable"
            }

            // ---- T021 KEY-COMPLETENESS adversarial ----
            test
                "Audit: key-completeness — a single-field difference (text|family|size|weight) MISSES with correct fresh metrics (FR-009)" {
                ...
                let differs (text: string) (f: FontSpec) (why: string) =
                    ...
                    Expect.equal
                        m
                        (Scene.measureText text f)
                        (sprintf "the %s miss returns the correct un-cached metrics" why)
            ...
            test
                "Audit: DISCRIMINATING — a warm cache serves the matching key and the equality oracle catches a divergence" {
                ...
                Expect.notEqual
                    mShort
                    mLong
                    "the two measurements genuinely differ — the equality oracle is discriminating (a stale cross-hit would be caught)"
            }

            // ---- T021 PARITY (wired) with discriminating proof ----
            test
                "Audit: cache-on ≡ cache-off byte-identical scene + bounds, with a discriminating divergence check (FR-004)" {
                ...
                Expect.isTrue
                    (on.WorkReduction.TextMeasureCacheMisses + on.WorkReduction.TextMeasureCacheHits > 0)
                    "the cache-on step measured text"
                ...
                Expect.notEqual
                    onDifferent.Render.Scene
                    on.Render.Scene
                    "a genuinely different render is caught (discriminating)"
            }

            // ---- T032 EFFECTIVENESS: repeated identical measure ⇒ high hit-rate vs disabled ----
            test
                "Audit: EFFECTIVENESS — repeated identical measure yields a high text-cache hit-rate vs the disabled oracle (T032)" {
                let requests = 30
                ...
                Expect.isTrue
                    (onRate > 0.95)
                    (sprintf "enabled steady-state hit-rate near-100%% (got %.3f over %d requests)" onRate requests)

                printfn
                    "AUDIT-MARGIN TextCache: enabled hits=%d misses=%d rate=%.3f | disabled hits=%d misses=%d"
                    onHits
                    onMisses
                    onRate
                    offHits
