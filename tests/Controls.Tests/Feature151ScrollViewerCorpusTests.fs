        ...
        [
                ...
                for item in Feature151ScrollViewerFixtures.cases do
                    ...
                    | Some viewport ->
                        Expect.isGreaterThanOrEqual
                            ...
                            $"{item.CaseId}: content width"

                        Expect.isGreaterThanOrEqual
                            viewport.ContentHeight
                            viewport.Viewport.Height
                            $"{item.CaseId}: content height"
            ...
            test "dynamic content changes the accepted extent without changing the viewport box" {
                let item =
                    ...
                    |> List.find (fun item -> item.CaseId = "dynamic-content-change")

                match
                    Feature151ScrollViewerFixtures.viewportOf item,
                    Feature151ScrollViewerFixtures.changedViewportOf item
                with
            ...
            test "empty content is classified with the empty extent source" {
                let item =
                    Feature151ScrollViewerFixtures.cases
                    |> List.find (fun item -> item.CaseId = "empty-content")
