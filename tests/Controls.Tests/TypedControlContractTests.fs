...
let typedControlContractTests =
    testList
        "Typed standard controls contract"
        [
            test "Types signature declares typed standard controls events attributes and values" {
                ...
                |> List.iter (fun required -> Expect.stringContains typesFsi required $"Types.fsi exposes {required}")
            }
            ...
            test "current FSI transcript covers typed front doors and custom escape hatch" {
                ...
                Expect.isTrue
                    (File.Exists(Path.Combine(repositoryRoot, transcriptPath)))
                    "current Controls FSI transcript exists"
            ...
            test "known standard kinds events and attributes reject misspelled standard names through typed unions" {
                let unionCaseNames (typ: Type) =
                    FSharpType.GetUnionCases typ |> Array.map _.Name |> Set.ofArray
                ...
                |> List.iter (fun name ->
                    Expect.isTrue (knownAttributes.Contains name) $"KnownAttribute exposes {name}")

                [ "Buton"; "TxtBox"; "LineChar"; "DataGird" ]
                |> List.iter (fun misspelled ->
                    Expect.isFalse
                        (knownControls.Contains misspelled)
                        $"misspelled control {misspelled} is not a known standard kind")

                [ "Clik"; "OnChanged"; "FocusChange" ]
                |> List.iter (fun misspelled ->
                    Expect.isFalse
                        (knownEvents.Contains misspelled)
                        $"misspelled event {misspelled} is not a known standard event")

                [ "Seriez"; "Colums"; "VisibleRanges" ]
                |> List.iter (fun misspelled ->
                    Expect.isFalse
                        (knownAttributes.Contains misspelled)
                        $"misspelled attribute {misspelled} is not a known standard attribute")
            }
            ...
            test "chart and DataGrid typed data front doors lower to stable attribute names and typed payloads" {
                ...
                | UntypedValue(:? (DataGridColumn list) as lowered) ->
                    Expect.equal lowered columns "DataGrid.columns preserves typed columns"
                | other -> failtestf "DataGrid.columns lowered to unexpected payload %A" other

                match (attrByName "rows" grid).Value with
                | UntypedValue(:? (DataGridRow list) as lowered) ->
                    Expect.equal lowered rows "DataGrid.rows preserves typed rows"
                | other -> failtestf "DataGrid.rows lowered to unexpected payload %A" other

                match (attrByName "visibleRange" grid).Value with
                | UntypedValue(:? VisibleRange as lowered) ->
                    Expect.equal lowered visibleRange "DataGrid.visibleRange preserves typed range"
                | other -> failtestf "DataGrid.visibleRange lowered to unexpected payload %A" other

                match (attrByName "selectedRows" grid).Value with
                | UntypedValue(:? Set<string> as lowered) ->
                    Expect.equal lowered (Set.singleton "row-1") "DataGrid.selectedRows preserves typed selection"
                | other -> failtestf "DataGrid.selectedRows lowered to unexpected payload %A" other

                match (attrByName "focusedCell" grid).Value with
                | UntypedValue(:? (DataGridFocusedCell option) as lowered) ->
                    Expect.equal lowered focusedCell "DataGrid.focusedCell preserves typed focus"
                | other -> failtestf "DataGrid.focusedCell lowered to unexpected payload %A" other
            }

            test
                "schema-backed diagnostics report missing attributes unsupported attributes unsupported events and custom usage" {
                let missing =
                    Control.standard
                ...
                Expect.exists
                    missing
                    (fun diagnostic -> diagnostic.Code = MissingRequiredAttribute && diagnostic.Message.Contains "Rows")
                    "schema reports missing required rows"
                ...
                Expect.exists
                    ...
                    (fun diagnostic -> diagnostic.Message.Contains "Rows" && diagnostic.Message.Contains "Button")
                    "schema reports unsupported standard attribute"

                let unsupportedEvent =
                    Control.standard
                        StandardControlKind.TextBlock
                        [
                ...
                Expect.exists
                    unsupportedEvent
                    (fun diagnostic -> diagnostic.Message.Contains "Click" && diagnostic.Message.Contains "TextBlock")
                    "schema reports unsupported standard event"
                ...
                Expect.exists
                    custom
                    (fun diagnostic ->
                        diagnostic.Severity = ControlDiagnosticSeverity.Info
                        && diagnostic.Message.Contains "Custom extension")
                    "custom controls are visibly classified"
            }
        ]
...
let typedControlsFrontDoorContractTests =
    testList
        "Typed controls front door (065) contract"
        [
            test "Widget seam exists and round-trips a Control" {
                ...
                Expect.equal
                    (sprintf "%A" (Widget.toControl widget))
                    (sprintf "%A" control)
                    "Widget.toControl (ofControl c) = c"
            }
            ...
            test "six typed modules under FS.GG.UI.Controls.Typed lower to Control" {
                ...
                let lowered =
                    [
                        Widget.toControl (
                            FS.GG.UI.Controls.Typed.TextBlock.view FS.GG.UI.Controls.Typed.TextBlock.defaults
                        )
                        Widget.toControl (FS.GG.UI.Controls.Typed.Button.view FS.GG.UI.Controls.Typed.Button.defaults)
                        Widget.toControl (
                            FS.GG.UI.Controls.Typed.CheckBox.view FS.GG.UI.Controls.Typed.CheckBox.defaults
                        )
                ...
                Expect.equal
                    lowered
                    [ "text-block"; "button"; "check-box"; "stack" ]
                    "primitive typed views lower to legacy kinds"
                ...
                let textBox =
                    Widget.toControl (FS.GG.UI.Controls.Typed.TextBox.view textBoxProps textBoxModel)
                ...
                let grid =
                    Widget.toControl (FS.GG.UI.Controls.Typed.DataGrid.view gridProps gridModel)
            ...
            test "typed .fsi surface declares no obj field and no string-named event (FR-005)" {
                ...
                let primitives =
                    signatures |> List.find (fun (path, _) -> path.EndsWith "Primitives.fsi") |> snd
                ...
                Expect.stringContains
                    ...
                    "CheckBox event is a typed function option"
            }
            ...
            test "Widget and typed signature files exist on disk" {
                ...
                |> List.iter (fun path ->
                    Expect.isTrue
                        (File.Exists(
                            Path.Combine(repositoryRoot, path.Replace("/", string Path.DirectorySeparatorChar))
                        ))
                        $"{path} exists")
            }
        ]
