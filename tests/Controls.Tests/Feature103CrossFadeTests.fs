...
let private hoverModel (id: ControlId) =
    { emptyModel with
        HoveredControl = Some id
    }
...
let private labelView () : Control<Msg> =
    Stack.create
        [
            Stack.children [ TextBlock.create [ TextBlock.text "label" ] |> Control.withKey "lbl" ]
        ]
...
let rec private colorsOfNode (n: SceneNode) : (Rgb * byte) list =
    let paint (p: Paint) =
        p.Fill |> Option.map (fun c -> (rgbOf c, c.Alpha)) |> Option.toList
...
let private rgbSet (s: Scene) : Set<Rgb> =
    colorsOfScene s |> List.map fst |> Set.ofList
...
module private Evidence =
    let readinessRoot =
        Path.GetFullPath(
            Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "specs", "103-visual-state-cross-fade", "readiness")
        )
...
let crossFade =
    testList
        "103 US1 a visual-state transition genuinely cross-fades its colours (not a fade-in from transparent)"
        [

            test
                "mid-flight: the prior colour fades OUT under the next fading IN; the displayed colour is strictly between the endpoints" {
                let view = switchView ()
                ...
                let normalColors =
                    colorsOfScene (Scene.group normalOwn) |> List.map fst |> Set.ofList
                ...
                Expect.isNonEmpty
                    priorOnly
                    "precondition: the Normal own-paint has a colour the Hover own-paint does not (the cross-FROM colour)"

                Expect.isNonEmpty
                    nextOnly
                    "precondition: the Hover own-paint has a colour the Normal own-paint does not (the cross-TO colour)"
                ...
                Expect.isTrue
                    (RetainedRender.clockActive clock)
                    "the clock is genuinely mid-flight at the sampled frame"
                ...
                Expect.isTrue
                    (opNext > 0.0 && opNext < 1.0)
                    "mid-flight: the next layer is partially (not fully) faded in"
                ...
                Expect.isTrue
                    (Set.contains cpRgb midRgb)
                    "INV-3 (red→green): the prior (Normal) colour is present mid-flight — it fades OUT, not absent"
                ...
                let alphaOf rgb =
                    mid |> List.filter (fun (c, _) -> c = rgb) |> List.map snd

                Expect.isTrue
                    (alphaOf cpRgb |> List.forall (fun a -> a < 255uy))
                    "the prior colour is faded (alpha < full) — it is on its way OUT"

                Expect.isTrue
                    (alphaOf cnRgb |> List.exists (fun a -> a > 0uy && a < 255uy))
                    "the next colour is partially faded IN (alpha strictly between 0 and full)"
                ...
                Evidence.write
                    ...
                    [
                        ...
                        sprintf
                            "displayed-red=%f (between %d and %d)"
                            (overChannel cpR cnR opPrior opNext)
                            (min cpR cnR)
                            (max cpR cnR)
...
let byteIdentity =
    testList
        "103 US2 at-rest and settled output is byte-identical to the static render"
        [

            test
                "at-rest: no clock in flight ⇒ the assembled scene equals the static render and emits no animation attribute (INV-1)" {
                let view = switchView ()
                let r0 = initFrom emptyModel view
            ...
            test
                "final-frame: a settled transition paints the static Hover render byte-identically for every channel (INV-2)" {
                let view = switchView ()
                let hover = hoverModel "sw"
                ...
                Expect.equal
                    settled.Render.Scene
                    hoverStatic
                    "the settled frame is byte-identical to the snapped static Hover render (every channel)"
            }

            test
                "settle path is UNCHANGED: an at-rest re-step recomputes ZERO nodes (the cross-fade is a mid-flight-only overlay)" {
                let view = switchView ()
                let r0 = initFrom emptyModel view
...
let determinism =
    testList
        "103 US2 the cross-fade is deterministic under injected deltas"
        [
            ...
            test "replaying an identical injected-delta sequence reproduces an identical sampled-frame sequence" {
                ...
                Expect.equal
                    s2.Render.Scene
                    s1.Render.Scene
                    "a non-positive delta never rewinds — the sampled frame is unchanged"
            }
...
let edges =
    testList
        "103 US2 cross-fade edge cases (retarget, held-state, return-to-Normal, no-colour-delta)"
        [

            test
                "retarget continuity (INV-5): a mid-flight state change re-seeds From from the previous target's snapshot and resets Elapsed (no snap to a stale endpoint)" {
                let view = switchView ()
                let hover = hoverModel "sw"

                let press =
                    { emptyModel with
                        HoveredControl = Some "sw"
                        PressedControls = Set.ofList [ "sw" ]
                    }
                ...
                Expect.isTrue
                    (RetainedRender.clockActive hoverClock)
                    "precondition: the Hover clock is mid-flight before the retarget"
                ...
                Expect.equal
                    retClock.From
                    hoverOwn
                    "INV-5: From is re-seeded from the PREVIOUS target's (Hover) own-scene snapshot — not a stale at-rest endpoint"
            }

            test
                "held-state scoped repaint (INV-6): a held state stays a Keep after settle — single scoped repaint, not per-frame" {
                let view = switchView ()
                let hover = hoverModel "sw"
                ...
                Expect.equal
                    held2.WorkReduction.RecomputedNodeCount
                    0
                    "a held, settled state repaints no nodes (single scoped repaint, not per-frame)"
                ...
                Expect.equal
                    held2.Render.Scene
                    (Control.renderTree theme size (bridged hover view)).Scene
                    "a held, settled state stays byte-identical to the static Hover render"
            }

            test
                "return-to-Normal (INV-1): a settled Hover clock unhovered to Normal is DROPPED, returning the identity to byte-identical at-rest output" {
                let view = switchView ()
                let hover = hoverModel "sw"
                ...
                Expect.isTrue
                    (clockOf "sw" settled.Retained).IsSome
                    "precondition: the settled Hover clock is kept while held"
                ...
                Expect.isNone
                    (clockOf "sw" returned.Retained)
                    "the settled return-to-Normal clock is dropped (discarding From)"

                Expect.equal
                    returned.Render.Scene
                    (Control.renderTree theme size (bridged emptyModel view)).Scene
                    "the identity returns to byte-identical at-rest (Normal) output"
            }

            test
                "no-colour-delta transition: a control whose Normal and Hover paint are identical introduces no permanent artifact (settles + returns byte-identical)" {
                let view = labelView ()
                ...
                Expect.equal
                    (rgbSet normalStatic)
                    (rgbSet hoverStatic)
                    "precondition: the label has no Normal/Hover colour delta"
                ...
                Expect.isTrue
                    (Set.isSubset (rgbSet s2.Render.Scene) (rgbSet normalStatic))
                    "no new colour appears mid-flight when the endpoints share their colours"
                ...
                Expect.equal
                    returned.Render.Scene
                    normalStatic
                    "returns byte-identical at-rest after unhover (no permanent artifact)"
            }
        ]
