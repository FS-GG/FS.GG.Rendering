...
let private button onClick =
    Button.create [ Button.text "b"; Button.onClick onClick ]
...
let private row children =
    Stack.create [ Stack.children children ]

[<Tests>]
let tests =
    testList
        "Feature175UnkeyedSiblingDiagnostic"
        [
            ...
            test "unkeyed NON-interactive same-kind siblings raise nothing (no stamp to smear)" {
                ...
                Expect.isEmpty
                    (Diagnostics.unkeyedInteractiveSiblings tree)
                    "non-interactive siblings carry no visual state"
            }
            ...
            test "a single unkeyed interactive control among keyed siblings does not collide" {
                ...
                Expect.isEmpty
                    (Diagnostics.unkeyedInteractiveSiblings tree)
                    "one unkeyed member alone shares its id with no sibling"
            }
