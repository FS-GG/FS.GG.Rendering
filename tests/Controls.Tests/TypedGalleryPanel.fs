...
let private noteProps: TextAreaProps<int> =
    { TextArea.defaults "typed-note" with
        Value = "draft"
    }

let private noteModel = fst (TextArea.init noteProps)

let private listProps: ListBoxProps<int> =
    { ListBox.defaults "typed-list" with
