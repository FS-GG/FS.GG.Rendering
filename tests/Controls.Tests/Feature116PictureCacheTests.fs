...
let tests =
    testList
        "Feature 116 picture cache (US2, FR-005/006/007, SC-002/003)"
        [
            ...
            test "a HIT is byte-identical to a fresh full rebuild (FR-005)" {
                ...
                Expect.equal
                    (flattenScene s.Render.Scene)
                    (flattenScene full.Scene)
                    "the reused (hit) scene is byte-identical (paint-order) to a fresh paint, transparent to the replay boundary"
            }

            test "perturbing CONTENT (font/text) forces a miss on exactly that row (FR-006)" {
                let after =
                    grid [ plainRow "r0" "zero"; plainRow "r1" "ONE-CHANGED"; plainRow "r2" "two" ]
            ...
            test "perturbing BOX (width) forces a miss on exactly that row (FR-006)" {
                let after =
                    grid [ plainRow "r0" "zero"; row "r1" "one" 260.0 Normal; plainRow "r2" "two" ]
            ...
            test
                "the correctness key is the painted picture: a paint-affecting change misses, a paint-neutral one hits (FR-006)" {
                ...
                let stateOnly =
                    grid [ plainRow "r0" "zero"; row "r1" "one" 200.0 Hover; plainRow "r2" "two" ]

                let neutral = RetainedRender.step theme size r0 stateOnly

                Expect.equal
                    neutral.WorkReduction.PictureCacheMisses
                    0
                    "a paint-neutral change keeps the picture identical → no stale-free miss"
                ...
                let painted =
                    grid [ plainRow "r0" "zero"; plainRow "r1" "DIFFERENT"; plainRow "r2" "two" ]

                let changed = RetainedRender.step theme size r0 painted

                Expect.equal
                    changed.WorkReduction.PictureCacheMisses
                    1
                    "a paint-affecting change misses exactly that row"
            }
            ...
            test "cache-on ≡ cache-off: the always-miss oracle renders byte-identically (FR-007, SC-003)" {
                ...
                Expect.equal
                    (flattenScene off.Render.Scene)
                    (flattenScene on.Render.Scene)
                    "cache-off scene is byte-identical (paint-order) to cache-on, transparent to the replay boundary (FR-007)"
            }
        ]
