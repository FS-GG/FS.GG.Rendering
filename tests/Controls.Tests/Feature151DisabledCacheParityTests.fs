...
let tests =
    testList
        "Feature151DisabledCacheParity"
        [
            test "repeated ScrollViewer renders are observable-parity equivalent" {
                let item =
                    Feature151ScrollViewerFixtures.cases
                    |> List.find (fun item -> item.CaseId = "substantially-overflowing")
                ...
                Expect.equal
                    (Control.scrollViewport second item.ScrollViewerId)
                    (Control.scrollViewport first item.ScrollViewerId)
                    "scroll viewport parity"
            }
                        ...
                        [
                            Stack.children
                                [
                                    ...
                                    Feature151ScrollViewerFixtures.scrollViewer
                                        "sv-disabled-cache"
                                        (Feature151ScrollViewerFixtures.rows 12)
                                ]
                        ]
