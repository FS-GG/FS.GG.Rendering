            ...
            test "every (intent variant x state) pair preserves the variant's hue (issue #181)" {
                for variantName, variant in intentVariants do
                    ...
                    let baseHue =
                        ...
                        | None ->
                            failtestf
                                "%s: the resting fill must be chromatic for the hue test to mean anything"
                                variantName
                ...
                for themeName, t in themes do
                    for variantName, variant in intentVariants do
                        ...
                        Expect.isGreaterThan
                            hovered
                            rest
                            (sprintf "%s: Hover is a visible step off the resting fill" label)
            ...
            test "Disabled discards intent and Focused leaves the fill alone (issue #181)" {
                for variantName, variant in intentVariants do
                    ...
                    Expect.equal
                        disabled.Fill
                        theme.Muted
                        (sprintf "%s/disabled greys out — an inactive control claims no intent" variantName)
                    ...
                    Expect.equal
                        focused.Stroke
                        theme.Accent
                        (sprintf "%s/focused wears the accent focus ring" variantName)
            }
