...
let private allStates =
    [
        ...
        Selected
        Loading
        VisualState.Validation(Invalid "x")
    ]

[<Tests>]
let feature093StyleResolverTests =
    testList
        "Feature 093 style resolver"
        [
            ...
            test "the six built-in variants are pairwise distinguishable on one base under one theme (SC-001)" {
                let resolved =
                    allVariants
                    |> List.map (fun v -> v, Style.resolve theme neutralBase [ Variant v ] Normal)
            ...
            test "Primary derives accent-family fill; Danger derives danger-family fill (SC-001)" {
                let primary =
                    Style.resolve theme neutralBase [ Variant StyleVariant.Primary ] Normal
            ...
            test "each differentiated VisualState resolves to a distinct token-derived style (SC-002)" {
                ...
                let resolved =
                    distinct |> List.map (fun s -> s, Style.resolve theme neutralBase [] s)
            ...
            test
                "the visual state wins over a class for an overlapping field; the class's other fields remain (SC-002, FR-003)" {
                ...
                let resolved =
                    Style.resolve theme neutralBase [ Variant StyleVariant.Ghost ] Focused

                Expect.equal
                    resolved.Stroke
                    theme.Accent
                    "the Focused state's Stroke overrides the Ghost class's Stroke"
                ...
                Expect.equal
                    resolved.Foreground
                    theme.Foreground
                    "the Ghost class's non-overlapping Foreground is retained"
            }

            test "a later-attached class wins over an earlier one (SC-002, FR-003 last-writer-wins)" {
                let resolved =
                    Style.resolve theme neutralBase [ Variant StyleVariant.Primary; Variant StyleVariant.Danger ] Normal
            ...
            test "Disabled + Danger compose per the fixed order (state over class) (SC-002 edge case)" {
                let resolved =
                    Style.resolve theme neutralBase [ Variant StyleVariant.Danger ] Disabled

                Expect.equal
                    resolved.Fill
                    theme.Muted
                    "Disabled de-emphasis (state) wins the Fill over the Danger class"
            }
