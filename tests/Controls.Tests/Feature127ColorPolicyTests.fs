...
let private antNeutralBorderWaiver: (string * Color * Color) list =
    [
        ...
        FS.GG.UI.Themes.AntDesign.AntTheme.antLight.Background
        ...
        [
            ...
            test "alpha is composited before measurement; transparent fg is Indeterminate (edge cases)" {
                ...
                Expect.equal
                    semiResult.Measured
                    expectedMeasured
                    "alpha foreground must be composited over bg before measuring"
                ...
                let transparentResult =
                    ColorPolicy.evaluatePairing ColorPolicy.wcag transparentPairing
            ...
            test "ant carries a no-overclaim AuthorityNote where it certifies a WCAG-failing pairing (FR-010)" {
                ...
                Expect.equal
                    (resultFor wcagResults "primary-hover-fg-on-surface").Outcome
                    ColorPolicy.Failed
                    "wcag fails it"
            }
            ...
            test "committed reports match the live render — drift gate (FR-009/SC-004)" {
                for name, policy in [ "wcag", ColorPolicy.wcag; "ant", ColorPolicy.ant ] do
                    ...
                    Expect.isTrue
                        (File.Exists path)
                        (sprintf "committed report %s must exist (regenerate with UPDATE_POLICY_REPORTS=1)" path)
                    ...
                    Expect.equal
                        committed
                        live
                        (sprintf "committed docs/reports/color-policy-%s.md is out of date (drift)" name)
            }
            ...
            test "the emitted catalog is derived from StyleResolver, not restated (issue #174)" {
                ...
                let style =
                    StyleResolver.resolve
                        ...
                        (VisualState.Validation(Invalid ""))

                Expect.notEqual
                    style.Foreground
                    style.Fill
                    "issue #359: the invalid label is no longer painted onto its own fill"
                ...
                let rows =
                    StyleCatalog.pairingsOfStyle theme.Background "button/danger/invalid" false style
            ...
            test "the emitted catalog covers the closed kind x scenario x state domain (issue #174 / F-DS-1)" {
                Expect.equal
                    StyleCatalog.scenarios.Length
                    (StyleCatalog.variants.Length + StyleCatalog.intents.Length)
                    "scenarios = variants + intents"

                Expect.equal
                    (StyleCatalog.kinds.Length
                     * StyleCatalog.scenarios.Length
                     * StyleCatalog.states.Length)
                    264
                    "closed domain size"
                ...
                for slug, theme in gatedThemes do
                    ...
                    Expect.equal
                        keys.Length
                        (List.distinct keys).Length
                        (sprintf "%s rows are deduplicated by (fg, bg, role)" slug)
                    ...
                    for p in emitted do
                        ...
                        Expect.contains
                            (StyleCatalog.states |> List.map fst)
                            parts.[2]
                            (sprintf "%s: %s names a real state" slug p.Name)
            }
            ...
            test "disabled pairings are exempt; a boundary-less control emits no boundary row (issue #174)" {
                ...
                for p in disabled do
                    ...
                    Expect.notEqual
                        ...
                        ColorPolicy.Failed
                        (sprintf "%s must not fail" p.Name)

                // the ghost button at rest: transparent fill, zero stroke width -> no boundary to measure
                let ghost =
                    StyleResolver.resolve theme "button" "" [ Variant StyleVariant.Ghost ] Normal
                ...
                let rows =
                    StyleCatalog.pairingsOfStyle theme.Background "button/ghost/normal" false ghost

                Expect.equal
                    (rows |> List.map (fun p -> p.Name))
                    [ "button/ghost/normal#text" ]
                    "only the text pairing — no unmeasurable boundary row"
            }
            ...
            test "a deliberately low-contrast theme is rejected (issue #174)" {
                ...
                let results =
                    ColorPolicy.evaluate ColorPolicy.wcag (StyleCatalog.emittedPairings lowContrast)
                ...
                for r in failed do
                    Expect.isLessThan
                        r.Measured
                        3.0
                        (sprintf "%s: a flat theme's every shade is near-invisible" r.Pairing)
                ...
                let shipped =
                    ColorPolicy.evaluate
                        ColorPolicy.wcag
                        (StyleCatalog.emittedPairings FS.GG.UI.Themes.Default.Theme.light)

                Expect.isNonEmpty
                    (shipped |> List.filter (fun r -> r.Outcome = ColorPolicy.Passed))
                    "the shipped light theme passes some pairings"
            }
            ...
            test "committed emitted-style reports match the live render — drift gate (issue #174)" {
                for slug, theme in gatedThemes do
                    ...
                    Expect.equal
                        committed
                        live
                        (sprintf "committed docs/reports/color-policy-emitted-%s.md is out of date (drift)" slug)
            }
            ...
            test "every emitted Text label of every built-in theme passes wcag — blocking gate (issue #360)" {
                for slug, theme in gatedThemes do
                    let textLabels =
                        StyleCatalog.emittedPairings theme |> List.filter (fun p -> p.Role = Role.Text)
            ...
            test "every emitted boundary passes wcag except the evidenced Ant neutral-border waiver (F-DS-1)" {
                let boundaryFails theme =
                    ...
                    |> List.filter (fun p ->
                        (ColorPolicy.evaluatePairing ColorPolicy.wcag p).Outcome = ColorPolicy.Failed)
                ...
                for slug, fg, bg in antNeutralBorderWaiver do
                    ...
                    | None ->
                        failtestf
                            "%s: waived boundary (%A on %A) is no longer emitted — remove the stale waiver"
                            slug
                            fg
                            bg
                    | Some p ->
                        Expect.equal
                            ...
                            ColorPolicy.Failed
                            (sprintf
                                "%s: waived boundary (%A on %A) now PASSES wcag — remove the stale waiver"
                                slug
                                fg
                                bg)
            }
            ...
            test "each default theme satisfies its own declared contrastRequiredRatio token (F-DS-2)" {
                ...
                for slug, theme, required, tokenFg, tokenBg in declared do
                    ...
                    Expect.isGreaterThan
                        required
                        1.0
                        (sprintf "%s: contrastRequiredRatio must be a real floor, not vacuous" slug)
                    ...
                    Expect.equal
                        theme.Foreground
                        tokenFg
                        (sprintf "%s: theme foreground is the DTCG token primitive" slug)

                    Expect.equal
                        theme.Background
                        tokenBg
                        (sprintf "%s: theme background is the DTCG token primitive" slug)
