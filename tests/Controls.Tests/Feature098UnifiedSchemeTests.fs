...
let private genTree: Gen<Control<Msg>> =
    let genLeaf =
        gen {
            ...
            let attrs =
                if bound then
                    [ Button.text "x"; Button.onClick (Tagged 0) ]
                else
                    [ Button.text "x" ]
...
let private allPathIds (control: Control<Msg>) : (string * Control<Msg>) list =
    let rec go path (c: Control<Msg>) =
        ...
        (id, c)
        :: (c.Children
            |> List.mapi (fun i child -> go (path + "." + string i) child)
            |> List.concat)
    ...
    testList
        ...
        [
            ...
            test "two unkeyed same-kind bound siblings route only to their own binding (no cross-routing)" {
                ...
                Expect.equal
                    (Control.nearestAuthored result secondId)
                    (Some secondId)
                    "the second sibling recovers itself (bound)"

                Expect.equal
                    (Control.nearestAuthored result firstId)
                    (Some firstId)
                    "the first sibling recovers itself (bound)"

                // Dispatch a click targeted at the second id → only Second fires.
                let clickSecond =
                    {
                        Kind = "click"
                        ControlId = Some secondId
                        Origin = ControlEventOrigin.Pointer
                        Nav = None
                    }

                Expect.equal
                    (Control.dispatch clickSecond siblings)
                    [ Second ]
                    "a click on the second sibling dispatches ONLY its own message"

                let clickFirst =
                    {
                        Kind = "click"
                        ControlId = Some firstId
                        Origin = ControlEventOrigin.Pointer
                        Nav = None
                    }

                Expect.equal
                    (Control.dispatch clickFirst siblings)
                    [ First ]
                    "a click on the first sibling dispatches ONLY its own message"
            }
            ...
            test "single scheme spans Bounds / EventBindings / BoundIds / recovery (SC-003)" {
                            ...
                            Stack.children
                                [
                                    ...
                                    Stack.create
                                        [
                                            Stack.children
                                                [ Button.create [ Button.text "deep"; Button.onClick Second ] ]
                                        ]
                                ]
                        ]
                ...
                let bindingIds =
                    result.EventBindings |> List.map (fun b -> b.ControlId) |> Set.ofList
                ...
                Expect.isTrue
                    (Set.isSubset result.BoundIds boundsIds)
                    "every BoundIds id is a laid-out Bounds id (same scheme)"

                // The two authored buttons live at "0.0" and "0.1.0"; each recovers itself by the same id.
                for id in result.BoundIds do
                    Expect.equal
                        (Control.nearestAuthored result id)
                        (Some id)
                        (sprintf "nearestAuthored is a fixed point on the bound id %s" id)
            }
            ...
            test "render.BoundIds is populated while render.Bounds stays empty (FR-002)" {
                ...
                Expect.isFalse
                    (Set.isEmpty preview.BoundIds)
                    "the preview render populates BoundIds from its bound nodes"
                // The bound button is the sole child "0.0"; its binding id agrees with BoundIds.
                let bindingIds =
                    preview.EventBindings |> List.map (fun b -> b.ControlId) |> Set.ofList
