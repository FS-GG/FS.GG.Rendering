...
let tests =
    testList
        "Feature144 overlay pointer routing"
        [
            test "topmost surface receives inside pointer before lower content" {
                let opened, _ =
                    openOne (metadata TransientSurfaceKind.Menu "pointer-menu" 10 true true false)

                let result =
                    Pointer.routeOverlay opened "down" [ "lower"; "pointer-menu-item-1" ] (Some "pointer-menu-item-1")
                ...
                Expect.exists
                    result.Effects
                    (function
                    | RecordTopmostHit _ -> true
                    | _ -> false)
                    "hit evidence recorded"
            }

            test "outside pointer dismisses non-modal surface before lower content" {
                let opened, _ =
                    openOne (metadata TransientSurfaceKind.ContextMenu "pointer-context" 20 true true false)
                ...
                Expect.exists
                    result.Effects
                    (function
                    | RequestOpenStateChange("pointer-context", false) -> true
                    | _ -> false)
                    "close request emitted"
            }

            test "modal pointer blocks covered content" {
                let opened, _ =
                    openOne (metadata TransientSurfaceKind.DialogModal "pointer-dialog" 100 true true true)

                let result =
                    Pointer.routeOverlay opened "covered-click" [ "covered" ] (Some "covered")
                ...
                Expect.exists
                    result.Diagnostics
                    (fun diagnostic -> diagnostic.Code = LowerLayerBlocked)
                    "blocking diagnostic emitted"
            }
            ...
            test "ignore-dismiss policy allows pass-through" {
                let item =
                    { metadata TransientSurfaceKind.Menu "pass-through-menu" 10 true true false with
                        DismissalPolicy =
                            { (OverlayState.defaultDismissalPolicy ()) with
                                OutsidePointer = IgnoreDismiss
                            }
                    }
