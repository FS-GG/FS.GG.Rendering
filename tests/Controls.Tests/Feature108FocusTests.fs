...
let private button () : Control<unit> =
    Control.create "button" [ Attr.text "B" ]

[<Tests>]
let tests =
    testList
        "Feature 108 Focus.markFocused (US1, FR-001..005, SC-001/002/012)"
        [
            ...
            test "unkeyed same-kind siblings are distinguished by structural path (FR-002, SC-001/002)" {
                ...
                Expect.equal
                    (visualStateOf stamped.Children.[1])
                    Focused
                    "the second (path 0.1) button carries the ring"
            ...
            test "markFocused None returns the tree byte-identical (SC-012)" {
                let root =
                    Control.create "stack" [ Attr.children [ button () |> Control.withKey "a" ] ]
