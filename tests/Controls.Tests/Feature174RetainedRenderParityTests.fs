...
let tests =
    testList
        "Feature174 retained render-result parity"
        [
            test "button activation keeps visual metadata, event binding, bound id, and diagnostics parity" {
                ...
                Feature174RetainedRenderFixtures.assertRenderMetadataParity
                    "button activation"
                    step.Render
                    (Feature174RetainedRenderFixtures.direct after)
            ...
            test "duplicate-key diagnostics remain byte-equivalent through retained metadata" {
                ...
                Expect.equal
                    step.Render.Diagnostics
                    direct.Diagnostics
                    "duplicate-key diagnostics are retained from metadata"
            }
            ...
            test "theme changes keep retained metadata parity while repainting the whole tree" {
                ...
                let direct =
                    Control.renderTree FS.GG.UI.Themes.Default.Theme.dark Feature174RetainedRenderFixtures.size tree

                Feature174RetainedRenderFixtures.assertRenderMetadataParity "theme change" step.Render direct

                Expect.equal
                    step.WorkReduction.RepaintedNodeCount
                    step.WorkReduction.BaselineNodeCount
                    "theme change still repaints all nodes"
            }
        ]
