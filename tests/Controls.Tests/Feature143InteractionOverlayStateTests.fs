...
let tests =
    testList
        "Feature143 open and dismiss overlay state"
        [
            ...
            test "disabled triggers do not open transient surfaces" {
                ...
                let state, effects =
                    OverlayState.update (OpenRequested closed) (OverlayState.init ())

                Expect.isEmpty state.OpenSurfaces "disabled trigger leaves overlay closed"

                Expect.exists
                    state.Diagnostics
                    (fun d -> d.Code = DisabledOverlayTrigger)
                    "diagnostic names disabled trigger"
