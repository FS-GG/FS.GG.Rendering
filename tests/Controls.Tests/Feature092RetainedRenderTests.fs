        ...
        [
            test "a sibling-shifting insert: RecomputedNodeCount = ChangedSubtreeBound + ShiftedNodeCount < Baseline" {
                ...
                Expect.isLessThan
                    w.RecomputedNodeCount
                    w.BaselineNodeCount
                    "SC-003: recomputed < baseline (work bounded)"
                ...
                Expect.isGreaterThan
                    ...
                    "the 091 `recomputed <= changed` doc cannot survive a shift"
            }
        ]
...
let themeReuse =
    testList
        ...
        [
            test
                "a theme change between frames repaints: second frame is byte-identical to a full rebuild under the new theme" {
                ...
                Expect.notEqual
                    s.Render.Scene
                    lightRebuild.Scene
                    "no fragment painted under the OLD (light) theme is reused"
            ...
            test "no theme change: an identical tree still reuses everything (no spurious repaint)" {
                ...
                Expect.equal
                    ...
                    0
                    "same theme + identical tree => zero recompute (identity-at-rest preserved)"
            }
        ]
        ...
        [
            test "init paints the first frame once: its Render is byte-identical to a full rebuild (no double paint)" {
                ...
                Expect.equal
                    i.Render.Scene
                    full.Scene
                    "first-frame Render.Scene == full rebuild (the adapter reuses this, no second renderTree)"
    ...
    testList
        ...
        [
            test "two unkeyed same-kind siblings resolve to DISTINCT RetainedIds (no shared-id collapse)" {
                ...
                let id0 =
                    RetainedRender.retainedHitTest (box0.X + box0.Width / 2.0) (box0.Y + box0.Height / 2.0) r0

                let id1 =
                    RetainedRender.retainedHitTest (box1.X + box1.Width / 2.0) (box1.Y + box1.Height / 2.0) r0
    ...
    testList
        ...
        [
            test "a chained sequence of 4 frames stays byte-identical to a full rebuild at every step (SC-004)" {
                ...
                Expect.equal
                    s2.Render.Scene
                    (Control.renderTree theme size f2).Scene
                    "frame 2 == full rebuild (chained)"

                Expect.equal
                    s3.Render.Scene
                    (Control.renderTree theme size f3).Scene
                    "frame 3 == full rebuild (chained)"
            }
        ]
    ...
    testList
        ...
        [
            test "capture work-reduction (SC-003) + theme-reuse (SC-006) + first-frame (SC-005) + multi-frame (SC-004)" {
                ...
                File.WriteAllText(
                    ...
                    String.concat
                        ...
                        [
                            ...
                            sprintf
                                "frame1-differs-from-light-rebuild=%b"
                                (ts.Render.Scene <> (Control.renderTree Theme.light size tree).Scene)
                ...
                let frame0Collisions =
                    dupInit.Diagnostics |> List.filter (fun d -> d.Code = KeyCollision)
