...
let private rect (x: float) (paint: Paint) : SceneNode =
    PaintedRectangle(
        {
            X = x
            Y = 0.0
...
let private translucent =
    { Paint.fill Colors.black with
        Opacity = 0.4
    }

let private shadowed =
    { Paint.fill Colors.black with
        ImageFilter = DropShadow(2.0, 2.0, 4.0, Colors.black)
    }
...
let tests =
    testList
        "Feature 116 offscreen-effect diagnostic (US4, FR-011, SC-005)"
        [
            ...
            test "detector: a drop-shadow / image-filter forces offscreen (FR-011)" {
                Expect.equal
                    (RetainedRender.offscreenEffect (scene [ rect 0.0 shadowed ]))
                    (Some "drop-shadow")
                    "a drop-shadow is offscreen-forcing"
            }

            test "detector: a PathClip forces offscreen (FR-011)" {
                let clipped =
                    ClipNode(
                        PathClip(Path.create Winding [ Path.moveTo 0.0 0.0; Path.lineTo 5.0 5.0; Path.close ]),
                        { Nodes = [ rect 0.0 opaque ] }
                    )

                Expect.equal
                    (RetainedRender.offscreenEffect (scene [ clipped ]))
                    (Some "path clip")
                    "a PathClip is offscreen-forcing"
            }

            test "detector: a non-opaque paint over a multi-node group forces offscreen (FR-011)" {
                Expect.equal
                    (RetainedRender.offscreenEffect (scene [ rect 0.0 translucent; rect 20.0 opaque ]))
                    (Some "opacity group")
                    "a low-opacity paint over >1 node is offscreen-forcing"
            }

            test "detector: a plain opaque single-node scene is silent (FR-011, SC-005)" {
                Expect.equal
                    (RetainedRender.offscreenEffect (scene [ rect 0.0 opaque ]))
                    None
                    "a plain opaque scene forces no offscreen composition"
            }
            ...
            test "detector: a RectClip is the cheap label clip — NOT flagged" {
                ...
                Expect.equal
                    (RetainedRender.offscreenEffect (scene [ clipped ]))
                    None
                    "a RectClip lowers to canvas.ClipRect (no layer) → not flagged"
            }
            ...
            test "a control forcing offscreen composition surfaces an advisory diagnostic via step (FR-011, SC-005)" {
                ...
                let offscreen =
                    s.Diagnostics |> List.filter (fun d -> d.Code = OffscreenComposition)

                Expect.isNonEmpty offscreen "the offscreen-forcing chart is flagged"

                Expect.all
                    offscreen
                    (fun d -> d.Severity = ControlDiagnosticSeverity.Info)
                    "the diagnostic is advisory (Info), never build-failing"
            ...
            test "a plain control surfaces NO offscreen diagnostic and renders identically (FR-011, SC-005)" {
                ...
                Expect.isEmpty
                    (s.Diagnostics |> List.filter (fun d -> d.Code = OffscreenComposition))
                    "a plain control forces no offscreen composition"
