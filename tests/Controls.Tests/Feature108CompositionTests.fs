...
let tests =
    testList
        "Feature 108 composition (US5, FR-014/015, SC-007/008)"
        [
            test "Control.map lowers structurally equal to authoring directly in 'b (SC-007)" {
                ...
                let directB: Control<Outer> =
                    Button.create [ Button.text "x"; Button.onClick (Wrap Inc) ]
                    |> Control.withKey "b1"
            ...
            test "Control.map preserves key / structure through nested children (FR-014)" {
                ...
                let inA: Control<Msg> =
                    Control.create "stack" [ Attr.children [ child ] ] |> Control.withKey "root"
