...
let private hoverFocusOf id =
    { empty with
        HoveredControl = Some id
        FocusedControl = Some id
    }
...
let private ghostButton: Control<int> =
    Button.create [ Button.text "Nav"; Attr.styleClasses [ StyleClass.Custom "ghost" ] ]
    |> Control.withKey "g"

let private plainButton: Control<int> =
    Button.create [ Button.text "Go" ] |> Control.withKey "b"

let private textBox: Control<int> =
    TextBox.create [ TextBox.value "x" ] |> Control.withKey "t"

let private displayLabel: Control<int> =
    TextBlock.create [ TextBlock.text "static" ] |> Control.withKey "d"

[<Tests>]
let tests =
    testList
        "Feature175HoverFocus"
        [
            ...
            test "a default button shows a visible hover state (F-009: lightened fill, was a no-op)" {
                ...
                Expect.notEqual
                    (sceneOf (hoverOf "b") plainButton)
                    (sceneOf empty plainButton)
                    "hover restyles a default button"
            }
            ...
            test "a text-box shows a visible focus state" {
                Expect.notEqual
                    (sceneOf (focusOf "t") textBox)
                    (sceneOf empty textBox)
                    "text-box focus restyles its border"
            }
            ...
            test "deriveVisualState yields the combined state when both hovered and focused" {
                Expect.equal
                    (ControlRuntime.deriveVisualState (hoverFocusOf "b") "b")
                    FocusedHover
                    "both → FocusedHover (not just Focused)"
            ...
            test "a display-only control stays static under a hover+focus model" {
                ...
                Expect.equal
                    (sceneOf (hoverFocusOf "d") displayLabel)
                    (sceneOf empty displayLabel)
                    "text-block is inert under input"
            }
            ...
            test "a hover change re-stamps damage-locally — not the whole tree" {
                let tree: Control<int> =
                    Stack.create [ Stack.children [ plainButton; ghostButton ] ]

                let stamp0 = ControlRuntime.runtimeStampFor None empty tree

                let stamp1 =
                    ControlRuntime.runtimeStampFor (Some(empty, stamp0.Stamped)) (hoverOf "g") tree

                Expect.isGreaterThan
                    stamp1.RuntimeStateTouchedNodeCount
                    0
                    "the hover change re-stamped the hovered control"

                Expect.isLessThan
                    stamp1.RuntimeStateTouchedNodeCount
                    (Control.count tree)
                    "the re-stamp is damage-local (fewer than all nodes touched)"
            }
        ]
