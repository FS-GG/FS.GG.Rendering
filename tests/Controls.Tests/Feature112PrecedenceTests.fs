...
let tests =
    testList
        "Feature 112 visual-state precedence preserved under targeting (US2, FR-003/SC-004)"
        [
            ...
            test "a consumer-set Disabled control keeps Disabled when hover moves onto it (FR-003)" {
                ...
                let r =
                    ControlRuntime.applyRuntimeVisualStateTargeted prevModel curModel (stampedUnder prevModel tree) tree

                Expect.equal
                    (findState "d" r.Stamped)
                    (Some Disabled)
                    "the consumer-set Disabled state wins over the derived Hover (SC-004)"
                // "d"'s final state is Disabled under both models -> it is never re-stamped (only "a" changes).
                Expect.equal
                    r.RuntimeStateTouchedNodeCount
                    2
                    "only 'a' (Hover->Normal) + the root are rebuilt; the Disabled 'd' is untouched"
            }
            ...
            test "a consumer-set Selected control is not overridden by a derived focus" {
                let selTree: Control<int> =
                    Stack.create
                        [
                            Stack.children
                                [
                                    Button.create [ Button.text "s"; Attr.visualState Selected ]
                                    |> Control.withKey "s"
                                    Button.create [ Button.text "o" ] |> Control.withKey "o"
                                ]
                        ]
                ...
                let r =
                    ControlRuntime.applyRuntimeVisualStateTargeted
                        prevModel
                        curModel
                        (stampedUnder prevModel selTree)
                        selTree
            ...
            test "a derived Normal emits NO visualState attribute (byte-identity at rest, FR-008)" {
                ...
                Expect.equal
                    (findState "a" r.Stamped)
                    (Some Normal)
                    "a control reverting to Normal carries no derived visualState (Normal)"
            }
        ]
