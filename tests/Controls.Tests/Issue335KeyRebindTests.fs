    ...
    testList
        ...
        [
            ...
            test "capture→rebind shows the new key and re-routes it in live dispatch (R3, #333)" {
                ...
                Expect.equal
                    (mapKey (Letter 'j') true)
                    (Some(Command "MoveUp"))
                    "'j' now routes to MoveUp in live dispatch"
            ...
            test "onRebind dispatches the activated command id from the event payload" {
                ...
                Expect.equal
                    (Control.dispatch ev control)
                    [ Command "MoveUp" ]
                    "onRebind fires the activated command id"
            }
            ...
            test "catalog-aware rebind maps the player label back to the stable command id" {
                ...
                let control =
                    KeyRebind.ofActions catalog [ KeyRebind.onActionRebind catalog Command ]
                ...
                Expect.equal
                    (Control.dispatch ev control)
                    [ Command "move-up" ]
                    "display text never leaks into the runtime command id"
            }
        ]
