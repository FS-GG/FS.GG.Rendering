...
let tests =
    testList
        "Feature 114 overscan parity + opt-in correctness (US2, SC-002/SC-003)"
        [

            test
                "overscan 0 realizes exactly the historic visible slice — same keys, byte-identical scene (FR-006, SC-002)" {
                let total = 10000
                ...
                let expectedKeys =
                    [ for i in vr0.FirstIndex .. vr0.FirstIndex + vr0.Count - 1 -> sprintf "r%d" i ]
                ...
                Expect.equal
                    (rowKeys grid)
                    expectedKeys
                    "overscan-0 realizes exactly the contiguous historic slice (no fabricated/missing rows)"
            ...
            test "opt-in overscan N adds only real, contiguous adjacent rows; visible region unchanged (FR-007, SC-003)" {
                ...
                Expect.equal
                    (vrN.FirstIndex + vrN.Count)
                    (vr0.FirstIndex + vr0.Count + n)
                    "trailing edge extended by exactly N real rows"

                let keysN = rowKeys (gridFor total vrN)

                let expectedN =
                    [ for i in vrN.FirstIndex .. vrN.FirstIndex + vrN.Count - 1 -> sprintf "r%d" i ]

                Expect.equal
                    keysN
                    expectedN
                    "all materialized rows are real, contiguous logical rows (no duplicates/fabrication)"
                ...
                Expect.isTrue
                    (visibleKeys |> List.forall (fun k -> List.contains k keysN))
                    "every original visible row is still materialized"
            }
            ...
            test "opt-in overscan is edge-clamped: no row index < 0 or >= Total, no fabricated rows (FR-007)" {
                ...
                Expect.isTrue
                    (keysTop |> List.forall (fun k -> k <> "r-1"))
                    "no negative-index row fabricated at the top edge"

                Expect.equal keysTop.Head "r0" "top edge starts at the first real row"

                // bottom edge
                let vrBot =
                    Collections.visibleRange rowHeight viewport (float total * rowHeight) total n

                let keysBot = rowKeys (gridFor total vrBot)

                Expect.isTrue
                    (keysBot |> List.forall (fun k -> k <> sprintf "r%d" total))
                    "no past-the-end row fabricated at the bottom edge"
            ...
            test "keyed rows are stable across a scroll — same key reused where the window overlaps (FR-008)" {
                ...
                Expect.isNonEmpty
                    overlap
                    "a one-row scroll keeps most realized row keys stable (keyed reuse is possible)"
            }
        ]
