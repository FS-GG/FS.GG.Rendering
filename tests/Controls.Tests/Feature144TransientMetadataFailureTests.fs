...
let tests =
    testList
        "Feature144 transient metadata failure modes"
        [
            test "disabled triggers suppress open activation requests" {
                let disabled =
                    metadata TransientSurfaceKind.Menu "disabled-menu" 10 false false false
                ...
                Expect.equal
                    (request.Diagnostic |> Option.map _.Code)
                    (Some DisabledOverlayTrigger)
                    "diagnostic is classified"
            }

            test "open metadata without current-frame anchor evidence is a readiness failure" {
                let current =
                    metadata TransientSurfaceKind.DatePickerCalendar "missing-anchor" 60 true true false

                let diagnostics =
                    TransientWidget.validate (Some(missingAnchor current.AnchorId)) current

                Expect.exists
                    diagnostics
                    (fun diagnostic -> diagnostic.Code = MissingOverlayAnchor)
                    "missing anchor is explicit"
            }

            test "disabled open request does not mutate overlay state" {
                let disabled =
                    metadata TransientSurfaceKind.ComboDropdown "disabled-combo" 40 true false false
                ...
                Expect.exists
                    effects
                    (function
                    | ReportOverlayDiagnostic diagnostic when diagnostic.Code = DisabledOverlayTrigger -> true
                    | _ -> false)
                    "diagnostic effect emitted"
            }
        ]
