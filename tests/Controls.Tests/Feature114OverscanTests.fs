...
let tests =
    testList
        "Feature 114 overscan + bounded materialization (US1, SC-001)"
        [
            ...
            test "realized window does NOT scale with total: identical Count at 100/1000/10000 (FR-003, SC-001)" {
                ...
                Expect.allEqual
                    counts
                    visibleCount
                    "overscan-0 realized Count is identical (=V) across totals — does not scale"
            }
            ...
            test "with overscan N a mid-list window realizes exactly V + 2N real rows (FR-007)" {
                ...
                for total in [ 100; 1000; 10000 ] do
                    ...
                    Expect.equal
                        vr.Count
                        (visibleCount + 2 * n)
                        (sprintf "total %d: Count = V + 2N when away from edges" total)
                    ...
                    Expect.equal
                        ...
                        vr.Count
                        (sprintf "total %d: materialized rows == realized Count" total)
            }
            ...
            test "materialized count is identical across totals at a fixed overscan (non-scaling, SC-001)" {
                ...
                Expect.allEqual
                    materialized
                    (visibleCount + 2 * n)
                    "materialized count bounded by V+2N and identical across totals"
            }
