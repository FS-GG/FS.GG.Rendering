...
let private step (prev: RetainedRender<int>) (next: Control<int>) =
    RetainedRender.step theme size prev next

[<Tests>]
let tests =
    testList
        "Feature 113 memo-on ≡ memo-off parity + no staleness (US2, FR-006/FR-007, C5/C6)"
        [
            ...
            test "no staleness: changing the grid's real inputs forces a Miss and a fresh, different scene (C6/FR-007)" {
                ...
                Expect.notEqual
                    sChanged.Render.Scene
                    sStable.Render.Scene
                    "the changed inputs produce a different scene (no staleness)"
                ...
                let offChanged =
                    step offStable.Retained (viewTree "r2" [ "Name"; "Qty"; "ZZZ"; "9" ])

                Expect.equal
                    sChanged.Render.Scene
                    offChanged.Render.Scene
                    "the memo-on changed scene equals the memo-off build (dependency captures the change)"
            }
        ]
