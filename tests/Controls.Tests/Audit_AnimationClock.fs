...
let private ownScene: Scene list =
    [ Scene.rectangle (0.0, 0.0, 10.0, 10.0) Colors.black ]
...
let tests =
    testList
        "Audit: Animation clock determinism + clamp + redraw gating (FR-007)"
        [
            ...
            test
                "Audit: replaying an identical injected-delta sequence reproduces identical state (>=1000 cases) (FR-007)" {
            ...
            test "Audit: a very-large delta clamps Elapsed to the duration and settles at the endpoint (NO overshoot)" {
                ...
                Expect.equal
                    ...
                    1.0
                    "the sampled opacity is exactly the End value — no overshoot past the endpoint"
            ...
            test "Audit: clockActive gates redraw — TRUE while in flight, FALSE once settled (DISCRIMINATING)" {
                ...
                Expect.isFalse
                    (RetainedRender.clockActive settled)
                    "a settled (clamped) clock is inactive — gating stops the redraw"
            }
            ...
            test "Audit: sampleOnPaint is a pure deterministic function of (clock, ownScene)" {
                ...
                Expect.notEqual
                    (sampledOpacity earlier)
                    (sampledOpacity active)
                    "distinct Elapsed ⇒ distinct sampled opacity (the sampler is sensitive, not constant)"
            }
        ]
