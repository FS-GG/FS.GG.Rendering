...
let tests =
    testList
        "Feature 117 bounded text-measure cache (US1, FR-003, SC-005)"
        [
            ...
            test "Entries.Count never exceeds the cap, even under eviction pressure (FR-003/SC-005)" {
                ...
                for i in 0 .. overCap - 1 do
                    ...
                    Expect.isTrue
                        (c.Entries.Count <= cap)
                        (sprintf
                            "after %d inserts the entry count %d is bounded by the cap %d"
                            (i + 1)
                            c.Entries.Count
                            cap)
            ...
            test
                "eviction is deterministic: same input order → same surviving entries + same hit/miss sequence (FR-003/SC-005)" {
                let run () =
