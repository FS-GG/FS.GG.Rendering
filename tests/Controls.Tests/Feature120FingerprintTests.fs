...
let tests =
    testList
        "Feature 120 structural fingerprint + damage union (US3/US4, FR-008/010/015)"
        [
            ...
            test "identical scenes hash identically; the fingerprint is deterministic (FR-008)" {
                ...
                Expect.equal
                    ...
                    "equal scenes ⇒ equal fingerprint"
            ...
            test
                "the key proof: a long-list difference the truncating %A digest collides on yields a different fingerprint (SC-005)" {
                ...
                let values i =
                    [ for k in 0..199 -> if k = 150 then float i else float k ]
                ...
                Expect.equal
                    truncatedA
                    truncatedB
                    "the superseded `%A` digest TRUNCATES and collides on the two long lists"

                // the new fingerprint does not
                Expect.notEqual
                    (RetainedRender.hashScene sceneA)
                    (RetainedRender.hashScene sceneB)
                    "hashScene distinguishes the structural difference the truncating key missed (SC-005)"
            }

            test
                "DirtyArea is the UNION of overlapping damage rects, never the sum, never over the frame (FR-015/SC-007)" {
                ...
                Expect.equal
                    (CompositorPolicy.unionArea [ huge ] frameArea)
                    frameArea
                    "union never exceeds the frame area"
