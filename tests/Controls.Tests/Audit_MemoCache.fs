...
let private scene (tag: float) : Scene list =
    [ Scene.rectangle (tag, 0.0, 10.0, 10.0) Colors.black ]
...
let private dataGrid (items: string list) : Control<int> =
    {
        ...
        Attributes =
            [
                {
                    Name = "items"
                    Category = AttrCategory.Data
                    Value = StringListValue items
                }
                {
                    Name = "width"
                    Category = AttrCategory.Style
                    Value = FloatValue 220.0
                }
                {
                    Name = "height"
                    Category = AttrCategory.Style
                    Value = FloatValue 140.0
                }
            ]
        Children = []
        Content = None
...
let private step prev next =
    RetainedRender.step theme size prev next

[<Tests>]
let tests =
    testList
        "Audit_MemoCache: Memo cache parity + key-completeness + effectiveness (FR-004/008/009)"
        [
            ...
            test "Audit: Memo scaffold reachability — MemoEnabled + MemoHits/MemoMisses (T004)" {
                ...
                let off =
                    { init.Retained with
                        MemoEnabled = false
                    } // oracle field reachable

                let s = step off (viewTree "r1" [ "a"; "1" ])

                Expect.isTrue
                    (s.WorkReduction.MemoHits >= 0 && s.WorkReduction.MemoMisses >= 0)
                    "memo counters reachable"
            ...
            test "Audit: memo-on ≡ memo-off renders byte-identical scenes (FR-004)" {
                ...
                let render seed =
                    frames
                    |> List.fold
                        (fun (prev, acc) next -> let s = step prev next in s.Retained, acc @ [ s.Render.Scene ])
                        (seed, [])
                    |> snd
            ...
            test
                "Audit: DISCRIMINATING — a memo HIT returns the OLD subtree even if compute changed (proves parity has teeth)" {
                ...
                Expect.notEqual
                    hit
                    (scene 2.0)
                    "a stale hit diverges from the fresh compute — the parity oracle is discriminating"
            }
            ...
            test "Audit: cache-key completeness — a changed dependency does NOT collide (Miss) (FR-009)" {
                ...
                let _, cache1, _ =
                    RetainedRender.memoize "grid" ([ "Name"; "A"; "1" ] :> obj) (fun () -> scene 1.0) Map.empty

                let _, _, o =
                    RetainedRender.memoize "grid" ([ "Name"; "A"; "2" ] :> obj) (fun () -> scene 2.0) cache1
                ...
                Expect.notEqual
                    changed.Render.Scene
                    stable.Render.Scene
                    "the changed inputs render a different scene (no staleness)"
            }

            // ---- T030 EFFECTIVENESS: steady-state hit-rate ≫ disabled baseline ----
            test
                "Audit: EFFECTIVENESS — repeated unchanged render drives MemoHits→~100%, misses→0 vs disabled baseline (T030)" {
                ...
                Expect.equal
                    offHits
                    0
                    "the disabled baseline produces ZERO memo hits (no reuse — the effectiveness margin)"
                ...
                Expect.equal
                    offMisses
                    0
                    "FINDING: disabled bypasses the memo seam — recomputes are UNCOUNTED (MemoMisses stays 0), not tallied as misses"

                Expect.equal onMisses 0 "the enabled steady-state stops missing (unchanged data ⇒ all hits)"

                Expect.isTrue
                    (onRate > 0.95)
                    (sprintf "enabled steady-state hit-rate is near-100%% (got %.3f over %d frames)" onRate frameCount)
                // Recorded margin: onHits/onMisses vs offHits (printed to the run log for the report).
                printfn
                    "AUDIT-MARGIN MemoCache: enabled hits=%d misses=%d rate=%.3f | disabled hits=%d misses=%d (disabled bypasses seam: recomputes uncounted)"
                    onHits
                    onMisses
                    onRate
                    offHits
