...
let tests =
    testList
        "Feature144 compatibility contract"
        [
            ...
            test "overlay bridge dispatch records are derived from effects" {
                let opened, effects =
                    openOne (metadata TransientSurfaceKind.Menu "compat-menu" 10 true true false)
                ...
                Expect.exists
                    records
                    (fun record -> record.Kind = "request-open-state-change" && record.ProductVisible)
                    "open request is product-visible"

                Expect.exists
                    records
                    (fun record -> record.Kind = "request-focus" && record.ProductVisible)
                    "focus request is product-visible"

                Expect.isFalse
                    (records |> List.exists (fun record -> record.Kind = "selected-state"))
                    "selection state is not runtime-owned"
            }
            ...
            test "metadata surface changes are additive to existing coordinator contract" {
                ...
                Expect.equal
                    surface.DismissalPolicy.SelectionCompletion
                    CloseOnSelection
                    "default selection policy preserved"
