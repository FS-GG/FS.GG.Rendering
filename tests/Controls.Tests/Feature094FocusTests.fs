...
let private focusable (kind: string) (key: string) (fo: int option) : Control<int> =
    Control.create
        ...
        [
            Attr.text key
            Attr.accessibility (Accessibility.defaultFor kind key |> withFocusOrder fo)
        ]
    |> Control.withKey key
...
let private ids (o: TabOrder) =
    o.Stops |> List.map (fun s -> s.Control)
...
let tabOrderTests =
    testList
        "094 US1 tab order (Focus.order, SC-001)"
        [
            test "order yields focusable-only stops in (FocusOrder ascending, None last, doc-order) order" {
                ...
                Expect.equal
                    (ids order)
                    [ "act-0"; "nav-1"; "act-none" ]
                    "stops are focusable-only and FocusOrder-then-doc ordered"
            ...
            test "a focusable composite is a SINGLE stop — its subtree is not descended" {
                            ...
                            Stack.children
                                [
                                    Control.create
                                        ...
                                        [
                                            Attr.items [ "a"; "b"; "c" ]
                                            Attr.accessibility (Accessibility.defaultFor "radio-group" "rg")
                                        ]
                                    |> Control.withKey "rg"
                                    button "b1" None
                                ]
                ...
                Expect.equal
                    (ids order)
                    [ "rg"; "b1" ]
                    "the composite contributes one stop; descendants are not separate stops"
            }
...
let traversalTests =
    testList
        "094 US1 traversal (Focus.traverse, SC-001)"
        [
            ...
            test "Previous reverses and wraps cyclically at the start" {
                Expect.equal
                    (Focus.traverse order (Some "nav-1") Previous)
                    (Some "act-0")
                    "retreats to the previous stop"
            ...
            test "Next then Previous is identity" {
                for start in [ "act-0"; "nav-1"; "act-none" ] do
                    ...
                    Expect.equal
                        (Focus.traverse order next Previous)
                        (Some start)
                        $"Next then Previous returns to {start}"
            }
            ...
            test "a stale current id recovers to first (Next) / last (Previous), never throws" {
                ...
                Expect.equal
                    (Focus.traverse order (Some "removed") Previous)
                    (Some "act-none")
                    "stale + Previous -> last"
            }
        ]
...
let routeTests =
    testList
        "094 US2 key routing (Focus.route, SC-002/FR-007)"
        [
            ...
            test "ActivationKeys -> Activate" {
                Expect.equal
                    (Focus.route AccessibilityRole.Button buttonKb None "Enter" false false)
                    Activate
                    "Enter activates"

                Expect.equal
                    (Focus.route AccessibilityRole.Button buttonKb None "Space" false false)
                    Activate
                    "Space activates"
            }
            ...
            test "NavigationKeys -> Navigate (R5: carries a role-derived NavIntent)" {
                Expect.equal
                    (Focus.route AccessibilityRole.Slider sliderKb sliderRange "ArrowLeft" false false)
                    (Navigate(ValueStep -0.1))
                    "ArrowLeft -> value step down"

                Expect.equal
                    (Focus.route AccessibilityRole.Slider sliderKb sliderRange "ArrowRight" false false)
                    (Navigate(ValueStep 0.1))
                    "ArrowRight -> value step up"
            }

            test "an unconsumed Tab -> Traverse (Next, or Previous with shift)" {
                Expect.equal
                    (Focus.route AccessibilityRole.Button buttonKb None "Tab" true false)
                    (Traverse Next)
                    "Tab -> Traverse Next"

                Expect.equal
                    (Focus.route AccessibilityRole.Button buttonKb None "Tab" true true)
                    (Traverse Previous)
                    "Shift+Tab -> Traverse Previous"
            }
            ...
            test "consumption wins: a control listing a traversal key consumes it (never Traverse)" {
                ...
                Expect.equal
                    (Focus.route AccessibilityRole.Button tabConsuming None "Tab" true false)
                    Activate
                    "a control whose ActivationKeys include Tab activates on Tab (not Traverse)"
            }

            test "no match -> Fallthrough (never throws)" {
                Expect.equal
                    (Focus.route AccessibilityRole.Button buttonKb None "Q" false false)
                    Fallthrough
                    "an unmatched non-Tab key falls through"
            }
        ]
...
let r1CorrectionTests =
    testList
        "094 R1 Accessibility correction (defaultFor / validate)"
        [
            test "a focusable activation-only Button is VALID and does NOT seed Tab into NavigationKeys" {
                ...
                Expect.isFalse
                    (List.contains "Tab" md.Keyboard.NavigationKeys)
                    "Tab is NOT a per-control navigation key (traversal is engine-level)"

                Expect.isFalse
                    (List.contains "Shift+Tab" md.Keyboard.NavigationKeys)
                    "Shift+Tab is NOT a per-control navigation key"
                ...
                let errors =
                    ...
                    |> List.filter (fun d -> d.Severity = ControlDiagnosticSeverity.Error)
...
let propertyTests =
    testList
        "094 properties (FsCheck, SC-006)"
        [
            testCase
                "route is deterministic and total — identical inputs, identical verdict, never throws (>=1000)"
                (fun () ->
                    ...
                    let prop (kb, key, isTab, shift) =
                        let safe () =
                            try
                                Focus.route AccessibilityRole.Button kb None key isTab shift =
                                    Focus.route AccessibilityRole.Button kb None key isTab shift
                            with _ ->
                                false
