...
let private stampedUnder model fresh =
    ControlRuntime.applyRuntimeVisualState model fresh

let private btn (k: string) : Control<int> =
    Button.create [ Button.text "Go" ] |> Control.withKey k
...
let private unkeyedSiblings: Control<int> =
    Stack.create
        [
            Stack.children [ Button.create [ Button.text "x" ]; Button.create [ Button.text "y" ] ]
        ]
    ...
    Stack.create
        [
            Stack.children
                [
                    btn "a"
                    Button.create [ Button.text "off"; Attr.visualState Disabled ]
                    |> Control.withKey "d"
                ]
        ]

// Assert the targeted stamp's scene equals the full-tree oracle's, for a prev->cur transition.
let private assertParity
    (name: string)
    (fresh: Control<int>)
    (prevModel: ControlRuntimeModel)
    (curModel: ControlRuntimeModel)
    =
    let prevStamped = stampedUnder prevModel fresh

    let targeted =
        ControlRuntime.applyRuntimeVisualStateTargeted prevModel curModel prevStamped fresh

    let oracle = stampedUnder curModel fresh

    Expect.equal
        (sceneOf targeted.Stamped)
        (sceneOf oracle)
        (sprintf "%s: targeted scene == full-tree oracle scene (FR-005)" name)

[<Tests>]
let tests =
    testList
        "Feature 112 targeted stamp is scene-identical to the full-tree oracle (US2, FR-005/SC-002)"
        [
            ...
            test "hover move parity over flat / nested / consumer-set trees" {
                assertParity
                    "flat hover b1->b6"
                    flat
                    (modelWith (fun m -> { m with HoveredControl = Some "b1" }))
                    (modelWith (fun m -> { m with HoveredControl = Some "b6" }))

                assertParity
                    "nested hover a->d"
                    nested
                    (modelWith (fun m -> { m with HoveredControl = Some "a" }))
                    (modelWith (fun m -> { m with HoveredControl = Some "d" }))

                assertParity
                    "consumer-set hover a->d(Disabled)"
                    consumerSet
                    (modelWith (fun m -> { m with HoveredControl = Some "a" }))
                    (modelWith (fun m -> { m with HoveredControl = Some "d" }))
            }
            ...
            test "focus move parity over flat / nested trees" {
                assertParity
                    "flat focus b0->b7"
                    flat
                    (modelWith (fun m -> { m with FocusedControl = Some "b0" }))
                    (modelWith (fun m -> { m with FocusedControl = Some "b7" }))

                assertParity
                    "nested focus b->c"
                    nested
                    (modelWith (fun m -> { m with FocusedControl = Some "b" }))
                    (modelWith (fun m -> { m with FocusedControl = Some "c" }))
            }
            ...
            test "at-rest parity: an emptyModel->emptyModel stamp equals the un-bridged build (FR-008)" {
                ...
                let targeted =
                    ControlRuntime.applyRuntimeVisualStateTargeted emptyModel emptyModel prevStamped flat

                Expect.equal
                    (sceneOf targeted.Stamped)
                    (sceneOf flat)
                    "at rest the stamp emits nothing — byte-identical to the un-stamped tree (FR-008)"
            }
            ...
            test "runtimeStampFor picks the targeted route on a model-unchanged frame (scene == oracle)" {
                ...
                let routed =
                    ControlRuntime.runtimeStampFor (Some(prevModel, prevStamped)) curModel flat

                let oracle = stampedUnder curModel flat

                Expect.equal
                    (sceneOf routed.Stamped)
                    (sceneOf oracle)
                    "the targeted route's scene equals the oracle (FR-002)"

                Expect.isTrue
                    (routed.RuntimeStateTouchedNodeCount < Control.count flat)
                    "the targeted route touches fewer than all nodes"
            }
            ...
            test "runtimeStampFor falls back to the full-tree oracle on a first/model-changing frame (prior = None)" {
                ...
                Expect.equal
                    (sceneOf routed.Stamped)
                    (sceneOf oracle)
                    "the oracle route's scene equals the full-tree stamp (FR-006)"

                Expect.equal
                    routed.RuntimeStateTouchedNodeCount
                    (Control.count flat)
                    "the oracle route counts the whole tree"
            }
        ]
