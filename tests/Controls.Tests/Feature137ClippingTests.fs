...
let tests =
    testList
        "Feature137 container clipping (US1, FR-001/002/003)"
        [
            ...
            test "composeContainerScene composes flat for a leaf (no children) and a box-less node" {
                ...
                Expect.equal
                    (ControlInternals.composeContainerScene None own child)
                    (own @ child)
                    "box-less composes flat"
                                                    ...
                                                    Stack.create
                                                        [
                                                            ...
                                                            Stack.children
                                                                [
                                                                    TextBlock.create
                                                                        [ TextBlock.text "overflowing child" ]
                                                                ]
                                                        ]
                                                ]
                                        ]
                                ...
                                [
                                    ...
                                    Stack.create
                                        [
                                            ...
                                            Stack.children
                                                [ Button.create [ Button.text "A" ]; Button.create [ Button.text "B" ] ]
                                        ]
...
let private tallScrollViewer () : Control<Msg> =
    ...
    let rows =
        [ for i in 1..40 -> TextBlock.create [ TextBlock.text (sprintf "row %d" i) ] ]

    let content = Stack.create [ Stack.children rows ]

    let sv =
        Control.create "scroll-viewer" [ Attr.children [ content ] ]
        |> Control.withKey "sv"
...
let scrollTests =
    testList
        "Feature137 ScrollViewer viewport (US3, FR-008)"
        [

            // ---- T018: viewport clips content, exposes a scroll offset, renders an affordance ----
            test
                "a scroll-viewer clips overflowing content to its box, exposes a scroll offset, and shows an affordance" {
                ...
                | Some vp ->
                    Expect.isTrue
                        (vp.ContentHeight > vp.Viewport.Height)
                        "content is taller than the viewport (scrollable)"
                ...
                Expect.isLessThan
                    (List.min heights)
                    (List.max heights)
                    "the thumb is shorter than the track (content overflows)"
            }
            ...
            test "a page taller than its content region is bounded by the viewport clip (nothing spills)" {
                ...
                let lastRowBottom =
                    rendered.Bounds |> List.map (fun (_, (r: Rect)) -> r.Y + r.Height) |> List.max
...
let private clipsFor (text: string) (s: Scene) : Rect list =
    textClipPaths [] s
    |> List.filter (fun (t, _) -> t.Contains text)
    |> List.collect snd
...
let overlayTests =
    testList
        "Feature137 overlay pass (US2, FR-004/005/006/007)"
        [
            ...
            test "an overlay paints above its in-flow sibling and escapes its ancestor container clip" {
                ...
                let idxOf (needle: string) =
                    glyphs |> List.findIndex (fun (g: string) -> g.Contains needle)

                Expect.isGreaterThan
                    (idxOf "OVERLAYITEM")
                    (idxOf "INFLOWSIB")
                    "the overlay item paints after (above) the in-flow sibling"

                // the in-flow sibling IS confined by the wrap container clip ...
                Expect.isTrue
                    (clipsFor "INFLOWSIB" rendered.Scene
                     |> List.exists (fun r -> rectClose r wrapBox))
                ...
                Expect.isFalse
                    (clipsFor "OVERLAYITEM" rendered.Scene
                     |> List.exists (fun r -> rectClose r wrapBox))
            ...
            test "hitTest/nearestAuthored prefer the overlay (its bounds are consulted before in-flow)" {
                ...
                Expect.isGreaterThan
                    (idxOf "ov")
                    (idxOf "0.0.0")
                    "the overlay surface is consulted after the in-flow sibling"
                ...
                | Some hit ->
                    Expect.equal
                        (Control.nearestAuthored rendered hit)
                        (Some "ov")
                        "the hit resolves to the overlay surface"
            ...
            test "full ≡ retained with an overlay present; the in-flow pass is unchanged without one" {
                ...
                let stepped =
                    (RetainedRender.step theme size inited.Retained withOverlay).Render.Scene
                        ...
                        [
                            Stack.children
                                [
                                    ...
                                    TextBlock.create [ TextBlock.text "b" ]
                                ]
                        ]
                ...
                Expect.equal
                    plainRetained
                    plainFull
                    "an overlay-free page renders byte-identically (empty overlay group)"
            }
        ]
