...
let private twoButtons =
    layoutOf
        [
            ...
            "B", bounds 100.0 0.0 100.0 40.0, Visible
        ]
...
let private run layout msgs =
    Pointer.replay policy layout msgs (Pointer.init ())
    ...
    testList
        ...
        [
            test "moving from A to B emits leave A then enter B in order" {
                let _, effects = run twoButtons [ PointerMsg.Move inA; PointerMsg.Move inB ]
            ...
            test "moving within the same control emits no redundant transition" {
                let _, effects =
                    run twoButtons [ PointerMsg.Move(50.0, 20.0); PointerMsg.Move(60.0, 25.0) ]
            ...
            test "moving to empty space emits leave only" {
                ...
                Expect.equal
                    effects
                    [ HoverEnter("A", 50.0, 20.0); HoverLeave "A" ]
                    "leave with no enter on empty space"
            }
    ...
    testList
        ...
        [
            ...
            test "release off the pressed control dispatches no click and clears the press" {
                let state, effects =
                    run
                        ...
                        [
                            PointerMsg.Down(PointerButton.Primary, 50.0, 20.0)
                            PointerMsg.Up(PointerButton.Primary, 150.0, 20.0)
                        ]

                Expect.isFalse
                    (hasClick PointerButton.Primary effects)
                    "no click when released over a different control"
            ...
            test "press over empty space emits a hit-test-miss diagnostic" {
                let _, effects =
                    run twoButtons [ PointerMsg.Down(PointerButton.Primary, 300.0, 300.0) ]
    ...
    testList
        ...
        [
            test "secondary press/release yields a secondary click and no primary click" {
                let _, effects =
                    run
                        twoButtons
                        [
                            PointerMsg.Down(PointerButton.Secondary, 50.0, 20.0)
                            PointerMsg.Up(PointerButton.Secondary, 50.0, 20.0)
                        ]
            ...
            test "middle press/release yields a distinct middle click" {
                let _, effects =
                    run
                        twoButtons
                        [
                            PointerMsg.Down(PointerButton.Middle, 50.0, 20.0)
                            PointerMsg.Up(PointerButton.Middle, 50.0, 20.0)
                        ]
            ...
            test "secondary press does not steal focus (only primary does)" {
                ...
                Expect.equal
                    interactions
                    [ PressedDown("A", PointerButton.Secondary, 50.0, 20.0) ]
                    "no focus-move interaction"
            ...
            test "overlapping primary + secondary presses resolve independently" {
                ...
                let s1, _, _ =
                    step twoButtons (PointerMsg.Down(PointerButton.Primary, 50.0, 20.0)) state0

                let s2, _, _ =
                    step twoButtons (PointerMsg.Down(PointerButton.Secondary, 60.0, 20.0)) s1

                let s3, primaryEffects, _ =
                    step twoButtons (PointerMsg.Up(PointerButton.Primary, 50.0, 20.0)) s2

                let _, secondaryEffects, _ =
                    step twoButtons (PointerMsg.Up(PointerButton.Secondary, 60.0, 20.0)) s3

                Expect.isTrue
                    (hasClick PointerButton.Primary primaryEffects)
                    "primary release resolves to a primary click"

                Expect.isFalse
                    (hasClick PointerButton.Secondary primaryEffects)
                    "primary release does not emit a secondary click"

                Expect.isTrue
                    (s3.Presses.ContainsKey PointerButton.Secondary)
                    "secondary press survives the primary release"

                Expect.isTrue
                    (hasClick PointerButton.Secondary secondaryEffects)
                    "secondary release resolves to a secondary click"
            }
        ]
    ...
    testList
        ...
        [
            test "wheel over a control emits a scroll with the signed delta" {
                ...
                Expect.equal
                    effects
                    [ Scroll("A", 0.0, -3.0, 50.0, 20.0) ]
                    "scroll addressed to A with the signed delta"
            }
