...
let private knownIntents =
    [ "primary"; "secondary"; "danger"; "ghost"; "totally-unknown-intent" ]
...
let feature130PublicSurfaceTests =
    testList
        "Feature 130 public token/resolver surface"
        [
            ...
            test "promoted StyleResolver + DesignTokensExt are reachable through the public API only (INV-4/SC-001)" {
                ...
                let viaBackHalf =
                    Style.resolve Theme.light (StyleResolver.baseStyleFor Theme.light "button") [] Normal
            ...
            test "representative promoted DesignTokensExt values equal their known literals (INV-2/SC-003)" {
                ...
                Expect.equal
                    ...
                    (Colors.rgba 31uy 41uy 55uy 255uy)
                    "Map.Light.colorText"

                Expect.equal
                    DesignTokensExt.Map.Dark.colorText
                    (Colors.rgba 241uy 245uy 249uy 255uy)
                    "Map.Dark.colorText"

                Expect.equal
                    DesignTokensExt.Alias.Light.textDefault
                    (Colors.rgba 31uy 41uy 55uy 255uy)
                    "Alias.Light.textDefault"

                Expect.equal
                    DesignTokensExt.Component.Button.primaryBg
                    (Colors.rgba 22uy 119uy 255uy 255uy)
            ...
            test
                "a neutral-policy theme resolves byte-identically to the Feature129 oracle across the cross-product (INV-3/SC-003)" {
            ...
            // ZERO control edits.
            test
                "a theme's custom public IntentPolicy makes danger diverge from the neutral resolution — no control edits (INV-5/SC-005)" {
                ...
                for kind in [ "button"; "icon-button" ] do
                    ...
                    let dangerNeutralResolved =
                        StyleResolver.resolve Theme.light kind "danger" [] Normal
