    ...
    testList
        ...
        [
            ...
            test "layout, paint, and order invalidation categories are distinct and shared with RetainedRender" {
                ...
                Expect.equal
                    (RetainedRender.classifyModifierEffect (Composition.LocalZOrder 9))
                    z
                    "retained evidence reads the composition table"
            }
            ...
            test "effect order is preserved as authored through normalization when effects do not algebraically combine" {
                ...
                let normalizedNames =
                    chain.NormalizedEffects |> List.map (fun e -> effectName e.Effect)

                Expect.equal
                    chain.NormalizedEffects.Length
                    allEffects.Length
                    "all non-identity, non-combinable effects remain present"
                ...
                Expect.stringContains
                    ...
                    "layer"
                    "last authored effect remains last"
            }
            ...
            test "malformed payloads produce diagnostics instead of disappearing silently" {
                ...
                Expect.contains
                    (chain.Diagnostics |> List.map _.Code)
                    "cache-boundary-zero"
                    "reserved cache id is diagnosed"

                Expect.contains
                    (chain.Diagnostics |> List.map _.Code)
                    "empty-layer-hint"
                    "empty layer target is diagnosed"
            }
        ]
