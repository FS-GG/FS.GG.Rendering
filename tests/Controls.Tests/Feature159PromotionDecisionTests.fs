    ...
    testList
        ...
        [
            ...
            test "Synthetic stability overhead and parity failures fail closed with stable reason tokens" {
                let observing =
                    CompositorPolicy.feature159EvaluatePromotion
                        { candidate with
                            ObservedStabilityFrames = 2
                        }
                        None
                ...
                let lowBenefit =
                    CompositorPolicy.feature159EvaluatePromotion
                        { candidate with
                            ExpectedSavedWork = 10
                            MeasuredOverhead = 12
                        }
                        None

                let parity =
                    CompositorPolicy.feature159EvaluatePromotion { candidate with ParityPassed = false } None

                Expect.equal (CompositorPolicy.feature159PromotionStatusToken observing.Status) "observing" "observing"

                Expect.equal
                    (observing.PrimaryReason |> Option.map CompositorPolicy.feature159ReasonToken)
                    (Some "instability")
                    "instability"

                Expect.equal
                    (CompositorPolicy.feature159PromotionStatusToken lowBenefit.Status)
                    "non-beneficial"
                    "non-beneficial"

                Expect.equal
                    (lowBenefit.PrimaryReason |> Option.map CompositorPolicy.feature159ReasonToken)
                    (Some "overhead-exceeds-saved-work")
                    "overhead"

                Expect.equal
                    (CompositorPolicy.feature159PromotionStatusToken parity.Status)
                    "rejected"
                    "parity rejected"

                Expect.equal
                    (parity.PrimaryReason |> Option.map CompositorPolicy.feature159ReasonToken)
                    (Some "parity-mismatch")
                    "parity"
            }
        ]
