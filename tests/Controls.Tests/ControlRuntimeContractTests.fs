...
let controlRuntimeContractTests =
    testList
        "Controls ControlRuntime contract"
        [
            ...
            test "ControlRuntime cancellation clears transient text and drag state" {
                ...
                let withCaret, _ =
                    ControlRuntime.update (SetCaret(Some { ControlId = "name"; Index = 2 })) runtime
                ...
                let composing, _ =
                    ControlRuntime.update (StartComposition("name", "a")) withSelection

                let dragging, _ = ControlRuntime.update (StartDrag("name", 0.0, 0.0)) composing

                let cancelled, effects =
                    ControlRuntime.update (CancelInteraction(Some "name")) dragging
                ...
                Expect.exists
                    effects
                    (function
                    | CancelledInteraction(Some "name") -> true
                    | _ -> false)
                    "cancel effect names control"
            }
        ]
