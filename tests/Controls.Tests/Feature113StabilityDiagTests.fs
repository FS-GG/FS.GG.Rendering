...
let private attr name cat value : Attr<int> =
    {
        Name = name
        Category = cat
        Value = value
...
let tests =
    testList
        "Feature 113 stability diagnostic (US4, FR-011/FR-012)"
        [
            ...
            test "a tree built twice with stable value attributes reports NO findings (FR-012)" {
                ...
                Expect.isEmpty
                    (Diagnostics.stabilityReport a b)
                    "stable value attributes produce no instability findings"
            }
            ...
            test "an injected always-new UntypedValue attribute is flagged (rebuilt value)" {
                let a =
                    treeWith [ attr "payload" AttrCategory.Data (UntypedValue(System.Object())) ]
                ...
                Expect.isTrue
                    (findings |> List.exists (fun f -> f.Code = UnstableReuseInput))
                    "a rebuilt UntypedValue is an instability"
            }
            ...
            test "a structurally-equal rebuilt list does NOT flag (structural equality is stable)" {
                ...
                let a =
                    treeWith
                        [
                            attr "items" AttrCategory.Data (StringListValue(List.init 3 (sprintf "i%d")))
                        ]
                ...
                Expect.isEmpty
                    (Diagnostics.stabilityReport a b)
                    "a structurally-equal rebuilt list is stable (no false positive)"
            }
            ...
            test "an unstable key on the same logical node is flagged" {
                ...
                Expect.isTrue
                    (findings |> List.exists (fun f -> f.Code = UnstableReuseInput))
                    "a changed key across builds is an instability"
            }
        ]
