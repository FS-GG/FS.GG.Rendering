...
let interactionTests =
    testList
        "Controls interaction dispatch"
        [
            test "pointer activation dispatches exactly one current-view message" {
                let view value =
                    Button.create [ Button.text "Save"; Button.onClick (Save value) ]
                    |> Control.withKey "save-button"

                Expect.equal
                    (Control.dispatch (click "save-button") (view 1))
                    [ Save 1 ]
                    "first view dispatches first model value"

                Expect.equal
                    (Control.dispatch (click "save-button") (view 2))
                    [ Save 2 ]
                    "re-rendered view dispatches current model value"
            }

            test "disabled and read-only controls suppress disallowed dispatch" {
                let disabled =
                    Button.create [ Button.text "Save"; Button.enabled false; Button.onClick (Save 1) ]
                    |> Control.withKey "save-button"

                let readOnly =
                    TextBox.create [ TextBox.value "Ada"; TextBox.readOnly true; TextBox.onChanged Changed ]
            ...
            test "keyboard activation uses the same message-oriented event path" {
                ...
                Expect.equal
                    (Control.dispatch key button)
                    [ Save 7 ]
                    "keyboard activation dispatches through current event binding"
            }
        ]
...
let typedInteractionTests =
    testList
        "Typed controls interaction dispatch"
        [
            test "typed Button OnClick dispatches the same message as legacy onClick" {
                ...
                Expect.equal
                    (Control.dispatch (click "save-button") typed)
                    [ Save 1 ]
                    "typed Button binds identically to Button.onClick"
            }
            ...
            test "typed Button with disabled state suppresses dispatch" {
                ...
                Expect.equal
                    (Control.dispatch (click "save-button") disabled)
                    []
                    "typed disabled button suppresses click"
            }
            ...
            test "typed CheckBox OnChanged maps payload identically to legacy onChanged" {
                ...
                let changed =
                    {
                        Kind = "changed"
                        ControlId = Some "agree"
                        Origin = ControlEventOrigin.Text
                        Nav = Some(SteppedValue 1.0)
                    }

                Expect.equal
                    (Control.dispatch changed typed)
                    [ Changed "on" ]
                    "typed CheckBox maps the boolean payload identically"
            }
        ]
...
let private selectedWith payload =
    {
        Kind = "selected"
        ControlId = None
        Origin = ControlEventOrigin.Selection
        Nav = Some(MovedSelection(0, Some payload))
    }
...
let private eventAttrs (control: Control<'msg>) =
    ...
    all control
    |> List.collect (fun c -> c.Attributes |> List.filter (fun a -> a.Category = Event))

[<Tests>]
let typedExpansionInteractionTests =
    testList
        "Feature 072 typed expansion interaction dispatch"
        [
            ...
            test "DatePicker with no value renders an empty calendar and dispatches nothing (T013 edge)" {
                ...
                Expect.equal
                    (Control.dispatch (clickAt "day-1") picker)
                    []
                    "no selection => no day buttons => no dispatch"
            }
            ...
            test "SplitButton dispatches OnClick for the primary action and OnSelected for a menu item (T025)" {
                ...
                Expect.equal
                    (Control.dispatch (selectedWith "copy") split)
                    [ KeyPicked "copy" ]
                    "menu item dispatches OnSelected with its key"
            }
                ...
                let picker =
                    TimePicker.view
                        { TimePicker.defaults with
                            ...
                            OnChange = Some TimePicked
                        }
                    |> Widget.toControl

                Expect.equal
                    (Control.dispatch (clickAt "hour-segment") picker)
                    [ TimePicked(TimeOnly(11, 30)) ]
                    "hour segment advances the hour"

                Expect.equal
                    (Control.dispatch (clickAt "minute-segment") picker)
                    [ TimePicked(TimeOnly(10, 31)) ]
                    "minute segment advances the minute"
            }
                ...
                let picker =
                    ColorPicker.view
                        { ColorPicker.defaults with
                            Swatches = [ red; blue ]
                            Selected = Some red
                            OnSelected = Some ColorPicked
                        }
                    |> Widget.toControl

                Expect.equal
                    (Control.dispatch (clickAt "swatch-Blue") picker)
                    [ ColorPicked blue ]
                    "clicking a swatch dispatches OnSelected with it"
