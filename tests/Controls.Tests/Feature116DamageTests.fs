...
let tests =
    testList
        "Feature 116 damage set (US1, FR-001/002/003/004, SC-001)"
        [
            ...
            test
                "localized change: one leaf's content change reports a small region, not frame-spanning (FR-001/FR-002)" {
                ...
                Expect.equal
                    s.WorkReduction.DirtyArea
                    (120 * 24)
                    "damaged area = exactly the changed leaf's box (120*24)"
            ...
            test "theme switch: all paint invalidated → every node repainted, frame-spanning (FR-002)" {
                ...
                Expect.equal
                    ...
                    total
                    "theme switch repaints every node (TotalNodeCount)"

                Expect.isTrue
                    (s.WorkReduction.DirtyArea > (120 * 24))
                    "theme-switch damage is frame-spanning (≫ a single leaf)"
            }
            ...
            test "localized vs theme: damage is proportional to the change (SC-001)" {
                ...
                let localizedAfter =
                    stack [ leaf "a" "A"; leaf "b" "B2"; leaf "c" "C"; leaf "d" "D" ]
