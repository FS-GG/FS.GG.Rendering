...
module private G =
    ...
    let private genKind: Gen<ControlKind> =
        Gen.elements [ "TextBlock"; "Button"; "Stack" ]
        ...
        gen {
            ...
            let! children =
                if size <= 0 then
                    ...
                    gen { let! n = Gen.choose (0, 3) in return! Gen.listOfLength n (genControlOf (size / 2)) }
...
let tests =
    testList
        "Audit: Reconcile diff/apply round-trip + discriminating power (FR-005, SC-003)"
        [
            ...
            test "Audit: round-trip holds on the hand-built keyed / positional / kind-mismatch corpus" {
                ...
                Expect.isTrue
                    (roundTrip
                        (parent [ leaf "a" "A"; leaf "b" "B"; leaf "c" "C" ])
                        (parent [ leaf "c" "C"; leaf "a" "A"; leaf "b" "B" ]))
                    "keyed reorder round-trips"
                // positional unkeyed change
                Expect.isTrue
                    (roundTrip
                        (parent [ node "T" None [] [] (Some "u"); node "T" None [] [] (Some "v") ])
                        (parent [ node "T" None [] [] (Some "u2"); node "T" None [] [] (Some "v") ]))
                    "positional unkeyed round-trips"
                // kind mismatch ⇒ whole-subtree replace
                Expect.isTrue
                    (roundTrip (node "Stack" None [] [ leaf "a" "A" ] None) (node "Grid" None [] [ leaf "a" "A" ] None))
                    "kind mismatch round-trips"
                // insert + remove
                Expect.isTrue
                    (roundTrip (parent [ leaf "a" "A"; leaf "b" "B" ]) (parent [ leaf "a" "A"; leaf "c" "C" ]))
            ...
            test "Audit: Feature141 retained child reorder/remove cannot leave stale rendered output" {
                ...
                Expect.equal
                    reorderStep.Render.Scene
                    (Control.renderTree theme size reordered).Scene
                    "reordered retained output equals direct output"

                let removed = view [ keyed "a" "A"; keyed "c" "C" ]
                let removalStep = RetainedRender.step theme size init.Retained removed

                Expect.equal
                    removalStep.Render.Scene
                    (Control.renderTree theme size removed).Scene
                    "removed-child retained output equals direct output"

                Expect.isFalse
                    ((sprintf "%A" removalStep.Render.Scene).Contains("\"B\""))
                    "removed child text is not left visible in retained output"
            }
            ...
            test "Audit: DISCRIMINATING — a mutated Replace patch reconstructs the wrong tree" {
                ...
                Expect.notEqual
                    (repr (canon applied))
                    (repr (canon next))
                    "a mutated Replace target does not match next (oracle catches it)"

                Expect.equal
                    (repr (canon applied))
                    (repr (canon wrongTarget))
                    "Replace faithfully reconstructs its (wrong) target"
            }
        ]
