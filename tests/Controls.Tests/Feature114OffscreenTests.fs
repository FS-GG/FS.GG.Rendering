...
let private realizedKeys (total: int) (model: DataGridModel) : string list =
    DataGrid.create columns [ DataGrid.rows (rowsUpTo total); DataGrid.visibleRange model.VisibleRange ]
    |> rowKeys
...
let tests =
    testList
        "Feature 114 offscreen addressability + boundary nav (US3, SC-004/SC-005)"
        [
            ...
            test "selecting an offscreen row records it logically WITHOUT materializing the path (FR-010, SC-004)" {
                ...
                Expect.equal
                    ...
                    (Set.singleton "r9999")
                    "the offscreen row is selected on the logical model"

                Expect.exists
                    effects
                    (function
                    | DataGridSelectionChanged [ "r9999" ] -> true
                    | _ -> false)
                    "selection emits its effect"
                ...
                Expect.equal
                    ...
                    model.VisibleRange
                    "selection does not move or expand the realized window"

                let keys = realizedKeys total selected

                Expect.isTrue
                    (keys.Length <= visibleCount)
                    "materialized count stays bounded by V (no path materialization)"
            ...
            test "toggling an offscreen row updates the logical selection set without materializing it (FR-010)" {
                ...
                Expect.isFalse
                    (off.SelectedRows.Contains "r8000")
                    "toggle off clears it — pure logical state, no materialization"

                Expect.isFalse
                    (List.contains "r8000" (realizedKeys total off))
                    "the toggled offscreen row is never materialized"
            }
            ...
            test "focusing an offscreen cell records it logically without materializing the path (FR-009)" {
                ...
                Expect.exists
                    effects
                    (function
                    | DataGridFocusChanged(Some c) -> c = cell
                    | _ -> false)
                    "focus emits its effect"

                Expect.isFalse
                    (List.contains "r7777" (realizedKeys total focused))
                    "the focused offscreen row is not materialized by FocusCell alone"
            }
            ...
            test "relocating to an offscreen target brings it into the window — relocate, do NOT expand (FR-009, O4)" {
                ...
                Expect.isTrue
                    (List.contains (sprintf "r%d" targetIndex) keys)
                    "the relocated window materializes the target row"
                // the window MOVED (its first index jumped near the target) ...
                Expect.isTrue
                    (relocated.VisibleRange.FirstIndex >= targetIndex - 2 * n)
                    "window relocated to (near) the target"
                // ... and stayed BOUNDED — it did not expand to span r0..r9000.
                Expect.isTrue
                    (relocated.VisibleRange.Count <= visibleCount + 2 * n)
                    "window stays bounded by V + 2N after relocation (relocate, not expand)"

                Expect.isFalse (List.contains "r0" keys) "the path from the old position is NOT materialized"
            }

            test
                "boundary-crossing relocation lands on the correct next LOGICAL row and advances the window (FR-011, SC-005)" {
                let n = 0
                let model = gridWithOverscan n
                ...
                Expect.isTrue
                    (List.contains (sprintf "r%d" nextLogical) keys)
                    "the next logical row across the boundary is now realized"

                Expect.isTrue
                    (advanced.VisibleRange.FirstIndex > model.VisibleRange.FirstIndex)
                    "the realized window advanced forward"

                Expect.isTrue
                    (advanced.VisibleRange.Count <= visibleCount + 2 * n)
                    "the advanced window stays bounded (it advanced, did not grow)"
            }
            ...
            test "dispatch outcome for an already-materialized (visible) row is byte-identical to pre-feature (FR-016)" {
                ...
                Expect.equal
                    selected.VisibleRange
                    model.VisibleRange
                    "visible-row selection leaves the window unchanged (byte-identical)"

                Expect.exists
                    effects
                    (function
                    | DataGridSelectionChanged [ "r3" ] -> true
                    | _ -> false)
                    "visible-row selection emits the same effect as before"
            }
        ]
