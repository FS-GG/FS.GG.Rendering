        ...
        [
            ...
            test "Result.Error is reachable through `open FS.GG.UI.Controls` (severity does not shadow it)" {
                ...
                Expect.equal
                    ...
                    "...and its Error case is Result's, not ControlDiagnosticSeverity's"

                Expect.equal
                    ControlDiagnosticSeverity.Error
                    ControlDiagnosticSeverity.Error
                    "the severity case is still reachable, qualified"
            }

            test "boundary diagnostics name stale references and leaking dependencies" {
                let stale =
                    Diagnostics.stalePackageReference "FS.GG.UI.Charts" "template/capabilities.yml"

                let leak =
                    Diagnostics.dependencyLeak "FS.GG.UI.Controls" "src/SkiaViewer/SkiaViewer.fsproj"
            ...
            test "runtime catalog target and scope diagnostics name actionable subjects" {
                ...
                let duplicate =
                    Diagnostics.duplicateRuntimeDefinition "KeyboardInput" "src/Input/KeyboardInput.fs"

                let staleTarget = Diagnostics.staleEventTarget "save-button" "click"

                let unsupported =
                    Diagnostics.unsupportedScopeExpansion "renderer-neutral controls" "template guidance"
            ...
            test
                "validation diagnostics name packages capabilities controls profiles adapters runtime environment and migration gaps" {
                let diagnostics =
                    [
                        Diagnostics.stalePackageReference "FS.GG.UI.Charts" "template/capabilities.yml",
                        [ "FS.GG.UI.Charts"; "template/capabilities.yml" ]
                        Diagnostics.create
                            None
                            "capability:charts"
                            StaleGeneratedReference
                            ControlDiagnosticSeverity.Error
                            "Generated capability `charts` is not active.",
                        [ "charts" ]
                        ...
                        Diagnostics.create
                            None
                            "generated-profile:app"
                            MissingRequiredAttribute
                            ControlDiagnosticSeverity.Error
                            "Generated profile `app` is missing `FS.GG.UI.Controls.Elmish`.",
                        [ "app"; "FS.GG.UI.Controls.Elmish" ]
                        Diagnostics.create
                            None
                            "adapter-contract"
                            MissingRequiredAttribute
                            ControlDiagnosticSeverity.Error
                            "Adapter contract `ControlsElmish.program` is missing.",
                        [ "ControlsElmish.program" ]
                        Diagnostics.duplicateRuntimeDefinition "KeyboardModel" "src/Controls/KeyboardInput.fs",
                        [ "KeyboardModel"; "src/Controls/KeyboardInput.fs" ]
                        ...
                        Diagnostics.create
                            ...
                            MissingRequiredAttribute
                            ControlDiagnosticSeverity.Error
                            "Migration guidance is missing the legacy Charts replacement path.",
                        [ "legacy Charts"; "replacement path" ]
                    ]

                diagnostics
                |> List.iter (fun (diagnostic, expectedTerms) ->
                    expectedTerms |> List.iter (assertMessageContains diagnostic))
            }
        ]
