...
let private mkEvent kind (payload: string option) : ControlEvent =
    {
        ...
        Nav = payload |> Option.map EditedText
    }

[<Tests>]
let feature132NewControlContractTests =
    testList
        "Feature 132 net-new control contract (FR-008/SC-004, R6)"
        [
                ...
                [
                    for (id, _) in netNewControls do
                        test (sprintf "%s has a complete, supported catalog row" id) {
                            ...
                            | Some row ->
                                Expect.isFalse
                                    (System.String.IsNullOrWhiteSpace row.Purpose)
                                    (sprintf "%s has a purpose" id)

                                Expect.equal
                                    (Set.ofList row.VisualStates)
                                    standardStates
                                    (sprintf "%s declares the standard 8 visual states" id)
                                ...
                                Expect.contains
                                    (Catalog.categories ())
                                    row.Category
                                    (sprintf "%s category is a known catalog category" id)
                        }
                ]
                ...
                [
                    for (id, _) in netNewControls do
                        test (sprintf "%s advertises accessibility role + state metadata" id) {
                            ...
                            Expect.isFalse
                                (System.String.IsNullOrWhiteSpace row.Accessibility.Role)
                                (sprintf "%s has an accessibility role" id)

                            Expect.isNonEmpty row.Accessibility.StateMetadata (sprintf "%s reports state metadata" id)

                            Expect.isFalse
                                (System.String.IsNullOrWhiteSpace row.Accessibility.KeyboardOperation)
                                (sprintf "%s documents keyboard operation" id)
                        }
                ]

            // --- Rendering family (DUAL THEME, SC-004) --------------------------------------------
            testList
                "Rendering (dual theme)"
                [
                    for (id, ctrl) in netNewControls do
                        test (sprintf "%s renders coherently under BOTH Default and AntDesign" id) {
                            for (tname, theme) in [ "Default", DefaultTheme.light; "AntDesign", Ant.antLight ] do
                                ...
                                Expect.isEmpty
                                    ...
                                    (sprintf "%s renders with no diagnostics under %s" id tname)

                                Expect.isGreaterThan
                                    rendered.NodeCount
                                    0
                                    (sprintf "%s has a non-empty render tree under %s" id tname)

                                let evidence =
                                    Scene.renderReadbackEvidence { Width = 320; Height = 160 } rendered.Scene

                                Expect.isNonEmpty
                                    evidence.DeterministicHash
                                    (sprintf "%s produces deterministic render evidence under %s" id tname)
                        }
                ]
                    ...
                    test "popconfirm onConfirm / onCancel dispatch independently" {
                        let c =
                            Feedback2.Popconfirm.create
                                [
                                    Feedback2.Popconfirm.onConfirm Confirmed
                                    Feedback2.Popconfirm.onCancel Cancelled
                                ]

                        Expect.equal
                            (Control.dispatch (mkEvent "confirm" None) c)
                            [ Confirmed ]
                            "onConfirm fires Confirmed"

                        Expect.equal
                            (Control.dispatch (mkEvent "cancel" None) c)
                            [ Cancelled ]
                            "onCancel fires Cancelled"
                    }
                    test "segmented onChange carries the selected payload" {
                        let c =
                            Navigation2.Segmented.create
                                [ Attr.items [ "Day"; "Week" ]; Navigation2.Segmented.onChange Changed ]

                        Expect.equal
                            (Control.dispatch (mkEvent "change" (Some "Week")) c)
                            [ Changed "Week" ]
                            "onChange carries the payload"
                    }
                    test "rate onChange carries the new rating payload" {
                        let c =
                            Interactive2.Rate.create [ Interactive2.Rate.value 2.0; Interactive2.Rate.onChange Changed ]

                        Expect.equal
                            (Control.dispatch (mkEvent "change" (Some "4")) c)
                            [ Changed "4" ]
                            "onChange carries the rating"
                    }
                    test "upload onChange carries the file reference payload" {
                        let c =
                            DataEntry2.Upload.create
                                [ DataEntry2.Upload.text "Upload"; DataEntry2.Upload.onChange Changed ]

                        Expect.equal
                            (Control.dispatch (mkEvent "change" (Some "f.png")) c)
                            [ Changed "f.png" ]
                            "onChange carries the file ref"
                    }
                ]
        ]
