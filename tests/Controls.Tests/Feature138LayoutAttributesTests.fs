...
let private horizontalGap first second = second.X - first.X - first.Width
...
let private expectUniformPadding expected actual =
    Expect.equal
        ...
        "uniform padding/margin is projected"

[<Tests>]
let tests =
    testList
        "Feature138LayoutAttributes"
        [
            test "public builders emit canonical layout names and values" {
                ...
                for attr, name, expected in alignBuilders do
                    ...
                    match attr.Value with
                    | UntypedValue(:? LayoutAlign as actual) ->
                        Expect.equal actual expected $"{name} carries the authored alignment"
                    | other -> failtestf "%s carried unexpected value %A" name other
            }
            ...
            test "padding gap margin alignment and justification affect evaluated bounds" {
                ...
                expectClose
                    (box "a" aligned).X
                    (float size.Width - (box "a" aligned).Width)
                    "justifyContent end moves child on main axis"

                expectClose
                    (box "a" aligned).Y
                    ((float size.Height - (box "a" aligned).Height) / 2.0)
                    "alignItems center moves child on cross axis"
            }
            ...
            test "flex grow shrink basis and min/max constraints affect evaluated bounds" {
                ...
                let flexed =
                    row
                        ...
                        [
                            panel
                                "a"
                                [ Attr.flexBasis 20.0; Attr.flexGrow 1.0 ]
                                [ leaf "a.text" [ Attr.width 1.0; Attr.height 1.0 ] ]
                            panel
                                "b"
                                [ Attr.flexBasis 20.0; Attr.flexGrow 3.0 ]
                                [ leaf "b.text" [ Attr.width 1.0; Attr.height 1.0 ] ]
                        ]
                        ...
                        [
                            rowPanel
                                ...
                                [
                                    panel
                                        "a"
                                        [ Attr.flexBasis 40.0; Attr.flexShrink 1.0 ]
                                        [ leaf "a.text" [ Attr.width 1.0; Attr.height 1.0 ] ]
                                    panel
                                        "b"
                                        [ Attr.flexBasis 40.0; Attr.flexShrink 3.0 ]
                                        [ leaf "b.text" [ Attr.width 1.0; Attr.height 1.0 ] ]
                                ]
                        ]
                ...
                Expect.isGreaterThan
                    (box "a" shrink).Width
                    (box "b" shrink).Width
                    "larger flexShrink yields the smaller box"
                ...
                let clamped =
                    row
                        ...
                        [
                            panel
                                "min"
                                [ Attr.flexBasis 20.0; Attr.minWidth 40.0 ]
                                [ leaf "min.text" [ Attr.width 1.0; Attr.height 1.0 ] ]
                            panel
                                "max"
                                [ Attr.flexBasis 80.0; Attr.maxWidth 30.0 ]
                                [ leaf "max.text" [ Attr.width 1.0; Attr.height 1.0 ] ]
                        ]
            ...
            test "omitted values preserve compatibility defaults and explicit zero overrides them" {
                ...
                expectClose
                    (horizontalGap (box "a" omitted) (box "b" omitted))
                    8.0
                    "omitted gap keeps compatibility spacing"
                ...
                Expect.equal
                    (bounds authoredDefaults)
                    (bounds omitted)
                    "authoring compatibility defaults keeps bounds unchanged"
                ...
                expectClose
                    (horizontalGap (box "a" zeroed) (box "b" zeroed))
                    0.0
                    "explicit zero gap overrides compatibility spacing"
            }
            ...
            test "last writer wins across canonical gap and spacing alias" {
                ...
                expectClose
                    (horizontalGap (box "a" spacingLast) (box "b" spacingLast))
                    7.0
                    "spacing alias can win as the last gap writer"

                expectClose
                    (horizontalGap (box "a" gapLast) (box "b" gapLast))
                    1.0
                    "canonical gap can win as the last gap writer"
