...
let selectionRouteTests =
    testList
        "100 US1 Focus.route selection classification (FR-003)"
        [
            test "a RadioGroup role maps arrows/Home/End to the exact SelectionMove Direction" {
                let route key =
                    Focus.route AccessibilityRole.RadioGroup navKb None key false false
            ...
            test "a Tab role (horizontal) maps Left/Right to Previous/Next" {
                ...
                Expect.equal
                    (Focus.route AccessibilityRole.Tab kb None "ArrowLeft" false false)
                    (Navigate(SelectionMove Direction.Previous))
                    "ArrowLeft -> Previous"

                Expect.equal
                    (Focus.route AccessibilityRole.Tab kb None "ArrowRight" false false)
                    (Navigate(SelectionMove Direction.Next))
                    "ArrowRight -> Next"
            }
        ]
...
let valueRouteTests =
    testList
        "100 US2 Focus.route value classification (FR-002/FR-007)"
        [
            test "a Slider role with a declared NavRange maps arrows to a signed step delta" {
                let route key =
                    Focus.route AccessibilityRole.Slider rangeKb sliderRange key false false
            ...
            test "Home/End fold to a delta that clamps to Min/Max at the host" {
                ...
                Expect.equal
                    (Focus.route AccessibilityRole.Slider rangeKb range "Home" false false)
                    (Navigate(ValueStep -100.0))
                    "Home -> delta to Min"

                Expect.equal
                    (Focus.route AccessibilityRole.Slider rangeKb range "End" false false)
                    (Navigate(ValueStep 100.0))
                    "End -> delta to Max"
            }
            ...
            test "a value role with NO declared NavRange cannot step -> Fallthrough (FR-008)" {
                Expect.equal
                    (Focus.route AccessibilityRole.Slider rangeKb None "ArrowRight" false false)
                    Fallthrough
                    "no NavRange -> no value step"
            }
        ]
...
let gridRouteTests =
    testList
        "100 US3 Focus.route grid classification (FR-004)"
        [
            test "a Grid role maps arrows to a 2-D unit delta (row by Up/Down, column by Left/Right)" {
                let route key =
                    Focus.route AccessibilityRole.Grid navKb None key false false
...
let fallthroughTests =
    testList
        "100 Focus.route no-op cases (FR-008)"
        [
            test "a key absent from NavigationKeys -> Fallthrough" {
                ...
                Expect.equal
                    (Focus.route AccessibilityRole.RadioGroup kb None "Home" false false)
                    Fallthrough
                    "Home not in NavigationKeys -> Fallthrough"

                Expect.equal
                    (Focus.route AccessibilityRole.RadioGroup kb None "Q" false false)
                    Fallthrough
                    "an arbitrary key -> Fallthrough"
            }
            ...
            test "a non-navigable role (Button) never forms a navigation intent" {
                ...
                Expect.equal
                    (Focus.route AccessibilityRole.Button kb None "ArrowRight" false false)
                    Fallthrough
                    "Button arrow -> Fallthrough (no nav intent)"

                Expect.equal
                    (Focus.route AccessibilityRole.Button kb None "Enter" false false)
                    Activate
                    "Button Enter still activates (E4 unchanged)"
            }
        ]
...
module private Gen100 =
    let direction =
        Gen.elements [ Direction.Previous; Direction.Next; Direction.First; Direction.Last ]
...
let closedModelTests =
    testList
        ...
        [
            testCase
                "NavIntent is a closed, totally-matched set; its match is total and never throws (>=1000)"
                (fun () ->
                    ...
                    Check.One(
                        ...
                        Prop.forAll (Arb.fromGen Gen100.intent) prop
                    ))

            testCase
                "NavPayload is a closed, totally-matched set; its match is total and never throws (>=1000)"
                (fun () ->
                    ...
                    Check.One(
                        Config.QuickThrowOnFailure.WithMaxTest 1000,
                        Prop.forAll (Arb.fromGen Gen100.payload) prop
                    ))

            testCase
                "NavIntent maps one-to-one onto NavPayload (each intent class has exactly one payload class) (>=1000)"
                (fun () ->
                    let prop (intent: NavIntent) =
                        payloadTag (intentToPayload intent) = intentTag intent
            ...
            test
                "navigation is reproduced PURELY from declared role + NavigationKeys (+ NavRange) — no per-kind host special-case" {
                // The three covered intent classes, each derived solely from the declared metadata.
                Expect.equal
                    (Focus.route AccessibilityRole.Slider rangeKb sliderRange "ArrowRight" false false)
                    (Navigate(ValueStep 0.1))
                    "value role -> ValueStep"

                Expect.equal
                    (Focus.route AccessibilityRole.RadioGroup navKb None "ArrowDown" false false)
                    (Navigate(SelectionMove Direction.Next))
                    "selection role -> SelectionMove"

                Expect.equal
                    (Focus.route AccessibilityRole.Grid navKb None "ArrowRight" false false)
                    (Navigate(GridMove(0, 1)))
                    "grid role -> GridMove"
            }
