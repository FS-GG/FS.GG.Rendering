...
let tests =
    testList
        "Feature174 retained metadata work scaling"
        [
            test "first frame exposes retained metadata equivalent to direct render" {
                ...
                let init =
                    RetainedRender.init
                        Feature174RetainedRenderFixtures.theme
                        Feature174RetainedRenderFixtures.size
                        tree
                ...
                Expect.equal
                    init.Retained.Root.Metadata.NodeCount
                    init.Render.NodeCount
                    "root metadata carries first-frame node count"

                Expect.equal
                    (init.Retained.Root.Metadata.InFlowBounds
                     @ init.Retained.Root.Metadata.OverlayBounds)
                    init.Render.Bounds
                    "root metadata carries first-frame bounds"
            }
            ...
            test "button-click metadata visits changed ancestors instead of the full tree" {
                ...
                Expect.isLessThan
                    work.MetadataVisitedNodeCount
                    work.BaselineNodeCount
                    "metadata work is below the full-tree baseline"

                Expect.isLessThanOrEqual
                    work.MetadataVisitedNodeCount
                    (work.ChangedSubtreeBound + 4)
                    "metadata visits are bounded by the changed leaf plus ancestors"
            }
            ...
            test "idle retained step reuses metadata snapshots without visits" {
                ...
                Feature174RetainedRenderFixtures.assertRenderMetadataParity
                    "idle"
                    step.Render
                    (Feature174RetainedRenderFixtures.direct tree)
            }
            ...
            test "metadata counters remain valid when the page has no replay-cache boundary" {
                ...
                let after =
                    Feature174RetainedRenderFixtures.shell
                        "plain-page"
                        [ Feature174RetainedRenderFixtures.text "plain-0" "Updated row" ]

                let step = Feature174RetainedRenderFixtures.retainedStep before after

                Expect.equal
                    step.WorkReduction.MetadataFallbackCount
                    0
                    "no replay-cache boundary does not force metadata fallback"

                Expect.isLessThan
                    step.WorkReduction.MetadataVisitedNodeCount
                    step.WorkReduction.BaselineNodeCount
                    "plain retained metadata still improves"
            }
        ]
