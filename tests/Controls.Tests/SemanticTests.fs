    ...
    testList
        ...
        [
            test "representative view function renders through public Control surface" {
                ...
                Expect.contains
                    (Scene.describe rendered.Scene)
                    TextRunElement
                    "render produces fitted text scene output"
            ...
            test "model-owned state changes are reflected by re-evaluating the view" {
                ...
                Expect.notEqual
                    ...
                    "control values are model-owned descriptions"

                Expect.isTrue (second.Children[1].Content = Some "Grace") "text box value reflects the current model"

                Expect.isTrue
                    (second.Children[2].Attributes
                     |> List.exists (fun attr ->
                         match attr.Name, attr.Value with
                         | "enabled", BoolValue false -> true
                         | _ -> false))
                    "enabled state reflects current model"
            }
            ...
            test "custom control definitions advertise no callbacks the renderer never invokes" {
                let properties =
                    typeof<CustomControlDefinition>.GetProperties(BindingFlags.Public ||| BindingFlags.Instance)
                    |> Array.map _.Name
                    |> Set.ofArray
                ...
                |> List.iter (fun property ->
                    Expect.isFalse
                        (Set.contains property properties)
                        $"CustomControlDefinition no longer exposes the dead {property} hook")
            ...
            test "chart and graph controls render as Controls-owned scene elements" {
                ...
                let graph = GraphView.create [ GraphView.nodes [ "form"; "chart"; "grid" ] ]
                ...
                Expect.isFalse
                    (kinds |> List.contains ChartElement)
                    "charts no longer emit the opaque Chart node (feature 080)"
