                    ...
                    Some
                        {
                            Start = 0.0
                            End = 1.0
                            Duration = RetainedRender.defaultTransitionDuration
...
let determinism =
    testList
        "099 US3 determinism + edges (pure clock core)"
        [
            ...
            test "a very-large delta clamps Elapsed to the duration and the sample settles at the end (no overshoot)" {
                ...
                Expect.equal
                    advanced.Elapsed
                    RetainedRender.defaultTransitionDuration
                    "Elapsed clamps to the tween duration"

                Expect.isFalse (RetainedRender.clockActive advanced) "a clamped clock is settled (no longer active)"

                Expect.equal
                    (sampledOpacity advanced)
                    1.0
                    "the sampled opacity is exactly the End value (no overshoot past target)"
            }
            ...
            test "a mid-flight retarget re-aims from the CURRENT sampled value (no snap to start)" {
                ...
                Expect.floatClose
                    Accuracy.high
                    (sampledOpacity retargeted)
                    currentBeforeFlip
                    "continues from the displayed value (no snap to start)"
            }
            ...
            test "a settled return-to-Normal clock is DROPPED to None (at-rest restored)" {
                ...
                Expect.isFalse
                    (RetainedRender.clockActive settledNormal)
                    "precondition: the return-to-Normal clock has settled"

                Expect.isNone
                    (RetainedRender.updateClockForState Normal [] (Some settledNormal))
                    "a settled Normal-targeted clock drops to None"
            }
            ...
            test "a held non-Normal state does NOT re-fire: a settled clock at the same state advances-only (kept)" {
                ...
                Expect.equal
                    kept
                    (Some settledHover)
                    "a settled clock at its own state is kept unchanged (no spurious re-start)"
            }
            ...
            test "multiple clocks advance INDEPENDENTLY: one completing does not perturb another's Elapsed/sample" {
                ...
                Expect.equal
                    near'.Elapsed
                    RetainedRender.defaultTransitionDuration
                    "the near clock clamps/settles at its own duration"
                ...
                Expect.equal
                    (sampledOpacity early')
                    (sampledOpacity (RetainedRender.advance delta early))
                    "the early clock's sample is unperturbed by the other clock completing"
            }
            ...
            test "capture determinism evidence (SC-004/FR-006)" {
                ...
                let runA =
                    deltas |> List.fold (fun c d -> RetainedRender.advance d c) (freshClock Hover)
...
let identityAtRest =
    testList
        "099 US3 identity-at-rest (byte-identical, zero recompute)"
        [
