...
let private bounds viewport contentHeight =
    let _, b, _ =
        ControlInternals.evaluateLayout viewport (shell viewport contentHeight)
...
let tests =
    testList
        "Feature138ShellChrome"
        [
            test "fixed chrome and flexible content at 640x480" { assertShell { Width = 640; Height = 480 } 120.0 }

            test "fixed chrome and flexible content at 400x300" { assertShell { Width = 400; Height = 300 } 120.0 }
