...
let private frozenOutlineScene (theme: Theme) (label: string) : Scene list =
    ...
    [
        ...
        mkText theme (box.X + theme.SpaceMd) (by + h / 2.0 + 5.0) theme.FontSize theme.Accent label
    ]
        ...
        [
            ...
            testList
                ...
                [
                    ...
                    test
                        "migrated Button/IconButton Scene byte-matches the pre-migration geometry, intent-independent (G2)" {
                        for (tname, theme) in themes do
                            for intent in knownIntents do
                                ...
                                Expect.equal
                                    ...
                                    (sprintf
                                        "icon-button.%s intent=%s Scene matches the frozen outline geometry"
                                        tname
                                        intent)
                    }
                ]
            ...
            testList
                ...
                [
                    ...
                    test "resolve is total + deterministic over the full kind×intent×state cross-product (G3)" {
                        ...
                        Expect.equal
                            first.Length
                            (kinds.Length * intents.Length * allStates.Length)
                            "every combination resolved"
                        ...
                        for kind in knownKinds do
                            for state in [ Hover; Pressed; Selected; Disabled ] do
                                ...
                                let viaBackHalf =
                                    Style.resolve
                                        Theme.light
                                        (StyleResolver.baseStyleFor Theme.light kind)
                                        sampleClasses
                                        state
                                ...
                                Expect.equal
                                    ...
                                    viaBackHalf
                                    (sprintf
                                        "front half supplies only baseStyle; 093 overlay unchanged (%s/%A)"
                                        kind
                                        state)
                    }
                ]
            ...
            testList
                ...
                [
                    ...
                    test
                        "a theme's divergent IntentPolicy makes danger differ from primary; neutral keeps them equal (G4)" {
                        ...
                        for kind in knownKinds do
                            ...
                            Expect.notEqual
                                dangerDiv
                                primaryDiv
                                (sprintf "divergent policy: danger ≠ primary for %s" kind)
                    ...
                    test
                        "divergence needs no control edit: neutral-policy path stays neutral; no per-intent control type (G5)" {
                        ...
                        let intentForkedKinds =
                            ...
                            |> List.filter (fun d -> knownIntents |> List.exists (fun i -> d.Id.Contains i))
