...
let rec private flattenScene (scene: Scene) : SceneNode list = scene.Nodes |> List.collect flattenNode
...
and private flattenNode (node: SceneNode) : SceneNode list =
    ...
    | PictureNode picture ->
        [
            PictureNode
                { picture with
                    Scene = { Nodes = flattenScene picture.Scene }
                }
        ]
    | other -> [ other ]
                        ...
                        [
                            ...
                            Stack.children
                                [
                                    ...
                                    button "go" "Go"
                                    Overlay.create [ Overlay.child (text "floating" "Overlay") ]
...
let private assertEquivalent name (tree: Control<Msg>) =
    ...
    Expect.equal
        (visibleShape cold.Render)
        (visibleShape direct)
        (name + ": cold retained visible output matches direct")

    Expect.equal
        (visibleShape warm.Render)
        (visibleShape direct)
        (name + ": warm retained visible output matches direct")
...
let parityTests =
    testList
        "Feature141 direct/cold/warm retained parity"
        [
            test
                "focused nested controls, cache boundaries, overlays, diagnostics, fingerprints, and glyph proof stay equivalent" {
                ...
                Expect.equal
                    warm.Retained.Root.Fragment.InvalidationEvidence.Head.Decision
                    Reused
                    "warm idle frame records root reuse"

                Expect.equal
                    warm.Retained.Root.Fragment.InvalidationEvidence.Head.Reason
                    StableInputs
                    "warm idle frame records stable inputs"
            }

            test
                "empty, no-reuse, bounds, diagnostics, events, overlays, cached subtrees, and public result fields remain compatible" {
                assertEquivalent "empty" (Stack.create [ Stack.children [] ])

                assertEquivalent
                    "no-reuse screen"
                    (Stack.create [ Stack.children [ text "plain" "Plain"; button "action" "Action" ] ])
...
let invalidationEvidenceTests =
    testList
        "Feature141 retained invalidation evidence"
        [
            test
                "visual, layout, identity, child-order, insertion, and removal changes are recorded without stale output" {
                let initial =
                    Stack.create [ Stack.children [ text "a" "A"; text "b" "B"; button "go" "Go" ] ]

                let init = RetainedRender.init theme size initial

                let changedVisual =
                    Stack.create [ Stack.children [ text "a" "A changed"; text "b" "B"; button "go" "Go" ] ]

                let visual = RetainedRender.step theme size init.Retained changedVisual

                Expect.contains
                    (collectEvidence visual.Retained.Root |> List.map _.Reason)
                    VisualInput
                    "visual input changes are recorded"

                Expect.equal
                    (visibleShape visual.Render)
                    (visibleShape (Control.renderTree theme size changedVisual))
                    "visual change output matches direct"
                    ...
                    Stack.create
                        [
                            Stack.children
                                [
                                    text "a" "A"
                                    TextBlock.create [ TextBlock.text "B"; Attr.width 260.0; Attr.height 28.0 ]
                                    |> Control.withKey "b"
                                    button "go" "Go"
                                ]
                        ]
                ...
                Expect.contains
                    (collectEvidence layout.Retained.Root |> List.map _.Reason)
                    LayoutInput
                    "layout input changes are recorded"

                Expect.equal
                    (visibleShape layout.Render)
                    (visibleShape (Control.renderTree theme size changedLayout))
                    "layout change output matches direct"
                ...
                Expect.contains
                    (collectEvidence identity.Retained.Root |> List.map _.Reason)
                    ExplicitIdentity
                    "explicit identity changes discard/rebuild"

                let reordered =
                    Stack.create [ Stack.children [ button "go" "Go"; text "a" "A"; text "b" "B" ] ]

                let reorder = RetainedRender.step theme size init.Retained reordered

                Expect.contains
                    (collectEvidence reorder.Retained.Root |> List.map _.Reason)
                    ChildOrdering
                    "child ordering changes are recorded"

                Expect.equal
                    (visibleShape reorder.Render)
                    (visibleShape (Control.renderTree theme size reordered))
                    "reordered output matches direct"
                ...
                Expect.contains
                    (collectEvidence insertion.Retained.Root |> List.map _.Reason)
                    ChildInsertion
                    "child insertion changes are recorded"

                let removed = Stack.create [ Stack.children [ text "a" "A"; button "go" "Go" ] ]

                let removal = RetainedRender.step theme size init.Retained removed

                Expect.contains
                    (collectEvidence removal.Retained.Root |> List.map _.Reason)
                    ChildRemoval
                    "child removal changes are recorded"

                Expect.equal
                    (visibleShape removal.Render)
                    (visibleShape (Control.renderTree theme size removed))
                    "removed-child output matches direct"
            }
            ...
            test "Feature 140 composition evidence is the retained reuse classification source" {
                let chain =
                    ([
                        {
                            Effect = Composition.Offset(4.0, 8.0)
                            Source = Composition.AuthoredModifier
                        }
                        {
                            Effect = Composition.LocalZOrder 2
                            Source = Composition.AuthoredModifier
                        }
                    ]
                ...
                Expect.equal
                    evidence.NormalizedModifierFingerprint
                    (Composition.fingerprint chain.NormalizedEffects)
                    "reuse evidence shares the normalized modifier fingerprint"
...
let randomizedEquivalenceTests =
    testList
        "Feature141 RetainedRandomizedEquivalence"
        [
...
let architectureGuardTests =
    testList
        "Feature141 one-owner architecture guards"
        [
            test
                "RetainedRender stores owner-produced assembly results and does not contain retained-local composition rule sets" {
                let retainedFsi = readRepo "src/Controls/RetainedRender.fsi"
                let retainedFs = readRepo "src/Controls/RetainedRender.fs"

                Expect.isTrue
                    (retainedFsi.Contains("Assembly: ControlInternals.CurrentNodeAssemblyResult"))
                    "RenderFragment stores owner-produced assembly"

                Expect.isTrue
                    (retainedFsi.Contains("RetainedInvalidationEvidence"))
                    "RenderFragment stores invalidation evidence"

                Expect.isFalse
                    (retainedFs.Contains("composeRetainedScenes"))
                    "old retained-local composition helper stays removed"

                Expect.isFalse
                    (retainedFs.Contains("composeContainerScene"))
                    "retained renderer does not call the container composition helper directly"

                Expect.isFalse
                    (retainedFs.Contains("isOverlayNode"))
                    "retained renderer does not branch on overlay semantics directly"

                Expect.isTrue
                    (retainedFs.Contains("ControlInternals.assembleCurrentNode"))
                    "fresh/replay assembly routes through the shared owner"
            }
            ...
            test "ControlInternals has exactly one assembly owner and Feature 141 stays inside scope" {
                ...
                Expect.isTrue
                    (control.Contains("type CurrentNodeAssemblyResult"))
                    "assembly result is owned by ControlInternals"

                Expect.isFalse
                    (retainedFsi.Contains("val .*retained renderer"))
                    "no new public retained renderer API is introduced"
