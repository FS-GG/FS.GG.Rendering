    ...
    container
        ...
        [
            container
                ...
                [ leaf "leafA" leafAWidth 20.0; leaf "leafB" 50.0 20.0 ]
...
let tests =
    testList
        "Feature097 wired incremental layout"
        [

            test
                "localized geometry edit re-measures a proper subset and stays byte-identical to a full rebuild (SC-001/SC-003/FR-008)" {
                ...
                Expect.isLessThan
                    ...
                    baseline
                    "localized edit re-measures a strict subset"

                Expect.isGreaterThan
                    s.WorkReduction.RemeasuredNodeCount
                    0
                    "but the geometry change DID re-measure (not stale)"
                ...
                Expect.equal
                    (sceneOf s.Render)
                    (sceneOf full)
                    "wired incremental render == full rebuild (geometry change)"
            }
            ...
            test "at-rest frame (identical tree) re-measures nothing and renders byte-identical (FR-008 at-rest)" {
                ...
                Expect.equal
                    (sceneOf s.Render)
                    (sceneOf (Control.renderTree theme size (tree 50.0)))
                    "at-rest render byte-identical"
            }
            ...
            test "content-only change re-measures nothing yet stays byte-identical (SC-004 non-layout)" {
                ...
                Expect.equal
                    (sceneOf s.Render)
                    (sceneOf (Control.renderTree theme size next))
                    "content change repaints, byte-identical"
            }
            ...
            test "a whole-tree geometry relayout re-measures the baseline (SC-003 never under-reports)" {
                ...
                let withOrientation (o: string) =
                    { container
                          ...
                          [ leaf "a" 50.0 20.0; leaf "b" 50.0 20.0 ] with
                        Content = None
                    }
                ...
                Expect.equal
                    ...
                    "root geometry change re-measures the whole tree"
            }
