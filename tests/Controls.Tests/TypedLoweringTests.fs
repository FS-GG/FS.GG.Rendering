...
let typedLoweringTests =
    testList
        "Typed controls lowering parity (keystone, six controls)"
        [
            // --- US1 primitives -------------------------------------------------
            test "TextBlock lowers structurally equal to legacy TextBlock.create" {
                ...
                let legacy = LTextBlock.create [ LTextBlock.text "Sign in" ]
            ...
            test "Button Intent lowers to the legacy variant style attribute" {
                ...
                let legacy =
                    LButton.create [ LButton.text "Delete"; LButton.enabled true; Attr.style "danger" ]
            ...
            test "CheckBox lowers equal to legacy and OnChanged payload maps identically" {
                ...
                let legacy =
                    LCheckBox.create [ LCheckBox.text "Agree"; LCheckBox.checked' true; LCheckBox.onChanged Toggled ]
            ...
            test "Stack lowers equal to legacy with children in author order" {
                ...
                let order =
                    (Widget.toControl typed).Children |> List.map (fun child -> child.Content)
            ...
            test "Widget.ofControl bridges a legacy control and round-trips unchanged" {
                ...
                Expect.equal
                    (sprintf "%A" (Widget.toControl (Widget.ofControl legacy)))
                    (sprintf "%A" legacy)
                ...
                Expect.equal
                    (sprintf "%A" children.[0])
                    (sprintf "%A" legacy)
                    "bridged legacy control is unchanged inside typed children"
            }
            ...
            test "TextBox init/update delegate to TextInput and view lowers to legacy IR" {
                let props: TextBoxProps<Msg> =
                    { TextBox.defaults "email" with
                        Value = "Ada"
                    }
            ...
            test "DataGrid init/update delegate to DataGrid and view lowers to legacy IR" {
                ...
                let legacyModel, legacyEffects =
                    LDataGrid.init "grid" columns rows.Length 24.0 240.0
