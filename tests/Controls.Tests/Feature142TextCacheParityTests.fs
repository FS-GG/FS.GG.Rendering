...
let tests =
    testList
        "Feature142 shaped text cache parity"
        [
            test "provider bucket changes force a text-cache miss" {
                let _, warm, firstHit =
                    RetainedRender.measureTextCachedWithBucket "bucket-a" empty true "cache me" font

                Expect.isFalse firstHit "cold measurement misses"

                let _, _, sameBucketHit =
                    RetainedRender.measureTextCachedWithBucket "bucket-a" warm true "cache me" font

                Expect.isTrue sameBucketHit "same bucket hits"

                let _, _, changedBucketHit =
                    RetainedRender.measureTextCachedWithBucket "bucket-b" warm true "cache me" font
