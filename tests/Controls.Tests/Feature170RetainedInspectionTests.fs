    ...
    testList
        ...
        [
            test "retained transition classifies reused repainted shifted added and removed nodes" {
                ...
                let status nodeId =
                    artifact.RetainedNodes
                    |> List.find (fun node -> node.NodeId = nodeId)
                    |> fun node -> node.Status
                ...
                Expect.exists
                    artifact.RetainedNodes
                    (fun node -> node.NodeId = "stable" && node.RetainedIdentity.IsSome)
                    "opaque retained identity recorded"
            ...
            test "first retained frame reports not-inspected damage with a first-frame diagnostic" {
                ...
                Expect.equal
                    damage.DamageStatus
                    DamageInspectionStatus.NotInspected
                    "first frame cannot inspect prior damage"

                Expect.exists
                    damage.Diagnostics
                    (fun diagnostic -> diagnostic.Contains("no prior"))
                    "diagnostic names no prior frame"

                Expect.exists
                    artifact.RetainedNodes
                    (fun node -> node.NodeId = "title" && node.Status = RetainedNodeStatus.Added)
                    "first-frame nodes are added"
            }
        ]
