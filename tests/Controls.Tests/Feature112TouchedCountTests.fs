...
let private flat (n: int) : Control<int> =
    Stack.create
        [
            Stack.children
                [
                    for i in 0 .. n - 1 -> Button.create [ Button.text "Go" ] |> Control.withKey (sprintf "b%d" i)
                ]
        ]
...
let tests =
    testList
        "Feature 112 targeted-stamp touched-node count (US1/US3, FR-001/FR-004/FR-007)"
        [
            ...
            test "a hover move A->B touches only A, B and their shared ancestor — far below the node count (SC-001)" {
                ...
                Expect.isTrue
                    (r.RuntimeStateTouchedNodeCount < total)
                    "touched is far below the total node count (SC-001)"
            }
            ...
            test "a focus move A->B touches only the affected identities + ancestors (SC-001)" {
                ...
                let r =
                    ControlRuntime.applyRuntimeVisualStateTargeted
                        prevModel
                        curModel
                        (stampedUnder prevModel fresh)
                        fresh

                Expect.equal
                    r.RuntimeStateTouchedNodeCount
                    3
                    "focus move touches the two leaves + their shared ancestor"
            }
            ...
            test "a no-change frame (hover persists on the same control) touches 0 and reuses the tree (SC-003/FR-004)" {
                ...
                Expect.isTrue
                    (obj.ReferenceEquals(r.Stamped, prevStamped))
                    "the whole tree is reused (the prev-stamped instance is returned)"
            }
            ...
            test "a fully at-rest frame touches 0 (SC-003)" {
                ...
                let r =
                    ControlRuntime.applyRuntimeVisualStateTargeted emptyModel emptyModel prevStamped fresh
                    ...
                    [
                        for i in 0..8 ->
                            let prevM =
                                modelWith (fun m ->
                                    { m with
                                        HoveredControl = Some(sprintf "b%d" i)
                                    })

                            let curM =
                                modelWith (fun m ->
                                    { m with
                                        HoveredControl = Some(sprintf "b%d" (i + 1))
                                    })

                            (ControlRuntime.applyRuntimeVisualStateTargeted prevM curM (stampedUnder prevM fresh) fresh)
                                .RuntimeStateTouchedNodeCount
                    ]

                Expect.isTrue
                    (counts |> List.forall (fun c -> c <= 3))
                    "every sweep step touches <= 3 nodes regardless of the 51-node tree (SC-006)"

                Expect.isTrue
                    (counts |> List.forall (fun c -> c < total))
                    "the touched count never scales with the control count"
            }
            ...
            test "the count is the regression guard: the full-tree oracle route reports the whole node count (FR-007)" {
                ...
                Expect.equal
                    oracleRoute.RuntimeStateTouchedNodeCount
                    (Control.count fresh)
                    "a whole-tree stamp makes the count jump to the node count"
                // The targeted route (a model-unchanged repaint) stays small.
                let prevModel = modelWith (fun m -> { m with HoveredControl = Some "b4" })

                let targetedRoute =
                    ControlRuntime.runtimeStampFor (Some(prevModel, stampedUnder prevModel fresh)) curModel fresh

                Expect.isTrue
                    (targetedRoute.RuntimeStateTouchedNodeCount < oracleRoute.RuntimeStateTouchedNodeCount)
                    "the targeted route touches far fewer nodes than the whole-tree oracle"
            }
        ]
