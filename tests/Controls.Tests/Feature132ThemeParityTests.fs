...
let private paint theme (c: Control<obj>) =
    ControlInternals.faithfulContent theme box c

[<Tests>]
let feature132ThemeParityTests =
    testList
        "Feature 132 theme parity (FR-013/FR-014, SC-001/SC-007)"
        [
            ...
            test "the parity tree spans every catalog category, including each net-new family (SC-001)" {
                ...
                let required =
                    Set.difference allCategories (Set.ofList [ "chart"; "graph"; "custom" ])
            ...
            test
                "every control renders identically under two themes differing ONLY in Name (FR-014: no theme-identity branch)" {
                let renamed =
                    { defaultLight with
            ...
            test
                "behaviour/accessibility contract is theme-independent (identical node count + diagnostics across themes)" {
                for (id, c) in fullSample do
                    Expect.equal (LControl.count c) (LControl.count c) (sprintf "%s node count is structural" id)

                    Expect.equal
                        (LControl.diagnostics c)
                        (LControl.diagnostics c)
                        (sprintf "%s diagnostics are theme-independent" id)
            }

            test
                "at least one resolved visual property diverges between Default and AntDesign (FR-013 visual divergence)" {
                let divergent =
                    fullSample
            ...
            test "every Ant button intent resolves to a pairwise-distinct style (issue #173)" {
                ...
                for theme in [ antLight; antDark ] do
                    let resolved =
                        intents
                        |> List.map (fun i -> i, StyleResolver.resolve theme "button" i [] Normal)
                        ...
                        for (b, sb) in resolved do
                            if a < b then
                                Expect.notEqual
                                    sa
                                    sb
                                    (sprintf "%s: intents %s and %s must resolve differently" theme.Name a b)
            }
            ...
            test "dashed/text/link Ant Buttons render structurally distinct Scenes (issue #173)" {
                let render intent =
                    paint antLight (Button.create [ Button.text "Save"; Attr.style intent ])

                let byIntent =
                    [ "default"; "dashed"; "text"; "link" ] |> List.map (fun i -> i, render i)
                    ...
                    for (b, sb) in byIntent do
                        if a < b then
                            Expect.notEqual
                                sa
                                sb
                                (sprintf "a %s Button and a %s Button must paint different Scenes" a b)
                ...
                let scenesFor intent =
                    byIntent |> List.find (fst >> (=) intent) |> snd
