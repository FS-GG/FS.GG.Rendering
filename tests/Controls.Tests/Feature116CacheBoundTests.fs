...
let tests =
    testList
        "Feature 116 bounded picture cache (US3, FR-009/010, SC-004)"
        [
            ...
            test "over the cap: PictureCacheEntryCount <= cap at all times (FR-009, SC-004)" {
                ...
                Expect.isTrue
                    ...
                    (sprintf
                        "entry count %d is bounded by the cap %d"
            ...
            test "an evicted entry re-misses with fresh correct paint, never a stale hit (FR-010, SC-004)" {
                ...
                Expect.equal
                    s.Render.Scene
                    full.Scene
                    "the recomputed scene is byte-identical to a fresh paint (never stale)"
            }
        ]
