...
let typedGalleryRenderingTests =
    testList
        "Feature 071 typed gallery rendering (US2)"
        [
            ...
            test "typed gallery panel renders at two viewports and covers every mechanic group (SC-004, SC-005, G5)" {
                ...
                for width, height in [ 320, 240; 1024, 768 ] do
                    ...
                    let evidence =
                        Scene.renderReadbackEvidence { Width = width; Height = height } rendered.Scene

                    Expect.isEmpty rendered.Diagnostics $"typed gallery panel has no diagnostics at {width}x{height}"

                    Expect.isNonEmpty
                        evidence.DeterministicHash
                        $"typed gallery panel render evidence has a deterministic hash at {width}x{height}"
...
let expansionRenderingTests =
    ...
    testList
        "Feature 072 new-control rendering (SC-005)"
        [
                        ...
                        [
                            for width, height in [ 320, 240; 1024, 768 ] do
                                ...
                                Expect.isEmpty
                                    ...
                                    $"{name} renders with no diagnostics at {width}x{height}"

                                Expect.isNonEmpty
                                    evidence.DeterministicHash
                                    $"{name} render evidence has a deterministic hash at {width}x{height}"
...
let renderingTests =
    testList
        "Controls rendering and collections"
        [
            ...
            test "rich text reports unsupported Skia effect diagnostics during measurement" {
                ...
                let evidence =
                    Scene.renderReadbackEvidence { Width = 160; Height = 90 } rendered.Scene

                Expect.isLessThanOrEqual measurement.Width 32.0 "measurement respects max width"

                Expect.exists
                    measurement.Diagnostics
                    (fun item -> item.Code = UnsupportedEnvironment && item.Message.Contains "drop-shadow")
                    "unsupported effect is diagnosed"
            ...
            test "typed views render byte-for-byte identical to legacy IR at multiple viewports" {
                            ...
                            Children =
                                [
                                    FS.GG.UI.Controls.Typed.TextBlock.view
                                        { FS.GG.UI.Controls.Typed.TextBlock.defaults with
                                            Text = "Catalog"
                                        }
                ...
                Expect.equal
                    typedScreen.Accessibility
                    legacyScreen.Accessibility
                    "root accessibility metadata matches legacy"
                ...
                for width, height in [ 320, 240; 1024, 768 ] do
                    ...
                    let typedEvidence =
                        Scene.renderReadbackEvidence { Width = width; Height = height } typedRender.Scene

                    let legacyEvidence =
                        Scene.renderReadbackEvidence { Width = width; Height = height } legacyRender.Scene

                    Expect.isEmpty typedRender.Diagnostics $"typed view has no diagnostics at {width}x{height}"

                    Expect.equal
                        typedRender.NodeCount
                        legacyRender.NodeCount
                        $"node count matches legacy at {width}x{height}"
