...
type GridMsg = RowSelected of string
        ...
        [
            test "large DataGrid keeps visible range bounded at ten thousand rows" {
                ...
                Expect.equal
                    ...
                    "initial visible range is viewport plus buffer row"

                Expect.equal
                    effects
                    [ DataGridVisibleRangeChanged model.VisibleRange ]
                    "initial visible range effect is emitted"

                Expect.isLessThan
                    stopwatch.ElapsedMilliseconds
                    100L
                    "10,000 row visible range initialization stays bounded"
                ...
                Expect.exists
                    scrollEffects
                    (function
                    | DataGridVisibleRangeChanged range -> range = scrolled.VisibleRange
                    | _ -> false)
                    "scroll emits clamped visible range"
            ...
            test "large DataGrid selection and focus emit product-owned effects" {
                ...
                Expect.equal
                    ...
                    (Set.singleton "row-04200")
                    "selected row stays in product-owned DataGrid state"

                Expect.exists
                    selectionEffects
                    (function
                    | DataGridSelectionChanged [ "row-04200" ] -> true
                    | _ -> false)
                    "selection emits explicit effect"
                ...
                Expect.exists
                    focusEffects
                    (function
                    | DataGridFocusChanged(Some focusedCell) -> focusedCell = cell
                    | _ -> false)
                    "focus emits explicit effect"
            }
            ...
            test "large DataGrid render uses visible rows instead of all rows or an empty shell" {
                ...
                Expect.isGreaterThan
                    rendered.NodeCount
                    model.VisibleRange.Count
                    "render includes visible row/header nodes, not just an empty shell"

                Expect.isLessThanOrEqual
                    rendered.NodeCount
                    maxNodes
                    "render stays bounded by the visible range instead of all 10,000 rows"
            ...
            test "two distinct row sets render distinct scenes carrying their own values (issue #175)" {
                ...
                for invented in [ "Widget"; "Gadget" ] do
                    Expect.isFalse
                        (firstTexts.Contains invented)
                        (sprintf "a bound grid never paints the retired sample datum %s" invented)
            }
            ...
            test "invalid DataGrid viewport reports diagnostics through model and effects" {
                ...
                Expect.exists
                    model.Diagnostics
                    (fun item -> item.Code = UnsupportedStateCombination || item.Code = UnsupportedEnvironment)
                    "invalid row height or viewport is diagnosed on the model"

                Expect.exists
                    effects
                    (function
                    | ReportDataGridDiagnostic item -> item.ControlKind = "data-grid"
                    | _ -> false)
                    "invalid viewport emits a diagnostic effect"
            }
        ]
