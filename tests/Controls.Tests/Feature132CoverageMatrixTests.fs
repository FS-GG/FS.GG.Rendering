...
let feature132CoverageMatrixTests =
    testList
        "Feature 132 coverage-matrix honesty check (SC-002/SC-003)"
        [
            ...
            test "every covered row references only live Catalog control ids (H3, SC-003)" {
                for r in
                    ...
                    Expect.isNonEmpty
                        r.RepoControls
                        (sprintf "%s (%s) names at least one repo control" r.Component r.Disposition)
            }
            ...
            test "every covered row references only live DesignSystem token entries (H4, SC-003)" {
                for r in
                    ...
                    Expect.isNonEmpty
                        r.TokenEntries
                        (sprintf "%s (%s) names at least one token entry" r.Component r.Disposition)
            }
