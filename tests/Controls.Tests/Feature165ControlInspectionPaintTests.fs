        ...
        [
            test "root surface coverage overlay roles popup roles and scroll clipping are emitted" {
                ...
                let scroll =
                    Control.create
                        ...
                        [
                            Attr.children
                                [
                                    Stack.create [ Stack.children [ TextBlock.create [ TextBlock.text "row" ] ] ]
                                ]
                        ]
                ...
                Expect.exists
                    ...
                    (fun p -> p.TargetId = "0" && p.CoverageStatus = VisualInspectionCoverageStatus.Complete)
                    "root paint coverage"

                Expect.exists
                    artifact.Regions
                    (fun r -> r.RegionId = "overlay" && r.Role = VisualInspectionSurfaceRole.Overlay)
                    "overlay role"

                Expect.exists
                    artifact.Regions
                    (fun r -> r.RegionId = "popup" && r.Role = VisualInspectionSurfaceRole.Popup)
                    "popup role"

                Expect.exists
                    artifact.ClipFacts
                    (fun c -> c.NodeId = "scroll" && c.ClipStatus = VisualInspectionClipStatus.Intentional)
                    "scroll clipping classified"
            }
            ...
            test "unsupported transformed bounds are explicit facts instead of inferred passing facts" {
                let transformed =
                    Control.customControl
                        "transform-card"
                        [
                            Attr.children [ TextBlock.create [ TextBlock.text "Unsupported transform" ] ]
                        ]
                ...
                Expect.exists
                    ...
                    (fun f -> f.Fact = "transform-bounds" && f.OwnerId = Some "transform")
                    "unsupported transform fact"

                Expect.exists
                    artifact.Nodes
                    (fun n -> n.NodeId = "transform" && not n.UnsupportedFacts.IsEmpty)
                    "node carries unsupported fact"
            }
        ]
