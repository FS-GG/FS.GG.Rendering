...
let private authorScene: Scene =
    {
        Nodes =
            [
                Rectangle((0.0, 0.0, 40.0, 30.0), red)
                Circle({ X = 20.0; Y = 15.0 }, 10.0, white)
            ]
    }

let private canvasCtl (extra: Attr<int> list) (scene: Scene option) : Control<int> =
    let sceneAttrs =
        scene |> Option.map (fun s -> [ Canvas.scene s ]) |> Option.defaultValue []
    ...
    { Canvas.create ([ Attr.width 120.0; Attr.height 80.0 ] @ sceneAttrs @ extra) with
        Key = Some "cv"
    }
...
let private treeWith (canvas: Control<int>) : Control<int> =
    ...
    let chrome =
        { TextBlock.create [ Attr.text "chrome" ] with
            Key = Some "chrome"
        }

    Stack.create [ Attr.children [ canvas; chrome ] ]

let private render (canvas: Control<int>) =
    Control.renderTree theme size (treeWith canvas)
...
let private approxRect (r: Rect) (b: Rect) =
    approx r.X b.X
    && approx r.Y b.Y
    && approx r.Width b.Width
    && approx r.Height b.Height
...
let private hasAuthorRect (nodes: SceneNode list) =
    ...
    |> List.exists (function
        | Rectangle((0.0, 0.0, 40.0, 30.0), _) -> true
        | _ -> false)
...
let tests =
    testList
        "Feature 191 embedded canvas (US1)"
        [
            ...
            test "canvas content paints at the box origin, clipped to the box; explicit size is honoured" {
                ...
                Expect.isTrue
                    (approx box.Width 120.0 && approx box.Height 80.0)
                    "explicit width/height size the control box"
                ...
                | Some body ->
                    ...
                    Expect.isTrue
                        (descend r.Scene
                         |> List.exists (function
                             | Translate((dx, dy), _) -> approx dx box.X && approx dy box.Y
                             | _ -> false))
            ...
            test "an identical model renders a byte-identical Scene + fingerprint; a scene change flips it" {
                ...
                Expect.equal
                    ...
                    (RetainedRender.hashScene [ r2.Scene ])
                    "same model ⇒ identical fingerprint"

                let changed =
                    {
                        Nodes = [ Rectangle((0.0, 0.0, 40.0, 30.0), Colors.rgb 10uy 200uy 10uy) ]
                    } // colour changed
            ...
            test "a canvas with no scene paints a placeholder; a zero-area canvas paints nothing and does not error" {
                ...
                Expect.isFalse
                    (hasAuthorRect (descend placeholder.Scene))
                    "no author content paints when no scene is supplied"
            ...
            test "the existing custom-control placeholder behaviour is unchanged (FR-001)" {
                let custom: Control<int> =
                    { Control.create "custom-control" [ Attr.width 100.0; Attr.height 40.0; Attr.text "widget" ] with
                        Key = Some "cc"
                    }
            ...
            test "a viewport pans/zooms the content only — the box size and clip are unchanged" {
                ...
                Expect.equal
                    (boundsOf "cv" zoomed)
                    (boundsOf "cv" plain)
                    "the viewport leaves the laid-out box unchanged"
                    ...
                    Expect.isFalse
                        (descend body
                         |> List.exists (function
                             | PerspectiveNode _ -> true
                             | _ -> false))
                        "no viewport ⇒ no extra transform"
...
let private volatileCanvas (scene: Scene) : Control<int> =
    { Canvas.create [ Attr.width 120.0; Attr.height 80.0; Canvas.volatile'; Canvas.scene scene ] with
        Key = Some "cv"
    }

let private nonVolatileCanvas (scene: Scene) : Control<int> =
    { Canvas.create [ Attr.width 120.0; Attr.height 80.0; Canvas.scene scene ] with
        Key = Some "cv"
    }
...
let private chromeTreeWith (canvas: Control<int>) (r0: string) : Control<int> =
    {
        ...
        Key = None
        Attributes = []
        Children = [ canvas; dgRow "r0" r0; dgRow "r1" "one" ]
        Content = None
        Accessibility = None
    }
...
let private frameScene (n: int) : Scene =
    {
        Nodes = [ Rectangle((0.0, 0.0, float n, 10.0), red) ]
    }
...
let us2Tests =
    testList
        "Feature 191 embedded canvas (US2)"
        [
            ...
            test "a volatile canvas repainting every frame leaves chrome at cache hits (0 chrome repaints, SC-003)" {
                ...
                let s =
                    RetainedRender.step theme size r0 (chromeTree (volatileCanvas (frameScene 2)))

                Expect.equal
                    s.WorkReduction.PictureCacheHits
                    2
                    "both chrome rows stay picture-cache hits while the canvas repaints"
            ...
            test "a non-volatile canvas with an identical scene between frames is reused, not repainted (FR-003)" {
                ...
                Expect.equal
                    ...
                    0
                    "an identical-scene frame recomputes no cached picture"
                ...
                Expect.equal
                    (strip s.Render.Scene)
                    (strip fresh.Scene)
                    "a reused identical frame is byte-identical to a fresh paint"
            }
            ...
            test
                "volatile' is always-dirty: repaints when a sibling changes though its own scene is unchanged (D4/FR-004)" {
