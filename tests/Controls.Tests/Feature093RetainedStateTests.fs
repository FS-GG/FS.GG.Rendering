...
let feature093RetainedStateTests =
    testList
        "Feature 093 retained state-driven styling (SC-005)"
        [
            ...
            test "a Disabled look survives a sibling-shifting re-render via the live retained path (SC-005)" {
                ...
                let frame2 =
                    Stack.create
                        [
                            Stack.children [ TextBlock.create [ TextBlock.text "header" ]; disabledButton () ]
                        ]

                let step = RetainedRender.step theme size init.Retained frame2

                let before =
                    findByKey "go" init.Retained.Root
                    |> Option.defaultWith (fun () -> failtest "button missing in frame 1")

                let after =
                    findByKey "go" step.Retained.Root
                    |> Option.defaultWith (fun () -> failtest "button missing in frame 2")

                // (1) the retained identity survived the shift (the 067/091/092 mechanism E3 builds on).
                Expect.equal
                    after.Identity
                    before.Identity
                    "the keyed button's RetainedId is stable across the sibling shift"

                // (2) the resolver drove the button's paint through the LIVE retained path at its new box.
                let box =
                    after.Fragment.Box
                    |> Option.defaultWith (fun () -> failtest "button has no evaluated box")

                Expect.equal
                    after.Fragment.OwnScene
                    (ControlInternals.faithfulContent theme box after.Control)
                    "the retained path paints the button via the resolver"
                ...
                Expect.notEqual
                    after.Fragment.OwnScene
                    normalAtSameBox
                    "the surviving look is the Disabled state's, not the Normal-state render"
            }
