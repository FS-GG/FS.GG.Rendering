...
let tests =
    testList
        "Feature 108 theming + WCAG contrast (US6, FR-017/018, SC-010)"
        [
            ...
            test "AA thresholds are checkable via Contrast.verdict (SC-010)" {
                Expect.equal
                    (FS.GG.UI.Color.Contrast.verdict FS.GG.UI.Color.Text 4.5)
                    FS.GG.UI.Color.Aa
                    "4.5:1 passes AA for normal text"

                Expect.equal
                    (FS.GG.UI.Color.Contrast.verdict FS.GG.UI.Color.GraphicOrUi 3.0)
                    FS.GG.UI.Color.Aa
                    "3:1 passes AA for large/graphic"

                Expect.equal
                    (FS.GG.UI.Color.Contrast.verdict FS.GG.UI.Color.Text 2.0)
                    FS.GG.UI.Color.Fail
                    "2:1 fails AA for normal text"
            }
        ]
