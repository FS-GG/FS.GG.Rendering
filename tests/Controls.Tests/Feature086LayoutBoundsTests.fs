...
let private center (r: Rect) =
    r.X + r.Width / 2.0, r.Y + r.Height / 2.0

[<Tests>]
let feature086LayoutBoundsTests =
    testList
        "Feature 086 renderTree bounds + hit-test (US3/US4)"
        [
            ...
            test "unkeyed same-kind siblings get distinct non-overlapping bounds (FR-008)" {
                ...
                let kids =
                    [ "0.0"; "0.1" ]
                    |> List.collect (fun id -> boundsOf id (Control.renderTree theme size vertical))
            ...
            test "Bounds exposes every laid-out control keyed by ControlId (FR-011)" {
                ...
                let buttonId =
                    rendered.EventBindings |> List.map (fun b -> b.ControlId) |> List.head
            ...
            test "hitTest resolves an inside point to its control and a gap to None (FR-012)" {
                let tree: Control<Msg> =
                    Stack.create
                        [
                            Stack.children [ Button.create [ Button.text "Go"; Button.onClick Clicked ] ]
                        ]
                ...
                Expect.equal
                    (Control.hitTest rendered cx cy)
                    (Some "0.0")
                    "inside point hits the button (structural path id)"
