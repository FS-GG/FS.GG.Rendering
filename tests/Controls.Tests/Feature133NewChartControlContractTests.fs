...
let feature133NewChartControlContractTests =
    testList
        "Feature 133 net-new chart control contract (R1/R3/R5/R6)"
        [
            ...
            testList
                "Pure render (no Model/Msg state)"
                [
                    for (id, ctrl) in netNewCharts do
                        test (sprintf "%s dispatches nothing (presentational, parent owns data)" id) {
                            Expect.isEmpty
                                (Control.dispatch (mkEvent "click") ctrl)
                                (sprintf "%s has no event handlers" id)
                        }
                ]
        ]
