...
let private rinit (c: Control<int>) =
...
let private tree (leafAContent: string) (leafAWidth: float) : Control<int> =
    stack
        ...
        [
            stack
                "panel"
                [ attr "width" 200.0; attr "height" 100.0 ]
                [ leaf "leafA" leafAContent leafAWidth 20.0; leaf "leafB" "B" 50.0 20.0 ]
...
let private manualCompose (thm: Theme) (prev: RetainedRender<int>) (next: Control<int>) : RetainedRenderStep<int> =
    ...
    let measureCached (text: string) (font: FontSpec) : TextMetrics =
        ...
        let metrics, tc', wasHit =
            RetainedRender.measureTextCached st.Tc prev.TextCacheEnabled text font
...
let tests =
    testList
        "Feature190 stage pipeline"
        [

            // ---- C-DIFF (T009) ---------------------------------------------------------------------
            test
                "diffStage standalone == Reconcile.diff oracle + layoutDirtySet; duplicate key surfaces KeyCollision (C-DIFF, FR-010)" {
                ...
                let dupPrev =
                    rinit (stack "root" [] [ leaf "x" "A" 40.0 20.0; leaf "x" "B" 40.0 20.0; leaf "y" "Y" 40.0 20.0 ])

                let dupNext = stack "root" [] [ leaf "x" "A" 40.0 20.0; leaf "y" "Y" 40.0 20.0 ]
                let dupResult, _, _ = RetainedRender.diffStage dupPrev dupNext

                Expect.isNonEmpty
                    (dupResult.Diagnostics |> List.filter (fun d -> d.Code = KeyCollision))
                    "FR-010: duplicate key surfaces KeyCollision through diffStage"
            }

            // ---- C-LAYOUT (T010) -------------------------------------------------------------------
            test
                "layoutStage standalone: Remeasured == |Invalidated|, ThemeChanged honest, empty dirty re-measures nothing (C-LAYOUT)" {
                let prev = rinit (tree "A" 50.0)
                ...
                Expect.equal
                    layout.Remeasured
                    (layout.LayoutResult.Invalidated |> List.length)
                    "Remeasured == post-propagation Invalidated count"
                ...
                Expect.isTrue
                    (RetainedRender.layoutStage ctxThemed (seedState prev) next dirty).ThemeChanged
                    "different theme -> ThemeChanged = true"
            }

            // ---- C-PAINT (T011) --------------------------------------------------------------------
            test
                "paintStage standalone: a Replace mints fresh + records ChangedBound/repaint damage matching the inline build (C-PAINT)" {
                                ...
                                [
                                    {
                                        Kind = "button"
                                        Key = Some "leafA"
                                        Attributes = [ attr "width" 60.0; attr "height" 24.0 ]
                                        Children = []
                                        Content = Some "A"
                                        Accessibility = None
                                    }
                ...
                let newRoot =
                    RetainedRender.paintStage ctx st result.Patch layout.BoundsById nextReplace
                ...
                Expect.equal
                    st.Recomputed
                    s.WorkReduction.RecomputedNodeCount
                    "paintStage Recomputed == step RecomputedNodeCount"

                Expect.equal
                    st.ChangedBound
                    s.WorkReduction.ChangedSubtreeBound
                    "paintStage ChangedBound == step ChangedSubtreeBound"
            ...
            test
                "assemblyStage golden: the 40-field WorkReductionRecord from the manual composition == step's, across scenarios (C-ASM)" {
                let scenarios =
                    [
                            ...
                            stack
                                ...
                                [
                                    stack
                                        ...
                                        [ attr "width" 200.0; attr "height" 100.0 ]
                                        [
                                            leaf "leafA" "A" 50.0 20.0
                                            leaf "leafB" "B" 50.0 20.0
                                            leaf "leafC" "C" 50.0 20.0
                                        ]
                                    leaf "sibling" "S" 100.0 30.0
                                ])
                    ]
                ...
                for name, mkPrev, mkNext in scenarios do
                    ...
                    Expect.equal
                        manual.WorkReduction
                        real.WorkReduction
                        (sprintf "%s: 40-field WorkReductionRecord golden matches step" name)

                    Expect.equal manual.Diagnostics real.Diagnostics (sprintf "%s: diagnostics match step" name)
            }

            test
                "composition byte-identity: step.Render at rest == a full Control.renderTree, and the manual composition == step (C-COMPOSE)" {
                let prev = rinit (tree "A" 50.0)
                let next = tree "A" 70.0
            ...
            test "trace parity: every retained-step-* span is emitted under FS_GG_RENDER_LAG_TRACE=1 (C-TRACE, FR-008)" {
                ...
                if enabled then
                    for span in expectedSpans do
                        Expect.isTrue
                            (captured.Contains("event=" + span))
                            (sprintf "FR-008: span %s emitted under the trace" span)
                else
                    Expect.isFalse
                        (captured.Contains "event=retained-step-")
                        "trace channel is silent when FS_GG_RENDER_LAG_TRACE is unset"
            }
        ]
