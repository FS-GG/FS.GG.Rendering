    ...
    testList
        ...
        [
            test "single changed attribute touches only that node and names that attribute (SC-003)" {
                let x0 =
                    node "TextBlock" (Some "x") [ attr "text" (TextValue "hi"); attr "color" (TextValue "red") ] [] None

                let x1 =
                    node
                        "TextBlock"
                        (Some "x")
                        [ attr "text" (TextValue "bye"); attr "color" (TextValue "red") ]
                        []
                        None
            ...
            test "identical trees -> Keep (round-trip identity)" {
                let t =
                    parent
                        [
                            leaf "a" "A"
                            node "Button" (Some "b") [ attr "text" (TextValue "go") ] [] None
                        ]
