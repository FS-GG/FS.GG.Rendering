    ...
    testList
        ...
        [
            ...
            test "container-keyed composite: an inner unbound hit resolves to the container key (R1)" {
                ...
                Expect.equal
                    (Control.nearestAuthored result innerHit)
                    (Some "picker")
                    "inner positional hit resolves to the authored container id"
            }
            ...
            test "directly-keyed leaf resolves to itself (R3)" {
                let tree =
                    Stack.create
                        [
                            Stack.children
                                [
                                    Button.create [ Button.text "go"; Button.onClick Changed ]
                                    |> Control.withKey "go"
                                ]
                        ]
                ...
                Expect.equal
                    (Control.nearestAuthored result "go")
                    (Some "go")
                    "a directly-keyed leaf's hit id is its key; it resolves to itself"
            }
            ...
            test "unkeyed/unbound subtree returns None (R2)" {
                ...
                Expect.equal
                    (Control.nearestAuthored result "0.0")
                    None
                    "no keyed ancestor on the path ⇒ None (never invents an id)"

                Expect.equal
                    (Control.nearestAuthored result "does-not-exist")
                    None
                    "a hit id absent from the tree ⇒ None (total)"
            }
            ...
            test "property: nearestAuthored is an idempotent fixed point, total and deterministic (R4)" {
                ...
                let build (depth: int) (key: string) : Control<Msg> =
                    let leaf =
                        Button.create [ Button.text "t"; Button.onClick Changed ] |> Control.withKey key

                    let rec wrap n inner =
                        if n <= 0 then
                            inner
                        else
                            wrap (n - 1) (Stack.create [ Stack.children [ inner ] ])
