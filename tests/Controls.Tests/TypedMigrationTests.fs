...
let typedMigrationContractTests =
    ...
    testList
        "Feature 070 typed migration contract (SC-005)"
        [
            ...
            test "all 41 typed modules expose defaults/view and lower to their catalog kind" {
                ...
                Expect.equal
                    (kindOf (ValidationMessage.view ValidationMessage.defaults))
                    "validation-message"
                    "validation-message"
                ...
                Expect.equal
                    (kindOf (ScrollViewer.view (ScrollViewer.defaults "sv" leaf)))
                    "scroll-viewer"
                    "scroll-viewer"
...
let typedMigrationParityTests =
    testList
        "Feature 070 lowering parity (41 controls, SC-002)"
        [
            // --- Group 1 Display ------------------------------------------------
            test "Display group lowers structurally equal to legacy *.create" {
                ...
                parityEqual
                    (RichText.view { RichText.defaults with Runs = runs })
                    (LRichText.create (LRichText.block runs) [])
                    "rich-text"
                ...
                parityEqual
                    ...
                    (LImage.create [ LImage.source "img.png" ])
                    "image"
                ...
                parityEqual
                    (Badge.view { Badge.defaults with Text = "beta" })
                    (LBadge.create [ LBadge.text "beta" ])
                    "badge"
                ...
                parityEqual
                    ...
                    (LProgressBar.create [ LProgressBar.value 0.5 ])
                    "progress-bar"
                ...
                parityEqual
                    (ValidationMessage.view
                        { ValidationMessage.defaults with
                            Text = "bad"
                            Severity = Invalid "x"
                        })
                ...
                parityEqual
                    ...
                    (LIconButton.create
                        [
                            ...
                            Attr.enabled true
                            Attr.style "danger"
                            LIconButton.onClick Save
                        ])
                    "icon-button"

                parityEqual
                    (NumericInput.view
                        { NumericInput.defaults with
                            Value = 3.0
                            OnChanged = Some NumberChanged
                        })
                    (LNumericInput.create
                        [
                            LNumericInput.value 3.0
                            Attr.readOnly false
                            LNumericInput.onChanged NumberChanged
                        ])
                    "numeric-input"

                parityEqual
                    (RadioGroup.view
                        { RadioGroup.defaults with
                            Items = [ "a"; "b" ]
                            SelectedKey = Some "a"
                            OnChanged = Some Picked
                        })
                    (LRadioGroup.create
                        [
                            LRadioGroup.items [ "a"; "b" ]
                            LRadioGroup.selected "a"
                            LRadioGroup.onChanged Picked
                        ])
                    "radio-group"
            ...
            test "Container group lowers structurally equal, child order preserved" {
                ...
                parityEqual
                    ...
                    (LWrap.create
                        [
                            ...
                            Attr.create "spacing" Layout (FloatValue 4.0)
                            LWrap.children [ Widget.toControl a ]
                        ])
                    "wrap"

                parityEqual
                    (Border.view
                        { Border.defaults a with
                            Thickness = 2.0
                            Padding = 3.0
                        })
                    (LBorder.create
                        [
                            LBorder.child (Widget.toControl a)
                            Attr.create "thickness" Layout (FloatValue 2.0)
                            Attr.padding 3.0
                        ])
                    "border"
                ...
                parityEqual
                    ...
                    (LControl.standard (StandardControlKind.Custom "scroll-viewer") [ Attr.child (Widget.toControl a) ]
                     |> LControl.withKey "sv")
                    "scroll-viewer"

                parityEqual
                    (SplitView.view
                        { SplitView.defaults with
                            Children = [ a; b ]
                        })
                    (LControl.standard
                        (StandardControlKind.Custom "split-view")
                        [
                            Attr.children [ Widget.toControl a; Widget.toControl b ]
                            Attr.create "orientation" Layout (TextValue "horizontal")
                        ])
                    "split-view"
                    ...
                    (LControl.standard
                        ...
                        [
                            ...
                            transientMetadata
                                ...
                                false
                                (Some "onSelected")
                            Attr.onWith "onSelected" (fun e ->
                                ControlEvent.navText e |> Option.defaultValue "" |> Picked)
                        ]
            ...
            test "Overlay group lowers structurally equal to legacy *.create" {
                ...
                parityEqual
                    (Tooltip.view { Tooltip.defaults with Text = "tip" })
                    (LTooltip.create [ LTooltip.text "tip" ])
                    "tooltip"

                parityEqual
                    (Dialog.view
                        { Dialog.defaults with
                            Title = Some "T"
                            IsOpen = true
                            Children = [ a ]
                            OnSelected = Some Picked
            ...
            test "Charts/graph group lowers structurally equal to legacy *.create" {
                ...
                parityEqual
                    ...
                    (LLineChart.create [ LLineChart.series series ])
                    "line-chart"
                ...
                parityEqual
                    ...
                    (LBarChart.create [ LBarChart.series series ])
                    "bar-chart"
                ...
                parityEqual
                    ...
                    (LPieChart.create [ LPieChart.values points ])
                    "pie-chart"

                parityEqual
                    (ScatterPlot.view
                        { ScatterPlot.defaults with
                            Series = series
                        })
                    (LScatterPlot.create [ LScatterPlot.series series ])
                    "scatter-plot"
            ...
            test "Stateful collections lower structurally equal to Control.standard for the model state" {
                ...
                let legacy =
                    LControl.standard
                        (StandardControlKind.Custom "list-view")
                        [
                            Attr.items [ "a"; "b" ]
            ...
            test "custom-control bridge round-trips a legacy control with structural equality" {
                ...
                Expect.equal
                    (sprintf "%A" (Widget.toControl (CustomControl.ofControl legacy)))
                    (sprintf "%A" legacy)
                    "toControl (ofControl c) = c"
            }
            ...
            test "every optional event prop set to None lowers to no event binding (FR-005)" {
                let noEvents (w: Widget<Msg>) name =
                    Expect.isEmpty (eventAttrs (Widget.toControl w)) $"{name} unset event => no binding"
...
let typedMigrationCatalogTests =
    ...
    let rowsById =
        Catalog.supportedControls |> List.map (fun r -> r.Id, r) |> Map.ofList

    testList
        "Feature 070 catalog cross-check (SC-007)"
        [
...
let typedMigrationDelegationTests =
    testList
        "Feature 070 stateful delegation (SC-003)"
        [
            test "TextArea init/update delegate to TextInput (multiline)" {
                let props: TextAreaProps<Msg> =
                    { TextArea.defaults "note" with
                        Value = "hi"
                    }
            ...
            test "every selection collection's init/update delegate to Collections" {
                ...
                Expect.equal
                    (MultiSelectList.update msg baseModel)
                    expected
                    "MultiSelectList.update == Collections.update"
            ...
            test "stateful facades introduce no parallel model type (SC-003)" {
                ...
                let taInit: TextAreaProps<Msg> -> TextInputModel * TextInputEffect list =
                    TextArea.init

                let lvInit: ListViewProps<Msg> -> CollectionModel * CollectionEffect list =
                    ListView.init
