...
let private stateOf (control: Control<'msg>) =
    ControlInternals.visualStateOf control.Attributes
...
let feature096RuntimeBridgeTests =
    testList
        "Feature096 runtime visual-state bridge"
        [
            ...
            test "T010 — deriveVisualState is deterministic for identical inputs" {
                let m =
                    modelWith (fun m ->
                        { m with
                            PressedControls = Set.ofList [ "a"; "b" ]
                            HoveredControl = Some "c"
                        })

                Expect.equal
                    (ControlRuntime.deriveVisualState m "a")
                    (ControlRuntime.deriveVisualState m "a")
            ...
            test "T011 — a migrated control restyles on hover/press/select with a NO-attribute view" {
                ...
                Expect.equal
                    (stateOf (ControlRuntime.applyRuntimeVisualState hovered (button "btn")))
                    Hover
                    "the bridge stamped Hover onto a control the consumer left unstyled"
                ...
                Expect.equal
                    (stateOf (ControlRuntime.applyRuntimeVisualState selected (button "btn")))
                    Selected
                    "selection stamps Selected"
            }
            ...
            test "T011 — a non-interacted sibling resolves Normal and is returned unchanged" {
                ...
                Expect.equal
                    (dump bridged)
                    (dump sibling)
                    "the untouched sibling is structurally unchanged (no attribute added)"
            }
            ...
            test "T012 — a Normal-and-unset tree is returned UNCHANGED by the bridge" {
                ...
                Expect.equal
                    (dump bridged)
                    (dump tree)
                    "an empty model adds no attribute anywhere — the tree is unchanged"
            }
            ...
            test "T012 — the live retained step recomputes 0 nodes at rest (RecomputedNodeCount unchanged)" {
                ...
                let rest =
                    RetainedRender.step
                        ...
                        size
                        init.Retained
                        (ControlRuntime.applyRuntimeVisualState emptyModel tree)

                Expect.equal
                    rest.WorkReduction.RecomputedNodeCount
                    0
                    "an at-rest bridged frame is identity ⇒ zero recompute"
            }
            ...
            test "T017 — a focused control gains the Focused indicator with no consumer focus attribute" {
                ...
                Expect.equal
                    (stateOf (ControlRuntime.applyRuntimeVisualState focusA (button "a")))
                    Focused
                    "focused control gets Focused"
            ...
            test "T017 — moving focus moves the indicator; the previously-focused control returns to Normal" {
                ...
                Expect.equal
                    (stateOf (ControlRuntime.applyRuntimeVisualState focusA (button "a")))
                    Focused
                    "a is focused under focusA"

                Expect.equal
                    (stateOf (ControlRuntime.applyRuntimeVisualState focusB (button "a")))
                    Normal
                    "a returns to Normal once focus moves to b"

                Expect.equal
                    (stateOf (ControlRuntime.applyRuntimeVisualState focusB (button "b")))
                    Focused
                    "b now carries the indicator"
            }
            ...
            test "T021 — a consumer-Disabled control the runtime reports hovered/pressed/focused stays Disabled" {
                ...
                let disabled: Control<int> =
                    Button.create [ Button.text "Go"; Attr.visualState Disabled ]
                    |> Control.withKey "btn"

                Expect.equal
                    (stateOf (ControlRuntime.applyRuntimeVisualState busy disabled))
                    Disabled
                    "consumer Disabled out-ranks every derived state"
            }
            ...
            test "T021 — a consumer-Selected control the runtime reports Pressed stays Selected" {
                ...
                let selected: Control<int> =
                    Button.create [ Button.text "Go"; Attr.visualState Selected ]
                    |> Control.withKey "btn"

                Expect.equal
                    (stateOf (ControlRuntime.applyRuntimeVisualState pressedModel selected))
                    Selected
                    "consumer Selected out-ranks derived Pressed"
            }
            ...
            test "T021 — a consumer-Normal control the runtime reports focused becomes Focused (derived fills the slot)" {
                ...
                Expect.equal
                    (stateOf (ControlRuntime.applyRuntimeVisualState focused (button "btn")))
                    Focused
                    "derived fills the consumer's Normal slot"
            }
            ...
            test "T024 — a single hover entering one control is a bounded (< baseline) repaint" {
                let tree (m: ControlRuntimeModel) : Control<int> =
                    ControlRuntime.applyRuntimeVisualState
                        m
                ...
                Expect.isLessThan
                    ...
                    w.BaselineNodeCount
                    "a localized hover repaints fewer than all nodes (SC-005)"
            ...
            test "T015/T023 — each widened kind restyles under a runtime state and is byte-identical at rest" {
                ...
                let withState (state: VisualState) (c: Control<'msg>) : Control<'msg> =
                    { c with
                        Attributes = c.Attributes @ [ Attr.visualState state ]
                    }
                ...
                let cases: (string * Control<int> * VisualState) list =
                    [
                        ...
                        "radio-group",
                        RadioGroup.create [ RadioGroup.items [ "a"; "b" ]; RadioGroup.selected "a" ],
                        Pressed
                ...
                for (name, control, visible) in cases do
                    ...
                    let normalAttr =
                        ControlInternals.faithfulContent theme box (withState Normal control)

                    Expect.equal
                        normalAttr
                        atRest
                        (sprintf "%s: a Normal attribute is byte-identical to the unset (at-rest) render" name)

                    let restyled =
                        ControlInternals.faithfulContent theme box (withState visible control)

                    Expect.notEqual
                        restyled
                        atRest
                        (sprintf "%s: a runtime visual state restyles its resolved paint" name)
            }
            ...
            test "T023 — an unmigrated kind shows no render delta under a runtime state" {
                ...
                let unmigrated: Control<int> list =
                    [
                        ProgressBar.create [ ProgressBar.value 0.5 ]
                        NumericInput.create [ NumericInput.value 3.0 ]
                    ]
...
module private Gen096 =
    ...
    let private genIdOpt: Gen<ControlId option> =
        Gen.elements (None :: List.map Some ids)
    ...
    let control (id: ControlId) (consumer: VisualState) : Control<int> =
        let attrs =
            if consumer = Normal then
                []
            else
                [ Attr.visualState consumer ]
...
let feature096BridgePropertyTests =
    testList
        "Feature096 bridge properties (FsCheck, SC-004)"
        [

            testCase "deriveVisualState is total + deterministic over ≥1000 generated combos" (fun () ->
                let prop (m, id, _) =
                    ControlRuntime.deriveVisualState m id = ControlRuntime.deriveVisualState m id

                let config = Config.QuickThrowOnFailure.WithMaxTest 1000
                Check.One(config, Prop.forAll (Arb.fromGen Gen096.tuple) prop))

            testCase
                "the closed order holds: consumer non-Normal preserved, else derived fills the slot (≥1000)"
                (fun () ->
                    ...
                    let prop (m, id, consumer) =
                        let result =
                            Gen096.control id consumer
                            |> ControlRuntime.applyRuntimeVisualState m
                            |> stateOf

                        let expected =
                            if consumer = Normal then
                                ControlRuntime.deriveVisualState m id
                            else
                                consumer
            ...
            testCase "applyRuntimeVisualState is deterministic over ≥1000 generated combos" (fun () ->
                let prop (m, id, consumer) =
                    let once () =
                        Gen096.control id consumer |> ControlRuntime.applyRuntimeVisualState m |> dump
