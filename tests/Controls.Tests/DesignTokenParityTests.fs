...
let designTokenParityTests =
    testList
        "Feature 069 design-token parity"
        [
            ...
            test "the controls gallery renders byte-identically against the token-derived themes (SC-003)" {
                ...
                Expect.equal
                    (hashOf Theme.light)
                    (hashOf frozenLight)
                    "token-derived light render matches the frozen light render"

                Expect.equal
                    (hashOf Theme.dark)
                    (hashOf frozenDark)
                    "token-derived dark render matches the frozen dark render"
            }
            ...
            test "Controls.fsproj gains no new package reference (SC-007)" {
                ...
                Expect.isFalse
                    ...
                    "Controls.fsproj declares no NuGet package reference"

                for forbidden in [ "Fable.Elmish"; "System.Text.Json"; "Newtonsoft.Json"; "FSharp.Data" ] do
                    Expect.isFalse
                        (project.Contains forbidden)
                        (sprintf "Controls.fsproj does not reference %s" forbidden)
            }
            ...
            test "a consumer can reference a generated token by typed name and it resolves to the DTCG value (SC-008)" {
                ...
                Expect.equal
                    branded.Accent
                    (Colors.rgba 37uy 99uy 235uy 255uy)
                    "the typed token resolves to the DTCG accent value"
            }
        ]
