    ...
    stack
        ...
        [
            stack
                ...
                [ leaf "leafA" content w 20.0; leaf "leafB" "B" 50.0 20.0 ]
...
let tests =
    testList
        ...
        [
            ...
            test
                "gate: the retained step scene is byte-identical to a full renderTree at rest over the corpus (FR-005/SC-002)" {
                let corpus =
                    [
                        tree "A" 50.0
                        ...
                        stack
                            ...
                            [ attr "gap" 4.0 ]
                            [
                                stack "panel" [ attr "width" 180.0; attr "height" 90.0 ] [ leaf "x" "X" 40.0 16.0 ]
                                leaf "y" "Y" 60.0 24.0
                            ]
                    ]
                ...
                for c in corpus do
                    ...
                    Expect.equal
                        atRest.Render.Scene
                        full.Scene
                        "retained step at rest == full renderTree (byte-identity)"
            }
            ...
            test
                "gate goes RED on an injected regression (a perturbed damage set) and GREEN on the real decomposition (FR-015/SC-008)" {
                let prev = rinit (tree "A" 50.0)
                let next = tree "A" 70.0
                ...
                Expect.notEqual
                    perturbed.WorkReduction
                    real.WorkReduction
                    "RED: the injected damage-box regression changes the WorkReductionRecord golden"

                Expect.notEqual
                    perturbed.WorkReduction.DirtyRectCount
                    real.WorkReduction.DirtyRectCount
                    "the dropped/added damage box is exactly what the gate detects"
