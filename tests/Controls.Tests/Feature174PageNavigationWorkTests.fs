...
let tests =
    testList
        "Feature174 page navigation metadata work"
        [
            test "navigation to text-numeric-input reuses stable shell metadata" {
                ...
                Expect.isLessThan
                    ...
                    "stable shell metadata is reused"

                Expect.isTrue
                    (step.Render.Bounds |> List.exists (fun (id, _) -> id = "stable-chrome"))
                    "stable chrome remains in rendered metadata"
