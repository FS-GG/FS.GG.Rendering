...
let tests =
    testList
        "Feature147 promotion and placement reuse policy"
        [
            test "stable beneficial parity-clean boundary promotes" {
                let decision =
                    CompositorPolicy.promotionDecision
                        {
                            BoundaryId = "panel"
                            ObservedStabilityFrames = 4
                            ObservationWindow = 3
                            ExpectedSavedWork = 120
                            MeasuredOverhead = 12
                            ParityPassed = true
                        }
            ...
            test "placement-only movement damages old and new regions" {
                let damage =
                    CompositorPolicy.placementDamage 100 100 (rect 0.0 0.0 20.0 20.0) (rect 10.0 0.0 20.0 20.0)
