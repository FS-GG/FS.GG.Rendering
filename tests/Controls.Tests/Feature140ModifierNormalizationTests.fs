...
let private fp effects =
    effects |> List.map entry |> Composition.fingerprint

let private normalized effects =
    effects |> List.map entry |> Composition.normalize
        ...
        [
            test "normalization is idempotent" {
                ...
                Expect.equal
                    ...
                    "fingerprint input is stable after idempotence"
            }
            ...
            test "at least twelve representative equivalent chains have byte-stable fingerprints" {
                let cases =
                    [
                        ...
                        [ Composition.CacheBoundary 1UL; Composition.CacheBoundary 2UL ],
                        [ Composition.CacheBoundary 2UL ]
                ...
                for index, (left, right) in cases |> List.indexed do
                    Expect.equal
                        (fp left)
                        (fp right)
                        (sprintf "case %d equivalent normalized chains fingerprint equally" (index + 1))
            }
            ...
            test "cache-enabled and cache-disabled parity reads the same normalized fingerprint input" {
                ...
                let disabled =
                    [ Composition.CacheBoundary 10UL; Composition.Offset(5.0, 0.0) ] |> normalized

                Expect.equal
                    enabled.FingerprintInput
                    disabled.FingerprintInput
                    "normalization gives cache-on/cache-off parity the same replay input"

                Expect.equal
                    (fp authored)
                    (fp [ Composition.CacheBoundary 10UL; Composition.Offset(5.0, 0.0) ])
                    "fingerprint parity follows the normalized input"
            }
        ]
