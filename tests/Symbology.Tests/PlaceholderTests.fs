...
let private degenLabelled render =
    render
        { Symbology.defaultToken with
            ...
            Cy = 10.0
            R = 0.0
            Label = Some(LabelText.Plain "OVERLONG-CALLSIGN")
        }
            ...
            for gname, render in [ "token", Symbology.token; "badge", Symbology.badge; "ring", Symbology.ring ] do
                test (sprintf "[%s] R <= 0 with a label => placeholder, no throw, no label glyph" gname) {
                    ...
                    Expect.isFalse
                        ...
                        "the placeholder rule wins: no label on a degenerate token"
                }
                ...
                test (sprintf "[%s] R <= 0 with a MULTI-LINE label => placeholder, no throw, no label glyph" gname) {
                    ...
                    Expect.isFalse
                        (List.contains GlyphRunElement kinds)
                        "the placeholder rule wins over a multi-line label"
                ...
                test (sprintf "[%s] R <= 0 with a STYLED rich label => placeholder, no throw, no label glyph" gname) {
                    ...
                    Expect.isFalse
                        (List.contains GlyphRunElement kinds)
                        "the placeholder rule wins over a styled label (B9)"
                ...
                test (sprintf "[%s] R <= 0 with a LAID-OUT / decorated label => placeholder, no throw, no glyph" gname) {
                                        ...
                                        Symbology.laidLabel
                                            [
                                                Symbology.align
                                                    Justify
                                                    [
                                                        { Symbology.run "ALPHA BRAVO" with
                                                            Italic = Some true
                                                            Color = Some(Colors.rgb 24uy 144uy 255uy)
                                                        }
                                                    ]
                    ...
                    Expect.isFalse
                        (List.contains GlyphRunElement kinds)
                        "the placeholder rule wins over a laid-out label (B14)"
    ...
    testList
        ...
        [
            test "a degenerate auto+motion token renders the placeholder, no glyph run (C11/FR-014)" {
                ...
                Expect.isFalse
                    (List.contains GlyphRunElement kinds)
                    "auto/motion suppressed on a degenerate token (placeholder wins)"
            }
            ...
            test "a degenerate motion token never throws at any phase" {
                for ph in [ 0.0; 0.25; 0.5; 0.75; 1.0 ] do
                    ...
                    Expect.isNonEmpty
                        ...
                        (sprintf "placeholder drawn without throwing at phase %f" ph)
            }
        ]
