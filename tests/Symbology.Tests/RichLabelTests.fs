            ...
            for gname, render in grammars do
                ...
                test (sprintf "[%s] Rich [run \"HMR-7\"] (all-default) ≡ Plain \"HMR-7\"" gname) {
                    let plain =
                        render
                            { baseT with
                                Label = Some(LabelText.Plain "HMR-7")
                            }

                    let rich =
                        render
                            { baseT with
                                Label = Some(LabelText.Rich [ Symbology.run "HMR-7" ])
                            }

                    Expect.equal
                        (bytesOf rich)
                        (bytesOf plain)
                        "an all-default single run reproduces the plain label byte-for-byte (FR-002)"
                }
                ...
                test (sprintf "[%s] Rich of all-default runs ≡ the joined Plain label" gname) {
                    let rich =
                        render
                            { baseT with
                                Label = Some(LabelText.Rich [ Symbology.run "BRAVO"; Symbology.run " ac12" ])
                            }

                    let plain =
                        render
                            { baseT with
                                Label = Some(LabelText.Plain "BRAVO ac12")
                            }

                    Expect.equal
                        (bytesOf rich)
                        (bytesOf plain)
                        "default runs join to the equivalent plain label (FR-002)"
                }
                ...
                test (
                    sprintf "[%s] the documented richLabel/plainLabel constructors build and render their label" gname
                ) {
                    ...
                    Expect.equal
                        (Symbology.plainLabel "HMR-7")
                        (LabelText.Plain "HMR-7")
                        "plainLabel builds a Plain label"

                    let rich =
                        render
                            { baseT with
                                Label = Some(Symbology.richLabel runs)
                            }

                    let plain =
                        render
                            { baseT with
                                Label = Some(Symbology.plainLabel "HMR-7")
                            }

                    Expect.equal
                        (bytesOf rich)
                        (bytesOf plain)
                        "an all-default richLabel reproduces the plainLabel byte-for-byte (FR-002)"
                }
        ]
            ...
            for gname, render in grammars do
                            ...
                            { baseT with
                                Label =
                                    Some(
                                        LabelText.Rich
                                            [
                                                { Symbology.run "BRAVO" with
                                                    Weight = Some 700
                                                    Color = Some blue
                                                }
                                                { Symbology.run "x9" with
                                                    Scale = Some 0.6
                                                    Color = Some amber
                                                }
                                            ]
                ...
                test (sprintf "[%s] same chars, different run styling ⇒ different bytes (style is a channel)" gname) {
                    ...
                    let plain =
                        render
                            { baseT with
                                Label = Some(LabelText.Plain "ALFA")
                            }

                    Expect.notEqual
                        (bytesOf styled)
                        (bytesOf plain)
                        "run styling observably alters the bytes (B5/SC-002)"
                }
                ...
                test (sprintf "[%s] an author-supplied run colour survives unchanged into the node (B14)" gname) {
                    ...
                    Expect.contains
                        fills
                        custom
                        "the exact author colour is the node's fill — not re-mapped/rejected (FR-013)"
                }
        ]
            ...
            for gname, render in grammars do
                ...
                test (sprintf "[%s] styled label on the no-measurer path: nodes emitted, deterministic, no throw" gname) {
                    ...
                    Expect.equal
                        (bytesOf (mk ()))
                        (bytesOf (mk ()))
                        "styled layout is a deterministic function of Token (FR-009)"

                    for r in runs do
                        Expect.equal
                            r.Data.FallbackMode
                            PureFallbackMode
                            "with no measurer the node is the deterministic pure fallback (FR-010)"
                }
        ]
            ...
            for gname, render in grammars do
                ...
                test (sprintf "[%s] an over-wide styled run is fitted ≤ region, no mid-glyph clip" gname) {
                            ...
                            { baseT with
                                Label =
                                    Some(
                                        LabelText.Rich
                                            [
                                                { Symbology.run "ALPHA BRAVO CHARLIE DELTA ECHO FOXTROT" with
                                                    Weight = Some 700
                                                }
                                            ]
                                    )
                    ...
                    for r in runs do
                        ...
                        Expect.isLessThanOrEqual
                            drawn
                            (region + 1e-6)
                            (sprintf "segment %A width %f ≤ region %f (FR-006)" r.Data.Text drawn region)
                }
                ...
                test (sprintf "[%s] an over-budget styled label caps to the budget with a trailing ellipsis" gname) {
                    ...
                    Expect.equal
                        runs.Length
                        (budget gname)
                        (sprintf "drawn line count capped to the %s budget (SC-005)" gname)
                ...
                test (sprintf "[%s] an unbreakable over-wide styled word degrades to one fitted line" gname) {
                            ...
                            { baseT with
                                Label =
                                    Some(
                                        LabelText.Rich
                                            [
                                                { Symbology.run "THIS-CALLSIGN-IS-FAR-TOO-LONG-TO-FIT-1234567890" with
                                                    Weight = Some 700
                                                }
                                            ]
                                    )
                    ...
                    Expect.isLessThanOrEqual
                        drawn
                        (regionWidth gname baseT.R + 1e-6)
                        "the unbreakable word is shrunk/ellipsised ≤ region (FR-006)"
                }
        ]
            ...
            for gname, render in grammars do
                ...
                test (sprintf "[%s] mixed-size runs on one line share the first-line baseline (common baseline)" gname) {
                            ...
                            { baseT with
                                Label =
                                    Some(
                                        LabelText.Rich
                                            [
                                                { Symbology.run "BIG" with
                                                    Scale = Some 1.0
                                                    Weight = Some 700
                                                }
                                                { Symbology.run "sm" with
                                                    Scale = Some 0.5
                                                }
                                            ]
                                    )
                    ...
                    Expect.equal
                        ys.Length
                        1
                        "both segments of the single line share one baseline Y (common baseline, B7)"

                    Expect.floatClose
                        Accuracy.high
                        (List.head ys)
                        (firstBaseline gname baseT)
                        "the line keeps the spec-197 first-line baseline (zero-drift anchor)"
                }
                ...
                test (sprintf "[%s] mixed-size stacked lines are non-overlapping and stack downward" gname) {
                    ...
                    Expect.equal
                        (List.distinct ys).Length
                        ys.Length
                        "each stacked line has a distinct baseline (non-overlapping, B7)"
                }
        ]
            ...
            for gname, render in grammars do
                ...
                for label, desc in
                    ...
                    test (sprintf "[%s] %s ⇒ no glyph node, no throw" gname desc) {
                        ...
                        Expect.isFalse
                            ...
                            (sprintf "%s is equivalent to no label (FR-007)" desc)
                    }
                ...
                test (sprintf "[%s] interior empty runs normalise without a drawn gap" gname) {
                            ...
                            { baseT with
                                Label =
                                    Some(
                                        LabelText.Rich
                                            [
                                                { Symbology.run "A" with
                                                    Weight = Some 700
                                                }
                                                Symbology.run ""
                                                { Symbology.run "B" with
                                                    Color = Some amber
                                                }
                                            ]
                                    )
                    ...
                    Expect.isGreaterThan
                        runs.Length
                        0
                        "non-empty styled runs still draw with interior empties dropped (FR-007)"
                }
        ]
            ...
            for gname, render in grammars do
                ...
                for attrName, mk in
                    [
                        ...
                        "tracking", (fun r -> { r with Tracking = Some 0.3 })
                    ] do
                    test (
                        sprintf
                            "[%s] a run setting %s differs in bytes from the same styled run without it (T010)"
                            gname
                            attrName
                    ) {
                        let baseRun =
                        ...
                        let plain =
                            render
                                { baseT with
                                    Label = Some(LabelText.Rich [ baseRun ])
                                }

                        let dec =
                            render
                                { baseT with
                                    Label = Some(LabelText.Rich [ mk baseRun ])
                                }

                        Expect.notEqual
                            (bytesOf dec)
                            (bytesOf plain)
                            (sprintf "%s is a per-run channel (B5/SC-002)" attrName)
                    }
                ...
                test (sprintf "[%s] new attrs set to no-op defaults ≡ Plain (all-default, T011)" gname) {
                    ...
                    let rich =
                        render
                            { baseT with
                                Label = Some(LabelText.Rich [ explicit ])
                            }
                    ...
                    Expect.equal
                        (bytesOf rich)
                        (bytesOf plain)
                        "explicit no-op 199 attrs stay all-default (FR-004/SC-003)"
                }
            ...
            for gname, render in grammars do
                ...
                test (sprintf "[%s] a tracked run spaces glyphs per-character and stays ≤ region (T012)" gname) {
                    ...
                    Expect.isGreaterThan
                        tr.Length
                        untr.Length
                        "tracking is realised as per-glyph positioning (folded into layout, FR-007)"
                    ...
                    for r in tr do
                        ...
                        Expect.isLessThanOrEqual
                            (r.Position.X + w)
                            (right + 1e-6)
                            "tracked glyph not past the region right edge (no overflow, FR-007)"
                }
        ]
        ...
        [
            for gname, render in grammars do
                ...
                test (sprintf "[%s] an underlined wrapped run gets one decoration line per drawn fragment (T013)" gname) {
                    ...
                    let drawnTextLines =
                        collectRuns underlined
                        |> List.map (fun r -> r.Position.Y)
                        |> List.distinct
                        |> List.length

                    let delta =
                        (collectLines underlined |> List.length)
                        - (collectLines undecorated |> List.length)

                    Expect.equal
                        delta
                        drawnTextLines
                        "one underline rule per drawn text line — decoration follows each fragment (B11/FR-008)"
                }
        ]
            ...
            for gname, render in grammars do
                for lname, lbl in labels do
                    test (sprintf "[%s] opt-out (AutoLabel=None,LabelMotion=None) with %s label is inert" gname lname) {
                        ...
                        Expect.equal
                            (bytesOf (render off))
                            (bytesOf (render explicitNone))
                            "the two new None fields are inert (C5/SC-003)"
                    }
                ...
                test (sprintf "[%s] explicit label unchanged by a present-but-overridden AutoLabel (C2)" gname) {
                    ...
                    Expect.equal
                        (bytesOf (render explicit))
                        (bytesOf (render alsoAuto))
                        "AutoLabel is inert when an explicit Label is present (FR-003)"
                }
        ]
