...
let private canonicalSha (scene: Scene) =
    sha (SceneCodec.export scene).CanonicalBytes
...
let private multiline =
    { sample with
        Label = Some(LabelText.Plain "ALPHA\nBRAVO")
    }
                ...
                Symbology.laidLabel
                    [
                        Symbology.align
                            Justify
                            [
                                { Symbology.run "ALPHA BRAVO CHARLIE DELTA" with
                                    Color = Some(Colors.rgb 24uy 144uy 255uy)
                                }
                            ]
                        Symbology.align
        ...
        [
            ...
            test "pure-fallback (no measurer): laid-out label emits a deterministic scene, never throws (B16)" {
                ...
                Expect.contains
                    kinds
                    GlyphRunElement
                    "the pure path still emits the laid-out label glyph nodes (FR-012)"
    ...
    testList
        ...
        [
            ...
            test "the resolved (projected) label still emits glyph nodes on the pure path" {
                let kinds =
                    Symbology.animate Idle autoMotionT 0.5 |> Scene.describe |> List.distinct

                Expect.contains
                    kinds
                    GlyphRunElement
                    "the projected, animated label draws glyph-run nodes (no measurer required)"
            }
