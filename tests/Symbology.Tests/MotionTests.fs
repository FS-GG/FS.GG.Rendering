    ...
    testList
        ...
        [
            yield!
                ...
                |> List.map (fun m ->
                    test (sprintf "animate %A is pure in (motion, token, phase)" m) {
                        Expect.equal
                            (bytesOf (Symbology.animate m t 0.33))
                            (bytesOf (Symbology.animate m t 0.33))
            ...
            test "an active rhythm overlays something on top of the base symbol" {
                ...
                Expect.notEqual
                    (bytesOf (Symbology.animate Pulse t 0.1))
                    (bytesOf (Symbology.token t))
                    "Pulse overlays a rhythm"
            }

            test "phase drives the overlay (different phase => different frame)" {
                Expect.notEqual
                    (bytesOf (Symbology.animate Pulse t 0.1))
                    (bytesOf (Symbology.animate Pulse t 0.8))
                    "phase-dependent"
            }
        ]
                    ...
                    for g in grammars do
                        for m in rhythms ->
                            test (sprintf "animateIn %A %A is deterministic" g m) {
                                Expect.equal
                                    (bytesOf (Symbology.animateIn g m t 0.33))
                                    (bytesOf (Symbology.animateIn g m t 0.33))
                                    "pure in (grammar, motion, token, phase)"
                            }
                ]
            ...
            yield!
                ...
                |> List.map (fun m ->
                    test (sprintf "animateIn Grammar.Token %A reproduces `animate` byte-for-byte" m) {
                        Expect.equal
                            (bytesOf (Symbology.animateIn Grammar.Token m t 0.4))
                            (bytesOf (Symbology.animate m t 0.4))
                            "Token path is the existing animate"
                    })

            test "animateIn Grammar.Badge Idle is the static badge base" {
                Expect.equal
                    (bytesOf (Symbology.animateIn Grammar.Badge Idle t 0.5))
                    (baseOf Grammar.Badge)
                    "Idle = badge base"
            }

            test "a grammar-agnostic overlay (Pulse) draws on top of the Badge base" {
                Expect.notEqual
                    (bytesOf (Symbology.animateIn Grammar.Badge Pulse t 0.1))
                    (baseOf Grammar.Badge)
                    "Pulse overlays a rhythm on the badge"
            }

            test "a directional motion (Spin) degrades to the static base on Badge" {
                Expect.equal
                    (bytesOf (Symbology.animateIn Grammar.Badge Spin t 0.3))
                    (baseOf Grammar.Badge)
                    "Spin is not grammar-agnostic — static base on Badge"
            }

            test "a directional motion (Moving) degrades to the static base on Ring" {
                Expect.equal
                    (bytesOf (Symbology.animateIn Grammar.Ring Moving t 0.3))
                    (baseOf Grammar.Ring)
                    "Moving degrades to the static ring base"
            }

            test "the same overlay differs across grammar bases (Badge vs Ring Pulse)" {
                Expect.notEqual
                    (bytesOf (Symbology.animateIn Grammar.Badge Pulse t 0.2))
                    (bytesOf (Symbology.animateIn Grammar.Ring Pulse t 0.2))
                    "the overlay rides a different base per grammar"
            }
        ]
