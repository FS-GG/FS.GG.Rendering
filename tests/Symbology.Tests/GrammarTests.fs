...
let private bytesOf (scene: Scene) =
    ...
    testList
        ...
        [
            test "ring health arc sweep is monotone non-decreasing across Health in [0,1] (FR-007)" {
                ...
                let monotone = counts |> List.pairwise |> List.forall (fun (a, b) -> b >= a)

                Expect.isTrue
                    monotone
                    (sprintf "ring health element counts must be non-decreasing in Health; got %A" counts)
            }
            ...
            test "ring health sweep strictly grows from empty to full (not a degenerate constant)" {
                ...
                Expect.isLessThan
                    low
                    high
                    "full health must draw a longer sweep than empty health (the test is meaningful)"
            }
        ]
