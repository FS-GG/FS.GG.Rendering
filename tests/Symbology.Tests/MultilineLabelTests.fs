            ...
            for gname, render in grammars do
                test (sprintf "[%s] a `\\n`-bearing label emits N stacked nodes, first at the 196 baseline" gname) {
                    ...
                    Expect.floatClose
                        ...
                        "first line sits at the spec-196 baseline (zero-drift anchor)"
                    ...
                    Expect.equal
                        ...
                        ys.Length
                        "each line has a distinct baseline (non-overlapping)"
                }
                ...
                test (sprintf "[%s] a one-line-fitting label is a SINGLE node at the 196 baseline" gname) {
                    ...
                    Expect.floatClose
                        Accuracy.high
                        (List.head runs).Position.Y
                        (firstBaseline gname baseT)
                        "the single line keeps spec-196's baseline"
                }
                ...
                test (
                    sprintf
                        "[%s] multi-line on the no-measurer pure path: nodes emitted, deterministic, no throw (FR-009)"
                        gname
                ) {
                    let mk () =
                        render
                    ...
                    Expect.isGreaterThan
                        (mk () |> collectRuns |> List.length)
                        0
                        "the pure path still emits the line nodes"

                    Expect.equal
                        (bytesOf (mk ()))
                        (bytesOf (mk ()))
                        "multi-line layout is a deterministic function of Token (FR-009)"
                }
        ]
            ...
            for gname, render in grammars do
                test (sprintf "[%s] a too-wide whitespace label wraps to multiple lines, each ≤ region width" gname) {
                    ...
                    let runs =
                        render
                            { baseT with
                                Label = Some(LabelText.Plain wide)
                            }
                        |> collectRuns
                    ...
                    for r in runs do
                        ...
                        Expect.isLessThanOrEqual
                            drawn
                            (region + 1e-6)
                            (sprintf "wrapped line %A width %f ≤ region %f (FR-005)" r.Data.Text drawn region)
                }

                test (
                    sprintf
                        "[%s] an over-budget label is capped to the budget; the last drawn line ends with the ellipsis"
                        gname
                ) {
                    let many = "L1\nL2\nL3\nL4\nL5\nL6"
                    ...
                    let runs =
                        render
                            { baseT with
                                Label = Some(LabelText.Plain many)
                            }
                        |> collectRuns
                    ...
                    Expect.equal
                        runs.Length
                        (budget gname)
                        (sprintf "drawn line count is capped to the %s budget (FR-005)" gname)

                    Expect.stringEnds
                        (List.last runs).Data.Text
                        "…"
                        "the last drawn line is ellipsised to signal dropped content (SC-005)"
                }
                ...
                test (sprintf "[%s] a blank-lines-only label (\"\\n  \\n\") draws no label and does not throw" gname) {
                    ...
                    Expect.isFalse
                        ...
                        "a blank-lines-only label is equivalent to None (FR-006)"
                }

                test (
                    sprintf
                        "[%s] a single unbroken word wider than the region degrades to one fitted line (no overflow)"
                        gname
                ) {
                    let runs =
                        render
                            { baseT with
                                Label = Some(LabelText.Plain "THIS-CALLSIGN-IS-FAR-TOO-LONG-TO-FIT-1234567890")
                            }
                        |> collectRuns
                    ...
                    Expect.isLessThanOrEqual
                        drawn
                        (region + 1e-6)
                        "the unbroken word is shrunk/ellipsised ≤ region, never overflowed (FR-005)"
                }
        ]
