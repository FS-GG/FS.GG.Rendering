...
let private linesByY (scene: Scene) : (float * GlyphRun list) list =
    collectRuns scene |> List.groupBy (fun r -> r.Position.Y) |> List.sortBy fst

let private rightExtent (nodes: GlyphRun list) =
    nodes
    |> List.map (fun r -> r.Position.X + (Scene.measureGlyphRun r.Data).Width)
    |> List.max

let private leftStart (nodes: GlyphRun list) =
    nodes |> List.map (fun r -> r.Position.X) |> List.min

// styled (non-default) run so a single Center paragraph routes through `laidLabelNodes` (not the reduction).
let private styledRun text =
    { Symbology.run text with
        Color = Some blue
    }

let private laid alignment runs =
    Some(Symbology.laidLabel [ Symbology.align alignment runs ])
            ...
            for gname, render in grammars do
                ...
                test (sprintf "[%s] justify fills wrapped lines; last line un-justified (T024)" gname) {
                    ...
                    let just =
                        render
                            { baseT with
                                Label = laid Justify content
                            }

                    let lead =
                        render
                            { baseT with
                                Label = laid Leading content
                            }
                    ...
                    Expect.floatClose
                        ...
                        (List.last jext)
                        (List.last lext)
                        "the last drawn line is un-justified (matches Leading, FR-008)"
                }
            ...
            for gname, render in grammars do
                ...
                test (sprintf "[%s] single Center all-default paragraph ≡ Rich ≡ Plain (B4, T025)" gname) {
                    let laidL =
                        render
                            { baseT with
                                Label = Some(Symbology.laidLabel [ Symbology.paragraph [ Symbology.run "HMR-7" ] ])
                            }
                    ...
                    Expect.equal
                        (bytesOf laidL)
                        (bytesOf rich)
                        "default Center single paragraph = the Rich flow (FR-004)"
                ...
                test (sprintf "[%s] two paragraphs with different alignment produce distinct lines (T025)" gname) {
                    ...
                    Expect.notEqual
                        topX
                        botX
                        "the Leading and Trailing paragraphs start at different x (per-paragraph alignment)"
                }
        ]
                    ...
                    test (sprintf "[%s] %A over-numerous content caps to budget + ellipsis, ≤ region (T026)" gname a) {
                        ...
                        for r in collectRuns many do
                            ...
                            Expect.isGreaterThanOrEqual
                                r.Position.X
                                (left - 1e-6)
                                "no overflow past the region left edge"

                            Expect.isLessThanOrEqual
                                (r.Position.X + w)
                                (right + 1e-6)
                                "no overflow past the region right edge (SC-005)"
            ...
            for gname, render in grammars do
                for label, desc in
                    [
                        ...
                        "Laid of all-whitespace decorated runs"
                    ] do
                    test (sprintf "[%s] %s ⇒ no glyph node, no throw (T027)" gname desc) {
                        ...
                        Expect.isFalse
                            ...
                            (sprintf "%s is equivalent to no label regardless of alignment/decoration (FR-009)" desc)
                    }
        ]
