    ...
    testList
        ...
        [
                ...
                [
                    for grammar in grammars ->
                        test (sprintf "auto-label adds an observable label node — %A" grammar) {
                            ...
                            Expect.notEqual
                                (bytesIn grammar auto)
                                (bytesIn grammar off)
                                "the projected label is drawn (bytes differ from no-auto-label)"
                        }
                ]
            ...
            test "T011 differing projected channel ⇒ differing auto-label (HealthTier, C3/FR-004)" {
                ...
                Expect.notEqual
                    (bytesIn Grammar.Token lo)
                    (bytesIn Grammar.Token hi)
                    "HealthTier reads Health: differing health ⇒ differing label"
            }
            ...
            test "T011 identical channels ⇒ byte-identical auto-label (deterministic pure projection, FR-004)" {
                ...
                Expect.equal
                    (bytesIn Grammar.Token a)
                    (bytesIn Grammar.Token b)
                    "identical channels ⇒ identical projection bytes"
            }
                ...
                [
                    for grammar in grammars ->
                        test (sprintf "explicit Label wins over AutoLabel — %A" grammar) {
                            ...
                            Expect.equal
                                (bytesIn grammar explicit)
                                (bytesIn grammar explicitOnly)
                                "explicit label drawn (AutoLabel ignored when Label present)"

                            Expect.notEqual
                                (bytesIn grammar explicit)
                                (bytesIn grammar projectionOnly)
                                "the projection is NOT what gets drawn"
                        }
                ]
                ...
                [
                    for grammar in grammars ->
                        test (sprintf "empty fields & all-dropped fields ⇒ no label — %A" grammar) {
                            ...
                            let emptyFields =
                                { off with
                                    AutoLabel = Some(Symbology.autoLabel [])
                                }
                            ...
                            let droppedOnly =
                                { offNoShield with
                                    AutoLabel = Some(Symbology.autoLabel [ ShieldFlag ])
                                }

                            Expect.equal (bytesIn grammar emptyFields) (bytesIn grammar off) "[] fields ⇒ no label node"

                            Expect.equal
                                (bytesIn grammar droppedOnly)
                                (bytesIn grammar offNoShield)
                                "[ShieldFlag] with Shield=false ⇒ projects to nothing ⇒ no label"
                        }
                ]
        ]
