        ...
        [
            ...
            test "empty roster renders reproducibly in every grammar" {
                for g in grammars do
                    Expect.equal
                        (bytesOf (Symbology.galleryIn g 2 80.0 []))
                        (bytesOf (Symbology.galleryIn g 2 80.0 []))
                        "empty roster is reproducible"
            }
            ...
            test "single-unit roster renders reproducibly in every grammar" {
                ...
                for g in grammars do
                    Expect.equal
                        (bytesOf (Symbology.galleryIn g 2 80.0 one))
                        (bytesOf (Symbology.galleryIn g 2 80.0 one))
                        "single roster is reproducible"
            }
        ]
...
let multilineBoardTests =
    ...
    let strip = multilineRoster |> List.map (fun t -> Pulse, t)
...
let styledBoardTests =
    ...
    let plainRoster =
        ...
        |> List.mapi (fun i t ->
            { t with
                R = 40.0
                Label = Some(LabelText.Plain(sprintf "U-%d ac" i))
            })
                            ...
                            [
                                Symbology.align
                                    ...
                                    [
                                        { Symbology.run (sprintf "U-%d" i) with
                                            Weight = Some 700
                                            Color = Some authorBlue
                                        }
                                    ]
                                Symbology.align
                                    Justify
                                    [
                                        Symbology.run "alpha bravo charlie"
        ...
        [
            ...
            test "author-supplied colour is used as-is in the laid-out board (B19/FR-015)" {
                ...
                Expect.contains
                    fills
                    authorBlue
                    "the exact author colour reaches the node — never re-mapped/rejected (FR-015)"
            }
            ...
            test "author-supplied alignment is honoured as-is (Trailing ≠ Leading) (B19/FR-015)" {
                ...
                Expect.notEqual
                    (one Leading)
                    (one Trailing)
                    "the author's alignment choice is applied, not normalised away (FR-015)"
            }
        ]
...
let private autoMotionRoster =
    [
        { roster Ally Mobile with
            AutoLabel = Some(Symbology.autoLabel [ FactionCode; HealthTier ])
            LabelMotion = Some LabelMotion.TypeOn
        }
        { roster Enemy Heavy with
            AutoLabel = Some(Symbology.autoLabel [ KlassCode; SpeedPips ])
            LabelMotion = Some LabelMotion.Fade
            Speed = 3
        }
        { roster Neutral Scout with
            Label = Some(Symbology.plainLabel "ZULU")
            LabelMotion = Some LabelMotion.Pulse
        }
        { roster Ally Scout with
            AutoLabel = Some(Symbology.autoLabel [ StateCode; ThreatTier ])
            State = Suspected
        }
    ]
        ...
        [
            for grammar in [ Grammar.Token; Grammar.Badge; Grammar.Ring ] do
                test (sprintf "[%A] galleryIn of an auto/motion roster is byte-reproducible" grammar) {
                    ...
                    Expect.contains
                        (a |> Scene.describe |> List.distinct)
                        GlyphRunElement
                        "the roster draws labels (projected and/or hand-authored)"
                }

                test (
                    sprintf
                        "[%A] filmstripIn of an auto/motion roster is byte-reproducible under fixed sampling"
                        grammar
                ) {
                    let a =
                        Symbology.filmstripIn grammar 4 [ Idle, autoMotionRoster.[0]; Idle, autoMotionRoster.[1] ]
            ...
            test "the projection reads ONLY Token channels (a channel delta moves the board; nothing else does)" {
                ...
                Expect.notEqual
                    (bytesOf (Symbology.galleryIn Grammar.Token 1 80.0 lo))
                    (bytesOf (Symbology.galleryIn Grammar.Token 1 80.0 hi))
                    "auto-label projects the Token's own Health channel (FR-002)"
                ...
                Expect.equal
                    (bytesOf (Symbology.galleryIn Grammar.Token 1 80.0 same1))
                    (bytesOf (Symbology.galleryIn Grammar.Token 1 80.0 same2))
                    "identical channels ⇒ identical projection (per-game stats stay the caller's, FR-021)"
            }
        ]
