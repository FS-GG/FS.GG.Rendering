...
let private kinds =
    [ LabelMotion.TypeOn; LabelMotion.Fade; LabelMotion.Pulse; LabelMotion.Scroll ]
...
let private frame kind phase =
    bytes (Symbology.animate Idle { baseT with LabelMotion = Some kind } phase)
    ...
    testList
        ...
        [
            ...
            test "T020 filmstrip first sample (phase 0) of a motion-bound token equals the static label" {
                ...
                let staticStripFrame =
                    Symbology.filmstrip 4 [ Idle, { baseT with LabelMotion = None } ]
                // The first column (phase 0) is identical; differences (if any) appear only at later phases.
                Expect.notEqual
                    (bytes strip)
                    (bytes staticStripFrame)
                    "later samples animate (so the strips differ overall)"
            }
                    ...
                    for kind in kinds ->
                        test (sprintf "%A on a NO-label token is phase-invariant" kind) {
                            ...
                            Expect.equal
                                (noLabel kind 0.0)
                                (noLabel kind 0.5)
                                "no label ⇒ nothing to animate ⇒ other channels unaffected"
                        }
                ]
                    ...
                    for kind in kinds ->
                        test (sprintf "%A at a fixed phase renders byte-identically twice" kind) {
                            Expect.equal
                                (frame kind 0.5)
                                (frame kind 0.5)
                                "same (Token, phase) ⇒ byte-identical (in-process)"
                        }
                ]
            ...
            test "T022 pinned motion-frame cross-process golden (TypeOn @ 0.5)" {
                ...
                Expect.equal
                    sha
                    "aec4ad589b1285fdb8e1eb45f6c75a791324e42980cf940683f38ffdb9f18902"
                    "motion-frame canonical bytes drifted from the pinned golden (SC-004)"
            }
            ...
            test "T023 Scroll clips to the region span (no overflow into adjacent channels, C8/FR-011)" {
                let scrollT =
                    { baseT with
                        Label = Some(Symbology.plainLabel "ALPHABRAVOCHARLIEDELTA")
                        LabelMotion = Some LabelMotion.Scroll
                    }
                ...
                for r in clips do
                    ...
                    Expect.isLessThanOrEqual
                        (r.X + r.Width)
                        (cx + halfW + 1e-6)
                        "clip right edge ≤ region right (within region span)"
            }
            ...
            test "T023 TypeOn reveals a whole-glyph prefix (never mid-glyph), shorter than the full label" {
                ...
                let fullText =
                    glyphsOf (mk 1.0) |> List.map (fun g -> g.Data.Text) |> String.concat ""

                let revealed =
                    glyphsOf (mk 0.34) |> List.map (fun g -> g.Data.Text) |> String.concat ""

                Expect.isLessThan
                    revealed.Length
                    fullText.Length
                    "an early phase reveals fewer glyphs than the full label"
                ...
                [
                    for kind in kinds ->
                        test (sprintf "auto-label + %A resolves the projection then animates it" kind) {
                            ...
                            Expect.equal
                                (bytes (Symbology.animate Idle am 0.0))
                                (bytes (Symbology.token { am with LabelMotion = None }))
                                "rest = static projected label"

                            Expect.notEqual
                                (bytes (Symbology.animate Idle am 0.5))
                                (bytes (Symbology.animate Idle am 0.0))
                                "the projected label animates"
                        }
                ]
                ...
                let empty kind ph =
                    bytes (
                        Symbology.animate
                            Idle
                            { baseT with
                                Label = Some(Symbology.plainLabel "   ")
                                AutoLabel = None
                                LabelMotion = Some kind
                            }
                            ph
                ...
                for kind in kinds do
                    for ph in [ 0.0; 0.25; 0.5; 0.75 ] do
                        Expect.equal
                            (empty kind ph)
                            bare
                            (sprintf "%A on an empty label is a no-op at phase %f" kind ph)
            }
        ]
