...
let private channelChanges name (a: Token) (b: Token) =
    test (sprintf "channel '%s' observably alters output" name) {
        Expect.notEqual
            (bytesOf a)
            (bytesOf b)
            (sprintf "%s changes the rendered Scene (canonical-bytes identity)" name)
    }
...
let private grammarChannelChanges (gname: string) (render: Token -> Scene) =
    let changes name (a: Token) (b: Token) =
        test (sprintf "[%s] channel '%s' observably alters output" gname name) {
            Expect.notEqual
                (bytesOfG render a)
                (bytesOfG render b)
                (sprintf "%s changes the %s render (canonical-bytes identity)" name gname)
        }
    ...
    [
        ...
        changes
            ...
            baseT
            { baseT with
                Faction = Custom(Colors.rgb 200uy 20uy 200uy)
            }
        ...
        [
            for gname, render in labelGrammars do
                test (sprintf "[%s] label observably alters output" gname) {
                    ...
                    let lab =
                        { baseT with
                            Label = Some(LabelText.Plain "A-7")
                        }

                    Expect.notEqual
                        (bytesOfG render bare)
                        (bytesOfG render lab)
                        (sprintf "the label changes the %s render" gname)
                }
                ...
                test (sprintf "[%s] two distinct labels render distinguishably" gname) {
                    Expect.notEqual
                        ...
                        (bytesOfG
                            render
                            { baseT with
                                Label = Some(LabelText.Plain "B-9")
                            })
                ...
                test (sprintf "[%s] one-line vs embedded-\\n of the same text differ; neither throws" gname) {
                    let oneLine =
                        bytesOfG
                            render
                            { bigT with
                                Label = Some(LabelText.Plain "A B")
                            }
                    ...
                    Expect.notEqual
                        oneLine
                        twoLine
                        (sprintf "an embedded line break observably alters the %s render (FR-001)" gname)
                }
        ]
            ...
            for gname, render in labelGrammars do
                test (sprintf "[%s] same chars as styled runs vs plain ⇒ differing bytes" gname) {
                    ...
                    Expect.notEqual
                        styled
                        plain
                        (sprintf "run styling observably alters the %s render (B5/SC-002)" gname)
                }
                            ...
                            { bigT with
                                Label =
                                    Some(
                                        LabelText.Rich
                                            [
                                                { Symbology.run "AL" with
                                                    Weight = Some 700
                                                }
                                                { Symbology.run "fa" with
                                                    Scale = Some 0.6
                                                }
                                            ]
                                    )
...
let secondaryHeadingTests =
    let elementCount (render: Token -> Scene) (t: Token) =
        render t |> Scene.describe |> List.length
    ...
    testList
        ...
        [
            for gname, render in labelGrammars do
                test (
                    sprintf
                        "[%s] an unset secondary heading adds no node; setting it adds exactly the barrel + tip"
                        gname
                ) {
                    ...
                    Expect.equal
                        aimed
                        (bare + 2)
                        "the indicator is one line plus one tip mark, and absence draws neither"
                }
    ...
    testList
        ...
        [
            ...
            test "an east-pointing barrel stays inside its own filmstrip cell" {
                ...
                let aimed =
                    { bare with
                        SecondaryHeading = Some(System.Math.PI / 2.0)
                    }
            ...
            test "the widened cell is only used when a barrel is present" {
                ...
                let spanOf scene =
                    ...
                    | [] -> 0.0
                    | cs ->
                        (cs |> List.map (fun (c, _) -> c.X) |> List.max)
                        - (cs |> List.map (fun (c, _) -> c.X) |> List.min)
    ...
    testList
        ...
        [
            ...
            test "identical channels ⇒ byte-identical auto-label (deterministic projection)" {
                ...
                Expect.equal
                    (bytesOf { projT with AutoLabel = spec })
                    (bytesOf { projT with AutoLabel = spec })
                    "pure projection"
            }
        ]
