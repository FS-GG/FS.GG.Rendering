...
let private specs =
    ...
    |> Seq.choose (fun dir ->
        ...
        if not (File.Exists specMd) then
            None
        else
...
let private statusLine =
    Regex(@"^\*\*Status\*\*:[ \t]*(?<value>.+?)[ \t]*$", RegexOptions.Multiline)
...
let specLifecycleTests =
    testList
        "Feature 187 — spec lifecycle currency"
        [
            ...
            test "every spec.md declares exactly one **Status** line" {
                let bad =
                    specs
                    |> List.map (fun s -> s.Name, List.length (statusesOf s))
                    |> List.filter (fun (_, n) -> n <> 1)

                Expect.isEmpty
                    bad
                    (sprintf "spec(s) with a missing or duplicated **Status** line (name, count): %A" bad)
            }
            ...
            test "the retired 'Final' status does not reappear" {
                let offenders =
                    specs
                    |> List.filter (fun s -> statusesOf s |> List.contains "Final")
                    |> List.map (fun s -> s.Name)
