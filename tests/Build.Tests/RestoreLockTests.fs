...
let restoreLockTests =
    testList
        "Feature 211 — locked-restore policy"
        [

            test "the gate solution membership is the expected 42-project LOCKED set" {
                // Guards against the slnx silently gaining/losing a project without the lockfile coverage
                // assertion below being updated; 19 src + 20 tests + 2 samples + 1 tools = 42.
                //
                ...
                // 41 -> 42 in SVG-RUNTIME-01.4, which added the portable browser-session policy suite.
                Expect.equal
                    slnxProjectDirs.Length
                    42
                    (sprintf "expected 42 slnx projects, found %d: %A" slnxProjectDirs.Length slnxProjectDirs)
            }
            ...
            test
                "root Directory.Build.props carries the restore policy (RestorePackagesWithLockFile + gated RestoreLockedMode + NU1603 as-error)" {
                ...
                let warnAsErrorsHasNu1603 = Regex.IsMatch(props, "<WarningsAsErrors>[^<]*NU1603")

                Expect.isTrue
                    warnAsErrorsHasNu1603
            ...
            test "the repo pins a root nuget.config that clears inherited sources" {
                ...
                Expect.isFalse
                    (cfg.Contains "nuget-local" || cfg.Contains "local-feed")
            ...
            test "1089: the published template package project is locked, though it is not an slnx member" {
                ...
                Expect.isFalse
                    (Regex.IsMatch(proj, @"<RestorePackagesWithLockFile>\s*false\s*</RestorePackagesWithLockFile>"))
            ...
            test "1089: the template package's lock is not merely present — it matches the central FSharp.Core pin" {
                ...
                let centralPin =
                    ...
                    let m =
                        Regex.Match(props, @"<PackageVersion\s+Include=""FSharp\.Core""\s+Version=""([^""]+)""")
            ...
            test "1096: lockfile-sync.yml still records WHY its restore-target does not cover the template lock" {
                ...
                let callers =
                    Regex
                        .Matches(
                            wf,
                            @"^\s*uses:\s*FS-GG/\.github/\.github/workflows/lockfile-sync\.yml@",
