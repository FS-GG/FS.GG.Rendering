...
let private packageIdOf (relFsproj: string) : string option =
    ...
    if not (text.Contains "<IsPackable>true</IsPackable>") then
        ...
        if m.Success then
            ...
            failwithf "packable project %s declares no <PackageId>" relFsproj
...
let private claudeMdPlanPointer =
    ...
    let block =
        let b =
            Regex.Match(text, "<!--\\s*SPECKIT START\\s*-->(.*?)<!--\\s*SPECKIT END\\s*-->", RegexOptions.Singleline)
...
let private docLibraryCounts (relDoc: string) =
    libraryCountsIn (File.ReadAllText(repoPath relDoc))
...
let private versionMentionsIn (text: string) =
    [
        ...
        for m in Regex.Matches(text, "dotnet add package FS\\.GG\\.UI\\.\\S+\\s+--version\\s+(\\S+)") ->
            m.Groups.[1].Value
    ]

let private docVersionMentions (relDoc: string) =
    versionMentionsIn (File.ReadAllText(repoPath relDoc))
...
let private generatedRegionText (relDoc: string) =
    ...
    [ for m in generatedRegionRx.Matches(text) -> m.Groups.[1].Value ]
    |> String.concat "\n"
...
let docsCurrencyTests =
    testList
        "Feature 242 — front-door docs currency"
        [
            ...
            test "the packable set is non-empty and complete (sanity on the slnx parse)" {
                ...
                |> List.iter (fun id ->
                    Expect.isTrue (Set.contains id packableIds) (sprintf "expected %s in the packable set" id))
            }
            ...
            test "README/usage.md reflect the shipped release pipeline (nuget.org, no stale feed)" {
                for doc in frontDoorDocs do
                    ...
                    Expect.isFalse
                        (f.Contains "not on a public feed yet")
                        (sprintf "%s still says not on a public feed" doc)

                    Expect.isFalse
                        (f.Contains "not yet on a public nuget feed")
                        (sprintf "%s still says not yet on a public feed" doc)
            }
            ...
            test
                "README/usage.md quote the shipped FS.GG.UI version via a generated fragment (not a hand-typed literal)" {
                for doc in frontDoorDocs do
                    ...
                    Expect.isNonEmpty
                        regionVersions
                        (sprintf
                            "%s states no FS.GG.UI version inside a generated fsgg-doc fragment (scripts/generate-doc-fragments.fsx) for the currency gate to check"
                            doc)
                    ...
                    for v in regionVersions do
                        Expect.equal
                            ...
                            fsGgUiVersion
                            (sprintf
                                "%s's generated fragment states version %s but the pin is $(FsGgUiVersion)=%s — run scripts/generate-doc-fragments.fsx and commit"
                                doc
                                v
                                fsGgUiVersion)
                    ...
                    Expect.isEmpty
                        strayVersions
                        (sprintf
                            "%s states FS.GG.UI version(s) %A OUTSIDE a generated fragment — move them into a fsgg-doc region so they render from source (#966/#968)"
                            doc
                            strayVersions)
            }
            ...
            for doc in frontDoorDocs do
                test (sprintf "%s states the library count via a generated fragment (derived from the slnx)" doc) {
                    ...
                    Expect.isNonEmpty
                        regionCounts
                        (sprintf
                            "%s states no 'N libraries plus the BOM' count inside a generated fsgg-doc fragment for the currency gate to check"
                            doc)
                    ...
                    for n in regionCounts do
                        Expect.equal
                            ...
                            libraryCount
                            (sprintf
                                "%s's generated fragment states %d libraries but the slnx ships %d (packable minus the %s BOM) — run scripts/generate-doc-fragments.fsx and commit"
                                doc
                                n
                                libraryCount
                                bomPackageId)
                    ...
                    Expect.isEmpty
                        strayCounts
                        (sprintf
                            "%s states library count(s) %A OUTSIDE a generated fragment — move them into a fsgg-doc region so they render from source (#966/#968)"
                            doc
                            strayCounts)
                }
            ...
            test "docs/product/module-map.md states the shipped library count (derived from the slnx)" {
                ...
                for n in counts do
                    Expect.equal
                        ...
                        libraryCount
                        (sprintf
                            "docs/product/module-map.md states %d libraries but the slnx ships %d (packable minus the %s BOM)"
                            n
                            libraryCount
                            bomPackageId)
            }
