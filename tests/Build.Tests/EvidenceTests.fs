...
let private freshFixtureDir () =
    let dir =
        Path.Combine(Path.GetTempPath(), "fsggbuild-tests", Guid.NewGuid().ToString("N"))
...
let private writeReadiness (dir: string) (relName: string) (content: string) =
    ...
    Directory.CreateDirectory(Path.GetDirectoryName target |> Option.ofObj |> Option.defaultValue dir)
    |> ignore
...
let evidenceTests =
    testList
        "FS.GG.UI.Build evidence engine"
        [
            ...
            test "EvidenceGraph over a healthy headless readiness surface synthesizes a real graph and passes" {
                ...
                Expect.isFalse
                    (graph.Contains "completed for generated product")
                    "graph is not a completion-only log stub"
            }
            ...
            test "empty surface (only build logs present) FAILS the required-baseline floor (F-BUILD-1)" {
                ...
                Expect.stringContains
                    audit
                    "product-evidence-defect"
                    "absent baseline is classed as a product-evidence defect"
                ...
                Expect.stringContains
                    ...
                    "audit names the absent scene baseline"
            }
            ...
            test "partial baseline (layout present, scene absent) FAILS the required floor" {
                ...
                | Verdict.Fail reason ->
                    ...
                    Expect.isFalse
                        (reason.Contains "layout-evidence.txt (required")
                        "the PRESENT layout baseline is not reported absent"
                | Verdict.Pass -> failtest "a missing scene baseline must not audit Pass"
            }

            test
                "EvidenceAudit honest-fail: a present malformed artifact returns non-zero with verdict=FAIL and product-defect class" {
                let dir = freshFixtureDir ()
                ...
                Expect.stringContains
                    audit
                    "product-evidence-defect"
                    "audit classes the failure as a product-evidence defect"

                Expect.stringContains
                    audit
                    "framework/feed"
                    "audit distinguishes a framework/feed condition from a product defect"

                Expect.stringContains audit "window-options.txt" "audit names the failing artifact"

                Expect.isFalse
                    (audit.Contains "required baseline evidence absent")
                    "with the baseline present, no absent-required reason is emitted"
            }
            ...
            test
                "near-vacuous present baseline (non-whitespace, no structural tokens) FAILS the token contract (F-BUILD-2)" {
                ...
                | Some {
                           State = EvidenceState.PresentInvalid reason
                       } ->
                ...
                Expect.isFalse
                    (audit.Contains "required baseline evidence absent")
                    "the artifact is present, so it is malformed not absent"
            }
