...
let private labelled (lines: string list) (t: Token) =
    { t with
        Label = Some(LabelText.Plain(String.concat "\n" lines))
    }
    ...
    testList
        ...
        [
            test "scoreIn Grammar.Token reproduces score exactly when no grammar-conditional channel is in use" {
                Expect.equal
                    (Legibility.scoreIn Grammar.Token roster)
                    (Legibility.score roster)
                    "Token is the reference grammar"
            }
            ...
            test "usage stays the 12 per-unit table channels — Label is not a table row" {
                for g in grammars do
                    ...
                    Expect.isFalse
                        (usage |> List.exists (fun u -> u.Channel = Legibility.Label))
                        "Label has no usage row"

                    Expect.isFalse
                        (usage |> List.exists (fun u -> u.Channel = Legibility.Motion))
                        "Motion has no usage row"
            }
            ...
            test "the warned-against budget is the budget the renderer actually draws" {
                ...
                for g in grammars do
                    ...
                    let drawn lines =
                        (SceneCodec.export (Symbology.render g (labelled lines wide))).CanonicalBytes
                    ...
                    Expect.equal
                        (drawn (ls (budget + 1)))
                        (drawn truncated)
                        $"%A{g} truncates one line past its budget of {budget}"
                    ...
                    Expect.isEmpty
                        (findingsOn Legibility.Label (Legibility.scoreIn g [ labelled (ls budget) roster.Head ]))
                        "at budget: silent"

                    Expect.hasLength
                        (findingsOn Legibility.Label (Legibility.scoreIn g [ labelled (ls (budget + 1)) roster.Head ]))
                        1
                        "over budget: warned"
            }
            ...
            test "a 3-line label fits Token but busts Badge and Ring" {
                ...
                Expect.isEmpty
                    (findingsOn Legibility.Label (Legibility.scoreIn Grammar.Token board))
                    "3 lines is exactly Token's budget"
                ...
                for g in [ Grammar.Badge; Grammar.Ring ] do
                    ...
                    Expect.equal
                        findings.Head.Severity
                        Legibility.Severity.Warning
                        "the surplus is dropped, not un-encodable"
            ...
            test "a 4-line label busts every grammar, Token included" {
                for g in grammars do
                    ...
                    Expect.hasLength
                        (findingsOn Legibility.Label (Legibility.scoreIn g board))
                        1
                        $"4 lines exceeds %A{g}"
            }
            ...
            test "blank and whitespace-only lines are dropped before counting, as wrapLabel drops them" {
                ...
                Expect.isEmpty
                    (findingsOn Legibility.Label (Legibility.scoreIn Grammar.Token board))
                    "blank segments do not consume budget"

                Expect.hasLength
                    (findingsOn Legibility.Label (Legibility.scoreIn Grammar.Badge board))
                    1
                    "three drawable lines still bust Badge"
            }
            ...
            test "an AutoLabel whose separator carries hard breaks is scored, not ignored" {
                ...
                Expect.hasLength
                    (findingsOn Legibility.Label (Legibility.scoreIn Grammar.Badge [ t ]))
                    1
                    "3 projected lines bust Badge's 2"

                Expect.isEmpty
                    (findingsOn Legibility.Label (Legibility.scoreIn Grammar.Token [ t ]))
                    "...and fit Token's 3"
            }

            test "a Laid label counts one line per paragraph even with no hard break" {
                let para text =
                    Symbology.paragraph [ Symbology.run text ]

                let t =
                    { roster.Head with
                        Label = Some(Symbology.laidLabel [ para "ONE"; para "TWO"; para "THREE" ])
                    }

                Expect.hasLength
                    (findingsOn Legibility.Label (Legibility.scoreIn Grammar.Ring [ t ]))
                    1
                    "three paragraphs bust Ring's 2"
            }
        ]
    ...
    testList
        ...
        [
            test "the bug: scoreAnimated calls a moving Badge unit Clean, and the scene proves it is Idle" {
                ...
                Expect.equal
                    (Legibility.scoreAnimated movingBoard).Verdict
                    Legibility.Clean
                    "the grammar-blind linter is silent"
            ...
            test "scoreAnimatedIn Badge/Ring reports the dropped rhythm as an Error naming the unit" {
                for g in [ Grammar.Badge; Grammar.Ring ] do
                    for motion in [ Spin; Moving ] do
                        ...
                        Expect.equal
                            findings.Head.Severity
                            Legibility.Severity.Error
                            "a dropped channel is an Error, not a Warning"
            ...
            test "scoreAnimatedIn Grammar.Token reproduces scoreAnimated exactly on an unlabelled board" {
                ...
                Expect.equal
                    (Legibility.scoreAnimatedIn Grammar.Token board)
            ...
            test "the whole-board motion Warning still fires, and sorts ahead of the per-unit Errors" {
                ...
                let motionFindings =
                    findingsOn Legibility.Motion (Legibility.scoreAnimatedIn Grammar.Badge board)

                Expect.hasLength motionFindings 3 "one board Warning + two per-unit Errors"

                Expect.equal
                    motionFindings.Head.Severity
                    Legibility.Severity.Warning
                    "the board-level overload sorts first"

                Expect.equal motionFindings.Head.Units [] "...and names no unit"

                Expect.equal
                    (motionFindings |> List.map (fun f -> f.Units))
                    [ []; [ 0 ]; [ 1 ] ]
                    "then ascending unit index"
            }
            ...
            test "scoreAnimatedIn also carries the scoreIn label budget" {
                ...
                Expect.hasLength
                    (findingsOn Legibility.Label (Legibility.scoreAnimatedIn Grammar.Ring board))
                    1
                    "3 lines bust Ring's 2"
            }
        ]
