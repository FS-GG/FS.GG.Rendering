...
let private singleUnitModel (token: Token) : Model =
    ...
    {
        Step = FS.GG.Game.Core.Loop.init { Units = [ unit ]; T = 0.0 }
        Seed = 0
    }
    ...
    testList
        ...
        [
            ...
            test "on-board invariant — every unit centre stays within [radius, extent-radius] (FR-011/SC-003)" {
                ...
                let stepped = List.fold (fun m _ -> update (Tick dt) m) (init 7) [ 1..600 ]
            ...
            test "non-empty board — a zero-area symbol still renders via the grammar placeholder (edge case)" {
                ...
                Expect.isNonEmpty
                    (Scene.describe (renderScene model))
                    "the zero-area placeholder must describe ≥1 element"
            }
        ]
        ...
        [
            test "C13 — the approved M5/M6 roster lints clean (FR-014/SC-005)" {
                ...
                Expect.equal
                    report.Verdict
                    Legibility.Clean
                    "the approved roster lints Clean — the linter agrees with the human approval"
            }
