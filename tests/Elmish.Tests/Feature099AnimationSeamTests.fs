...
let private buttonView () : Control<Msg> =
    Stack.create
        [
            Stack.children [ Button.create [ Button.text "Go" ] |> Control.withKey "go" ]
        ]
...
module private Evidence =
    let readinessRoot =
        Path.GetFullPath(
            Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "specs", "099-live-animation-clock", "readiness")
        )
...
let animatesVsSnaps =
    testList
        "099 US1 a visual-state transition animates (not snaps) on the live seam"
        [
            ...
            test "hover a button → the appearance eases in across frames and converges to exactly the snapped target" {
                ...
                Expect.notEqual
                    frames.[0]
                    snapTarget
                    "the first hover frame is an intermediate (eased) appearance — it does NOT snap to the target"
                ...
                Expect.isGreaterThan
                    idx
                    0
                    "at least one intermediate frame precedes the settled target (animates, not snaps)"

                let intermediates = frames |> List.take idx

                Expect.isTrue
                    (intermediates |> List.forall (fun f -> f <> snapTarget))
                    "every pre-settle frame is a distinct intermediate appearance"

                Expect.isGreaterThan
                    (List.length (List.distinct intermediates))
                    1
                    "the intermediate appearances progress (the fade advances frame to frame)"
...
let survival =
    testList
        "099 US2 an in-flight animation survives an unrelated re-render and completes"
        [

            test
                "hover → tick a few frames → sibling-shifting re-render → the SAME identity's clock continues from prior Elapsed and completes" {
                let view = buttonView ()
                let hover = hoverModel "go"
                ...
                Expect.equal
                    (idOfKey "go" shifted.Retained)
                    idBeforeShift
                    "FR-008: the button keeps its stable RetainedId across the positional shift (no parallel identity scheme)"
                ...
                Expect.isGreaterThan
                    clockAfterShift.Elapsed
                    elapsedBeforeShift
                    "SC-002: the clock continued from its prior Elapsed across the shift (not reset, not dropped)"
                ...
                let completed =
                    clockOf "go" cur
                    |> Option.map RetainedRender.clockActive
                ...
                let elapsedSeq (shiftAt: int option) =
                    ...
                    [
                        for i in 1..18 do
                            let v =
                                match shiftAt with
                                | Some k when i > k -> bannerThenButton ()
                                | _ -> view
                ...
                Expect.equal
                    (elapsedSeq (Some 3))
                    (elapsedSeq None)
                    "FR-004: the shifted run's clock trajectory is identical to the un-shifted run (same final result)"
...
let gc =
    testList
        "099 US4 a removed identity's animation clock is garbage-collected"
        [
            ...
            test "animate a control, then remove it → its identity (incl. its clock) is absent the next frame" {
                ...
                Expect.isTrue
                    (Map.containsKey goId s2.Retained.StateByIdentity)
                    "the button has retained state while animating"
                ...
                let removed =
                    Stack.create
                        [
                            Stack.children [ TextBlock.create [ TextBlock.text "only" ] |> Control.withKey "only" ]
                        ]

                let s3 = frame frameDelta hover removed s2.Retained

                Expect.isFalse
                    (Map.containsKey goId s3.Retained.StateByIdentity)
                    "SC-005/FR-007: the removed identity's state (incl. its clock) is absent the next frame (no leak)"
...
let scopedRepaint =
    testList
        "099 scoped repaint — animation does not force a whole-tree repaint/re-measure"
        [

            test
                "a steady-state hover frame advances the clock (the scene changes) yet recomputes/re-measures ZERO nodes" {
                let view = buttonView ()
                let hover = hoverModel "go"
                ...
                Expect.notEqual
                    s2.Render.Scene
                    s3.Render.Scene
                    "the animation advances — consecutive frames differ (the clock is sampled)"

                // ...yet a steady-state animating frame forces NO measure/paint recompute of the tree.
                Expect.equal
                    s3.WorkReduction.RecomputedNodeCount
                    0
                    "SC-006: advancing the clock repaints no nodes (paint-level overlay, not a whole-tree repaint)"

                Expect.equal
                    s3.WorkReduction.RemeasuredNodeCount
                    0
                    "FR-010: advancing the clock re-measures no nodes (R2 incremental measure preserved)"

                Expect.isTrue
                    ((clockOf "go" s3.Retained |> Option.map RetainedRender.clockActive).Value)
                    "the clock is genuinely active during these frames"
