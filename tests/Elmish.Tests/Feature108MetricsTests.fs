...
let private counts (frames: FrameMetrics list) =
    frames
    |> List.map (fun f ->
        f.RemeasuredNodeCount, f.PointerSamplesReceived, f.PointerMovesProcessed, f.ProductModelChanged)

[<Tests>]
let tests =
    testList
        "Feature 108 ControlsElmish.Perf.runScript metrics + coalescing (US2/3/4, SC-003/004/005/012)"
        [
            ...
            test "a pure-hover frame does not rebuild the view (SC-005)" {
                ...
                Expect.isFalse
                    ...
                    "hover dispatches no product message, so the model does not change"
            }
            ...
            test "a click interleaved with moves is processed within one frame (SC-006)" {
                ...
                Expect.isTrue
                    frames.[1].ProductModelChanged
                    "the click is processed in its frame (onClick -> Inc changed the model)"
            ...
            test "a Tick frame advancing clocks reports no whole-tree rebuild (SC-005, animation edge)" {
                let frames =
                    ControlsElmish.Perf.runScript host size [ FrameInput.Tick(TimeSpan.FromMilliseconds 16.0) ]

                Expect.isFalse
                    frames.[0].ProductModelChanged
                    "an event-driven Tick (no consumer message) changes no model"

                Expect.isTrue
                    (frames.[0].RemeasuredNodeCount >= 0)
                    "remeasure is a bounded count, never a false full rebuild"
            }
