    ...
    let rows =
        [
            for i in 1..rowCount ->
                Button.create
                    [
                        Button.text (sprintf "row %d" i)
                        Button.onClick (RowClicked i)
...
let private stampedFrame (prev: RetainedRender<Msg>) (svId: ControlId) (scroll: ScrollState) =
    let runtimeModel =
        { fst (ControlRuntime.init ()) with
            ScrollOffsets = Map.ofList [ svId, scroll ]
        }
...
let tests =
    testList
        "Feature175ScrollViewerBehaviour"
        [
            test "offset + clamp + thumb + offset-aware hit-test are one cohesive unit driven by one ScrollState" {
                ...
                Expect.isTrue
                    (scroll.Offset > 0.0 && scroll.Offset <= ScrollState.maxOffset scroll)
                    "the offset is clamped within range"
                ...
                Expect.isLessThanOrEqual
                    (thumb + ScrollState.thumbHeight scroll)
                    (track + 0.001)
                ...
                Expect.notEqual
                    after
                    before
                    "the offset-aware hit-test resolves the post-scroll control (same offset, one unit)"
            }
            ...
            test "a huge scroll clamps to the bottom and pins the thumb to the track end (no overscroll)" {
                ...
                Expect.equal
                    scroll.Offset
                    (ScrollState.maxOffset scroll)
                    "a huge delta clamps exactly to maxOffset (no overscroll)"

                let track = viewportHeight

                Expect.floatClose
                    Accuracy.medium
                    (ScrollState.thumbPosition track scroll)
                    (track - ScrollState.thumbHeight scroll)
                    "the thumb sits flush at the track end at max offset"
            }
        ]
