                ...
                [
                    RadioGroup.create
                        [
                            RadioGroup.items items
                            RadioGroup.selected selected
                            Attr.onWith "onSelected" Captured
                        ]
                    |> Control.withKey "rg"
                ]
        ]
                ...
                [
                    Slider.create
                        [
                            ...
                            Attr.accessibility (sliderMeta range)
                        ]
                    |> Control.withKey "sld"
                ]
        ]
...
let private defaultSliderView (value: float) : Control<Msg> =
    Stack.create
        [
            Stack.children
                [
                    Slider.create [ Slider.value value; Attr.onWith "onChanged" Captured ]
                    |> Control.withKey "sld"
                ]
        ]
...
let selectionTests =
    testList
        "100 US1 selection move at the host seam (SC-001)"
        [
            test "ArrowDown moves a focused radio-group's selection to the next item and dispatches its changed binding" {
                ...
                Expect.equal
                    (route r focused ViewerKey.ArrowDown)
                    [ RadioChanged "C" ]
                    "Down: B -> C via the changed fallback (radio binds onChanged)"
            ...
            test "the selection dispatch carries the closed Nav (MovedSelection)" {
                ...
                | [ Captured ev ] ->
                    Expect.equal
                        ev.Nav
                        (Some(MovedSelection(2, Some "C")))
                        "Nav carries the closed MovedSelection (newIndex, item)"
            ...
            test "boundary clamp is a verified no-op: last + Next and first + Previous dispatch NOTHING" {
                ...
                Expect.isEmpty
                    (route rFirst (idOfKey "rg" rFirst) ViewerKey.ArrowUp)
                    "first item + Previous -> no dispatch"
            }
            ...
            test "an empty group and an unresolvable current index dispatch NOTHING (research R-7)" {
                ...
                Expect.isEmpty
                    (route rUnset (idOfKey "rg" rUnset) ViewerKey.ArrowDown)
                    "selected value not in items -> no dispatch"
            }
        ]
        ...
        [
            test "a non-default-step slider steps by EXACTLY its declared step within bounds" {
                ...
                | [ SliderChanged v ] ->
                    Expect.floatClose
                        ...
                        v
                        55.0
                        "ArrowRight steps by the declared step (50 -> 55), NOT the hardcoded 0.1"
                | other -> failtestf "expected one SliderChanged, got %A" other

                match route r focused ViewerKey.ArrowLeft with
                | [ SliderChanged v ] ->
                    Expect.floatClose Accuracy.high v 45.0 "ArrowLeft steps down by the declared step (50 -> 45)"
                | other -> failtestf "expected one SliderChanged, got %A" other
            }
            ...
            test "a DEFAULT-step slider's dispatched value is byte-identical to the pre-R5 numeric path (FR-007 golden)" {
                ...
                | [ Captured ev ] ->
                    Expect.equal
                        ev.Nav
                        (Some(SteppedValue preR5Value))
                        "Nav carries the closed SteppedValue equal to the pre-R5 value"
...
let gridTests =
    testList
        "100 US3 grid 2-D move at the host seam (SC-003)"
        [
            test "ArrowDown/ArrowRight move the focused cell by a 2-D delta and dispatch the resulting cell" {
                ...
                match route r focused ViewerKey.ArrowDown with
                | [ Captured ev ] -> Expect.equal ev.Nav (Some(MovedCell(2, 0))) "ArrowDown: (1,0) -> (2,0)"
            ...
            test "an edge cell + an outward arrow is a verified no-op (edge clamp)" {
                ...
                Expect.isEmpty
                    (route rBottom (idOfKey "grid" rBottom) ViewerKey.ArrowDown)
                    "bottom row + Down -> no dispatch"

                let rLeft = gridView (Some { RowKey = "r1"; ColumnKey = "c0" }) |> rinit

                Expect.isEmpty
                    (route rLeft (idOfKey "grid" rLeft) ViewerKey.ArrowLeft)
                    "left column + Left -> no dispatch"
            }
        ]
...
let nonNavigableTests =
    testList
        "100 US4 non-navigable control no-op (FR-008)"
        [
            test "a focused button is an arrow no-op but still activates on Space/Enter" {
                let view =
                    Stack.create
                        [
                            Stack.children
                                [
                                    Button.create [ Button.text "Go"; Button.onClick (RadioChanged "clicked") ]
                                    |> Control.withKey "btn"
                                ]
                        ]
                ...
                Expect.equal
                    (route r focused ViewerKey.Enter)
                    [ RadioChanged "clicked" ]
                    "Enter still activates the button (E4 unaffected)"
            }
        ]
