...
let tests =
    testList
        "Feature169 Controls.Elmish adapter diagnostic mapping"
        [
            test "Synthetic adapter diagnostic maps to developer-action warning" {
                let context =
                    RuntimeDiagnostics.context (Some "feature169-adapter") None None [ "stream", "runtime" ]

                let adapter =
                    ControlsElmish.diagnostic "control-runtime" "StaleTarget" "Stale control target was ignored."
