...
let private generatedCases () : Rect list list =
    ...
    [
        for _ in 1..200 ->
            ...
            [
                for _ in 1..n -> rect (rng.Next(0, 15)) (rng.Next(0, 15)) (rng.Next(1, 10)) (rng.Next(1, 10))
            ]
    ]
...
let private fullChangeGrid (model: int) : Control<Msg> =
    wrap
        [
            for i in 0 .. gridSize - 1 -> row (sprintf "r%d" i) (sprintf "row-%d-%d" i model)
        ]

[<Tests>]
let tests =
    testList
        "Audit damage-rect mechanism (T007 / T026 US2 / T033 US3)"
        [
            ...
            test
                "Audit: unionArea counts a genuine overlap ONCE — returns the union, not the naive sum (T026, discriminating)" {
                ...
                Expect.isTrue
                    (sum > union)
                    "the rects genuinely overlap: naive sum (20000) strictly exceeds the union (15000)"

                Expect.notEqual
                    union
                    sum
                    "DISCRIMINATING: unionArea returns the union, NOT the naive sum (a sum-based impl would fail here)"
            ...
            test "Audit: unionArea matches an independent brute-force union across 200 generated rect sets (T026)" {
                ...
                Expect.isNonEmpty
                    overlapping
                    "the corpus contains genuine overlaps (union strictly below the naive sum)"
            }
            ...
            test "Audit: a localized change damages a SMALL fraction of the full-repaint baseline (T033, effectiveness)" {
                ...
                Expect.isLessThan
                    ...
                    0.10
                    (sprintf
                        "localized DirtyArea (%d) is a small fraction of full (%d) — fraction %.3f"
                        localized.DirtyArea
                        full.DirtyArea
                        areaFraction)
                ...
                Expect.isLessThan
                    ...
                    0.10
                    (sprintf
                        "localized DirtyRectCount (%d) is a small fraction of full (%d)"
                        localized.DirtyRectCount
                        full.DirtyRectCount)

                Expect.notEqual
                    localized.DirtyArea
                    full.DirtyArea
                    "localized damage is NOT a full repaint (would be a FINDING)"
            }
        ]
