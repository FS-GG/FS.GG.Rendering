            ...
            test "a SWITCH flips both ways under click (the reported select-page bug)" {
                let switchView (_: Size) (on: bool) : Control<Msg> =
                    Stack.create
                        [
                            Stack.children
                                [
                                    Switch.create [ Switch.checked' on; Switch.onChanged (fun v -> SetOn v) ]
                                    |> Control.withKey "sw"
                                ]
                        ]
                ...
                let s2, _, _, _ =
                    ControlsElmish.routeRetainedPointer
                        ...
                        size
                        false
                        (pointer ViewerPointerPhaseKind.Pressed x1 y1)

                let _, secondMsgs, _, _ =
                    ControlsElmish.routeRetainedPointer
                        swHost
                        s.Retained
                        s.Render
                        ...
                        size
                        false
                        (pointer ViewerPointerPhaseKind.Released x1 y1)
