...
let tests =
    testList
        "Feature 110 retained pointer routing — zero routing full renders + preserved coalescing (US1)"
        [
            ...
            test "a routed click on an unbound control performs zero routing full renders (SC-002)" {
                ...
                Expect.equal
                    frames.[0].FullRenderCount
                    0
                    "an unbound click is a pure routing frame: zero full renders (SC-002)"
            ...
            test
                "a routed click on a bound control dispatches its binding FROM the retained frame, no routing render (SC-002)" {
                ...
                Expect.isTrue
                    ...
                    "the authored onClick binding fired from the retained frame"

                Expect.equal frames.[0].FullRenderCount 1 "exactly the model-driven re-render; zero routing renders"

                Expect.equal
                    frames.[0].FullRenderFallbackCount
                    0
                    "the binding was resolved from the retained frame, no fallback (SC-002/SC-005)"
            }

            test
                "a burst of N move samples in one frame: <= 1 processed move, zero routing renders, no fallback (SC-009)" {
                let n = 250

                let moves =
                    [
                        for i in 0 .. n - 1 -> FrameInput.Pointer(HoverEnter("btn", float i, float i))
                    ]
                ...
                Expect.isTrue
                    (frames.[0].PointerMovesProcessed <= 1)
                    "at most one processed move after coalescing (SC-009)"

                Expect.equal
                    frames.[0].FullRenderCount
                    0
                    "the coalesced move performs zero routing full renders (SC-009)"

                Expect.equal frames.[0].FullRenderFallbackCount 0 "no fallback on the burst (SC-005)"
            }

            test
                "a move burst interleaved with discrete interactions drops none and keeps zero routing renders (FR-012)" {
                let cx, cy = centreOf host 0 "btn"
                ...
                Expect.equal
                    frames.Length
                    4
                    "moves coalesce; all three discrete interactions survive as their own frames (FR-012)"
            ...
            test "drag/freehand path fidelity is retained through the retained route (FR-012)" {
                ...
                let frames =
                    ControlsElmish.Perf.runScript host size (dragPath |> List.map FrameInput.Pointer)
                ...
                Expect.equal
                    frames.[0].FullRenderCount
                    0
                    "the coalesced drag performs zero routing full renders (FR-004)"
