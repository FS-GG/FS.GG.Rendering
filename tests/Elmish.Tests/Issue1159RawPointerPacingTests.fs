...
let private host: InteractiveAppHost<Msg list, Msg> =
    {
        ...
        View =
            fun _ _ ->
                Button.create [ Button.text "Fire"; Button.onClick BoundClick ]
                |> Control.withKey "fire"
        Theme = Theme.light
        MapKey = fun _ _ -> None
...
let tests =
    testList
        "issue-1159 raw pointer composition"
        [
            test "raw fallback follows Controls binding without bypassing it" {
                let state, pressed =
                    ControlsElmish.routeInteractivePointerWithRawFallback
                        ...
                        size
                        []
                        (pointer ViewerPointerPhaseKind.Pressed 10.0 10.0)
                ...
                let _, released =
                    ControlsElmish.routeInteractivePointerWithRawFallback
                        ...
                        size
                        []
                        (pointer ViewerPointerPhaseKind.Released 10.0 10.0)
            ...
            test
                "deterministic Controls paced composition applies 1000 moves, preserves click order, and retains counters" {
                let frames =
                    [
                        for frame in 0..59 ->
                            [
                ...
                Expect.equal
                    (receipt.Metrics |> List.sumBy _.RawSamplesReceived)
                    1002
                    "every raw move and discrete sample reaches the Viewer queue"

                Expect.equal
                    (receipt.Metrics |> List.sumBy _.FoldedSamplesApplied)
                    62
                    "one aim plus the lossless press/release pair applies per frame"

                Expect.equal
                    (receipt.Metrics |> List.sumBy _.CoalescedSamples)
                    940
                    "the receipt records dropped raw moves"

                Expect.equal
                    (receipt.Metrics |> List.sumBy _.ModelUpdates)
                    63
                    "the click binding and raw mapper are folded through the host update path"

                Expect.equal receipt.Metrics.[59].PresentedFrames 60L "present counter reaches all 60 frame boundaries"

                Expect.equal
                    receipt.Model.[0..4]
                    [ Aim(10.0, 10.0); BoundClick; Aim(10.0, 10.0); Aim(17.0, 0.0); Aim(17.0, 1.0) ]
                    "discrete press/release and its binding stay ordered ahead of coalesced aims"
            }
        ]
