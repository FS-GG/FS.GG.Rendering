...
let private pressed x y =
    sample PointerPhase.Pressed x y (Some PointerButton.Primary)

let private released x y =
    sample PointerPhase.Released x y (Some PointerButton.Primary)
...
let private corpus: (string * PointerSample list) list =
    [
        ...
        "hover A, then out to empty space (leave with no enter)", [ moved 10.0 10.0; moved 300.0 300.0 ]
        ...
        [
            ...
            <| fun () ->
                ...
                Expect.contains
                    seen
                    (HoverLeave "A")
                    "Perf must route the HoverLeave — the live host always delivers it"
