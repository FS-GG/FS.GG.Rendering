...
let private constView (_: int) : Control<Msg> =
    Stack.create
        [
            Stack.children [ Button.create [ Button.text "fixed" ] |> Control.withKey "b" ]
        ]

[<Tests>]
let tests =
    testList
        "Feature 111 phase-invalidation record (US2, FR-002/SC-002/SC-004)"
        [

            test "an idle frame runs no phase — all four phase fields false (SC-002)" {
                let f =
                    (ControlsElmish.Perf.runScript (mkHost constView) size [ FrameInput.Idle ]).[0]
            ...
            test "a model frame with no visual difference runs view+diff+paint but NOT layout (FR-002)" {
                let frames =
                    ControlsElmish.Perf.runScript (mkHost constView) size [ key (); key () ]
