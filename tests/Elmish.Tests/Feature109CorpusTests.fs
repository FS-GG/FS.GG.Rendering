...
//
// Scheduled snapshots are written to specs/109-perf-metrics-baseline/readiness/perf-corpus/
// <name>.golden.txt by setting PERF_CORPUS_REGEN=1, then uploaded by pending-tests.yml. Each scenario
// also runs twice and asserts byte-identical output, so the scheduled evidence cannot hide instability.
