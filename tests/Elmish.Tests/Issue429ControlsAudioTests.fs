...
let private clickAt (model: Model) (x: float) (y: float) =
    ...
    let _state2, up =
        ControlsElmish.routeInteractivePointer menuHost state1 size model (pointer ViewerPointerPhaseKind.Released x y)
    ...
    testList
        ...
        [
            ...
            test "clicking an authored Button requests the sound its update emits" {
                ...
                Expect.equal
                    audio
                    [ Audio.playSfx click 1.0 ]
                    "the pointer-driven update requested exactly one click sfx"
            }
            ...
            test "a volume change is a pure audio request like any other" {
                ...
                Expect.equal
                    audio
                    [ Audio.setBusVolume Bus.Music 0.25 ]
                    "a settings slider requests a bus-volume change"
            }
