...
let private boundView (_: Size) (model: Model) : Control<Msg> =
    Stack.create
        [
            Stack.children
                [
                    Button.create [ Button.text (string model.Count); Button.onClick Increment ]
                    |> Control.withKey "inc"
                    CheckBox.create
                        [
                            CheckBox.text "on"
...
let private clickAt (host: InteractiveAppHost<Model, Msg>) (model: Model) (x: float) (y: float) =
    ...
    let _state2, up =
        ControlsElmish.routeInteractivePointer host state1 size model (pointer ViewerPointerPhaseKind.Released x y)
    ...
    testList
        ...
        [
            ...
            test "authored onClick + onChanged fire with no MapPointer clauses; MapPointer does not double-fire (G1/G2)" {
                ...
                Expect.isFalse
                    (List.contains Mapped routed)
                    "the competing MapPointer clause did NOT also fire (authored binding wins, no double-advance)"
                ...
                Expect.contains
                    chkRouted
                    (Toggled true)
                    "the authored onChanged binding fired with the flipped value (false → true)"
            }
            ...
            test "container-keyed composite routes its bound message from an inner-node click (R1/G1)" {
                let containerView (_: Size) (model: Model) : Control<Msg> =
                    Stack.create
                        [
                            Stack.children [ Button.create [ Button.text (string model.Count) ] ]
                            Attr.on "onClick" Increment
                        ]
                ...
                Expect.contains
                    routed
                    Increment
                    "a click inside the container-keyed composite routed the container's authored binding via nearestAuthored"
            }
            ...
            test "no authored binding + a MapPointer clause still routes via MapPointer (G3, additive)" {
                ...
                let plainView (_: Size) (_: Model) : Control<Msg> =
                    Stack.create
                        [
                            Stack.children [ Button.create [ Button.text "plain" ] |> Control.withKey "plain" ]
                        ]
            ...
            test "responds-proof: Responsive for a live host, Inert for an inert one (P1/P3, SC-004)" {
                ...
                let proof =
                    ControlsElmish.captureRespondsProof
                        host
                        pressed
                        size
                        model0
                ...
                let inertView (_: Size) (model: Model) : Control<Msg> =
                    Stack.create
                        [
                            Stack.children
                                [ Button.create [ Button.text (string model.Count) ] |> Control.withKey "inc" ]
                        ]
            ...
            test "text seam: a keystroke reaches the focused text control, not an unfocused one (T1/T3)" {
                ...
                let textView (_: Size) (model: Model) : Control<Msg> =
                    Stack.create
                        [
                            Stack.children
                                [
                                    TextBox.create [ TextBox.value model.Name; TextBox.onChanged SetName ]
                                    |> Control.withKey "name"
                                    TextBox.create [ TextBox.value "other"; TextBox.onChanged SetName ]
                                    |> Control.withKey "other"
                                ]
                        ]
                ...
                Expect.equal
                    focusedState.Text.Value.DraftText
                    "a"
                    "the character reached the FOCUSED control's RetainedId-keyed text state"

                Expect.contains
                    msgs
                    (SetName "a")
                    "the focused control's onChanged binding folded the new text into a product message"

                // The other (unfocused) control acquired no text state — only the focused id advances.
                Expect.equal
                    (Map.count r1.StateByIdentity)
                    1
                    "exactly one control (the focused one) has text state; the unfocused control is untouched"
            }
            ...
            test "text seam: with no focus, nothing is delivered and the structure is unchanged (T1)" {
                let textView (_: Size) (_: Model) : Control<Msg> =
                    Stack.create
                        [
                            Stack.children [ TextBox.create [ TextBox.onChanged SetName ] |> Control.withKey "name" ]
                        ]
