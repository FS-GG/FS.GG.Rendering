...
let tests =
    testList
        "Feature 120 timing + replay metrics over Perf.runScript (US1/US3)"
        [

            test
                "per-phase timing is excluded from the deterministic surface: PaintDuration/ComposeDuration are Zero (SC-001/FR-002)" {
                ...
                for f in frames do
                    Expect.equal
                        f.PaintDuration
                        TimeSpan.Zero
                        "PaintDuration is live-only (Zero on the deterministic path)"

                    Expect.equal
                        f.ComposeDuration
                        TimeSpan.Zero
                        "ComposeDuration is live-only (Zero on the deterministic path)"
            }
            ...
            test "a stable grid frame: replay hits coincide with picture-cache hits and skip node work (FR-014/SC-004)" {
                ...
                Expect.equal
                    ...
                    "replay misses coincide with picture-cache misses"
                ...
                Expect.isTrue
                    (f.ReplaySkippedNodeCount > 0)
                    "replayed boundaries skip subtree paint nodes (SC-004 signal)"
