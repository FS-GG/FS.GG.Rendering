...
let private clickOracle (host: InteractiveAppHost<int, Msg>) (model: int) (x: float) (y: float) : Msg list =
    ...
    let _s2, up =
        ControlsElmish.routeInteractivePointer host s1 size model (pointer ViewerPointerPhaseKind.Released x y)
...
let private assertParity host model name x y =
    ...
    Expect.equal
        retained
        oracle
        (sprintf "%s: retained route dispatches the identical message list as the oracle (FR-006/SC-003)" name)

    Expect.equal
        fb
        0
        (sprintf "%s: the retained route resolved from the frame with no full-render fallback (SC-005)" name)
...
let private keyedView (_: Size) (_: int) : Control<Msg> =
    Stack.create
        [
            Stack.children
                [
                    Button.create [ Button.text "a"; Button.onClick (Clicked 0) ]
                    |> Control.withKey "a"
                    CheckBox.create [ CheckBox.text "c"; CheckBox.checked' false; CheckBox.onChanged Toggled ]
                    |> Control.withKey "c"
                ]
        ]
...
let private compositeView (_: Size) (_: int) : Control<Msg> =
    Stack.create
        [
            Stack.children [ Button.create [ Button.text "inner" ] ]
            Attr.on "onClick" (Clicked 7)
        ]
                                ...
                                [
                                    Stack.create
                                        [
                                            Stack.children
                                                [
                                                    Button.create [ Button.text "deep"; Button.onClick (Clicked 9) ]
                                                    |> Control.withKey "deep"
                                                ]
                                        ]
                                ]
                        ]
...
let tests =
    testList
        "Feature 110 retained routing is dispatch-identical to the full-render oracle (US2, FR-006/SC-003)"
        [
            ...
            test
                "unkeyed same-kind siblings: each sibling selects the same one and fires the same binding (SC-004/FR-005)" {
                ...
                Expect.equal
                    second
                    [ Clicked 2 ]
                    "the second unkeyed sibling fires its OWN, different binding (retained identity, FR-005)"
            }
            ...
            test "composite: a click on the inner node dispatches the binding authored ABOVE it (SC-003/FR-003)" {
                ...
                Expect.equal
                    retained
                    [ Clicked 7 ]
                    "the retained-id→authored-id lookup dispatched the container's binding (FR-003)"
            }
            ...
            test "slider: click and drag dispatch onChanged with a clamped numeric payload (SC-003)" {
                ...
                Expect.equal
                    retainedClick
                    [ SliderChanged 0.75 ]
                    "clicking the slider dispatches the pointer-derived value payload"
            ...
            test "MapPointer fallback parity: an unbound control routes through MapPointer identically (FR-006)" {
                ...
                Expect.equal
                    retained
                    [ Clicked 42 ]
                    "the retained route fell through to MapPointer exactly as the oracle"
            }
            ...
            test "focus outcome parity: a click on a focusable control resolves the same focused identity (FR-006)" {
                        ...
                        [
                            Stack.children
                                [
                                    ...
                                    TextBox.create [ TextBox.value "f"; TextBox.onChanged (fun _ -> Clicked 0) ]
                ...
                Expect.equal
                    focused
                    hit
                    "the focus-on-click identity equals the retained hit identity (FR-006 focus clause)"
            }
        ]
