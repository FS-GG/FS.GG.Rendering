...
let tests =
    testList
        "Feature172 retained interactive responsiveness"
        [
            test "pointer click applies product state in the discrete input frame" {
                ...
                Expect.isTrue
                    ...
                    "click dispatched the product message before catch-up frames"
            ...
            test "pointer move burst is coalesced and remains retained-routed" {
                let frames =
                    Feature167ResponsivenessFixtures.run
                        [ for i in 0..5 -> FrameInput.Pointer(HoverEnter("btn", float (8 + i), 12.0)) ]
