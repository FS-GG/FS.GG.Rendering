        ...
        [
            Stack.children
                [
                    ...
                    |> Control.withKey "btn"
                    Slider.create [ Slider.value 0.5; Slider.onChanged SliderChanged ]
                    |> Control.withKey "sld"
                ]
        ]
...
let private textView () : Control<Msg> =
    Stack.create
        [
            Stack.children
                [
                    TextBox.create [ TextBox.value "hi"; TextBox.onChanged TextChanged ]
                    |> Control.withKey "editor"
                ]
        ]
...
let activationTests =
    testList
        "094 US2 routeFocusedKey activation + navigation (SC-002)"
        [
            test "a focused Button + an ActivationKey produces exactly the pointer-equivalent message ONCE" {
                ...
                Expect.equal
                    msgs
                    [ Clicked ]
                    "Enter dispatches the same message a pointer click would, exactly once (no double-dispatch)"

                Expect.isEmpty ctrlMsgs "activation emits no traversal message"

                let _, _, spaceMsgs =
                    ControlsElmish.routeFocusedKey r focused (order r) ViewerKey.Space false
            ...
            test "a focused Slider + ArrowRight/ArrowLeft produces its value-change message" {
                ...
                let _, _, rightMsgs =
                    ControlsElmish.routeFocusedKey r focused (order r) ViewerKey.ArrowRight false

                match rightMsgs with
                | [ SliderChanged v ] ->
                    Expect.floatClose Accuracy.medium v 0.6 "ArrowRight steps the slider value up (0.5 -> 0.6)"
                | other -> failtestf "expected exactly one SliderChanged, got %A" other

                let _, _, leftMsgs =
                    ControlsElmish.routeFocusedKey r focused (order r) ViewerKey.ArrowLeft false

                match leftMsgs with
                | [ SliderChanged v ] ->
                    Expect.floatClose Accuracy.medium v 0.4 "ArrowLeft steps the slider value down (0.5 -> 0.4)"
                | other -> failtestf "expected exactly one SliderChanged, got %A" other
            }
            ...
            test "an unmatched key falls through — no msgs, no traversal (SC-006 totality)" {
                ...
                let _, ctrlMsgs, msgs =
                    ControlsElmish.routeFocusedKey r focused (order r) (ViewerKey.Unknown "Q") false
...
let traversalWiringTests =
    testList
        "094 US1 traversal at the host seam (SC-001)"
        [
            test "Tab advances focus to the next stop, Shift+Tab to the previous" {
                ...
                let _, fwd, _ =
                    ControlsElmish.routeFocusedKey r btn (order r) (ViewerKey.Unknown "Tab") false

                Expect.equal fwd [ FocusControl(Some "sld") ] "Tab from btn moves focus to sld"

                let _, back, _ =
                    ControlsElmish.routeFocusedKey r sld (order r) (ViewerKey.Unknown "Shift+Tab") true

                Expect.equal back [ FocusControl(Some "btn") ] "Shift+Tab from sld moves focus back to btn"

                let _, wrap, _ =
                    ControlsElmish.routeFocusedKey r sld (order r) (ViewerKey.Unknown "Tab") false
...
let textSeamTests =
    testList
        "094 US2 E1 text seam preserved (SC-003)"
        [
            test "a focused text control still receives typed text through the unchanged routeFocusedText path" {
                ...
                Expect.equal
                    draft
                    "hiX"
                    "SC-003: the first keystroke appends to the pre-filled value, unchanged from E1"
...
let stabilityTests =
    testList
        "094 US3 focus stability over the live retained path (SC-004)"
        [
            test
                "after a sibling-shifting RetainedRender.step the focused control keeps its RetainedId and still activates" {
                ...
                Expect.equal
                    (idOfKey "btn" s.Retained)
                    focused
                    "SC-004: btn keeps its stable RetainedId across the positional shift (not a hand-seeded map)"

                // the SAME focused RetainedId still routes activation on the post-shift retained tree.
                let _, _, msgs =
                    ControlsElmish.routeFocusedKey s.Retained focused (order s.Retained) ViewerKey.Enter false
...
let pointerCompositionTests =
    testList
        "094 US3 pointer<->keyboard composition (FR-006)"
        [
            test "a pointer press sets focus to the focusable control under it; traversal continues from there" {
                ...
                Expect.isTrue
                    (node.Control.Accessibility |> Option.exists (fun m -> m.Keyboard.Focusable))
                    "the slider is focusable, so the host adopts it as focus"

                // subsequent Tab continues from the pointer-set focus (sld -> wraps to btn).
                let _, fwd, _ =
                    ControlsElmish.routeFocusedKey r hit (order r) (ViewerKey.Unknown "Tab") false
            ...
            test "a press on a NON-focusable region leaves the current focus unchanged (not silently cleared)" {
                ...
                Expect.isFalse
                    isFocusable
                    "the banner under the press is non-focusable, so the host leaves the current focus unchanged (FR-006)"
            }
        ]
...
let focusIndicatorTests =
    ...
    testList
        "094 US3 focus indicator via E3 resolver (SC-005)"
        [
            test "the Focused visual state resolves to a distinct style — the indicator is resolver-driven" {
                ...
                Expect.notEqual
                    focusedStyle
                    normal
                    "Focused resolves distinctly from Normal (no procedural per-kind focus branch)"

                Expect.equal
                    focusedStyle.Stroke
                    theme.Accent
                    "the focus indicator IS the resolver's Focused stroke (theme.Accent)"
            }
...
let respondsProofTests =
    testList
        "094 US3 responds-proof for a key-driven focus change"
        [
            test "a key-driven focus change yields a visible change (Responsive); an inert view yields Inert" {
                ...
                let focusOn (focusedKey: string) : Control<Msg> =
                    let btnFocus =
                        if focusedKey = "btn" then
                            [ Attr.visualState Focused ]
                        else
                            []

                    let sldFocus =
                        if focusedKey = "sld" then
                            [ Attr.visualState Focused ]
                        else
                            []
                    ...
                    Stack.create
                        [
                            Stack.children
                                [
                                    TextBlock.create [ TextBlock.text $"focused: {focusedKey}" ]
                                    |> Control.withKey "indicator"
                                    Button.create ([ Button.text "Go"; Button.onClick Clicked ] @ btnFocus)
                                    |> Control.withKey "btn"
                                    Slider.create ([ Slider.value 0.5; Slider.onChanged SliderChanged ] @ sldFocus)
                                    |> Control.withKey "sld"
                                ]
                        ]
                ...
                Expect.equal
                    proof.Verdict
                    Responsive
                    "the key-driven focus change produced a visible change (Responsive)"
