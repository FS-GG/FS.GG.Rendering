        ...
        [
            ...
            test "US1 AS1: an unkeyed Button.onClick dispatches; MapPointer is not consulted" {
                ...
                Expect.contains
                    routed
                    Inc
                    "the unkeyed onClick binding fired in the live route (dead button is now alive)"

                Expect.isFalse
                    (List.contains Mapped routed)
                    "the competing MapPointer clause did NOT also fire (binding wins)"
            }
            ...
            test "US1 AS2: a nested unkeyed bound control inside an unbound container dispatches" {
                    ...
                    Stack.create
                        [
                            Stack.children
                                [
                                    Stack.create
                                        [ Stack.children [ Button.create [ Button.text "deep"; Button.onClick Inc ] ] ]
                                ]
                        ]
                ...
                Expect.contains
                    routed
                    Inc
                    "a click on the deeply-nested unkeyed bound control recovered it and dispatched"
            }
                ...
                let keyedView (_: Size) (_: Model) : Control<Msg> =
                    Stack.create
                        [
                            Stack.children
                                [
                                    Button.create [ Button.text "go"; Button.onClick Inc ] |> Control.withKey "go"
                                ]
                        ]
            ...
            test "US2 AS2: a container-keyed composite dispatches from an inner unbound-node click" {
                let containerView (_: Size) (_: Model) : Control<Msg> =
                    Stack.create
                        [
                            Stack.children [ Button.create [ Button.text "label" ] ]
                            Attr.on "onClick" Outer
                        ]
                ...
                Expect.contains
                    routed
                    Outer
                    "the inner-node click climbed to the keyed container and dispatched its binding"
            }
            ...
            test "US2 AS3: a keyed+bound control dispatches exactly once (no double-dispatch)" {
                ...
                Expect.equal
                    (routed |> List.filter ((=) Inc) |> List.length)
                    1
                    "exactly one dispatch for a keyed+bound control (no double-dispatch)"
            }
            ...
            test "US2: a MapPointer-only consumer is bit-for-bit unchanged" {
                ...
                Expect.equal
                    routed
                    [ MappedInner ]
                    "a MapPointer-only consumer routes exactly its mapped message (unchanged)"
            }
        ]
