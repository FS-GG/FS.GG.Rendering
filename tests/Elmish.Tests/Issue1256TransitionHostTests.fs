        ...
        [
            test "delayed worker and feature responses share one revision-fenced transaction" {
                ...
                let withFeatures, featureEffects =
                    withWorker
                    |> step (
                        response revision0 TransitionResponseKind.ClientFeatures (Features [ "tools"; "timeline" ])
                    )
            ...
            test "hidden tabs retain the latest target and converge exactly once on resume" {
                ...
                Expect.equal beginEffects [] "hidden begin withholds presentation and physical focus"
                ...
                Expect.equal hiddenSimulateEffects [] "hidden replacement emits no physical focus directive"
            ...
            test "pending host suppresses capture and global dispatch while controlled input remains synchronous" {
                ...
                let withFile, fileEffects =
                    withText
                    |> step (
                        TransitionHostMsg.InputAttempted(
                            TransitionHostInput.ControlledFileChanged("map", Some "map-v2")
                        )
                    )
            ...
            test "production Fable React Chromium route stays inside the filed frame budget" {
                ...
                let artifactRoot, artifact, traceDirectory =
                    ...
                    | None ->
                        let root =
                            Path.Combine(Path.GetTempPath(), $"fsgg-transition-host-{Guid.NewGuid():N}")
                ...
                try
                    ...
                    let transcript =
                        runCommand
                            fixture
                            "npm"
                            [ "run"; "measure"; "--"; "--out"; artifact; "--trace-dir"; traceDirectory ]
                    ...
                    let candidateHead =
                        root.GetProperty("candidate").GetProperty("gitHead").GetString() |> Option.ofObj
