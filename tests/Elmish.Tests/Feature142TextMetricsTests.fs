...
let tests =
    testList
        "Feature142 Elmish text metrics contract"
        [
            test "shaped metric evidence is stable enough for retained frame metrics" {
                ...
                Expect.equal
                    direct.Fingerprint
                    cold.Fingerprint
                    "direct and cold retained evidence match for unchanged text"

                Expect.equal cold.Fingerprint warm.Fingerprint "warm retained evidence remains stable"

                Expect.equal
                    (Scene.measureShapedText direct)
                    (Scene.measureShapedText warm)
                    "metrics are equivalent across modes"
            }
        ]
