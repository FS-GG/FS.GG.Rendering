...
let tests =
    testList
        "Feature167 interaction semantics"
        [
            test "continuous pointer movement is deterministic and coalesced" {
                let script =
                    [ for i in 0..4 -> FrameInput.Pointer(HoverEnter("btn", float i, float i)) ]
            ...
            test "Perf.runScript remains clock-free and repeatable" {
                ...
                let a =
                    Feature167ResponsivenessFixtures.run script
                    |> List.map Feature167ResponsivenessFixtures.deterministicShape
