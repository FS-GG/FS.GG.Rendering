...
let adapterCmdTests =
    testList
        "AdapterCmd bridge (AdapterCommand <-> Elmish Cmd)"
        [
            ...
            test
                "two product effects dispatch exactly those messages, in order, none dropped or duplicated (US2 acceptance 1)" {
            ...
            test
                "FsCheck round-trip over real commands: product-message dispatch equals productMessages (>=1000 cases, SC-003/FR-008)" {
            ...
            test
                "FsCheck totality/order: dispatch order equals List.map route over every effect case (>=1000 cases, FR-003)" {
