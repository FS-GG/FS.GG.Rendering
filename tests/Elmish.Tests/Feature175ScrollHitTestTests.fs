...
let private scrolledFrame (prev: RetainedRender<Msg>) (offset: float) =
    ...
    let runtimeModel =
        { fst (ControlRuntime.init ()) with
            ScrollOffsets = Map.ofList [ "sv", scroll ]
        }
...
let private rowY (render: ControlRenderResult<Msg>) i =
    render.Bounds
    |> List.find (fun (id, _) -> id = sprintf "row%d" i)
    |> snd
    |> fun (r: Rect) -> r.Y
        ...
        [
            test "the live pointer route resolves the POST-scroll control over scrolled content (F2)" {
                ...
                | [ RowClicked a ], [ RowClicked b ] ->
                    Expect.notEqual
                        b
                        a
                        "the same screen point resolves to a DIFFERENT row after scrolling (not the stale pre-scroll control)"
            ...
            test "pointer route and focus resolution AGREE on the scrolled control (F2 two-consumer parity)" {
                ...
                let focusRow =
                    ...
                    |> Option.bind (fun id ->
                        RetainedRender.authoredControlIds s.Render.BoundIds s.Retained |> Map.tryFind id)
