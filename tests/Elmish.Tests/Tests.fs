    ...
    testList
        ...
        [
            test "init maps viewer effects" {
                ...
                | other ->
                    failtestf
                        "Expected DispatchViewer OpenWindow, ApplyWindowOptions, and startup diagnostic effects, got %A"
                        other
            }
            ...
            test "update re-renders the scene from the user model on a viewer message" {
                ...
                let next, _ =
                    ElmishAdapter.update render (ViewerMsg(FramePresented { Width = 320; Height = 240 })) model
                ...
                Expect.isTrue
                    (effects
                     |> List.forall (function
                         | DispatchViewer _ -> true
                         | DispatchUser _ -> false))
