...
let tests =
    testList
        "Feature 113 memo metrics over Perf.runScript (US3, FR-009/FR-010, C7/C8)"
        [

            test "steady-state unchanged data accrues MemoHitCount > 0 with MemoMissCount = 0 (SC-004)" {
                let frames =
                    ControlsElmish.Perf.runScript (mkHost steadyView) size [ key (); key (); key () ]
            ...
            test "perturbed inputs accrue MemoMissCount (the dependency changed each frame)" {
                let frames =
                    ControlsElmish.Perf.runScript (mkHost perturbedView) size [ key (); key (); key () ]
            ...
            test "an idle frame reports both memo counts 0 (C8/FR-009)" {
                let frames =
                    ControlsElmish.Perf.runScript (mkHost steadyView) size [ key (); FrameInput.Idle ]
            ...
            test "a host with no memoizable control reports 0/0 on every frame" {
                let frames =
                    ControlsElmish.Perf.runScript (mkHost plainView) size [ key (); key () ]
