...
let tests =
    testList
        "Feature144 Controls.Elmish public FSI contracts"
        [
            test "ControlsElmish exposes overlay host interpretation" {
                let contract =
                    File.ReadAllText(Path.Combine(repositoryRoot, "src", "Controls.Elmish", "ControlsElmish.fsi"))

                [ "val interpretOverlayEffect"; "val interpretOverlayOutcome"; "OverlayEffect" ]
