...
let private editorView (name: string) : Control<Msg> =
    Stack.create
        [
            Stack.children
                [
                    TextBox.create [ TextBox.value name; TextBox.onChanged NameChanged ]
    ...
    testList
        ...
        [
            test "focus → type x → unrelated shift → type y ⇒ draft is 'hixy' (continued, not reset)" {
                ...
                Expect.equal
                    (Map.find editorId r1.StateByIdentity).Text.Value.DraftText
                    "hix"
                    "first keystroke appends to the pre-filled value (FR-005)"
                ...
                Expect.equal
                    (idOfKey "editor" s.Retained)
                    (Some editorId)
                    "the editor keeps its identity across the positional shift"
                ...
                Expect.equal
                    st.Text.Value.DraftText
                    "hixy"
                    "SC-001: in-progress text survived the shift (continued, not reset)"
            }
            ...
            test "baseline (rebuild every frame, no retained identity) FAILS the same proof" {
                ...
                Expect.notEqual
                    baseId1
                    baseId0
                    "baseline fails: rebuilding every frame mints a new id, losing the keyed state"
            }
        ]
    ...
    testList
        ...
        [
            test "a Replace (kind change at the same key) drops the prior identity's state — no false carry" {
                ...
                Expect.isFalse
                    (Map.containsKey oldId s.Retained.StateByIdentity)
                    "Replace drops the prior identity's text state (no false carry)"
            }
            ...
            test "a removed control's state is filtered out (focus would clear)" {
                ...
                Expect.isFalse
                    (Map.containsKey oldId s.Retained.StateByIdentity)
                    "a removed control's state leaves the live set (focus clears)"
            }
        ]
    ...
    testList
        ...
        [
            test "keyed, unkeyed, and keyed-container-wrapped fields each resolve to a distinct RetainedId" {
                let view: Control<Msg> =
                    Stack.create
                        [
                            Stack.children
                                [
                                    TextBox.create [ TextBox.value "k"; TextBox.onChanged NameChanged ]
                                    |> Control.withKey "keyed"
                        ...
                        ] // unkeyed field nested under a keyed container
                ...
                resolved
                |> List.iter (fun r -> Expect.isSome r "each field resolves to a RetainedId")
                ...
                Expect.equal
                    ...
                    3
                    "SC-002: all three fields resolve to DISTINCT ids (no unkeyed-sibling collapse)"
            ...
            test "a pre-filled multi-line field's first keystroke appends (zero characters lost) in MultiLine mode" {
                let view: Control<Msg> =
                    Stack.create
                        [
                            Stack.children
                                [
                                    TextArea.create [ TextArea.value "line1"; TextArea.onChanged NameChanged ]
                                    |> Control.withKey "area"
                                ]
                        ]
                ...
                Expect.equal
                    st.DraftText
                    "line1X"
                    "FR-005: the first keystroke appends to the pre-filled value (no truncation)"

                Expect.equal
                    st.Mode
                    MultiLine
                    "FR-005: a text-area seeds MultiLine mode (fixes the 090 hard-coded SingleLine)"
            }
            ...
            test "a control with more than one onChanged binding dispatches EVERY matched binding (FR-006)" {
                ...
                Expect.contains
                    msgs
                    (AlsoChanged "z")
                    "the second onChanged binding ALSO dispatched (FR-006, not just List.tryHead)"
            }
        ]
