    ...
    Stack.create
        [
            ...
            Stack.children
                [
                    for i in 1..30 ->
                        Button.create [ Button.text (sprintf "row %d" i); Attr.height 30.0 ]
    ...
    testList
        ...
        [
            test "T011 — a wheel over the scroll-viewer routes to (id, delta, contentHeight, viewportHeight)" {
                ...
                let svRect =
                    r.Render.Bounds |> List.find (fun (id, _) -> id = "content-scroll") |> snd
                ...
                | other ->
                    failtestf
                        "expected exactly one resolved scroll delta, got %A"
                        (other |> List.map (fun (i, d, _, _) -> i, d))
            }
            ...
            test "T011 — applying the resolved offset changes the frame, damage-locally (chrome sibling reused)" {
                ...
                let svRect =
                    r0.Render.Bounds |> List.find (fun (id, _) -> id = "content-scroll") |> snd
                ...
                let _, _, _, scrollDeltas =
                    ControlsElmish.routeRetainedPointer
                        ...
                        size
                        0
                        (wheel (svRect.X + 10.0) (svRect.Y + 10.0) -40.0)

                let (svId, deltaY, ch, vh) = List.exactlyOne scrollDeltas

                let scroll =
                    ScrollState.empty
                    |> ScrollState.withExtent ch vh
                    |> ScrollState.applyScrollDelta -deltaY
                ...
                Expect.notEqual
                    s.Render.Scene
                    r0.Render.Scene
                    "scrolling produced a visibly different frame (responds-proof)"

                Expect.isLessThan
                    s.WorkReduction.RepaintedNodeCount
                    r0.Render.NodeCount
