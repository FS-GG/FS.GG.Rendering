...
let private mkHost
    (view: int -> Control<Msg>)
    (mapKey: ViewerKey -> bool -> Msg option)
    : InteractiveAppHost<int, Msg> =
    {
        Init = fun () -> 0, []
        Update =
            fun msg model ->
...
let private textView (model: int) : Control<Msg> =
    Stack.create
        [
            Stack.children
                [
                    Button.create [ Button.text (string model); Button.onClick Inc ]
                    |> Control.withKey "btn"
                ]
        ]
...
let private constView (_: int) : Control<Msg> =
    Stack.create
        [
            Stack.children
                [
                    Button.create [ Button.text "fixed"; Button.onClick Inc ]
                    |> Control.withKey "btn"
                ]
        ]
...
let private hover id x y =
    FrameInput.Pointer(HoverEnter(id, x, y))
...
let tests =
    testList
        "Feature 109 honest FrameMetrics + coalescing fidelity (US1/US3, SC-001/002/003/004/011)"
        [
            ...
            test
                "a pure-hover frame routes from the retained frame: zero routing full renders (feature 110, SC-001/FR-004)" {
                ...
                Expect.equal
                    frames.[0].RemeasuredNodeCount
                    0
                    "no product message → renderStep did not run → no remeasure"
            }
            ...
            test "a model-changing message with no visual difference re-measures nothing (SC-001b / FR-003/004)" {
                ...
                let frames =
                    ControlsElmish.Perf.runScript
                        constKeyHost
                        size
                        [ FrameInput.Key(Enter, noMods); FrameInput.Key(Enter, noMods) ]

                Expect.isTrue frames.[1].ProductModelChanged "Inc changed the model (1 -> 2)"
                Expect.isTrue frames.[1].ViewCalled "the view ran"

                Expect.equal
                    frames.[1].FullRenderCount
                    1
                    "one full render (the key-frame renderStep; no routing render)"

                Expect.equal
                    frames.[1].RemeasuredNodeCount
                    0
                    "the view did not change, so nothing re-measured — no field over-reports work"
            }
            ...
            test "an animation-only tick is a PAINT-ONLY frame: no view, PaintRan true (feature 111, FR-004/SC-003)" {
                ...
                Expect.equal
                    tickFrame.FullRenderCount
                    0
                    "zero full renders — the overlay re-sample is not a host.View materialization"
            ...
            test "OnFrameMetrics yields exactly one record per produced frame; a burst is ONE record, not N (SC-010)" {
                ...
                Expect.equal
                    frames.Length
                    3
                    "exactly one FrameMetrics per produced frame (the 7-move burst is ONE frame)"

                Expect.equal
                    frames.[0].PointerSamplesReceived
                    7
                    "the burst is a single record carrying all 7 raw samples"
            ...
            test
                "a move burst interleaved with press/release/click/scroll drops NO discrete interaction (SC-003 / FR-010)" {
                let script =
                    [
                        hover "btn" 1.0 1.0
                        hover "btn" 2.0 2.0
                ...
                Expect.equal
                    frames.Length
                    5
                    "the moves coalesce; all four discrete interactions survive as their own frames"

                Expect.equal frames.[0].PointerMovesProcessed 1 "the leading move burst collapses to one processed move"

                for i in 1..4 do
                    Expect.equal
                        frames.[i].PointerSamplesReceived
                        1
                        "each discrete interaction is processed as a single un-coalesced sample"

                    Expect.equal
                        frames.[i].PointerMovesProcessed
                        0
                        "a discrete interaction is never counted as a processed move"
            }
            ...
            test "a continuous drag of hundreds of samples keeps its full raw path available to consumers (FR-011)" {
                ...
                Expect.equal
                    frames.[0].PointerSamplesReceived
                    pathLen
                    "every raw drag sample is counted (the path's length)"
                ...
                Expect.equal
                    (List.last rawPath)
                    (float (pathLen - 1), float ((pathLen - 1) * 2))
                    "the path's final sample is intact"
            }

            // ---- US4 boundary: FrameDuration is excluded from the deterministic count/bool surface ----

            test
                "FrameDuration is excluded from the deterministic surface: identical scripts give byte-identical counts (FR-012 / SC-009)" {
                let script =
                    [
                        FrameInput.Key(Enter, noMods)
                        hover "btn" 3.0 3.0
                ...
                let r1 =
                    ControlsElmish.Perf.runScript textKeyHost size script |> List.map countTuple
                ...
                let durations =
                    ControlsElmish.Perf.runScript textKeyHost size script
                    |> List.map (fun f -> f.FrameDuration)
            ...
            test
                "observation-only: the production render path is byte-identical regardless of metric observation (FR-020 / SC-008)" {
                let model = 2
                ...
                Expect.equal
                    (sceneOf observingHost)
                    (sceneOf textKeyHost)
                    "the rendered scene does not depend on the OnFrameMetrics sink"

                Expect.equal
                    (sceneOf textKeyHost)
                    (sceneOf textKeyHost)
                    "the at-rest production render is byte-stable across renders"
                ...
                let a =
                    ControlsElmish.Perf.runScript textKeyHost size script |> List.map metricTuple

                let b =
                    ControlsElmish.Perf.runScript observingHost size script |> List.map metricTuple
