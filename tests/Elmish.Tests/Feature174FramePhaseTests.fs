...
let tests =
    testList
        "Feature174 frame phase attribution"
        [
            test "Synthetic timing contribution separates frame preparation from paint and presentation" {
                ...
                let contribution =
                    ControlsElmish.responsivenessTimingContribution (metric 100.0 15.0 5.0 true true)

                Expect.equal
                    contribution.RetainedStepDuration
                    (TimeSpan.FromMilliseconds 80.0)
                    "retained step is pre-paint preparation"

                Expect.equal
                    contribution.UpdateDuration
                    (TimeSpan.FromMilliseconds 80.0)
                    "model update contribution uses the same pre-paint budget"

                Expect.equal
                    contribution.LayoutDuration
                    (TimeSpan.FromMilliseconds 80.0)
                    "layout contribution is phase-attributed when layout ran"
            ...
            test "Synthetic timing contribution clamps impossible paint/presentation overrun to zero" {
                ...
                let contribution =
                    ControlsElmish.responsivenessTimingContribution (metric 10.0 8.0 8.0 true true)
                ...
                Expect.equal
                    contribution.LayoutDuration
                    TimeSpan.Zero
                    "layout attribution follows the clamped preparation value"
            }
        ]
