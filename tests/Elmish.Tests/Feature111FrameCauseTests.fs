...
let private view (model: int) : Control<Msg> =
    Stack.create
        [
            Stack.children
                [
                    Button.create [ Button.text (string model); Button.onClick Bump ]
                    |> Control.withKey "btn"
...
let private causesOf (script: FrameInput<Msg> list) =
    ...
    |> List.map (fun f -> f.FrameCause)

[<Tests>]
let tests =
    testList
        "Feature 111 FrameCause classification (US1, FR-001/SC-001/SC-005)"
        [
            ...
            test "a coalesced pointer-move burst's cause is PointerMove" {
                ...
                Expect.equal
                    ...
                    "a move burst is one frame caused by PointerMove"
            }

            test "a discrete pointer interaction's cause is PointerDiscrete" {
                let causes =
                    causesOf [ FrameInput.Pointer(PressedDown("btn", PointerButton.Primary, 5.0, 5.0)) ]
            ...
            test "an animation-only tick's cause is Tick" {
                ...
                let causes =
                    causesOf [ FrameInput.Key(Enter, noMods); FrameInput.Key(Enter, noMods); tick 16.0 ]

                Expect.equal
                    causes
                    [ FrameCause.Key; FrameCause.Key; FrameCause.Tick ]
                    "the tick frame is caused by Tick"
            }
