        ...
        [
            test "tall radio-group click reports the option in the PAINTED band, capped on theme.ControlHeight" {
                ...
                Expect.isGreaterThan
                    (rad.Height / float n)
                    theme.ControlHeight
                    "rows must exceed theme.ControlHeight for the cap to bind"
                ...
                Expect.isLessThan
                    yProbe
                    (rad.Y + paintRowH * 2.0)
                    "probe must stay inside the green band under the correct cap"

                match clickAt r.Retained r.Render cx yProbe with
                | [ RadioChanged "green" ] -> ()
                | [ RadioChanged "blue" ] ->
                    failtest
                        "F-CTL-1 regression: lower green band dispatched 'blue' — click cap is not theme.ControlHeight"
