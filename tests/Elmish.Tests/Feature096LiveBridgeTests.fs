...
let private stateOfNode (node: RetainedNode<'msg>) =
    ControlInternals.visualStateOf node.Control.Attributes
...
let private readinessRoot =
    Path.GetFullPath(
        Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "specs", "096-runtime-visual-state-bridge", "readiness")
    )
...
let feature096LiveBridge =
    testList
        "Feature 096 runtime bridge — live retained path"
        [
            ...
            test "T018 — the Focused indicator stays on the same retained identity across a sibling shift" {
                ...
                Expect.equal
                    (stateOfNode before)
                    Focused
                    "the bridge stamped Focused onto the focused editor (no consumer attribute)"
                ...
                Expect.equal
                    after.Identity
                    before.Identity
                    "FR-007: the editor keeps its stable RetainedId across the shift (E2 identity)"

                Expect.equal
                    (stateOfNode after)
                    Focused
                    "SC-002: the Focused indicator stays on the SAME control after the shift"
                ...
                let baselineId0 =
                    (nodeOf "editor" ((RetainedRender.init theme size (bridged m [ editor "hi" ])).Retained)).Identity
                ...
                Expect.notEqual
                    baselineId1
                    baselineId0
                    "baseline: rebuilding every frame mints a new id (the wired path is what carries identity)"

                writeEvidence
                    "focus-survives-reshuffle.md"
                    [
            ...
            test "T020 — a focus/hover input restyles on the live path; an inert (un-bridged) build does not" {
                ...
                let focusBridged =
                    (Control.renderTree theme size (bridged (focusModel "editor") [ editor "x" ])).Scene
                ...
                let pressModel =
                    { emptyModel with
                        PressedControls = Set.ofList [ "go" ]
                    }

                let btn: Control<Msg> = Button.create [ Button.text "Go" ] |> Control.withKey "go"

                let pressBridged =
                    (Control.renderTree theme size (bridged pressModel [ btn ])).Scene
                ...
                Expect.equal
                    focusInert
                    restInert
                    "the un-bridged build does not respond to interaction state (identical frames)"
