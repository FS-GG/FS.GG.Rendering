...
let tests =
    testList
        "Feature 110 counted full-render fallback (US3, FR-007/FR-009, SC-005/SC-006)"
        [
                ...
                for script in scripts do
                    ...
                    for f in frames do
                        Expect.equal
                            ...
                            0
                            "a normal scripted pointer scenario never falls back (SC-005)"
            }

            test
                "a deliberately unroutable case increments FullRenderFallbackCount by one and the fallback matches the oracle (SC-006)" {
                let model = 0
                ...
                let stale =
                    RetainedRender.init host.Theme { Width = 1; Height = 1 } (host.View { Width = 1; Height = 1 } model)
                ...
                let msgs, fallbacks =
                    ControlsElmish.routeRetainedInteraction
                        ...
                        stale.Retained
                        stale.Render
                        (Click("btn", PointerButton.Primary, cx, cy))

                Expect.equal
                    fallbacks
                    1
                    "the unresolvable bindable hit incremented FullRenderFallbackCount by exactly one (FR-009/SC-006)"
            ...
            test
                "a retained frame that DOES contain the hit resolves with no fallback (the fallback is an escape hatch only)" {
                let model = 0
                let cx, cy = centreOf model "btn"
