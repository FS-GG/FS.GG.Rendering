...
let tests =
    testList
        "Feature 116 paint-cache metrics over Perf.runScript (US5, FR-012/013/015)"
        [
            ...
            test "the cache entry count stays bounded by the cap under pressure (FR-009, SC-007)" {
                ...
                Expect.isTrue
                    (f2.PictureCacheEntryCount <= RetainedRender.PictureCacheCap)
                    "entry count bounded by the cap"

                Expect.equal
                    f2.PictureCacheEntryCount
                    RetainedRender.PictureCacheCap
                    "the cache is full at the cap under pressure"
            }
