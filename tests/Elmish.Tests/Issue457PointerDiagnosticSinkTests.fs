            ...
            fun _ _ ->
                Stack.create
                    [
                        Stack.children
                            [
                                Button.create [ Button.text "go"; Button.onClick Bump ] |> Control.withKey "btn"
                            ]
                    ]
        Theme = Theme.light
        MapKey = fun _ _ -> None
    ...
    { hostWith options with
        View =
            fun _ _ ->
                Stack.create
                    [
                        Stack.children [ Button.create [ Button.text "go" ] |> Control.withKey "btn" ]
                    ]
    }
...
let private unresolved (captured: ResizeArray<ViewerDiagnosticEvent>) =
    captured
    |> Seq.filter (fun e -> e.Message.Contains "UnresolvedControlId")
    |> List.ofSeq

[<Tests>]
let tests =
    testList
        "Issue #457 — a pointer diagnostic reaches an observer instead of being filtered out"
        [
            ...
            test "a scripted click at an id NO control carries is reported, and names the id" {
                ...
                let frames =
                    ControlsElmish.Perf.runScript
                        ...
                        size
                        [
                            FrameInput.Pointer(Click("this-button-does-not-exist", PointerButton.Primary, cx, cy))
                        ]
                ...
                Expect.equal
                    ...
                    "a named id that resolves to nothing is a caller defect, not routine"

                Expect.equal
                    reported.[0].Category
                    ViewerDiagnosticCategory.Input
                    "pointer routing is an Input-category diagnostic"

                Expect.equal
                    frames.[0].ProductModelChanged
                    false
                    "the click still dispatched nothing — the SILENCE is what is fixed, not the routing"
            }
            ...
            test "a case-typo of a real id is reported, not silently ignored" {
                ...
                Expect.stringContains
                    reported.[0].Message
                    "BTN"
                    "the diagnostic names the id as written, so the typo is visible"
            ...
            test "a click on a REAL control with no binding is NOT reported as unresolved" {
                ...
                Expect.isEmpty
                    (unresolved captured)
                    "'btn' EXISTS — having no binding is not a defect, and must not be reported as one"

                Expect.equal
                    frames.[0].ProductModelChanged
                    false
                    "an unbound click still dispatches nothing (unchanged)"
            }
            ...
            test "a hover at an absent id is NOT reported — only activating interactions are checked" {
                ...
                Expect.isEmpty
                    (unresolved captured)
                    "a hover is not an activation — naming an absent id there is not a defect"
            }
            ...
            test "a genuine geometric hit-test miss reaches the sink instead of being filtered out" {
                ...
                ControlsElmish.routeInteractivePointer
                    ...
                    size
                    0
                    (pointer ViewerPointerPhaseKind.Pressed -50.0 -50.0)
                |> ignore

                let misses =
                    captured |> Seq.filter (fun e -> e.Message.Contains "HitTestMiss") |> List.ofSeq

                Expect.isNonEmpty
                    misses
                    "the press resolved to no control — `Pointer.update` raised HitTestMiss and the host must not swallow it"

                Expect.equal
                    misses.[0].Level
                    ViewerDiagnosticLevel.Info
                    "a press on empty space is routine, not a defect"
            ...
            test "a host with no diagnostics sink is inert" {
                ...
                let bogus =
                    ControlsElmish.Perf.runScript
                        host
                        size
                        [ FrameInput.Pointer(Click("nope", PointerButton.Primary, cx, cy)) ]
            ...
            test "AdapterCmd.diagnostics extracts exactly what productMessages discards" {
                ...
                let command: AdapterCommand<Msg> =
                    [ DispatchProductMessage Bump; ReportAdapterDiagnostic d ]

                Expect.equal
                    (AdapterCmd.productMessages command)
                    [ Bump ]
                    "productMessages keeps only the product messages"

                Expect.equal
                    (AdapterCmd.diagnostics command)
                    [ d ]
                    "diagnostics keeps the reports productMessages drops"

                Expect.isEmpty
                    (AdapterCmd.diagnostics (AdapterCmd.ofMessage Bump))
                    "law: diagnostics (ofMessage m) = []"
            }
        ]
