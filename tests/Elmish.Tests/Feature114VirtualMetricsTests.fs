...
let private twoGridView (a: int) (b: int) (model: int) : Control<Msg> =
    Stack.create
        [
            ...
            Stack.children
                [
                    DataGrid.create columns [ DataGrid.rows (gridRows a) ]
                    |> Control.withKey "gridA"
                    DataGrid.create columns [ DataGrid.rows (gridRows b) ]
                    |> Control.withKey "gridB"
                ]
        ]
...
let private buttonView (model: int) : Control<Msg> =
    Stack.create
        [
            Stack.children
                [
                    Button.create [ Button.text (sprintf "n%d" model); Button.onClick Bump ]
                    |> Control.withKey "b"
                ]
        ]
...
let tests =
    testList
        "Feature 114 virtual metrics (US4, FR-013/FR-014, SC-006)"
        [
            ...
            test "a frame building a virtualized grid reports materialized <= visible and total = RowCount (FR-013)" {
                ...
                Expect.isTrue
                    ...
                    "materialized count is bounded by the realized window"
                ...
                Expect.isTrue
                    ...
                    "the window is far smaller than the logical total"
            }

            test "materialized count does NOT scale with total across 100/1000/10000 (FR-014, SC-006)" {
                let materialized total =
                    (runWith (gridView total) [ key (); key () ]).[1].VirtualItemsMaterialized

                let totals =
                    (runWith (gridView 100) [ key (); key () ]).[1].VirtualItemsTotal,
                    (runWith (gridView 1000) [ key (); key () ]).[1].VirtualItemsTotal,
                    (runWith (gridView 10000) [ key (); key () ]).[1].VirtualItemsTotal

                Expect.allEqual
                    [ materialized 100; materialized 1000; materialized 10000 ]
                    (materialized 10000)
                    "materialized count identical across totals — does not scale"
            ...
            test "multiple virtualized controls in one frame AGGREGATE the counts (FR-013)" {
                ...
                Expect.isTrue
                    (f.VirtualItemsMaterialized <= 2 * fallbackVisible)
                    "materialized aggregates across both grids, still bounded"

                Expect.isTrue
                    (f.VirtualItemsMaterialized > fallbackVisible)
                    "materialized reflects BOTH grids' windows, not one"
            }
        ]
