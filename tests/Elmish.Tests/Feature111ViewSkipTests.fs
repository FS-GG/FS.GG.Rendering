...
let tests =
    testList
        "Feature 111 view-skip + byte-identity (US3, FR-003/FR-004/FR-006/SC-003/SC-007/SC-008)"
        [

            test "an animation-only tick performs no host.View (ViewCalled false, FullRenderCount 0) (SC-003)" {
                let frames =
                    ControlsElmish.Perf.runScript (mkHost animView) size [ key (); key (); tick 16.0 ]
            ...
            test "byte-identity: the retained tree equals a fresh host.View of the unchanged model (FR-003)" {
                        ...
                        StateByIdentity =
                            ...
                            |> Map.map (fun _ st ->
                                { st with
                                    Animation =
                                        st.Animation
                                        |> Option.map (RetainedRender.advance (TimeSpan.FromMilliseconds 16.0))
                                })
                    }
                ...
                let sceneViewSkipped =
                    (RetainedRender.step theme size advanced advanced.Root.Control).Render.Scene

                let sceneViewReRun =
                    (RetainedRender.step theme size advanced (animView 2)).Render.Scene

                Expect.equal
                    sceneViewSkipped
                    sceneViewReRun
                    "the view-skipped overlay is byte-identical to the view-re-run overlay (SC-007)"
            }
            ...
            test "continuous drag is frame-rate work: <= 1 processed move, zero host.View, none dropped (FR-006/SC-008)" {
                ...
                Expect.isTrue
                    ...
                    "at most one processed move (coalescing preserved)"
            ...
            test "continuous animation is frame-rate work: every tick is view-free (FR-006/SC-008)" {
                ...
                let frames =
                    ControlsElmish.Perf.runScript (mkHost animView) size (key () :: key () :: ticks)
                // The 8 tick frames are frames.[2..9]; while the clock is live each is view-free.
                let tickFrames =
                    frames |> List.skip 2 |> List.filter (fun f -> f.FrameCause = FrameCause.Tick)

                Expect.isTrue
                    (tickFrames |> List.forall (fun f -> not f.ViewCalled))
                    "every animation tick skips the view phase"

                Expect.isTrue
                    (tickFrames |> List.forall (fun f -> f.FullRenderCount = 0))
                    "every animation tick performs zero full renders"
            }
            ...
            test "a move burst interleaved with discrete interactions drops none (FR-006)" {
                ...
                Expect.equal
                    frames.Length
                    4
                    "the moves coalesce; all three discrete interactions survive as their own frames"
                ...
                for i in 1..3 do
                    Expect.equal
                        ...
                        "each discrete interaction is its own PointerDiscrete frame"
            }
        ]
