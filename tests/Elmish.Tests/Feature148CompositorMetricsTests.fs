module Feature148CompositorMetricsTests
...
[<Tests>]
let tests =
    testList "Feature148 compositor metrics" [
        test "diagnostics expose damage, reuse hits, misses, repeated-work, and snapshot bytes" {
            ...
            Expect.equal diagnostics.ScissorCandidateArea 1600 "scissor candidate"
            ...
            Expect.equal diagnostics.DemotionCount 0 "replay skipped work avoids demotion"
        }

        test "fallback and no-benefit demotion are visible when proof or replay evidence is weak" {
            let diagnostics = ControlsElmish.compositorDiagnostics false (Some "stale proof") (metrics 0 2)

            Expect.equal diagnostics.ProofStatus "not-ready" "proof"
            Expect.equal diagnostics.ScissorCandidateArea 0 "no scissor"
            Expect.equal diagnostics.FallbackReason (Some "stale proof") "fallback"
