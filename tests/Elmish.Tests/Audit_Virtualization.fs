...
let private gridView (rowCount: int) (model: int) : Control<Msg> =
    Stack.create
        [
            ...
            Stack.children
                [
                    DataGrid.create columns [ DataGrid.rows (gridRows rowCount) ]
...
let private virtualOf (rowCount: int) : FrameMetrics =
    (runWith (gridView rowCount) [ key (); key () ]).[1]

[<Tests>]
let tests =
    testList
        "Audit virtualization mechanism (T007 / T034 US3)"
        [
            ...
            test "Audit: virtualization counters reachable — VirtualItemsMaterialized/Total touchable (T007)" {
                ...
                Expect.isTrue
                    (f.VirtualItemsTotal > 0)
                    "a virtualized grid surfaces a positive logical total (counters reachable)"
            ...
            test "Audit: a 10000-row grid materializes a viewport-bounded window, NOT the total (T034, effectiveness)" {
                ...
                Expect.isTrue
                    (f.VirtualItemsMaterialized <= fallbackVisible)
                    (sprintf
                        "materialized (%d) is bounded by the realized window (<= %d), not the total"
                ...
                Expect.isLessThan
                    ...
                    0.05
                    (sprintf
                        "materialized %d / total %d = %.4f — a small fraction (no-op would be ~1.0, a FINDING)"
                        f.VirtualItemsMaterialized
                        f.VirtualItemsTotal
                        ratio)

                Expect.isTrue
                    (f.VirtualItemsMaterialized < f.VirtualItemsTotal)
                    "materialized is strictly below the total"
            }

            test
                "Audit: the materialized window does NOT scale with the total across 100/1000/10000 (T034, discriminating)" {
                ...
                Expect.equal
                    m100
                    m10000
                    "materialized count is identical at 100 and 10000 rows — the window does not scale"

                Expect.equal
                    m1000
                    m10000
                    "materialized count is identical at 1000 and 10000 rows — the window does not scale"

                Expect.notEqual
                    m10000
                    t10000
                    "DISCRIMINATING: materialized != total (an all-realized impl would equal it)"
            }
        ]
