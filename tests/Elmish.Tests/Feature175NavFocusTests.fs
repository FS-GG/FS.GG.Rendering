...
let private view (_: Size) (page: string) : Control<Msg> =
    let nav id =
        Button.create [ Button.text id; Button.onClick (Navigate id) ]
        |> Control.withKey ("nav-" + id)
            ...
            Stack.create
                [
                    Stack.children
                        [
                            for i in 1..4 ->
                                TextBlock.create [ TextBlock.text (sprintf "A-row-%d" i) ]
                                |> Control.withKey (sprintf "a-%d" i)
                        ]
                ]
        | _ ->
            Stack.create
                [
                    Stack.children
                        [
                            for i in 1..9 ->
                                Button.create [ Button.text (sprintf "B-btn-%d" i) ]
                                |> Control.withKey (sprintf "b-%d" i)
                        ]
                ]

    Stack.create [ Stack.orientation "vertical"; Stack.children [ nav "A"; nav "B"; content ] ]
...
let private press x y : ViewerPointerInput =
    {
...
let private release x y : ViewerPointerInput =
    {
...
let rec private findByIdentity (id: RetainedId) (n: RetainedNode<'msg>) : RetainedNode<'msg> option =
    if n.Identity = id then
        ...
        n.Children |> List.tryPick (findByIdentity id)

let private stateOf (key: ControlId) (r: RetainedRender<Msg>) =
    findByKey key r.Root
    |> Option.map (fun n -> ControlInternals.visualStateOf n.Control.Attributes)
            ...
            test "clicking a nav button focuses it on the SAME click as the navigation" {
                ...
                let r0 =
                    RetainedRender.init
                        theme
                        size
                        (ControlRuntime.applyRuntimeVisualState (fst (ControlRuntime.init ())) (view size model0))
                ...
                let focusedNode =
                    focusedRid |> Option.bind (fun rid -> findByIdentity rid r0.Retained.Root)
                ...
                let focusedControlId =
                    focusedNode
                    |> Option.map (fun n -> n.Control.Key |> Option.defaultValue n.Control.Kind)

                Expect.equal
                    focusedControlId
                    (Some "nav-B")
                    "focus resolves to the nav-B control id (not the bare kind)"
                ...
                let renderFrame (prev: RetainedRender<Msg>) (page: string) =
                    ...
                    let fcid =
                        focusedRid
                        |> Option.bind (fun rid -> findByIdentity rid prev.Root)
                        |> Option.map (fun n -> n.Control.Key |> Option.defaultValue n.Control.Kind)
                ...
                let _, msgs, _, _ =
                    ControlsElmish.routeRetainedPointer host r0.Retained r0.Render s1 size model0 (release x y)
                ...
                Expect.equal
                    (stateOf "nav-B" rB)
                    (Some Focused)
                    "nav-B is focused on the navigation frame (one click, not two)"

                Expect.equal
                    (stateOf "nav-A" rB)
                    (Some Normal)
                    "the sibling nav-A is NOT focused (keying isolates focus)"
            }
        ]
