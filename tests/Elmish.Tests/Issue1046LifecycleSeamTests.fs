...
let tests =
    testList
        "Issue 1046 frame-loop lifecycle seam"
        [
            ...
            test "move and discrete boundaries preserve the existing latest-pending accounting" {
                ...
                Expect.equal
                    (FrameLoopLifecycle.completeMoveBoundary state)
                    0
                    "the first move carries into the next boundary"
                ...
                Expect.equal
                    (FrameLoopLifecycle.takePendingMove state)
                    (Some second)
                    "a discrete boundary flushes the pending move first"

                Expect.equal
                    (FrameLoopLifecycle.completeDiscreteBoundary state)
                    2
                    "the carried move and discrete sample share one batch"
            ...
            test "diagnostic disclosure is a one-time lifecycle transition" {
                ...
                Expect.isTrue
                    (FrameLoopLifecycle.surfaceDiagnosticOnce diagnostic state)
                    "the first observation is surfaced"

                Expect.isFalse
                    (FrameLoopLifecycle.surfaceDiagnosticOnce diagnostic state)
                    "the same observation is suppressed"
