...
let tests =
    testList
        "Feature 117 text-cache + layout-invalidated metrics over Perf.runScript (FR-005/006/010)"
        [
            ...
            test
                "a geometry frame reports LayoutInvalidatedNodeCount <= RemeasuredNodeCount, both bounded (FR-006, SC-006)" {
                ...
                Expect.isTrue
                    ...
                    (sprintf
                        ...
                        geom.LayoutInvalidatedNodeCount
                        geom.RemeasuredNodeCount)
            }
            ...
            test "the three new metrics re-run byte-identically (FR-005/FR-006, regression sentinel)" {
                let run () =
                    ...
                    |> List.map (fun f ->
                        f.TextMeasureCacheHitCount, f.TextMeasureCacheMissCount, f.LayoutInvalidatedNodeCount)
