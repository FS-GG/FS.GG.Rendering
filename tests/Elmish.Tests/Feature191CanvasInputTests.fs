...
let private demoScene =
    {
        Nodes = [ Rectangle((0.0, 0.0, 20.0, 20.0), Colors.black) ]
    }
...
let private pointedSamples (msgs: Msg list) =
    msgs
...
let tests =
    testList
        "Feature 191 canvas raw input forwarding (US2)"
        [
            ...
            test "an in-box pointer forwards the raw sample (canvas-local) to onPointer; out-of-box does not" {
                ...
                let _, moveMsgs =
                    ControlsElmish.routeInteractivePointer
                        ...
                        size
                        model
                        (pointer ViewerPointerPhaseKind.Moved cx cy)
                ...
                let _, outMsgs =
                    ControlsElmish.routeInteractivePointer
                        ...
                        size
                        model
                        (pointer ViewerPointerPhaseKind.Pressed ox cy)
            ...
            test "a focused canvas forwards ViewerKey + KeyModifiers to onKey; an unfocused canvas does not" {
                ...
                let _, _, focusedMsgs =
                    ControlsElmish.routeFocusedKey r cv (order r) (ViewerKey.Letter 'a') true
                ...
                let _, _, otherMsgs =
                    ControlsElmish.routeFocusedKey r btn (order r) (ViewerKey.Letter 'a') false
                ...
                Expect.isEmpty
                    (otherMsgs
                     |> List.choose (function
                         | Keyed _ as m -> Some m
                         | _ -> None))
                    "an unfocused canvas receives no key"
