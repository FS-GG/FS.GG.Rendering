...
let typedAdapterTests =
    testList
        "Typed controls Elmish boundary and dependency guard"
        [
            ...
            test
                "programOfWidget runs a Widget-returning view with no Widget.toControl in product code (US1, SC-001/SC-002/FR-004)" {
                let init () = { Saved = false }, []
                ...
                let program =
                    ControlsElmish.programOfWidget init update typedWidgetView (fun _ -> [])
                ...
                Expect.equal
                    renderedWidget.NodeCount
                    renderedLegacy.NodeCount
                    "lowering parity: same node count as view >> Widget.toControl"
                ...
                Expect.equal
                    (Control.dispatch click viaWidgetPath)
                    [ Save ]
                    "typed event dispatches through the Widget-view path"
            }

            test
                "Widget.ofControl lowers identically to the wrapped control; Widget and Control programs coexist (US4, FR-010)" {
                ...
                Expect.equal
                    (Control.dispatch click bridged)
                    (Control.dispatch click legacy)
                    "ofControl >> toControl preserves dispatch"
                ...
                Expect.isGreaterThan
                    (Control.render Theme.light (widgetProgram.View widgetModel)).NodeCount
                    0
                    "widget-path program renders"

                Expect.isGreaterThan
                    (Control.render Theme.light (controlProgram.View controlModel)).NodeCount
                    0
                    "control-path program renders"
            }
            ...
            test "base Controls package gains no Fable.Elmish dependency (FR-011, SC-004)" {
                ...
                [
                    @"..\Scene\Scene.fsproj"
                    @"..\Layout\Layout.fsproj"
                    @"..\KeyboardInput\KeyboardInput.fsproj"
                ]
                |> List.iter (fun reference ->
                    Expect.stringContains text reference $"existing reference {reference} retained")
            }
        ]
