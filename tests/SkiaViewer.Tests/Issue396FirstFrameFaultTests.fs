            ...
            testList
                ...
                [
                    test "a product that throws on its first View fails as an App-stage ProductDefect, not a crash" {
                        ...
                        match
                            Viewer.runAppEvidence
                                (firstFrameRequest (Some evidencePath))
                                evidenceOptions
                                firstFrameFaultHost
                        with
                        | Result.Ok _ ->
                            failtest "a first-frame product throw must not read as a successful evidence run"
                        | Result.Error failure ->
                            Expect.equal
                                failure.Classification
                                ProductDefect
                                "a first-frame product fault is a product defect, not an environment failure"

                            Expect.equal
                                failure.BlockedStage
                                ViewerRunBlockedStage.App
                                "the fault is staged at App — application code, not a render/environment stage"

                            Expect.stringContains failure.Message "View" "the failure names which product step raised"

                            Expect.stringContains
                                failure.Message
                                "product blew up on first View"
                                "the underlying exception message survives into the failure"
                        ...
                        if IO.File.Exists evidencePath then
                            ...
                            Expect.stringContains
                                evidenceText
                                "status=failed"
                                "the serialized evidence records the failed run"

                            Expect.stringContains
                                evidenceText
                                "classification=ProductDefect"
                                "the serialized evidence records the product-defect classification"

                            Expect.stringContains
                                evidenceText
                                "blocked-stage=App"
                                "the serialized evidence records the App stage"
                    }

                    test
                        "a healthy first View still runs the bounded evidence path to success (the guard is transparent)" {
                        match Viewer.runAppEvidence (firstFrameRequest None) evidenceOptions healthyHost with
                        | Result.Ok outcome ->
                            Expect.equal
                                outcome.Command
                                (Some "runAppEvidence")
                                "a well-behaved product is unaffected by the first-frame guard"
                        | Result.Error failure ->
                            failtestf "a healthy product must not be blocked by the first-frame guard: %A" failure
                    }
                ]
                    ...
                    test "a product that throws on its first View fails as an App-stage ProductDefect, not a crash" {
                        ...
                        | Result.Error failure ->
                            ...
                            Expect.stringContains
                                failure.Message
                                "first frame"
                                "the startup failure message is honest about the first-frame origin, not the runtime drop-and-continue wording"
                    }
