...
let tests =
    testList
        "Feature147 snapshot resource host policy"
        [
            ...
            test "environment-limited proof token remains distinct from failed proof" {
                Expect.equal
                    (CompositorProof.readinessToken (CompositorProof.ProofReadiness.EnvironmentLimited "no display"))
                    "environment-limited"
                    "environment token"

                Expect.equal
                    (CompositorProof.readinessToken (CompositorProof.ProofReadiness.Failed "cleared"))
                    "failed"
                    "failed token"
            }
        ]
