...
let tests =
    testList
        "Feature152 Synthetic live proof rejection simulations"
        [
            ...
            test "unsupported, blank, stale, host-mismatched, and proof-method-mismatched attempts fail closed" {
                ...
                let baseAttempt =
                    attempt "run" CompositorProof.PresentProofPassed (quality false true)
                ...
                |> List.iter (fun bad ->
                    let readiness =
                        CompositorProof.evaluateProofSet
                            ...
                            [ bad; baseAttempt; baseAttempt ]

                    Expect.notEqual
                        (CompositorProof.proofSetReadinessToken readiness)
                        "accepted"
                        $"bad attempt {bad.AttemptId}")
            }
        ]
