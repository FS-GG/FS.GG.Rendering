    ...
    testList
        ...
        [
            test "runKeyScriptToModel folds a key script through the host's own MapKey -> Update to the final model" {
                ...
                let model, _ =
                    GeneratedAppHost.runKeyScriptToModel
                        playableHost
                        [ down "Enter"; down "ArrowRight"; down "d"; down "ArrowRight" ]
            ...
            test "runKeyScriptToModel returns effects in order, Init's batch first" {
                let _, effects = GeneratedAppHost.runKeyScriptToModel playableHost [ down "Enter" ]

                let texts =
                    effects
                    |> List.choose (function
                        | RenderScene(Text(_, t, _)) -> Some t
                        | _ -> None)
            ...
            test "auditKeyWiring partitions a declared surface into wired and dead bindings" {
                ...
                Expect.equal
                    (wiring.Wired |> List.map snd)
                    [ StartRun; TogglePause; DescendFloor ]
                    "each live key names its message"

                Expect.equal
                    wiring.Dead
                    [ down "x" ]
                    "the declared-but-unbound key is dead — a key that is bound is a key that dispatches"
            }

            test "auditKeyWiring reports key-UP as dead when the host only routes key-down" {
                let wiring =
                    GeneratedAppHost.auditKeyWiring playableHost [ down "Enter"; up "Enter" ]
            ...
            test "reachableMessages over the full input surface covers the handled universe for a playable host" {
                ...
                let reachable =
                    GeneratedAppHost.reachableMessages playableHost surface (Some(TimeSpan.FromMilliseconds 16.0))

                let unwired =
                    Set.difference (Set.ofList handledUniverse) (reachable |> List.map caseName |> Set.ofList)
            ...
            test "reachableMessages exposes handled-but-unwired Msg cases on the unplayable host" {
                ...
                let reachable =
                    GeneratedAppHost.reachableMessages unplayableHost surface (Some(TimeSpan.FromMilliseconds 16.0))
