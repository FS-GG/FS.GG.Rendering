...
let private controlCentre logical =
    ...
    let bounds =
        layout.Bounds |> List.find (fun item -> item.NodeId = "semantic-control")
...
let private activateAtCorrespondingPhysicalPoint logical surface =
    ...
    let route phase =
        Viewer.pointerInProductSpace (Some logical) surface surface (pointer phase physicalX physicalY)
...
let logicalCanvasOwnerTests =
    testList
        "Issue 1014 interactive logical-canvas owner"
        [
            test "ApplyLogicalCanvas reaches the runtime owner in effect order" {
                let selected = ResizeArray<Size>()

                let effects =
                    [ ApplyLogicalCanvas(size 1280 720); ApplyLogicalCanvas(size 1920 1080) ]
