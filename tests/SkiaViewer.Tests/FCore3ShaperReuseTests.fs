...
let private mono: FontSpec =
    {
        Family = Some "Noto Sans Mono"
        Size = 18.0
        Weight = None
    }
...
let tests =
    testSequenced
    <| testList
        "F-CORE-3 shaper reuse and O(1) glyph assembly"
        [
            test "shaping reuses one cached shaper per typeface, not one per call" {
                Fonts.withInstalledShapingProvider (fun () ->
                    ...
                    Expect.equal
                        (Fonts.shaperCacheCount ())
                        2
                        "a distinct typeface adds exactly one shaper (still bounded by typefaces)"
            ...
            test "array-indexed glyph assembly stays boundary-consistent on a long string" {
                Fonts.withInstalledShapingProvider (fun () ->
                    ...
                    Expect.isTrue
                        (shaped.Glyphs
                         |> List.forall (fun g -> g.SourceCluster >= 0 && g.SourceCluster < longText.Length))
