...
let private boundary (cacheId: uint64) (fp: uint64) : CacheBoundary =
    {
        ...
        Scene =
            {
                Nodes = [ Rectangle((0.0, 0.0, 8.0, 8.0), blue) ]
            }
    }
...
let tests =
    testSequenced
    <| testList
        "Feature 120 backend replay cache (US3, FR-007/011/013)"
        [
            ...
            test
                "cache-on ≡ cache-off pixel readback parity: a replayed boundary is byte-identical to the direct walk (SC-003/FR-009/FR-011)" {
                ...
                let scene: Scene =
                    {
                        Nodes = [ boundaryScene 0UL 2.0; boundaryScene 1UL 20.0; boundaryScene 2UL 38.0 ]
                    }
                ...
                Expect.equal
                    replayWarm
                    direct
                    "a warmed replayed boundary is byte-identical to the direct walk (SC-003/FR-009)"
            }
            ...
            test "idle-skip decision: present iff first frame, scene changed, or size changed (US2, FR-004/005/006)" {
                ...
                let sceneB: Scene =
                    {
                        Nodes = [ Rectangle((0.0, 0.0, 9.0, 8.0), blue) ]
                    }

                Expect.isTrue (GlHost.shouldPresent None sceneA false) "first frame always presents"

                Expect.isFalse
                    (GlHost.shouldPresent (Some sceneA) sceneA false)
                    "unchanged scene + unchanged size ⇒ idle skip"

                Expect.isTrue (GlHost.shouldPresent (Some sceneA) sceneB false) "a changed scene presents"

                Expect.isTrue
                    (GlHost.shouldPresent (Some sceneA) sceneA true)
                    "a size change forces a present even if the scene is unchanged"
            }
        ]
