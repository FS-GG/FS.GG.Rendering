...
let tests =
    testList
        "Feature154 proof-set acceptance"
        [
            test "exactly three fresh matching attempts are accepted and selected" {
                ...
                | CompositorProof.ProofSetReadiness.Accepted proofSet ->
                    Expect.sequenceEqual
                        ...
                        [ "feature154-run-1"; "feature154-run-2"; "feature154-run-3" ]
                        "selected attempts"
            ...
            test "freshness, host profile, and proof method mismatches fail closed" {
                ...
                let otherProfile =
                    { profile with
                        ProfileId = "feature154-other-host"
                    }

                let hostMismatch =
                    { attempt "feature154-host" now with
                        Proof = proofWith otherProfile "feature154-host" CompositorProof.PresentProofPassed now
                    }

                let methodMismatch =
                    { attempt "feature154-method" now with
                        ProofMethod = "other-proof-method"
                    }
                ...
                |> List.iter (fun (attempts, token) ->
                    let readiness =
                        CompositorProof.evaluateProofSet profile now (TimeSpan.FromHours 1.0) attempts
            ...
            test "damaged update and undamaged preservation failures fail the proof set" {
                ...
                |> List.iter (fun badAttempt ->
                    ...
                    Expect.equal
                        ...
                        "failed"
                        "failed-pixel evidence cannot accept")
            }
        ]
