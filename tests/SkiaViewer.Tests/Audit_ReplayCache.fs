...
let tests =
    testSequenced
    <| testList
        "Audit ReplayCache (Feature 006: verify imported mechanisms)"
        [
            ...
            test
                "Audit: US2 parity — cache-on output is byte-identical to the disabled oracle, and the comparison is discriminating (T027, FR-004/FR-011)" {
                if not rasterAvailable then
                    ...
                    let sceneB: Scene =
                        {
                            Nodes = [ boundaryScene 0UL 6.0; boundaryScene 1UL 24.0; boundaryScene 9UL 42.0 ]
                        }
                    ...
                    Expect.equal
                        replayWarm
                        direct
                        "FR-004/FR-009: a warmed replayed boundary is byte-identical to the direct walk"
