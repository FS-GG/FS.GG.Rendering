        ...
        [
            test "init emits window-open effect" {
                ...
                Expect.exists
                    effects
                    (function
                    | OpenWindow("Product", { Width = 640; Height = 480 }) -> true
                    | _ -> false)
                    "init emits open effect"
            }
            ...
            test "render updates model and emits render effect" {
                ...
                Expect.exists
                    effects
                    (function
                    | RenderScene rendered when rendered = scene -> true
                    | _ -> false)
                    "render effect is emitted"
            }

            test "screenshot evidence reports explicit unsupported capture without claiming proof" {
                let outputPath =
                    IO.Path.Combine(IO.Path.GetTempPath(), $"skia-viewer-screenshot-{Guid.NewGuid():N}.md")
                ...
                Expect.equal
                    ...
                    "capture source names the offscreen CPU scene raster the capture really performs (#141), not a fabricated live viewer window"

                Expect.stringContains
                    result.Message
                    "offscreen"
                    "capture message names the offscreen raster path, not a viewer render target"

                Expect.contains
                    result.Diagnostics
                    "capture-source=offscreen-scene-raster"
                    "serialized capture-source line matches the real code path"

                Expect.isTrue
                    result.ProvesScreenshot
                    "offscreen scene raster still produced a real non-blank pixel artifact"
            ...
            test "evidence workflow init and update emit pure screenshot effects and success fields" {
                ...
                Expect.exists
                    effects
                    (function
                    | LaunchViewerForEvidence launched when launched = request -> true
                    | _ -> false)
                    "workflow requests viewer launch at interpreter edge"

                let launched, launchEffects =
                    Viewer.updateEvidenceWorkflow (LaunchCompleted ViewerOpenConfirmed) model
                ...
                let firstFrame, captureEffects =
                    Viewer.updateEvidenceWorkflow (FirstFrameObserved FirstFramePresentedStatus) launched
                ...
                Expect.exists
                    captureEffects
                    (function
                    | CaptureViewerScreenshot "readiness/screenshot-evidence.md" -> true
                    | _ -> false)
                    "first frame requests screenshot capture"
                ...
                | Some result ->
                    ...
                    Expect.equal
                        ...
                        (Some "readiness/artifacts/screenshot.png")
                        "successful result has PNG artifact path"
                    ...
                    Expect.equal
                        result.PixelContentValidation
                        PixelContentNonBlank
                        "successful result records non-blank pixel validation"

                    Expect.equal
                        result.ViewerOpenStatus
                        ViewerOpenConfirmed
                        "successful result preserves viewer-open fact"

                    Expect.equal
                        result.FirstFrameStatus
                        FirstFramePresentedStatus
                        "successful result preserves first-frame fact"

                    Expect.equal
                        result.CaptureAvailability
                        CaptureAvailable
                        "successful result records capture availability"
                    ...
                    Expect.exists
                        writeEffects
                        (function
                        | WriteScreenshotEvidenceReport written when written = result -> true
                        | _ -> false)
                        "successful result is handed to report writer effect"
            ...
            test "evidence workflow unsupported result separates viewer and capture capability facts" {
                ...
                let completed, effects =
                    Viewer.updateEvidenceWorkflow
                        (CaptureUnsupported(
                            "viewer host does not expose screenshot capture",
                            Some "deterministic-scene-evidence"
                        ))
                        firstFrame
                ...
                | Some result ->
                    ...
                    Expect.exists
                        ...
                        | WriteScreenshotEvidenceReport written when written = result -> true
                        | _ -> false)
                        "unsupported result is still reportable"
            ...
            test "viewer boundary guardrails preserve screenshot visual capability and window diagnostics" {
                ...
                Expect.equal
                    capability.RendererMode
                    Host.GlHost.backendLabel
                    "renderer mode names the backend that actually initializes (single source of truth), not a guessed label (#135)"
                ...
                Expect.contains
                    ...
                    "status=ok"
                    "accepted screenshot diagnostics keep stable status field"

                Expect.exists
                    screenshot.Diagnostics
                    (fun item -> item.StartsWith("scene-capabilities=", StringComparison.Ordinal))
                    "unsupported screenshot diagnostics include scene capability facts"
                ...
                Expect.exists
                    windowResults
                    (fun item ->
                        item.Option = "initial-size"
                        && item.Status = FailedOption
                        && item.Message.Contains "positive")
                    "window launch diagnostics keep positive-size validation"

                Expect.exists
                    windowResults
                    (fun item ->
                        item.Option = "startup-state"
                        && item.Status = Honored
                        && item.Message.Contains "Fullscreen")
                    "fullscreen startup is now honored, not host-unsupported"

                Expect.exists
                    windowResults
                    (fun item ->
                        item.Option = "backend"
                        && item.Status = UnsupportedOption
                        && item.Message.Contains "not supported")
                    "unsupported backend preferences remain explicit diagnostics"
            }
            ...
            test "runtime backend self-reports match the backend the real launch path initializes" {
                ...
                Expect.equal
                    capability.RendererMode
                    realLaunchBackend
                    "capability backend self-report matches the real launch path's backend (#135)"
                ...
                let completed, _ =
                    Viewer.updateRun (RecordFrame { Width = 320; Height = 200 }) started
                ...
                | Some(Ok evidence) ->
                    Expect.equal
                        ...
                        realLaunchBackend
                        "bounded-run evidence backend self-report matches the real launch path's backend (#135)"
            ...
            test "bounded run init and update expose pure lifecycle effects" {
                ...
                Expect.exists
                    effects
                    (function
                    | OpenBoundedWindow opened when opened.RendererMode = request.RendererMode -> true
                    | _ -> false)
                    "bounded run requests window opening at interpreter edge"
                ...
                Expect.exists
                    ...
                    | StopBoundedRun -> true
                    | _ -> false)
                    "first-frame target stops when evidence is collected"
            }
            ...
            test "bounded run records first-frame evidence with positive dimensions elapsed time and summary" {
                ...
                let started, startEffects =
                    Viewer.updateRun (RunStarted(DateTimeOffset.UnixEpoch)) model

                Expect.equal
                    started.StartedAt
                    (Some DateTimeOffset.UnixEpoch)
                    "start time is supplied by interpreter message"
                ...
                Expect.exists
                    ...
                    | RequestFrame -> true
                    | _ -> false)
                    "started run requests the first frame"

                let withDiagnostic, _ = Viewer.updateRun (RecordDiagnostic diagnostic) started

                let completed, effects =
                    Viewer.updateRun (RecordFrame { Width = 320; Height = 200 }) withDiagnostic
                ...
                | Some(Ok evidence) ->
                    ...
                    Expect.equal
                        evidence.RendererMode
                        Host.GlHost.backendLabel
                        "evidence names the real backend, not the requested renderer mode (#135)"

                    Expect.notEqual
                        evidence.RendererMode
                        request.RendererMode
                        "a bogus requested renderer mode does not leak into the self-reported backend (#135)"

                    Expect.equal
                        evidence.LastDiagnosticSummary
                        (Some "frame 1 presented")
                        "last diagnostic summary is preserved"
                ...
                Expect.exists
                    ...
                    | StopBoundedRun -> true
                    | _ -> false)
                    "bounded run stops after the target"
            }
                ...
                |> List.iter (fun (request, expected) ->
                    ...
                    | Result.Error failure ->
                        ...
                        Expect.equal
                            ...
                            App
                            $"{expected} validation is blocked by app/request configuration"
            ...
            test "bounded run frame-count target stops after the exact positive frame count" {
                ...
                | Some(Ok evidence) ->
                    ...
                    Expect.equal
                        evidence.LastDiagnosticSummary
                        (Some "frame 3 presented")
                        "last frame diagnostic is summarized"
            ...
            test "external observation failure preserves viewer-owned launch facts instead of headless classification" {
                ...
                Expect.equal
                    ...
                    "real-launch"
                    "observation diagnostics name real launch as source"
                ...
                Expect.equal
                    observation.BlockedStage
                    (Some Observation)
                    "external title/window miss is observation-blocked"

                Expect.notEqual
                    observation.BlockedStage
                    (Some DesktopPrerequisite)
                    "external observation miss is not a desktop-prerequisite/headless classification"

                Expect.isFalse
                    (observation.Message.Contains("headless-only", StringComparison.OrdinalIgnoreCase))
                    "message does not claim headless-only"
            ...
            test "window observation diagnostics include capture facts and missing fact names" {
                ...
                Expect.equal
                    observation.BlockedStage
                    (Some Capture)
                    "capture failure is capture-blocked when viewer facts pass"

                Expect.equal
                    observation.Classification
                    (Some UnsupportedEnvironment)
                    "capture failure is a host observation limitation"
            ...
            test "bounded run timeout uses last diagnostic summary and stops without shell timeout" {
                ...
                | Some(Result.Error failure) ->
                    ...
                    Expect.equal
                        failure.LastDiagnosticSummary
                        (Some "waiting for first frame")
                        "last diagnostic summary is retained"
                ...
                Expect.exists
                    ...
                    | StopBoundedRun -> true
                    | _ -> false)
                    "timeout stops the bounded run internally"
            }
            ...
            test "diagnostics and viewer key events flow through public update effects" {
                ...
                Expect.exists
                    diagnosticEffects
                    (function
                    | EmitDiagnostic emitted when emitted.Message = diagnostic.Message -> true
                    | _ -> false)
                    "diagnostic capture is emitted to the edge"
                ...
                Expect.exists
                    keyEffects
                    (function
                    | DispatchInput(Enter, true) -> true
                    | _ -> false)
                    "viewer key events dispatch normalized input"
            }
            ...
            test "interactive lifecycle update remains pure and emits interpreter effects" {
                ...
                Expect.exists
                    ...
                    | OpenWindow("Product", { Width = 640; Height = 480 }) -> true
                    | _ -> false)
                    "init requests window opening at the edge"
                ...
                Expect.equal
                    ...
                    "interactive start enters desktop-session precheck state"
                ...
                Expect.exists
                    ...
                    | _ -> false)
                    "interactive start requests desktop preflight as an effect"

                let framed, frameEffects =
                    Viewer.update (FramePresented { Width = 640; Height = 480 }) started

                Expect.equal
                    framed.LifecycleState
                    ViewerLifecycleState.FirstFramePresented
                    "first frame is modeled without completing launch"
                ...
                Expect.exists
                    frameEffects
                    (function
                    | EmitDiagnostic diagnostic when diagnostic.Category = Frame -> true
                    | _ -> false)
                    "frame presentation emits diagnostic effect"
                ...
                Expect.exists
                    keyEffects
                    (function
                    | DispatchInput(Space, true) -> true
                    | _ -> false)
                    "keyboard dispatch stays at interpreter boundary"
                ...
                Expect.equal
                    ...
                    "user close remains distinct from framework or evidence close"
                ...
                Expect.exists
                    ...
                    | CloseWindow -> true
                    | _ -> false)
                    "user close requests window close"
            }
            ...
            test "runApp interactive first-frame lifecycle stays open until explicit close for 30 second hold" {
                ...
                let firstFrame, frameEffects =
                    Viewer.update (FramePresented { Width = 640; Height = 480 }) running

                Expect.equal
                    firstFrame.LifecycleState
                    ViewerLifecycleState.FirstFramePresented
                    "first frame is a lifecycle milestone, not completion"
                ...
                Expect.isFalse
                    (frameEffects
                     |> List.exists (function
                         | CloseWindow -> true
                         | _ -> false))
                    "first frame must not emit close"
                ...
                Expect.exists
                    ...
                    | CheckDesktopSession -> true
                    | _ -> false)
                    "interactive launch still runs desktop precheck at the edge"
                ...
                Expect.exists
                    ...
                    | CloseWindow -> true
                    | _ -> false)
                    "explicit close emits close effect"
            }
            ...
            test "interactive launch close reasons complete only after explicit user app host or failure close" {
                ...
                Expect.equal
                    firstFrame.LifecycleState
                    ViewerLifecycleState.FirstFramePresented
                    "first frame does not complete the interactive launch"
                ...
                Expect.isFalse
                    (frameEffects
                     |> List.exists (function
                         | CloseWindow -> true
                         | _ -> false))
                    "first frame does not request evidence close"

                let userClosed, userEffects = Viewer.update ViewerMsg.UserCloseObserved firstFrame

                Expect.equal
                    userClosed.LifecycleState
                    UserCloseObservedState
                    "user close completes through user-close state"
                ...
                Expect.exists
                    ...
                    | CloseWindow -> true
                    | _ -> false)
                    "user close closes the window"

                let appClosed, appEffects = Viewer.update AppCloseRequested firstFrame

                Expect.equal
                    appClosed.LifecycleState
                    CloseRequested
                    "app close completes through app close-requested state"
                ...
                Expect.exists
                    failureEffects
                    (function
                    | EmitDiagnostic diagnostic when diagnostic.Message = "window host failed" -> true
                    | _ -> false)
                    "failure emits diagnostics instead of evidence close"
            }
            ...
            test "visibility diagnostics classify taskbar-only inaccessible windows before app lifecycle debugging" {
                ...
                Expect.equal
                    windowCreated.LifecycleState
                    ViewerLifecycleState.WindowCreated
                    "window creation is modeled before visibility success"

                Expect.exists
                    createdEffects
                    (function
                    | QueryNativeWindowState -> true
                    | _ -> false)
                    "window creation asks the interpreter for native state"
                ...
                let inaccessible, diagnosticEffects =
                    Viewer.update (VisibilityObserved taskbarOnly) windowCreated

                Expect.equal
                    inaccessible.LifecycleState
                    InaccessibleWindow
                    "taskbar-only state is degraded before app lifecycle debugging"

                Expect.exists
                    diagnosticEffects
                    (function
                    | EmitDiagnostic diagnostic when diagnostic.Message.Contains "taskbar entry" -> true
                    | _ -> false)
                    "visibility diagnostic is emitted"
            }
            ...
            test "visibility diagnostics classify inaccessible observed window states as degraded or failed" {
                ...
                for label, diagnostic in cases do
                    ...
                    Expect.equal
                        next.LifecycleState
                        InaccessibleWindow
                        $"{label} must not be reported as visible interactive success"

                    Expect.exists
                        effects
                        (function
                        | EmitDiagnostic event when event.Message.Contains diagnostic.Message -> true
                        | _ -> false)
                        $"{label} emits the native visibility diagnostic"
            }

            test
                "visibility diagnostics classify unsupported or unavailable native state separately from visible success" {
                let baseDiagnostic =
                    {
                        WindowInitialized = true
                ...
                let unsupported, unsupportedEffects =
                    Viewer.update (VisibilityObserved baseDiagnostic) model

                Expect.equal
                    unsupported.LifecycleState
                    Unsupported
                    "unsupported native visibility facts are not visible launch success"

                Expect.exists
                    unsupportedEffects
                    (function
                    | EmitDiagnostic event when event.Message.Contains "unsupported" -> true
                    | _ -> false)
                    "unsupported state emits a diagnostic"
                ...
                Expect.equal
                    unavailableModel.LifecycleState
                    InaccessibleWindow
                    "unavailable observable facts degrade rather than report visible success"
            }
            ...
            test "launch outcomes separate evidence close from user close and expose visibility fields" {
                ...
                | Result.Ok outcome ->
                    Expect.equal
                        outcome.Mode
                        "persistent-evidence"
                        "explicit evidence runs do not report interactive mode"

                    Expect.equal
                        outcome.CloseReason
                        (Some EvidenceRequestedClose)
                        "evidence close has its own close reason"
                    ...
                    Expect.equal
                        outcome.WindowVisible
                        ViewerObservedValue.Unsupported
                        "bounded evidence does not claim desktop visibility"
                | Result.Error failure ->
                    Expect.equal
                        failure.Classification
                        UnsupportedEnvironment
                        "unsupported hosts fail before claiming an evidence close"
                    ...
                    Expect.equal
                        ...
                        EnvironmentSession
                        "unsupported hosts keep environment diagnostics separate from app lifecycle"
            }
            ...
            test "MVU lifecycle transitions emit effects for session window visibility close timeout and failure states" {
                ...
                let starting, startingEffects =
                    Viewer.update (DesktopSessionChecked sessionReady) started
                ...
                Expect.exists
                    ...
                    | OpenWindow("Product", { Width = 640; Height = 480 }) -> true
                    | _ -> false)
                    "window opening stays at interpreter edge"
                ...
                Expect.equal
                    ...
                    ViewerLifecycleState.WindowCreated
                    "window-created state is represented"
                ...
                Expect.exists
                    ...
                    | QueryNativeWindowState -> true
                    | _ -> false)
                    "window creation queries native state"

                let checking, checkingEffects =
                    Viewer.update (VisibilityCheckStarted diagnostic) created
                ...
                Expect.exists
                    ...
                    | QueryNativeWindowState -> true
                    | _ -> false)
                    "visibility check requests native state"
                ...
                Expect.equal
                    ...
                    InteractiveRunning
                    "visible surface transitions to interactive running"

                Expect.exists
                    runningEffects
                    (function
                    | EmitDiagnostic event when event.Message.Contains "visible window" -> true
                    | _ -> false)
                    "visibility observation emits diagnostics"

                let requestedClose, requestCloseEffects = Viewer.update AppCloseRequested running

                Expect.equal
                    requestedClose.LifecycleState
                    CloseRequested
                    "app close requests are distinct from user close"
                ...
                Expect.exists
                    ...
                    | CloseWindow -> true
                    | _ -> false)
                    "app close emits close effect"

                let unsupportedSession =
                    { sessionReady with
                        DiagnosticClass = "unsupported-host"
                        Message = "DISPLAY is missing"
                    }

                let unsupported, unsupportedEffects =
                    Viewer.update (DesktopSessionChecked unsupportedSession) started
                ...
                Expect.exists
                    unsupportedEffects
                    (function
                    | EmitDiagnostic event when event.Category = EnvironmentSession -> true
                    | _ -> false)
                    "unsupported session emits environment diagnostic"

                let inaccessibleDiagnostic =
                    { diagnostic with
                        Visible = ViewerObservedValue.Observed false
                        FailureClass = Some "window-visibility"
                        Message = "taskbar only"
                    }

                let inaccessible, _ =
                    Viewer.update (VisibilityObserved inaccessibleDiagnostic) checking

                Expect.equal
                    inaccessible.LifecycleState
                    InaccessibleWindow
                    "inaccessible native windows are not success"
                ...
                Expect.exists
                    timeoutEffects
                    (function
                    | EmitDiagnostic event when event.Stage = Some Timeout -> true
                    | _ -> false)
                    "timeout emits diagnostic"
                ...
                Expect.exists
                    failureEffects
                    (function
                    | EmitDiagnostic event when event.Message = "window failed" -> true
                    | _ -> false)
                    "failure emits diagnostic"
            }
                    ...
                    if not (nativeWindowAvailable ()) then
                        ...
                        | Result.Ok outcome ->
                            ...
                            Expect.equal
                                ...
                                true
                                "successful interactive result requires an explicit close path"
                        ...
                        | Result.Error failure ->
                            Expect.equal
                                failure.Classification
                                UnsupportedEnvironment
                                "unsupported hosts report environment failure before lifecycle debugging"

                            Expect.equal
                                failure.BlockedStage
                                Window
                                "unsupported hosts are blocked before window lifecycle"
                else
                    let capability = Viewer.runtimeCapability ()

                    Expect.equal
                        capability.BoundedSmoke
                        true
                        "bounded evidence remains available without live persistent test opt-in"

                    Expect.equal
                        capability.KeyboardInput
                        true
                        "keyboard mapping remains part of the interactive contract"
            }
            ...
            test "evidence lifecycle update emits bounded effects and failure transitions" {
                ...
                Expect.exists
                    evidenceEffects
                    (function
                    | StartBoundedRun started when started.Target = FirstFrame -> true
                    | _ -> false)
                    "evidence mode starts bounded interpreter effect"
                ...
                Expect.exists
                    ...
                    | CloseWindow -> true
                    | _ -> false)
                    "evidence completion closes through an effect"
                ...
                Expect.exists
                    timeoutEffects
                    (function
                    | EmitDiagnostic diagnostic when diagnostic.Stage = Some Timeout -> true
                    | _ -> false)
                    "timeout emits a diagnostic effect"
                ...
                Expect.exists
                    failureEffects
                    (function
                    | EmitDiagnostic diagnostic when diagnostic.Message = "host failed" -> true
                    | _ -> false)
                    "failure emits diagnostic effect"
            }
                ...
                if livePersistentTestsEnabled () then
                    ...
                    | Result.Ok outcome ->
                        ...
                        Expect.equal
                            ...
                            true
                            "interactive launch reports first-frame presentation"

                        Expect.equal
                            outcome.SelfClosedForEvidence
                            false
                            "interactive launch is not reported as evidence self-close"

                        Expect.equal
                            outcome.UserCloseObserved
                            true
                            "successful interactive outcome requires explicit close observation"

                        Expect.equal outcome.ExitPath true "intentional exit path is explicit"

                        Expect.equal
                            outcome.InputDispatch
                            "not-applicable"
                            "scene-only launch marks input dispatch not applicable"
                    ...
                    | Result.Error failure ->
                        Expect.equal
                            failure.Classification
                            UnsupportedEnvironment
                            "headless or unsupported hosts are classified separately from product defects"
                        ...
                        Expect.stringContains
                            failure.Message
                            "DISPLAY"
                            "unsupported Linux diagnostics name the missing display host"
                else
                    let capability = Viewer.runtimeCapability ()
            ...
            test "desktop diagnostics expose environment-session classification before lifecycle debugging" {
                ...
                Expect.isTrue
                    (diagnostic.DiagnosticClass.StartsWith("environment-session", StringComparison.Ordinal)
                     || diagnostic.DiagnosticClass = "unsupported-host")
                    "desktop diagnostics use environment/session or unsupported-host class"

                Expect.equal
                    diagnostic.FallbackIsFullDesktopSession
                    false
                    "private runtime fallback is never a full desktop session"
                ...
                if OperatingSystem.IsLinux() then
                    withEnvironment
                        ...
                        (fun () ->
                            ...
                            Expect.isFalse
                                diagnostic.RuntimeDirectoryOwnerSuitable
                                "missing runtime directory is not owner-suitable"

                            Expect.isFalse
                                diagnostic.RuntimeDirectoryPermissionsSuitable
                                "missing runtime directory is not permission-suitable"
                            ...
                            Expect.isSome
                                diagnostic.FallbackRuntimeDirectory
                                "fallback runtime directory is named for diagnostics"

                            Expect.isFalse
                                diagnostic.FallbackIsFullDesktopSession
                                "fallback is explicitly not a full desktop session"

                            Expect.equal
                                diagnostic.DiagnosticClass
                                "unsupported-host"
                                "missing session prerequisites classify as unsupported host"

                            Expect.stringContains
                                diagnostic.Message
                                "XDG_RUNTIME_DIR"
                                "message names the missing runtime directory")
                else
                    ...
                    try
                        withEnvironment
                            ...
                            (fun () ->
                                ...
                                Expect.equal
                                    diagnostic.RuntimeDirectory
                                    (Some runtimeDirectory)
                                    "runtime directory path is reported"

                                Expect.isTrue
                                    diagnostic.RuntimeDirectoryExists
                                    "runtime directory existence is reported"

                                Expect.isTrue
                                    diagnostic.RuntimeDirectoryOwnerSuitable
                                    "owned runtime directory is owner-suitable"

                                Expect.isTrue
                                    diagnostic.RuntimeDirectoryPermissionsSuitable
                                    "runtime directory permissions are suitable"

                                Expect.equal
                                    diagnostic.DisplayVariable
                                    (Some "WAYLAND_DISPLAY=wayland-0")
                                    "Wayland display variable is preferred"

                                Expect.equal
                                    diagnostic.DisplaySocket
                                    (Some waylandSocket)
                                    "Wayland socket path is derived from runtime directory"

                                Expect.isTrue diagnostic.DisplaySocketExists "Wayland socket existence is reported"

                                Expect.equal
                                    diagnostic.SessionBus
                                    (Some "unix:path=/tmp/fs-gg-bus")
                                    "session bus is reported"

                                Expect.equal
                                    diagnostic.DiagnosticClass
                                    "environment-session-ready"
                                    "present prerequisites classify as ready"
                ...
                withEnvironment [ "DISPLAY", ":0"; "WAYLAND_DISPLAY", "wayland-1" ] (fun () ->
                    ...
                    if OperatingSystem.IsLinux() then
                        Expect.equal
                            observedInside
                            "<null>"
                            "WAYLAND_DISPLAY is nulled while the window is created so GLFW selects X11"
                        ...
                        Expect.equal
                            observedInside
                            "wayland-1"
                            "off Linux the override is a pass-through and leaves WAYLAND_DISPLAY untouched")
            }
            ...
            test "runtime capability distinguishes persistent window bounded smoke keyboard and unsupported reasons" {
                ...
                Expect.equal
                    capability.RendererMode
                    Host.GlHost.backendLabel
                    "renderer mode names the actual backend independently from host support (#135)"

                Expect.isEmpty
                    capability.MissingPackageCapabilities
                    "current package exposes the persistent contract and has no package-capability gap"
                ...
                |> List.iter (fun reason ->
                    Expect.isFalse
                        (capability.MissingPackageCapabilities |> List.contains reason)
                        "unsupported host reasons are not reported as missing package capabilities")
            }
            ...
            test "window behavior validation reports resize maximize startup state position and backend results" {
                ...
                Expect.exists
                    ...
                    (fun item ->
                        item.Option = "resize"
                        && item.Status = Honored
                        && item.Observed = Some "fixed-size")
                    "resize policy is reported"

                Expect.exists
                    results
                    (fun item ->
                        item.Option = "maximize"
                        && item.Status = Honored
                        && item.Observed = Some "not-maximizable")
                    "maximize policy is reported"
                ...
                Expect.exists
                    ...
                    (fun item ->
                        item.Option = "startup-state"
                        && item.Status = Honored
                        && item.Observed = Some "maximized")
                    "startup state is reported"

                Expect.exists
                    results
                    (fun item ->
                        item.Option = "startup-position"
                        && item.Status = Honored
                        && item.Observed = Some "20,30")
                    "startup position is reported"
                ...
                Expect.exists
                    ...
                    (fun item ->
                        item.Option = "backend"
                        && item.Status = Honored
                        && item.Observed = Some "opengl")
                    "backend preference is reported"
            }

            test
                "window behavior validation rejects invalid coordinates and unsupported backend settings with diagnostics" {
                let results =
                    Viewer.validateWindowBehavior
                        { Viewer.defaultWindowBehavior with
                ...
                Expect.exists
                    ...
                    (fun item ->
                        item.Option = "startup-state"
                        && item.Status = UnsupportedOption
                        && item.Message.Contains "visible interactive")
                    "minimized startup is unsupported for visible launch validation"
                ...
                Expect.exists
                    ...
                    (fun item ->
                        item.Option = "startup-position"
                        && item.Status = FailedOption
                        && item.Message.Contains "non-negative")
            ...
            test "default window behavior starts in windowed fullscreen" {
                ...
                Expect.equal
                    ...
                    "windowed fullscreen is the new no-flag default startup state"
            }
            ...
            test "fullscreen and windowed fullscreen validate as honored while minimized stays unsupported" {
                ...
                Expect.equal
                    (resultFor ViewerWindowStartupState.Fullscreen).Status
                    Honored
                    "fullscreen is honored in validateWindowBehavior"

                Expect.equal
                    (resultFor ViewerWindowStartupState.WindowedFullscreen).Status
                    Honored
                    "windowed fullscreen is honored in validateWindowBehavior"

                Expect.equal
                    (resultFor ViewerWindowStartupState.Minimized).Status
                    UnsupportedOption
                    "minimized is not a visible interactive launch state"

                Expect.equal
                    (launchResultFor ViewerWindowStartupState.Fullscreen).Status
                    Honored
                    "fullscreen is honored in validateWindowLaunchBehavior"

                Expect.equal
                    (launchResultFor ViewerWindowStartupState.WindowedFullscreen).Status
                    Honored
                    "windowed fullscreen is honored in validateWindowLaunchBehavior"

                Expect.equal
                    (launchResultFor ViewerWindowStartupState.Minimized).Status
                    UnsupportedOption
                    "minimized stays unsupported in validateWindowLaunchBehavior"
            }
            ...
            test "fullscreen and windowed fullscreen are distinct selectable states, not aliases" {
                ...
                Expect.equal
                    fullscreen.Observed
                    (Some "fullscreen")
                    "exclusive fullscreen observes the fullscreen value"

                Expect.equal
                    windowedFullscreen.Observed
                    (Some "windowed-fullscreen")
                    "windowed fullscreen observes a distinct value"

                Expect.notEqual
                    fullscreen.Observed
                    windowedFullscreen.Observed
                    "fullscreen and windowed fullscreen are never aliased"
            }
            ...
            test "window launch behavior validation includes positive size constraints and all public option families" {
                ...
                let valid =
                    Viewer.validateWindowLaunchBehavior { Width = 640; Height = 480 } behavior

                let invalid =
                    Viewer.validateWindowLaunchBehavior { Width = 0; Height = 480 } behavior
                ...
                Expect.exists
                    ...
                    (fun item ->
                        item.Option = "initial-size"
                        && item.Status = Honored
                        && item.Observed = Some "640x480")
                    "positive initial size is honored"
                ...
                Expect.exists
                    ...
                    (fun item ->
                        ...
                        && item.Status = Honored
                        && item.Observed = Some "fixed-size")
                    "resize policy remains part of launch validation"
                ...
                Expect.exists
                    ...
                    (fun item ->
                        ...
                        && item.Status = Honored
                        && item.Observed = Some "not-maximizable")
                    "maximize policy remains part of launch validation"
                ...
                Expect.exists
                    ...
                    (fun item ->
                        item.Option = "startup-state"
                        && item.Status = Honored
                        && item.Observed = Some "normal")
                    "startup state remains part of launch validation"
                ...
                Expect.exists
                    ...
                    (fun item ->
                        item.Option = "startup-position"
                        && item.Status = Honored
                        && item.Observed = Some "centered")
                    "startup position remains part of launch validation"
                ...
                Expect.exists
                    ...
                    (fun item ->
                        item.Option = "backend"
                        && item.Status = Honored
                        && item.Observed = Some "default")
                    "backend preference remains part of launch validation"
                ...
                Expect.exists
                    ...
                    (fun item ->
                        ...
                        && item.Status = FailedOption
                        && item.Message.Contains "positive")
                    "non-positive size fails before native launch"
            }
            ...
            test "init and start effects carry public window behavior through the MVU boundary" {
                ...
                Expect.exists
                    initEffects
                    (function
                    | ApplyWindowOptions request -> request = behavior
                    | _ -> false)
                    "init emits option application for interpreter startup"
                ...
                Expect.exists
                    ...
                    | ApplyWindowOptions request -> request = behavior
                    | _ -> false)
                    "desktop-session transition applies requested options before visibility checks"
            }
            ...
            test "persistent run preserves bounded APIs as explicit separate helpers" {
                ...
                | Result.Error persistentFailure, Result.Error boundedFailure ->
                    Expect.equal
                        persistentFailure.Classification
                        ProductDefect
                        "persistent option validation remains product-defect classification"

                    Expect.equal
                        boundedFailure.Classification
                        ProductDefect
                        "bounded request validation remains product-defect classification"

                    Expect.stringContains
                        boundedFailure.Message
                        "frame count"
                        "bounded helper keeps its own request validation"
            ...
            test "generated app host public boundary maps keyboard tick update and close effects" {
                ...
                Expect.exists
                    ...
                    | RenderScene _ -> true
                    | _ -> false)
                    "host update effects are emitted at the viewer boundary"

                Expect.equal
                    (host.Tick(TimeSpan.FromMilliseconds 16.0))
                    (Some Increment)
                    "tick mapping is public and pure"
                ...
                if livePersistentTestsEnabled () then
                    ...
                    | Result.Error failure ->
                        Expect.equal
                            failure.Classification
                            UnsupportedEnvironment
                            "runApp reports unsupported host separately from product defects"
                else
                    Expect.equal
                        (host.Tick(TimeSpan.FromMilliseconds 16.0))
                        (Some Increment)
                        "tick mapping remains testable without opening a live window"
            }
            ...
            test "generated host MVU boundary asserts init update effects keyboard tick first-frame and close" {
                ...
                Expect.exists
                    initEffects
                    (function
                    | EmitDiagnostic diagnostic when diagnostic.Message = "generated host init" -> true
                    | _ -> false)
                    "generated host init exposes startup effects"
                ...
                Expect.exists
                    ...
                    | RenderScene _ -> true
                    | _ -> false)
                    "keyboard update emits render refresh"
                ...
                | Some tickMsg ->
                    ...
                    Expect.exists
                        ...
                        | RenderScene _ -> true
                        | _ -> false)
                        "tick progression emits render refresh"
                ...
                Expect.equal
                    firstFrame.LifecycleState
                    ViewerLifecycleState.FirstFramePresented
                    "first-frame state is represented in viewer model"
                ...
                Expect.isFalse
                    (frameEffects
                     |> List.exists (function
                         | CloseWindow -> true
                         | _ -> false))
                    "first-frame state does not close the host"
            ...
            test "generated host observes input availability and native close through viewer lifecycle" {
                ...
                Expect.exists
                    ...
                    | RenderScene _ -> true
                    | _ -> false)
                    "manual input emits render refresh effect"
                ...
                let visible, visibilityEffects =
                    Viewer.update (VisibilityObserved diagnostic) running

                Expect.equal
                    visible.LifecycleState
                    InteractiveRunning
                    "input-capable visible window reaches interactive running"

                Expect.exists
                    visibilityEffects
                    (function
                    | EmitDiagnostic event when event.Message.Contains "keyboard input" -> true
                    | _ -> false)
                    "input-device observation is diagnostic evidence"

                let firstFrame, frameEffects =
                    Viewer.update (FramePresented { Width = 640; Height = 480 }) visible

                Expect.equal
                    firstFrame.LifecycleState
                    ViewerLifecycleState.FirstFramePresented
                    "first frame is modeled after input-capable visibility"
                ...
                Expect.isFalse
                    (frameEffects
                     |> List.exists (function
                         | CloseWindow -> true
                         | _ -> false))
                    "first frame does not close interactive host"

                let nativeClosed, nativeCloseEffects = Viewer.update HostCloseObserved firstFrame

                Expect.equal
                    nativeClosed.LifecycleState
                    Closing
                    "native host close is explicit and distinct from user close"
                ...
                Expect.exists
                    ...
                    | CloseWindow -> true
                    | _ -> false)
                    "native close emits close effect"
            }
            ...
            test "runAppEvidence is explicit and keeps invalid evidence requests separate from interactive launch" {
                ...
                | Result.Error failure ->
                    Expect.equal
                        failure.Classification
                        ProductDefect
                        "invalid evidence request is product configuration failure"
            ...
            test "runAppEvidence explicit launch reports persistent evidence self-close and first-frame fields" {
                ...
                | Result.Ok outcome ->
                    ...
                    Expect.equal
                        outcome.InputDispatch
                        "not-required"
                        "first-frame evidence does not require input dispatch"
                    ...
                    Expect.stringContains
                        ...
                        "serialized evidence records persistent evidence mode"
                    ...
                    Expect.stringContains
                        evidenceText
                        "window-opened=True"
                        "serialized evidence records window-opened status"

                    Expect.stringContains
                        evidenceText
                        "self-closed-for-evidence=True"
                        "serialized evidence records self-close semantics"

                    Expect.stringContains
                        evidenceText
                        "input-dispatch=not-required"
                        "serialized evidence records input-dispatch status"

                    Expect.stringContains
                        evidenceText
                        "first-frame-presented=True"
                        "serialized evidence records first-frame status"
                    ...
                    Expect.stringContains
                        evidenceText
                        "blocked-stage="
                        "serialized evidence records blocked stage field"

                    Expect.stringContains
                        evidenceText
                        "classification="
                        "serialized evidence records classification field"
                ...
                try
                    ...
                    | Result.Ok outcome ->
                        Expect.equal
                            outcome.Mode
                            "persistent-evidence"
                            "image evidence is an explicit evidence launch mode"

                        Expect.exists
                            outcome.VisualEvidence
                            (fun item -> item.Kind = ViewerVisualEvidenceKind.Image)
                            "requested image evidence records an image artifact"

                        Expect.exists
                            outcome.VisualEvidence
                            (fun item -> item.ImageDecodable = Some true)
                            "requested image evidence records decodability"

                        Expect.exists
                            outcome.VisualEvidence
                            (fun item -> item.ProvesSceneRendering && item.ProvesDesktopVisibility = false)
                            "scene-rendering and desktop-visibility claims are explicit"

                        Expect.isTrue
                            (isPngFile imagePath)
                            "requested image evidence path contains a decodable PNG image"
            ...
            test "metadata hash evidence is labeled separately and never written as screenshot image" {
                ...
                try
                    ...
                    | Result.Ok outcome ->
                        Expect.exists
                            outcome.VisualEvidence
                            (fun item -> item.Kind = ViewerVisualEvidenceKind.MetadataHash)
                            "metadata evidence is labeled metadata-hash"

                        Expect.isFalse
                            (outcome.VisualEvidence
                             |> List.exists (fun item ->
                                 item.Kind = ViewerVisualEvidenceKind.Image && item.Path = Some metadataPath))
                            "metadata/hash artifacts are not mislabeled as image evidence"

                        Expect.exists
                            outcome.VisualEvidence
                            (fun item ->
                                item.Kind = ViewerVisualEvidenceKind.MetadataHash
                                && not item.ProvesDesktopVisibility)
                            "metadata/hash evidence does not claim desktop visibility"
            ...
            test "evidence launch timeout and failure paths are explicit and bounded" {
                ...
                | Some(Result.Error failure) ->
                    ...
                    Expect.equal
                        failure.LastDiagnosticSummary
                        (Some "waiting for evidence target")
                        "timeout keeps last diagnostic summary"
                ...
                Expect.exists
                    ...
                    | StopBoundedRun -> true
                    | _ -> false)
                    "timeout stops bounded evidence"
                ...
                let evidenceModel, evidenceEffects =
                    Viewer.update (StartEvidence request) launchModel

                Expect.equal
                    evidenceModel.LifecycleState
                    EvidenceRunning
                    "evidence launch enters evidence-running state"

                Expect.exists
                    evidenceEffects
                    (function
                    | StartBoundedRun started when started.Target = request.Target -> true
                    | _ -> false)
                    "evidence launch emits bounded run effect"
            }
            ...
            test
                ...
                |> List.iter (fun item ->
                    Expect.isTrue
                        (Viewer.shouldCaptureDiagnostic options item)
                        $"captures {item.Category} {item.Level}")

                Expect.isFalse
                    (Viewer.shouldCaptureDiagnostic
                        options
                        (diagnostic ViewerDiagnosticLevel.Info ViewerDiagnosticCategory.Startup "startup info"))
                    "info below warning threshold is filtered"

                Expect.isFalse
                    (Viewer.shouldCaptureDiagnostic
                        options
                        (diagnostic ViewerDiagnosticLevel.Warning OpenGl "opengl detail"))
                    "unselected category is filtered"

                Expect.isFalse
                    (Viewer.shouldCaptureDiagnostic options (diagnostic ViewerDiagnosticLevel.Error Skia "skia detail"))
                    "unselected Skia category is filtered"
                ...
                Expect.isFalse
                    (Viewer.shouldCaptureDiagnostic
                        ...
                        (diagnostic
                            ...
                            "framebuffer detail"))
                    "unselected framebuffer category is filtered"

                Expect.isFalse
                    (Viewer.shouldCaptureDiagnostic
                        options
                        (diagnostic ViewerDiagnosticLevel.Error ViewerDiagnosticCategory.Scene "scene detail"))
                    "unselected scene category is filtered"

                Expect.isFalse
                    (Viewer.shouldCaptureDiagnostic
                        options
                        (diagnostic ViewerDiagnosticLevel.Error Frame "frame detail"))
                    "unselected frame category is filtered"
            }
            ...
            test "frame sampling excludes repeated per-frame diagnostics unless enabled and bounded by the frame limit" {
                ...
                Expect.isTrue
                    (Viewer.shouldCaptureDiagnostic startupOnly startup)
                    "startup diagnostics are still captured"

                Expect.isFalse
                    (Viewer.shouldCaptureDiagnostic startupOnly (frame 1))
                    "startup-only diagnostics exclude frame messages"
                ...
                Expect.isTrue
                    (Viewer.shouldCaptureDiagnostic sampledFrames (frame 2))
                    "second sampled frame is captured"

                Expect.isFalse
                    (Viewer.shouldCaptureDiagnostic sampledFrames (frame 3))
                    "frame diagnostics stop after the configured limit"
                ...
                Expect.isTrue
                    (Viewer.shouldCaptureDiagnostic unlimitedFrames (frame 25))
                    "unbounded frame diagnostics are explicit"
            }
            ...
            test "diagnostic sink captures startup input renderer and frame categories in-process" {
                ...
                Expect.equal
                    ...
                    "sink captures selected categories without stderr scraping"
            ...
            test "viewer update emits categorized diagnostics for startup input scene frame and failure milestones" {
                ...
                Expect.exists
                    initEffects
                    (function
                    | EmitDiagnostic diagnostic when
                        diagnostic.Category = ViewerDiagnosticCategory.Startup
                        && diagnostic.Message.Contains "Product"
                        ->
                        true
                    | _ -> false)
                    "startup emits categorized diagnostic"
                ...
                Expect.exists
                    renderEffects
                    (function
                    | EmitDiagnostic diagnostic when diagnostic.Category = ViewerDiagnosticCategory.Scene -> true
                    | _ -> false)
                    "scene render emits categorized diagnostic"
                ...
                Expect.exists
                    inputEffects
                    (function
                    | EmitDiagnostic diagnostic when diagnostic.Category = Input && diagnostic.Message.Contains "Space" ->
                        true
                    | _ -> false)
                    "input emits raw and normalized key diagnostic"

                let _, frameEffects =
                    Viewer.update (FramePresented { Width = 640; Height = 480 }) model

                Expect.exists
                    frameEffects
                    (function
                    | EmitDiagnostic diagnostic when
                        diagnostic.Category = ViewerDiagnosticCategory.Frame
                        && diagnostic.Message.Contains "640x480"
                        ->
                        true
                    | _ -> false)
                    "frame milestone emits categorized diagnostic"
                ...
                Expect.exists
                    ...
                    (function
                    | EmitDiagnostic diagnostic when
                        diagnostic.Category = ViewerDiagnosticCategory.Framebuffer
                        && diagnostic.Stage = Some ViewerRunBlockedStage.GlContext
                        ->
                        true
                    | _ -> false)
                    "framebuffer failure preserves category and stage"
            }
        ]
