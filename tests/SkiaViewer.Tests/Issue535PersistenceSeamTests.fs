...
let issue535PersistenceSeamTests =
    testList
        ...
        [
            ...
            test
                "runAppWithWindowBehaviorAndAudioAndPersistence never reaches the audio or persistence sink on a host that cannot open a window" {
            ...
            test
                "PersistenceOutcome.Absent and PersistenceOutcome.Unreadable are distinct answers — collapsing them silently eats corruption" {
            ...
            test
                "PersistenceOutcome.Failed is distinct from PersistenceOutcome.Unreadable — the request never completed" {
