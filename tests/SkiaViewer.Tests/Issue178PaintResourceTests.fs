...
let private tempDir () =
    let dir =
        Path.Combine(Path.GetTempPath(), "issue178-" + Guid.NewGuid().ToString("N"))
        ...
        [
            ...
            test "an edited file re-decodes rather than serving the stale bitmap" {
                ...
                try
                    ...
                    Expect.isFalse
                        (Object.ReferenceEquals(redImage, blueImage))
                        "the edit is not served from the stale entry"
                finally
                    SceneRenderer.ImageCache.dispose ()
                    Directory.Delete(dir, true)
            }
            ...
            test "a missing or undecodable source is cached, not retried every frame" {
                ...
                try
                            ...
                            [
                                {
                                    Nodes =
                                        [
                                            SceneNode.Image((0.0, 0.0, 8.0, 8.0), missing)
                                            SceneNode.Image((0.0, 0.0, 8.0, 8.0), corrupt)
                                        ]
                                }
                            ]
                    ...
                    Expect.isTrue
                        (isNull (SceneRenderer.ImageCache.resolve corrupt))
                        "an undecodable file resolves to null"
            ...
            test "the font cache is bounded under animated font sizes and disposes what it evicts" {
                ...
                Expect.equal
                    (Fonts.fontCacheCount ())
                    Fonts.fontCacheCap
                    "the cache settles at its cap, however many sizes are drawn"
            ...
            test "re-resolving the same font spec reuses the cached SKFont" {
                ...
                Expect.isTrue
                    (Object.ReferenceEquals(first, Fonts.resolveFont spec))
                    "the same spec returns the same font"
