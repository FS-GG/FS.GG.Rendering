...
let private interactiveAudioHost: InteractiveViewerHost<Model, Msg> =
    {
        ...
        Update =
            fun Hit model ->
                ...
                next,
                [
                    PlayAudio [ Audio.playSfx blip 0.5 ]
                    RenderScene(Text((0.0, 0.0), "hit", white))
                ]
    ...
    testList
        ...
        [
            ...
            test "a pointer-driven update requests sound the interactive host can now realize" {
                ...
                let msgs =
                    interactiveAudioHost.MapPointer click { Width = 320; Height = 200 } { Hits = 0 }
                ...
                Expect.equal
                    evidence.Requested
                    [ Audio.playSfx blip 0.5 ]
                    "the pointer interaction requested exactly one sfx"
            }
            ...
            test "runInteractiveViewerWithAudio never reaches the sink on a host that cannot open a window" {
                ...
                if Viewer.runtimeCapability().PersistentWindow then
                    ...
                    match
                        Viewer.runInteractiveViewerWithAudio
                            options
                            (fun batch -> played.AddRange batch)
                            interactiveAudioHost
                    with
            ...
            test "the sink a product installs drives a real backend end to end" {
                ...
                let run effects =
                    Viewer.interpretViewerEffects sink ignore ignore ignore ignore ignore effects
                    |> ignore
