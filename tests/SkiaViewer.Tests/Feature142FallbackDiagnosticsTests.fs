...
let tests =
    testSequenced
    <| testList
        "Feature142 fallback diagnostics"
        [
            ...
            test "scoped provider selection stays isolated and restores state under parallel stress" {
                        ...
                        try
                            if iteration % 2 = 0 then
                                Fonts.withClearedShapingProvider (fun () ->
                                    ...
                                    if
                                        shaped.Provider.Availability <> ProviderCleared
                                        || shaped.FallbackMode <> ProviderUnavailableFallback
                                    then
                                        failures.Enqueue
                                            $"iteration {iteration}: cleared scope observed {shaped.Provider.Availability}/{shaped.FallbackMode}")
                            else
                                Fonts.withInstalledShapingProvider (fun () ->
                                    let shaped = Text.shapeText "office" font

                                    if
                                        shaped.Provider.Availability <> ProviderInstalled
                                        || shaped.FallbackMode <> Shaped
                                    then
                                        failures.Enqueue
                                            $"iteration {iteration}: installed scope observed {shaped.Provider.Availability}/{shaped.FallbackMode}")
                        with ex ->
                ...
                Expect.equal
                    (Text.shapingProviderStatus ())
                    previous
                    "parallel scopes restore the exact prior provider status"
            }
        ]
