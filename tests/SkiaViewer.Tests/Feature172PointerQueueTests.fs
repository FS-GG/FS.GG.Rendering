...
let tests =
    testList
        "Feature172 pointer queue responsiveness"
        [
            test "discrete pointer input drains before a queued non-input tick" {
                let _, q1 =
                    Feature167SchedulerFixtures.enqueue ViewerResponsivenessInputKind.Tick "tick" Viewer.emptyInputQueue
                ...
                Expect.equal
                    ...
                    [
                        ...
                        ViewerResponsivenessInputKind.Tick
                    ]
                    "pointer click precedes background tick"
            ...
            test "coalesced pointer move keeps latest sample and records skipped movement" {
                ...
                Expect.equal
                    (drain.CoalescedPointer |> Option.map _.SequenceId)
                    (Some latest.SequenceId)
                    "latest pointer move wins"
