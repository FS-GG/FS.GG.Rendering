        ...
        [
            ...
            test "falls back when proof is missing or host-mismatched" {
                ...
                Expect.stringContains
                    (missing.FallbackReason |> Option.defaultValue "")
                    "missing present-path proof"
                    "missing proof reason"

                Expect.equal mismatch.Kind GlHost.DamageRenderDecisionKind.FullRedraw "host mismatch fallback"

                Expect.stringContains
                    (mismatch.FallbackReason |> Option.defaultValue "")
                    "host-mismatched"
                    "host mismatch reason"
            }

            test
                "Synthetic rejection fixture: rejects invalid damage and parity mismatch before accepting no-clear output" {
                ...
                Expect.stringContains
                    (invalid.FallbackReason |> Option.defaultValue "")
                    "out-of-bounds"
                    "invalid reason"
                ...
                Expect.stringContains
                    (parity.FallbackReason |> Option.defaultValue "")
                    "parity-mismatch"
                    "parity reason"
            }

            test
                "Synthetic rejection fixture: empty no-change damage skips repaint without publishing accepted partial-redraw artifacts" {
            ...
            test "viewer and host diagnostics expose stable Feature157 decision tokens" {
                Expect.equal
                    (Viewer.damageDecisionToken ViewerDamageDecision.DamageScopedAccepted)
                    "damage-scoped-accepted"
                    "viewer token"

                let diagnostic =
                    Diagnostics.damageScopedDecision "full-redraw" (Some "missing-retained-content")
