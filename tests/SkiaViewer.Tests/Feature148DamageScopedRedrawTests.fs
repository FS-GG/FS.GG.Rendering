...
let tests =
    testList
        "Feature148 damage-scoped redraw host decisions"
        [
            test "proof-gated damage union uses scissor when ready and localized" {
                let decision =
                    GlHost.decideScissorRedraw
                        ...
                        false
                        [ scissor 10 12 40 30; scissor 20 20 10 10 ]
                        100
                        100
            ...
            test "stale proof, host mismatch, disabled mode, and full-frame invalidation fall back" {
                ...
                |> List.iter (fun readiness ->
                    ...
                    | GlHost.ScissorDecision.FullRedraw reason ->
                        Expect.isFalse (System.String.IsNullOrWhiteSpace reason) "fallback reason"
