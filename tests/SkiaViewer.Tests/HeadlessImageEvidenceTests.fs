        ...
        [
            ...
            test "renderPng yields a decodable, correctly-sized, non-blank, deterministic PNG (FR-003/SC-001/SC-002)" {
                ...
                | Result.Ok first, Result.Ok second ->
                    ...
                    Expect.isGreaterThan
                        distinct
                        1
                        "PNG must be non-blank (more than one distinct pixel colour, i.e. real scene content, not a flat clear or a hash stub)"

                    Expect.isGreaterThan
                        first.Length
                        64
                        "a real PNG is far larger than the prior ~64-byte hash stub (SC-005)"

                    Expect.equal
                        first
                        second
                        "the same (scene, size) must render byte-for-byte identical bytes (FR-003)"
            ...
            test "renderPng is deterministic across independent scene instances (SC-002)" {
                ...
                | Result.Ok a, Result.Ok b ->
                    Expect.equal a b "two independently-built instances of the same scene must render identical bytes"
            ...
            test "concurrent renderPng calls are isolated and individually deterministic (Edge Case: concurrency)" {
                ...
                | Result.Ok expectedA, Result.Ok expectedB ->
                    ...
                    Expect.isTrue
                        (Array.forall id results)
                        "every concurrent render must equal its sequential baseline (isolated + deterministic)"
                | other -> failtestf "expected Ok baselines, got %A" other
            }

            // ── T018 [US3]: honest failure when no rasterizer is injected (FR-005/SC-005) ──────────
            test
                "renderPng returns a typed UnsupportedEnvironment failure (never a stub) when no rasterizer is injected" {
                ...
                try
                    ...
                    | Result.Error failure ->
                        Expect.equal
                            ...
                            SceneEvidenceFailureClassification.UnsupportedEnvironment
                            "no rasterizer ⇒ unsupported-environment classification"
                        ...
                        Expect.isFalse
                            ...
                            "the failure must carry a human-readable message"
                    | Result.Ok bytes ->
                        failtestf
                            "expected a typed failure, got %d bytes (success-shaped non-image is forbidden by SC-005)"
                            bytes.Length
                finally
                    Text.installPngRasterizer ()
            }
            ...
            test "renderPng rejects non-positive sizes as ProductDefect and never emits a stub for large sizes" {
                ...
                for badSize in
                    ...
                    | Result.Error failure ->
                        Expect.equal
                            failure.Classification
                            SceneEvidenceFailureClassification.ProductDefect
                            $"size {badSize.Width}x{badSize.Height} must classify as product-defect"
                ...
                | Result.Error failure ->
                    Expect.isFalse
                        (String.IsNullOrWhiteSpace failure.Message)
                        "a large-size failure must be a typed diagnostic, never a stub"
            }
            ...
            test "headless render discloses bundled-font fallback rather than silently substituting (Edge Case: fonts)" {
                ...
                | Result.Ok _ ->
                    ...
                    Expect.isGreaterThan
                        (report.SubstitutedCount + report.TofuCount)
                        0
                        "an uncovered code point must be disclosed via the fallback report, not silently substituted"
