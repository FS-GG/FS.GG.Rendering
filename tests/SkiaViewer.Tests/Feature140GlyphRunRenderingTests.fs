    ...
    <| testList
        ...
        [
            test "bundled-font glyph-run proof measures the same advance it draws" {
                ...
                Expect.floatClose
                    Accuracy.medium
                    data.Metrics.Advance
                    real.Width
                    "glyph-run proof advance comes from the bundled-font renderer"

                Expect.floatClose
                    Accuracy.medium
                    (data.Glyphs |> List.sumBy _.Advance)
                    data.Metrics.Advance
                    "glyph advances sum to the draw advance"
            ...
            test "non-opt-in text fallback compatibility is unchanged" {
                ...
                let proof =
                    Scene.glyphRun
                        { X = 20.0; Y = 40.0 }
                        (Fonts.buildGlyphRunData "Legacy" font)
