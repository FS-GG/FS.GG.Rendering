...
let tests =
    testList
        "Feature167 responsiveness summary"
        [
            ...
            test "writeResponsivenessRun writes all required artifacts" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fs-gg-feature167-" + Guid.NewGuid().ToString("N"))

                let records =
                    [
                        Feature167SchedulerFixtures.latency 1 ViewerResponsivenessInputKind.KeyDown 12.0
                    ]
