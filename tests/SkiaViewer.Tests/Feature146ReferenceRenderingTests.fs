...
let private tempOut name =
    let path =
        Path.Combine(Path.GetTempPath(), "fs-gg-feature146", name + "-" + Guid.NewGuid().ToString("N"))
...
let feature146ReferenceRenderingTests =
    testList
        "Feature146 reference rendering"
        [
            ...
            test "rejected package does not emit render effect" {
                ...
                Expect.exists
                    effects
                    (function
                    | WriteReferenceEvidence _ -> true
                    | _ -> false)
                    "failed evidence is written"
            }
            ...
            test "valid package produces passed or environment-limited evidence" {
                ...
                | ReferencePassed ->
                    ...
                    Expect.isTrue
                        (File.Exists(Path.Combine(req.OutputDirectory, "reference-evidence.md")))
                        "summary exists"
                | ReferenceEnvironmentLimited ->
                    ...
                    Expect.exists
                        ...
                        (fun d -> d.Length > 0)
                        "environment-limited evidence has diagnostics"
                | ReferenceFailed -> failtestf "valid package failed reference rendering: %A" evidence.Diagnostics
            }
        ]
