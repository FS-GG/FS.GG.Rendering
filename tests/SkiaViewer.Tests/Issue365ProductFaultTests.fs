        ...
        [
            testList
                ...
                [
                    test "a succeeding step returns its result and reports nothing" {
                        ...
                        let result =
                            GlHost.tryProductStep (fun d -> reports <- d :: reports) "Update" (fun () -> 41 + 1)
                    ...
                    test "a throwing step is dropped and reported as an App-stage Error — never frameRenderFailed" {
                        ...
                        | [ diagnostic ] ->
                            Expect.equal
                                ...
                                "a product fault is an error, not fatal — the window lives"

                            Expect.equal
                                diagnostic.Stage
                                DiagnosticStage.App
                                "an update/view fault is application code, not a draw failure"

                            Expect.notEqual
                                diagnostic.Stage
                                DiagnosticStage.FrameRender
                                "the whole point of #365: it is NOT mislabeled a render failure"

                            Expect.equal
                                diagnostic.Cause
                                (Some "product blew up")
                                "the underlying exception message survives into the cause"
            ...
            testList
                ...
                [
                    test "a succeeding step returns its result and reports nothing" {
                        ...
                        let result =
                            Viewer.tryProductStep (fun d -> reports <- d :: reports) "View" (fun () -> "scene")
                    ...
                    test "a throwing step is dropped and reported as an App-stage Error — never a Frame failure" {
                        ...
                        | [ diagnostic ] ->
                            Expect.equal
                                ...
                                "a product fault is an error, not a window teardown"

                            Expect.equal
                                diagnostic.Stage
                                (Some ViewerRunBlockedStage.App)
                                "an update/view fault is staged at App"

                            Expect.notEqual
                                diagnostic.Category
                                ViewerDiagnosticCategory.Frame
                                "it must not read as a render/frame failure"

                            Expect.stringContains
                                diagnostic.Message
                                "Update"
                                "the message names which product step raised"
                        | other -> failtestf "exactly one diagnostic should be reported, got %i" (List.length other)
                    }
            ...
            test "the two hosts agree: neither reuses the frame-render stage for a product fault" {
                ...
                Expect.equal
                    interactive.Stage
                    (Some ViewerRunBlockedStage.App)
                    "interactive host: App, not a render stage"
            }
        ]
