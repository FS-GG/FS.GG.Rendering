    ...
    testList
        ...
        [
            test "ordered discrete input replay reaches the same final state as immediate dispatch" {
                ...
                let _, q1 =
                    Viewer.enqueueInput receivedAt ViewerResponsivenessInputKind.KeyDown "A" Viewer.emptyInputQueue

                let _, q2 =
                    Viewer.enqueueInput
                        (receivedAt.AddMilliseconds 1.0)
                        ViewerResponsivenessInputKind.PointerDiscrete
                        "B"
                        q1

                let _, q3 =
                    Viewer.enqueueInput (receivedAt.AddMilliseconds 2.0) ViewerResponsivenessInputKind.KeyUp "C" q2
