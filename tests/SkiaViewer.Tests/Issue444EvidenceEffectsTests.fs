...
let tests =
    testList
        ...
        [
            ...
            test "CaptureScreenshot and CaptureImageEvidence each write a real PNG of the current scene" {
                for name, effect in
                    [
                        ...
                        "image", (fun p -> CaptureImageEvidence p)
                    ] do
            ...
            test
                "evidence rasterizes at the size each loop supplies — generated-app InitialSize vs interactive CurrentSize" {
                ...
                Expect.isEmpty
                    (sinkIntoAt generatedAppInitialSize (CaptureImageEvidence genPath))
                    "generated-app write succeeded"

                let interactiveDir = scratch "interactive-size"
                let interactivePath = Path.Combine(interactiveDir, "interactive.png")

                Expect.isEmpty
                    (sinkIntoAt interactiveCurrentSize (CaptureImageEvidence interactivePath))
                    "interactive write succeeded"
            ...
            test "the sink never throws, whatever the product asks it to write" {
                ...
                for effect in
                    [
                        ...
                        WriteRunEvidence(doomed, runEvidence)
                    ] do
