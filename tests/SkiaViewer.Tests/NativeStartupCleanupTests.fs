        ...
        [
            ...
            test "startup stage inventory covers every owned OpenGL and Skia resource category" {
                ...
                |> List.iter (fun category ->
                    Expect.contains stageResources category $"stage inventory contains {category}")
            ...
            test "injected acquisition failures Synthetic release acquired resources once in reverse order" {
                ...
                for stage in GlStartup.stages |> List.tail do
                    ...
                    Expect.equal
                        failure.ExpectedReleaseOrder
                        failure.ObservedReleaseOrder
                        $"release order is reversed for {stage.Name}"

                    Expect.equal
                        failure.DiagnosticStage
                        stage.DiagnosticStage
                        $"diagnostic stage is preserved for {stage.Name}"
            ...
            test "successful shutdown Synthetic releases all acquired resources once and repeated cleanup is idempotent" {
                ...
                Expect.equal
                    categories
                    (resourceCategories |> List.rev)
                    "successful shutdown releases resources in reverse acquisition order"

                let duplicates =
                    categories |> List.countBy id |> List.filter (fun (_, count) -> count <> 1)
