...
let runtimeWindowBehaviorTests =
    testList
        "Issue 1022 runtime window behavior"
        [
            test "the new host effect preserves the existing comparison contract" {
                let behavior =
                    native RuntimeWindowMode.Normal WindowBorder.Resizable (Some(100, 80)) (Some(1280, 720)) "windowed"
            ...
            test "windowed borderless and fullscreen requests all produce native plans" {
                ...
                |> List.iter (fun (requested, expectedMode, expectedToken) ->
                    ...
                    Expect.isFalse
                        (diagnostics
                         |> List.exists (fun diagnostic -> diagnostic.Level = ViewerDiagnosticLevel.Error))
            ...
            test "runtime lifecycle planning is deterministic behind the injected work-area seam" {
                ...
                Expect.equal
                    ...
                    "the requested lifecycle mode is preserved"
            ...
            test "native transitions are idempotent and restore prior windowed geometry" {
                ...
                let fullscreen =
                    native RuntimeWindowMode.Fullscreen WindowBorder.Hidden None None "fullscreen"

                let windowed =
                    native RuntimeWindowMode.Normal WindowBorder.Resizable None None "windowed"

                Expect.equal
                    (GlHost.applyRuntimeWindowBehavior controller (target fake) borderless)
                    (Ok true)
                    "borderless mutates the target"
                ...
                Expect.equal
                    (GlHost.applyRuntimeWindowBehavior controller (target fake) fullscreen)
                    (Ok true)
                    "fullscreen reaches WindowState.Fullscreen"

                Expect.equal fake.State WindowState.Fullscreen "exclusive fullscreen is active"

                Expect.equal
                    (GlHost.applyRuntimeWindowBehavior controller (target fake) windowed)
                    (Ok true)
                    "windowed exits fullscreen"
            ...
            test "borderless runtime transition uses the active output work area, not the planned default monitor" {
                ...
                Expect.equal
                    (GlHost.applyRuntimeWindowBehavior controller (target fake) plannedForDefault)
                    (Ok true)
                    "transition applies"
                ...
                let mapped =
                    Viewer.pointerInProductSpace (Some logical) postChangeSurface postChangeSurface native

                Expect.floatClose
                    Accuracy.high
                    mapped.X
                    960.0
                    "the emitted post-change surface drives inverse control X"

                Expect.floatClose
                    Accuracy.high
                    mapped.Y
                    540.0
                    "the emitted post-change surface drives inverse control Y"
            }
            ...
            test "post-change active-output surface drives both presentation fit and inverse pointer mapping" {
                ...
                let mapped =
                    Viewer.pointerInProductSpace (Some logical) activeSurface activeSurface native

                Expect.floatClose
                    Accuracy.high
                    mapped.X
                    logicalX
                    "the post-change presentation surface inverts to the control X"

                Expect.floatClose
                    Accuracy.high
                    mapped.Y
                    logicalY
                    "the post-change presentation surface inverts to the control Y"
            }
            ...
            test "native mutation failure restores the captured state and returns a Window diagnostic" {
                ...
                | Result.Error diagnostic ->
                    Expect.equal
                        ...
                        "failure is classified at the native window boundary"

                    Expect.stringContains
                        diagnostic.Message
                        "previous native-window state was restored"
