...
let tests =
    testList
        "Feature156 compositor timing support"
        [
            test "viewer timing path tokens are stable" {
                ...
                Expect.isTrue
                    (Viewer.timingPathCanSupportClaim ViewerTimingPath.DamageScoped false false)
                    "isolated path"

                Expect.isFalse
                    (Viewer.timingPathCanSupportClaim ViewerTimingPath.DamageScoped true false)
                    "proof readback limits claim"
            }
            ...
            test "CompositorProof timing overhead disclosure classifies limited measurements" {
                ...
                Expect.equal
                    (CompositorProof.timingPathToken CompositorProof.TimingPath.DamageScoped)
                    "damage-scoped"
                    "path"
            ...
            test "damage-scoped timing path still preserves full-redraw fallback boundary" {
                ...
                let ready =
                    GlHost.decideScissorRedraw CompositorProof.ProofReadiness.Ready false damage 100 100
                ...
                match fallback with
                | GlHost.ScissorDecision.FullRedraw reason ->
                    Expect.stringContains reason "missing display" "fallback reason"
