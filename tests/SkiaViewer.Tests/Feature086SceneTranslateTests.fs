...
let feature086SceneTranslateTests =
    testSequenced
    <| testList
        "Feature 086 Scene.translate / sizedText (US5)"
        [
                ...
                withRaster "nested translate additivity" (fun () ->
                    let nested =
                        Translate(
                            (20.0, 0.0),
                            {
                                Nodes = [ Translate((35.0, 0.0), { Nodes = [ trianglePath ] }) ]
                            }
                        )
                    ...
                    Expect.equal
                        (bbox (litPoints nestedPath))
                        (bbox (litPoints combinedPath))
                        "translate a then b == translate (a+b)")
            }
            ...
            test "sizedText renders larger glyphs at a larger explicit size; bare Text still renders (FR-014)" {
                withRaster "sizedText glyph extent" (fun () ->
                    ...
                    Expect.isGreaterThan
                        (width largePts)
                        (width smallPts)
                        "a larger explicit size yields wider glyph extent")
            }

            test "sizedText is structurally a SizedTextElement; translate wraps as TranslateElement" {
                Expect.contains
                    (Scene.describe (Scene.sizedText (10.0, 10.0) "hi" 14.0 Colors.white))
                    SizedTextElement
                    "sizedText describes as SizedTextElement"

                let wrapped =
                    Scene.translate 5.0 5.0 (Scene.rectangle (0.0, 0.0, 4.0, 4.0) Colors.white)
