    ...
    testList
        ...
        [
            ...
            test "toPhysicalPoint is the identity at scale 1 (window == framebuffer)" {
                ...
                Expect.equal
                    (LogicalCanvas.toPhysicalPoint (size 800 600) (size 800 600) 123.0 77.0)
                    (123.0, 77.0)
                    "no rescale when the framebuffer equals the window"
            }
            ...
            test "toPhysicalPoint scales a pointer by an integer HiDPI ratio" {
                ...
                Expect.equal
                    (LogicalCanvas.toPhysicalPoint (size 800 600) (size 1600 1200) 400.0 300.0)
                    (800.0, 600.0)
                    "a 2x framebuffer doubles the pointer coordinate"
            }
            ...
            test "toPhysicalPoint scales each axis independently under non-square DPI" {
                ...
                Expect.equal
                    (LogicalCanvas.toPhysicalPoint (size 100 100) (size 300 200) 10.0 10.0)
                    (30.0, 20.0)
                    "each axis uses its own physical/logical ratio"
            }

            test "toPhysicalPoint is inert on a degenerate size rather than dividing by zero" {
                Expect.equal
                    (LogicalCanvas.toPhysicalPoint (size 0 600) (size 1600 1200) 5.0 5.0)
                    (5.0, 5.0)
                    "a zero window width yields the input unchanged"

                Expect.equal
                    (LogicalCanvas.toPhysicalPoint (size 800 600) (size 1600 0) 5.0 5.0)
                    (5.0, 5.0)
                    "a zero framebuffer height yields the input unchanged"
            }
            ...
            test "without a LogicalSize the product is handed the PHYSICAL framebuffer (native resolution)" {
                ...
                Expect.equal
                    (nativeViewSize None (size 1600 1200))
                    (size 1600 1200)
                    "a size-aware product draws at native framebuffer resolution"
            }
            ...
            test "a LogicalSize product still renders in its fixed logical canvas, not the framebuffer" {
                ...
                Expect.equal
                    (nativeViewSize (Some(size 320 240)) (size 1600 1200))
                    (size 320 240)
                    "a fixed-resolution product keeps its logical canvas"
            }
                ...
                for scale in [ 1; 2; 3 ] do
                    ...
                    for x, y in [ 0.0, 0.0; 400.0, 300.0; 733.0, 111.0; 800.0, 600.0 ] do
                        ...
                        Expect.floatClose
                            ...
                            (fst native)
                            (fst pre400)
                            $"x invariant at scale {scale} for ({x},{y})"

                        Expect.floatClose
                            Accuracy.high
                            (snd native)
                            (snd pre400)
                            $"y invariant at scale {scale} for ({x},{y})"
            }
            ...
            test "a scale-1 readback authoring size equals the target, so the fit is the identity (no regression)" {
                ...
                Expect.equal
                    (LogicalCanvas.fit fb fb)
                    {
                        Scale = 1.0
                        OffsetX = 0.0
                        OffsetY = 0.0
                    }
                    "at scale 1 the readback is byte-identical to a 1:1 draw"
            }
        ]
