    ...
    testList
        ...
        [
            test "JSONL line carries required fields and stable tokens" {
                ...
                Expect.isTrue
                    (root.GetProperty("phaseTiming").TryGetProperty("totalInputToVisibleMs", &total))
                    "phase timing is present"
            }

            test "stable token helpers cover readiness vocabulary" {
                Expect.equal
                    (Viewer.responsivenessReadinessToken ViewerResponsivenessReadiness.Accepted)
                    "accepted"
                    "accepted token"
                ...
                Expect.equal
                    (Viewer.responsivenessReadinessToken ViewerResponsivenessReadiness.EnvironmentLimited)
                    "environment-limited"
                    "environment-limited token"
            }
        ]
