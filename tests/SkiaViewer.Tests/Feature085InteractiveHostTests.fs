            ...
            fun _size _model ->
                Stack.create
                    [
                        Stack.children
                            [
                                Button.create [ Button.text "Go"; Button.onClick Increment ]
                                |> Control.withKey "go"
                            ]
                    ]
        Theme = Theme.light
...
let interactiveHostTests =
    testList
        "Feature 085 interactive pointer host (US2)"
        [
            ...
            test "a press+release outside any control routes no msg (pure transition, FR-004)" {
                ...
                let state1, downMsgs =
                    ControlsElmish.routeInteractivePointer
                        ...
                        size
                        model0
                        (pointer ViewerPointerPhaseKind.Pressed 5.0 5.0)
                ...
                let _state2, upMsgs =
                    ControlsElmish.routeInteractivePointer
                        ...
                        size
                        model0
                        (pointer ViewerPointerPhaseKind.Released 5.0 5.0)
            ...
            test "size-aware View lays out to the actual extent at two sizes (US4, FR-009)" {
                ...
                let renderAt (sz: Size) =
                    Control.renderTree Theme.light sz (sizeAwareView sz { Count = 0 })
                ...
                Expect.notEqual
                    small.Scene
                    large.Scene
                    "size-aware View lays out differently at two extents (no fixed-size upscale)"
                ...
                let viewerHost: InteractiveViewerHost<Model, Msg> =
                    {
                        ...
                        View =
                            fun sz m ->
                                SceneNode.Group [ (Control.renderTree Theme.light sz (sizeAwareView sz m)).Scene ]
