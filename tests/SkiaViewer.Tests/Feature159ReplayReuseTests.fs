    ...
    testList
        ...
        [
            ...
            test "Synthetic content change re-records and disabled or unsafe replay fails closed" {
                ...
                Expect.equal
                    (PictureReplayCache.classifySplitReplay (Some prior) changed).Status
                    "content-re-recorded"
                    "content change records"

                Expect.equal
                    (PictureReplayCache.classifySplitReplay (Some prior) disabled).FallbackReason
                    (Some PictureReplayCache.DisabledReplay)
                    "disabled replay"

                Expect.equal
                    (PictureReplayCache.classifySplitReplay (Some prior) parity).FallbackReason
                    (Some PictureReplayCache.ParityMismatch)
                    "parity mismatch"
            }
        ]
