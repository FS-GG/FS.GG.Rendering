...
let tests =
    testList
        "Feature173 readiness rules"
        [
            test "stable readiness tokens include rejected" {
                Expect.equal
                    (Viewer.responsivenessReadinessToken ViewerResponsivenessReadiness.Rejected)
                    "rejected"
                    "rejected token"
            }
            ...
            test "measured max latency over budget rejects readiness" {
                ...
                Expect.equal
                    ...
                    "measured max budget miss is rejected"

                Expect.equal
                    (summary.FirstFailedBudget |> Option.map _.Kind)
                    (Some "input-to-visible-max")
                    "max budget is named"
            }
            ...
            test "environment boundary failure is ordered before timing budgets" {
                ...
                Expect.equal
                    ...
                    "environment-limited dominates budget rejection"

                Expect.equal
                    (summary.FirstFailedBudget |> Option.map _.Kind)
                    (Some "environment-boundary")
                    "environment failure is first"
            }
            ...
            test "slowest interaction reporting keeps five measured records" {
                let records =
                    ...
                    |> List.mapi (fun index total ->
                        Feature167SchedulerFixtures.latency
                            (index + 1)
                ...
                Expect.equal
                    summary.SlowestInteractions.Head.TotalInputToVisible
                    (Some(TimeSpan.FromMilliseconds 6.0))
                    "slowest is first"
            }
        ]
