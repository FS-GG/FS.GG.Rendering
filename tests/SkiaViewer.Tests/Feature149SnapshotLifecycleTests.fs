...
let tests =
    testList
        "Feature149 snapshot lifecycle host policy"
        [
            ...
            test "environment-limited and failed proof tokens stay distinct for lifecycle fallback diagnostics" {
                Expect.equal
                    (CompositorProof.readinessToken (
                        CompositorProof.ProofReadiness.EnvironmentLimited "unsupported-host"
                    ))
                    "environment-limited"
                    "limited"

                Expect.equal
                    (CompositorProof.readinessToken (CompositorProof.ProofReadiness.Failed "invalid-resource"))
                    "failed"
                    "failed"
            }
        ]
