...
let tests =
    testList
        "Feature153 live proof host classification"
        [
            test "capable host facts classify as capable and produce a matching profile" {
                Expect.equal
                    (GlHost.classifyLiveProofHost capableFacts)
                    GlHost.LiveProofHostReadiness.Capable
                    "capable host"
            ...
            test "missing display, renderer, readback, permission, and timeout are non-capable" {
                [
                    ...
                    GlHost.LiveProofHostReadiness.MissingDisplay
                    { capableFacts with Renderer = None }, GlHost.LiveProofHostReadiness.MissingRenderer
                    { capableFacts with
                        ReadbackAvailable = false
                    },
                    GlHost.LiveProofHostReadiness.ReadbackUnavailable
                    { capableFacts with
                        PermissionGranted = false
                    },
                    GlHost.LiveProofHostReadiness.PermissionDenied
            ...
            test "viewer host exposes whether the program can carry live proof interpreter effects" {
                ...
                Expect.isTrue
                    (FS.GG.UI.SkiaViewer.Host.Viewer.liveProofInterpreterSupported program)
                    "positive-sized viewer program can host proof effects"
            }
        ]
