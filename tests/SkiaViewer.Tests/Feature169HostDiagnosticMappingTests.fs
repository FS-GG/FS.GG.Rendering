...
let tests =
    testList
        "Feature169 SkiaViewer host diagnostic mapping"
        [
            test "Synthetic damage scoped host diagnostic maps to backend-cost informational" {
                let host =
                    Diagnostics.damageScopedDecision "offscreen-fallback" (Some "readback required")
            ...
            test "F-DIAG-1: a fatal framebuffer startup failure maps to readiness blocker, not rendering limitation" {
                ...
                Expect.equal
                    runtime.Severity
                    (Some FS.GG.UI.Diagnostics.DiagnosticSeverity.Error)
                    "fatal collapses to Error severity"

                Expect.equal
                    runtime.Category
                    (Some DiagnosticCategory.ReadinessBlocker)
                    "fatal framebuffer failure blocks readiness"
            }
            ...
            test "F-DIAG-1: a benign informational framebuffer diagnostic stays a rendering limitation" {
                ...
                let host =
                    Diagnostics.create
                        ...
                        DiagnosticStage.Framebuffer
                        "present-mode=DirectToSwapchain readback=false."
                        None
                ...
                Expect.equal
                    ...
                    "informational framebuffer note is not a blocker"
            }
        ]
