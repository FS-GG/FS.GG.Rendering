...
let tests =
    testList
        "Feature154 damage-scoped parity gate"
        [
            test "same-profile accepted proof set is prerequisite for parity acceptance" {
                ...
                let readiness =
                    CompositorProof.evaluateProofSet
                        ...
                        [
                            attempt "run-1" profile now
                            attempt "run-2" profile now
                            attempt "run-3" profile now
                        ]
            ...
            test "cross-profile or stale proof keeps parity fallback-gated" {
                ...
                let other =
                    { profile with
                        ProfileId = "feature154-other-profile"
                    }
