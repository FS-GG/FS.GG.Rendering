...
let tests =
    testList
        "Feature149 live proof readiness"
        [
            test "fresh matching accepted proof is ready and deterministic" {
                ...
                Expect.equal
                    (CompositorProof.readiness profile now (TimeSpan.FromHours 1.0) (Some proof))
            ...
            test "stale proof, host mismatch, and algorithm mismatch reject readiness" {
                ...
                let stale =
                    { passedProof DateTimeOffset.UnixEpoch with
                        CreatedAt = DateTimeOffset.UnixEpoch
                    }

                let hostMismatch =
                    { passedProof now with
                        HostProfile =
                            { profile with
                                ProfileId = "other-host"
                            }
                    }

                let algorithmMismatch =
                    { passedProof now with
                        HostProfile =
                            { profile with
                                ProofAlgorithmVersion = "old"
                            }
                    }

                Expect.equal
                    (CompositorProof.readiness profile now (TimeSpan.FromMinutes 5.0) (Some stale))
                    CompositorProof.ProofReadiness.Stale
                    "stale"

                Expect.equal
                    (CompositorProof.readiness profile now (TimeSpan.FromMinutes 5.0) (Some hostMismatch))
                    CompositorProof.ProofReadiness.HostMismatch
                    "host mismatch"

                Expect.equal
                    (CompositorProof.readiness profile now (TimeSpan.FromMinutes 5.0) (Some algorithmMismatch))
                    CompositorProof.ProofReadiness.HostMismatch
                    "algorithm mismatch"
            }
