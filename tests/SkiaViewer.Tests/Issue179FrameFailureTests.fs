                ...
                [
                    ...
                    test "a transient failure is retried up to the budget, then torn down" {
                        ...
                        for attempt in 1..budget do
                            match
                                GlHost.decideFrameFailure GlHost.FrameFailureKind.TransientDrawFailure attempt budget
                            with
                        ...
                        match
                            GlHost.decideFrameFailure GlHost.FrameFailureKind.TransientDrawFailure (budget + 1) budget
                        with
                        | GlHost.FrameFailureAction.TeardownRun _ -> ()
                    ...
                    test "the retry budget is bounded and positive" {
                        Expect.isGreaterThan
                            GlHost.transientFrameRetryBudget
                            0
                            "a zero budget would tear down on one hiccup"

                        Expect.isLessThan
                            GlHost.transientFrameRetryBudget
                            60
                            "a budget near a frame's worth of retries is a spin"
                    }
                ]
            ...
            testList
                ...
                [
                    ...
                    test "a recovered frame clears the streak, so a flaky device keeps rendering" {
                        ...
                        Expect.isNone
                            (driveFrameLoop healthy flaky)
                            "an intermittent failure recovers rather than tearing down"
                    }
                    ...
                    test "the streak resumes after recovery — a device that fails, recovers, then dies is torn down" {
                        ...
                        | Some(_, index) ->
                            Expect.equal
                                index
                                (budget + 1 + budget)
                                "teardown lands after the second streak exhausts the budget"
                    }
                ]

            test "the fatal diagnostic that ends a run is Fatal, staged at the frame render" {
                let fatal =
                    Diagnostics.frameLoopAbandoned "The OpenGL device was lost." (Some "GL_GUILTY_CONTEXT_RESET")
                ...
                Expect.equal
                    fatal.Cause
                    (Some "GL_GUILTY_CONTEXT_RESET")
                    "the underlying detail survives into the fatal diagnostic"
            }
        ]
