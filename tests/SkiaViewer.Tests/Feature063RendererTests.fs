...
let tests =
    testSequenced
    <| testList
        "Feature063 image-evidence renderer"
        [
                ...
                withRaster "Line/Path pixel rendering" (fun () ->
                    ...
                    let terrain =
                        Scene.line
                            { X = 20.0; Y = 180.0 }
                            { X = 300.0; Y = 190.0 }
                            (Paint.stroke (Colors.rgb 210uy 210uy 70uy) 3.0)
                    ...
                    let rightOfPlaceholder =
                        litPixels lit
                        |> Seq.filter (fun (x, _) -> x > placeholderRightEdge + 60)
                        |> Seq.length
            ...
            test "node count is structural; the image is the visual proof on a Line-only scene (SC-002)" {
                withRaster "Line-only visual proof" (fun () ->
                    let line =
                        Scene.line { X = 20.0; Y = 180.0 } { X = 300.0; Y = 185.0 } (Paint.stroke Colors.white 2.0)
