...
let tests =
    testList
        "Feature167 input queue"
        [
            test "discrete inputs preserve receipt order and sequence ids" {
                let a, q1 =
                    Feature167SchedulerFixtures.enqueue
                        ViewerResponsivenessInputKind.KeyDown
                        "Enter"
                        Viewer.emptyInputQueue

                let b, q2 =
                    Feature167SchedulerFixtures.enqueue ViewerResponsivenessInputKind.PointerDiscrete "click" q1
                ...
                Expect.equal
                    (drain.DiscreteInputs |> List.map _.Payload)
                    [ "Enter"; "click" ]
                    "discrete order is stable"
            }
            ...
            test "continuous pointer moves coalesce without removing discrete input" {
                ...
                let latest, q2 =
                    Feature167SchedulerFixtures.enqueue ViewerResponsivenessInputKind.PointerMove "move-2" q1

                let _, q3 =
                    Feature167SchedulerFixtures.enqueue ViewerResponsivenessInputKind.KeyDown "Enter" q2
            ...
            test
                "public frame-paced policy folds a 1000 Hz move stream to 60 updates and keeps press release click lossless" {
                let movesPerFrame =
                    [ for frame in 0..59 -> 1000 / 60 + (if frame < 1000 % 60 then 1 else 0) ]
