    ...
    testList
        ...
        [
            ...
            test "default present mode is DirectToSwapchain (readback-free GL default)" {
                let configuration =
                    Viewer.defaultConfiguration "Product" { Width = 640; Height = 480 }

                Expect.equal
                    configuration.PresentMode
                    ViewerPresentMode.DirectToSwapchain
                    "GL host defaults to DirectToSwapchain"
            }
            ...
            test "present modes are a distinct closed two-case DU" {
                Expect.notEqual
                    ...
                    "the two present modes are distinct"
            }
            ...
            test "reconciled GL diagnostic DUs exist and are distinct" {
                Expect.notEqual
                    ...
                    "OpenGl category is distinct from Renderer"

                Expect.notEqual
                    ViewerDiagnosticCategory.Framebuffer
                    ViewerDiagnosticCategory.Renderer
                    "Framebuffer category is distinct from Renderer"

                Expect.notEqual
                    ViewerRunBlockedStage.GlContext
                    ViewerRunBlockedStage.Renderer
                    "GlContext stage is distinct from Renderer"
            }
            ...
            test "GL startup ledger covers every owned resource category and releases in reverse order" {
                ...
                categories
                |> List.iter (fun c -> Expect.contains stageResources c $"startup inventory contains {c}")
                ...
                Expect.equal
                    releases
                    (categories |> List.rev)
                    "successful shutdown releases in reverse acquisition order"
            }
            ...
            test "GL-unavailable diagnostic is classified honestly with no false fallback" {
                ...
                Expect.stringContains
                    ...
                    "diagnostic states there is no fallback renderer"
