    ...
    <| testList
        ...
        [
            test "driving a click script changes the model, and the resulting frame is captured (or disclosed)" {
                ...
                let postScene =
                    SceneNode.Group [ (Control.renderTree theme size (view size finalModel)).Scene ]

                let shot =
                    try
                        Result.Ok(capture outPath postScene)
                    with ex ->
                ...
                | Result.Error message -> failtestf "capture raised instead of degrading-and-disclosing: %s" message
            }
        ]
