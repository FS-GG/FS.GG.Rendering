...
let tests =
    testList
        "Feature153 live proof interpreter"
        [
            test "MVU transition requests sentinel damage and artifact write effects" {
                ...
                Expect.equal
                    model0.Phase
                    CompositorProof.ProofPhase.DetectingProfile
                    "init starts by detecting host profile"

                Expect.contains effects0 CompositorProof.DetectProfile "init requests profile detection"

                let model1, effects1 =
                    CompositorProof.update now "proof.md" (CompositorProof.ProfileDetected profile) model0

                Expect.equal
                    model1.Phase
                    CompositorProof.ProofPhase.PresentingSentinel
                    "profile detection advances to sentinel"

                Expect.exists
                    effects1
                    (function
                    | CompositorProof.PresentSentinelFrame _ -> true
                    | _ -> false)
                    "sentinel frame requested"

                let model2, effects2 =
                    CompositorProof.update now "proof.md" CompositorProof.SentinelPresented model1
                ...
                Expect.exists
                    effects2
                    (function
                    | CompositorProof.PresentDamageFrame _ -> true
                    | _ -> false)
                    "damage frame requested"

                let model3, effects3 =
                    CompositorProof.update now "proof.md" CompositorProof.DamagePresented model2
                ...
                let model4, effects4 =
                    CompositorProof.update now "proof.md" (CompositorProof.ObservationCompleted observations) model3
                ...
                Expect.exists
                    effects4
                    (function
                    | CompositorProof.WriteProofArtifact("proof.md", _) -> true
                    | _ -> false)
                    "artifact write is emitted"
            }
            ...
            test "exactly three selected attempts are recorded when four accepted attempts are available" {
                ...
                | CompositorProof.ProofSetReadiness.Accepted proofSet ->
                    Expect.sequenceEqual
                        proofSet.SelectedAttemptIds
                        [ "run-1"; "run-2"; "run-3" ]
                        "selected trio is explicit"
            ...
            test "failed extra attempt prevents a selected trio from hiding bad evidence" {
                ...
                let bad =
                    { attempt "run-4" now with
                        Proof =
                            proof
                                "run-4"
                                (CompositorProof.PresentProofFailed
                                    CompositorProof.PresentProofFailureCause.ClearedPixels)
                                now
                    }
                ...
                let readiness =
                    CompositorProof.evaluateProofSet
                        ...
                        now
                        (TimeSpan.FromHours 1.0)
                        [ attempt "run-1" now; attempt "run-2" now; attempt "run-3" now; bad ]

                Expect.equal
                    (CompositorProof.proofSetReadinessToken readiness)
                    "failed"
                    "bad evidence in the set fails closed"
            }
        ]
