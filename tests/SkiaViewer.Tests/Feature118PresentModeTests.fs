...
let tests =
    testList
        "Feature 118 present-mode contract"
        [
            test "default ViewerOptions present mode is OffscreenReadback (SC-001 byte-identity default)" {
                ...
                Expect.equal
                    ...
                    "default present mode must be OffscreenReadback"
            }
            ...
            test "defaultConfiguration carries DirectToSwapchain (feature 119 GL readback-free default)" {
                ...
                Expect.equal
                    ...
                    "defaultConfiguration defaults to the GL readback-free DirectToSwapchain present path"
            }
            ...
            test "ViewerConfiguration.PresentMode mirrors the supplied ViewerOptions.PresentMode (config threading)" {
                ...
                Expect.equal
                    direct.PresentMode
                    ViewerPresentMode.DirectToSwapchain
                    "DirectToSwapchain must thread through"

                Expect.equal
                    offscreen.PresentMode
                    ViewerPresentMode.OffscreenReadback
                    "OffscreenReadback must thread through"
            }
            ...
            test "present modes are distinct (closed two-case DU)" {
                Expect.notEqual
                    ViewerPresentMode.OffscreenReadback
                    ViewerPresentMode.DirectToSwapchain
                    "the two present modes must be distinct"
            }
            ...
            test "FR-007 diagnostic categories Framebuffer/Frame exist and are distinct from Renderer" {
                ...
                Expect.notEqual
                    ...
                    "Framebuffer must differ from Renderer"

                Expect.notEqual
                    ViewerDiagnosticCategory.Frame
                    ViewerDiagnosticCategory.Renderer
                    "Frame must differ from Renderer"

                Expect.notEqual
                    ViewerDiagnosticCategory.Framebuffer
                    ViewerDiagnosticCategory.Frame
                    "Framebuffer must differ from Frame"
            }
        ]
