...
let emit effect : Cmd<InteractiveMsg> = Cmd.ofMsg (Interpret effect)

let batchEffects effects : Cmd<InteractiveMsg> = effects |> List.map emit |> Cmd.batch

let updateInteractive msg model =
    match msg with
    | Start -> { model with Initialized = false }, emit InitializeRenderer
    | RendererInitialized ->
    ...
    | ViewerInput event ->
        match event with
        | Loaded ->
            { model with Initialized = true }, emit (RenderFrame(interactiveView { model with Initialized = true }))
        | UpdateTick _ -> model, emit (Dispatch SubscriptionTick)
        | RenderTick _ -> model, emit (RenderFrame(interactiveView model))
        | ViewerEvent.KeyDown key ->
            { model with
                PressedKeys = key :: (model.PressedKeys |> List.filter ((<>) key))
            },
            emit (Dispatch FrameRequested)
        | ViewerEvent.KeyUp key ->
            { model with
                PressedKeys = model.PressedKeys |> List.filter ((<>) key)
            },
            Cmd.none
        | PointerMoved(x, y) -> { model with Pointer = Some(x, y) }, emit (Dispatch FrameRequested)
        ...
        | FramebufferResized _ ->
            ...
            model, Cmd.none
        | FramePresented -> model, emit (Dispatch FrameRendered)
        | CloseRequested -> { model with Closing = true }, emit Shutdown
        | DiagnosticReported diagnostic ->
            { model with
                LastDiagnostic = Some diagnostic
            },
            emit (ReportDiagnostic diagnostic)
    | FrameRequested -> model, emit (RenderFrame(interactiveView model))
    | FrameRendered ->
        { model with
            FrameCount = model.FrameCount + 1
        },
        Cmd.none
    | ScreenshotRequested request -> model, emit (CaptureScreenshot request)
    | ScreenshotCaptured ->
        { model with
            ScreenshotCount = model.ScreenshotCount + 1
        },
        Cmd.none
    | SubscriptionTick ->
        let next =
...
let readinessPath segments =
    let historicalFeature =
        Path.Combine(repositoryRoot, "specs", "002-skia-feature-parity")
    ...
    testList
        ...
        [
            test "configuration can be constructed without renderer selection" {
                let config = Viewer.defaultConfiguration "contract" { Width = 800; Height = 600 }
            ...
            test "viewer create preserves pure Elmish functions and default subscriptions" {
                let config = Viewer.defaultConfiguration "counter" { Width = 320; Height = 240 }
            ...
            test "viewer withEventMapping maps viewer events to application messages" {
                let config = Viewer.defaultConfiguration "events" { Width = 320; Height = 240 }
                ...
                Expect.equal
                    (program.EventMapper(ViewerEvent.KeyDown "Add"))
                    (Some Increment)
                    "matching viewer event maps to app message"
            ...
            test "viewer withEffectMapping identifies messages for host-side effect interpretation" {
                let config = Viewer.defaultConfiguration "effects" initialInteractiveModel.Size
                ...
                Expect.equal
                    (program.EffectMapper SubscriptionTick)
                    None
                    "ordinary application messages are handled by update"
            }
        ]
    ...
    testList
        ...
        [
            ...
            test "invalid size fails before Vulkan startup" {
                let config = Viewer.defaultConfiguration "invalid" { Width = 0; Height = 240 }
            ...
            test "frame render diagnostics identify OpenGL path without fallback switching" {
                ...
                Expect.stringContains
                    diagnostic.Message
                    "OpenGL/Skia frame rendering failed"
                    "diagnostic identifies the frame path"

                Expect.stringContains
                    diagnostic.Message
                    "no fallback renderer"
                    "diagnostic rules out fallback switching"
...
let us1ContractTests =
    testList
        "US1 Vulkan-only viewer contract"
        [
            test
                "primitive group image arc point vertices picture and nested scene constructors are semantic through public API" {
                let child =
                    Scene.group
                        [
            ...
            test "path-effect declarations expose lean scene-vocabulary diagnostics" {
                ...
                Expect.exists
                    diagnostics
                    (fun d -> d.Message.Contains "path effect")
                    "path-effect diagnostic is structured"
            }
            ...
            test "invalid image resources report frame diagnostics" {
                ...
                let malformedPath = Path.create Winding [ Path.lineTo 10.0 10.0; Path.close ]
                ...
                Expect.exists
                    diagnostics
                    (fun d -> d.Message.Contains "Invalid image resource")
                    "an empty image source is diagnosed"
            }
            ...
            test "path commands fill types boolean operations measurement segment extraction and helpers are semantic" {
                ...
                Expect.equal
                    combined.FillType
                    Winding
                    "Union composes overlapping subpaths under the nonzero-winding rule"
                ...
                for op in [ Intersect; Difference ] do
                    ...
                    | Result.Ok _ ->
                        failtestf "%A must fail loud in the Skia-free Scene layer, not return silent geometry" op
                    | Result.Error error ->
                        Expect.equal error.Operation op $"{op} failure names the unsupported operation"
                ...
                Expect.contains
                    (Scene.describe (Scene.path combined samplePaint))
                    PathElement
                    "path scene is constructible"
            }

            test "clipping region text font text-run picture color-space and perspective declarations are semantic" {
                let clipPath =
                    Path.create Winding [ Path.moveTo 0.0 0.0; Path.lineTo 20.0 0.0; Path.lineTo 20.0 20.0; Path.close ]
                        ...
                        [
                            ...
                            Scene.clipped
                                ...
                                (Scene.picture
                                    {
                                        Name = "text"
                                        Scene = Scene.text (2.0, 16.0) "picture" blue
                                    })
            ...
            test "drawing parity gallery render-readback evidence covers at least sixty public visual capabilities" {
                let paints =
                    [
                        ...
                        Paint.fill red
                        |> Paint.withShader (
                            LinearGradient({ X = 0.0; Y = 0.0 }, { X = 20.0; Y = 20.0 }, [ red; blue ])
                        )
                        Paint.fill red
                        |> Paint.withShader (RadialGradient({ X = 10.0; Y = 10.0 }, 8.0, [ red; green ]))
                        Paint.fill red
                        |> Paint.withShader (SweepGradient({ X = 10.0; Y = 10.0 }, [ blue; green ]))
                        Paint.fill red |> Paint.withColorFilter (BlendColor(blue, Multiply))
                        Paint.fill red |> Paint.withMaskFilter (Blur 2.0)
                        Paint.fill red
                        |> Paint.withImageFilter (DropShadow(2.0, 2.0, 2.0, Colors.black))
                ...
                let capabilityLines =
                    ...
                    |> List.mapi (fun index visual ->
                        let description = Scene.describe visual |> List.map string |> String.concat ","
                    ...
                    writeReadinessEvidence
                        ...
                        (String.concat
                            ...
                            ([
                                ...
                                $"semantic-capabilities={visualScenes.Length}"
                             ]
                             @ capabilityLines))
            ...
            test "minimal Elmish viewer program produces a scene and render effect" {
                let config = Viewer.defaultConfiguration "minimal" { Width = 640; Height = 480 }
...
let us2DiagnosticTests =
    testList
        "US2 Vulkan-unavailable diagnostics"
        [
            ...
            test "opengl unavailable diagnostic identifies OpenGL without fallback language" {
                ...
                Expect.stringContains
                    diagnostic.Message
                    "no fallback renderer"
                    "message states no fallback renderer is used"
                ...
                |> List.iter2
                    (fun (stage, cause) diagnostic ->
                        ...
                        Expect.stringContains
                            ...
                            "diagnostic names OpenGL initialization"

                        Expect.stringContains
                            diagnostic.Message
                            "no fallback renderer"
                            "diagnostic rules out fallback rendering")
                    simulatedFailures
            }
        ]
...
let us3ElmishFlowTests =
    testList
        "US3 Elmish-driven viewer flow"
        [
            test
                "application update handles keyboard pointer resize close lifecycle diagnostic frame screenshot and subscription messages" {
                let diagnostic = Diagnostics.frameRenderFailed "present failed"

                let screenshot =
                    {
                        Destination = "interactive.png"
                        Format = Png
                    }
                ...
                let afterStart, _ = updateInteractive Start initialInteractiveModel

                let afterInitialized, _ = updateInteractive RendererInitialized afterStart

                let afterLoaded, _ = updateInteractive (ViewerInput Loaded) initialInteractiveModel

                let afterDiagnostic, _ =
                    updateInteractive (ViewerInput(DiagnosticReported diagnostic)) initialInteractiveModel

                let afterFrame, _ = updateInteractive FrameRendered initialInteractiveModel
                ...
                let afterTick, _ = updateInteractive SubscriptionTick initialInteractiveModel
                ...
                Expect.equal
                    afterScreenshotRequest.ScreenshotCount
                    0
                    "screenshot request does not mutate completion count"
            ...
            test "update emits initialize render screenshot shutdown diagnostic and dispatch effects" {
                ...
                | CaptureScreenshot request ->
                    Expect.equal request screenshot "screenshot effect carries destination and format"
                ...
                match
                    ...
                    |> onlyEffect
                with
                | ReportDiagnostic reported ->
                    Expect.equal reported diagnostic "diagnostic report effect carries structured diagnostic data"
                | effect -> failtestf "expected ReportDiagnostic, got %A" effect

                match
                    updateInteractive (ViewerInput(ViewerEvent.KeyDown "Enter")) initialInteractiveModel
                    |> snd
                    |> onlyEffect
                with
            ...
            test "timer subscription dispatches messages without direct mutable scene pushes" {
                ...
                Expect.equal
                    ...
                    [ SubscriptionTick ]
                    "subscription communicates only through Elmish dispatch"

                let next, cmd = updateInteractive dispatched[0] initialInteractiveModel
...
let us4SampleAndScreenshotTests =
    testList
        "US4 complete Elmish viewer examples"
        [
            test "BasicViewer contract smoke compiles and exercises scene and screenshot command" {
                ...
                let project =
                    Path.Combine(repositoryRoot, "samples", "BasicViewer", "BasicViewer.fsproj")

                if File.Exists project then
                    let exitCode, stdout, stderr =
                        sampleSmokeResult
                            "BasicViewer"
                            "run --project samples/BasicViewer/BasicViewer.fsproj --no-build --no-restore -- --contract-smoke"
                    ...
                    skiptest
                        "SKIPPED(Stage R4 import): samples/BasicViewer not imported — the BasicViewer contract smoke needs the sample project; recorded skipped, not vacuously passed (SKIPPED-TESTS.md). Self-restores once the sample lands."
            }

            test "InteractiveViewer contract smoke compiles and exercises input state and screenshot command" {
                let project =
                    Path.Combine(repositoryRoot, "samples", "InteractiveViewer", "InteractiveViewer.fsproj")

                if File.Exists project then
                    let exitCode, stdout, stderr =
                        sampleSmokeResult
                            "InteractiveViewer"
                            "run --project samples/InteractiveViewer/InteractiveViewer.fsproj --no-build --no-restore -- --contract-smoke"
                    ...
                    Expect.stringContains
                        stdout
                        "initialize-effect=true"
                        "sample requests renderer initialization through Elmish effect mapping"

                    Expect.stringContains
                        stdout
                        "screenshot-format=Jpeg"
                        "interactive sample requests JPEG screenshot capture"
                else
                    skiptest
                        "SKIPPED(Stage R4 import): samples/InteractiveViewer not imported — the InteractiveViewer contract smoke needs the sample project; recorded skipped, not vacuously passed (SKIPPED-TESTS.md). Self-restores once the sample lands."
            }

            test "ScreenshotGallery contract smoke exercises screenshots diagnostics recovery and shutdown effects" {
                let project =
                    Path.Combine(repositoryRoot, "samples", "ScreenshotGallery", "ScreenshotGallery.fsproj")

                if File.Exists project then
                    let exitCode, stdout, stderr =
                        sampleSmokeResult
                            "ScreenshotGallery"
                            "run --project samples/ScreenshotGallery/ScreenshotGallery.fsproj --no-build --no-restore -- --contract-smoke"
                    ...
                    Expect.stringContains
                        stdout
                        "screenshot-effect=true"
                        "sample requests screenshot capture through Elmish effect mapping"

                    Expect.stringContains
                        stdout
                        "recovery-diagnostic-effect=true"
                        "sample reports recoverable frame diagnostics"

                    Expect.stringContains
                        stdout
                        "shutdown-effect=true"
                        "sample shuts down through Elmish effect mapping"
                else
                    skiptest
                        "SKIPPED(Stage R4 import): samples/ScreenshotGallery not imported — the ScreenshotGallery contract smoke needs the sample project; recorded skipped, not vacuously passed (SKIPPED-TESTS.md). Self-restores once the sample lands."
            }
            ...
            test "screenshot diagnostics describe capture before a successful frame" {
                ...
                Expect.stringContains
                    ...
                    "message identifies screenshot failure"

                Expect.stringContains
                    (diagnostic.Cause |> Option.defaultValue "")
                    "before the first successful OpenGL/Skia frame"
                    "cause identifies missing frame"
            }
            ...
            test "interactive update emits screenshot effect through public Elmish boundary" {
                ...
                with
                | CaptureScreenshot request ->
                    Expect.equal request screenshot "screenshot request is carried by public effect"
                | effect -> failtestf "expected CaptureScreenshot, got %A" effect
            }
        ]
