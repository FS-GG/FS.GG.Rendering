...
let run () =
    ...
    | Ok effective, Ok decoded when decoded = profile ->
        ...
        let released, releaseEffects =
            CommandResolver.update catalog
                (CommandResolverObservation.InputEvent { input with Id = "portable-release"; Phase = InputGesturePhase.Released })
                pressed
        let effectToken = function
        ...
        printfn "resolver-state profile=%s contexts=%d modals=%d capture=%b pending=%b owned=%d composing=%b disposed=%b"
            released.Profile.Id released.ActiveContexts.Length released.ModalStack.Length
            released.CaptureRestoreFocus.IsSome released.PendingSequence.IsSome released.OwnedPresses.Length
            released.IsComposing released.IsDisposed

        let command id trigger =
            { descriptor with Id = id; Label = id; Trigger = trigger }
        ...
        let modelProfile =
            { Id = "model-correspondence"
              Bindings =
                [ { Context = context.Id; Command = terminal.Id; Gesture = InputGesture.KeyChord k }
                  { Context = context.Id; Command = continuation.Id; Gesture = InputGesture.KeySequence [ k; c ] }
                  { Context = context.Id; Command = held.Id; Gesture = chord (InputKeyIdentity.LogicalKey "ArrowRight") none } ] }
        let modelEvent id source gesture available =
            { input with Id = id; Source = source; Gesture = gesture; AvailableCommands = available }
        let modelInitial = CommandResolver.init [ context.Id ] modelProfile
        let prefixState, _ =
            CommandResolver.update modelCatalog
                (CommandResolverObservation.InputEvent(modelEvent "prefix" "keyboard" (InputGesture.KeyChord k) [ terminal.Id; continuation.Id ]))
                modelInitial
        ...
        let heldState, heldEffects =
            CommandResolver.update modelCatalog
                (CommandResolverObservation.InputEvent(modelEvent "held" "pad" (chord (InputKeyIdentity.LogicalKey "ArrowRight") none) [ held.Id ]))
                modelInitial
        ...
        let _, lateReleaseEffects =
            CommandResolver.update modelCatalog
                (CommandResolverObservation.InputEvent { modelEvent "release" "pad" (chord (InputKeyIdentity.LogicalKey "ArrowRight") none) [] with Phase = InputGesturePhase.Released })
                recoveredState
        let invoked command effects =
            effects |> List.exists (function CommandResolverEffect.InvokeCommand value when value.Command = command -> true | _ -> false)
        let heldChanged expected effects =
            effects |> List.exists (function CommandResolverEffect.HeldActionChanged(command, value) when command = held.Id && value = expected -> true | _ -> false)
        printfn "model-correspondence terminalPending=%b terminalCleared=%b terminalInvoked=%b heldStarted=%b heldRecovered=%b lateReleaseSilent=%b"
            prefixState.PendingSequence.IsSome terminalState.PendingSequence.IsNone (invoked terminal.Id terminalEffects)
            (heldChanged true heldEffects) (heldChanged false recoveredEffects) lateReleaseEffects.IsEmpty
