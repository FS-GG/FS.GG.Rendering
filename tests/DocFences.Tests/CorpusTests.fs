...
let private brokenFixedTick model =
    if model.Paused then
        { model with
            NewlyAddedFuse = model.NewlyAddedFuse + 1
        }
    else
        { model with
            EnemyTurns = model.EnemyTurns + 1
            NewlyAddedFuse = model.NewlyAddedFuse + 1
        }
        ...
        [
            ...
            test "every extracted skill fence has a body and a locating line" {
                for f in Corpus.productSkillFences () do
                    Expect.isGreaterThan (List.length f.Body) 0 (sprintf "fence at %s:%d has no body" f.Doc f.StartLine)
            ...
            test "scenario-index guard reddens for N-minus-one and duplicate coverage" {
                ...
                Expect.isSome
                    (scenarioIndex 3 [ 1; 2 ])
                    "N-minus-one coverage must fail even when its remaining tests pass"
            ...
            test "whole-state pause encoding reddens when a newly added field advances" {
                ...
                Expect.notEqual
                    (encodePauseModel after)
                    (encodePauseModel before)
            ...
            test "the scaffold-source fence path runs and is well-formed (fences pending T014b)" {
                ...
                for f in fences do
                    ...
                    Expect.isGreaterThan
                        (List.length f.Body)
                        0
                        (sprintf "empty scaffold fence at %s:%d" f.Doc f.StartLine)
            }
            ...
            test "the map covers both fence-bearing corpora and excludes the generated mirror" {
                ...
                Expect.equal
                    (List.length kinds)
                    2
                    "exactly the two fence-bearing corpora — the generated mirror is not one"
            }
            ...
            test "every product-skill fence has a current explicit compilation disposition" {
                ...
                Expect.equal plan.Length 98 "the reviewed whole-corpus inventory remains exact"
                Expect.equal selfContained.Length 16 "the positive compiler corpus remains explicit"
                Expect.all selfContained (fun fence -> fence.Skip.IsNone) "positive compiler members cannot be skipped"
                Expect.equal contextualReasons.Length 82 "every remaining teaching fragment is accounted for"

                Expect.all
                    contextualReasons
                    (System.String.IsNullOrWhiteSpace >> not)
                    "contextual reasons are never blank"
            }
        ]
