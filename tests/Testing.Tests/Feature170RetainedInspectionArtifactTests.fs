...
let private node nodeId status prior current =
    {
        ...
        Repainted =
            status = RetainedNodeStatus.Repainted
            || status = RetainedNodeStatus.ShiftedAndRepainted
        Shifted =
            status = RetainedNodeStatus.Shifted
            || status = RetainedNodeStatus.ShiftedAndRepainted
        UnsupportedFacts = []
        Diagnostics = []
    }
    ...
    testList
        ...
        [
            test "normalization keeps stable node finding and unsupported-fact ordering" {
                ...
                let factA =
                    RetainedInspection.unsupportedFact "zeta" None true "reason" "diagnostic" false

                let factB =
                    RetainedInspection.unsupportedFact "alpha" None true "reason" "diagnostic" false
            ...
            test "artifact diagnostics report shifted nodes missing bounds and duplicate ids" {
                ...
                let shifted =
                    node "dup" RetainedNodeStatus.Shifted None (Some(rect 1.0 1.0 10.0 10.0))

                let diagnostics =
                    artifact [ shifted; shifted ] damage []
                    |> RetainedInspection.artifactDiagnostics

                Expect.exists
                    diagnostics
                    (fun item -> item.Contains("duplicate retained inspection node id"))
                    "duplicate node id"

                Expect.exists
                    diagnostics
                    (fun item -> item.Contains("missing prior or current bounds"))
                    "shifted missing bounds"
            }
        ]
