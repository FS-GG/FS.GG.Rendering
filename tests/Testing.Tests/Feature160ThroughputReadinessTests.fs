...
let tests =
    testList
        "Feature160 throughput readiness helper"
        [
            ...
            test "Synthetic helper fixture: missing scenarios sample policy mismatch and overclaim fail closed" {
                ...
                let invalid =
                    { check "passed" with
                        ...
                        Scenarios =
                            { scenario "timing/localized-update" with
                                SamplePolicy = "probe-readback-included"
                            }
                            :: (required |> List.tail |> List.map scenario)
            ...
            test "empty required-scenario set fails closed rather than vacuously accepting (F-TEST-3)" {
                let result =
                    Feature160ThroughputReadiness.validate
                        { check "passed" with
                            RequiredScenarioIds = []
                            Scenarios = []
                        }
