...
let private scenario status id =
    {
        ...
        Status = status
        PromotionDecision =
            if status = Feature159Accepted then
                "promoted"
            else
                Feature159Readiness.statusText status
        ReuseDecision =
            if status = Feature159Accepted then
        ...
        PrimaryReason =
            if status = Feature159Accepted then
                ...
                Some(Feature159Readiness.statusText status)
    }
