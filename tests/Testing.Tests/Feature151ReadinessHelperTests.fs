            ...
            test "missing evidence, blocked compatibility, and blocking limitations stay visible" {
                let report =
                    { Feature151ReadinessFixtures.acceptedReport with
                        ...
                        Evidence =
                            [
                                { Feature151ReadinessFixtures.evidence "validation-summary.md" with
                                    Path = None
                                    Status = LayoutReadinessMissingEvidence
                                }
                            ]
                        CompatibilityDeltas =
                            [
                ...
                Expect.contains
                    result.BlockingLimitations
                    "blocking: package validation missing"
                    "blocking limitation named"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains("unintentional layout compatibility delta"))
