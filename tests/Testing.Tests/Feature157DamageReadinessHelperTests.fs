...
let private scenario status id =
    {
        ...
        FallbackReason =
            if status = CompositorDamageAccepted then
                None
            else
                Some "full-redraw-fallback"
    }
    ...
    testList
        ...
        [
            ...
            test "empty required-scenario set fails closed rather than vacuously accepting (F-TEST-3)" {
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains("at least one required scenario"))
                    "disclosing diagnostic"
            }

            test
                "Synthetic helper fixture: environment-limited unsupported host requires zero accepted partial-redraw artifacts" {
                let limited =
                    { check CompositorDamageAccepted with
                        UnsupportedHostStatus = CompositorDamageEnvironmentLimited
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains("zero accepted partial-redraw"))
                    "zero accepted diagnostic"
            }
        ]
