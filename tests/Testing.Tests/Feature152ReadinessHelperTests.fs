...
let tests =
    testList
        "Feature152 compositor readiness helper"
        [
            ...
            test
                "accepted proof, parity, compatibility, and regression accept correctness even when timing is not a performance claim" {
                let report =
                    {
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains("timing claim status"))
                    "timing no-claim is visible"
            }
                ...
                let report =
                    {
                        ...
                        Evidence =
                            [
                                { evidence "validation-summary.md" CompositorReadinessAccepted with
                                    EvidencePath = None
                                    EvidenceStatus = CompositorReadinessMissingEvidence
                                }
                            ]
                        Limitations = []
                    }
