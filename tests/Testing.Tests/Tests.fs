...
let drawGlyphText (canvas: SKCanvas) (x0: float32) (y0: float32) (scale: float32) (text: string) =
    ...
    use paint =
        new SKPaint(Color = SKColors.Black, IsAntialias = false, Style = SKPaintStyle.Fill)
        ...
        |> List.iteri (fun row line ->
            ...
            |> Seq.iteri (fun column value ->
                if value = '1' then
                    canvas.DrawRect(
                        SKRect.Create(
                            x + float32 column * scale,
                            y0 + float32 row * scale,
                            scale * 0.86f,
                            scale * 0.86f
                        ),
                        paint
    ...
    testList
        ...
        [
            ...
            test "local consumer package reports include feed snippets restore command and drift" {
                ...
                Expect.equal
                    (report.Packages |> List.map _.PackageId)
                    [ "FS.GG.UI.Scene"; "FS.GG.UI.SkiaViewer" ]
                    "generated consumer package set is recorded"
                ...
                Expect.stringContains
                    (report.NuGetConfigSnippet |> Option.defaultValue "")
                    "/tmp/feed"
                    "NuGet.config snippet names feed path"

                Expect.stringContains report.RestoreCommand "dotnet restore" "restore command is included"

                Expect.exists
                    drift
                    (fun item -> item.PackageId = "FS.GG.UI.Scene" && item.ActualVersion = Some "1.2.2")
                    "stale package drift is reported before generated build failures"

                Expect.exists
                    drift
                    (fun item -> item.PackageId = "FS.GG.UI.SkiaViewer" && item.ActualVersion = None)
                    "missing package drift is reported before generated build failures"
                ...
                |> List.iter (fun item ->
                    Expect.stringContains
                        ...
                        "PackLocal"
                        "drift diagnostics name PackLocal remediation")
            }

            test "evidence report helper writes stable ordered fields and validates unsupported fallback" {
                let root =
                    IO.Path.Combine(IO.Path.GetTempPath(), $"fs-gg-evidence-report-{Guid.NewGuid():N}")
                ...
                Expect.equal
                    (report.Lines |> List.take 3)
                    [ "status=unsupported"; $"command={report.Command}"; $"output={path}" ]
                    "standard fields lead the report"
                ...
                Expect.sequenceEqual
                    (capturedOut.ToString().Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries))
                    report.Lines
                    "stdout output matches report lines"

                Expect.equal (EvidenceReports.statusText EvidenceOk) "ok" "ok status vocabulary is normalized"

                Expect.equal
                    (EvidenceReports.statusText EvidenceUnsupported)
                    "unsupported"
                    "unsupported status vocabulary is normalized"

                Expect.equal
                    (EvidenceReports.statusText EvidenceFailed)
                    "failed"
                    "failed status vocabulary is normalized"
                ...
                Expect.containsAll
                    invalidValidation.MissingFields
                    [ "unsupported-host-reason"; "fallback" ]
                    "unsupported required fields are reported"
            ...
            test "screenshot evidence report validator rejects unsupported records that claim the offscreen raster ran" {
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains("must not claim a real capture path", StringComparison.Ordinal))
                    "the offscreen capture claim is named as the contradiction"
            }
            ...
            test "ScreenshotEvidenceReport_Synthetic rejects unsupported records that hide capability detail" {
                ...
                Expect.containsAll
                    ...
                    [ "capture-availability"; "unsupported-host-reason"; "fallback" ]
                    "missing capability fields are named"

                Expect.equal
                    result.FailureClass
                    (Some "missing-screenshot-evidence-fields")
                    "missing fields get stable failure class"
            }
            ...
            test "ScreenshotEvidenceReport_Synthetic rejects invalid success proof fields" {
                ...
                Expect.equal
                    result.FailureClass
                    (Some "missing-screenshot-evidence-fields")
                    "missing success field has stable failure class"
            }
            ...
            test "ScreenshotEvidenceReport_Synthetic rejects hidden warnings in successful proof" {
                ...
                Expect.equal
                    ...
                    "hidden warnings have stable failure class"
            }
            ...
            test "ScreenshotEvidenceReport_Synthetic rejects hostile artifact paths" {
                ...
                Expect.equal
                    result.FailureClass
                    (Some "invalid-screenshot-evidence-fields")
                    "hostile paths have stable failure class"
            }

            test "default text glyph evidence accepts glyph-shaped PNG coverage" {
                let root =
                    IO.Path.Combine(IO.Path.GetTempPath(), $"fs-gg-default-text-{Guid.NewGuid():N}")
            ...
            test "default text glyph evidence rejects solid block and tofu-like screenshots" {
                let root =
                    IO.Path.Combine(IO.Path.GetTempPath(), $"fs-gg-default-text-negative-{Guid.NewGuid():N}")
                ...
                Expect.equal
                    solidResult.FailureClass
                    (Some "solid-block-default-text")
                    "solid block gets stable failure class"
                ...
                Expect.isFalse
                    tofuResult.Accepted
                    $"tofu-like boxes do not satisfy default text glyph evidence: glyph={tofuResult.GlyphCoverageMetric}; solid={tofuResult.SolidBlockMetric}; placeholder={tofuResult.PlaceholderMetric}; diagnostics={tofuDiagnostics}"

                Expect.equal
                    tofuResult.FailureClass
                    (Some "placeholder-default-text")
                    "tofu box gets stable failure class"
            }
            ...
            test "known GTK module warnings are benign only with first-frame launch evidence and preserved raw text" {
                ...
                Expect.equal
                    missingFirstFrame.WarningClass
                    RenderingFailure
                    "missing first-frame/render evidence is not hidden by benign marker"
            ...
            test "known GTK module warning variants are benign with first-frame success" {
                ...
                |> List.iter (fun raw ->
                    ...
                    Expect.equal
                        result.WarningClass
                        BenignEnvironmentWarning
                        $"{raw} is benign after first-frame success"
            ...
            test "generated consumer validation summaries expose category elapsed command and evidence" {
                ...
                Expect.stringContains
                    ...
                    "bounded smoke unsupported diagnostic is summarized"
            }
            ...
            test "generated package verification fails NU1603 exact-version drift and missing sources" {
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "missing package sources")
                    "missing sources are reported"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "package mismatch")
                    "requested/resolved drift is reported"
            }
            ...
            test "generated package verification reports version mismatch failure when NU1603 is absent" {
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "requested=0.1.16-persistent.1")
                    "requested version is reported"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "resolved=0.1.16-preview.1")
            ...
            test "generated verification is non-authoritative when tests exist but do not run" {
                ...
                Expect.equal
                    result.NonAuthoritativeReason
                    (Some "missing-generated-test-execution")
                    "missing generated tests are classified"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "did not run")
                    "diagnostic explains skipped tests"
            }
            ...
            test "generated verification is non-authoritative when tests bypass Verify" {
                ...
                Expect.isFalse
                    ...
                    "generated tests outside Verify do not prove generated Verify coverage"

                Expect.equal
                    result.NonAuthoritativeReason
                    (Some "verify-target-not-authoritative")
                    "Verify bypass has its own failure class"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "outside generated Verify")
                    "diagnostic names Verify bypass"
            }
            ...
            test "generated verification is non-authoritative when no generated tests exist" {
                ...
                Expect.equal
                    ...
                    "absent generated tests have their own failure class"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "no generated tests exist")
                    "diagnostic names the absent tests"
            }
            ...
            test "generated validation contract emits generated-tests-ran=false when no tests exist" {
                ...
                Expect.equal
                    result.FailureClass
                    "no-generated-tests"
                    "absent generated tests surface as the failure class"
...
"""
                ...
                let invalid =
                    GeneratedProductAssertions.validateDefaultInteractiveLaunch invalidSource

                Expect.isFalse invalid.InteractiveLaunchRequired "bounded-only default path is rejected"

                Expect.exists
                    invalid.Diagnostics
                    (fun item -> item.Contains "Viewer.runApp")
                    "missing runApp is diagnostic"

                Expect.exists
                    invalid.Diagnostics
                    (fun item -> item.Contains "Viewer.runBounded")
                    "bounded substitute is diagnostic"

                Expect.exists
                    invalid.Diagnostics
                    (fun item -> item.Contains "self-close")
                    "evidence self-close is diagnostic"

                Expect.exists
                    invalid.Diagnostics
                    (fun item -> item.Contains "metadata-only")
                    "metadata-only default is diagnostic"
            }
...
"""
                ...
                Expect.isFalse
                    rejected.InteractiveLaunchRequired
                    "a default branch that launches nothing is still rejected"
            }
            ...
            test "generated product validation rejects first-frame metadata and inaccessible default commands" {
                ...
                Expect.exists
                    firstFrame.Diagnostics
                    (fun item -> item.Contains "first frame")
                    "first-frame-only default is diagnostic"

                let metadataOnly =
                    GeneratedProductAssertions.validateDefaultInteractiveLaunch (invalidDefaults |> List.item 1 |> snd)

                Expect.exists
                    metadataOnly.Diagnostics
                    (fun item -> item.Contains "metadata-only")
                    "metadata-only default is diagnostic"
                ...
                Expect.exists
                    missingAccessible.Diagnostics
                    (fun item -> item.Contains "accessible desktop window")
                    "missing accessible-window claim is diagnostic"
            }
            ...
            test "generated layout validation rejects missing unsupported overlapping and deterministic-only reports" {
                let deterministic =
                    LayoutEvidence.fromRenderEvidence
                        Scene.empty
                        (Scene.renderReadbackEvidence { Width = 640; Height = 480 } Scene.empty)
                ...
                let unsupported =
                    LayoutEvidence.unsupported
                        ...
                        { Width = 640; Height = 480 }
                        {
                            ...
                            Diagnostic = "unsupported layout inspection"
                        }
                ...
                Expect.equal
                    deterministicResult.FailureClass
                    (Some DeterministicRenderOnlyClaim)
                    "deterministic render only cannot satisfy readability"

                Expect.equal
                    unsupportedResult.FailureClass
                    (Some UnsupportedLayoutFacts)
                    "unsupported facts are classified"

                Expect.equal overlappingResult.FailureClass (Some OverlappingLayoutBounds) "overlap is classified"
            }

            test
                "host warning classification keeps benign environment warnings non-fatal only after supporting checks pass" {
                let baseline =
                    {
...
"""
                ...
                Expect.isTrue
                    ...
                    "generated diagnostics include all required failure classes and native facts"
            ...
            test "generated diagnostic validation rejects taskbar-only success and missing native facts" {
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "environment-session")
                    "missing environment/session class is diagnostic"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "renderable-surface")
                    "missing renderable-surface fact is diagnostic"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "taskbar-only")
                    "taskbar-only status=ok is diagnostic"
            }
            ...
            test "visual evidence uses pixel-readback fallback only when screenshots are unavailable" {
                ...
                Expect.equal
                    ...
                    (Some "screenshot unavailable; pixel-readback selected")
                    "fallback reason is explicit"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "screenshot unavailable")
                    "diagnostics name screenshot fallback"
            }
            ...
            test "visual evidence reports unsupported host when screenshot and readback are unavailable" {
                ...
                Expect.equal
                    ...
                    (Some "headless session has no display socket")
                    "unsupported reason is retained"
            ...
            test "generated visual evidence command output rejects text hashes mislabeled as screenshots" {
                ...
                Expect.equal
                    ...
                    "metadata-only screenshot claims have a specific failure class"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "decodable image")
                    "diagnostic names decodable image requirement"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains "metadata")
                    "diagnostic names metadata/hash mislabeling"
            }
            ...
            test "PersistentLaunchArtifactValidation_Synthetic rejects missing required fields" {
                    ...
                    PersistentLaunchArtifactValidation.validate
                        {
                            ...
                            Lines = [ "status=failed"; "mode=interactive-window"; "command=synthetic-fixture" ]
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item = "synthetic-fixture=true")
                    "synthetic fixture is disclosed in diagnostics"
            }
            ...
            test "PersistentLaunchArtifactValidation_Synthetic rejects invalid field values" {
                ...
                Expect.exists
                    ...
                    (fun item -> item = "classification=headless-only")
                    "headless-only classification is rejected"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item = "invalid-field=mode=headless-only")
                    "invalid mode diagnostic is actionable"
            }
            ...
            test "PersistentLaunchArtifactValidation_Synthetic rejects contradictory supported-host pass claims" {
                ...
                Expect.exists
                    result.Contradictions
                    (fun item -> item.Contains "synthetic fixture cannot satisfy")
                    "synthetic pass claim is rejected"

                Expect.exists
                    result.Contradictions
                    (fun item -> item.Contains "window-opened=true")
                    "window contradiction is reported"

                Expect.exists
                    result.Contradictions
                    (fun item -> item.Contains "blocked-stage=none")
                    "blocked-stage contradiction is reported"
            }
            ...
            test "generated validation contract output includes all required validation fields" {
                ...
                let windowDiagnostics =
                    GeneratedProductAssertions.validateWindowDiagnostics
                        {
                            Output =
                                "status=failed diagnostic-class=window-visibility native-handle=observed:true visible=observed:false"
                ...
                Expect.stringContains
                    ...
                    "generated test execution field is emitted"

                Expect.stringContains
                    result.Output
                    "default-interactive-launch=true"
                    "interactive launch field is emitted"

                Expect.stringContains
                    result.Output
                    "bounded-evidence-validation=true"
                    "bounded evidence field is emitted"
                ...
                Expect.stringContains
                    ...
                    "window diagnostics field is emitted"
