...
let private tempDir () =
    let path =
        Path.Combine(Path.GetTempPath(), "fs-gg-feature164-" + Guid.NewGuid().ToString("N"))
    ...
    testList
        ...
        [
            test "matrix expands 3 pages x 2 themes x 2 sizes deterministically" {
                ...
                Expect.equal
                    (targets |> List.head |> _.RelativePath)
                    "preferred/light/overview.png"
                    "size/theme/page ordering"
            ...
            test "matrix rejects duplicate ids duplicate paths and escaping paths" {
                ...
                | Result.Error diagnostics ->
                    Expect.exists diagnostics (fun d -> d.Contains("duplicate page id")) "duplicate page rejected"
                | Ok _ -> failtest "duplicate page ids should fail"

                match VisualCaptureMatrix.expand pages themes sizes (fun _ _ _ -> "same.png") with
                | Result.Error diagnostics ->
                    Expect.exists diagnostics (fun d -> d.Contains("duplicate relative path")) "duplicate path rejected"
                ...
                | Result.Error diagnostics ->
                    Expect.exists diagnostics (fun d -> d.Contains("escapes evidence root")) "escape path rejected"
            ...
            test "correctly-sized fully-transparent PNG is not minted as complete (F-TEST-3)" {
                ...
                try
                    ...
                    Expect.equal
                        ...
                        "blank capture blocks rather than passing complete"

                    Expect.exists
                        records[0].Diagnostics
                        (fun d -> d.Contains("blank screenshot"))
                        "disclosing blank diagnostic"
            ...
            test
                "reviewer template and parser cover missing duplicate malformed unknown pending minor major and blocking rows" {
                ...
                let row (target: VisualCaptureTarget) severity impact =
                    sprintf
                        "| %s | %s | %s | %s | %s | none | %s | reviewer | 2026-06-19 | note |"
                        target.TargetId
                        target.Page.PageId
                ...
                Expect.exists
                    parsed.Diagnostics
                    (fun d -> d.Contains("duplicate reviewer row"))
            ...
            test "readiness gates pending review blocking defect all-clear and default no exceptions" {
                ...
                let blocked =
                    VisualReadiness.evaluate "run" "root" targets captures (reviews VisualReviewerBlocking) [] [] []

                Expect.equal blocked.ReadinessStatus VisualReadinessBlocked "blocking reviewer defect"

                let accepted =
                    VisualReadiness.evaluate "run" "root" targets captures (reviews VisualReviewerNone) [] [] []

                Expect.equal accepted.ReadinessStatus VisualReadinessAccepted "all-clear accepted"

                let wrongSize =
                    { complete targets[0] with
                        Status = VisualCaptureWrongSize
                        Diagnostics = [ "wrong-size" ]
                    }
                ...
                let noException =
                    VisualReadiness.evaluate
                        ...
                        "root"
                        targets
                        (wrongSize :: [ complete targets[1] ])
                        (reviews VisualReviewerNone)
                        []
                        []
                        []
            ...
            test "contact sheet metadata and Markdown JSON summaries expose counts diagnostics and readiness" {
                ...
                let report =
                    VisualReadiness.evaluate "run" "root" targets captures reviews [ sheet ] [ "manual caveat" ] []
            ...
            test "managed summary regeneration preserves manual content and fails safely for malformed markers" {
                ...
                let updated1 =
                    VisualReadinessMarkdown.updateManagedSection inserted.UpdatedText generatedB

                let updated2 =
                    VisualReadinessMarkdown.updateManagedSection updated1.UpdatedText generatedA
