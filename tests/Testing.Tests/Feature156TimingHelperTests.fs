    ...
    testList
        ...
        [
                    ...
                    let scenarios =
                        ...
                        |> List.map (fun id ->
                            if id = "timing/no-change" then
                                { scenario id verdict with
                                    RejectionReasons = [ CompositorTimingAssertions.verdictText verdict ]
                                }
                            else
            ...
            test "cross-profile missing scenario and overclaiming shipped status are rejected" {
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Contains("cannot accept the shipped P7 performance claim"))
                    "overclaim"
            }
        ]
