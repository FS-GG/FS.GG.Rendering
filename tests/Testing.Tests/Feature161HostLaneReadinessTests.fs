...
let tests =
    testList
        "Feature161 host lane readiness helper"
        [
            ...
            test "blocks complete host lane readiness when full validation or prior gates are not current" {
                [
                    { check "missing" with
                        FullValidationStatus = "missing"
                    }
                    { check "passed" with
                        PriorGateStatuses = [ "confirmed"; "blocked" ]
                    }
                ]
