...
let rec private setSize (id: LayoutNodeId) (w: float) (h: float) (n: LayoutNode) : LayoutNode =
    ...
    { n with
        Children = n.Children |> List.map (setSize id w h)
    }
...
let scaffold =
    testList
        "Audit IncrementalLayout scaffold"
        [
...
let equivalence =
    testList
        "Audit IncrementalLayout equivalence"
        [
            test
                "Audit: equivalence — incremental == full across constructed change sets, with discriminating proof (T024, FR-006/SC-003)" {
                ...
                let edits = [ "0.0.0", 70.0; "0.1.0", 90.0; "0.2", 65.0; "0.0.0", 35.0 ]
                ...
                eprintfn
                    "AUDIT T024 DiscriminatingProof: %b (rightMatches=%b wrongDiverges=%b)"
                    discriminatingProof
                    rightMatches
                    wrongDiverges
                ...
                Expect.isTrue
                    wrongDiverges
                    "wrong incremental (changed id omitted) DIVERGES — proves the equivalence test has teeth"
            }
...
let effectiveness =
    testList
        "Audit IncrementalLayout effectiveness"
        [
            test
                "Audit: effectiveness — localized change in a large tree re-measures a small subset (T029, FR-008/US3-AS1)" {
                ...
                Expect.equal
                    (boundsMap inc)
                    (boundsMap full)
                    "incremental geometry equals full evaluate on the large tree"
                ...
                Expect.isLessThan
                    remeasured
                    (n / 10)
                    $"localized change re-measures a small subset ({remeasured}/{n}), not the whole tree"
            }
        ]
