    ...
    testList
        ...
        [
            test "duplicate participant ids produce measurement diagnostics" {
                ...
                let constraints =
                    Layout.constraintsFromAvailable Parent (Defaults.availableSpace 100.0 100.0)

                let result = Layout.measureProtocol constraints root

                Expect.exists
                    result.Diagnostics
                    (fun d -> d.Code = DuplicateMeasurement)
            ...
            test "participant mismatch rejects intrinsic query" {
                ...
                let query =
                    Layout.intrinsicQuery "other" IntrinsicMaxHeight (Some 120.0) "key" DiagnosticProbe
                ...
                Expect.exists
                    result.Diagnostics
                    (fun d -> d.Code = UnsupportedIntrinsicQuery)
