...
let tests =
    testList
        "Feature151FullIncrementalParity"
        [
            ...
            test "changed-input incremental runs match a fresh full evaluation and reject stale geometry" {
                ...
                for item, changedRoot in changedCases do
                    ...
                    let incremental =
                        Layout.evaluateIncremental previous item.ChangedNodeIds item.Available changedRoot
                    ...
                    Expect.notEqual
                        (Layout.layoutInputKey changedRoot)
                        (Layout.layoutInputKey item.Root)
                        $"{item.CaseId}: changed input key"
            }

            test "changed content produces reviewer-visible invalidation evidence" {
                let item =
                    Feature151CorpusFixtures.allCases
                    |> List.find (fun item -> item.CaseId = "dynamic-content")
                ...
                Expect.contains
                    (Set.ofList incremental.Invalidated)
                    "dynamic-a"
                    "changed measured participant invalidated"
            }
        ]
