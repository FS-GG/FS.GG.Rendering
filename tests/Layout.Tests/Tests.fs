...
let node id = { Id = id; Label = id; Style = None }
...
let boundsOf id (result: LayoutResult) =
    result.Bounds
    |> List.find (fun item -> item.NodeId = id)
    |> fun item -> item.Bounds
        ...
        [
            ...
            test "horizontal and vertical stack measurement honors padding spacing and child count" {
                ...
                Expect.floatClose
                    ...
                    horizontal[0].Height
                    100.0
                    "vertical padding is applied to horizontal stack"
                ...
                Expect.floatClose
                    ...
                    vertical[0].Width
                    270.0
                    "horizontal padding is applied to vertical stack"
            }
            ...
            test "dock config and child sizing records retain public layout props" {
                ...
                Expect.equal
                    sized.Sizing.HorizontalAlignment
                    HorizontalAlignment.Center
                    "horizontal alignment is retained"
                ...
                | [ SceneNode.Group placed ] ->
                    ...
                    |> List.iteri (fun index (dx, dy) ->
                        Expect.floatClose
                            ...
                            dx
                            expected[index].X
                            $"child {index} X matches its measured bounds"

                        Expect.floatClose
                            Accuracy.medium
                            dy
                            expected[index].Y
                            $"child {index} Y matches its measured bounds")
            ...
            test "dock consumes a left edge sized by the child's DesiredWidth (P6/R6)" {
                ...
                let fill =
                    {
                        Content = Scene.rectangle (0.0, 0.0, 10.0, 10.0) Colors.black
                        Sizing = sizing
                        Dock = None
                    }
                ...
                | [ SceneNode.Group [ leftPlaced; fillPlaced ] ] ->
                    ...
                    Expect.floatClose
                        ...
                        fillX
                        (4.0 + 120.0)
                        "the fill child starts after the consumed left edge"
            ...
            test "dock splits the remaining extent evenly across unsized edge children (#369)" {
                ...
                | [ SceneNode.Group [ firstPlaced; secondPlaced ] ] ->
                    ...
                    Expect.floatClose
                        ...
                        (offsetY firstPlaced)
                        0.0
                        "the first unsized top child sits at the top of the dock"

                    Expect.floatClose
                        Accuracy.medium
                        (offsetY secondPlaced)
                        240.0
                        "the second unsized top child starts at the even-share boundary, not starved off-canvas"
                | other -> failtestf "dock returns a group of two positioned children, got %A" other
            }
            ...
            test "zero and negative stack bounds clamp measured child sizes to non-negative values" {
                ...
                let negative =
                    Layout.measureVertical (Defaults.stackConfig -20.0 -10.0) [ child "negative" ]
            ...
            test "layout resize keeps at least ten horizontal children non-overlapping at three sizes" {
                ...
                for width, height in sizes do
                    ...
                    for leftIndex in 0 .. bounds.Length - 1 do
                        for rightIndex in leftIndex + 1 .. bounds.Length - 1 do
                            Expect.isFalse
                                (overlaps bounds[leftIndex] bounds[rightIndex])
                                $"children {leftIndex} and {rightIndex} do not overlap at {width}x{height}"
            }
                ...
                for width, height in sizes do
                    ...
                    for topIndex in 0 .. bounds.Length - 1 do
                        for bottomIndex in topIndex + 1 .. bounds.Length - 1 do
                            Expect.isFalse
                                (overlaps bounds[topIndex] bounds[bottomIndex])
                                $"children {topIndex} and {bottomIndex} do not overlap at {width}x{height}"
            }
            ...
            test "graph validation reports duplicates missing endpoints self-loops and cycles" {
                ...
                Expect.exists
                    ...
                    | _ -> false)
                    "directed cycle is reported"
            }

            test "graph validation reports disconnected components and accepts dense edge sets" {
                let disconnected =
                    graph Undirected [ node "a"; node "b"; node "c"; node "d"; node "e" ] [ edge "a" "b"; edge "c" "d" ]
                ...
                Expect.isFalse
                    (GraphValidation.hasCycle dense)
                    "undirected dense graph cycle detection is intentionally not a DAG failure"
            }
            ...
            test "weighted undirected graph with fifty nodes has visible components and renders a scene" {
                ...
                | Ok layout, Ok scene ->
                    ...
                    Expect.isTrue
                        (layout.Nodes
                         |> List.forall (fun item -> item.Bounds.Width > 0.0 && item.Bounds.Height > 0.0))
                        "node bounds are visible"
            ...
            test "graph scene builders render edges labels weights and hit-test nodes and edges" {
                ...
                | Ok layout, Ok scene ->
                    ...
                    Expect.equal
                        (Graph.hitTest layout nodeCenterX nodeCenterY)
                        (Some(Node firstNode.Node.Id))
                        "node hit-test returns node target"
                ...
                let columnRoot =
                    { Defaults.layoutNode "column-root" with
                        Intent =
                            { Defaults.layoutIntent with
                                Direction = Column
                                Gap = { Row = 6.0; Column = 0.0 }
                            }
                        Children =
                            [
                                layoutLeaf "r1" 40.0 24.0
                                layoutLeaf "r2" 50.0 28.0
                                layoutLeaf "r3" 60.0 32.0
                            ]
                    }
            ...
            test "automatic layout honors padding margin gaps and alignment" {
                let root =
                    { Defaults.layoutNode "root" with
                        ...
                        Children =
                            [
                                { layoutLeaf "left" 40.0 20.0 with
                                    Intent =
                                        { (layoutLeaf "left" 40.0 20.0).Intent with
                                            Margin =
                                                {
                                                    Left = 3.0
                                                    Top = 0.0
                ...
                Expect.floatClose
                    Accuracy.medium
                    right.X
                    (left.X + left.Width + 5.0 + 3.0)
                    "gap and margin separate children"
            }
            ...
            test "automatic layout applies fixed min max flex grow shrink basis and deterministic repeated evaluation" {
                ...
                Expect.isGreaterThan
                    (boundsOf "two" first).Width
                    (boundsOf "one" first).Width
                    "larger flex grow receives more space"
            ...
            test
                "automatic layout incremental evaluation reports the actual re-measured set (FR-001a) and keeps bounds byte-identical to full evaluate" {
                let root size =
                    { Defaults.layoutNode "root" with
                        Children =
                            [
                                layoutLeaf "stable" 40.0 20.0
                                { layoutLeaf "changed" size 20.0 with
                                    Intent =
                                        { (layoutLeaf "changed" size 20.0).Intent with
                                            FlexGrow = 1.0
                                        }
                                }
                            ]
                ...
                let second =
                    Layout.evaluateIncremental first [ "changed" ] (Defaults.availableSpace 180.0 40.0) (root 80.0)
                ...
                Expect.equal
                    (Set.ofList second.Invalidated)
                    (Set.ofList [ "root"; "stable"; "changed" ])
                    "incremental reports the actual re-measured set"
            ...
            test "automatic layout render and hit-test consume computed bounds with shared pixel snapping" {
                let root =
                    { Defaults.layoutNode "root" with
                        Children =
                            [
                                layoutLeaf "button" 40.0 20.0
                                { layoutLeaf "label" 30.0 20.0 with
                                    Visibility = Hidden
                                }
                            ]
                    }
                ...
                let policy =
                    { Defaults.pixelSnapPolicy 1.5 with
                        Mode = Round
                    }
                ...
                Expect.equal
                    (Layout.hitTestComputed policy result (button.X + 1.0) (button.Y + 1.0))
                    (Some "button")
                    "hit testing returns visible node"

                Expect.notEqual
                    (Layout.hitTestComputed
                        policy
                        result
                        ((boundsOf "label" result).X + 1.0)
                        ((boundsOf "label" result).Y + 1.0))
                    (Some "label")
                    "hidden nodes are not hit-testable"

                Expect.isTrue
                    ((Layout.snapBounds policy { button with X = 0.2; Y = 0.2 }).X >= 0.0)
                    "snap bounds returns deterministic logical coordinates"
            }
            ...
            test "automatic layout workflow update is pure and emits resize visibility intent and measurement effects" {
                ...
                let model, startupEffects =
                    Layout.initWorkflow (Defaults.availableSpace 160.0 48.0) root
                ...
                let resized, resizeEffects =
                    Layout.updateWorkflow (LayoutHostResized(Defaults.availableSpace 220.0 48.0)) model

                Expect.equal resized.Available.Width 220.0 "resize updates owned available space"

                Expect.equal
                    resizeEffects
                    [ EvaluateIncrementalLayout [ "root" ] ]
                    "resize requests incremental layout from root constraints"

                let hidden, visibilityEffects =
                    Layout.updateWorkflow (LayoutVisibilityChanged("action", Hidden)) resized

                let hiddenAction =
                    hidden.Root.Children |> List.find (fun child -> child.Id = "action")

                Expect.equal hiddenAction.Visibility Hidden "visibility message updates the layout tree"

                Expect.equal
                    visibilityEffects
                    [ EvaluateIncrementalLayout [ "action" ] ]
                    "visibility change emits node invalidation"
                ...
                let intentChanged, intentEffects =
                    Layout.updateWorkflow (LayoutIntentChanged("title", nextIntent)) hidden

                let changedTitle =
                    intentChanged.Root.Children |> List.find (fun child -> child.Id = "title")

                Expect.equal changedTitle.Intent.FlexGrow 2.0 "intent message updates the target node"

                Expect.equal
                    intentEffects
                    [ EvaluateIncrementalLayout [ "title" ] ]
                    "intent change emits target invalidation"

                let measured, measurementEffects =
                    Layout.updateWorkflow (LayoutMeasurementChanged "title") intentChanged
                ...
                Expect.equal
                    measurementEffects
                    [ EvaluateIncrementalLayout [ "title" ] ]
                    "measurement change emits incremental evaluation"
            }
            ...
            test "automatic layout keyboard focus region aligns with visual bounds and pointer hit-test after snapping" {
                ...
                let policy =
                    { Defaults.pixelSnapPolicy 2.0 with
                        Mode = Expand
                    }
                ...
                Expect.equal
                    (Layout.hitTestComputed policy result focusX focusY)
                    (Some "focusable")
                    "pointer hit-test uses the same snapped bounds as the keyboard focus region"

                Expect.equal
                    focusRegion
                    (Layout.snapBounds policy visualBounds)
                    "focus region is derived from computed visual bounds with the shared snap policy"
            }

            test
                "automatic layout diagnostics report invalid available space values duplicate ids and min max conflicts" {
                let conflicted =
                ...
                let root =
                    { Defaults.layoutNode "root" with
                        Children = [ conflicted; layoutLeaf "dup" -10.0 20.0 ]
                    }
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Code = InvalidAvailableSpace)
                    "invalid available space is diagnosed"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Code = DuplicateLayoutNodeId)
                    "duplicate node ids are diagnosed"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Code = UnsatisfiedConstraint)
                    "min/max conflict is diagnosed"

                Expect.exists
                    result.Diagnostics
                    (fun item -> item.Code = FallbackBoundsApplied)
                    "fallback geometry is summarized"

                Expect.all
                    result.Bounds
                    (fun item -> item.Bounds.Width >= 0.0 && item.Bounds.Height >= 0.0)
                    "fallback bounds remain bounded"
            }
            ...
            test "automatic layout hidden and collapsed nodes are distinguishable and visible siblings remain stable" {
                    ...
                    { Defaults.layoutNode "root" with
                        Children =
                            [
                                ...
                                { layoutLeaf "toggle" 30.0 20.0 with
                ...
                let collapsed =
                    Layout.evaluate (Defaults.availableSpace 160.0 40.0) (root Collapsed)

                Expect.equal
                    (hidden.Bounds |> List.find (fun item -> item.NodeId = "toggle")).Visibility
                    Hidden
                    "hidden node is retained as hidden"

                Expect.equal
                    (collapsed.Bounds |> List.find (fun item -> item.NodeId = "toggle")).Visibility
                    Collapsed
                    "collapsed node is retained as collapsed"
