    ...
    testList
        ...
        [
            ...
            test "Synthetic-free: incremental over a fixed-size boundary is byte-identical to full evaluate (SC-001)" {
                ...
                let frame2 =
                    autoBox
                        "0"
                        [
                            fixedBox "0.0" 200.0 120.0 [ leaf "0.0.0" 70.0 35.0; leaf "0.0.1" 50.0 20.0 ]
                        ]
            ...
            test "Empty dirty set re-measures nothing and reuses cached bounds (FR-008 at-rest)" {
                ...
                Expect.equal
                    (boundsMap incremental)
                    (boundsMap (Layout.evaluate available frame))
            ...
            test "Content-sized chain to the root re-measures everything (SC-004 degenerate-correct)" {
                ...
                Expect.equal
                    (boundsMap incremental)
                    (boundsMap (Layout.evaluate available frame2))
                    "byte-identical (full fallback)"
...
let equivalence =
    testList
        ...
        [
            test
                "evaluateIncremental (cache carried) is byte-identical to full evaluate over >=1000 generated (tree, edit-sequence) cases (FR-007/SC-002)" {
