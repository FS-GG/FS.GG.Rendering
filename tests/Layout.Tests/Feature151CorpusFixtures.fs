...
let zeroRoot = container "zero-root" 0.0 0.0 [ measuredLeaf "zero-child" 12.0 12.0 ]
...
let veryLargeRoot =
    rowContainer
        ...
        4096.0
        2048.0
        [ measuredLeaf "large-a" 512.0 256.0; measuredLeaf "large-b" 768.0 256.0 ]

let measuredLeaves =
    container "measured-leaves" 180.0 160.0 [ measuredLeaf "measured-a" 72.0 18.0; measuredLeaf "measured-b" 96.0 24.0 ]

let emptyContainer = container "empty-container" 90.0 70.0 []
...
let childOrder reversed =
    let children =
        [ measuredLeaf "order-a" 40.0 20.0; measuredLeaf "order-b" 50.0 20.0 ]
...
let visibilityRoot =
    container
        ...
        [
            ...
            { measuredLeaf "hidden-child" 50.0 20.0 with
                Visibility = Hidden
            }
            { measuredLeaf "collapsed-child" 50.0 20.0 with
            ...
            ChangedRoot =
                Some(
                    container
                        ...
                        180.0
                        160.0
                        [ measuredLeaf "measured-a" 88.0 18.0; measuredLeaf "measured-b" 96.0 24.0 ]
                )
...
let resultOf (item: LayoutCase) =
    Layout.evaluate item.Available item.Root

let boundsOf (nodeId: LayoutNodeId) (result: LayoutResult) =
    result.Bounds
    |> List.find (fun item -> item.NodeId = nodeId)
