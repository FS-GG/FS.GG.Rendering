        ...
        [
            ...
            test "intent diagnostics survive on the Yoga success path" {
                ...
                Expect.exists
                    result.Diagnostics
                    (fun item ->
                        item.Code = UnsatisfiedConstraint
                        && item.NodeId = Some "child"
                        && item.Constraint = Some "width")
                    "min/max width conflict is still diagnosed"

                Expect.exists
                    result.Diagnostics
                    (fun item ->
                        item.Code = InvalidLayoutValue
                        && item.NodeId = Some "child"
                        && item.Constraint = Some "padding-left")
