...
let private updateTarget f node =
    let children =
        node.Children
        |> List.map (fun child ->
            if child.Id = "target" then
                { child with Intent = f child.Intent }
            else
                child)
...
let private cases: (string * LayoutNodeId * (LayoutNode -> LayoutNode)) list =
    [
        ...
        updateTarget (fun i ->
            { i with
                AlignSelf = Some LayoutAlign.End
            })
        "justifyContent",
        "root",
        fun root ->
            { root with
                Intent =
                    { root.Intent with
                        JustifyContent = LayoutAlign.End
                    }
            }
        ...
        updateTarget (fun i ->
            { i with
                MinSize = { i.MinSize with Width = Some 35.0 }
            })
        "minHeight",
        "target",
        updateTarget (fun i ->
            { i with
                MinSize = { i.MinSize with Height = Some 28.0 }
            })
        "maxWidth",
        "target",
        updateTarget (fun i ->
            { i with
                MaxSize = { i.MaxSize with Width = Some 18.0 }
            })
        "maxHeight",
        "target",
        updateTarget (fun i ->
            { i with
                MaxSize = { i.MaxSize with Height = Some 16.0 }
            })
    ]
