...
let tests =
    testList
        "Feature151ScrollLayoutProtocol"
        [
            ...
            test "content extent dependency keys change when content changes" {
                let baseline =
                    Layout.contentExtent 120.0 40.0 (Some(Feature151CorpusFixtures.dynamicContent 24.0))

                let changed =
                    Layout.contentExtent 120.0 40.0 (Some(Feature151CorpusFixtures.dynamicContent 72.0))

                Expect.notEqual
                    changed.DependencyKeys
                    baseline.DependencyKeys
                    "dynamic content changes intrinsic dependency identity"

                Expect.isGreaterThanOrEqual
                    changed.ContentHeight
                    baseline.ContentHeight
                    "changed content keeps or increases extent"
            }
