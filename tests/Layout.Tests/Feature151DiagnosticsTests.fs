...
let tests =
    testList
        "Feature151Diagnostics"
        [
            test "invalid available space fails closed with finite fallback bounds" {
                let item =
                    Feature151CorpusFixtures.allCases
                    |> List.find (fun item -> item.CaseId = "invalid-available")
                ...
                for nodeId in item.RequiredNodeIds do
                    Expect.isTrue
                        (Feature151CorpusFixtures.finiteBounds (Feature151CorpusFixtures.boundsOf nodeId result))
                        $"{nodeId} fallback finite"
            }

            test "contradictory min and max size is diagnostic rather than misleading" {
                let item =
                    Feature151CorpusFixtures.allCases
                    |> List.find (fun item -> item.CaseId = "contradictory-size")
                ...
                Expect.isTrue
                    (Feature151CorpusFixtures.finiteBounds (
                        Feature151CorpusFixtures.boundsOf "contradictory-size" result
                    ))
                    "fallback bounds"
            }
            ...
            test "duplicate participant ids are visible in evaluate and measureProtocol evidence" {
                let item =
                    Feature151CorpusFixtures.allCases
                    |> List.find (fun item -> item.CaseId = "duplicate-node")

                let result = Feature151CorpusFixtures.resultOf item

                let measured =
                    Layout.measureProtocol (Feature151CorpusFixtures.constraintsFor item) item.Root

                expectDiagnostic DuplicateLayoutNodeId result "duplicate node diagnostic"

                Expect.exists
                    measured.Diagnostics
                    (fun diagnostic -> diagnostic.Code = DuplicateMeasurement)
            ...
            test "unsupported intrinsic query rejects participant mismatch" {
                ...
                Expect.exists
                    result.Diagnostics
                    (fun diagnostic -> diagnostic.Code = UnsupportedIntrinsicQuery)
