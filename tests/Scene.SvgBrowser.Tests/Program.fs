...
let rectangle x y width height value =
    SceneNode.Rectangle((x, y, width, height), value)
...
let overlay =
    {
        ...
        Content =
            sceneOf
                [
                    SceneNode.SizedText((4.0, 92.0), "SVG foundation", 8.0, color 30uy 30uy 30uy)
                ]
    }
...
let documentContainer: HTMLElement =
    unbox (document.getElementById ("document-fixture"))

let duplicateContainer: HTMLElement =
    unbox (document.getElementById ("duplicate-fixture"))

let mutable documentHost: SvgDocumentBrowserHost option = None

let performanceContainer: HTMLElement =
    unbox (document.getElementById ("performance-fixture"))
...
let visiblePerformanceIndices kind =
    let index =
        if kind = "extent-far" then
            farSpatialIndex.Value
        else
            nearSpatialIndex.Value
...
let animationClip () =
    {
        ...
        Tracks =
            [
                ...
                [
                    ...
                    {
                        Time = TimeSpan.FromMilliseconds 200.0
                        Value = ScalarValue 40.0
                        EasingToNext = Linear
                    }
                ]
                Opacity,
        ...
        Cues =
            [
                {
                    Id = "midpoint"
                    Time = TimeSpan.FromMilliseconds 100.0
                    Payload = "sfx.midpoint"
                }
            ]
        Loop = Once
    }
...
let animationCallbacks =
    {
        ApplySample =
            fun id authority revision sample ->
        ...
        DispatchCues =
            fun id authority cues ->
                for cue in cues do
                    animationEvents.Add($"cue:{id}:{authority}:{cue.Cue.Id}:{cue.Iteration}")
...
let sessionCallbacks =
    {
        ...
        ApplyProjection =
            fun revision projection ->
                host.Value.Dispatch(
                    RetainedInteractionMessage.ReplaceScene(retained (int revision) (20.0 + float revision))
                )
                |> ignore
...
let sessionObservation () =
    ...
    createObj
        [
            ...
            "status" ==> string value.Status
            "revision"
            ==> (value.LastProjectionRevision |> Option.map (float >> box) |> Option.toObj)
            "listeners" ==> value.OwnedListenerCount
            "frames" ==> value.ScheduledFrameCount
...
let stateObject () =
    ...
    createObj
        [
            ...
            ==> createObj
                    [
                        "panX" ==> value.Scene.Camera.PanX
                        "panY" ==> value.Scene.Camera.PanY
                        "zoom" ==> value.Scene.Camera.Zoom
                    ]
        ]
...
let changedDocument () =
    let replace (element: SvgElement) =
        if element.Id = "luminance-element" then
            { element with
                Transform = SvgAffine.translate 3.0 0.0
            }
        else
            element
...
let performanceDocument kind selection cameraStep changedObject =
    ...
    let gradient =
        {
            ...
            Stops =
                [
                    {
                        Offset = 0.0
                        Color = color 36uy 116uy 214uy
                        StopOpacity = 1.0
                    }
                    {
                        Offset = 1.0
                        Color = color 38uy 190uy 132uy
                        StopOpacity = 1.0
                    }
                ]
            InheritFrom = None
    ...
    let symbolChild =
        leaf
            "shared-symbol-dot"
            {
                Nodes = [ SceneNode.Circle(point 3.0 3.0, 3.0, color 250uy 190uy 40uy) ]
            }
        ...
        { gradient with
            Stops =
                [
                    {
                        Offset = 0.0
                        Color = color (byte (40 + index % 180)) 90uy 180uy
                        StopOpacity = 1.0
                    }
                    {
                        Offset = 1.0
                        Color = color 30uy (byte (60 + index % 180)) 120uy
                        StopOpacity = 1.0
                    }
                ]
        }
    ...
    let definitions =
        ...
        @ [
            {
                Id = "shared-symbol"
                Content = SvgDefinitionContent.Symbol(Some(rect 0.0 0.0 6.0 6.0), [ symbolChild ])
            }
        ]
        @ (if kind = "dense" then
               [
                       ...
                       yield
                           {
                               ...
                               Content =
                                   SvgDefinitionContent.Clip(
                                       ...
                                       [ SvgClipShape.Rectangle(rect 0.0 0.0 1000.0 1000.0) ]
                                   )
                           }

                       let maskChild =
                           leaf
                               $"dense-mask-child-{index}"
                               {
                                   Nodes = [ SceneNode.Rectangle((0.0, 0.0, 1.0, 1.0), color 255uy 255uy 255uy) ]
                               }
    ...
    let pathAt x y =
        {
            Commands =
                ...
                :: [
                    for segment in 1..12 ->
                        PathCommand.LineTo(point (x + float segment * 2.0) (y + float (segment % 3)))
                ]
    ...
    let objectElement index =
        ...
        let shape =
            leaf
                $"object-{index}-shape"
                {
                    Nodes = [ SceneNode.Rectangle((x, y, shapeWidth, 15.0), color 70uy 130uy 220uy) ]
                }
        ...
        let path =
            leaf
                $"object-{index}-path"
                {
                    Nodes = [ SceneNode.Path(pathAt x (y + 18.0), pathPaint) ]
                }
        ...
        let label =
            leaf
                $"object-{index}-label"
                {
                    Nodes =
                        [
                            SceneNode.SizedText((x, y + 34.0), $"Unit {index}", 8.0, color 25uy 25uy 25uy)
                        ]
                }
...

        {
            ...
            Transform = SvgAffine.identity
            ClipId =
                if kind = "dense" && index < 10 then
                    Some($"dense-clip-{index}")
                else
                    None
            MaskId =
                if kind = "dense" && index >= 10 && index < 20 then
                    Some($"dense-mask-{index - 10}")
                else
                    None
            Presentation = Some presentation
            ...
            let galleryPath index =
...

                {
                    Commands =
                        PathCommand.MoveTo(point x y)
                        :: [
                            for segment in 1..256 ->
                                PathCommand.LineTo(point (x + float segment * 0.12) (y + float (segment % 7) * 0.25))
                        ]
                    FillType = PathFillType.Winding
                }
            ...
            let paths =
                List.init 200 (fun index ->
                    { leaf
                          $"gallery-path-{index}"
                          {
                              Nodes = [ SceneNode.Path(galleryPath index, pathPaint) ]
                          } with
                        MaskId = if index < 32 then Some($"gallery-mask-{index}") else None
                        Presentation =
                            Some
                                {
                                    FillSource = Some(SvgPaintSource.Definition $"gallery-gradient-{index % 64}")
                                    StrokeStyle = None
                                    OverallOpacity = 1.0
                                    FillRule = PathFillType.Winding
                                }
                    ...
                    leaf
                        $"gallery-text-{index}"
                        {
                            Nodes =
                                [
                                    SceneNode.SizedText(
                                        (float (index % 20) * 48.0, 365.0 + float (index / 20) * 12.0),
                                        $"G{index}",
                                        8.0,
                                        color 20uy 20uy 20uy
                                    )
                                ]
                        })
            ...
            [
                {
                    ...
                    MaskId = None
                    Presentation = None
                    Content = SvgElementContent.Group galleryChildren
                }
            ]
        else
            objects
        ...
        if kind = "dense" then
            ...
            [
                leaf
                    "selection-overlay"
                    {
                        Nodes = [ SceneNode.PaintedRectangle(rect 1.0 1.0 995.0 445.0, overlayPaint) ]
                    }
            ]
        else
            []
...
let replacePerformance () =
    performanceHost.Value.Replace(
        performanceDocument performanceKind performanceSelection performanceCameraStep performanceChangedObject
    )
    |> documentError
...
let mountPerformance kind =
    ...
    let candidate =
        performanceDocument kind performanceSelection performanceCameraStep performanceChangedObject
...
let excessivePerformanceDocument () =
    ...
    let excessive =
        { candidate with
            Children = [ for index in 0 .. SvgDocument.defaultLimits.MaxNodes -> empty index ]
        }
    ...
    createObj
        [
            "error" ==> error
            "rootRetained" ==> obj.ReferenceEquals(before, performanceHost.Value.Root)
        ]
...
let api =
    createObj
        [
            ...
            "replaceCurrent"
            ==> fun () ->
                host.Value.Dispatch(RetainedInteractionMessage.ReplaceScene(retained 2 28.0))
                |> resultObject
            "replaceReordered"
            ==> fun () ->
                        ...
                        Layers =
                            [
                                ...
                                }
                                {
                                    Id = "units"
                                    Visible = true
                                    Objects = [ beta; alpha 28.0 ]
                                }
                            ]
                    }

                host.Value.Dispatch(RetainedInteractionMessage.ReplaceScene reordered)
                |> resultObject
            "replaceStale"
            ==> fun () ->
                host.Value.Dispatch(RetainedInteractionMessage.ReplaceScene(retained 1 35.0))
                |> resultObject
            "hit" ==> fun x y -> host.Value.HitTest({ X = x; Y = y }) |> Option.toObj
            "scenePoint"
            ==> fun x y ->
                SvgRetained.tryToScenePoint host.Value.State.Scene.Camera { X = x; Y = y }
                |> Option.map (fun point -> createObj [ "x" ==> point.X; "y" ==> point.Y ])
                |> Option.toObj
            "zoomAt"
            ==> fun x y zoom -> host.Value.ZoomAt({ X = x; Y = y }, zoom) |> resultObject
            "panBy" ==> fun x y -> host.Value.PanBy({ X = x; Y = y }) |> resultObject
            "capture"
            ==> fun pointerId ->
                host.Value.Dispatch(
                    RetainedInteractionMessage.CapturePointer(host.Value.State.Scene.Revision, pointerId)
                )
                |> resultObject
            "observe"
            ==> fun () ->
                ...
                |> List.map (fun value ->
                    createObj
                        [
                            ...
                            "family" ==> value.Family
                            "ready" ==> value.Ready
                            "diagnostic" ==> (value.Diagnostic |> Option.toObj)
                        ])
                |> List.toArray
            "replaceInvalidDocument"
            ==> fun () -> documentHost.Value.Replace(invalidDocument ()) |> documentError
            "replaceReorderedDocument"
            ...
            "documentHit"
            ==> fun x y -> documentHost.Value.HitTest({ X = x; Y = y }) |> Option.toObj
            "duplicateDocumentMount"
            ==> fun () ->
                SvgBrowser.mountDocument duplicateContainer "gallery-browser" PortableDocumentFixture.document
                |> Result.map (fun value -> (value :> IDisposable).Dispose())
                |> documentError
            "performanceMount" ==> fun kind -> mountPerformance kind
            "performanceSelect"
            ==> fun index ->
                performanceSelection <- index
                replacePerformance ()
            "performanceCamera"
            ==> fun step ->
                performanceCameraStep <- step
                replacePerformance ()
            "performanceRevise"
            ==> fun index ->
                performanceChangedObject <- index
                replacePerformance ()
            ...
            ==> fun () ->
                let spatial =
                    if performanceKind = "extent-near" || performanceKind = "extent-far" then
                        ...
                        createObj
                            [
                                "visitedChunks" ==> result.VisitedChunkCount
                                "candidates" ==> result.CandidateCount
                                "totalEntries" ==> result.TotalEntryCount
                            ]
                    else
                        null
                ...
                createObj
                    [
                        ...
                        "objects"
                        ==> (if performanceKind = "dense" then 200
                             elif performanceKind = "gallery" then 400
                             else 100)
            ...
            "animationStartEssential"
            ==> fun (id: string) (revision: int) ->
                animationHost.Value.Start(
                    {
                        Id = id
                        AuthorityRevision = uint64 revision
                        Clip = animationClip ()
                ...
                animationObservation ()
            "animationStartContinuous"
            ==> fun () ->
                animationHost.Value.Start(
                    {
                        Id = "continuous"
                        AuthorityRevision = 7UL
                        Clip = continuousAnimationClip ()
                        Importance = Essential
                    }
                )

                animationObservation ()
            "animationStartDecorative"
            ==> fun (id: string) ->
                animationHost.Value.Start(
                    {
                        Id = id
            ...
            "animationPause"
            ==> fun () ->
                animationHost.Value.Pause()
                animationObservation ()
            "animationResume"
            ==> fun () ->
                animationHost.Value.Resume()
                animationObservation ()
            "animationSeek"
            ==> fun (id: string) (milliseconds: float) ->
                animationHost.Value.Seek(id, TimeSpan.FromMilliseconds milliseconds)
                animationObservation ()
            "animationMotion"
            ==> fun (reduced: bool) ->
                animationHost.Value.SetMotionPreference(
                    if reduced then
            ...
            "animationReplace"
            ==> fun (revision: int) ->
                animationHost.Value.ReplaceAuthority(uint64 revision)
                animationObservation ()
            "animationDispose"
            ==> fun () ->
                (animationHost.Value :> IDisposable).Dispose()
                animationObservation ()
            "sessionMount"
            ==> fun () ->
            ...
            "sessionPause"
            ==> fun () ->
                sessionHost.Value.Pause()
                sessionObservation ()
            "sessionResume"
            ==> fun () ->
                sessionHost.Value.Resume()
                sessionObservation ()
            "sessionStep"
            ==> fun () ->
                sessionHost.Value.StepOnce()
                sessionObservation ()
            "sessionReset"
            ==> fun () ->
                sessionHost.Value.Reset()
                sessionObservation ()
            "sessionReplace"
            ==> fun () ->
                sessionHost.Value.Replace()
                sessionObservation ()
            "sessionDemand"
            ==> fun () ->
                sessionHost.Value.DemandProjection()
                sessionObservation ()
            "sessionCompleteCurrent"
            ==> fun revision projection ->
