...
let eventObject =
    ...
    | BrowserPersistenceEvent.Persisted operation ->
        createObj
            [
                "kind" ==> "persisted"
                "generation" ==> float operation.Generation
                "operation" ==> float operation.Operation
            ]
    ...
    | BrowserPersistenceEvent.Exported value -> createObj [ "kind" ==> "exported"; "archive" ==> value ]
    ...
    | BrowserPersistenceEvent.Failed(operation, failure) ->
        ...
        createObj
            [
                ...
                "generation"
                ==> (operation
                     |> Option.map (fun value -> box (float value.Generation))
                     |> Option.toObj)
                "operation"
                ==> (operation
                     |> Option.map (fun value -> box (float value.Operation))
                     |> Option.toObj)
                "failure" ==> diagnostic
            ]
...
let api =
    createObj
        [
            "mount"
            ==> fun (databaseName: string) (maximum: int) -> mount databaseName maximum
            ...
            "load"
            ==> fun (familyId: string) (slot: string) ->
                host.Value.Load(
                    {
                        Family = family familyId
                        Slot = slot
            ...
            "importArchive"
            ==> fun (value: string) (generation: int) (operation: int) ->
                host.Value.ImportArchive(
                    {
                        Generation = uint64 generation
                        Operation = uint64 operation
                    },
