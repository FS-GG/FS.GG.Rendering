...
try {
  ...
  if(evidence.labels.join(",")!=="Rectangle,Ellipse,Polygon,Path,Undo,Redo,Apply translation"||evidence.created.Revision!==1||evidence.created.SelectionCount!==1||evidence.cancelled[0]!==evidence.cancelled[1]||!evidence.resource.embedded||evidence.resource.fonts!==1||evidence.resource.children!==1||!evidence.geometryCancelled||Object.values(geometry).some(value=>value.operations!==1||value.contours<1)||geometry.difference.contours<2||!font.ready||evidence.disposed!==0||evidence.disposedAgain!==0||evidence.roots!==0)throw new Error(JSON.stringify({evidence,font,geometry}));
