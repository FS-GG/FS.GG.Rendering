...
let tests =
    testList
        "Feature 247 Visibility helper (US1, FR-006/008/010/011)"
        [
            ...
            test "raySegment returns None for a wall behind the ray, off to the side, or collinear" {
                Expect.isNone
                    (Visibility.raySegment (p 0.0 0.0) (p 1.0 0.0) (seg -5.0 -5.0 -5.0 5.0))
                    "wall behind the origin"

                Expect.isNone
                    (Visibility.raySegment (p 0.0 0.0) (p 1.0 0.0) (seg 5.0 5.0 5.0 15.0))
                    "wall off to the side (u<0)"

                Expect.isNone
                    (Visibility.raySegment (p 0.0 0.0) (p 1.0 0.0) (seg 5.0 0.0 10.0 0.0))
                    "wall collinear with the ray (parallel, no NaN)"
            }
            ...
            test "a wall NOT between source and target does not block" {
                ...
                Expect.isTrue
                    (Visibility.isVisible (p 0.0 0.0) (p 4.0 0.0) [ seg 5.0 -5.0 5.0 5.0 ])
                    "wall past the target does not block"
            }
            ...
            test "polygon is deterministic: repeated runs are byte-identical" {
                ...
                let run () =
                    Visibility.polygon (settings 20.0) src walls
            ...
            test "empty and zero-length segments are total and never throw" {
                ...
                Expect.isTrue
                    (Visibility.isVisible src (p 4.0 0.0) [ seg 2.0 0.0 2.0 0.0 ])
                    "zero-length wall occludes nothing"
            ...
            test "source on a wall, non-finite input, and a non-positive radius are total" {
                ...
                let nanSeg =
                    Visibility.polygon (settings 10.0) (p 0.0 0.0) [ { A = p (0.0 / 0.0) 0.0; B = p 5.0 5.0 } ]

                Expect.isNonEmpty nanSeg.Vertices "a non-finite occluder is filtered; sweep still totals"

                Expect.all
                    nanSeg.Vertices
                    (fun v -> System.Double.IsFinite v.X && System.Double.IsFinite v.Y)
                    "no NaN coordinate leaks into the polygon"
                ...
                Expect.isFalse
                    (Visibility.isVisible (p 0.0 0.0) (p (0.0 / 0.0) 0.0) [])
                    "non-finite target is not visible (no throw)"
            }
        ]
