...
let private update (event: GameEvent) : PersistenceEffect =
    match event with
    | CheckpointReached score ->
        Persistence.save (Persistence.saveEnvelope 1 (SaveSlot "slot-1") (sprintf "{score:%d}" score))
...
let tests =
    testList
        "Feature 244 Persistence request surface + record-only interpreter (US1, US2)"
        [
            ...
            test "saveEnvelope carries the opaque payload verbatim (US1)" {
                let env =
                    Persistence.saveEnvelope 3 (SaveSlot "autosave") "<opaque\tbytes\n{not:parsed}>"

                Expect.equal
                    env.Payload
                    (SavePayload "<opaque\tbytes\n{not:parsed}>")
                    "payload stored byte-for-byte, never parsed"
            ...
            test "clampVersion is total (US1, Principle VI)" {
                ...
                Expect.equal
                    (Persistence.clampVersion -100)
                    Persistence.minVersion
                    "under-min -> minVersion, no exception"
            }

            testCase "clampVersion result is always >= minVersion (FsCheck >=1000 cases)"
            <| fun () ->
            ...
            test "interpretRecordOnly normalizes an out-of-range Save version in recorded evidence (US2)" {
                ...
                Expect.equal
                    ...
                    [
                        Save
                            {
                                Version = Persistence.minVersion
                                Slot = SaveSlot "s"
                                Payload = SavePayload "raw"
                            }
                    ]
            ...
            test "Load/DeleteSlot of an unknown slot is a well-defined recorded no-op, not an error (US2)" {
                ...
                let ev =
                    Persistence.interpretRecordOnly
                        [
                            Persistence.load (SaveSlot "never-saved")
                            Persistence.deleteSlot (SaveSlot "empty")
                        ]

                Expect.equal
                    ev.Requested
                    [ Load(SaveSlot "never-saved"); DeleteSlot(SaveSlot "empty") ]
                    "unknown-slot load/delete records without error"
            }
            ...
            test "every evidence value the framework produces is marked RecordOnly (#537)" {
                let ev =
                    Persistence.interpretRecordOnly
                        [ Persistence.load (SaveSlot "s"); Persistence.deleteSlot (SaveSlot "s") ]
            ...
            test "interpretRecordOnly persists nothing — the evidence is the request, and that is all (#445)" {
                ...
                let ev =
                    Persistence.interpretRecordOnly [ Persistence.save (Persistence.saveEnvelope 1 slot "payload") ]
                ...
                Expect.equal
                    reloaded.Requested
                    [ Load slot ]
                    "loading the slot just saved yields only the REQUEST — no payload, because no save occurred"
            }
            ...
            testCase
                "interpretRecordOnly preserves count, normalizes versions, carries payloads verbatim (FsCheck >=500 cases)"
            <| fun () ->
