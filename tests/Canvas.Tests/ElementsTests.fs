...
let tests =
    testList
        "Feature 191 Elements purity + composition (US3, FR-008)"
        [
            ...
            test "sprite is pure and sized from the local origin" {
                Expect.equal
                    (Elements.sprite "ball.png" 8.0 8.0)
                    (Elements.sprite "ball.png" 8.0 8.0)
                    "sprite is deterministic"
            ...
            test "polyline connects its points; degenerate inputs collapse to empty" {
                ...
                | [ Path(spec, _) ] -> Expect.equal (List.length spec.Commands) 3 "one MoveTo + two LineTo"
                | other -> failtestf "expected a single Path, got %A" other

                Expect.equal
                    (Elements.polyline [ { X = 1.0; Y = 1.0 } ] paint)
                    Scene.empty
            ...
            test "layer groups sub-scenes in paint order" {
                let scene =
                    Elements.layer [ Elements.rect 2.0 2.0 paint; Elements.at 5.0 5.0 (Elements.circle 1.0 red) ]
            ...
            test "cached wraps a fragment as a replay boundary keyed on identity + content" {
                ...
                | [ CachedSubtree b ] ->
                    Expect.equal b.Scene frag "the wrapped scene is transparent (recurses into content)"
                ...
                let fpOf (s: Scene) =
                    match s.Nodes with
                    | [ CachedSubtree b ] -> b.Fingerprint
                    | _ -> 0UL

                let changed = Elements.layer [ Elements.rect 3.0 2.0 paint ]

                Expect.notEqual
                    (fpOf (Elements.cached "hud" frag))
                    (fpOf (Elements.cached "hud" changed))
                    "changed content ⇒ changed fingerprint"
                // A different key under identical content takes a distinct cache slot.
                let idOf (s: Scene) =
                    match s.Nodes with
                    | [ CachedSubtree b ] -> b.CacheId
                    | _ -> 0UL

                Expect.notEqual
                    (idOf (Elements.cached "hud" frag))
                    (idOf (Elements.cached "bg" frag))
                    "distinct keys ⇒ distinct cache slots"
            }
            ...
            test "cached fingerprint is paint-aware: paint-only changes flip the fingerprint (#45)" {
                ...
                let cachedFp (node: SceneNode) =
                    fpOf (Elements.cached "k" { Nodes = [ node ] })

                let flips label a b =
                    Expect.notEqual (cachedFp a) (cachedFp b) label
                ...
                flips
                    "line paint change ⇒ fingerprint change"
                    (Line(a, z, Paint.fill red))
                    (Line(a, z, Paint.fill blue))
                ...
                let dashed =
                    { (Paint.stroke red 1.0) with
                        PathEffect = Dash([ 4.0; 2.0 ], 0.0)
                    }

                flips
                    "path dash change ⇒ fingerprint change"
                    (Path(square, Paint.stroke red 1.0))
                    (Path(square, dashed))
