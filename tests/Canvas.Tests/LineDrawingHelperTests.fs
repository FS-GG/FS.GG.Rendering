...
let tests =
    testList
        "Feature 248 LineDrawing helper (US1, FR-006/008/010/011)"
        [
            ...
            test "the endpoints themselves are never tested (you can look FROM and AT an opaque tile)" {
                ...
                Expect.isTrue
                    (LineDrawing.lineOfSight (fun x -> x <> a) a b)
                    "standing on an opaque tile still sees out"
            }
            ...
            test "line and supercover are deterministic: repeated runs are byte-identical" {
                for b in targets do
                    ...
                    Expect.equal
                        (LineDrawing.supercover (c 0 0) b)
                        (LineDrawing.supercover (c 0 0) b)
                        "supercover identical on replay"
            }
            ...
            test "start = goal returns the single start cell and is total" {
                ...
                Expect.isTrue
                    (LineDrawing.lineOfSight (fun _ -> false) (c 2 3) (c 2 3))
                    "lineOfSight a=a -> true even with an all-opaque map"
            }
            ...
            test "axis-aligned and pure-diagonal lines are exact and never throw" {
                ...
                Expect.equal
                    (LineDrawing.line (c 0 0) (c 3 3))
                    [ c 0 0; c 1 1; c 2 2; c 3 3 ]
                    "diagonal run (thin, diagonal-connected)"
            ...
            test "an always-true and always-false predicate are both total" {
                ...
                Expect.isFalse
                    (LineDrawing.lineOfSight (fun _ -> false) (c 0 0) (c 5 5))
                    "fully opaque interior -> not visible"
            }
            ...
            test "the emitted cell count is bounded by the endpoint separation (finite)" {
                for b in targets do
                    ...
                    Expect.equal
                        (List.length sc)
                        (abs b.Col + abs b.Row + 1)
                        (sprintf "supercover length is Manhattan+1 for b=%A" b)
            }
            ...
            test "large coordinates stay total and correct (delta/error arithmetic is int64, not int32)" {
                ...
                |> List.iter (fun (p, q) ->
                    Expect.equal
                        (abs (p.Col - q.Col) + abs (p.Row - q.Row))
                        1
                        "supercover stays strictly 4-connected at scale")
            }
        ]
