...
let private segOf (a: Vec2) (b: Vec2) : Visibility.Segment = { A = toSimPoint a; B = toSimPoint b }
        ...
        [
            ...
            test "a Vec2-modelled pair reaches Collision.contact, and the push-out reads back as a Vec2" {
                ...
                | Some c ->
                    ...
                    Expect.equal
                        (ofSimPoint c.A.Velocity)
                        (vec2 1.5 -2.5)
                        "A's velocity round-trips through Body.Velocity"
            }
            ...
            test "a full Vec2 -> sim -> Vec2 round trip is byte-identical across runs" {
                let step () =
                    let bodies =
                        [
                            bodyOf (entity 0.0 0.0 1.5 -2.5 10.0 1)
                            bodyOf (entity 6.0 0.0 0.0 0.0 10.0 2)
                        ]
                    ...
                    let poly =
                        Visibility.polygon
                            (sight 20.0)
                            (toSimPoint (vec2 3.0 -7.0))
                            [ segOf (vec2 8.0 -12.0) (vec2 8.0 -2.0) ]
