...
let tests =
    testList
        "Feature 246 Collision helper (US1, FR-002/006/008/010)"
        [
            ...
            test "resolve SeparateEqually removes the overlap (bodies no longer intersect)" {
                ...
                | Some c ->
                    ...
                    Expect.isFalse
                        (Geometry.intersects r.A.Bounds r.B.Bounds)
                        "separated pair does not overlap (touching is not intersect)"
            ...
            test "Bounce records a clamped restitution; Slide records none" {
                let c =
                    (Collision.contact (body 0.0 0.0 10.0 10.0 1) (body 6.0 0.0 10.0 10.0 2)).Value

                Expect.floatClose Accuracy.high (Collision.resolve (Collision.Bounce 50) c).Restitution 0.5 "50% -> 0.5"

                Expect.floatClose
                    Accuracy.high
                    (Collision.resolve (Collision.Bounce 250) c).Restitution
                    1.0
                    "over-100% clamps to 1.0"

                Expect.floatClose
                    Accuracy.high
                    (Collision.resolve Collision.Slide c).Restitution
                    0.0
                    "slide records no restitution"
            }
            ...
            test "collide is deterministic: repeated runs are byte-identical" {
                ...
                let step () =
                    Collision.step Collision.SeparateEqually 16.0 bodies
            ...
            test "contacts are emitted in ascending (i, j) index order" {
                ...
                let bodies =
                    [ body 0.0 0.0 10.0 10.0 1; body 3.0 0.0 10.0 10.0 2; body 6.0 0.0 10.0 10.0 3 ]
            ...
            test "no false negative: a distant-but-overlapping large body is still found" {
                ...
                let found =
                    Collision.collide 8.0 [ big; small ] |> List.map (fun c -> c.A.Tag, c.B.Tag)
            ...
            test "a fast round does not tunnel a thin target (the issue's 1200 u/s example)" {
                ...
                let endRound =
                    { round.Bounds with
                        X = round.Bounds.X + round.Velocity.X
                    }

                Expect.isFalse
                    (Geometry.intersects round.Bounds target.Bounds)
                    "start: the round is in front of the target"

                Expect.isFalse (Geometry.intersects endRound target.Bounds) "end: the round is past the target"

                let hits =
                    Collision.collide 16.0 [ round; target ] |> List.map (fun c -> c.A.Tag, c.B.Tag)
            ...
            test "slideCircle is deterministic: repeated runs are byte-identical" {
                ...
                let run () =
                    Collision.slideCircle bounds walls start { X = 40.0; Y = 12.0 }
            ...
            test "a fast mover is kept from tunnelling by sub-stepping (each chunk <= the radius)" {
                ...
                Expect.floatClose
                    Accuracy.high
                    (stepped.Center.X + stepped.Radius)
                    wall.X
            ...
            test "slideCircle is total on non-finite displacement and radius (never throws)" {
                ...
                Expect.isTrue
                    (System.Double.IsFinite out.Center.X && System.Double.IsFinite out.Center.Y)
                ...
                let bad =
                    Collision.slideCircle None [ wall ] (circle 80.0 50.0 nan) { X = 40.0; Y = 0.0 }
