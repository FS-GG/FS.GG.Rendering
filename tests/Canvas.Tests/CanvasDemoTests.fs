...
let private render (b: Ball) : Scene =
    Elements.layer
        [
            Elements.at (b.X - radius) (b.Y - radius) (Elements.circle radius (Colors.rgb 60uy 160uy 240uy))
        ]
...
let private run (seed: int) : StepState<Ball> * Scene * string =
    ...
    let final =
        script
        |> List.fold (fun st ft -> Loop.advance dt integrate ft st) (Loop.init (seedWorld seed))
...
let tests =
    testList
        "Feature 191 seeded-sample reproducibility (US3, FR-014/SC-006)"
        [
