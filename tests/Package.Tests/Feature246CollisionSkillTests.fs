...
let private skillBody =
    File.ReadAllText(repositoryPath "template/product-skills/fs-gg-collision/SKILL.md")

let private helperSource =
    repositoryPath "template/fragments/collision/src/Product/Collision.fs"
        ...
        [
            ...
            test "Product.fsproj compiles Collision.fs profile-gated and Exists-guarded (delete-safe)" {
                ...
                Expect.isLessThan
                    ...
                    "Collision.fs compiles before Model.fs (usable from update)"

                Expect.isGreaterThan
                    (idx "Collision.fs")
                    (idx "profile == \"game\" || profile == \"sample-pack\"")
                    "Collision.fs is inside the sim-profile gate"
            }

            test "both template.json sources (skill + fragment) are gated to game/sample-pack" {
                Expect.equal
                    (sourceCondition "template/product-skills/fs-gg-collision/")
                    (Some simGate)
                    "skill source gated to sim profiles"

                Expect.equal
                    (sourceCondition "template/fragments/collision/src/")
                    (Some simGate)
