...
let generatedInteractiveHostContract =
    testList
        "issue-1012 generated interactive host contract"
        [
            test "template host keeps geometry, both key edges, held ticks, and runtime settings on their owning seams" {
                ...
                Expect.stringContains
                    ...
                    "default and flagged launch behavior comes from the same parsed safe-default overlay"

                Expect.isTrue
                    (System.Text.RegularExpressions.Regex.IsMatch(
                        program,
                        "runInteractiveAppWithWindowBehaviorAndAudio\\s+viewerOptions\\s+launchRequest"
                    ))
                    "default and flagged game launches select one explicit overload"

                Expect.stringContains
                    host
                    "ApplyLogicalCanvas(AppRoot.GameShell.logicalSize settings)"
                    "DisplayChanged reaches the dynamic viewer-owned logical canvas"

                Expect.stringContains
                    host
                    "LogicalSize = Some { Width = 1280; Height = 720 }"
                    "the generated game seeds the initial logical canvas"
            }
            ...
            test "generated-product behavior proves retained pointer and down-ticks-up through InteractiveAppHost" {
                ...
                |> List.iter (fun token ->
                    Expect.stringContains behavior token $"generated resolution proof carries `{token}`")
            }
            ...
            test "release lane instantiates the packed game profile and runs its emitted tests" {
                ...
                let gameStart =
                    release.IndexOf("- name: Instantiate + build the game profile", System.StringComparison.Ordinal)
                ...
                Expect.stringContains
                    block
                    "dotnet new fs-gg-ui --name GameProduct --profile game"
                    "release lane instantiates the packed game profile"

                Expect.stringContains
                    block
                    "dotnet test \"$work/GameProduct\""
                    "release lane runs the emitted host acceptance tests"
            }
            ...
            test "packed-template pre-publish probe restores the staged framework set" {
                ...
                let probeStart =
                    release.IndexOf(
                        "- name: Packed template clean-checkout FSI contract",
                ...
                let source =
                    block.IndexOf(
                        "dotnet nuget add source \"$(pwd)/artifacts/packages\"",
                        System.StringComparison.Ordinal
                    )

                let scaffold =
                    block.IndexOf("dotnet new fs-gg-ui --name \"$name\"", System.StringComparison.Ordinal)
                ...
                Expect.isGreaterThan
                    scaffold
                    source
                    "the staging source is configured before the generated product restores"

                Expect.stringContains
                    block
                    "--configfile \"$user_config\""
                    "the /tmp product can see the staging source"
            }
        ]
