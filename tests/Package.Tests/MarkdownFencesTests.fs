    ...
    testList
        ...
        [
            ...
            test "code and prose partition the document, and never overlap" {
                ...
                let code =
                    MarkdownFences.fsharpLines scanned |> List.map (fun l -> l.Number) |> Set.ofList

                let text =
                    MarkdownFences.proseLines scanned |> List.map (fun l -> l.Number) |> Set.ofList
            ...
            test "an unclosed fence is reported, at any indent and either character" {
                ...
                Expect.isTrue
                    (scan "- like so:\n\n      ```fsharp\n      let x = 1\n").UnclosedFence
                    "a list-indented fence that never closes is not"

                Expect.isTrue
                    (scan "~~~fsharp\nlet x = 1\n").UnclosedFence
                    "...and a tilde fence is held to the same rule"

                Expect.isTrue
                    (scan "```fsharp\nlet x = 1\n~~~\n").UnclosedFence
                    "a ~~~ does not close a ``` — so this file is still open"
            }
