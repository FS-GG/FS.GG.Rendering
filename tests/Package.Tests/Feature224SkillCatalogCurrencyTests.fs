...
let private exemptionReason id =
    ...
    |> List.tryPick (fun (exemptId, reason) ->
        if exemptId = id && not (String.IsNullOrWhiteSpace reason) then
            Some reason
        else
            None)
                ...
                Some
                    {
                        ...
                        Doc = r.Doc
                        Line = r.Line
                        Message = sprintf "%s:%d  '%s' → no SKILL.md with name: %s in package" r.Doc r.Line r.Id r.Id
                    })
...
let private formatFindings (findings: Finding list) =
    findings |> List.map (fun f -> "  " + f.Message) |> String.concat "\n"
...
let tests =
    testList
        ...
        [
            ...
            test
                "deliberate dangling-id regression: inject → exactly one finding, real docs → none (SC-003 both directions)" {
                let resolvable = resolvableIds ()
            ...
            test "no silent exemption: a reason-less allowlist entry still fails (FR-009)" {
                ...
                let exempted id =
                    ...
                    |> List.tryPick (fun (i, r) ->
                        if i = id && not (String.IsNullOrWhiteSpace r) then
                            Some r
                        else
                            None)

                Expect.isNone
                    (exempted "fs-gg-does-not-exist")
                    "a whitespace-only reason does not exempt an unresolved id"
            }
