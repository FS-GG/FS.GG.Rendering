...
let nameCollisionSafetyTests =
    testList
        "Scene and Controls name collision inventory"
        [
            ...
            test "identified overlaps include text, geometry, color, event, and builder authoring names" {
                ...
                |> List.iter (fun name -> Expect.stringContains report name $"collision inventory includes {name}")
            }
            ...
            test "collision guidance uses explicit Scene and Controls qualifications" {
                ...
                |> List.iter (fun sample -> Expect.stringContains report sample $"collision guidance includes {sample}")
            }
