...
let private nativeSources =
    [
        ...
        "fs-gg-game-shell", "template/product-skills/fs-gg-game-shell/SKILL.md"
...
let private readManifest () =
    Expect.isTrue
        ...
        (sprintf "manifest missing at %s — run dotnet fsi scripts/generate-skill-manifest.fsx" manifestPath)
...
let private pathTokens (body: string) =
    [
        for m in System.Text.RegularExpressions.Regex.Matches(body, "`([^`\n]+)`") do
            ...
            if
                not isPlaceholder
                && not isProductConvention
                && (looksRooted || token.StartsWith "../")
            then
                yield token
    ]
...
let private resolves (skillId: string) (skillRoot: string) (containingFile: string) (token: string) =
    ...
    if token.StartsWith "../" then
        ...
        let containingDir =
            Path.GetDirectoryName containingFile
            |> Option.ofObj
            |> Option.defaultValue skillRoot
        ...
        if segments.[0] = ".." then
            ...
            let onDisk =
                Path.Combine(skillRoot, insideSkill.Replace('/', Path.DirectorySeparatorChar))
    ...
    testList
        ...
        [
            ...
            test "G-MANIFEST manifest conforms to schema v2 complete per-file shape, order, scope, paths" {
                ...
                Expect.equal
                    (entries |> List.map (fun e -> e.Id))
                    (entries |> List.map (fun e -> e.Id) |> List.sort)
                    "entries sorted by id (deterministic manifest)"

                for e in entries do
                    Expect.equal
                        e.Scope
                        "product"
                        (sprintf "%s: provider manifest carries scope 'product' only (dev surface never ships)" e.Id)

                    Expect.equal
                        e.ResolvablePath
                        (sprintf ".agents/skills/%s/SKILL.md" e.Id)
                        (sprintf "%s: resolvablePath is the provider-source-root skill path" e.Id)

                    Expect.isTrue
                        (e.Sha256.Length = 64
                         && e.Sha256 |> Seq.forall (fun c -> Char.IsAsciiDigit c || (c >= 'a' && c <= 'f')))
                        (sprintf "%s: sha256 is 64-char lowercase hex" e.Id)

                    Expect.isTrue
                        (e.Files
                         |> List.exists (fun (path, digest) -> path = "SKILL.md" && digest = e.Sha256))
                        (sprintf "%s: files preserves the canonical SKILL.md digest" e.Id)
            }
            ...
            test "G-MANIFEST digests are fresh against the canonical bodies (content-addressed, ADR-0014 §1/§3)" {
                ...
                Expect.equal
                    (entries |> List.map (fun e -> e.Id) |> Set.ofList)
                    (canonicalSources |> List.map fst |> Set.ofList)
                    "manifest ids equal the declared catalog"

                for id, source in canonicalSources do
                    let body = File.ReadAllText(repositoryPath source)

                    Expect.equal
                        (Map.find id byId).Sha256
                        (sha256Text body)
                        (sprintf
                            "%s: manifest sha256 stale vs %s — run dotnet fsi scripts/generate-skill-manifest.fsx"
                            id
                            source)

                    let directory =
                        Path.GetDirectoryName(repositoryPath source)
                        |> Option.ofObj
                        |> Option.defaultWith (fun () -> failwith "canonical source has no directory")

                    let actual =
                        Directory.GetFiles(directory, "*", SearchOption.AllDirectories)
                        |> Array.map (fun path ->
                            Path.GetRelativePath(directory, path).Replace(Path.DirectorySeparatorChar, '/'),
                            sha256Text (File.ReadAllText path))
                        |> Array.sortBy fst
                        |> Array.toList

                    Expect.equal
                        (Map.find id byId).Files
                        actual
                        (sprintf "%s: schema-v2 files closes every source byte" id)
            }
            ...
            test "G-MANIFEST catalog is coherent with the template.json emission rows" {
                ...
                Expect.equal
                    emittedIds
                    (nativeSources |> List.map fst |> Set.ofList)
                    "every native skill has an emission row and package-only delivery skills remain external"
            }
            ...
            test "G-PARITY constants: agentSkillRoots and providerSourceRoot match the library" {
                Expect.equal
                    FsGg.Vendored.SkillMirror.agentSkillRoots
                    Fsgg.Schemas.agentSkillRoots
                    "vendored AGENT_SKILL_ROOTS equals Fsgg.Schemas.agentSkillRoots"

                Expect.equal
                    FsGg.Vendored.SkillMirror.providerSourceRoot
                    Fsgg.SkillMirror.providerSourceRoot
                    "vendored providerSourceRoot equals the library's"
            }

            test
                "G-PARITY sha256 / skillPath / skillIdOfPath / mirrorTargetRoots / retargetSkillPath agree on the corpus" {
                let bodies =
                    [
                        ""
                        "x"
                        "line1\nline2\n"
                        "unicode ⚑ ünïcødé\r\nCRLF"
                        String.replicate 10000 "a"
                    ]

                for b in bodies do
                    Expect.equal
                        (FsGg.Vendored.SkillMirror.sha256 b)
                        (Fsgg.SkillMirror.sha256 b)
                        (sprintf "sha256 diverges for %A" b)

                let roots = [ ".claude"; ".codex"; ".agents"; ".other" ]

                for r in roots do
                    Expect.equal
                        (FsGg.Vendored.SkillMirror.skillPath r "fs-gg-scene")
                        (Fsgg.SkillMirror.skillPath r "fs-gg-scene")
                        "skillPath diverges"
                ...
                for p in paths do
                    Expect.equal
                        (FsGg.Vendored.SkillMirror.skillIdOfPath p)
                        (Fsgg.SkillMirror.skillIdOfPath p)
                        (sprintf "skillIdOfPath diverges for %A" p)
            ...
            test "G-PARITY mirror produces identical write plans (order, paths, bodies)" {
                ...
                for skills, roots in cases do
                    let vendored =
                        FsGg.Vendored.SkillMirror.mirror roots skills
                        |> List.map (fun w -> w.Path, w.Body)

                    let library =
                        Fsgg.SkillMirror.mirror roots skills |> List.map (fun w -> w.Path, w.Body)

                    Expect.equal vendored library (sprintf "mirror plan diverges for %A / %A" skills roots)
            }

            test
                "G-PARITY verify produces identical drift verdicts (missing / divergent / hash-mismatch / clean / no-digest)" {
                ...
                let expectedSpec =
                    [
                        "fs-gg-scene", "product", goodHash
                        "speckit-plan", "process", ""
                        "fs-gg-ghost", "product", goodHash
                    ]
                ...
                let vendoredDrift =
                    FsGg.Vendored.SkillMirror.verify
                        roots
                        (expectedSpec
                         |> List.map (fun (id, scope, h) ->
                             {
                                 FsGg.Vendored.SkillMirror.ExpectedSkill.Id = id
                                 Scope =
                                     (if scope = "product" then
                                          FsGg.Vendored.SkillMirror.Product
                                      else
                                          FsGg.Vendored.SkillMirror.Process)
                                 Sha256 = h
                             }))
                        (actualSpec
                         |> List.map (fun (r, id, b) ->
                             {
                                 FsGg.Vendored.SkillMirror.ActualCopy.Root = r
                                 Id = id
                                 Body = b
                             }))
                    |> List.map (fun d ->
                        ...
                        d.HashMismatchRoots,
                        (match d.Scope with
                         | FsGg.Vendored.SkillMirror.Product -> "product"
                         | _ -> "process"))
                         ...
                         |> List.map (fun (id, scope, h) ->
                             {
                                 ...
                                 Scope =
                                     (if scope = "product" then
                                          Fsgg.Schemas.SkillScope.Product
                                      else
                                          Fsgg.Schemas.SkillScope.Process)
                                 Sha256 = h
                             }))
                        (actualSpec
                         |> List.map (fun (r, id, b) ->
                             {
                                 Fsgg.SkillMirror.ActualCopy.Root = r
                                 Id = id
                                 Body = b
                             }))
                    |> List.map (fun d ->
                        ...
                        d.HashMismatchRoots,
                        (match d.Scope with
                         | Fsgg.Schemas.SkillScope.Product -> "product"
                         | _ -> "process"))

                Expect.equal
                    vendoredDrift
                    libraryDrift
                    "verify verdicts diverge between the vendored copy and the library"
                // sanity: the corpus actually exercises all three drift dimensions.
                Expect.equal
                    (vendoredDrift |> List.map (fun (id, _, _, _, _) -> id))
                    [ "fs-gg-ghost"; "fs-gg-scene"; "speckit-plan" ]
                    "expected drift set"
            }
            ...
            test "G-NODANGLE canonical fs-gg-* bodies carry no unresolvable path reference (env-free core)" {
                ...
                Expect.isEmpty
                    failures
                    (sprintf "dangling skill routes (audit F3 class): %s" (String.concat "; " failures))
            }
            ...
            test "G-NODANGLE the extractor flags the pre-231 wrapper shape (red-case proof)" {
                ...
                Expect.equal
                    (pathTokens wrapperBody)
                    [ "../../../src/Diagnostics/skill/SKILL.md" ]
                    "the 12-line wrapper's repo-internal route is extracted"

                let placeholderBody =
                    "See `src/<YourProduct>/<YourProduct>.fsproj` and record under `readiness/logs/`."
            ...
            test "G-NODANGLE normalizeRelative folds . and .. and keeps an uncancelled escape visible" {
                Expect.equal
                    (normalizeRelative ".agents/skills/x/reference/../SKILL.md")
                    ".agents/skills/x/SKILL.md"
                    "a .. cancels its parent"

                Expect.equal
                    (normalizeRelative ".agents/skills/x/./SKILL.md")
                    ".agents/skills/x/SKILL.md"
                    "a . is dropped"

                Expect.equal
                    (normalizeRelative ".agents/skills/x/../../../src/X")
                    "src/X"
                    "three .. reach the product root"

                Expect.equal
                    (normalizeRelative ".agents/skills/x/../../../../out")
                    "../out"
                    "an uncancelled .. survives"
                // an escape is the leading *segment* `..`, not any segment that starts with those two chars.
                Expect.equal
                    (normalizeRelative ".agents/skills/x/../../../..data/y")
                    "..data/y"
                    "a segment merely beginning with .. is not an escape"
            }
            ...
            test "G-TARGET Directory.Build.props carries the single FsGgMaterializeSkillRoots build step" {
                ...
                Expect.stringContains
                    props
                    ".specify/scripts/fs-gg/materialize-skill-roots.fsx"
                    "it is wired to the emitted vendored script"

                Expect.stringContains
                    props
                    "dotnet fsi"
                    "it invokes the script via dotnet fsi (SDK-bundled, no restore)"

                Expect.stringContains
                    props
                    "Exists('$(FsGgSkillMaterializeScript)')"
                    "it self-gates on the spec-kit-only script (never fires under sdd/none)"
