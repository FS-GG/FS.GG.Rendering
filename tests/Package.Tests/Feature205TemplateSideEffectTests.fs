...
let private postActions (doc: JsonDocument) =
    [
        for pa in doc.RootElement.GetProperty("postActions").EnumerateArray() ->
            let str name =
                tryProp pa name |> Option.map elemStr |> Option.defaultValue ""
    ...
    testList
        ...
        [
            ...
            test "GV-1 initGit opt-in symbol is present, bool, default false, and self-describing" {
                ...
                Expect.equal
                    (tryProp initGit "defaultValue" |> Option.map elemStr)
                    (Some "false")
                    "initGit defaults to false (side-effect-free)"

                let desc =
                    (tryProp initGit "description" |> Option.map elemStr |> Option.defaultValue "").ToLowerInvariant()
            ...
            test "GV-3 every process-spawning post-action is gated on initGit (no auto-run default)" {
                ...
                let runActions =
                    postActions doc |> List.filter (fun (_, a, _, _, _) -> a = RUN_PROCESSOR)

                Expect.isNonEmpty runActions "expected the opt-in Run post-actions to exist"

                for (cond, _, _, _, _) in runActions do
                    Expect.isTrue
                        (cond.Contains "initGit")
                        (sprintf "Run post-action must be gated on initGit; got condition %s" cond)
            ...
            test "GV-4 the initGit-gated actions retain the hardened safety guards" {
                ...
                Expect.isTrue
                    (gated.Length >= 2)
                    (sprintf "expected a Unix and a Windows gated action, found %d" gated.Length)
                ...
                for (_, _, _, hasManual, continueOnError) in gated do
                    Expect.isTrue
                        continueOnError
                        "gated action must keep continueOnError: true (cannot fail generation)"
                ...
                let allArgs =
                    gated |> List.map (fun (_, _, args, _, _) -> args) |> String.concat "\n"
            ...
            test "GV-6 no skipGitInit defensive flag survives in scripts or tests" {
                ...
                Expect.isEmpty
                    hits
                    (sprintf "no skipGitInit references may remain; found in: %s" (String.concat ", " hits))
            }
        ]
