...
let readValidationReport () =
    Expect.isTrue
        ...
        $"generated consumer validation report exists at {validationReportPath}"
...
let generatedConsumerValidationTests =
    testList
        "Generated clean package consumer validation"
        [
            ...
            test "validation diagnostics are actionable for restore, copied source, reflection, and compile failures" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains
                        report
                        required
                        $"generated consumer validation includes diagnostic {required}")
            }
        ]
