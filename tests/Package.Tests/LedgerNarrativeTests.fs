...
let private claimsByLedger =
    lazy
        (allLedgers
         |> List.map (fun l -> l, claimsIn (File.ReadAllText(repo l)))
         |> Map.ofList)
...
let private claimsOf (relative: string) =
    ...
    | None ->
        failwith
            $"{relative} was not discovered under tests/ — the glob and the narrating list disagree, which the discovery test below exists to catch."
    ...
    testList
        ...
        [
            ...
            test "the #690 sentence is caught — the exact prose that rode green" {
                let claims =
                    claimsIn
                        "# `$(FsGgUiVersion)` is 0.9.1, published FS.GG.UI.SkiaViewer 0.9.1 exports the case, and so M-MIR/TYPE takes"
                ...
                Expect.notEqual
                    claims.[0].Version
                    pin
                    "...and it does not equal the real pin, which is what reds the gate"
            }

            test "the `is now` spelling is caught too — both ledgers word it differently" {
                let claims =
                    claimsIn
                        "# spelling to rewrite them TO. `$(FsGgUiVersion)` is now 0.9.1 and the pinned packages export all four, so"
            ...
            test "a version is read WHOLE — 4 segments are not truncated, and 0.9.2 does not match inside 0.9.20" {
                let four =
                    claimsIn "# `$(FsGgUiVersion)` is 1.2.3.4 and the pinned packages export all four"
                ...
                let twenty =
                    claimsIn "# `$(FsGgUiVersion)` is 0.9.20, published FS.GG.UI.SkiaViewer 0.9.20 exports the case"
            ...
            test "prose ABOUT the property is not a claim about its value" {
                let claims =
                    claimsIn
                        "**Why `$(FsGgUiVersion)` is exempt from the staleness rule, and only that rule.** The 0.9.2 pin ..."
