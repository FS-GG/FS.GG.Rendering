    ...
    testList
        ...
        [
            test "Scene Controls and Testing expose retained inspection authoring names" {
                ...
                let rule: RetainedInspectionRule =
                    RetainedInspectionValidation.rule "dirty-region-unioned"

                let validation = RetainedInspectionValidation.validate artifact [ rule ] []

                let summary =
                    RetainedInspectionReadiness.aggregate "run" [ artifact ] [ validation ] [] [] []
                ...
                Expect.stringContains
                    (RetainedInspectionMarkdown.renderJson summary)
                    "\"overallStatus\""
                    "json renderer available"
