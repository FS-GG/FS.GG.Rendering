...
let visualCoverageTests =
    testList
        "Issue1071 production-bound visual coverage"
        [
            test "the product owns an independent typed inventory and the real view consumes its projection" {
                ...
                Expect.stringContains
                    ...
                    "DU enumeration cannot silently omit a new gameplay case"
                ...
                Expect.stringContains
                    ...
                    "product owns the state-to-scene projection"

                Expect.stringContains
                    project
                    "<Compile Include=\"GameplayVisualInventory.fs\" />"
                    "inventory compiles into production"

                Expect.stringContains
                    view
                    "GameplayVisualInventory.project model"
                    "the real view consumes the audited projection"
            }

            test
                "the generated gate starts from production inventory and checks registry plus observed production handles" {
                ...
                Expect.stringContains
                    ...
                    "shown handles resolve as exact element/handle pairs"

                Expect.stringContains
                    gate
                    "binding.RequiredStates"
                    "observations use per-element required runtime states"

                Expect.stringContains gate "View.view model" "evidence traverses the real view"

                Expect.stringContains
                    gate
                    "binding.Project model |> SceneCodec.export"
                    "distinctness is measured per element, not only on aggregate frames"
                ...
                Expect.stringContains
                    gate
                    "if List.isEmpty scene.Nodes"
                    "empty runtime scenes cannot count as observations"
            ...
            test "Rogue2-derived door/trapdoor regressions are durable" {
                ...
                Expect.stringContains
                    gate
                    "List.distinct sceneDigests"
                    "fixture proves required Rogue states are distinct"
            }
            ...
            test "both product skills require production audit plus an external exact-revision critic" {
                for path in
                    ...
                    Expect.stringContains
                        skill
                        "outside the authored tree"
                        $"{path} requires externally verifiable provenance"
            }
        ]
