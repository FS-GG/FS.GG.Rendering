...
let private auditJson root reportPath reportText status evidence =
    let report =
        Path.GetRelativePath(root, reportPath).Replace(Path.DirectorySeparatorChar, '/')
...
let private initFixtureRepository (root: string) =
    ...
    git root [ "config"; "user.email"; "fsgg-feedback-test@example.invalid" ]
    |> ignore
...
let private reportAt commit locator =
    validReport.Replace("commit: abc123", "commit: " + commit).Replace("file:readiness/build.log", locator)
    ...
    testList
        ...
        [
            test "canonical skill ships its deterministic helper" {
                let root =
                    Path.Combine(repositoryRoot, "template", "feedback-report", "skill", "scripts")
            ...
            test "packaged feedback skill validates file evidence from the report commit, not a dirty checkout" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-clean-head-" + Guid.NewGuid().ToString "N")
                ...
                try
                    ...
                    File.WriteAllText(
                        Path.Combine(root, "readiness", "render-baseline.json"),
                    ...
                    let goodExit, goodOutput, goodError =
                        runPackagedValidate committed "feedback/report.md" "feedback/report.audit.json"
                    ...
                    let untrackedReport =
                        reportAt reportHead "file:readiness/generated-performance.json"

                    File.WriteAllText(Path.Combine(untracked, "feedback", "report.md"), untrackedReport)

                    File.WriteAllText(
                        Path.Combine(untracked, "readiness", "generated-performance.json"),
                    ...
                    File.WriteAllText(
                        ...
                        auditJson
                            untracked
                            (Path.Combine(untracked, "feedback", "report.md"))
                            untrackedReport
                            "actionable"
                            [|
                                {|
                    ...
                    let untrackedExit, _, untrackedError =
                        runPackagedValidate untracked "feedback/report.md" "feedback/report.audit.json"

                    Expect.equal untrackedExit 1 "a locally generated but untracked artifact fails"

                    Expect.stringContains
                        untrackedError
                        "untracked at report head"
                        "the diagnostic distinguishes dirty generated evidence"
                    ...
                    File.WriteAllText(
                        ...
                        auditJson
                            ignored
                            (Path.Combine(ignored, "feedback", "report.md"))
                            ignoredReport
                            "actionable"
                            [|
                                {|
                                    locator = "file:readiness/ignored-performance.json"
                                    result = "verified"
                                    sha256 = Some(sha256Text "ignored")
                                |}
                            |]
                    )

                    let ignoredExit, _, ignoredError =
                        runPackagedValidate ignored "feedback/report.md" "feedback/report.audit.json"

                    Expect.equal ignoredExit 1 "an ignored artifact fails"

                    Expect.stringContains
                        ignoredError
                        "ignored at report head"
                        "the diagnostic distinguishes ignored evidence"
                    ...
                    File.WriteAllText(
                        ...
                        auditJson
                            absent
                            (Path.Combine(absent, "feedback", "report.md"))
                            absentReport
                            "actionable"
                            [|
                                {|
                                    locator = "file:readiness/absent-performance.json"
                                    result = "verified"
                                    sha256 = Some(sha256Text "absent")
                                |}
                            |]
                    )

                    let absentExit, _, absentError =
                        runPackagedValidate absent "feedback/report.md" "feedback/report.audit.json"

                    Expect.equal absentExit 1 "an absent artifact fails"

                    Expect.stringContains
                        absentError
                        "absent at report head"
                        "the diagnostic distinguishes an absent path"

                    let command = prepareClone "fsgg-feedback-command"

                    let generatorExit, _, generatorError =
                        runProcess command "dotnet" [ "fsi"; "readiness/generate-performance.fsx" ]

                    Expect.equal generatorExit 0 (sprintf "the clean-checkout generator runs: %s" generatorError)

                    let commandLocator =
                        "command:dotnet fsi readiness/generate-performance.fsx && inspect readiness/generated-performance.json"
                    ...
                    File.WriteAllText(
                        ...
                        auditJson
                            command
                            (Path.Combine(command, "feedback", "report.md"))
                            commandReport
                            "actionable"
                            [|
                                {|
                    ...
                    let commandExit, commandOutput, commandError =
                        runPackagedValidate command "feedback/report.md" "feedback/report.audit.json"

                    Expect.equal
                        commandExit
                        0
                        (sprintf "a command locator can describe generated performance evidence: %s" commandError)
                    ...
                    let nonGitReport =
                        reportAt "unresolvable-head" "file:readiness/render-baseline.json"
                    ...
                    File.WriteAllText(
                        ...
                        auditJson
                            nonGit
                            (Path.Combine(nonGit, "feedback", "report.md"))
                            nonGitReport
                            "actionable"
                            [|
                                {|
                                    locator = "file:readiness/render-baseline.json"
                                    result = "verified"
                                    sha256 = Some(sha256Text "local-only")
                                |}
                            |]
                    )

                    let nonGitExit, _, nonGitError =
                        runPackagedValidate nonGit "feedback/report.md" "feedback/report.audit.json"

                    Expect.equal nonGitExit 1 "an unknown Git workspace fails closed"

                    Expect.stringContains
                        nonGitError
                        "cannot establish Git workspace state"
                        "unknown is not treated as a clean checkout"
                finally
            ...
            test
                "validate emits a terminal PASS or FAIL verdict through tail while retaining stderr detail and exit codes" {
                let root =
                ...
                try
                    ...
                    File.WriteAllText(
                        validAuditPath,
                        auditJson root validReportPath committedReport "actionable" evidence
                    )
            ...
            test "complete actionable audit binds every finding and verified evidence digest" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-audit-" + Guid.NewGuid().ToString "N")
            ...
            test "stale non-ledger evidence remains fail-closed" {
                ...
                try
                    ...
                    Expect.exists
                        result.errors
                        (fun error -> error.Contains("evidence digest is stale"))
                        "ordinary evidence stays bound"
            ...
            test "workspace-relative evidence cannot escape through a symlink" {
                ...
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-symlink-" + Guid.NewGuid().ToString "N")

                let external =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-external-" + Guid.NewGuid().ToString "N")
                ...
                try
                    ...
                    let report =
                        validReport.Replace("file:readiness/build.log", "file:readiness/outside.log")
            ...
            test "null evidence locator is a validation error rather than an exception" {
                ...
                let errors = validateActionabilityAudit root reportPath validReport audit
            ...
            test "null status and result plus malformed digest all fail closed" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-malformed-" + Guid.NewGuid().ToString "N")
                ...
                try
                    ...
                    let audit =
                        (auditJson root reportPath validReport "actionable" evidence)
                            ...
                            .Replace($"\"sha256\":\"{expectedDigest}\"", "\"sha256\":\"not-a-digest\"")
            ...
            test "polished fact-free and circular finding stays incomplete" {
                let report =
                    validReport
                        ...
                        .Replace("file:readiness/build.log", "claim-only:no inspectable evidence")
            ...
            test "dead source locator and stale digest invalidate an actionable audit" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-dead-" + Guid.NewGuid().ToString "N")
                ...
                try
                    ...
                    let report =
                        validReport.Replace("file:readiness/build.log", "file:readiness/dead.log")
            ...
            test "an unrelated live audit locator cannot green the report's dead evidence" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-substitution-" + Guid.NewGuid().ToString "N")
            ...
            test "non-reproducing command cannot support actionable disposition" {
                let root = Path.GetTempPath()
                let reportPath = Path.Combine(root, "feedback", "report.md")
                let report = validReport.Replace("file:readiness/build.log", "command:dotnet test")
            ...
            test "critic dispositions preserve invented-root-cause and missed-dedupe findings honestly" {
                ...
                for status, result in [ "unsupported", "claim-only"; "duplicate", "verified" ] do
                    let report =
                        validReport.Replace("file:readiness/build.log", "issue:FS-GG/FS.GG.Rendering#24")
            ...
            test "loose finding and incomplete coverage fail" {
                let malformed =
                    validReport
                        .Replace("#### §4.1 Example friction", "- Filed #24: a loose unstructured finding")
                ...
                Expect.contains errors "coverage: missing surface 'worker-git-pr'" "missing coverage is rejected"
            }

            test "expected and observed fields that say the same thing fail" {
                let circular =
                    validReport.Replace("- **Expected:** one pass", "- **Expected:** two passes")
                            ...
                            findings =
                                [
                                    {|
                                        ...
                                        status = "incomplete"
                                        missingFacts =
                                            [| "user input/reproduction path"; "production route wiring evidence" |]
                                        checkedEvidence =
                                            [|
                                                {|
                                                    locator = "command:dotnet test"
                                                ...
                                                |}
                                            |]
                                        confidenceLimits = [| "green tests do not establish production reachability" |]
                                    |}
                                ]
                        |}
                    )
            ...
            test "checkpoint append is valid JSONL and preserves the complete event" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-test-" + Guid.NewGuid().ToString "N")
            ...
            test "valid zero-event activation receipt proves an exercised event-free cycle" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-activation-" + Guid.NewGuid().ToString "N")
                ...
                try
                    let path =
                        appendZeroEventActivation
                            root
                            "001-example"
                            [ "scaffold-onboarding"; "implementation-test-evidence"; "verify-ship-pr" ]
                            [ "command:dotnet test"; "file:readiness/ship-summary.json" ]
                    ...
                    Expect.equal (receipt.GetProperty("activationSchema").GetInt32()) 1 "receipt schema is explicit"
                    ...
                    Expect.equal (receipt.GetProperty("evidence").GetArrayLength()) 2 "activation evidence is retained"

                    Expect.isFalse (receipt.TryGetProperty("kind") |> fst) "event-only kind is not fabricated"
                finally
                    if Directory.Exists root then
                        Directory.Delete(root, true)
            }
            ...
            test "zero-event receipt rejects event-only and arbitrary private fields" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-strict-schema-" + Guid.NewGuid().ToString "N")
            ...
            test "missing checkpoint state is distinct from a verified zero-event receipt" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-missing-" + Guid.NewGuid().ToString "N")

                let errors = validateCheckpointState root "001-example"

                Expect.exists
                    errors
                    (fun error -> error.Contains("missing both checkpoint events and a zero-event activation receipt"))
            ...
            test "empty checkpoint JSONL cannot impersonate an intentional zero-event cycle" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-empty-" + Guid.NewGuid().ToString "N")
            ...
            test "checkpoint event state cannot escape the workspace through a symlink" {
                ...
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-event-link-" + Guid.NewGuid().ToString "N")

                let external =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-event-external-" + Guid.NewGuid().ToString "N")
                ...
                try
                    ...
                    File.CreateSymbolicLink(Path.Combine(directory, "001-example.jsonl"), externalPath)
                    |> ignore
            ...
            test "commit-time audit invalidation check is selective, deterministic, and fail-closed" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-invalidation-" + Guid.NewGuid().ToString "N")
                ...
                try
                    ...
                    File.WriteAllText(
                        Path.Combine(audits, "positive.audit.json"),
                        auditJson root reportPath validReport "actionable" (evidence "file:src/Changed.fs")
                    )

                    File.WriteAllText(
                        Path.Combine(audits, "negative.audit.json"),
                        auditJson root reportPath validReport "actionable" (evidence "file:src/Untouched.fs")
                    )

                    File.WriteAllText(
                        Path.Combine(audits, "rename.audit.json"),
                        auditJson root reportPath validReport "actionable" (evidence "file:src/Old.fs")
                    )

                    File.WriteAllText(
                        Path.Combine(audits, "copy.audit.json"),
                        auditJson root reportPath validReport "actionable" (evidence "file:src/Copied.fs")
                    )

                    File.WriteAllText(
                        Path.Combine(audits, "deleted.audit.json"),
                        auditJson root reportPath validReport "actionable" (evidence "file:src/Deleted.fs")
                    )
                    ...
                    Expect.equal
                        positive.invalidated.Head.report
                        "feedback/report.md"
                        "diagnostic names the merged report"

                    let renameAndDelete =
                        changedPathsFromNameStatus
                            "R100\tsrc/Old.fs\tsrc/Renamed.fs\nC100\tsrc/Source.fs\tsrc/Copied.fs\nD\tsrc/Deleted.fs\n"
                    ...
                    let largeNameStatus =
                        String.replicate 20000 "M\tsrc/Unrelated.fs\n" |> changedPathsFromNameStatus

                    Expect.equal
                        largeNameStatus
                        [ "src/Unrelated.fs" ]
                        "large name-status output drains and deduplicates deterministically"
                    ...
                    Expect.exists
                        malformed.errors
                        (fun error -> error.Contains("malformed audit feedback/audits/malformed.audit.json"))
                        "malformed audit metadata fails closed"

                    File.WriteAllText(
                        Path.Combine(audits, "malformed.audit.json"),
                        """{"auditSchema":0,"report":"","reportSha256":"old","findings":[]}"""
                    )

                    let malformedStructure = findInvalidatedAuditBindings root [ "src/Other.fs" ]

                    Expect.exists
                        malformedStructure.errors
                        (fun error -> error.Contains("auditSchema must be 1"))
                        "schema-invalid audits cannot render a safe empty index"

                    File.Delete(Path.Combine(audits, "malformed.audit.json"))

                    for index in 1..200 do
                        File.WriteAllText(
                            Path.Combine(audits, sprintf "scale-%03d.audit.json" index),
                            auditJson root reportPath validReport "actionable" (evidence "file:src/Changed.fs")
                        )
            ...
            test "audit-binding exception ledger is exact, versioned, materialized, and fails closed" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-exceptions-" + Guid.NewGuid().ToString "N")
                ...
                try
                    ...
                    let evidence =
                        [|
                            {|
                                ...
                                result = "verified"
                                sha256 = Some priorDigest
                            |}
                        |]

                    File.WriteAllText(
                        Path.Combine(audits, "binding.audit.json"),
                        auditJson root reportPath validReport "actionable" evidence
                    )
                    ...
                    Expect.equal
                        accepted.dispositions.Head.id
                        "replacement-001"
                        "the applied receipt names the ledger id"
                    ...
                    Expect.equal
                        packagedExit
                        0
                        (sprintf "the materialized skill accepts the same exact entry: %s" packagedError)

                    Expect.stringContains
                        packagedOutput
                        "applied exception replacement-001"
                        "receiver output reports the applied disposition"

                    if OperatingSystem.IsLinux() then
                        File.Delete(Path.Combine(root, replacement))

                        File.CreateSymbolicLink(Path.Combine(root, replacement), "missing-evidence.txt")
                        |> ignore
                        ...
                        Expect.exists
                            dangling.errors
                            (fun error -> error.Contains "symbolic link, not a regular file")
                            "the direct route rejects dangling symlink evidence"
                        ...
                        Expect.stringContains
                            ...
                            "the materialized diagnostic names the non-regular evidence"
                        ...
                        Expect.exists
                            directory.errors
                            (fun error -> error.Contains "path names a directory")
                            "the direct route rejects directory evidence"
                    ...
                    Expect.exists
                        missing.errors
                        (fun error -> error.Contains "replacement evidence is missing")
                        "the direct route rejects missing evidence"
                    ...
                    File.WriteAllText(
                        ledgerPath,
                        (ledger [| entry |]).Replace("\"schemaVersion\":1", "\"schemaVersion\":2")
                    )
                    ...
                    Expect.exists
                        unsupported.errors
                        (fun error -> error.Contains "schemaVersion must be 1")
                        "unsupported schema fails closed"

                    let stale =
                        {| entry with
                            replacementSha256 = sha256Text "stale"
                        |}
                    ...
                    Expect.exists
                        staleResult.errors
                        (fun error -> error.Contains "replacement digest is stale")
                        "stale replacement evidence fails closed"

                    let mismatched =
                        {| entry with
                            priorSha256 = sha256Text "different-prior"
                        |}
                    ...
                    Expect.exists
                        mismatchedResult.errors
                        (fun error -> error.Contains "is unused")
                        "a mismatched prior digest cannot bind"
                    ...
                    Expect.exists
                        duplicate.errors
                        (fun error -> error.Contains "duplicate id")
                        "duplicate ids fail closed"

                    Expect.exists
                        duplicate.errors
                        (fun error -> error.Contains "duplicate binding")
                        "duplicate bindings fail closed"

                    let unused =
                        {| entry with
                            id = "unused-001"
                            audit = "feedback/audits/other.audit.json"
                        |}
                    ...
                    Expect.exists
                        unusedResult.errors
                        (fun error -> error.Contains "is unused")
                        "an entry without an immutable binding fails closed"
                    ...
                    Expect.exists
                        overbroadResult.errors
                        (fun error -> error.Contains "wildcard paths are overbroad")
                        "wildcard bindings fail explicitly"

                    let unknownField =
                        (ledger [| entry |])
                            .Replace("\"id\":\"replacement-001\"", "\"surprise\":true,\"id\":\"replacement-001\"")
                    ...
                    Expect.exists
                        unknown.errors
                        (fun error -> error.Contains "unknown property 'surprise'")
            ...
            test "commit-time invalidation indexes audits from the base tree, not the candidate" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-base-index-" + Guid.NewGuid().ToString "N")
                ...
                try
                    ...
                    let mergedAudit =
                        auditJson root reportPath validReport "actionable" (evidence "file:src/Base.fs")

                    let candidateAudit =
                        auditJson root reportPath validReport "actionable" (evidence "file:src/Candidate.fs")
                                ...
                                exceptions =
                                    [|
                                        {|
                                            ...
                                            evidenceLocator =
                                                "command:dotnet test && inspect " + candidateReplacementPath
                                        |}
                                    |]
                            |}
                        )
                    ...
                    Expect.isEmpty
                        headDisposition.errors
                        "the candidate head's valid ledger wins over malformed working-tree bytes"

                    Expect.isEmpty
                        headDisposition.invalidated
                        "the candidate-head entry dispositions the touched merged binding"
                    ...
                    if OperatingSystem.IsLinux() then
                        ...
                        let danglingSha =
                            commitOnto root "candidate-head-dangling" baseSha (fun () ->
                                ...
                                File.CreateSymbolicLink(Path.Combine(root, danglingPath), danglingTarget)
                                |> ignore

                                writeFile
                                    root
                                    "scripts/audit-binding-exceptions.json"
                                    (headLedger "head-dangling-001" danglingPath (sha256Text danglingTarget)))

                        let danglingHead = checkInvalidationBetweenRefs root baseSha danglingSha

                        Expect.exists
                            danglingHead.errors
                            (fun error -> error.Contains "mode 120000 type blob")
                            "the ref-aware route rejects a dangling Git symlink blob"
                    ...
                    let treeSha =
                        commitOnto root "candidate-head-tree" baseSha (fun () ->
                            ...
                            writeFile
                                root
                                "scripts/audit-binding-exceptions.json"
                                (headLedger "head-tree-001" treePath replacementDigest))

                    let treeHead = checkInvalidationBetweenRefs root baseSha treeSha

                    Expect.exists
                        treeHead.errors
                        (fun error -> error.Contains "mode 040000 type tree")
                        "the ref-aware route rejects a Git tree"
                    ...
                    let missingSha =
                        commitOnto root "candidate-head-missing" baseSha (fun () ->
                            ...
                            writeFile
                                root
                                "scripts/audit-binding-exceptions.json"
                                (headLedger "head-missing-001" missingPath replacementDigest))
                    ...
                    Expect.exists
                        missingHead.errors
                        (fun error -> error.Contains "replacement evidence is missing")
                        "the ref-aware route rejects a missing candidate-head path"
                    ...
                    let badBase =
                        checkInvalidationBetweenRefs root "fsgg-no-such-ref-1243" touchesMergedSha
                    ...
                    let brokenBaseSha =
                        commitOnto root "broken-base" baseSha (fun () ->
                            writeFile root "feedback/audits/broken.audit.json" "{")

                    let afterBrokenSha =
                        commitOnto root "after-broken" brokenBaseSha (fun () ->
                            writeFile root "src/Other.fs" "let other = 1\n")
                    ...
                    let unreadableIndex =
                        {
                            ...
                            errors =
                                [
                                    "invalidation: could not read the audit index at deadbeef: fatal: bad object"
                                ]
                        }
                    ...
                    Expect.equal
                        unreadable.subject
                        "synthetic index"
                        "the scan carries its index's subject into the verdict"
                    ...
                    writeFile
                        root
                        "scripts/audit-binding-exceptions.json"
                        (headLedger "head-gitlink-001" gitlinkPath replacementDigest)
                    ...
                    git
                        ...
                        [
                            ...
                            "--add"
                            "--cacheinfo"
                            sprintf "160000,%s,%s" baseSha gitlinkPath
                        ]
                    |> ignore
                    ...
                    Expect.exists
                        gitlinkHead.errors
                        (fun error -> error.Contains "mode 160000 type commit")
                        "the ref-aware route rejects a Git submodule entry"
            ...
            test "unreadable zero-event activation receipt is not reported as merely missing" {
                let root =
                    Path.Combine(Path.GetTempPath(), "fsgg-feedback-unreadable-" + Guid.NewGuid().ToString "N")

                try
                    activationReceiptPath root "001-example" |> Directory.CreateDirectory |> ignore
