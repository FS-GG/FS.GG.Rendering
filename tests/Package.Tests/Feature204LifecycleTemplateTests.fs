...
let private enumeratedChoices () =
    ...
    [
        for c in doc.RootElement.GetProperty("symbols").GetProperty("lifecycle").GetProperty("choices").EnumerateArray() ->
...
let private classifySource (row: SourceRow) : string * string list =
    ...
    let isSkillistException =
        target = "docs/skillist-reference.md"
        || List.contains "docs/skillist-reference.md" row.Includes
...
let private ruleMessage (row: SourceRow) (ruleId: string) =
    ...
    | "manifest/copy-only" -> sprintf "skill-manifest %s -> %s missing copyOnly (verbatim provider data, ADR-0014)" s t
    ...
    | "workspace/speckit-blanket-include" ->
        sprintf
            ".agents/skills/ blanket must include only speckit-*/** (dev-surface vendoring, F3); found %A"
            row.Includes
...
let private agreementFixture: (string * SourceRow) list =
    ...
    [
        ...
        row
            "template/base/.agents/skills/fs-gg-project/"
            ".agents/skills/fs-gg-project/"
            "(profile == \"game\")"
            []
            verbatim
        ...
        row
            "template/product-skills/fs-gg-gated/"
            ".agents/skills/fs-gg-gated/"
            "(profile == \"app\") && lifecycle == \"spec-kit\""
            []
            verbatim
        ...
        "manifest: ungated provider data, clean", row "template/skill-manifest/" ".agents/skills/" "" [] verbatim
        "manifest: gated AND not copyOnly",
        row "template/skill-manifest/" ".agents/skills/" "(lifecycle == \"spec-kit\")" [] []
        "manifest: wrong target root", row "template/skill-manifest/" ".agents/skillsets/" "" [] verbatim
        ...
        "workspace: the generated agent-context tree, ungated", row ".template.config/generated/" "./" "" [] []
        ...
        "product: the ungated base source", row "template/base/" "./" "" [] [ "docs/api-surface/**" ]
        "product: a profile-gated source fragment",
        row "template/fragments/vec2/src/" "src/" "(profile == \"game\")" [] []
        "product: wrongly lifecycle-gated", row "template/fragments/bad/" "src/" "(lifecycle == \"spec-kit\")" [] []
        ...
        [
            ...
            test "GV-1 coverage equals the template's lifecycle choice set" {
                ...
                Expect.stringContains
                    report
                    "covered-values: spec-kit, sdd, typed-sdd, none"
                    "covered-values lists the 4 values in declaration order"
            }
            ...
            test
                "GV-2 sources partition into framework-skill / manifest / lifecycle-workspace / product (ADR-0014 gating)" {
                let framework, capability, manifest, workspace, product, violations =
                    gatedSourceAudit ()
                ...
                Expect.equal
                    ...
                    17
                    (sprintf
                        "expected exactly 17 framework product-skill sources (no twins; ADR-0063 retired the 4 game-owned copies; #991 added fs-gg-game-shell), found %d"
                        framework)
                ...
                Expect.equal
                    ...
                    1
                    (sprintf
                        "expected exactly 1 capability-scope skill source (fs-gg-feedback-report — ungated since #434), found %d"
                        capability)
            ...
            test "GV-3 spec-kit is byte-identical to today and adds only gated paths vs the sdd default" {
                ...
                for p in profiles do
                    Expect.stringContains
                        report
                        (sprintf
                            "spec-kit/%s: generate=pass diff-vs-today=none diff-vs-default=gated-only guard-sentinel=absent"
                            p)
            ...
            test "GV-4 sdd is the default lane, guarded, with the framework skills present" {
                ...
                for p in profiles do
                    Expect.stringContains
                        report
                        (sprintf
                            "sdd/%s: generate=pass gated-absent=ok product-present=ok diff-vs-default=none guard-sentinel=present"
                            p)
                    ...
                    Expect.stringContains
                        ...
                        (sprintf
                            "sdd/%s .claude holds zero product skills (ADR-0011) and .codex/ is absent (ADR-0067 §5)"
                            p)

                Expect.stringContains
                    report
                    "dangling-refs: none"
                    "no directive agent-context doc references a suppressed path (CC-1)"

                Expect.stringContains
                    report
                    "catalog-dangling: none"
                    "no scaffold emits a catalog listing absent skills (FR-005/FR-006)"
            }
            ...
            test "GV-5 none suppresses the lifecycle workspace, framework skills present, no guard" {
                ...
                for p in profiles do
                    ...
                    Expect.stringContains
                        ...
                        (sprintf
                            "none/%s .claude holds zero product skills (ADR-0011) and .codex/ is absent (ADR-0067 §5)"
                            p)
            }
            ...
            test "GV-6 composition matrix generates and unknown value is rejected" {
                ...
                Expect.stringContains
                    report
                    "composition-matrix: 16/16 generate; ant-overlay-present=ok; feedback-gated-under-non-speckit=ok"
                    "16-combo composition matrix holds"
            ...
            test "GV-7 provenance is disclosed" {
                ...
                Expect.isTrue
                    (report.Contains "provenance: live"
                     || report.Contains
                         "provenance: verdict-core (env-free; full live proof gated behind FS_GG_RUN_LIFECYCLE_VALIDATION=1)")
