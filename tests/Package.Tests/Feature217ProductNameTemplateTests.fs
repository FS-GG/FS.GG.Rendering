...
let private auditContract () =
    ...
    let productNameTextEmpty =
        ...
        | Some pn ->
            ...
            && (match tryProp pn "defaultValue" with
                | Some d -> elemStr d = ""
                | None -> false)
        | None -> false
    ...
    let coalesceOk =
        ...
        | Some en ->
            ...
            && (match tryProp en "parameters" with
                | Some p ->
                    (match tryProp p "sourceVariableName" with
                     | Some s -> elemStr s = "productNameTrimmed"
                     | None -> false)
                    && (match tryProp p "fallbackVariableName" with
                        | Some f -> elemStr f = "name"
                        | None -> false)
                | None -> false)
            && (match tryProp symbols "productNameTrimmed" with
                | Some t ->
                    ...
                    && (match tryProp t "parameters" with
                        | Some tp ->
                            (match tryProp tp "source" with
                             | Some s -> elemStr s = "productName"
                             | None -> false)
                        | None -> false)
                | None -> false)
            ...
            && (match tryProp symbols "effectiveNameLower" with
                | Some l ->
                    ...
                    && (match tryProp l "replaces" with
                        | Some r -> elemStr r = "product"
                        | None -> false)
                    && (match l.TryGetProperty "fileRename" with
                        | true, _ -> false
                        | _ -> true)
                    && (match tryProp l "parameters" with
                        | Some lp ->
                            (match tryProp lp "source" with
                             | Some s -> elemStr s = "effectiveName"
                             | None -> false)
                        | None -> false)
                | None -> false)
        | None -> false
    ...
    note
        coalesceOk
        "effectiveName coalesce(productNameTrimmed ?? name) + productNameTrimmed regex(productName) + effectiveNameLower casing(replaces product) missing/wrong"

    let renameOk =
        match effectiveName with
        | Some en ->
            (match tryProp en "replaces" with
             | Some r -> elemStr r = "Product"
             | None -> false)
            && (match tryProp en "fileRename" with
                | Some f -> elemStr f = "Product"
                | None -> false)
        | None -> false
    ...
    testList
        ...
        [
            ...
            test "GV-1b FSI loader uses the scoped Product filename contract and truthful frontend guidance" {
                ...
                Expect.isFalse
                    (File.Exists staleLoader)
                    "lowercase loader path cannot be renamed by the scoped Product token"
                ...
                Expect.stringContains
                    references[0]
                    "src/Product/bin/Debug/net10.0/Product.dll"
                    "loader references the renamed main assembly"

                Expect.isFalse
                    (loaderText.Contains "FS.GG.UI.")
                    "transitive package references must not be hand-maintained"
                ...
                for rel in [ "template/base/README.md"; "template/base/docs/product.md" ] do
                    ...
                    Expect.stringContains
                        ...
                        (sprintf "%s documents the renameable loader" rel)

                    Expect.isFalse
                        (guidance.Contains "dotnet fsi load-product.fsx")
                        (sprintf "%s does not document a path that stays lowercase" rel)

                    Expect.isFalse (guidance.Contains "dotnet fake") (sprintf "%s does not advertise an absent CLI" rel)

                    Expect.isFalse
                        (guidance.Contains "FAKE-backed")
                        (sprintf "%s calls the build script what it is" rel)
            }
            ...
            test "GV-7 Acme README build and renamed FSI loader succeed" {
                ...
                Expect.stringContains
                    report
                    "sc002-build: readme-build=Debug warn=0 err=0 loader=load-Acme.fsx fsi=ok"
                    "SC-002 README sequence"
            }
            ...
            test "GV-8 provenance is disclosed" {
                ...
                Expect.isTrue
                    (report.Contains "provenance: live"
                     || report.Contains
                         "provenance: verdict-core (env-free; full live proof gated behind FS_GG_RUN_PRODUCTNAME_VALIDATION=1)")
