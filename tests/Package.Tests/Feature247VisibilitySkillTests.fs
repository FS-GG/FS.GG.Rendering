...
let private skillBody =
    File.ReadAllText(repositoryPath "template/product-skills/fs-gg-visibility/SKILL.md")

let private helperSource =
    repositoryPath "template/fragments/visibility/src/Product/Visibility.fs"
        ...
        [
            ...
            test "Product.fsproj compiles Visibility.fs profile-gated and Exists-guarded (delete-safe)" {
                ...
                Expect.isLessThan
                    ...
                    "Visibility.fs compiles before Model.fs (usable from update/view)"

                Expect.isGreaterThan
                    (idx "Visibility.fs")
                    (idx "profile == \"game\" || profile == \"sample-pack\"")
                    "Visibility.fs is inside the sim-profile gate"
            }

            test "both template.json sources (skill + fragment) are gated to game/sample-pack" {
                Expect.equal
                    (sourceCondition "template/product-skills/fs-gg-visibility/")
                    (Some simGate)
                    "skill source gated to sim profiles"

                Expect.equal
                    (sourceCondition "template/fragments/visibility/src/")
                    (Some simGate)
            ...
            test "scaffold-map classifies Visibility.fs as replaceable/adaptable" {
