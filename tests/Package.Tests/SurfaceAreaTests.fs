...
let exportedMembers (assembly: Assembly) =
    memberSignatures assembly |> Set.ofArray
    ...
    testList
        ...
        [
            test "surface baselines use stable root readiness path" {
                ...
                Expect.isFalse
                    (baselinePath.Contains("specs/002-skia-feature-parity", StringComparison.Ordinal))
                    "baseline path is not historical feature readiness"
            }
            ...
            test "SkiaViewer package exposes selected generated persistent viewer entry point" {
                ...
                Expect.contains
                    methodNames
                    "runAppWithWindowBehavior"
                    "window-behavior overload remains packaged but is not the generated default"
            }
            ...
            test "V3 capability packages declare package-specific contracts and baselines" {
                [
                    "Build",
                    "src/Build/FS.GG.UI.Build.fsproj",
                    "src/Build/Evidence.fsi",
                ...
                |> List.iter (fun (name, project, contract, baseline) ->
                    ...
                    Expect.isTrue
                        (File.Exists(Path.Combine(repositoryRoot, contract)))
                        $"{name} public .fsi contract exists"

                    Expect.isTrue
                        (File.Exists(Path.Combine(repositoryRoot, baseline)))
                        $"{name} package surface baseline exists")
            }
            ...
            test "controls boundary public contracts include runtime rich rendering and FSI transcript coverage" {
                ...
                |> List.iter (fun contract ->
                    Expect.isTrue
                        (File.Exists(Path.Combine(repositoryRoot, contract)))
                        $"{contract} is a curated public contract")
                ...
                |> List.iter (fun transcriptScript ->
                    Expect.isTrue
                        (File.Exists(Path.Combine(repositoryRoot, transcriptScript)))
                        $"{transcriptScript} produces public FSI evidence")
                ...
                |> List.iter (fun baselinePath ->
                    Expect.isTrue
                        (File.Exists(Path.Combine(repositoryRoot, baselinePath)))
                        $"{baselinePath} exists for package surface review")
            }
            ...
            test "removed Charts package has no active surface baseline participation" {
                ...
                Expect.isFalse
                    (File.Exists chartsBaseline)
                    "legacy Charts package surface baseline is removed from active package review"
            }

            test "Controls FSI transcript authors chart graph and DataGrid without Charts package" {
                let transcript =
                    File.ReadAllText(Path.Combine(repositoryRoot, "scripts", "controls-prelude.fsx"))
                ...
                [ "FS.GG.UI.Charts"; "open FS.GG.UI.Charts"; "#r \"../src/Charts" ]
                |> List.iter (fun forbidden ->
                    Expect.isFalse
                        (transcript.Contains(forbidden, StringComparison.Ordinal))
                        $"Controls FSI transcript does not use {forbidden}")
            }

            test "Scene package stays dependency-light" {
                let sceneProject =
                    File.ReadAllText(Path.Combine(repositoryRoot, "src", "Scene", "Scene.fsproj"))

                [ "Fable.Elmish"; "Silk.NET"; "SkiaSharp"; "Yoga.Net"; "YamlDotNet" ]
                |> List.iter (fun forbidden ->
                    Expect.isFalse (sceneProject.Contains forbidden) $"Scene does not reference {forbidden}")
            }
            ...
            test "top-level F# visibility modifiers do not replace signature ownership" {
                ...
                let offending =
                    ...
                    |> Seq.collect (fun file ->
                        ...
                        |> Seq.mapi (fun index line -> file, index + 1, line)
            ...
            test "US4 maintained package surfaces are governed by paired signatures and active baselines" {
                let implementationRoots =
                    [ "src/Controls"; "src/KeyboardInput"; "src/Controls.Elmish" ]
                ...
                [
                    ...
                    "readiness/surface-baselines/FS.GG.UI.Controls.Elmish.txt",
                    "FS.GG.UI.Controls.Elmish.ControlsElmish"
                ]
                ...
                Expect.isFalse
                    (File.Exists(Path.Combine(repositoryRoot, "readiness", "surface-baselines", "FS.GG.UI.Charts.txt")))
                    "removed Charts baseline is not active"
            }
        ]
