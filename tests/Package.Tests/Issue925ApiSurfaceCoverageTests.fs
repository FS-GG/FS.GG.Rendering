...
let private mAudit =
    key "FS.GG.UI.Scene" "SceneHost.fsi" "val" "GeneratedAppHost.auditKeyWiring"

let private mReach =
    key "FS.GG.UI.Scene" "SceneHost.fsi" "val" "GeneratedAppHost.reachableMessages"

let private mWiring =
    key "FS.GG.UI.Scene" "SceneHost.fsi" "type" "SceneHostKeyWiring`1"
        ...
        [
            test "a member neither taught nor waived is reported UNTAUGHT — and named" {
                ...
                Expect.equal
                    v.Untaught
                    universe
                    "every member of the universe is untaught when nothing is taught or waived"
            ...
            test "a waiver for a member the pin does NOT export is STALE" {
                ...
                let ghost =
                    key "FS.GG.UI.Scene" "SceneHost.fsi" "val" "GeneratedAppHost.removedInThePin"

                let v =
                    reconcile universe (Set.ofList [ mAudit; mReach; mWiring ]) (Set.ofList [ ghost ])
            ...
            test "a waiver for a member that is ALSO taught is STALE" {
                ...
                let v =
                    reconcile universe (Set.ofList [ mAudit; mReach; mWiring ]) (Set.ofList [ mAudit ])
            ...
            test "reports are sorted, so a diff shows what entered or left the surface" {
                ...
                Expect.equal
                    v.Untaught
                    (List.sortBy (fun m -> m.Package, m.Source, m.Path, m.Kind) universe)
                    "untaught is ordered"
            }
        ]
    ...
    testList
        ...
        [
            test "a declared module with NO taught member is a gap — the whole-module drop the reconcile misses" {
                ...
                Expect.equal
                    (gaps |> List.map (fun g -> g.Source, g.Reason))
                    [ ("Effects.fsi", "AllWaived") ]
                    "an entirely-waived declared module is reported AllWaived"
            }
            ...
            test "a declared module the pin ships no member for is a ROTTED declaration — fails closed" {
                ...
                Expect.equal
                    (gaps |> List.map (fun g -> g.Source, g.Reason))
                    [ ("Removed.fsi", "Vanished") ]
                    "a declared module absent from the pin is Vanished"
            }
            ...
            test "gaps are reported for exactly the undecided modules, sorted" {
                ...
                Expect.equal
                    (gaps |> List.map (fun g -> g.Source))
                    [ "Effects.fsi" ]
                    "only the entirely-waived declared module is reported"
            }
...
let private inc source kind path : Include =
    {
        Source = source
        Kind = kind
        Path = path
    ...
    testList
        ...
        [
            ...
            test "each member reported ONCE however many times it repeats" {
                ...
                Expect.equal
                    (duplicateIncludes includes)
                    [ inc "F.fsi" "type" "Foo" ]
                    "a member tripled is one duplicate, not two"
            }
        ...
        [
            ...
            test "a `module internal` prunes its ENTIRE subtree — children carry no keyword of their own" {
                ...
                let tree =
                    [
                        ...
                        d
                            ...
                            "Reconcile"
                            true
                            [ d "val" "Reconcile.step" false []; d "type" "Reconcile.State" false [] ]
                    ]
                ...
                Expect.equal
                    paths
                    [ "publicOne" ]
                    "only the public sibling survives; the whole internal module is pruned"
            }
            ...
            test "an `and` continuing an internal type-group is non-public, though its own line is bare" {
                ...
                Expect.equal
                    paths
                    [ "PublicType"; "PublicCont" ]
                    "the internal group (Foo/Bar) is excluded; the public group survives"
            }
        ...
        [
            test "the generator loads the coverage module and calls reconcile" {
                Expect.stringContains
                    ...
                    "the generator #loads the pure verdict"
            ...
            test "the gate runs the generator with --check (merge-blocking)" {
                Expect.stringContains
                    gate
                    "refresh-api-surface-mirror.fsx --check"
                    "the required gate exercises the coverage check via --check"
            }
            ...
            test "the generator calls the profile-completeness gate and fails on an entirely-dropped module (#984)" {
                Expect.stringContains
                    generator
                    "Coverage.profileGaps"
                    "the generator calls the module-level completeness decision"
            ...
            test "the generator loads the coverage module and fails on a duplicate include (#982)" {
                Expect.stringContains
                    generator
                    "Coverage.duplicateIncludes"
                    "the generator calls the well-formedness decision"
            ...
            test "no `file` stanza in the committed manifest repeats a `+` include (#982)" {
                ...
                for raw in lines do
                    ...
                    if s.StartsWith "file " then
                        ...
                        if not (perFile.ContainsKey current) then
                            perFile.[current] <- ResizeArray<Include>()
                ...
                Expect.isEmpty
                    offenders
                    (sprintf "stanza(s) with a repeated `+` include (renders N times): %A" (offenders |> List.map fst))
            }
        ]
