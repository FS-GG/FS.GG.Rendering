...
let private identifierLeakPatterns =
    [
        "let/and binding",
        Regex(
            @"^\s*(let|and)\s+(mutable\s+|rec\s+|inline\s+|private\s+|internal\s+|public\s+)*[A-Za-z0-9_']*[Pp]roduct",
            RegexOptions.Compiled
        )
        "type/module declaration", Regex(@"^\s*(type|module)\s+[A-Za-z0-9_']*[Pp]roduct", RegexOptions.Compiled)
        "DU case declaration", Regex(@"^\s*\|\s*[A-Za-z0-9_']*[Pp]roduct", RegexOptions.Compiled)
        "member declaration",
        ...
        [
            ...
            test "the scan actually enumerates the scaffold sources (must not silently narrow to zero)" {
                ...
                Expect.isTrue
                    (scaffoldSourceFiles
                     |> List.exists (fun p ->
                         p.Replace('\\', '/').EndsWith "template/base/src/Product/EvidenceCommands.fs"))
                    "EvidenceCommands.fs (the #149 leak site) must be in the scanned set"

                Expect.isTrue
                    (scaffoldSourceFiles
                     |> List.exists (fun p ->
                         p.Replace('\\', '/').EndsWith "template/base/tests/Product.Tests/BehaviorTests.fs"))
            ...
            test "the scanner detects a synthetic identifier leak (not narrowed shut)" {
                ...
                let mustFlag =
                    [
                        ...
                        "    member _.readProductFile parts = ()"
                    ] // #152: embedded member
                ...
                mustFlag
                |> List.iter (fun l -> Expect.isNonEmpty (scanLine l) (sprintf "synthetic leak not detected: %s" l))
