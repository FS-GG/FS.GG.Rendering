...
let fsiTranscriptCoverageTests =
    testList
        "Package-shaped FSI transcript coverage"
        [
            ...
            test "Controls-adjacent transcript authors controls without relying on repository source" {
                ...
                [ "#load"; "../src/"; "Assembly.Load"; "GetExportedTypes" ]
                |> List.iter (fun forbidden ->
                    Expect.isFalse
                        (transcript.Contains(forbidden, StringComparison.OrdinalIgnoreCase))
                        $"Controls transcript does not use {forbidden}")
            }
            ...
            test "FSI transcript run logs are captured as readiness artifacts" {
                ...
                |> List.iter (fun logName ->
                    ...
                    Expect.stringContains
                        ...
                        "FSI transcript PASS"
                        $"{logName} records a passing FSI run")
            }
            ...
            test "Feature146 transcripts cover SceneCodec and ReferenceRendering authoring" {
                ...
                let referenceRendering =
                    readFeature146Transcript "reference-rendering-authoring.fsx"
                ...
                [ "ReferenceRendering.init"; "PackageBytes"; "OutputDirectory"; "OutputSize" ]
                |> List.iter (fun required ->
                    Expect.stringContains
                        referenceRendering
                        required
                        $"ReferenceRendering transcript includes {required}")

                [ "scene-codec-authoring.log"; "reference-rendering-authoring.log" ]
                |> List.iter (fun logName ->
            ...
            test "Feature147 transcripts cover compositor proof and metrics authoring" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains proof required $"proof transcript includes {required}")

                [ "FS.GG.UI.Controls.Elmish"; "CompositorFrameDiagnostics"; "FrameMetrics" ]
                |> List.iter (fun required ->
                    Expect.stringContains metrics required $"metrics transcript includes {required}")

                [ "compositor-proof-authoring.log"; "compositor-metrics-authoring.log" ]
                |> List.iter (fun logName ->
            ...
            test "Feature148 transcripts cover live proof and compositor diagnostics authoring" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains transcript required $"Feature148 transcript includes {required}")
            ...
            test "Feature149 transcripts cover final compositor readiness authoring" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains transcript required $"Feature149 transcript includes {required}")
            ...
            test "Feature152 transcripts cover proof-set and readiness helper authoring" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains transcript required $"Feature152 transcript includes {required}")
            ...
            test "Feature153 transcripts cover proof interpreter readiness authoring" {
                let transcript =
                    readFeature153Transcript "compositor-proof-interpreter-authoring.fsx"
                ...
                |> List.iter (fun required ->
                    Expect.stringContains transcript required $"Feature153 transcript includes {required}")
            ...
            test "Feature154 transcripts cover proof acceptance and final readiness authoring" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains proof required $"Feature154 proof transcript includes {required}")
                ...
                |> List.iter (fun required ->
                    Expect.stringContains readiness required $"Feature154 readiness transcript includes {required}")
            ...
            test "Feature156 transcripts cover timing policy readiness and Testing helpers" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains timing required $"Feature156 timing transcript includes {required}")
                ...
                |> List.iter (fun required ->
                    Expect.stringContains readiness required $"Feature156 readiness transcript includes {required}")

                [ "compositor-performance-authoring.log"; "compositor-readiness-authoring.log" ]
                |> List.iter (fun logName ->
            ...
            test "Feature157 transcripts cover damage decisions and Testing helpers" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains damage required $"Feature157 damage transcript includes {required}")
                ...
                |> List.iter (fun required ->
                    Expect.stringContains readiness required $"Feature157 readiness transcript includes {required}")

                [ "compositor-damage-authoring.log"; "compositor-readiness-authoring.log" ]
                |> List.iter (fun logName ->
            ...
            test "Feature159 transcripts cover split identity promotion and readiness helpers" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains identity required $"Feature159 identity transcript includes {required}")
                ...
                |> List.iter (fun required ->
                    Expect.stringContains promotion required $"Feature159 promotion transcript includes {required}")
                ...
                |> List.iter (fun required ->
                    Expect.stringContains readiness required $"Feature159 readiness transcript includes {required}")
            ...
            test "Feature150 transcript covers intrinsic layout and readiness authoring" {
                ...
                |> List.iter (fun required ->
                    Expect.stringContains transcript required $"Feature150 transcript includes {required}")
            ...
            test "Feature151 records no new public FSI delta on top of Feature150" {
                let ledger =
                    File.ReadAllText(repositoryPath "specs/151-complete-p8-layout/readiness/compatibility-ledger.md")
