...
let private onFixture (files: (string * string) list) (assertion: Verdict -> unit) =
    let dir =
        Path.Combine(Path.GetTempPath(), "tc706-" + Guid.NewGuid().ToString("N").Substring(0, 8))
        ...
        [
            ...
            test "zero directives is a GUARD ERROR, not a pass — the fail-closed rule" {
                onFixture [ "Product.fs", "let a = 1\n// no conditionals here at all\n" ] (fun v ->
                    ...
                    Expect.notEqual v.ExitCode 0 "an empty subject must never be a pass")
            }
            ...
            test "a STRAY #endif is caught — the #680 defect, exactly" {
                onFixture [ "Product.fs", "let a = 1\n//#endif\n" ] (fun v ->
                    Expect.equal
                        v.ExitCode
                        1
                        $"an `#endif` with no open `#if` must be reported as an imbalance.\n\n{v.Output}"

                    Expect.stringContains
                        v.Output
                        "orphan-endif"
                        $"the defect must be NAMED, so a reader knows what to fix.\n\n{v.Output}")
            }

            test "an UNCLOSED #if is caught" {
                onFixture [ "Product.fs", "//#if (profile == \"game\")\nlet a = 1\n" ] (fun v ->
            ...
            test "an #else outside any #if is caught" {
                onFixture [ "Product.fs", "let a = 1\n//#else\nlet b = 2\n" ] (fun v ->
                    Expect.equal
                        v.ExitCode
                        1
                        $"an `#else` with no open `#if` guards nothing — its body is emitted for EVERY profile.\n\n{v.Output}"
                ...
                onFixture
                    ...
                    (fun v ->
                        Expect.equal
                            v.ExitCode
                            1
                            $"two `#else` branches in one region means one of them is dead, and which one is up to the template engine.\n\n{v.Output}"
                ...
                onFixture
                    ...
                    (fun v ->
                        Expect.equal
                            v.ExitCode
                            1
                            $"an `#elif` after the catch-all `#else` is unreachable.\n\n{v.Output}"
            ...
            test "this suite really is REQUIRED — Package.Tests is slnx-derived, and the gate runs the slnx" {
                let path rel =
                    Path.Combine(repositoryRoot, (rel: string).Replace('/', Path.DirectorySeparatorChar))
                ...
                let glProjects =
                    Text.RegularExpressions.Regex
                        .Match(gate, @"GL_TEST_PROJECTS:\s*""(?<v>[^""]*)""")
                        .Groups.["v"].Value
