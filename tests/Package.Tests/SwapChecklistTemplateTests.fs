...
let private branchFor (family: string) (text: string) =
    ...
    let included () =
        levels
        |> List.forall (fun (named, inElse) -> if inElse then not named else named)
...
let private familyReaders =
    [
        ...
        [
            ...
            "scoreTextBounds"
            "layoutEvidenceForSize"
            "movementUsesGameplayRegion"
            "collisionUsesGameplayRegion"
            "validateGeneratedLayout"
            "mapKey"
            "layoutEvidenceCommand"
            "sceneEvidence"
        ]
        "app",
        [
        ...
        "governed", [ "layoutEvidenceForSize"; "layoutEvidenceCommand"; "sceneEvidence" ]
    ]
...
let private definedIn (source: string) (sym: string) =
    Regex.IsMatch(
        source,
        sprintf @"(?m)^\s*(let(\s+(rec|inline|private|mutable))*\s+|and\s+)%s\s*[\s(:]" (Regex.Escape sym)
    )
        ...
        [
                ...
                for family, readers in familyReaders do
                    ...
                    for reader in readers do
                        Expect.stringContains
                            text
                            reader
                            (sprintf "%s checklist lists re-point reader %s" family reader)
            }
            ...
            test "NO PHANTOM: every reader a checklist names is defined in THAT family's scaffold branch" {
                for family, readers in familyReaders do
                    ...
                    for reader in readers do
                        Expect.isTrue
                            (definedIn source reader)
                            (sprintf
                                "%s checklist reader %s resolves in the %s branch a generated tree actually ships"
                                family
                                reader
                                family)
            }
            ...
            test "checklists keep Product-scoped paths literal (copyOnly, no sourceName rewrite)" {
                for family, _ in familyReaders do
                    ...
                    Expect.stringContains
                        text
                        "<ProductDir>"
                        (sprintf "%s checklist keeps the literal <ProductDir> token" family)
            }
        ]
...
let private productMd =
    File.ReadAllText(repositoryPath "template/base/docs/product.md")
    ...
    testList
        ...
        [
            test "build.fsx exposes the bare `help` token (fsi reserves --help/-h on the script path)" {
                ...
                Expect.isFalse
                    (buildFsx.Contains("writeLog \"help\""))
                    "help path writes no completion-marker log (side-effect-free)"
            }
