...
let private workloadDigests artifactPath =
    ...
    |> Seq.map (fun workload ->
        workload.GetProperty("id").GetString(), workload.GetProperty("definitionDigest").GetString())
    |> Seq.map (fun (id, digest) ->
        Option.ofObj id
        |> Option.defaultWith (fun () -> failwith "workload id was null"),
        Option.ofObj digest
        |> Option.defaultWith (fun () -> failwith "workload digest was null"))
    |> Map.ofSeq

let private performanceIntentBindings artifactPath =
    use document = JsonDocument.Parse(File.ReadAllText artifactPath)

    document.RootElement.GetProperty("performanceIntent").GetProperty("workloadDefinitionDigests").EnumerateArray()
    |> Seq.map (fun binding -> binding.GetString())
    |> Seq.map (
        Option.ofObj
        >> Option.defaultWith (fun () -> failwith "performance intent binding was null")
    )
    |> List.ofSeq
...
let private rewriteWorkloadBlock id rewrite (source: string) =
    ...
    let finish =
        source.IndexOf(endMarker, start + beginMarker.Length, StringComparison.Ordinal)
    ...
    + rewrite (source.Substring(start, length))
...
let private authorWorkloads (digests: Map<string, string>) (source: string) =
    digests
    |> Map.fold
        (fun (authored: string) (id: string) (digest: string) ->
            let beginMarker = $"// WORKLOAD-SOURCE-BEGIN {id}"
            let endMarker = $"// WORKLOAD-SOURCE-END {id}"
            let start = authored.IndexOf(beginMarker, StringComparison.Ordinal)

            let finish =
                authored.IndexOf(endMarker, start + beginMarker.Length, StringComparison.Ordinal)
            ...
            + rewritten
            + authored.Substring(finish + endMarker.Length))
        source
    ...
    testList
        ...
        [
            ...
            test "generated declaration uses the published Contracts 7.x performance-intent shape" {
                ...
                Expect.stringContains
                    packages
                    "<FsGgContractsVersion>7.5.2</FsGgContractsVersion>"
                    "producer version is exact"
                ...
                Expect.stringContains
                    ...
                    "non-game profiles retain a compilable no-op"
            }
            ...
            test "generated game scaffold executes placeholder, authored, stale-route and debt verdicts" {
                ...
                let artifactPath =
                    Path.Combine(productRoot, "readiness", "performance-evidence.json")

                let criticRequestPath =
                    Path.Combine(productRoot, "readiness", "performance-critic-request.json")
                ...
                try
                    ...
                    Expect.equal
                        install.ExitCode
                        0
                        $"local template install succeeds:{Environment.NewLine}{install.Output}"
                    ...
                    Expect.stringContains
                        ...
                        "declared stimulus must traverse production"
                        ...
                        |> rewriteWorkloadBlock "firing" (fun block ->
                            block
                                ...
                                .Replace(
                                    ...
                                    "MessageAt = (fun _ -> ViewerInput(Letter 'F', true))"
                                ))
                    ...
                    Expect.equal
                        representativePendingAuthorship.ExitCode
                        1
                        "representative routes remain red while unauthored"

                    let representativeDigests = workloadDigests artifactPath

                    let representativeAuthoredSource =
                        authorWorkloads representativeDigests representativePlaceholderSource
                    ...
                    |> List.iter (fun token ->
                        Expect.stringContains
                            authoredIntentYaml
                            token
                            $"published Contracts 7.x field is projected: {token}")
                    ...
                    let duplicateIdSource =
                        Regex.Replace(
                            authoredSource,
                            "(// WORKLOAD-SOURCE-BEGIN firing\\s+\\{\\s+Id = )\"firing\"",
                            "$1\"idle\"",
                            RegexOptions.CultureInvariant
                        )
...
let expectedWorkloads =""",
                        ...
                        |> rewriteWorkloadBlock "idle" (fun block ->
                            block
                                .Replace(
                                    "InitialState = (fun () -> initialModel)",
                                    "InitialState = helperBoundInitialState"
                                )
                    ...
                    Expect.equal
                        helperBoundPending.ExitCode
                        1
                        "helper-bound workloads remain unauthored before their digest is acknowledged"

                    let helperBoundDigests = workloadDigests artifactPath

                    let helperBoundAuthoredSource =
                        authorWorkloads helperBoundDigests helperBoundPlaceholderSource
                    ...
                    Expect.stringContains
                        helperOnlyModel.Output
                        "authored declaration is stale"
                        "helper-only model mutation is diagnosed as stale"

                    Expect.stringContains
                        helperOnlyModel.Output
                        "workload 'idle'"
                        "the helper-backed workload is identified"
                    ...
                    Expect.equal
                        frameTwoOnlyMessage.ExitCode
                        1
                        "a frame-2-only helper message mutation invalidates Authored"

                    Expect.stringContains
                        frameTwoOnlyMessage.Output
                        "authored declaration is stale"
                        "frame-2 helper mutation is diagnosed as stale"

                    Expect.stringContains
                        frameTwoOnlyMessage.Output
                        "workload 'idle'"
                        "the later-frame workload is identified"
            ...
            test "structural fingerprint distinguishes same-named records and union cases" {
                ...
                let encoder =
                    sliceBetween "let private canonicalEscape" "let private structuralFingerprint" source
...
"""
                ...
                try
                    ...
                    let result =
                        runDotnet fixtureRoot dotnetHome [ "fsi"; "--define:DEBUG"; scriptPath ]
            ...
            test "generated collision-scale observers distinguish 120 stored entities from zero candidates" {
                let runCandidate observer expectedExit expectedCandidate =
                    ...
                    let evidenceSourcePath =
                        Path.Combine(productRoot, "src", "CollisionFixture", "PerformanceEvidence.fs")
                    ...
                    try
                        ...
                        Expect.equal
                            install.ExitCode
                            0
                            $"template install succeeds:{Environment.NewLine}{install.Output}"
                        ...
                        Expect.equal
                            instantiate.ExitCode
                            0
                            $"collision fixture instantiates:{Environment.NewLine}{instantiate.Output}"
                ...
                Some(fun _ model ->
                    ...
                    Some candidatesConsidered)""",
                        ...
                        Expect.isTrue
                            ...
                            $"failing evidence still emits digests for authoring:{Environment.NewLine}{pending.Output}"
                        ...
                        Expect.equal
                            evidence.ExitCode
                            expectedExit
                            $"observer-reported candidates produce the expected coverage verdict:{Environment.NewLine}{evidence.Output}"

                        Expect.stringContains
                            evidenceSourcePath
                            "PerformanceEvidence.fs"
                            "the exercised product owns its observer"

                        if expectedExit = 1 then
                            Expect.stringContains
                                evidence.Output
                                "simulation.fixed-step"
                                "zero candidates fail the coverage gate despite 120 stored entities"
                        ...
                        let observed =
                            ...
                            |> Seq.map (fun workload ->
                                workload.GetProperty("observedScale").GetProperty("simulation.fixed-step").GetInt32())
                            |> Seq.distinct
                            |> Seq.toList
                        ...
                        Expect.equal
                            observed
                            [ expectedCandidate ]
                            "the observer result is aggregated into every required workload"
                        ...
                        Expect.equal
                            critic.ExitCode
                            expectedExit
                            "critic request preserves the machine coverage outcome"

                        use request = JsonDocument.Parse(File.ReadAllText criticRequestPath)

                        Expect.equal
                            (request.RootElement.GetProperty("machineExitCode").GetInt32())
                            expectedExit
                            "critic input records the zero/positive candidate machine outcome"
                    finally
                        if Directory.Exists fixtureRoot then
                            Directory.Delete(fixtureRoot, true)
            ...
            test "all four generated-product skills teach one bounded-versus-live recipe" {
                ...
                Expect.stringContains
                    testingSkill
                    "complete deterministic value returned by `InitialState()`"
                    "testing guidance closes helper-backed initial-state evidence"

                Expect.stringContains
                    testingSkill
                    "frame 2"
                    "testing guidance closes helper-backed later-frame evidence"
            }
        ]
