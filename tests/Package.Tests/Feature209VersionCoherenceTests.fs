...
let private releaseWindowSource =
    File.ReadAllText(repo "scripts/lib/ReleaseWindow.fsx")

let private apiMirrorSource =
    File.ReadAllText(repo "scripts/refresh-api-surface-mirror.fsx")
...
let private currentPin =
    Regex
        .Match(
            File.ReadAllText(repo "template/base/Directory.Packages.props"),
            "<FsGgUiVersion>([^<]+)</FsGgUiVersion>"
        )
        .Groups.[1].Value.Trim()
    ...
    lazy
        (let tmp =
            Path.Combine(
                Path.GetTempPath(),
                "vcoh-released-"
                + (Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(Text.Encoding.UTF8.GetBytes root)))
                    .Substring(0, 12)
            )
         ...
         for tag in
             ...
             let exists, _ =
                 runIn tmp "git" [ "rev-parse"; "--verify"; "--quiet"; $"refs/tags/{tag}" ]
...
let private parse (s: string) =
    ...
    let n i =
        if i < nums.Length then int nums.[i] else 0
...
let private pinOccurrences =
    Regex.Matches(propsText, "<FsGgUiVersion>([^<]*)</FsGgUiVersion>").Count
...
let private templateFsprojText =
    File.ReadAllText(repo ".template.package/FS.GG.UI.Template.fsproj")

let private pkgVersion =
    Regex.Match(templateFsprojText, "<Version>([^<]+)</Version>").Groups.[1].Value.Trim()

let private pkgOccurrences =
    Regex.Matches(templateFsprojText, "<Version>([^<]*)</Version>").Count
...
let internal tagsOrFailClosed (glob: string) (ec: int) (versions: string list) : string list =
    if ec <> 0 then
        failwithf
            "git tag --list %s failed — tags must be visible (fetch-depth: 0); fail closed rather than green-by-absence"
            glob

    if versions.IsEmpty then
        failwithf
            "no %s tags visible — CI must fetch tags (fetch-depth: 0); fail closed rather than green-by-absence"
            glob
...
let private templateTagCutFor v =
    List.contains v (gitTagVersions "fs-gg-ui-template/v*" "fs-gg-ui-template/v")

let private releaseTagCutFor v =
    List.contains v (gitTagVersions "v*" "v")
...
let feature209VersionCoherenceTests =
    testList
        "Feature209 version coherence (structural verdict mirror)"
        [
            ...
            test "preview-aware comparator orders the spec edge pairs" {
                Expect.isTrue
                    (cmp "0.1.9-preview.1" "0.1.10-preview.1" < 0)
                    "0.1.9-preview.1 < 0.1.10-preview.1 (numeric core, not lexical)"
            ...
            test "coherent baseline: single literal, pin matches latest snapshot tag (no lag, no phantom)" {
                ...
                Expect.isFalse
                    (cmp pinVersion latest < 0)
                    (sprintf "pin %s must not lag latest tag %s (pin-lags-tag)" pinVersion latest)
                ...
                let pinPending =
                    pinWaived
                        releaseLane
                        (cmp pinVersion latest > 0)
                        (templateTagCutFor pinVersion)
                        (releaseTagCutFor pinVersion)

                if not pinPending then
                    Expect.isTrue
                        (List.contains pinVersion tags)
                        (sprintf
                            "pin %s is untagged and its fs-gg-ui/v%s snapshot tag is not pending (release lane, successor already cut, or pin not ahead of tags) ⇒ the tag was never cut (pin-no-tag)"
                            pinVersion
                            pinVersion)
            }
            ...
            test "fixture: a phantom pin has no snapshot tag" {
                ...
                Expect.isFalse
                    (List.contains "0.1.99-preview.1" tags)
                    "0.1.99-preview.1 is a phantom (no fs-gg-ui/v tag)"
            }
            ...
            test "release lane: template package matches v*/template tags (no lag) and pin does not lead" {
                ...
                Expect.isFalse
                    (cmp pkgVersion latestTemplate < 0)
                    (sprintf
                        "package %s must not lag latest fs-gg-ui-template/v* tag %s (pkg-lags-template-tag)"
                        pkgVersion
                        latestTemplate)

                if not (templateTagWaived releaseLane ahead cut) then
                    Expect.isTrue
                        (List.contains pkgVersion templateTags)
                        (sprintf
                            "package %s has no fs-gg-ui-template/v%s tag, and it is not pending (release lane, successor already cut, or version not ahead of tags) ⇒ template-dispatch.yml never fired (pkg-no-template-tag)"
                            pkgVersion
                            pkgVersion)

                Expect.isFalse
                    (cmp pkgVersion latestRelease < 0)
                    (sprintf "package %s must not lag latest v* tag %s (pkg-lags-release-tag)" pkgVersion latestRelease)

                if not (releaseTagWaived releaseLane ahead) then
                    Expect.isTrue
                        cut
                        (sprintf
                            "package %s is untagged and not pending (release lane or version not ahead of tags) ⇒ the v%s release tag was never cut (pkg-no-release-tag)"
                            pkgVersion
                            pkgVersion)

                Expect.isFalse
                    (cmp pkgVersion pinVersion < 0)
                    (sprintf
                        "framework pin %s must not lead the released package %s (pin-leads-package)"
                        pinVersion
                        pkgVersion)
            }
            ...
            test
                "waiver truth table: an ahead version stays pending across repair commits, only before successors and outside the release lane" {
                // fs-gg-ui/v<pin> — successors: fs-gg-ui-template/v<pkg>, v<pkg>
                ...
                Expect.isFalse
                    (pinWaived false true true false)
                    "fs-gg-ui-template/v* cut first ⇒ snapshot tag was DUE BEFORE it (announce-before-publish)"
                ...
                Expect.isFalse
                    (templateTagWaived false true true)
                    "v* cut first ⇒ template tag was DUE BEFORE it (publish-before-announce, #250)"
                ...
                Expect.isFalse
                    (releaseTagWaived true true)
                    "release lane ⇒ a publish must be triggered by its own v* tag"
            }
                            ...
                            for tCut in [ false; true ] do
                                for rCut in [ false; true ] do
                                    if pinWaived lane pinB tCut rCut && pinB && pkgB then
                                        Expect.isTrue
                                            (templateTagWaived lane pkgB rCut)
                                            "pin waived ⇒ template tag waived (its successor set is a superset)"

                                    if templateTagWaived lane pkgB rCut then
                                        Expect.isTrue
                                            (releaseTagWaived lane pkgB)
                                            "template tag waived ⇒ v* waived (v* lands last)"
            }
            ...
            test "an empty tag list would grant every waiver, so it must be unreachable (fail closed)" {
                ...
                Expect.isTrue
                    (templateTagWaived false true cutPerEmptyList)
                    "…and 'not cut' waives pkg-no-template-tag — the #250 publish-before-announce waiver, granted by absence"

                Expect.isTrue
                    (pinWaived false true cutPerEmptyList cutPerEmptyList)
            ...
            test "guard error exits 2, not 1: an unreadable input is not reported as drift" {
                let tmp =
                    Path.Combine(
                        Path.GetTempPath(),
                        "vcoh188-unreadable-" + Guid.NewGuid().ToString("N").Substring(0, 8)
                    )
                ...
                try
                    ...
                    let ec, out =
                        runIn tmp "dotnet" [ "fsi"; Path.Combine("scripts", "validate-version-coherence.fsx") ]
            ...
            test "guard error exits 2, not 1: git failing to answer is not reported as drift" {
                let tmp =
                    Path.Combine(Path.GetTempPath(), "vcoh188-nogit-" + Guid.NewGuid().ToString("N").Substring(0, 8))
                ...
                try
                    ...
                    File.Copy(
                        repo "template/base/Directory.Packages.props",
                        Path.Combine(tmp, "template", "base", "Directory.Packages.props")
                    )
            ...
            test "release-tags.yml parses the current RELEASE-PENDING header without weakening its count check" {
                ...
                let header =
                    "RELEASE-PENDING: current versions require %d tag(s) that are not cut yet."
            ...
            test "BOM: single [$version$] token, exact bracket, B.ids == P.members" {
                ...
                for id, v in deps do
                    ...
                    Expect.isTrue
                        ...
                        (sprintf "%s must be exact-bracket" id)
            }
            ...
            test "template pins all derive through $(FsGgUiVersion) and equal the 11-member manifest" {
                ...
                for id, v in pins do
                    Expect.equal
                        v
                        "$(FsGgUiVersion)"
                        (sprintf "%s must derive through $(FsGgUiVersion), not a hardcoded literal" id)
            ...
            test "build.fsx runtime regex still resolves the literal" {
                ...
                Expect.isTrue
                    (Regex.IsMatch(buildText, "<FsGgUiVersion>\\(\\[\\^<\\]\\+\\)</FsGgUiVersion>"))
                    "build.fsx keeps the resolution regex"

                Expect.isTrue
                    (Regex.IsMatch(propsText, "<FsGgUiVersion>([^<]+)</FsGgUiVersion>"))
                    "the literal still matches that regex"
            }
            ...
            test "a release tag cut does not make the committed verdict artifact stale" {
                let tmp =
                    Path.Combine(Path.GetTempPath(), "vcoh514-" + Guid.NewGuid().ToString("N").Substring(0, 8))

                let git args =
                    let ec, out = runIn tmp "git" args

                    if ec <> 0 then
                        failwithf "git %s failed in the clone:\n%s" (String.concat " " args) out
                ...
                try
                    ...
                    let ec, out =
                        runIn (Path.GetTempPath()) "git" [ "clone"; "--quiet"; "--no-hardlinks"; root; tmp ]

                    if ec <> 0 then
                        failwithf "could not clone the repo under test:\n%s" out
                    ...
                    for (rel, currentLit) in
                        [
                            ...
                            "template/product-skills/fs-gg-symbology/reference.fsx", pinVersion
                        ] do
                    ...
                    for tag in
                        [
                            sprintf "fs-gg-ui/v%s" next
                            sprintf "fs-gg-ui-template/v%s" next
                            sprintf "v%s" next
                        ] do
                        git [ "tag"; tag ] |> ignore
                    ...
                    Expect.isFalse
                        (out.Contains "artifact-stale")
                        (sprintf
                            "cutting the release tags must not rot the committed artifact — this is #515, and it stopped the whole repo:\n%s"
                            out)
                    ...
                    let artifact =
                        File.ReadAllText(
                            Path.Combine(
                                tmp,
                                "specs",
                                "209-version-staleness-guard",
                                "readiness",
                                "version-coherence.md"
                            )
                        )

                    Expect.stringContains
                        artifact
                        (sprintf "snapshot-tag-for-pin: fs-gg-ui/v%s" next)
                        "the artifact records the snapshot tag the PIN owes — a value the tag cut cannot change"

                    Expect.isFalse
                        (artifact.Contains "latest-snapshot-tag")
                ...
                testList
                    ...
                    [
                        ...
                        test "feed lane: a cut tag whose packages ARE on the feed is a real release" {
                            let feedUrl =
                                FakeFeed.arm (fun _ -> flatContainer [ currentPin; currentPkgVersion ])
                            ...
                            Expect.equal
                                ...
                                0
                                (sprintf
                                    "every package the cut tags promise is on the feed — that is a real release:\n%s"
                                    out)

                            Expect.isFalse
                                (out.Contains "release-phantom")
                                (sprintf
                                    "a published release must never be reported as a phantom — a rule that fires on everything is as useless as one that fires on nothing:\n%s"
                                    out)
                            ...
                            Expect.isMatch
                                out
                                @"feed lane: probed [1-9]\d* package\(s\)"
                                (sprintf
                                    "the lane probed NOTHING, and still reported success — the fixture is vacuous:\n%s"
                                    out)
                        }
                        ...
                        test "feed lane: a cut tag with NO package behind it is the #679 phantom, not coherence" {
                            ...
                            Expect.equal
                                ...
                                1
                                (sprintf
                                    "a tag with nothing behind it is DRIFT (exit 1), not coherence — this is #679:\n%s"
                                    out)
                            ...
                            Expect.stringContains
                                out
                                (sprintf "fs-gg-ui/v%s" currentPin)
                            ...
                            // inside it.
                            let coreEc, _ =
                                runIn root "dotnet" [ "fsi"; repo "scripts/validate-version-coherence.fsx" ]

                            Expect.equal
                                coreEc
                                0
                                "the structural verdict-core is blind to a phantom release by construction — it never asks the feed"
                        }
                        ...
                        test "feed lane: a publish still IN FLIGHT is not a phantom" {
                            ...
                            Expect.equal
                                ...
                                0
                                (sprintf
                                    "a tag younger than the grace has its publish in flight, not abandoned:\n%s"
                                    out)

                            Expect.isFalse
                                (out.Contains "release-phantom")
                                (sprintf
                                    "an in-flight publish must never be reported as a phantom — that is the false red that kills the gate:\n%s"
                                    out)
                        ...
                        test "feed lane: a feed that does not answer is never drift, and never a silent pass" {
                            ...
                            Expect.equal
                                ...
                                0
                                (sprintf
                                    "a feed that did not answer says NOTHING about this repo and must not fail it:\n%s"
                                    out)

                            Expect.isFalse
                                (out.Contains "release-phantom")
                                (sprintf
                                    "an outage is not a phantom — collapsing the two is how a feed outage reads as 'every release is abandoned':\n%s"
                                    out)
                        ...
                        test
                            "feed lane: a 200 that is not a flat-container index is the feed NOT answering, not a phantom" {
                            ...
                            let feedUrl =
                                FakeFeed.arm (fun _ -> 200, "<html><body>503 Service Unavailable</body></html>")
                            ...
                            Expect.equal
                                ...
                                0
                                (sprintf
                                    "a 200 that is not the document we asked for is the feed failing to answer, not a phantom release:\n%s"
                                    out)
                        ...
                        test "feed lane: a member added AFTER the release tag is not a phantom of that release" {
                            let tmp =
                                Path.Combine(
                                    Path.GetTempPath(),
                                    "vcoh718-newmember-" + Guid.NewGuid().ToString("N").Substring(0, 8)
                                )
                            ...
                            try
                                ...
                                let ec, out =
                                    gitInRepo [ "clone"; "--no-hardlinks"; "--quiet"; releasedRoot.Value; tmp ]

                                Expect.equal
                                    ec
                                    0
                                    (sprintf
                                        "clone the released repo (with the pin's tag triple) to a throwaway root:\n%s"
                                        out)
                                ...
                                Expect.stringContains
                                    nuspec
                                    anchor
                                    "the BOM must carry FS.GG.UI.Scene — this test hangs its new entry off it"
                                ...
                                let ec, out =
                                    gitInClone
                                        [
                                            "-c"
                                            "user.email=fixture@fsgg"
                                            "-c"
                                            "user.name=fixture"
                                            "commit"
                                            "-am"
                                            "feat: a new packable member, no pin bump"
                                        ]
                                ...
                                let feedUrl =
                                    FakeFeed.arm (fun path ->
                                        if path.Contains "brandnew" then
                                            404, ""
                                        else
                                            flatContainer [ currentPin; currentPkgVersion ])
                                ...
                                Expect.isFalse
                                    (out.Contains "release-phantom")
                                    (sprintf
                                        "the release at the pin published everything ITS OWN BOM named; a member added afterwards was never part of it, and calling that release abandoned is a false red that camouflages a real one:\n%s"
                                        out)
                                ...
                                Expect.equal
                                    ...
                                    0
                                    (sprintf
                                        "a new packable member is a coherent commit — the feed lane must stay green on it:\n%s"
                                        out)

                                Expect.isFalse
                                    ((FakeFeed.requests ()) |> List.exists (fun p -> p.Contains "brandnew"))
                                    (sprintf
                                        "the lane must not even ASK the feed about a member the tag never promised — it reads the BOM AT THE TAG, not HEAD's src/**. Paths requested: %A"
                                        (FakeFeed.requests ()))
                            finally
                                try
                                    for f in Directory.EnumerateFiles(tmp, "*", SearchOption.AllDirectories) do
                        ...
                        test
                            "the feed rules are self-checked on every run, so a dead rule fails the guard rather than the repo" {
