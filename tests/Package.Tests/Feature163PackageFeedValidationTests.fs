...
let private packageVersions () : Map<string, string> =
    PackablePackages.packablePackageVersions ()
...
let tests =
    testList
        "Feature163 package feed validation"
        [
            test "all package-consuming samples' FS.GG.UI pins match source-controlled package versions" {
                ...
                for sample in guardedSamples do
                    let projects =
                        Directory.GetFiles(repo sample, "*.fsproj", SearchOption.AllDirectories)

                    let pins =
                        projects
                        |> Array.collect (fun project ->
                            packageReferences project |> List.map (fun pin -> project, pin) |> List.toArray)
                ...
                for sample in guardedSamples do
                    ...
                    for project in projects do
                        ...
                        Expect.isFalse
                            ...
                            $"no source ProjectReference in {project}"
            ...
            test "source-controlled readiness evidence links package proof lanes and validation records" {
                [
                    "specs/163-package-feed-validation-lanes/readiness/compatibility-ledger.md",
                    "repository validation harness/script contracts changed"
                    "specs/163-package-feed-validation-lanes/readiness/package-validation.md", "Feature163"
                    "specs/163-package-feed-validation-lanes/readiness/regression-validation.md",
