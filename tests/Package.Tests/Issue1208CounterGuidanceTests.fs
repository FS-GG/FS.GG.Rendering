...
let private skillPath =
    Path.Combine(root, "template", "product-skills", "fs-gg-testing", "SKILL.md")
...
let private fenceUnderPath path (heading: string) =
    ...
    if start < 0 then
        failwith $"counter-guidance heading is missing: {heading}"

    let fenceStart = text.IndexOf("```fsharp", start, StringComparison.Ordinal)
    let bodyStart = text.IndexOf('\n', fenceStart) + 1
    let bodyEnd = text.IndexOf("\n```", bodyStart, StringComparison.Ordinal)

    if fenceStart < 0 || bodyStart = 0 || bodyEnd < 0 then
        failwith "counter-guidance F# fence is malformed"
    ...
    testList
        ...
        [
            ...
            test "the contextual witness compiles and executes against a generated product's real API" {
                let fixtureRoot =
                    Path.Combine(Path.GetTempPath(), "fsgg-counter-api-" + Guid.NewGuid().ToString("N"))
                ...
                try
                    ...
                    Expect.equal
                        ...
                        0
                        $"game product instantiates:{Environment.NewLine}{instantiate.Output}"
                    ...
                    let materializedSkill =
                        Path.Combine(productRoot, ".agents", "skills", "fs-gg-testing", "SKILL.md")
                    ...
                    File.WriteAllText(
                        ...
                        project.Replace(
                            anchor,
                            "    <Compile Include=\"CounterCostDriverWitness.fs\" />"
                            + Environment.NewLine
                            + anchor,
                    ...
                    let executed =
                        runDotnet
                            ...
                            [
                                ...
                                projectPath
                                "--filter"
                                "Name~published-template-counter-witness"
                                "--logger"
                    ...
                    Expect.equal
                        executed.ExitCode
                        0
                        $"published template API witness compiles and executes:{Environment.NewLine}{executed.Output}"
                finally
