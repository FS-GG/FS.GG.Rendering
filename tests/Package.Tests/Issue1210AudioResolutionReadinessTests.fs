...
let private generatedArtifactProbe () =
    let temp =
        Path.Combine(Path.GetTempPath(), "fsgg-audio-artifacts-" + System.Guid.NewGuid().ToString("N"))
    ...
    try
        let templateProject =
            Path.Combine(RepositoryRoot.value, ".template.package/FS.GG.UI.Template.fsproj")

        run RepositoryRoot.value ($"pack {quote templateProject} -o {quote pack}") []

        let package =
            Directory.GetFiles(pack, "FS.GG.UI.Template.*.nupkg") |> Array.exactlyOne

        run RepositoryRoot.value ($"new install {quote package} --force --debug:custom-hive {quote hive}") []

        run
            RepositoryRoot.value
            ($"new fs-gg-ui --name ArtifactAudio --output {quote product} --profile app --lifecycle none --debug:custom-hive {quote hive}")
            []

        let tests =
            Path.Combine(product, "tests/ArtifactAudio.Tests/ArtifactAudio.Tests.fsproj")
        ...
        run
            product
            ($"test {quote tests} --no-restore --filter {writerFilter}")
            [ "FSGG_AUDIO_FIXTURE_SOURCE_ROOT", sourceAssets ]

        run product ($"build {quote project} -c Release --no-restore") []

        let buildAssets =
            Path.Combine(product, "src/ArtifactAudio/bin/Release/net10.0/assets/audio")
        ...
        let probe expected root =
            ...
            run
                product
                ($"test {quote tests} --no-restore --no-build --filter {probeFilter}")
...
let audioResolutionReadinessTests =
    testList
        "generated audio resolution readiness (#1210)"
        [
            test "cue ids are production-owned and resolution is independent from request evidence" {
                ...
                Expect.stringContains
                    ...
                    "one product-owned declaration owns the cue vocabulary"

                Expect.stringContains
                    text
                    "let resolutionEvidence () : CueResolution list"
                    "the resolver produces distinct content evidence"

                Expect.stringContains
                    text
                    "let audioContentReady ()"
                    "build/publish checks have an explicit readiness predicate"

                Expect.stringContains
                    text
                    "let writeDeterministicPlaceholder"
                    "scaffold authors have an executable placeholder option"

                Expect.stringContains
                    text
                    "request-only test must never stand in for it"
                    "guidance forbids request evidence from certifying assets"
            }
            ...
            test "missing and malformed WAVs are explicit findings, while runtime stays safely degradable" {
                ...
                Expect.stringContains
                    ...
                    "a missing declared asset is named"
                ...
                Expect.stringContains
                    text
                    "bytes.Length >= 44"
                    "a deterministic binary header floor prevents renamed text files"

                Expect.stringContains
                    text
                    "if isWave bytes then Some bytes else None"
                    "runtime resolver deliberately degrades malformed content"
            }

            test "scaffold default is deliberately incomplete instead of silently shipping silent assets" {
                let assetDirectory =
                    Path.Combine(RepositoryRoot.value, "template/base/assets/audio")

                Expect.isFalse (Directory.Exists assetDirectory) "the base scaffold has no pretend audio assets"

                Expect.stringContains
                    (source ())
                    "intentionally asset-less scaffold"
                    "the red default is documented beside its executable predicate"

                Expect.stringContains
                    (source ())
                    "deterministic reviewable PCM WAV bytes from committed source"
                    "generated binary placeholders are a reviewable option, not hidden output"

                let project =
                    File.ReadAllText(Path.Combine(RepositoryRoot.value, "template/base/src/Product/Product.fsproj"))

                Expect.stringContains
                    project
                    "CopyToOutputDirectory=\"PreserveNewest\""
                    "authored WAVs are copied into real build/publish output"
            }
