...
let packageApiReferenceTests =
    testList
        "Package API reference generation"
        [
            test "curated fsi package reference emits one source-shaped index per package" {
                ...
                |> List.iter (fun (packageId, sourcePaths) ->
                    ...
                    Expect.isTrue
                        (File.Exists(referencePath packageId))
                        $"{packageId} has a package-specific reference file"
                    ...
                    Expect.stringContains
                        ...
                        $"{packageId} reference records package id"
                    ...
                    Expect.stringContains
                        reference
                        "symbol-count:"
                        $"{packageId} reference records sampled symbol count"

                    Expect.stringContains
                        reference
                        "omitted-symbol-reasons:"
                        $"{packageId} reference records omitted symbol reasons"
                    ...
                    |> List.iter (fun sourcePath ->
                        Expect.stringContains
                            reference
                            sourcePath
                            $"{packageId} reference cites curated input {sourcePath}"))
            }
            ...
            test "source-shaped reference preserves F# authoring names and parameter labels" {
                ...
                |> List.iter (fun sample ->
                    Expect.stringContains controls sample $"Controls reference preserves {sample}")
                ...
                |> List.iter (fun sample ->
                    Expect.stringContains viewer sample $"SkiaViewer reference preserves {sample}")

                [ "KeyboardModel"; "KeyboardEvent"; "KeyDown"; "KeyUp" ]
                |> List.iter (fun sample ->
                    Expect.stringContains keyboard sample $"KeyboardInput reference preserves {sample}")
            }
            ...
            test "reference output carries XML summaries and unsupported symbol diagnostics" {
                ...
                Expect.stringContains
                    controls
                    "typed `Props`"
                    "Controls reference preserves substantive XML summaries (typed Props cross-reference)"
                ...
                |> List.iter (fun reference ->
                    Expect.stringContains
                        reference
                        "omitted-symbol-reasons:"
                        "omitted symbols are reported even when the list is empty"

                    Expect.stringContains
                        reference
                        "unsupported-symbols:"
                        "unsupported signatures are diagnostic instead of silently dropped")
            }
            ...
            test "archive API reference decision covers required packages dimensions and authority" {
                let evaluationPath =
                    Path.Combine(archiveFeatureReadinessRoot, "api-reference-generator-evaluation.md")

                Expect.isTrue
                    (File.Exists evaluationPath)
                    $"API reference generator evaluation exists at {evaluationPath}"
                ...
                |> List.iter (fun required ->
                    Expect.stringContains evaluation required $"evaluation records {required}")
            }

            test "fsdocs spike blocker records command log path reason and next action" {
                let spikePath =
                    Path.Combine(archiveFeatureReadinessRoot, "fsharp-formatting-spike.md")
