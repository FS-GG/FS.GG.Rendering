...
let private expectedGate =
    Map
        [
            "FS.GG.Audio.Core", viewerProfileGate
        ...
        [
            ...
            test "all four FS.GG.Audio.* packages are pinned through $(FsGgAudioVersion)" {
                ...
                for pkg in audioPackages do
                    let m =
                        Regex.Match(props, $"<PackageVersion\\s+Include=\"{Regex.Escape pkg}\"\\s+Version=\"([^\"]+)\"")
                    ...
                    Expect.equal
                        (m.Groups.[1].Value)
                        "$(FsGgAudioVersion)"
                        $"{pkg} derives through the audio axis, not $(FsGgUiVersion)/$(FsGgGameVersion) or a literal"
            }
            ...
            test "BOTH hosts route the INITIAL model through the audio cue seam (#458, #436)" {
                let evidenceCommands =
                    File.ReadAllText(repositoryPath "template/base/src/Product/EvidenceCommands.fs")

                let model = File.ReadAllText(repositoryPath "template/base/src/Product/Model.fs")

                let audioCues =
                    File.ReadAllText(repositoryPath "template/base/src/Product/AudioCues.fs")
