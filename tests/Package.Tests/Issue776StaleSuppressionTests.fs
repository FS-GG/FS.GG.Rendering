...
let private runSelfTest () =
    ...
    [ "scripts/apicompat-check.sh"; "--self-test" ]
...
let private runProcess workingDirectory environment command arguments =
    ...
    environment |> List.iter (fun (name, value) -> psi.Environment[name] <- value)
    ...
    testList
        ...
        [
            ...
            test "--self-test is not vacuous — it exercises every branch, break-over-stale included" {
                ...
                let oks =
                    out.Split '\n'
                    |> Array.filter (fun l -> l.TrimStart().StartsWith "ok ")
                    |> Array.length
...
"""
                    ...
                    let buildEnvironment =
                        [ "NUGET_PACKAGES", buildPackages; "NUGET_HTTP_CACHE_PATH", buildHttpCache ]
                    ...
                    for source, feed in [ authoritative, authoritativeFeed; poison, poisonFeed ] do
                        let exitCode, output =
                            runProcess
                                ...
                                [
                                    "pack"
                                    Path.Combine(source, "Probe.csproj")
                                    "-c"
                                    "Release"
                                    "-o"
                                    feed
...
"""
                    ...
                    let poisonEnvironment =
                        [ "NUGET_PACKAGES", ambientPackages; "NUGET_HTTP_CACHE_PATH", ambientHttpCache ]
                    ...
                    Expect.stringContains
                        gateOutput
                        "OK            (compatible with 1.0.0)"
                        "the authoritative compatible baseline was compared"
                    ...
                    Expect.equal
                        (Path.GetFileName gatePackages)
                        "packages"
                        "global packages live under the gate workdir"
