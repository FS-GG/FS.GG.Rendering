...
let paritySamplesPresent =
    Directory.Exists(Path.Combine(repositoryRoot, "samples", "BasicViewer"))
...
let readinessPath segments =
    let activeFeature =
        Path.Combine(repositoryRoot, "specs", "011-controls-boundary-refactor")

    let historicalFeature =
        Path.Combine(repositoryRoot, "specs", "004-keyboard-state-display")
...
let runProcess (fileName: string) (arguments: string) =
    ...
    if proc.WaitForExit(timeoutMilliseconds) then
        ...
        let stdout = if stdoutTask.Wait(1000) then stdoutTask.Result else ""

        let stderr = if stderrTask.Wait(1000) then stderrTask.Result else ""
        ...
        [
                ...
                |> List.iter (fun (sample, project) ->
                    ...
                    let programPath =
                        Path.Combine(
                            ...
                            |> Option.ofObj
                            |> Option.defaultValue repositoryRoot,
                            "Program.fs"
                        )
            ...
            test "KeyboardInputGallery contract smoke captures keyboard state display evidence" {
                ...
                let exitCode, stdout, stderr =
                    runProcess
                        "dotnet"
                        "run --project samples/KeyboardInputGallery/KeyboardInputGallery.fsproj --no-build --no-restore -- --contract-smoke"
                ...
                Expect.stringContains
                    ...
                    "smoke includes hidden display evidence"

                Expect.stringContains stdout "TextRunElement" "smoke includes rendered scene text primitive"
            }

            test
                "Controls boundary gallery contract smoke sources cover Controls-owned chart DataGrid and adapter paths" {
                skipIfNoSamples ()
                ...
                |> List.iter (fun (sample, project, requiredSource) ->
                    ...
                    Expect.stringContains
                        ...
                        $"{sample} uses Controls project"

                    Expect.isFalse
                        (projectContent.Contains("FS.GG.UI.Charts", StringComparison.Ordinal))
                        $"{sample} does not use Charts package"

                    Expect.isFalse
                        (projectContent.Contains(@"..\..\src\Charts\Charts.fsproj", StringComparison.Ordinal))
                        $"{sample} does not use removed Charts project"
...
let controlsGalleryConsumerContract =
    ...
    testList
        "Controls Gallery (feature 123) consumer-path contract"
        [
            test "the gallery's three projects exist" {
                ...
                |> List.iter (fun proj -> Expect.isTrue (File.Exists(Path.Combine(galleryRoot, proj))) $"{proj} exists")
            }

            test "consumes packed FS.GG.UI.* packages only — no src/ project references (FR-013/SC-005)" {
                let projects =
                    Directory.GetFiles(galleryRoot, "*.fsproj", SearchOption.AllDirectories)
                ...
                |> Array.iter (fun f ->
                    ...
                    Expect.isFalse
                        (content.Contains(@"..\..\src\", StringComparison.Ordinal))
                        $"{Path.GetFileName f}: no Windows-style src project ref"

                    Expect.isFalse
                        (content.Contains("/src/", StringComparison.Ordinal))
                        $"{Path.GetFileName f}: no posix src project ref"

                    Expect.isTrue
                        (content.Contains("FS.GG.UI.Controls", StringComparison.Ordinal))
                        $"{Path.GetFileName f}: references the packed Controls package")
            }
            ...
            test "the page registry references all 10 page ids (52→10 coverage source)" {
                let pages =
                    File.ReadAllText(Path.Combine(galleryRoot, "ControlsGallery.Core", "Pages.fs"))
