      ...
      Tracks =
        [ Opacity,
          [ { Time = TimeSpan.Zero; Value = ScalarValue 0.0; EasingToNext = Linear }
            { Time = ms 100.0; Value = ScalarValue 1.0; EasingToNext = Linear } ] ]
      Cues = [ { Id = "impact"; Time = ms 50.0; Payload = "sfx.impact" } ]
      Loop = Once }
    |> AnimationClip.validate
    |> Result.defaultWith (fun issues -> failwithf "%A" issues)

let private request id importance revision =
    { Id = id; AuthorityRevision = revision; Clip = animationClip (); Importance = importance }
...
let tests =
    testList "SVG session browser policy" [
        ...
        test "animation frame sampling advances presentation revision and emits only crossed live cues" {
            ...
            let initial = SvgAnimationPolicy.initialize animationConfig 7UL SvgMotionPreference.Full
            let started, startEffects = initial |> SvgAnimationPolicy.update animationConfig (SvgAnimationObservation.Start(request "player" Essential 7UL))
            Expect.equal (SvgAnimationPolicy.observe started) (7UL, 1UL, SvgAnimationStatus.Running, SvgMotionPreference.Full, 1) "start owns one effect outside authority state"
            Expect.isTrue (startEffects |> List.exists (function SvgAnimationEffect.ScheduleFrame -> true | _ -> false)) "an active effect schedules a frame"
            let advanced, effects = started |> SvgAnimationPolicy.update animationConfig (SvgAnimationObservation.Frame(ms 50.0))
            Expect.equal (SvgAnimationPolicy.observe advanced) (7UL, 2UL, SvgAnimationStatus.Running, SvgMotionPreference.Full, 1) "one sample advances only presentation revision"
            Expect.isTrue (effects |> List.exists (function SvgAnimationEffect.CueBatch("player", 7UL, [ cue ]) when cue.Cue.Id = "impact" -> true | _ -> false)) "the crossed cue is delivered once"
            let sought, seekEffects = advanced |> SvgAnimationPolicy.update animationConfig (SvgAnimationObservation.Seek("player", ms 25.0))
            Expect.isFalse (seekEffects |> List.exists (function SvgAnimationEffect.CueBatch _ -> true | _ -> false)) "seek is visually reproducible and silent"
            Expect.equal (SvgAnimationPolicy.observe sought) (7UL, 3UL, SvgAnimationStatus.Running, SvgMotionPreference.Full, 1) "seek has a monotonic presentation revision"
        }

        test "reduced motion settles declared decoration without replaying cues" {
            let animationConfig = { MaxDecorativeEffects = 1 }
            let initial = SvgAnimationPolicy.initialize animationConfig 3UL SvgMotionPreference.Reduced
            let settled, effects = initial |> SvgAnimationPolicy.update animationConfig (SvgAnimationObservation.Start(request "spark" (Decorative SvgReducedMotionBehavior.Settle) 3UL))
            Expect.equal (SvgAnimationPolicy.observe settled) (3UL, 1UL, SvgAnimationStatus.Running, SvgMotionPreference.Reduced, 0) "settled decoration owns no frame"
            Expect.isTrue (effects |> List.exists (function SvgAnimationEffect.ApplySample(_, _, _, sample) when sample.IsComplete -> true | _ -> false)) "the accessible settled value is applied"
            Expect.isFalse (effects |> List.exists (function SvgAnimationEffect.CueBatch _ -> true | _ -> false)) "reduction never replays historical cues"
            Expect.isTrue (effects |> List.exists (function SvgAnimationEffect.CancelFrame -> true | _ -> false)) "no decorative frame remains"
        }

        test "decorative limit, stale authority and lifecycle refusal preserve accepted state" {
            let animationConfig = { MaxDecorativeEffects = 1 }
            let initial = SvgAnimationPolicy.initialize animationConfig 9UL SvgMotionPreference.Full
            let one, _ = initial |> SvgAnimationPolicy.update animationConfig (SvgAnimationObservation.Start(request "one" (Decorative SvgReducedMotionBehavior.Substitute) 9UL))
            let unchanged, limit = one |> SvgAnimationPolicy.update animationConfig (SvgAnimationObservation.Start(request "two" (Decorative SvgReducedMotionBehavior.Settle) 9UL))
            Expect.equal unchanged one "excess decoration cannot mutate accepted ownership"
            Expect.equal limit [ SvgAnimationEffect.EffectRefused(SvgAnimationRefusal.DecorativeLimitReached 1) ] "degradation is deterministic"
            let same, stale = one |> SvgAnimationPolicy.update animationConfig (SvgAnimationObservation.Start(request "old" Essential 8UL))
            Expect.equal same one "stale authority cannot start presentation"
            Expect.equal stale [ SvgAnimationEffect.EffectRefused(SvgAnimationRefusal.StaleAuthorityRevision(9UL, 8UL)) ] "the revision mismatch is explicit"
            let replaced, replaceEffects = one |> SvgAnimationPolicy.update animationConfig (SvgAnimationObservation.ReplaceAuthority 10UL)
            Expect.equal (SvgAnimationPolicy.observe replaced) (10UL, 1UL, SvgAnimationStatus.Running, SvgMotionPreference.Full, 0) "replacement drops obsolete presentation only"
            Expect.equal replaceEffects [ SvgAnimationEffect.CancelFrame ] "the old frame is cancelled"
            let disposed, _ = replaced |> SvgAnimationPolicy.update animationConfig SvgAnimationObservation.Dispose
            let after, refusal = disposed |> SvgAnimationPolicy.update animationConfig (SvgAnimationObservation.Start(request "late" Essential 10UL))
            Expect.equal after disposed "disposed is terminal"
            Expect.equal refusal [ SvgAnimationEffect.EffectRefused SvgAnimationRefusal.Disposed ] "post-disposal work is refused"
        }
    ]
