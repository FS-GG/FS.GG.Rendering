...
let run () =
    ...
    let narrow, _ =
        SvgWorkspace.update (SvgWorkspaceMessage.SetViewportWidth 600.0) initial
    ...
    let restored =
        SvgWorkspace.decodeLayout (SvgWorkspace.encodeLayout closed.Layout)
    ...
    [
        ...
        yield!
            ...
            |> List.map (fun panel -> panel.Id + "=" + placement panel.Effective + "/" + placement panel.Preferred)
    ]
    |> String.concat "\n"
    |> fun value -> value + "\n"
