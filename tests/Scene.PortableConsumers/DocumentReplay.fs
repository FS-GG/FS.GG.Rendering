...
let private document objectAAvailable objectBAvailable =
    {
        ...
        Children =
            [
                if objectAAvailable then
                    element "a"
                if objectBAvailable then
                    element "b"
            ]
    }
...
let private invalidDocument () =
    let unresolved =
        { element "a" with
            ClipId = Some "missing-clip"
        }

    { document true true with
        Children = [ unresolved ]
    }
...
let private dispatch (expected: Expected) state =
    ...
    let message =
        ...
        | "SetCamera" ->
            SvgDocumentInteractionMessage.SetCamera(
                a[0],
                {
                    A = float a[3]
                    B = 0.0
...
let private replayCore mutation (text: string) =
    ...
    for line in rows do
        if firstDivergence.IsNone then
            ...
            let dispatchState =
                match mutation with
                | Some ReplayMutation.WrongOrder when
                    not mutationApplied && traceStepCount > 0 && state <> initialState ()
                    ->
            ...
            let result =
                ...
                | Some ReplayMutation.AcceptInvalidReference when
                    not mutationApplied && errorCode actualResult.Error = 6
                    ->
                    mutationApplied <- true
                    { actualResult with Error = None }
                | Some ReplayMutation.PreserveReleasedCapture when
                    not mutationApplied
                    && expected.Action = "ReleasePointer"
                    && actualResult.Error.IsNone
                    ->
                    mutationApplied <- true

                    { actualResult with
                        State =
                            { actualResult.State with
                                CapturedPointerId = stateBefore.CapturedPointerId
                            }
                    }
            ...
            let revision, selected, focused, camera, captured, objectAAvailable, objectBAvailable, outcome =
                projection result

            let actual =
                revision, selected, focused, camera, captured, objectAAvailable, objectBAvailable, outcome
            ...
            if actual <> expectedProjection then
                firstDivergence <-
                    Some
                        $"DOCUMENT-TRACE-DIVERGENCE trace={expected.Trace} step={expected.Step} action={expected.Action} expected={expectedProjection} actual={actual}"
            else
                state <- result.State
                count <- count + 1
