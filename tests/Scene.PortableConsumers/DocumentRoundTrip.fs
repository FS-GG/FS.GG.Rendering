        ...
        Shader =
            Some(
                Shader.LinearGradient(
                    point 0.0 0.0,
                    point 20.0 0.0,
                    [ color 255uy 0uy 0uy 255uy; color 0uy 0uy 255uy 255uy ]
                )
            )
...
let document =
    let hole =
        {
            Commands =
                [
                    ...
                    PathCommand.LineTo(point 35.0 5.0)
                    PathCommand.LineTo(point 35.0 35.0)
                    PathCommand.LineTo(point 5.0 35.0)
                    PathCommand.Close
                    PathCommand.MoveTo(point 15.0 15.0)
                    PathCommand.LineTo(point 25.0 15.0)
                    PathCommand.LineTo(point 25.0 25.0)
                    PathCommand.LineTo(point 15.0 25.0)
                    PathCommand.Close
                ]
                ...
                Shader =
                    Some(
                        Shader.RadialGradient(
                            point 92.0 58.0,
                            7.0,
                            [ color 255uy 255uy 255uy 255uy; color 30uy 40uy 180uy 255uy ]
                        )
                    )
            }

        {
            Nodes =
                [
                    ...
                    SceneNode.Path(
                        {
                            Commands = [ PathCommand.ArcTo(rect 80.0 5.0 20.0 20.0, 20.0, -300.0) ]
                    ...
                    SceneNode.Translate(
                        (2.0, 3.0),
                        {
                            Nodes =
                                [
                                    SceneNode.SizedText((75.0, 75.0), "all rows", 6.0, color 20uy 20uy 20uy 255uy)
                                ]
                        }
                    )
                    SceneNode.ClipNode(
                        Clip.RectClip(rect 70.0 40.0 45.0 38.0),
                        {
                            Nodes = [ SceneNode.Text((72.0, 44.0), "clip", color 0uy 0uy 0uy 255uy) ]
                        }
                    )
                ]
        }
    ...
    let symbolChild =
        leaf
            "symbol-child"
            {
                Nodes = [ SceneNode.Circle(point 5.0 5.0, 5.0, color 10uy 180uy 80uy 255uy) ]
            }
            None
    ...
    let maskChild =
        leaf
            "mask-child"
            {
                Nodes = [ SceneNode.Rectangle((0.0, 0.0, 1.0, 1.0), color 255uy 255uy 255uy 160uy) ]
            }
            None
    ...
    let gradient =
        {
            ...
            Stops =
                [
                    {
                        Offset = 0.0
                        Color = color 255uy 0uy 0uy 255uy
                        StopOpacity = 1.0
                    }
                    {
                        Offset = 0.4
                        Color = color 0uy 255uy 0uy 255uy
                        StopOpacity = 0.7
                    }
                    {
                        Offset = 1.0
                        Color = color 0uy 0uy 255uy 255uy
                        StopOpacity = 1.0
                    }
                ]
            InheritFrom = None
    ...
    let radialGradient =
        {
            ...
            Stops =
                [
                    ...
                    {
                        Offset = 1.0
                        Color = color 20uy 40uy 160uy 255uy
                        StopOpacity = 1.0
                    }
                ]
            InheritFrom = None
    ...
    let presentation =
        {
            ...
            StrokeStyle =
                Some
                    {
                        Source = SvgPaintSource.Solid(color 10uy 10uy 10uy 255uy)
                        Width = 1.5
    ...
    let luminanceElement =
        { leaf
              "luminance-element"
              {
                  Nodes = [ SceneNode.Rectangle((102.0, 2.0, 12.0, 12.0), color 255uy 255uy 255uy 255uy) ]
              }
              None with
            ...
            Presentation =
                Some
                    { presentation with
                        FillSource = Some(SvgPaintSource.Definition "radial-gradient")
                        StrokeStyle = None
                    }
        }

    {
        ...
        Definitions =
            [
                ...
                {
                    Id = "radial-gradient"
                    Content = SvgDefinitionContent.Gradient radialGradient
                }
                {
                    Id = "symbol"
                    Content = SvgDefinitionContent.Symbol(Some(rect 0.0 0.0 10.0 10.0), [ symbolChild ])
                }
                {
                    Id = "clip-base"
                    Content =
                        SvgDefinitionContent.Clip(
                            ...
                            [ SvgClipShape.Rectangle(rect 0.0 0.0 110.0 75.0) ]
                        )
                }
                {
                    Id = "clip-nested"
                    Content =
                        SvgDefinitionContent.Clip(
                            SvgCoordinateUnits.UserSpaceOnUse,
                            [ SvgClipShape.Intersection [ "clip-base" ]; SvgClipShape.Path hole ]
                        )
                }
                {
                    Id = "mask-alpha"
                    ...
                    Content =
                        SvgDefinitionContent.Mask(
                            ...
                            [
                                ...
                                |> fun child ->
                                    { child with
                                        Id = "mask-luminance-child"
                                        SemanticId = Some "semantic:mask-luminance-child"
                                    }
                            ]
                        )
                }
                {
                    ...
                    Content =
                        SvgDefinitionContent.Font
                            {
                                Family = "Noto Sans"
                                Source = "fonts/missing-noto.woff2"
...
let verifyRoundTrip runtime =
    ...
    | Ok serialized ->
        ...
        | Ok restored when SvgDocument.serialize restored <> Ok serialized ->
            failwith $"{runtime} typed document canonical round trip drifted"
        | Ok _ ->
