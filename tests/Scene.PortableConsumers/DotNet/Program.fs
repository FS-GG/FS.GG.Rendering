...
let main args =
    ...
    | SvgAdapterResult.Rendered projected when projected.RootId = scene.RootId ->
        ...
        | Ok accepted ->
            ...
            if asset.Document.Id <> scene.RootId || extension.Capabilities.Length <> 4 then
        ...
        | Ok replayed ->
            ...
            | Error divergence when divergence.StartsWith "TRACE-DIVERGENCE" ->
                printfn
                    $"retained-trace-replay: runtime=dotnet transitions={replayed.Count} action-mapping-mutant=killed stale-acceptance-mutant=killed"
