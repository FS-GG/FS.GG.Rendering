...
dotnet run --project "$work/DotNet/DotNet.fsproj" --no-build -- \
  ...
  "$work/document-interaction.traces.tsv" "$work/dotnet-document-projections.tsv" | tee "$work/dotnet-replay.log"
...
node "$work/javascript/Program.js" "$work/retained-interaction.traces.tsv" "$work/fable-projections.tsv" \
  ...
  "$work/document-interaction.traces.tsv" "$work/fable-document-projections.tsv" | tee "$work/fable-replay.log"
...
echo "portable-document-roundtrip: runtimes=dotnet,fable-node projection-sha256=$projection_sha document-interaction-sha256=$document_projection_sha document-sha256=$document_sha"
...
if [[ -n "$evidence" ]]; then
  ...
  python3 - "$work" "$evidence" "$projection_sha" "$document_projection_sha" "$document_sha" <<'PY'
import json, pathlib, re, sys
work, output = map(pathlib.Path, sys.argv[1:3])
projection_sha, document_projection_sha, document_sha = sys.argv[3:]
rows = []
...
evidence = {
    ...
    "digests": {"retainedProjectionSha256": projection_sha, "documentProjectionSha256": document_projection_sha, "documentSha256": document_sha},
    "mutants": rows,
    "claims": {"allFiveMutantsKilledAtFirstDivergenceInBothRuntimes": len(rows) == 10, "arbitraryImport": False},
}
