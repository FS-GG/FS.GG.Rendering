...
let run () =
    ...
    let clip =
        AnimationClip.validate source
        |> Result.defaultWith (fun issues -> failwith $"clip rejected: {issues}")
    ...
    |> List.map (fun (elapsed, mode) ->
        ...
        let scalarValue property =
            match sample.Values[property] with
            | ScalarValue value -> string value
            | _ -> failwith "unexpected value"

        let cues =
            sample.Cues
            |> List.map (fun occurrence -> $"{occurrence.Cue.Id}@{occurrence.Iteration}:{occurrence.Direction}")
            |> String.concat ","
