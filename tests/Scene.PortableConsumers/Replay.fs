...
let private parseLine (line: string) =
...

    {
        ...
        Camera =
            {
                PanX = float (number 12)
                PanY = float (number 13)
                Zoom = float (number 14)
            }
...
let replay mapAction mapOutcome (text: string) =
    ...
    for line in rows do
        if firstDivergence.IsNone then
            ...
            let actual =
                revision, selected, focused, camera, captured, objectAAvailable, objectBAvailable, mapOutcome outcome
            ...
            if actual <> expectedProjection then
                firstDivergence <-
                    Some
                        $"TRACE-DIVERGENCE trace={expected.Trace} step={expected.Step} action={expected.Action} expected={expectedProjection} actual={actual}"
            else
                state <- result.State
                count <- count + 1
