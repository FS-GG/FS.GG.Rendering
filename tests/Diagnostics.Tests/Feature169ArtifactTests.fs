    ...
    testList
        ...
        [
            ...
            test "Synthetic artifact writer overwrites stale prior blocker artifact" {
                let dir =
                    Path.Combine(Path.GetTempPath(), "feature169-artifacts-" + Guid.NewGuid().ToString("N"))

                try
                    Directory.CreateDirectory dir |> ignore

                    File.WriteAllText(
                        Path.Combine(dir, "diagnostics-summary.json"),
                        "{\"status\":\"blocked\",\"stale\":true}"
                    )
            ...
            test "F-DIAG-4: the .jsonl records carry the synthesized exception-problem record that drove the verdict" {
                let dir =
                    Path.Combine(Path.GetTempPath(), "feature169-jsonl-synth-" + Guid.NewGuid().ToString("N"))
                ...
                try
                    ...
                    Expect.equal
                        ...
                        "the unmatched exception drove review-required"

                    let jsonl = File.ReadAllText(Path.Combine(dir, "diagnostics-records.jsonl"))

                    Expect.stringContains
                        jsonl
                        "\"code\":\"UnmatchedDiagnosticException\""
                        "the synthesized exception-problem record is in the .jsonl"
            ...
            test
                "F-DIAG-3: a persisted summary discloses another artifact's write failure (no returned-vs-persisted drift)" {
                let dir =
                    Path.Combine(Path.GetTempPath(), "feature169-persist-drift-" + Guid.NewGuid().ToString("N"))

                try
                    Directory.CreateDirectory dir |> ignore
                    // Make the .jsonl write fail (its path is a directory) while the summary writes succeed.
                    Directory.CreateDirectory(Path.Combine(dir, "diagnostics-records.jsonl"))
                    |> ignore
                    ...
                    Expect.equal
                        summary.Status
                        ReadinessDiagnosticStatus.ReviewRequired
                        "the write failure requires review"
                    ...
                    Expect.stringContains
                        ...
                        "the persisted .json discloses the .jsonl write failure"
                    ...
                    Expect.stringContains
                        ...
                        "the persisted .json artifactWriteDiagnostics array is populated"
                    ...
                    Expect.stringContains
                        md
                        "status: `review-required`"
                        "the persisted .md discloses the .jsonl write failure"

                    Expect.stringContains
                        md
                        "## Artifact Write Warnings"
                        "the persisted .md carries the write-warnings section"
