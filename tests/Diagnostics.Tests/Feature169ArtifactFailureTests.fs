...
let tests =
    testList
        "Feature169 artifact failure"
        [
            test "Synthetic artifact write failure emits developer-action diagnostic" {
                let filePath =
                    Path.Combine(Path.GetTempPath(), "feature169-artifact-root-" + Guid.NewGuid().ToString("N"))
                ...
                try
                    ...
                    Expect.equal
                        summary.ArtifactWriteDiagnostics.Head.Category
                        (Some DiagnosticCategory.DeveloperAction)
                        "developer-action category"
                finally
