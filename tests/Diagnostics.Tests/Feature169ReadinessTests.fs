...
let tests =
    testList
        "Feature169 readiness"
        [
            test "Synthetic non-blocking classified diagnostics are accepted" {
                let summary =
                    Feature169Fixtures.summarize
                        [ Feature169Fixtures.backendCostAt 1; Feature169Fixtures.renderingLimitation ]

                Expect.equal
                    summary.Status
                    ReadinessDiagnosticStatus.Accepted
                    "classified non-blocking diagnostics are accepted"
            }
            ...
            test "Synthetic expired or unmatched exceptions require review" {
                ...
                let expiredSummary =
                    RuntimeDiagnostics.summarize None [ expired ] [] [ Feature169Fixtures.blocker ]

                let unmatchedSummary =
                    RuntimeDiagnostics.summarize None [ unmatched ] [] [ Feature169Fixtures.backendCostAt 1 ]

                Expect.equal
                    expiredSummary.Status
                    ReadinessDiagnosticStatus.ReviewRequired
                    "expired exception requires review"

                Expect.equal
                    unmatchedSummary.Status
                    ReadinessDiagnosticStatus.ReviewRequired
                    "unmatched exception requires review"
            }
            ...
            test "F-DIAG-2: summarizeAt evaluates ExpiresOn against the injected date, deterministically" {
                ...
                Expect.equal
                    dayBefore.Status
                    ReadinessDiagnosticStatus.Accepted
                    "valid the day before expiry accepts the blocker"

                Expect.equal
                    onExpiry.Status
                    ReadinessDiagnosticStatus.Accepted
                    "valid on the expiry day (expires >= now) accepts the blocker"

                Expect.equal
                    dayAfter.Status
                    ReadinessDiagnosticStatus.ReviewRequired
                    "expired the day after -> exception invalid -> review required"
            ...
            test "F-DIAG-2: summarize equals summarizeAt at today's UTC date (adapter is a faithful wrapper)" {
                ...
                let viaAdapter =
                    RuntimeDiagnostics.summarize None [] [] [ Feature169Fixtures.blocker ]

                let viaCore =
                    RuntimeDiagnostics.summarizeAt today None [] [] [ Feature169Fixtures.blocker ]
            ...
            test "Synthetic environment error becomes environment-limited when no blocker remains" {
                ...
                Expect.equal
                    summary.Status
                    ReadinessDiagnosticStatus.EnvironmentLimitedStatus
            ...
            test "F-DIAG-1: an Error-severity rendering limitation blocks, it does not fall through to accepted" {
                ...
                let summary =
                    Feature169Fixtures.summarize [ Feature169Fixtures.renderingLimitationError ]

                Expect.equal
                    summary.Status
                    ReadinessDiagnosticStatus.Blocked
                    "Error-severity rendering limitation blocks"
            }
            ...
            test "F-DIAG-1: a benign warning-severity rendering limitation is still accepted" {
                ...
                let summary =
                    Feature169Fixtures.summarize
                        [ Feature169Fixtures.renderingLimitation; Feature169Fixtures.backendCostAt 1 ]

                Expect.equal
                    summary.Status
                    ReadinessDiagnosticStatus.Accepted
                    "warning-severity limitation stays accepted"
            }
            ...
            test "F-DIAG-1: a valid exception clears the Error-severity floor to accepted" {
                ...
                Expect.equal
                    summary.Status
                    ReadinessDiagnosticStatus.Accepted
                    "excepted Error-severity limitation no longer blocks"
            }
        ]
