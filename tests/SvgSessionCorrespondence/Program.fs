...
let contract =
    { Initialize = fun _ -> Ok { Ticks = 0UL; Revision = 0UL }
      ...
      Advance = fun advance world -> Ok { Ticks = world.Ticks + advance.StepCount; Revision = world.Revision + 1UL }
      Project = fun world -> { SessionId = "correspondence"; Revision = world.Revision; Value = world.Ticks }
      Snapshot = snapshot
      Restore = fun value -> Ok value.Value }

let runtimeConfig = { StepMicroseconds = 16667UL; MaxCatchUpSteps = 4u }
let request = { SessionId = "correspondence"; Compatibility = compatibility; Configuration = () }
let mutable runtime = SessionRuntime.initialize runtimeConfig contract request |> Result.defaultWith (fun error -> failwithf "%A" error)
...
let runtimeObservation observation =
    ...
    |> List.iter (function
        ...
        | SessionRuntimeEffect.ProjectionReady projection -> output.Add($"game-projection:{projection.Revision}:{projection.Value}")
        | SessionRuntimeEffect.Disposed -> output.Add("game-disposed")
        | _ -> ())

let apply observation =
    let next, effects = SvgSessionPolicy.update { SuspensionMilliseconds = 1000.0 } observation policy
    policy <- next
    effects
    |> List.iter (function
        | SvgSessionPolicyEffect.AdvanceElapsed elapsed -> runtimeObservation (SessionRuntimeObservation.AdvanceElapsed elapsed)
        ...
        | SvgSessionPolicyEffect.ProjectionRejected(expected, actual, revision) -> output.Add($"reject-generation:{expected}:{actual}:{revision}")
        | SvgSessionPolicyEffect.ProjectionRevisionRejected(accepted, candidate) -> output.Add($"reject-revision:{accepted}:{candidate}")
