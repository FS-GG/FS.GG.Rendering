        ...
        [
            ...
            test "a capability skill's canonical-id wrapper does NOT satisfy it — that name is the framework's" {
                ...
                try
                    ...
                    Expect.isNonEmpty
                        findings
                        "the product wrapper `fs-gg-product-scene` is absent; a bare `fs-gg-scene` wrapper is the FRAMEWORK skill's and must not satisfy the product requirement (Feature 223)"

                    Expect.all
                        findings
                        (fun f -> f.Severity = SkillParity.High)
                        "High — FailOnSeverity is High, so a Warning here is a gate that does not fail"

                    Expect.stringContains
                        findings.Head.Message
                        "fs-gg-product-scene"
                        "the finding names the wrapper that is actually required"
                finally
            ...
            test "a capability skill is satisfied by its fs-gg-product-* alias" {
                ...
                try
                    ...
                    Expect.isEmpty
                        (missingWrapperFindings root)
                        "the alias is the required name for a body supplied from template/product-skills/"
                finally
                    Feature168SkillParityFixtures.deleteTempRoot root
            }
            ...
            test "an off-convention skill takes the canonical id, and the alias does not satisfy it" {
                ...
                try
                    ...
                    Expect.isNonEmpty
                        findings
                        "a body supplied off the template/product-skills/ convention is wrapped under its canonical id"
            ...
            test "an off-convention skill is satisfied by its canonical id" {
                ...
                try
                    ...
                    Expect.isEmpty
                        (missingWrapperFindings root)
                        "the canonical id is the required name for a body supplied off the convention — this is what the feedback pair, fs-gg-project and fs-gg-samples all do"
                finally
                    Feature168SkillParityFixtures.deleteTempRoot root
            }
            ...
            test "when two rules collide on one finding id, the WORSE severity survives" {
                ...
                try
                    ...
                    let collided =
                        report.Findings
                        |> List.filter (fun f -> f.Category = SkillParity.MissingWrapper && f.SkillName = "fs-gg-scene")
            ...
            test "this repository's product wrappers all obey the rule, and both branches are populated" {
                ...
                Expect.isGreaterThan
                    (List.length onConvention)
                    0
                    "some product skills are supplied from the convention (else the alias branch guards nothing)"

                Expect.isGreaterThan
                    (List.length offConvention)
                    0
                    "some product skills are supplied OFF the convention (else the canonical-id branch guards nothing — and it is the branch #489 was about)"
