...
let private discoveryErrorFrom (f: unit -> 'a) : string =
    try
        ...
        failtest
            "expected a PackageDiscoveryError, but discovery returned a result — the input was silently dropped, which is the #677 fails-open bug"
...
let tests =
    testList
        "Feature677 pack-path discovery fails closed"
        [
            ...
            test "a packable project with no inline <Version> is discovered at the version it inherits" {
                ...
                try
                    Feature163TestFixtures.writeFile root "Directory.Build.props" (propsWithVersion "9.9.9")
                    |> ignore

                    Feature163TestFixtures.writeFile root "src/Scene/Scene.fsproj" (packableProject "FS.GG.UI.Scene")
                    |> ignore
            ...
            test "Directory.Build.local.props wins over Directory.Build.props (it is imported last)" {
                ...
                try
                    Feature163TestFixtures.writeFile root "Directory.Build.props" (propsWithVersion "1.0.0-org")
                    |> ignore

                    Feature163TestFixtures.writeFile root "Directory.Build.local.props" (propsWithVersion "2.0.0-repo")
                    |> ignore
            ...
            test "an inline <Version> still wins over the inherited one" {
                ...
                try
                    ...
                    Feature163TestFixtures.writePackageProject root "src/Scene/Scene.fsproj" "FS.GG.UI.Scene" "1.2.3"
                    |> ignore
            ...
            test "a packable project whose version resolves to nothing is a hard error, not an omission" {
                ...
                try
                    ...
                    let message =
                        discoveryErrorFrom (fun () -> PackageFeed.discoverPackablePackages root (feedOf root))
            ...
            test "an unparseable .fsproj under src/ fails the run and names the file" {
                ...
                try
                    ...
                    Feature163TestFixtures.writeFile root "src/Broken/Broken.fsproj" "<Project><PropertyGroup>"
                    |> ignore
...
"""
                    ...
                    Feature163TestFixtures.writeFile root "src/Other/Other.fsproj" (packableProject "Contoso.Widgets")
                    |> ignore

                    let packages = PackageFeed.discoverPackablePackages root (feedOf root)

                    Expect.equal
                        (packages |> List.map _.PackageId)
                        [ "FS.GG.UI.Scene" ]
                        "only the project that declares itself ours is discovered — and the others raise nothing"
                finally
                    Feature163TestFixtures.deleteTempRoot root
            }
...
"""
            ...
            for trailing in [ ""; string Path.DirectorySeparatorChar ] do
                let label =
                    if trailing = "" then
                        ...
                        " (even when the root is given with a trailing separator)"
                ...
                test $"version resolution never walks above the repository root{label}" {
                    ...
                    try
                        ...
                        Feature163TestFixtures.writeFile
                            ...
                            (propsWithVersion "6.6.6-outside-the-repo")
                        |> ignore

                        Feature163TestFixtures.writeFile
                            root
                            "src/Scene/Scene.fsproj"
                            (packableProject "FS.GG.UI.Scene")
                        |> ignore

                        let message =
                            discoveryErrorFrom (fun () ->
                                PackageFeed.discoverPackablePackages (root + trailing) (feedOf root))

                        Expect.stringContains
                            message
                            "FS.GG.UI.Scene"
                            "the version is UNRESOLVED — a props file above the checkout is not this repository's"
                    finally
            ...
            test "an unparseable sample project fails the pin read rather than contributing no pins" {
                ...
                try
                    ...
                    Feature163TestFixtures.writeFile root "samples/Demo/Demo.fsproj" "<Project><ItemGroup>"
                    |> ignore
                    ...
                    let message =
                        discoveryErrorFrom (fun () ->
                            PackageFeed.readSelectedPackagePins root [ "samples/Demo" ] packages Set.empty [])
