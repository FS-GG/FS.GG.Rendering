...
let private attempt scenario promotion reuse reason net : Compositor.Types.Feature159Attempt =
    {
        ...
        ArtifactPaths =
            [
                $"promotion/attempts/{Compositor.FeatureState.feature159ScenarioFileName scenario}"
            ]
        Diagnostics = []
    }
    ...
    testList
        ...
        [
            ...
            test "promotion attempt report renders split identity and counters" {
                let rendered =
                    Compositor.Render3.emitFeature159AttemptReport (
                        attempt "promotion/placement-only-move" "promoted" "content-reused-placement-updated" None 10
                    )
            ...
            test "workflow records attempts and publishes artifacts as effects" {
                ...
                Expect.contains
                    effects
                    (Compositor.Types.Feature159DeclarePolicy Compositor.Config.feature159PolicyId)
                    "policy effect"

                let model', effects' =
                    Compositor.FeatureState.updateFeature159
                        (Compositor.Types.Feature159AttemptRecorded(
                            attempt "promotion/static-retained" "promoted" "content-recorded" None 12
                        ))
                        model
                ...
                Expect.contains
                    effects'
                    (Compositor.Types.Feature159WriteArtifact Compositor.Config.feature159PromotionSummaryPath)
                    "summary effect"
            }
        ]
