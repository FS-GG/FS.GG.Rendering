    ...
    testList
        ...
        [
            test "declares host lane constants required scenarios and exclusion tokens" {
                ...
                Expect.equal
                    Compositor.Config.feature161RequiredScenarioIds
                    Compositor.Config.feature160RequiredScenarioIds
                    "inherits Feature 160 timing scenarios"
            ...
            test "validates complete host facts and names primary missing or contradictory facts" {
                Expect.equal
                    (Compositor.FeatureState.feature161LaneIdFromFacts facts)
                    Compositor.Config.feature161HostLaneId
                    "lane id"

                Expect.isNone (Compositor.FeatureState.feature161ValidateHostFacts facts) "complete facts"

                Expect.equal
                    (Compositor.FeatureState.feature161ValidateHostFacts { facts with DisplayIdentity = "" })
                    (Some Perf.MissingDisplay)
                    "missing display"

                Expect.equal
                    (Compositor.FeatureState.feature161ValidateHostFacts { facts with RendererIdentity = "" })
                    (Some Perf.UnknownRenderer)
                    "unknown renderer"

                Expect.equal
                    (Compositor.FeatureState.feature161ValidateHostFacts
                        { facts with
                            DirectRendering = Some false
                        })
                ...
                Expect.equal
                    (Compositor.FeatureState.feature161ValidateHostFacts
                        { facts with
                            TimingPolicyIdentity = "other-policy"
                        })
            ...
            test "MVU records host facts prior gates ledger entries and publication artifacts" {
                ...
                Expect.contains
                    effects0
                    (Compositor.Types.Feature161DeclarePolicy Compositor.Config.feature161PolicyId)
                    "policy"
                ...
                Expect.contains
                    effects0
                    (Compositor.Types.Feature161LoadThroughputPackage "throughput")
                    "source throughput"

                let accepted = entry Compositor.Types.Feature161ReadinessStatus.Accepted None

                let model1, _ =
                    Compositor.FeatureState.updateFeature161
                        (Compositor.Types.Feature161HostProfileDetected profile)
                        model0

                let model2, _ =
                    Compositor.FeatureState.updateFeature161
                        (Compositor.Types.Feature161PolicyDeclared Compositor.Config.feature161PolicyId)
                        model1

                let model3, _ =
                    Compositor.FeatureState.updateFeature161
                        (Compositor.Types.Feature161HostFactsCollected facts)
                        model2

                let model4, _ =
                    Compositor.FeatureState.updateFeature161
                        (Compositor.Types.Feature161PriorGateLinked Compositor.Config.feature161PriorGateLinks.Head)
                        model3

                let model5, _ =
                    Compositor.FeatureState.updateFeature161
                        (Compositor.Types.Feature161LedgerEntryRecorded accepted)
                        model4

                let model6, _ =
                    Compositor.FeatureState.updateFeature161
                        (Compositor.Types.Feature161ArtifactPublished "lane-ledger/summary.md")
                        model5
            ...
            test "renders historical P7 gate status in reviewer-visible ledger entry" {
                let rendered =
                    Compositor.Render4.emitFeature161LedgerEntry (
                        entry Compositor.Types.Feature161ReadinessStatus.Accepted None
                    )
