...
let feature146RenderAnywhereEvidenceTests =
    testList
        "Feature146 render-anywhere evidence"
        [
            test "corpus exposes three representative packages" {
                ...
                Expect.equal
                    (corpus |> List.map _.ScenarioId)
                    [ "basic-primitives"; "layered-portal"; "shaped-text" ]
                    "scenario ids are stable"

                corpus
                |> List.iter (fun item ->
                    Expect.stringStarts item.Package.PackageIdentity "sha256:" "package identity is sha256")
            }

            test "reference command writes summary evidence" {
                let out =
                    System.IO.Path.Combine(System.IO.Path.GetTempPath(), "fs-gg-feature146-reference-test")
            ...
            test "written reference summary reads back into the entries the report cites" {
                let out =
                    System.IO.Path.Combine(System.IO.Path.GetTempPath(), "fs-gg-feature146-reference-roundtrip")
            ...
            test "an absent reference summary reads as no evidence rather than throwing" {
                let out =
                    System.IO.Path.Combine(System.IO.Path.GetTempPath(), "fs-gg-feature146-reference-absent")
