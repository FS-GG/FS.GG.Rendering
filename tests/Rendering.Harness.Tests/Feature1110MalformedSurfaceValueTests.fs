...
let private scratch (name: string) =
    Feature168SkillParityFixtures.createTempRoot name

let private reportPathIn (outRoot: string) =
    Path.Combine(outRoot, "out", "report.md")

let private summaryPathIn (outRoot: string) =
...
let private contractLines () =
    File.ReadAllLines contractPath |> Array.toList
...
let private documentedSurfaceErrorCode () =
    ...
    |> List.tryPick (fun (code, meaning) ->
        if
            meaning.IndexOf("surface configuration error", StringComparison.OrdinalIgnoreCase)
            >= 0
        then
            Some code
        else
...
let malformedSurfaceValueTests =
    testSequenced (
        testList
            "Feature1110 a malformed --surface value is refused, not dropped"
            [

                // ---------- Acceptance criteria 1, 2 and 4 ----------

                test
                    "every --surface value the contract calls an error exits with the documented code and writes nothing" {
                    ...
                    try
                        ...
                        let controlExit, controlOut, _ =
                            captureStreams (fun () ->
                                SkillParity.runCli (
                                    cliArgs controlRoot [ "--surface"; "fsgg-1110-probe=" + probeRoot ]
                                ))
                        ...
                        for value in refused do
                            ...
                            try
                                let exitCode, stdout, stderr =
                                    captureStreams (fun () ->
                                        SkillParity.runCli (cliArgs outRoot [ "--surface"; value ]))
                                ...
                                Expect.isNone
                                    (lines stdout
                                     |> List.tryFind (fun line ->
                                         line.StartsWith("skill-parity status:", StringComparison.Ordinal)))
                                    (sprintf
                ...
                test "the contract's refused list names both the empty-id and the empty-path spelling" {
                    ...
                    Expect.isTrue
                        (refused
                         |> List.exists (fun value -> value.StartsWith("=", StringComparison.Ordinal)))
                        (sprintf
                    ...
                    Expect.isTrue
                        (refused
                         |> List.exists (fun value -> value.EndsWith("=", StringComparison.Ordinal) && value.Length > 1))
                        (sprintf
                ...
                test "a trailing --surface with no value is refused too, and is not an unknown option" {
                    ...
                    try
                        ...
                        Expect.equal
                            ...
                            expectedCode
                            (sprintf
                                "a `--surface` with no value is a surface configuration error. stderr was:\n%s"
                                stderr)
                        ...
                        Expect.isFalse (File.Exists(reportPathIn outRoot)) "and nothing was regenerated"
                ...
                test "one malformed value refuses the whole run, even alongside a well-formed one" {
                    ...
                    try
                        ...
                        let exitCode, _, stderr =
                            captureStreams (fun () ->
                                SkillParity.runCli (
                                    cliArgs
                                        outRoot
                                        [ "--surface"; "fsgg-1110-probe=" + probeRoot; "--surface"; "no-equals-here" ]
                                ))
                        ...
                        Expect.stringContains stderr "no-equals-here" "and names the offending value"
