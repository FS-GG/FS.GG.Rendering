...
let private summary entries : Compositor.Types.Feature161Summary =
    ...
    { provisional with
        Status = Compositor.FeatureState.feature161OverallStatus provisional
    }

[<Tests>]
let tests =
    testList
        "Feature161 LaneLedger Unsupported"
        [
            ...
            test "cross-lane aggregation is rejected and not counted as accepted evidence" {
                let wrongLane =
                    entry
                        "wayland"
                        "wayland-wayland-0-direct-opengl-amd-mesa"
                        Compositor.Types.Feature161ReadinessStatus.Rejected
                        (Some Perf.CrossLaneEvidence)
                        0
                ...
                let rendered =
                    Compositor.Render4.emitFeature161ExcludedEvidenceReport Perf.CrossLaneEvidence [ wrongLane ]
                ...
                let cases =
                    [
                        { facts with
                            ...
                            DisplayIdentity = "missing-display"
                        },
                        Perf.MissingDisplay
                        { facts with
                        ...
                        { facts with
                            RendererIdentity = "llvmpipe software rasterizer"
                        },
                        Perf.SoftwareRaster
                        { facts with RendererIdentity = "" }, Perf.UnknownRenderer
                        { facts with
                            EnvironmentLimits = [ "virtualized-presentation" ]
                        },
                        Perf.VirtualizedPresentation
                        { facts with
                            PackageVersionSet = "stale-package"
                        },
                        Perf.PackageVersionMismatch
                        { facts with
                            CpuLoadNote = "non-representative load"
                        },
                        Perf.LoadNonRepresentative
                    ]

                cases
                |> List.iter (fun (caseFacts, expected) ->
                    Expect.equal
                        (Compositor.FeatureState.feature161ValidateHostFacts caseFacts)
                        (Some expected)
                        (Perf.exclusionReasonToken expected))
            }
            ...
            test "summary renders lane facts excluded evidence unsupported result and final claim status" {
                ...
                let rejected =
                    entry
                        ...
                        Compositor.Config.feature161HostLaneId
                        Compositor.Types.Feature161ReadinessStatus.Rejected
                        (Some Perf.NoisyTiming)
                        0

                let rendered =
                    Compositor.Render4.emitFeature161LaneLedgerSummary (summary [ accepted; rejected ])
