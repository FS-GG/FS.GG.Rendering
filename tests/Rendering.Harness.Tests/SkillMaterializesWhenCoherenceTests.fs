module SkillMaterializesWhenCoherenceTests

// FS-GG/FS.GG.Rendering#366 — a PR-VISIBLE twin of the release-only skill-manifest materializes-when
// gate.
//
// WHY THIS EXISTS. The product `template/skill-manifest/skill-manifest.json` records, per skill, WHEN
// its body materializes (`materializes-when`) and WHERE the body comes from (`supplied-by`). The
// skill-union gate (.github scripts/skill-union-assert.sh) and the typed Fsgg.Registry validator read
// `materializes-when` to tell a legitimate profile-scoped suppression from a real `[missing]`, so a
// manifest whose predicate drifts out of the ADR-0017 canonical grammar — or out of agreement with the
// authoritative template.json body-source `condition` — makes the union gate misjudge every scaffold
// (issue #77). `Feature238SkillMaterializesWhenTests` (tests/Package.Tests) is the gate that keeps the
// manifest honest against template.json. But Package.Tests is RELEASE-ONLY: it is not in
// `FS.GG.Rendering.slnx` and runs only under `dotnet test … -c Release` in release.yml. So a PR that
// edits template.json's conditions without regenerating the manifest (or vice versa) compiles green and
// only reds the release lane post-merge — the exact "PR-gated drift gates must be in the slnx" gap
// #350, #382, the BOM slice and the api-surface-mirror slice already closed.
//
// WHAT IT LOCKS. `Feature238SkillMaterializesWhenTests` is already fully static and self-contained: no
// pack, no restore, no `dotnet new`, no GL — it reads the manifest and template.json as files on disk
// and evaluates their conditions over a fixed parameter grid. So this hoist is a FAITHFUL mirror: it
// reads the SAME two source-of-truth inputs (the skill-manifest and template.json) and asserts the SAME
// invariants, one gate earlier —
//   G-PRESENT     every entry carries non-empty materializes-when + supplied-by.
//   G-GRAMMAR     every materializes-when is in the ADR-0017 canonical grammar and parses under the
//                 tiny gate evaluator.
//   G-GRID        the parameter grid varies every param a condition branches on (keeps G-EQUIV honest).
//   G-EQUIV       materializes-when(id) is SEMANTICALLY EQUAL to the verbatim template.json body-source
//                 condition over the full parameter grid — manifest → template.json without pinning the
//                 exact normalized string.
//   G-SUPPLIEDBY  supplied-by(id) == dirname(canonical source) + "/".
//   G-HONESTY     fs-gg-project materializes on every lifecycle for a composed profile (issue #91) and
//                 the canonical evaluator is faithful to the grammar over the gated skills.
//
// Kept in deliberate lockstep with tests/Package.Tests/Feature238SkillMaterializesWhenTests.fs: the two
// read the same repo inputs and assert the same invariants, so a real drift fails BOTH — the release
// check is mirrored earlier, never weakened. ReleaseOnlyTwinLockstepTests guards that pairing so the
// mirror cannot silently desync. This body is a verbatim hoist of the release-only rule (module name,
// [<Tests>] binding name and testList label aside); keeping it byte-faithful is what makes the L-INPUTS
// lockstep check exact.
...
[<Tests>]
let skillMaterializesWhenCoherenceTests =
    testList
        "#366 — skill-manifest materializes-when coherence (PR-time twin of the release-only Feature238SkillMaterializesWhenTests gate)"
        [
