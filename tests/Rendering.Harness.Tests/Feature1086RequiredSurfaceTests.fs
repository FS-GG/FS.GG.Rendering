...
let requiredSurfaceTests =
    testList
        "Feature1086 a required surface that resolves to zero files fails the gate"
        [
            ...
            test "an empty REQUIRED synthetic fixture surface is reported" {
                ...
                try
                    ...
                    let report =
                        SkillParity.runCheck (Feature168SkillParityFixtures.request root "wrapper-only")

                    let reported =
                        unreadable report |> List.map (fun finding -> finding.SurfaceId) |> Set.ofList
            ...
            test "an empty NON-required surface is left alone" {
                ...
                try
                    ...
                    Expect.contains
                        reported
                        "fixture-canonical"
                        "the required empty surface IS reported in this very run"
