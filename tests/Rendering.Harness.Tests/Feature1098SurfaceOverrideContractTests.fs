...
let private lineStartingWith (prefix: string) (text: string) =
    lines text
    |> List.tryFind (fun line -> line.StartsWith(prefix, StringComparison.Ordinal))

let private declaredSurfaces () =
...
let surfaceOverrideContractTests =
    testSequenced (
        testList
            "Feature1098 --surface and its contract say the same thing, out loud"
            [
                ...
                test "the documented meaning of --surface is the measured meaning of --surface" {
                    ...
                    try
                        ...
                        let inspected =
                            report.SupportedSurfaces |> List.map (fun surface -> surface.SurfaceId)
                ...
                test "a narrowed run says so on stdout, and a full run says it was full" {
                    ...
                    try
                        ...
                        let fullLine =
                            ...
                            | Some line -> line
                            | None ->
                                failtestf "the unnarrowed run printed no `surfaces:` line. stdout was:\n%s" fullOut
                        ...
                        let narrowLine =
                            ...
                            | Some line -> line
                            | None ->
                                failtestf "the narrowed run printed no `surfaces:` line. stdout was:\n%s" narrowOut
                ...
                test "the narrowing reaches the machine-readable channels too, and names what went unchecked" {
                    ...
                    try
                        ...
                        Expect.stringContains
                            jsonOut
                            "\"surfacesChecked\":1"
                            "the --json object reports how many surfaces were checked"
                        ...
                        let report =
                            SkillParity.runCheck (
                                requestIn reportRoot [ "fsgg-1098-probe", "docs/product/ant-design/skill/SKILL.md" ]
                            )

                        let caveat =
                            report.Caveats |> List.tryFind (fun caveat -> caveat.Contains "--surface")
                ...
                test "--fixture with --surface replaces the FIXTURE set, resolved beneath the fixture root" {
                    ...
                    try
                        ...
                        let fixtureIds =
                            control.SupportedSurfaces |> List.map (fun surface -> surface.SurfaceId)
                        ...
                        let overridden =
                            SkillParity.runCheck (fixtureRequest [ "fixture-canonical", "canonical" ])

                        Expect.equal
                            (overridden.SupportedSurfaces
                             |> List.map (fun surface -> surface.SurfaceId, surface.Roots))
                        ...
                        let caveat =
                            ...
                            | None ->
                                failtestf
                                    "the narrowed fixture run carries no narrowing caveat; caveats were %A"
                                    overridden.Caveats
