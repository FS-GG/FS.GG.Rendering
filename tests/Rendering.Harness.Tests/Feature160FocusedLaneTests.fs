...
let private scenarioReport scenario : Compositor.Types.Feature158ScenarioReport =
    {
        ...
        ArtifactPaths =
            [
                $"throughput/iterations/{Compositor.FeatureState.feature160ScenarioFileName scenario}"
            ]
        Diagnostics = []
    }
...
let private acceptedIteration index : Compositor.Types.Feature160Iteration =
    ...
    let reports =
        Compositor.Config.feature160RequiredScenarioIds |> List.map scenarioReport

    {
        ...
        ArtifactPaths =
            [
                $"throughput/iterations/{Compositor.FeatureState.feature160IterationFileName iterationId}"
            ]
        ...
        [
            test "declares focused lane constants scenarios bound and exclusion tokens" {
                ...
                Expect.equal
                    ...
                    "Feature 158 scenario set"

                Expect.contains Compositor.Config.feature160RequiredScenarioIds "timing/edge-clipping" "edge clipping"

                Expect.contains
                    Compositor.Config.feature160ScenarioIds
                    "timing/sparse-heavy-localized-update"
                    "damage-friendly debug scenario"

                Expect.isFalse
                    (Compositor.Config.feature160RequiredScenarioIds
                     |> List.contains "timing/sparse-heavy-localized-update")
                    "debug scenario is not an acceptance requirement"
            ...
            test "MVU declares host lane policy bound timeout and iteration publication effects" {
                ...
                Expect.contains
                    effects0
                    (Compositor.Types.Feature160DeclareFocusedLane Compositor.Config.feature160FocusedLaneId)
                    "lane"

                Expect.contains
                    effects0
                    (Compositor.Types.Feature160DeclarePolicy Compositor.Config.feature160PolicyId)
                    "policy"
                ...
                let model1, _ =
                    Compositor.FeatureState.updateFeature160
                        (Compositor.Types.Feature160HostProfileDetected profile)
                        model0

                let model2, _ =
                    Compositor.FeatureState.updateFeature160
                        (Compositor.Types.Feature160LaneDeclared Compositor.Config.feature160FocusedLaneId)
                        model1

                let model3, _ =
                    Compositor.FeatureState.updateFeature160
                        (Compositor.Types.Feature160PolicyDeclared Compositor.Config.feature160PolicyId)
                        model2

                let model4, _ =
                    Compositor.FeatureState.updateFeature160 (Compositor.Types.Feature160BoundDeclared 10) model3

                let model5, _ =
                    Compositor.FeatureState.updateFeature160
                        (Compositor.Types.Feature160IterationCompleted iteration)
                        model4

                let model6, _ =
                    Compositor.FeatureState.updateFeature160
                        (Compositor.Types.Feature160ArtifactPublished "throughput/summary.md")
                        model5
            ...
            test "restricted single-scenario debugging cannot satisfy final throughput acceptance" {
                ...
                Expect.equal
                    (Compositor.FeatureState.feature160FocusedThroughputStatus summary)
                    Compositor.Types.Feature160ReadinessStatus.Rejected
                    "restricted scenario rejected"
            ...
            test "CLI writes focused package and preserves broad validation separation" {
                let root =
                    Path.Combine(Path.GetTempPath(), "feature160-focused-" + Guid.NewGuid().ToString("N"))
