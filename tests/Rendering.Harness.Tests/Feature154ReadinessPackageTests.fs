        ...
        [
            ...
            test "MVU publication contract records proof parity timing and artifact effects" {
                ...
                Expect.exists
                    effects0
                    (function
                    | Compositor.Types.WriteFeature154Artifact path -> path.EndsWith("validation-summary.md"))
                    "initial summary effect"

                let model1, _ =
                    Compositor.FeatureState.updateFeature154
                        (Compositor.Types.ProofEvidenceRecorded "environment-limited")
                        model0

                let model2, _ =
                    Compositor.FeatureState.updateFeature154
                        (Compositor.Types.ParityEvidenceRecorded "fallback-gated")
                        model1

                let model3, _ =
                    Compositor.FeatureState.updateFeature154
                        (Compositor.Types.TimingEvidenceRecorded "inconclusive")
                        model2

                let model4, _ =
                    Compositor.FeatureState.updateFeature154 (Compositor.Types.ArtifactPublished "proof-set.md") model3
