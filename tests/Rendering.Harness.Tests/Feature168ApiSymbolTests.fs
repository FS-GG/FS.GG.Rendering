...
let tests =
    testList
        "Feature168 ApiSymbols"
        [
            test "a documented symbol that the surface baseline carries and a test names is exercised" {
                Expect.equal
                    (statusOf "Widget.render")
                    [ "Widget.render", SkillParity.Exercised ]
                    "resolved and exercised"
            }

            test "a documented symbol absent from the surface baseline is unresolved" {
                Expect.equal
                    (statusOf "Widget.missing")
                    [ "Widget.missing", SkillParity.Unresolved ]
                    "the API does not exist"
            }

            test "a documented symbol that exists but no test names is unexercised" {
                Expect.equal
                    (statusOf "Widget.hidden")
                    [ "Widget.hidden", SkillParity.Unexercised ]
                    "the seam may be dead"
            }

            test "only F# code fences are read, so prose naming an API is not a claim about it" {
                let symbols =
                    SkillParity.evaluateApiSymbols surfaceMembers exercised [ documenting "Widget.render" ]

                Expect.equal
                    (symbols |> List.map (fun item -> item.Symbol))
                    [ "Widget.render" ]
                    "the prose `Widget.missing` is ignored"
            }
            ...
            test "a test source only mentioning an API in a comment or string does not exercise it" {
                ...
                try
                    ...
                    for mentioned in
                        [
                            "Widget.commented"
                            "Widget.blockCommented"
                            "Widget.quoted"
                            "Widget.trailing"
                        ] do
                        Expect.isFalse
                            (exercised |> Set.contains mentioned)
                            $"{mentioned} is only mentioned, never called"
                finally
            ...
            test "symbols outside the surface baseline's modules are product-local and not judged" {
                ...
                Expect.isEmpty
                    (SkillParity.evaluateApiSymbols surfaceMembers exercised [ entry ])
                    "no closed-world module matches"
            }

            test "wrapper entries are not a documentation surface" {
                let wrapper =
                    { documenting "Widget.missing" with
                        EntryKind = SkillParity.WrapperEntry
                    }

                Expect.isEmpty
                    (SkillParity.evaluateApiSymbols surfaceMembers exercised [ wrapper ])
                    "only canonical and command skills document APIs"
            }
