...
let private bulletBlocks (lines: string list) =
    ...
    let rec go acc current rest =
        ...
        | (line: string) :: tail ->
            if isBulletStart line then
                go (closed current acc) (Some [ line ]) tail
            elif line.Trim() = "" then
                go (closed current acc) None tail
            else
                match current with
...
let private parseBullet (line: string) : Result<SurfaceRestatement, string> =
    ...
    let malformed reason =
        Error(sprintf "bullet %A %s" body reason)
    ...
    | [ idPart; rootsPart; selectorPart ] ->
        ...
        | [ surfaceId ], Some rootsText, Some selectorText ->
            ...
            if residue idPart <> "" then
                ...
                malformed (
                    sprintf
                        "carries prose %A in its roots cell; roots are comma-separated code spans and nothing else"
                        (residue rootsText)
                )
            else
                ...
                | [ _ ] ->
                    malformed (
                        sprintf
                            "carries prose %A in its selector cell; a surface declares one selector and nothing else"
                            (residue selectorText)
                    )
                | _ ->
                    malformed (
                        sprintf
                            "publishes %d selector spans in %A; a surface declares exactly one"
...
let private parseContract () =
    parseSection (File.ReadAllText contractPath)
...
let private mismatches (section: ParsedSection) (surfaces: SkillParity.SkillSurface list) =
    disagreements
        bulletSubject
        {
            Entries = section.Bullets
            Unreadable = section.Malformed
        }
        surfaces
...
let cliSurfaceListContractTests =
    testList
        "Feature1111 Default Repository Surfaces and discoverDefaultSurfaces say the same thing"
        [
            ...
            test "every bullet of Default Repository Surfaces agrees with the surface it restates" {
                ...
                | problems ->
                    failtestf
                        "the contract and the resolver disagree:\n  %s\n\n%s"
                        (String.concat "\n  " problems)
                        remedy
            }
            ...
            test "the comparison rejects a surface list the section does not describe" {
                ...
                Expect.isNonEmpty
                    (mismatches
                        { section with
                            Bullets = List.tail section.Bullets
                        }
                        surfaces)
                ...
                let inventedBullet =
                    { List.head section.Bullets with
                        SurfaceId = "fsgg-1111-invented-bullet"
                    }
                    :: section.Bullets
                ...
                let repeatedBullet =
                    ...
                    { first with
                        Roots = [ "docs" ]
                        Selector = Some "every-skill-body"
                    }
                    :: section.Bullets
            ...
            test "the section as it stood before #1111 is rejected by this check" {
                ...
                let section =
                    ...
                    | None ->
                        failtest
                            "non-vacuity: the historical section text is located by the same parser the contract is read with"
            ...
            test "every path-shaped code span in a bullet is a root the resolver actually reads" {
                ...
                let declaredRoots =
                    surfaces |> List.collect (fun surface -> surface.Roots) |> Set.ofList

                Expect.isNonEmpty
                    section.BulletSpans
                    "non-vacuity: the bullets' code spans were read and are not an empty list"

                Expect.isNonEmpty
                    declaredRoots
                    "non-vacuity: the resolver declares at least one root to compare against"

                let pathShaped = section.BulletSpans |> List.filter (fun span -> span.Contains '/')

                Expect.isNonEmpty
                    pathShaped
                    "non-vacuity: some bullet span looks like a path, so this rule has a subject"
            ...
            test
                "ant-canonical names the post-#1082 canonical location, and the pre-#1082 path appears nowhere in the file" {
                let section = parsedOrFail ()

                let ant =
                    match
                        section.Bullets
                        |> List.tryFind (fun bullet -> bullet.SurfaceId = "ant-canonical")
                    with
                    | Some bullet -> bullet
            ...
            test "spec-kit-command has a bullet with both of its roots, and the speckit narrowing is its selector" {
                ...
                let commandBullet =
                    match
                        section.Bullets
                        |> List.tryFind (fun bullet -> bullet.SurfaceId = "spec-kit-command")
                    with
                    | Some bullet -> bullet
                    | None ->
                        failtestf
                ...
                Expect.equal
                    commandBullet.Roots
                    declared.Roots
                    "the spec-kit-command bullet publishes both roots the resolver reads, in full"
