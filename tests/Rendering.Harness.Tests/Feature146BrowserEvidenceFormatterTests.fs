...
let feature146BrowserEvidenceFormatterTests =
    testList
        "Feature146 browser evidence formatters"
        [
            test "browser report formatter states no comparison was performed" {
                ...
                Expect.stringContains
                    ...
                    "candidate backend is formatted"
                ...
                Expect.stringContains
                    text
                    "NOT cross-backend fidelity evidence"
                    "report refuses to be read as fidelity evidence"
            }
            ...
            test "browser report never emits a tolerance, diff, or candidate-identity field" {
                ...
                let emitsField (name: string) =
                    ...
                    |> List.exists (fun line ->
                        line.Trim().TrimStart('-').Trim().StartsWith(name + ":", System.StringComparison.Ordinal))
                ...
                Expect.isFalse
                    (emitsField "candidate")
                    "a report whose candidate never ran must not print a candidate identity"
            }

            test "browser report writer persists markdown" {
                let out =
                    System.IO.Path.Combine(System.IO.Path.GetTempPath(), "fs-gg-feature146-browser-test")

                if System.IO.Directory.Exists out then
