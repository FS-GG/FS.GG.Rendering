...
let unreadableSurfaceTests =
    testList
        "Feature1093 an unreadable skill file is reported, never silently dropped"
        [
            ...
            test "a skill file that cannot be read fails the gate and names the path and the reason" {
                ...
                try
                    ...
                    Expect.isNonEmpty
                        findings
                        "the unreadable file produces a finding rather than vanishing from the inventory"
                    ...
                    Expect.equal
                        finding.Severity
                        SkillParity.High
                        "an unreadable FILE is a fact about the repository, reported at High"
            ...
            test "an unexpected exception is reported as a harness defect, not as an unreadable file" {
                ...
                try
                    ...
                    match finding with
                    | None ->
                        failtest "an unexpected exception type is still reported — silence is the defect #1093 closed"
                    | Some finding ->
                        Expect.equal
                            finding.Severity
                    ...
                    Expect.equal report.OverallStatus SkillParity.Failed "and the overall status is failed"
                finally
                    Feature168SkillParityFixtures.deleteTempRoot root
            }
            ...
            test "inventorySkillsWith drops the entry and hands back the reason for it" {
                ...
                try
                    ...
                    Expect.isNonEmpty
                        cleanEntries
                        "the surface really does inventory a body when the read succeeds (non-vacuity)"
                    ...
                    let entries, failures =
                        SkillParity.inventorySkillsWith reader inventoryRequest surfaces
                    ...
                    Expect.equal
                        failure.Kind
                        SkillParity.UnreadableFile
                        "UnauthorizedAccessException is the filesystem's answer, not a harness defect"
            ...
            test "every unreadable file under one surface is its own finding" {
                ...
                try
                    ...
                    let reader =
                        readerFailingOn (fun path -> (normalize path).Contains "/canonical/") (fun path ->
                            IOException($"synthetic injected read failure for {path}") :> exn)
                    ...
                    let unreadableFiles =
                        Directory.GetFiles(
                            Path.Combine(fixtureRoot, "canonical"),
                            "SKILL.md",
                    ...
                    Expect.isGreaterThan
                        unreadableFiles
                        1
                        "the `all` fixture writes more than one canonical body (non-vacuity)"
                    ...
                    Expect.equal
                        (canonicalSurfaceFindings
                         |> List.map (fun f -> f.SkillName)
                         |> List.distinct
                         |> List.length)
                        unreadableFiles
