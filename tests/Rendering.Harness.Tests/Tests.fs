...
let private withPresent =
    { baseFacts with
        SwapControl = Some 1
        VblankSource = Some "HDMI-A-1"
    }

[<Tests>]
let runPlanTests =
    testList
        "RunPlan (pure overclaim/degradation core)"
        [
            ...
            test "T2 runs on X11" { Expect.equal (RunPlan.plan T2 (withBackend X11)).Degradation Run "T2 runs on X11" }
            test "vsync-faithful claimable ONLY with present facts (T3)" {
                ...
                Expect.contains
                    without.NotAuthoritativeFor
                    "vsync-faithful"
                    "missing facts → listed as NOT authoritative"
            }
...
let tierTests =
    ...
    let tmp tag =
        Path.Combine(Path.GetTempPath(), sprintf "harness-%s-%s" tag (Guid.NewGuid().ToString("N")))

    testList
        "Tiers (deterministic, headless)"
        [
...
let inputTests =
    ...
    let tmp tag =
        Path.Combine(Path.GetTempPath(), sprintf "harness-input-%s-%s" tag (Guid.NewGuid().ToString("N")))
        ...
        [
            ...
            test "run.json carries proofLevel and notAuthoritativeFor" {
                ...
                Expect.stringContains
                    json
                    "\"notAuthoritativeFor\": [\"desktop-visibility\"]"
                    "not-authoritative present"
            }
