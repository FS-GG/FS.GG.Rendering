...
let private baseOptions
    ...
    (feed: string)
    (outDir: string)
    (mode: PackageFeed.PackageFeedMode)
    : PackageFeed.PackageFeedOptions =
    {
        RepositoryRoot = root
        ...
        [
            test "generated NuGet config constrains FS.GG.UI packages to local feed rule" {
                ...
                try
                    ...
                    let rules =
                        PackageFeed.writeGeneratedNuGetConfig config (Path.Combine(root, "feed"))
            ...
            test "proof mode fails closed when expected local feed package is missing" {
                ...
                try
                    ...
                    Feature163TestFixtures.writePackageProject
                        root
                    ...
                    let result =
                        PackageFeed.runWorkflow (baseOptions root feed outDir PackageFeed.Proof)
                    ...
                    Expect.exists
                        proof.Violations
                        (fun v -> v.Contains "missing-local-package")
                        "missing package violation"
            ...
            test "pinViolations names each stale pin with expected and actual versions" {
                ...
                Expect.isFalse
                    (stale.Contains root)
                    "the path is relativised — an absolute runner path is noise in a CI log"
                ...
                Expect.isFalse
                    (violations |> List.exists (fun v -> v.Contains "FS.GG.UI.Scene"))
                    "current pin is silent"

                Expect.isFalse
                    (violations |> List.exists (fun v -> v.Contains "FS.GG.UI.Waived"))
                    "accepted exception is silent"
            }
            ...
            test "package-consuming samples are discovered by their nuget.config source mapping" {
                ...
                try
                    ...
                    writeConfig
                        "AntShowcase"
                        "<configuration><packageSourceMapping><package pattern=\"FS.GG.UI.*\" /></packageSourceMapping></configuration>"
                    ...
                    Directory.CreateDirectory(Path.Combine(root, "samples", "SymbologyBoard"))
                    |> ignore
                    ...
                    Expect.equal
                        discovered
                        [ "samples/AntShowcase"; "samples/ControlsGallery" ]
                        "only the package-consuming samples, sorted"
                finally
                    Feature163TestFixtures.deleteTempRoot root
            }
            ...
            test "proof mode fails closed when a selected sample contributes nothing to build" {
                ...
                try
                    ...
                    Expect.exists
                        proof.Violations
                        (fun v -> v.Contains "no-package-pins" || v.Contains "no-consumer-projects")
                        "fails closed, and says which"
            ...
            test "proof mode reports no-selected-samples and no-package-pins distinctly" {
                ...
                try
                    ...
                    Expect.exists
                        noSamples.SourceProof.Value.Violations
                        (fun v -> v.Contains "no-selected-samples")
                        "no samples"
                    ...
                    let noPins =
                        PackageFeed.runWorkflow (baseOptions root feed (Path.Combine(root, "out2")) PackageFeed.Proof)
            ...
            test "evidence from two different checkouts of the same content is byte-identical" {
                ...
                try
                    ...
                    Expect.stringContains
                        pinsA
                        "samples/Demo/Demo.fsproj"
                        "the pin names its project, relative to the root"

                    Expect.stringContains
                        versionsA
                        "src/Controls/Controls.fsproj"
                        "the package names its project, relative to the root"

                    Expect.isFalse (pinsA.Contains rootA) "no absolute path survives into the committed pin table"

                    Expect.isFalse
                        (versionsA.Contains rootA)
                        "no absolute path survives into the committed version table"
                finally
            ...
            test "source proof evidence names paths under the root relatively" {
                ...
                try
                    ...
                    let result =
                        PackageFeed.runWorkflow (baseOptions root (Path.Combine(root, "feed")) outDir PackageFeed.Proof)
                    ...
                    Expect.stringContains
                        ...
                        "the nuget.org URL survives the markdown rules list intact"

                    Expect.stringContains
                        json
                        "https://api.nuget.org/v3/index.json"
                        "the nuget.org URL survives the json rules list intact"

                    Expect.isFalse
                        (markdown.Contains "https:/api.nuget.org")
                        "the URL's scheme separator is not collapsed"
                finally
                    Feature163TestFixtures.deleteTempRoot root
            }
            ...
            test "a feed outside the repository root is left absolute, not relativised into ../.." {
                ...
                try
                    ...
                    PackageFeed.runWorkflow (baseOptions root feed outDir PackageFeed.Check)
                    |> ignore
            ...
            test "clear-global-cache without cold mode is a cache policy violation" {
                ...
                try
                    ...
                    Expect.exists
                        result.SourceProof.Value.Violations
                        (fun v -> v.Contains "cache-policy-violation")
                        "cache violation"
                finally
                    Feature163TestFixtures.deleteTempRoot root
            }
