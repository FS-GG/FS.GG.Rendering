...
let laneWith root id role script timeout noProgress concurrencyGroup outputScope : ValidationLanes.LaneDefinition =
...
let result runRoot id role status : ValidationLanes.LaneResult =
...

    {
        ...
        Reason =
            if status = ValidationLanes.Passed then
                None
            else
                Some(status |> ValidationLanes.statusToken)
        Diagnostics = []
        Caveats = []
