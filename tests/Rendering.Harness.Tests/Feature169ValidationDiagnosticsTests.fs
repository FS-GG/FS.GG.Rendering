...
let tests =
    testList
        "Feature169 validation lane typed diagnostics"
        [
            test "Synthetic lane status derives from diagnostic summary status" {
                Expect.equal
                    (ValidationLanes.laneStatusFromDiagnosticSummary (emptySummary ReadinessDiagnosticStatus.Accepted))
                    ValidationLanes.Passed
                    "accepted"

                Expect.equal
                    (ValidationLanes.laneStatusFromDiagnosticSummary (emptySummary ReadinessDiagnosticStatus.Blocked))
                    ValidationLanes.Failed
                    "blocked"

                Expect.equal
                    (ValidationLanes.laneStatusFromDiagnosticSummary (
                        emptySummary ReadinessDiagnosticStatus.ReviewRequired
                    ))
                    ValidationLanes.Failed
                    "review"
                ...
                Expect.equal
                    (ValidationLanes.laneStatusFromDiagnosticSummary (
                        emptySummary ReadinessDiagnosticStatus.EnvironmentLimitedStatus
                    ))
            ...
            test "Synthetic diagnostics lane is selectable with include semantics" {
                ...
                try
                    ...
                    let lanes =
                        ValidationLanes.defaultLaneDefinitions root (System.IO.Path.Combine(outDir, runId))
                    ...
                    | Result.Ok p ->
                        Expect.isTrue
                            (p.SelectedLanes |> List.exists (fun lane -> lane.Id = "diagnostics"))
                            "diagnostics lane selected"
