...
let surfaceRootTests =
    testList
        "Feature1092 a surface's declared roots are the roots the resolver reads"
        [
                ...
                try
                    for surface in declaredSurfaces () do
                        ...
                        Expect.isNonEmpty
                            (inventory [ surface ])
                            (sprintf
                                "non-vacuity: surface '%s' resolves to at least one body in the live tree"
                                surface.SurfaceId)
                ...
                for surface in declaredSurfaces () do
                    ...
                    for root in surface.Roots do
                        Expect.isFalse
                            ...
                            (sprintf
                                "surface '%s' declares root '%s', which contains whitespace — prose belongs in Notes"
                                surface.SurfaceId
                                root)
            ...
            test "an operator override of a surface id the resolver has a rule for is genuinely applied" {
                ...
                try
                    ...
                    let control =
                        SkillParity.runCheck (requestWithOverrides outRoot [ "ant-canonical", real ])
                    ...
                    Expect.equal
                        (control.Findings
                         |> List.map (fun finding -> finding.Category, finding.WrapperPath))
                    ...
                    let overridden =
                        SkillParity.runCheck (requestWithOverrides outRoot [ "ant-canonical", bogus ])
            ...
            test "an override is honoured for every surface id the repository declares" {
                ...
                try
                    ...
                    for surface in declaredSurfaces () do
                        let report =
                            SkillParity.runCheck (requestWithOverrides outRoot [ surface.SurfaceId, missing ])
                        ...
                        Expect.equal
                            ...
                            (sprintf
                                "non-vacuity: the run for '%s' is a run over the overridden surface"
                                surface.SurfaceId)
