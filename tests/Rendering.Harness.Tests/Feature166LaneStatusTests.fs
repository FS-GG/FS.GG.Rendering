    ...
    testList
        ...
        [
            ...
            test "process runner classifies pass fail timeout no-progress and evidence infrastructure failures" {
                ...
                try
                    let passed =
                        ValidationLanes.runLane (Feature166TestFixtures.lane root "passed" "printf 'ok\n'")

                    Expect.equal passed.Status ValidationLanes.Passed "passed"
                    Expect.isTrue (File.Exists passed.ResultPath) "result"

                    let failed =
                        ValidationLanes.runLane (Feature166TestFixtures.lane root "failed" "printf 'fail\n'; exit 7")
            ...
            test "Synthetic MVU transitions emit preflight heartbeat timeout cancellation and summary effects" {
                ...
                try
                    ...
                    let _, timeoutEffects =
                        ValidationLanes.update (ValidationLanes.LaneTimedOut(lane.Id, "timeout")) running

                    Expect.contains timeoutEffects (ValidationLanes.StopProcess lane.Id) "stop timeout"

                    let _, noProgressEffects =
                        ValidationLanes.update (ValidationLanes.LaneNoProgressTimedOut(lane.Id, "stalled")) running

                    Expect.contains noProgressEffects (ValidationLanes.StopProcess lane.Id) "stop no-progress"

                    let _, infraEffects =
                        ValidationLanes.update
                            (ValidationLanes.InfrastructureErrorRaised(Some lane.Id, "write failed"))
                            running

                    Expect.contains infraEffects ValidationLanes.WriteSummary "summary on infra"

                    let canceled, cancelEffects =
                        ValidationLanes.update (ValidationLanes.OperatorCanceled "ctrl-c") running
                    ...
                    let _, summaryEffects =
                        ValidationLanes.update ValidationLanes.SummaryRequested running
