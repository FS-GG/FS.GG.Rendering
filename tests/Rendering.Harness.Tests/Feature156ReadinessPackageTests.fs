...
let private report scenario verdict : Compositor.Types.Feature156ScenarioReport =
    {
        ...
        DamageScoped =
            Some
                { dist $"raw/{Compositor.FeatureState.feature156ScenarioFileName scenario}-damage.csv" with
                    P50Ms = 6.0
                    P95Ms = 7.0
                    P99Ms = 8.0
                }
        ...
        [
            test "summary includes scenario table policy host claim boundary and remaining gates" {
                let reports =
                    ...
                    |> List.map (fun scenario -> report scenario Compositor.Types.Feature156Positive)
            ...
            test "validation and compatibility summaries keep correctness and performance boundaries separate" {
                let reports =
                    [ report "timing/localized-update" Compositor.Types.Feature156Incomplete ]
                ...
                Expect.stringContains
                    ledger
                    "Feature 155 proof, parity, fallback, and correctness vocabulary remains authoritative"
                    "Feature155 boundary"
            }
        ]
