...
let private timingSample scenario index policy status reason : Compositor.Types.Feature158TimingSample =
    {
        ...
        ScenarioDefinitionId =
            "feature156-required-v1:"
            + (Compositor.FeatureState.feature158ScenarioFileName scenario).Replace(".md", "")
...
let private report scenario : Compositor.Types.Feature158ScenarioReport =
    let included =
        [
            for i in 1..5 -> timingSample scenario i Perf.ReadbackFree Perf.Included None
        ]

    let excluded =
        [
            timingSample scenario 99 Perf.ProbeReadbackIncluded Perf.Probe (Some Perf.ProbeRunExcluded)
        ]
    ...
    testList
        ...
        [
            ...
            test "scenario report separates included timing from excluded probe readback" {
                let rendered =
                    Compositor.Render3.emitFeature158ScenarioReport (report "timing/localized-update")
