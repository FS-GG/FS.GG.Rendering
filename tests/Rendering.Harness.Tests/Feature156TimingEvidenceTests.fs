...
let private distribution path samples = Perf.summarizeSamples path samples

[<Tests>]
let tests =
    testList
        "Feature156 timing evidence"
        [
            ...
            test "MVU workflow records host policy scenario and summary publication" {
                ...
                Expect.contains
                    effects0
                    (Compositor.Types.Feature156DeclarePolicy Compositor.Config.feature156PolicyId)
                    "policy declaration"
                ...
                let model1, _ =
                    Compositor.FeatureState.updateFeature156
                        (Compositor.Types.Feature156HostProfileDetected profile)
                        model0

                let model2, _ =
                    Compositor.FeatureState.updateFeature156
                        (Compositor.Types.Feature156PolicyDeclared Compositor.Config.feature156PolicyId)
                        model1

                let model3, _ =
                    Compositor.FeatureState.updateFeature156
                        (Compositor.Types.Feature156ScenarioEvaluated report)
                        model2

                let model4, _ =
                    Compositor.FeatureState.updateFeature156
                        (Compositor.Types.Feature156SummaryPublished "timing/summary.md")
                        model3
                ...
                Expect.equal
                    model4.Verdict
                    Compositor.Types.Feature156Incomplete
                    "overall incomplete until all required scenarios are positive"
