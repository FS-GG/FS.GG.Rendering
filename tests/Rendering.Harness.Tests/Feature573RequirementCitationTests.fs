        ...
        [
            test "a skill citing FR-nnn with no metadata.source is a High finding (#466/#573)" {
                ...
                try
                    ...
                    Expect.hasLength
                        findings
                        1
                        "a bare FR citation names nothing — FR numbers are unique only within a feature"
                    ...
                    Expect.stringContains
                        ...
                        "FR-014"
                        "the finding names the citations it could not place"
                finally
            ...
            test "a citation naming a REAL spec that does not state the requirement is still a finding" {
                ...
                try
                    ...
                    Expect.hasLength
                        findings
                        1
                        "the spec resolves, so #529's rule is satisfied — but it does not STATE FR-017/FR-018, and a citation that resolves to the wrong feature is the whole of #466"

                    Expect.stringContains findings.Head.Message "FR-017" "the finding names the phantom requirement"

                    Expect.isFalse
                        (findings.Head.Message.Contains "FR-014")
                        "FR-014 IS stated by that spec, so it is not phantom — the rule must not cry wolf on the citations that do resolve"
                finally
                    Feature168SkillParityFixtures.deleteTempRoot root
            }
            ...
            test "a source that resolves but has no spec.md is a finding, not a free pass" {
                ...
                try
                    ...
                    Expect.hasLength
                        (citationFindings root)
                        1
                        "an existing directory with no spec.md checks NOTHING — if this passes, any skill can silence this gate by pointing `source:` at any folder in the repo"
                finally
                    Feature168SkillParityFixtures.deleteTempRoot root
            }
            ...
            test "a requirement the source only mentions in passing does not count as stated" {
                ...
                try
                    ...
                    Expect.hasLength
                        (citationFindings root)
                        1
                        "FR-021 is name-dropped in 192's prose, not DEFINED by it — a passing mention is not a requirement this spec states"
                finally
                    Feature168SkillParityFixtures.deleteTempRoot root
            }
            ...
            test "an unpadded citation is the same requirement, not a phantom" {
                ...
                try
                    ...
                    Expect.isEmpty
                        (citationFindings root)
                        "FR-14 means the spec's FR-014 — comparing as strings would red a correct citation"
                finally
                    Feature168SkillParityFixtures.deleteTempRoot root
            }
            ...
            test "the externally-owned coordination kit never reaches this rule (it is not inventoried)" {
                ...
                try
                    ...
                    Expect.contains
                        inventoried
                        "fs-gg-ordinary"
                        "the fixture root really is being inventoried (else this test proves nothing)"
