                ...
                for scenarioId, outcome in rendered do
                    ...
                    | Equivalent comparison ->
                        Expect.equal
                            ...
                            0
                            (sprintf "%s: zero differing pixels under exact" scenarioId)

                        Expect.equal
                            comparison.MaxChannelDelta
                            0
                            (sprintf "%s: zero channel delta under exact" scenarioId)
            ...
            test "a perturbed reference drifts beyond the perceptual budget (gate is discriminating)" {
                ...
                | Drifted comparison ->
                    Expect.isGreaterThan
                        comparison.DiffPixelCount
                        GoldenImage.perceptual.MaxDiffPixels
                        "drift exceeds the pixel budget"

                    Expect.isGreaterThan
                        comparison.MaxChannelDelta
                        GoldenImage.perceptual.ChannelTolerance
                        "drift exceeds the channel budget"
