...
let tests =
    testList
        "Feature167 responsiveness readiness summary"
        [
            test "validation lane reader consumes summary.json without Markdown parsing" {
                let dir =
                    Path.Combine(Path.GetTempPath(), "feature167-lanes-" + Guid.NewGuid().ToString("N"))
                ...
                | Ok summary ->
                    ...
                    Expect.equal
                        (ValidationLanes.responsivenessSummaryLaneStatus summary)
                        ValidationLanes.EnvironmentLimited
                        "lane status maps environment limitation"
            }
        ]
