...
let tests =
    testList
        "Feature149 live proof evidence"
        [
            ...
            test "Feature149 proof corpus includes rejection and environment-limited cases" {
                ...
                Expect.isTrue
                    (Compositor.Config.feature149TargetHostProfiles
                     |> List.exists (fun profile -> profile.ProfileId = "feature149-capable-host-candidate"))
            ...
            test "Feature149 Synthetic simulated host evidence remains environment-limited" {
                ...
                let token =
                    Compositor.Config.proofVerdictToken (Compositor.Types.ProofEnvironmentLimited "synthetic host")

                Expect.isTrue
                    (TestAssertions.feature149EnvironmentLimited token)
                    "synthetic proof remains diagnostic-only"
            }
        ]
