...
let private declaredSurfaces =
    lazy (SkillParity.discoverDefaultSurfaces repositoryRoot)
...
let surfaceIdKeyedClassificationTests =
    testList
        "Feature1137 entry classification is declared, never keyed on SurfaceId"
        [
            ...
            test "every entry's kind is what its surface's declaration predicts, for every declared surface" {
                ...
                for entry in entries do
                    ...
                    | None ->
                        ...
                        failtestf
                            "entry '%s' claims surface '%s', which no declaration produced"
                            entry.Path
                            entry.SurfaceId
                    | Some surface ->
                        Expect.equal
            ...
            test "the surfaces demanding wrapper exposure are exactly the canonical, non-generated-product ones" {
                ...
                let demanding =
                    ...
                    |> List.filter (fun surface ->
                        surface.Kind = SkillParity.Canonical
                        && surface.Agent <> SkillParity.GeneratedProduct)
            ...
            test "two overrides over the SAME directory are classified and required identically" {
                ...
                try
                    ...
                    let asAnt =
                        SkillParity.runCheck (requestWithOverrides outRoot [ "ant-canonical", directory ])

                    let asOther =
                        SkillParity.runCheck (requestWithOverrides outRoot [ "some-other-surface", directory ])
            ...
            test "fixture missing-wrapper findings name only required wrapper surfaces declared by that fixture run" {
                let outRoot =
                    Feature168SkillParityFixtures.createTempRoot "fsgg-1143-wrapper-targets"

                try
                    let report =
                        SkillParity.runCheck (Feature168SkillParityFixtures.request outRoot "all")
            ...
            test "the surface ids still written as literals in SkillParity.fs are exactly the known, tracked set" {
                ...
                let ids = declaredSurfaces.Force() |> List.map (fun surface -> surface.SurfaceId)
                ...
                let present =
                    ...
                    |> Array.filter (fun line ->
                        ...
                        && quoted
                           |> List.exists (fun token -> line.Contains(token, StringComparison.Ordinal)))
                    |> Set.ofArray
