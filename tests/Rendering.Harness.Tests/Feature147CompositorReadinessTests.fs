...
let tests =
    testList
        "Feature147 compositor readiness package"
        [
            test "readiness update records proof, parity, tier verdict, and diagnostics" {
                ...
                Expect.isTrue
                    (effects
                     |> List.exists (function
                         | Compositor.Types.WriteValidationSummary _ -> true
                         | _ -> false))
                    "summary effect"

                let model1, _ =
                    Compositor.FeatureState.updateReadiness (Compositor.Types.DiagnosticRecorded "loaded") model0
                ...
                let model3, _ =
                    Compositor.FeatureState.updateReadiness
                        (Compositor.Types.TierEvaluated(Compositor.Types.DamageScissorTier, Compositor.Types.Ready))
                        model2
