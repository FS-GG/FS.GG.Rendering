    ...
    testList
        ...
        [
            ...
            test "overall status accepts covered same-profile net-positive parity attempts" {
                Expect.equal
                    (Compositor.FeatureState.feature159OverallStatus summary)
                    Compositor.Types.Feature159ReadinessStatus.Accepted
                    "accepted"
            ...
            test "unsupported host report records zero accepted artifacts" {
                let rendered =
                    Compositor.Render3.emitFeature159UnsupportedHostReport "missing display"
