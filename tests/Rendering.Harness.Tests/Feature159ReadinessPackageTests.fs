...
let private attempt scenario : Compositor.Feature159Attempt =
...
let private summary : Compositor.Feature159Summary =
    { RunId = "run-159"
      HostProfile = profile
      PolicyId = Compositor.feature159PolicyId
      Status = Compositor.Feature159ReadinessStatus.Accepted
      Attempts = Compositor.feature159RequiredScenarioIds |> List.map attempt
...
let tests =
    testList "Feature159 readiness package" [
        test "validation summary links required package artifacts and preserves claim boundary" {
            let rendered = Compositor.renderFeature159ValidationSummary summary
        ...
        test "overall status accepts covered same-profile net-positive parity attempts" {
            Expect.equal (Compositor.feature159OverallStatus summary) Compositor.Feature159ReadinessStatus.Accepted "accepted"
            Expect.equal (Compositor.feature159StatusToken summary.Status) "accepted" "status token"
        }

        test "unsupported host report records zero accepted artifacts" {
            let rendered = Compositor.renderFeature159UnsupportedHostReport "missing display"
