...
let artifact (readiness: string) (runId: string) (state: Evidence.VisualArtifactState) : Evidence.VisualArtifact =
    ...
    {
        ArtifactId =
            sprintf "%s-%s" Evidence.feature144DatePickerProofScenario.ScenarioId (Evidence.artifactStateToken state)
        Path = path
        State = state
        Width = 12
        Height = 12
        ...
        TopmostHitTarget =
            if state = Evidence.OpenOverlay then
                Some Evidence.feature144DatePickerProofScenario.ExpectedTopmostHitTarget
            else
                None
...
let tempReadiness () =
    let dir =
        Path.Combine(Path.GetTempPath(), "feature145-" + Guid.NewGuid().ToString("N"))
...
let tests =
    testList
        "Feature145 overlay visual proof"
        [
            test "evidence model exposes stable scenario and artifact tokens" {
                ...
                Expect.equal
                    (Evidence.visualArtifactRelativePath "run-1" Evidence.OpenOverlay)
                    "artifacts/run-1/open.png"
                    "open path is stable"

                Expect.equal
                    (Evidence.visualProofStatusToken Evidence.VisualProofEnvironmentLimited)
                    "environment-limited"
                    "status token is stable"
            }
            ...
            test "capable-host proof accepts current-run open and closed artifacts" {
                ...
                let openResult =
                    Evidence.validateVisualArtifact readiness runId scenario openArtifact

                let closedResult =
                    Evidence.validateVisualArtifact readiness runId scenario closedArtifact
            ...
            test "artifact validation rejects missing blank zero-sized stale unreadable and disconnected artifacts" {
                ...
                let missing =
                    { valid with
                        Path = Evidence.visualArtifactRelativePath runId Evidence.ClosedOverlay
                        State = Evidence.ClosedOverlay
                        TopmostHitTarget = None
                        NoStaleOverlayPixel = Some true
                    }
                ...
                let stale =
                    { valid with
                        RunId = "old-run"
                        Path = Evidence.visualArtifactRelativePath "old-run" Evidence.OpenOverlay
                    }
            ...
            test "Synthetic artifacts deterministic logs and unsupported records cannot satisfy real visual proof" {
                ...
                let synthetic =
                    { artifact readiness runId Evidence.OpenOverlay with
                        CaptureSource = Evidence.VisualSynthetic
                    }
                ...
                Expect.contains
                    result.Diagnostics
                    "synthetic artifacts cannot satisfy real visual proof"
                    "diagnostic names synthetic rejection"
            }
            ...
            test "correlation validation rejects artifact state hit target focus and dispatch disagreement" {
                ...
                let result =
                    Evidence.validateOverlayVisualCorrelation scenario openArtifact mismatch

                Expect.isFalse result.Accepted "correlation mismatch is rejected"

                Expect.equal
                    result.FailureCategory
                    Evidence.OverlayBehavior
                    "mismatch is classified as overlay behavior"
            }
            ...
            test "readiness decisions close gate gate unsupported host or fail classified runs" {
                ...
                let limitation =
                    Evidence.unsupportedHostLimitation
                        { (host Evidence.HostUnsupported) with
                            Cause = "missing-display"
                        }

                let gated =
                    { passed with
                        Status = Evidence.VisualProofEnvironmentLimited
                        OpenArtifact = None
                        ClosedArtifact = None
                        FailureCategory = Evidence.Environment
                        Limitation = Some limitation
                    }

                let failed =
                    { passed with
                        Status = Evidence.VisualProofFailed
                        FailureCategory = Evidence.Capture
                    }

                Expect.equal
                    (Evidence.evaluateReadinessCaveat passed).Decision
                    Evidence.CaveatClosed
                    "passed proof closes the caveat"

                Expect.equal
                    (Evidence.evaluateReadinessCaveat gated).Decision
                    Evidence.CaveatEnvironmentGated
                    "unsupported proof keeps gate environment-gated"

                Expect.equal
                    (Evidence.evaluateReadinessCaveat failed).Decision
                    Evidence.CaveatFailed
                    "failed proof does not close the caveat"
            }
        ]
