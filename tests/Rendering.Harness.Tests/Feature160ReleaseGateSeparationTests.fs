...
let tests =
    testList
        "Feature160 ReleaseGate separation"
        [
            test "focused throughput can pass while missing full validation blocks release ready" {
                let package = summary None

                Expect.equal
                    (Compositor.FeatureState.feature160FocusedThroughputStatus package)
                    Compositor.Types.Feature160ReadinessStatus.Accepted
                    "focused accepted"

                Expect.equal
                    (Compositor.FeatureState.feature160OverallStatus package)
                    Compositor.Types.Feature160ReadinessStatus.Blocked
                    "overall blocked"

                Expect.equal
                    (Compositor.FeatureState.feature160FullValidationStatus package.FullValidation)
                    "missing"
                    "missing"
            }
            ...
            test "current passing full validation unblocks release-ready status while claim remains separate" {
                let package =
                    summary (Some(fullValidation "passed" "dotnet test FS.GG.Rendering.slnx --no-restore"))
