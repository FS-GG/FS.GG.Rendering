...
let private createRoot () =
    let root =
        Path.Combine(Path.GetTempPath(), "feature223-parity-" + System.Guid.NewGuid().ToString("N"))
...
let private productRoute (id: string) =
    sprintf "../../../template/product-skills/%s/SKILL.md" id

let private packageRoute (id: string) =
    sprintf "../../../src/%s/skill/SKILL.md" id
...
let private missingWrapperFor (name: string) findings =
    findings
    |> List.filter (fun (f: SkillParity.ParityFinding) -> f.Category = SkillParity.MissingWrapper && f.SkillName = name)
        ...
        [
            ...
            test "product alias satisfies: fs-gg-product-symbology present ⇒ no finding" {
                ...
                try
                    ...
                    Expect.isEmpty
                        missing
                        "product alias on both surfaces satisfies the product-skill wrapper requirement"
                finally
                    deleteRoot root
            }
            ...
            test "regression: delivered product alias + bare-named package canonical ⇒ no MissingWrapper" {
                ...
                try
                    ...
                    Expect.isEmpty
                        (missingWrapperFor "fs-gg-scene" findings)
                        "delivered product alias keeps the six green"

                    Expect.isEmpty
                        (missingWrapperFor "fs-gg-foo" findings)
                        "non-product canonical is still satisfied by its bare name"
                finally
                    deleteRoot root
            }
        ]
