    ...
    testList
        ...
        [
            ...
            test "unsupported host and excluded evidence record zero accepted lane artifacts" {
                let unsupported =
                    Compositor.Render4.emitFeature161UnsupportedHostReport "missing display"
                ...
                let excluded =
                    Compositor.Render4.emitFeature161ExcludedEvidenceReport
                        Perf.HostFactsMissing
                        [
                            { entry with
                                PrimaryExclusionReason = Some Perf.HostFactsMissing
            ...
            test "CLI writes Feature 161 readiness package and reviewer entry point" {
                let root =
                    Path.Combine(Path.GetTempPath(), "feature161-readiness-" + Guid.NewGuid().ToString("N"))

                try
                    let exitCode =
                        Cli.main [| "compositor-readiness"; "--feature"; "161"; "--out"; root |]
