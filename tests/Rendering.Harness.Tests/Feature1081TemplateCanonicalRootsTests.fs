...
let templateCanonicalRootsTests =
    testList
        "Feature1081 template-canonical treats every historical root as a mirror"
        [
            ...
            test "a skill body under any historical root inside template/ is NOT a canonical source" {
                ...
                try
                    ...
                    Expect.isTrue
                        (paths
                         |> List.exists (fun p -> p.Contains "template/product-skills/fs-gg-canonical"))
            ...
            test "the public materializer establishes the generated view in a clean checkout" {
                ...
                try
                    let addExit, addOutput =
                        run source "git" (sprintf "worktree add --detach %s HEAD" root)
                    ...
                    let checkExit, checkOutput =
                        run root "bash" "scripts/materialize-skill-roots.sh --check"

                    Expect.equal checkExit 0 (sprintf "public check validates the established view: %s" checkOutput)

                    Expect.isTrue
                        (Directory.Exists(Path.Combine(root, ".agents", "skills")))
                        "apply created the declared generated view"

                    Expect.stringContains
                        applyOutput
                        "generating declared runtime view"
                        "apply records the delegated view generation"
                    ...
                    let removeExit, removeOutput =
                        run source "git" (sprintf "worktree remove --force %s" root)

                    if removeExit <> 0 then
                        failtestf "clean checkout cleanup failed: %s" removeOutput
            }
        ]
