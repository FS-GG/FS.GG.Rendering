...
let private attempt scenario : Compositor.Types.Feature157DamageAttempt =
    let stem =
        (Compositor.FeatureState.feature157ScenarioFileName scenario).Replace(".md", "")

    {
        ...
        ArtifactPaths =
            [
                $"attempts/{Compositor.FeatureState.feature157ScenarioFileName scenario}"
                $"parity/{Compositor.FeatureState.feature157ScenarioFileName scenario}"
            ]
    ...
    testList
        ...
        [
            ...
            test "renders accepted attempt records with proof, retained backing, parity, and artifacts" {
                let rendered =
                    Compositor.Render3.emitFeature157AttemptReport (attempt "damage/localized-update")
            ...
            test "Synthetic fallback fixture: renders fail-closed evidence with zero accepted partial-redraw artifacts" {
                let rendered =
                    Compositor.Render3.emitFeature157FallbackReport (fallback "damage/out-of-bounds" "invalid-damage")
            ...
            test "overall status accepts only when three attempts and five required scenarios are covered" {
                ...
                Expect.equal
                    (Compositor.FeatureState.feature157OverallStatus summary)
