...
let private attempt scenario : Compositor.Types.Feature157DamageAttempt =
    let stem = (Compositor.FeatureState.feature157ScenarioFileName scenario).Replace(".md", "")
    { AttemptId = $"feature157-test-{stem}"
      ...
      ArtifactPaths = [ $"attempts/{Compositor.FeatureState.feature157ScenarioFileName scenario}"; $"parity/{Compositor.FeatureState.feature157ScenarioFileName scenario}" ]
...
let private fallback scenario reason : Compositor.Types.Feature157Fallback =
    { ScenarioId = scenario
      Reason = reason
      DamageValidationStatus = if reason = "invalid-damage" then "out-of-bounds" else "fallback-gated"
      AcceptedPartialRedrawArtifacts = 0
      ArtifactPaths = [ $"fallbacks/{Compositor.FeatureState.feature157ScenarioFileName scenario}" ]
...
let tests =
    testList
        ...
        [ test "declares accepted profile, required scenarios, fallbacks, and command aliases" {
              Expect.equal Compositor.Config.feature157AcceptedProfileId "probe-08a47c01" "accepted profile"
              Expect.equal Compositor.Config.feature157RequiredScenarioIds.Length 5 "required scenario count"
              Expect.contains Compositor.Config.feature157RequiredScenarioIds "damage/static-preserved" "static preserved"
              Expect.contains Compositor.Config.feature157FallbackScenarioIds "damage/parity-mismatch" "parity fallback"
          }

          test "renders accepted attempt records with proof, retained backing, parity, and artifacts" {
              let rendered = Compositor.Render3.emitFeature157AttemptReport (attempt "damage/localized-update")
          ...
          test "Synthetic fallback fixture: renders fail-closed evidence with zero accepted partial-redraw artifacts" {
              let rendered = Compositor.Render3.emitFeature157FallbackReport (fallback "damage/out-of-bounds" "invalid-damage")
          ...
          test "overall status accepts only when three attempts and five required scenarios are covered" {
              ...
              let summary : Compositor.Types.Feature157DamageSummary =
                  { RunId = "feature157-test"
                    HostProfile = profile
              ...
              Expect.equal (Compositor.FeatureState.feature157OverallStatus summary) Compositor.Types.Feature157DamageStatus.Accepted "overall accepted"
              let rendered = Compositor.Render3.emitFeature157DamageSummary summary
