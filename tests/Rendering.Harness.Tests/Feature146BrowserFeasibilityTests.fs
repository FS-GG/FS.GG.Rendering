...
let feature146BrowserFeasibilityTests =
    testList
        "Feature146 browser feasibility MVU"
        [
            test "init requests reference evidence loading" {
                ...
                Expect.equal
                    effects
                    [ RenderAnywhere.LoadReferenceEvidence "out/browser" ]
                    "init requests reference lookup"
            }
            ...
            test "references loaded emits candidate capability assessment effect" {
                ...
                let updated, effects =
                    RenderAnywhere.updateBrowserFeasibility (RenderAnywhere.ReferencesLoaded []) model
            ...
            test "report records fallback decision and never claims a candidate ran" {
                let report =
                    RenderAnywhere.buildBrowserCapabilityReport
                        (RenderAnywhere.corpus ())
                        []
                        "canvaskit-command-stream/proof"
                ...
                |> List.iter (fun item ->
                    Expect.equal
                        item.Status
                        RenderAnywhere.CandidateMissingReference
                        "no reference evidence means no citable reference")
            }
            ...
            test "a passed reference is cited by identity and still reports the candidate as not executed" {
                ...
                let report =
                    RenderAnywhere.buildBrowserCapabilityReport corpus references "canvaskit-command-stream/proof"

                let scenario =
                    report.Scenarios |> List.find (fun item -> item.ScenarioId = head.ScenarioId)

                Expect.equal
                    scenario.ReferenceIdentity
                    (Some "sha256:reference-image")
                    "the passed reference identity reaches the report"

                Expect.equal
                    scenario.Status
                    RenderAnywhere.CandidateNotExecuted
                    "a passed reference does not imply the candidate ran"
                ...
                |> List.iter (fun item ->
                    Expect.equal
                        item.Status
                        RenderAnywhere.CandidateMissingReference
                        "scenarios without reference evidence stay missing-reference")
            }
            ...
            test "a failed reference is not citable as a passed reference" {
                ...
                Expect.equal
                    scenario.Status
                    RenderAnywhere.CandidateMissingReference
                    "a failed reference is not a reference"
