    ...
    testList
        ...
        [
            ...
            test "sample classification rejects cross-run cross-profile package scenario and invalid durations" {
                let definitions =
                    Map.ofList [ "timing/localized-update", "feature156-required-v1:timing-localized-update" ]

                let accepted = classifiedSample Perf.ReadbackFree

                Expect.equal
                    (Perf.classifyTimingSample
                        "feature158-test"
                ...
                Expect.equal
                    (Perf.classifyTimingSample
                        ...
                        invalid)
                        .ExclusionReason
                    (Some Perf.UnverifiableMeasurementPolicy)
                    "invalid duration"
            }
            ...
            test "MVU workflow records policy scenario probe evidence and summary publication" {
                ...
                Expect.contains
                    effects0
                    (Compositor.Types.Feature158DeclarePolicy Compositor.Config.feature158PolicyId)
                    "policy declaration"
                ...
                let model1, _ =
                    Compositor.FeatureState.updateFeature158
                        (Compositor.Types.Feature158HostProfileDetected profile)
                        model0

                let model2, _ =
                    Compositor.FeatureState.updateFeature158
                        (Compositor.Types.Feature158PolicyDeclared Compositor.Config.feature158PolicyId)
                        model1

                let model3, _ =
                    Compositor.FeatureState.updateFeature158
                        (Compositor.Types.Feature158ScenarioEvaluated report)
                        model2

                let model4, _ =
                    Compositor.FeatureState.updateFeature158
                        (Compositor.Types.Feature158ProbeEvidenceRecorded evidence)
                        model3

                let model5, _ =
                    Compositor.FeatureState.updateFeature158
                        (Compositor.Types.Feature158SummaryPublished "timing/summary.md")
                        model4
            ...
            test "explicit probe command does not overwrite existing timing summary" {
                let root =
                    Path.Combine(
                        Path.GetTempPath(),
                        "feature158-probe-preserves-summary-" + Guid.NewGuid().ToString("N")
                    )
                ...
                try
                    ...
                    Expect.equal
                        ...
                        beforeProbe
                        "probe command preserves the accepted timing summary"
                    ...
                    Expect.isTrue
                        hasProbeOrUnsupportedExclusion
                        "probe command writes excluded probe or unsupported-host evidence"
                finally
