...
let private sample policy status reason : Compositor.Types.Feature158TimingSample =
    { SampleId = "feature158-sample"
      ...
      HostProfileId = Compositor.Config.feature158AcceptedProfileId
...
let tests =
    testList "Feature158 measurement policy" [
        test "declares accepted profile policy required scenarios and command aliases" {
            Expect.equal Compositor.Config.feature158AcceptedProfileId "probe-08a47c01" "accepted profile"
            Expect.equal Compositor.Config.feature158PolicyId "readback-free-timing-v1" "policy"
            Expect.equal Compositor.Config.feature158RequiredScenarioIds Compositor.Config.feature156RequiredScenarioIds "same required scenario set"
            Expect.contains Compositor.Config.feature158RequiredScenarioIds "timing/edge-clipping" "edge clipping"
            Expect.equal Compositor.Config.feature158PerformanceCommand "compositor-performance --feature 158" "performance command"
            Expect.equal Compositor.Config.feature158ProbeCommand "compositor-performance --feature 158 --probe-readback" "probe command"
        }
        ...
        test "sample classification rejects cross-run cross-profile package scenario and invalid durations" {
            ...
            Expect.equal (Perf.classifyTimingSample "feature158-test" Compositor.Config.feature158AcceptedProfileId Compositor.Config.feature156PackageVersion definitions accepted).InclusionStatus Perf.Included "accepted"
            Expect.equal (Perf.classifyTimingSample "other" Compositor.Config.feature158AcceptedProfileId Compositor.Config.feature156PackageVersion definitions accepted).ExclusionReason (Some Perf.RunIdentityMismatch) "run mismatch"
            Expect.equal (Perf.classifyTimingSample "feature158-test" "other-profile" Compositor.Config.feature156PackageVersion definitions accepted).ExclusionReason (Some Perf.CrossProfileEvidence) "profile mismatch"
            Expect.equal (Perf.classifyTimingSample "feature158-test" Compositor.Config.feature158AcceptedProfileId "other-package" definitions accepted).ExclusionReason (Some Perf.PackageVersionMismatch) "package mismatch"

            let invalid = { accepted with DurationMs = nan }
            Expect.equal (Perf.classifyTimingSample "feature158-test" Compositor.Config.feature158AcceptedProfileId Compositor.Config.feature156PackageVersion definitions invalid).ExclusionReason (Some Perf.UnverifiableMeasurementPolicy) "invalid duration"
        }

        test "MVU workflow records policy scenario probe evidence and summary publication" {
            let model0, effects0 = Compositor.FeatureState.initFeature158 3 5
            Expect.contains effects0 Compositor.Types.Feature158DetectHostProfile "host detection"
            Expect.contains effects0 (Compositor.Types.Feature158DeclarePolicy Compositor.Config.feature158PolicyId) "policy declaration"
            ...
            let report : Compositor.Types.Feature158ScenarioReport =
                { ScenarioId = "timing/localized-update"
                  ScenarioDefinitionId = "feature156-required-v1:timing-localized-update"
                  FullRedraw = None
                  DamageScoped = None
            ...
            let model1, _ = Compositor.FeatureState.updateFeature158 (Compositor.Types.Feature158HostProfileDetected profile) model0
            let model2, _ = Compositor.FeatureState.updateFeature158 (Compositor.Types.Feature158PolicyDeclared Compositor.Config.feature158PolicyId) model1
            let model3, _ = Compositor.FeatureState.updateFeature158 (Compositor.Types.Feature158ScenarioEvaluated report) model2
            let model4, _ = Compositor.FeatureState.updateFeature158 (Compositor.Types.Feature158ProbeEvidenceRecorded evidence) model3
            let model5, _ = Compositor.FeatureState.updateFeature158 (Compositor.Types.Feature158SummaryPublished "timing/summary.md") model4

            Expect.equal model5.ActiveProfile (Some profile) "profile"
            Expect.equal model5.PolicyId (Some Compositor.Config.feature158PolicyId) "policy"
