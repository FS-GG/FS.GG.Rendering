...
let tests =
    testList
        "Feature142 text fixture corpus"
        [
            ...
            test "fallback oracle accepts the declared corpus expectations" {
                for fixture in TextShapingFixtures.all do
                    ...
                    Expect.isTrue
                        (TextShapingOracle.assertFingerprintStable shaped)
                        $"fingerprint stable for {fixture.Id}"

                    Expect.isTrue
                        (TextShapingOracle.measureDrawAdvanceDelta shaped <= 1.0)
                        $"measure/draw advance parity for {fixture.Id}"
            }
        ]
