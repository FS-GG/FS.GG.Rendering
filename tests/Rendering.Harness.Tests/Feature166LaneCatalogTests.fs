        ...
        [
            test "default catalog declares required lanes and optional aggregate" {
                ...
                try
                    ...
                    Expect.equal
                        (lanes |> List.find (fun lane -> lane.Id = "aggregate-solution")).ReadinessRole
                        ValidationLanes.Optional
                        "aggregate role"

                    Expect.isTrue
                        (lanes |> List.forall (fun lane -> lane.Timeout > TimeSpan.Zero))
                        "timeouts are explicit"

                    Expect.isTrue
                        (lanes
                         |> List.forall (fun lane -> lane.ProgressInterval <= TimeSpan.FromSeconds 60.0))
            ...
            test "lane ids result ids and evidence paths are unique" {
                ...
                try
                    ...
                    Expect.equal
                        (lanes |> List.map _.EvidenceDirectory |> Set.ofList |> Set.count)
                        lanes.Length
                        "evidence paths"
                finally
                    Feature166TestFixtures.deleteTempRoot root
            ...
            test "documented lanes declare concurrency group and output scope metadata" {
                ...
                try
                    ...
                    Expect.isTrue
                        (lanes |> List.forall (fun lane -> lane.ConcurrencyGroup.IsSome))
