...
let private labelFont =
    {
        ...
        Size = 13.0
        Weight = None
    } // matches the label channel (default sans, screen-aligned)
    ...
    testList
        ...
        [
            test "a labelled board rasterises to a non-blank PNG" {
                let path =
                    Render.toPng size (labelledBoard [ "A-7"; "HMR"; "K9"; "ZULU" ]) (outDir "label-pass")
            ...
            test "label characters resolve NON-TOFU under the renderer's real font registry (FR-004)" {
                ...
                for label in [ "A-7"; "HAMMER"; "K9"; "ZULU-1" ] do
                    ...
                    Expect.equal
                        report.TofuCount
                        0
                        (sprintf "label %s draws real glyphs (no tofu) under the bundled font" label)
            }
    ...
    testList
        ...
        [
            test "a multi-line labelled token rasterises to a non-blank PNG" {
                ...
                let path =
                    Render.toPng { Width = 160; Height = 160 } board (outDir "multiline-pass")
            ...
            test "EVERY line of a multi-line label resolves non-tofu under the renderer's real font registry (FR-004)" {
                ...
                for line in [ "ALPHA"; "BRAVO"; "CHARLIE"; "HOTEL-1"; "K9" ] do
                    ...
                    Expect.equal
                        report.TofuCount
                        0
                        (sprintf "multi-line line %s draws real glyphs (no tofu) under the bundled font" line)
            }
        ]
    ...
    testList
        ...
        [
            ...
            test "EVERY styled run resolves non-tofu in its own weight/size under the real font registry (FR-005)" {
                ...
                for text, weight, scale in runs do
                    ...
                    Expect.equal
                        report.TofuCount
                        0
                        (sprintf "styled run %s (weight=%A scale=%f) draws real glyphs, no tofu" text weight scale)
            }
            ...
            test "same characters / different run styling produce distinguishable rasterised output (SC-002)" {
                ...
                Expect.notEqual
                    (SceneCodec.export styled).CanonicalBytes
                    (SceneCodec.export plain).CanonicalBytes
                    "run styling observably changes the output (B5/SC-002)"
            }
        ]
    ...
    testList
        ...
        [
            test "a decorated (italic/underline/strike/tracking) labelled token rasterises to a non-blank PNG" {
                        ...
                        [
                            styledToken
                                [
                                    ...
                                    { Symbology.run " S P A C E D" with
                                        Tracking = Some 0.25
                                    }
                                ]
                        ]
                ...
                let path =
                    Render.toPng { Width = 240; Height = 180 } board (outDir "decorated-pass")
            ...
            test "EVERY decorated run resolves non-tofu under the real font registry (FR-006)" {
                ...
                for text in [ "QUOTED"; "TAG"; "OLD"; "SPACED"; "K9-1" ] do
                    ...
                    Expect.equal
                        report.TofuCount
                        0
                        (sprintf "decorated run %s draws real glyphs (no tofu) under the bundled font" text)
            }
        ]
                            ...
                            laidToken
                                [
                                    Symbology.align
                                        Justify
                                        [
                                            { Symbology.run "ALPHA BRAVO CHARLIE" with
                                                Italic = Some true
                                                Color = Some blue
                                            }
                                        ]
            ...
            test "EVERY run of a justified / multi-paragraph / decorated label resolves non-tofu (FR-006)" {
                for text in [ "ALPHA"; "BRAVO"; "CHARLIE"; "RETIRED"; "SP" ] do
                    ...
                    Expect.equal
                        report.TofuCount
                        0
                        (sprintf "laid-out run %s draws real glyphs (no tofu) under the bundled font" text)
            }
        ]
            ...
            test "an auto-labelled token (AutoLabel=Some, Label=None) rasterises to a non-blank PNG" {
                let board =
                    Symbology.gallery
                        1
                        140.0
                        [ autoToken [ FactionCode; KlassCode; HealthTier; SpeedPips; ShieldFlag ] ]

                let path =
                    Render.toPng { Width = 200; Height = 160 } board (outDir "auto-label-pass")

                Expect.isTrue (File.Exists path) "image file was written"

                Expect.isTrue
                    ((FileInfo path).Length > 0L)
                    "the auto-labelled board is non-blank (projected glyphs drawn)"
            }
        ...
        [
            test "an auto+motion token rasterises to a non-blank PNG at sampled non-rest phases" {
                for i, phase in List.indexed [ 0.25; 0.5; 0.75 ] do
                    ...
                    let path =
                        Render.toPng { Width = 240; Height = 120 } board (outDir (sprintf "auto-motion-%d" i))

                    Expect.isTrue (File.Exists path) "image file was written"

                    Expect.isTrue
                        ((FileInfo path).Length > 0L)
                        (sprintf "the auto+motion filmstrip is non-blank (phase sample %f)" phase)
            }

            test "a single non-rest animateIn frame of an auto+motion token is non-blank" {
                let centred =
                    { autoMotionToken with
                        Cx = 80.0
                        Cy = 60.0
                    }
                ...
                let path =
                    Render.toPng { Width = 160; Height = 120 } scene (outDir "auto-motion-frame")

                Expect.isTrue
                    ((FileInfo path).Length > 0L)
                    "the animated projected label draws real pixels at a non-rest phase"
            }
