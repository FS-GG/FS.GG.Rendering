...
let private representativeScene =
    let path =
        Path.create EvenOdd [ Path.moveTo 0.0 0.0; Path.lineTo 64.0 0.0; Path.lineTo 64.0 48.0; Path.close ]
...
let feature146PortableSceneRoundTripTests =
    testList
        "Feature146 portable scene round trip"
        [
            test "exports imports and compares representative scene" {
                ...
                | Result.Ok restored ->
                    ...
                    Expect.equal
                        restored.PackageIdentity
                        package.PackageIdentity
                        "package identity is computed from canonical bytes"
            ...
            test "fifty exports of the same scene are byte identical" {
                let packages = [ for _ in 1..50 -> SceneCodec.export representativeScene ]
            ...
            test "glyph-run proof data is preserved" {
                ...
                | Result.Ok restored ->
                    ...
                    Expect.equal
                        glyphRuns.Head.Provider.ProviderId
                        "scene-pure-fallback"
                        "provider evidence is preserved"
            }
            ...
            test "image sources become package-local resource ids" {
                ...
                Expect.equal
                    package.Resources.Head.SourceLabel
                    (Some "/producer/local/logo.png")
                    "local source is diagnostic-only"
