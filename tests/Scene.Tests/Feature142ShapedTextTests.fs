...
let tests =
    testList
        "Feature142 Scene shaped text data"
        [
            test "fallback shaped text exposes metrics, glyphs, runs, provider evidence, and fingerprint" {
                ...
                Expect.floatClose
                    ...
                    (shaped.Glyphs |> List.sumBy _.Advance)
                    "advance comes from the glyph evidence"
            ...
            test "glyph-run data can be projected from shaped text" {
                ...
                Expect.equal
                    (Scene.measureGlyphRun data).Width
                    shaped.Metrics.Width
                    "metrics project to the legacy shape"
            }
