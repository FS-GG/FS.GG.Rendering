    ...
    retained
        ...
        [
            ...
            objectValue
                ...
                false
                {
                    Nodes = [ SceneNode.Line({ X = 25.0; Y = 35.0 }, { X = 35.0; Y = 35.0 }, paint) ]
                }
            objectValue
                "player-red"
                true
                {
                    Nodes =
                        [
                            SceneNode.Circle({ X = 25.0; Y = 35.0 }, 4.0, black)
                            SceneNode.Text((22.0, 37.0), "R", black)
                        ]
                }
        ]
                    ...
                    Nodes =
                        [
                            SceneNode.Translate(
                                (0.5, 0.75),
                                {
                                    Nodes = [ SceneNode.Path(curve, paint) ]
                                }
                            )
                            SceneNode.FilledEllipse(
                                {
                                    X = 7.125
                                    Y = 3.25
                                    Width = 2.5
...
let private moveModelFocus direction state =
    ...
    | available ->
        let current =
            state.Focused |> Option.bind (fun id -> available |> List.tryFindIndex ((=) id))
...
let private modelStep command state =
    match command with
    | ModelReplace(candidate, available, replacementCamera) when candidate <= state.Revision ->
        state, Some "non-increasing"
    ...
    | ModelReplace(candidate, available, replacementCamera) ->
        { state with
            ...
            Focused = state.Focused |> Option.filter (fun id -> List.contains id available)
        },
        None
    ...
    | ModelSelect(_, objectId) ->
        { state with
            Selected = Some objectId
            Focused = Some objectId
        },
        None
    | ModelClearSelection _ -> { state with Selected = None }, None
    | ModelFocusNext _ ->
        { state with
            Focused = moveModelFocus 1 state
        },
        None
    | ModelFocusPrevious _ ->
        { state with
            Focused = moveModelFocus -1 state
        },
        None
        ...
        [
            test "grid and continuous fixtures preserve stable identities and camera meaning" {
                for fixture in [ gridFixture 7; freeFixture 7 ] do
                    ...
                    | SvgAdapterResult.Rendered projected ->
                        ...
                        Expect.equal
                            (projected.Layers |> List.map _.Id)
                            (fixture.Layers |> List.map _.Id)
                            "layer identities and order are unchanged"
                        ...
                        Expect.equal
                            (SvgRetained.tryToScenePoint projected.Camera screen)
                            (Some source)
                            "camera inverse preserves continuous coordinates"
            ...
            test "unsupported nodes and non-finite inputs are distinct explicit outcomes" {
                    ...
                    retained
                        ...
                        [
                            objectValue
                                ...
                                true
                                {
                                    Nodes = [ SceneNode.Circle({ X = nan; Y = 0.0 }, 1.0, black) ]
                                }
                        ]
                ...
                | SvgAdapterResult.Invalid issues ->
                    Expect.isTrue
                        (issues |> List.exists (fun issue -> issue.ObjectId = Some "bad"))
                        "invalid result identifies its semantic object"
            ...
            test "revision replacement preserves valid selection and rejects stale mutation" {
                ...
                let selected =
                    SvgRetained.update (RetainedInteractionMessage.Select(4, "player-red")) initial

                Expect.isNone selected.Error "current revision selection applies"
                let replacement = gridFixture 5

                let replaced =
                    SvgRetained.update (RetainedInteractionMessage.ReplaceScene replacement) selected.State

                Expect.equal
                    replaced.State.SelectedObjectId
                    (Some "player-red")
                    "stable identity preserves selection across a newer revision"

                let stale =
                    SvgRetained.update (RetainedInteractionMessage.Select(4, "cell-2-3")) replaced.State

                Expect.equal stale.State replaced.State "stale command cannot mutate state"

                Expect.equal
                    stale.Error
                    (Some(RetainedInteractionError.StaleRevision(4, 5)))
                    "stale revision is observable"
            }
            ...
            test "bounded retained-interaction model corresponds to every public reducer action" {
                ...
                for state in states do
                    for command in commands do
                        ...
                        Expect.equal
                            (projectRealState actual.State)
                            expectedState
                            $"real reducer state corresponds for {state} / {command}"

                        Expect.equal
                            (projectRealError actual.Error)
                            expectedError
                            $"real reducer outcome corresponds for {state} / {command}"
            }
            ...
            test "replacement retains surviving selection and focus and clears removed identities" {
                ...
                Expect.equal
                    (projectRealState retainedActual.State)
                    retainedExpected
                    "surviving identities and capture are retained"
                ...
                Expect.equal
                    (projectRealState clearedActual.State)
                    clearedExpected
                    "production replacement matches retention rules"
            }
            ...
            test "mutant controls distinguish stale acceptance, mapping, retention, focus, camera, and capture" {
                ...
                Expect.notEqual
                    ...
                    staleExpected
                    "stale acceptance mutant is killed"
                ...
                let retainedExpected, _ =
                    modelStep (ModelReplace(1, [ "a"; "b" ], camera)) { state with Focused = Some "b" }

                Expect.notEqual
                    { retainedExpected with
                        Selected = None
                        Focused = None
                    }
                    retainedExpected
                    "replacement retention mutant is killed"
                ...
                Expect.notEqual
                    { captureExpected with Captured = None }
                    captureExpected
                    "capture ownership mutant is killed"
            }
        ]
