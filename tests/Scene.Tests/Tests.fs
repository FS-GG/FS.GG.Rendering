...
let tests =
    testList
        "Scene public contract"
        [
            test "rectangle descriptions are stable" {
                let node = Scene.rectangle (0.0, 0.0, 10.0, 20.0) (Colors.rgb 1uy 2uy 3uy)
            ...
            test "filled circle and ellipse constructors are public and describe deterministic shape kinds" {
                let circle = Scene.circle { X = 32.0; Y = 24.0 } 12.0 (Colors.rgb 240uy 64uy 32uy)
            ...
            test "shape evidence records fill bounds and partial out-of-bounds placement" {
                ...
                let circle =
                    Scene.circleEvidence { Width = 64; Height = 48 } { X = 4.0; Y = 8.0 } 12.0 fill
                ...
                Expect.equal
                    ...
                    "circle evidence derives bounds from center/radius"
            ...
            test "scene evidence helper returns deterministic hash without a viewer window" {
                ...
                let evidence = SceneEvidence.renderHash { Width = 64; Height = 64 } scene
            ...
            test "scene evidence writes metadata output with stable size renderer and value" {
                ...
                | Result.Ok evidence ->
                    ...
                    Expect.equal
                        ...
                        "deterministic-scene"
                        "metadata uses deterministic scene renderer"
            ...
            test "render with Format = Png fails loud instead of writing a hash to a .png (P6/R10)" {
                ...
                let path =
                    Path.Combine(Path.GetTempPath(), "fs-gg-scene-evidence-png-should-not-exist.png")
            ...
            test "renderHash bytes are deterministic and do not require viewer startup" {
                ...
                match
                    SceneEvidence.renderHash { Width = 80; Height = 40 } scene,
                    SceneEvidence.renderHash { Width = 80; Height = 40 } scene
                with
                | Result.Ok first, Result.Ok second ->
                    Expect.equal
                        first.Value
                        second.Value
                        "capability-hash evidence is stable for the same scene (no viewer startup)"
            ...
            test "scene evidence reports unsupported renderer capabilities explicitly" {
                ...
                | Result.Error failure ->
                    Expect.equal
                        ...
                        "unsupported renderer mode is environment/capability failure"
            ...
            test "layout evidence classifies complete non-overlapping HUD and gameplay facts as readable" {
                ...
                Expect.equal
                    (LayoutEvidence.classify report).ProofLevel
                    ReadableLayout
                    "complete layout facts prove readability"
            }
            ...
            test "layout evidence keeps deterministic render metadata separate from readability proof" {
                ...
                let renderEvidence =
                    Scene.renderReadbackEvidence { Width = 640; Height = 480 } scene

                let report = LayoutEvidence.fromRenderEvidence scene renderEvidence

                Expect.equal
                    report.ProofLevel
                    DeterministicRenderOnly
                    "render hash evidence is not relabeled as readable layout"
            ...
            test "layout evidence reports unsupported inspection without claiming readability" {
                ...
                Expect.exists
                    report.Diagnostics
                    (fun item -> item.Contains "font-metrics")
                    "diagnostic names unsupported fact"
            }
            ...
            test "layout evidence detects missing and overlapping bounds" {
                ...
                | LayoutOverlaps overlaps ->
                    Expect.exists overlaps (fun item -> item.Kind = HudTextOverlap) "HUD/HUD overlap is reported"
