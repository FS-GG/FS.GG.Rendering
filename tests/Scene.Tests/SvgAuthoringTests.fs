...
let private hash document =
    match SvgAsset.contentHash document with
    | Ok value -> value
    | Error issues -> failtestf "hash failed: %A" issues
    ...
    testList
        ...
        [
            ...
            test "supported export and reimport preserve geometry order and presentation" {
                ...
                let exported =
                    SvgDocument.exportSvg "roundtrip" first |> Result.defaultWith (failtestf "%A")

                let restored =
                    SvgImport.importXml (request "restored" SvgDocument.defaultLimits) exported
                ...
                Expect.equal
                    (restored.Children |> List.map (fun value -> value.Presentation.IsSome))
                    (first.Children |> List.map (fun value -> value.Presentation.IsSome))
                    "presentation remains explicit"

                let sceneKinds (document: SvgDocument) =
                    document.Children
                    |> List.map (fun value ->
                        match value.Content with
                        | SvgElementContent.Group _ -> "group"
                        | SvgElementContent.SceneLeaf {
                                                          Nodes = SceneNode.PaintedRectangle _ :: _
                                                      } -> "rect"
                        | SvgElementContent.SceneLeaf { Nodes = SceneNode.Path _ :: _ } -> "path"
                        | SvgElementContent.SymbolInstance _ -> "use"
                        | _ -> "other")

                Expect.equal
                    (sceneKinds restored)
                    [ "group"; "group"; "group" ]
                    "export wrappers preserve top-level order"
            }
            ...
            test "active malformed over-budget and external XML fail with locations" {
                let cases =
                    [
                        ...
                        "external",
                        richSvg.Replace("href=\"#badge\"", "href=\"https://example.test/badge.svg#x\""),
                        "external-reference"
            ...
            test "asset schema hash references and cycles fail closed" {
                ...
                let serialized =
                    SvgAsset.serializeCatalog (catalog [ valid ])
                    |> Result.defaultWith (failtestf "%A")

                Expect.equal
                    (SvgAsset.deserializeCatalog serialized)
                    (Ok(catalog [ valid ]))
                    "catalog wire round trip is canonical"

                match
                    SvgAsset.deserializeCatalog (serialized.Replace(SvgAsset.catalogSchema, "fsgg.svg-asset-catalog/9"))
                with
                | Error issues ->
                    Expect.exists
                        issues
                        (fun value -> value.Code = "unknown-catalog-schema")
                        "wire schema version is checked"
                | Ok _ -> failtest "unknown wire schema imported"

                let wrong =
                    { valid with
                        ContentHash = String.replicate 64 "0"
                    }

                Expect.exists
                    (SvgAsset.validateCatalog (catalog [ wrong ]))
                    (fun value -> value.Code = "content-hash-mismatch")
                    "hash mismatch diagnosed"

                let missing =
                    { valid with
                        Dependencies = [ { AssetId = "missing"; Revision = 1 } ]
                    }

                Expect.exists
                    (SvgAsset.validateCatalog (catalog [ missing ]))
                    (fun value -> value.Code = "missing-asset-reference")
                    "missing dependency diagnosed"

                let first =
                    { valid with
                        AssetId = "a"
                        Dependencies = [ { AssetId = "b"; Revision = 1 } ]
                    }

                let second =
                    { valid with
                        AssetId = "b"
                        Dependencies = [ { AssetId = "a"; Revision = 1 } ]
                    }

                Expect.exists
                    (SvgAsset.validateCatalog (catalog [ first; second ]))
                    (fun value -> value.Code = "cyclic-asset-reference")
                    "cycle diagnosed"

                Expect.exists
                    (SvgAsset.validateCatalog
                        { catalog [ valid ] with
                            Schema = "fsgg.svg-asset-catalog/99"
                        })
                    (fun value -> value.Code = "unknown-catalog-schema")
                    "unknown version diagnosed"

                let overBudget =
                    [
                        for revision in 1 .. SvgDocument.defaultLimits.MaxDefinitions + 1 ->
                            { valid with Revision = revision }
                    ]

                Expect.exists
                    (SvgAsset.validateCatalog (catalog overBudget))
                    (fun value -> value.Code = "asset-limit")
                    "catalog allocation budget is enforced"
            }
            ...
            test "shared revision update preserves overrides and reports removed targets" {
                ...
                let revisionTwo =
                    { revisionOne with
                        Children = revisionOne.Children.Tail
                    }

                let assets =
                    catalog [ asset "shape" 1 revisionOne []; asset "shape" 2 revisionTwo [] ]

                let overrideValue =
                    {
                        ElementId = revisionOne.Children.Head.Id
                        Property = SvgPrefabProperty.Visibility
                        Value = SvgPrefabOverrideValue.Visibility false
                    }
                ...
                let state =
                    SvgAuthoring.tryCreate 0 revisionOne assets instances
                    |> Result.defaultWith (failtestf "%A")

                let resolved, conflicts =
                    SvgAsset.resolveInstance assets instances.Head
                    |> Result.defaultWith (failtestf "%A")
                ...
                let transaction =
                    {
                        ...
                        Id = "upgrade"
                        Operations = [ SvgAuthoringOperation.UpdateInstances("shape", 1, 2) ]
                    }

                let updated =
                    SvgAuthoring.commit 0 transaction state |> Result.defaultWith (failtestf "%A")

                Expect.equal
                    (updated.Instances |> List.map _.AcceptedRevision)
                    [ 2; 2 ]
                    "shared instances update explicitly"
            ...
            test "grouped preview commits once; cancellation stale and invalid work are atomic" {
                ...
                let state =
                    SvgAuthoring.tryCreate 4 document assets []
                    |> Result.defaultWith (failtestf "%A")
                ...
                let gesture =
                    {
                        ...
                        Operations =
                            [
                                SvgAuthoringOperation.TransformElements(ids, SvgAffine.translate 2.0 3.0)
                                SvgAuthoringOperation.TransformElements(ids, SvgAffine.translate 1.0 0.0)
                            ]
                    }

                let previewed =
                    SvgAuthoring.preview 4 gesture state |> Result.defaultWith (failtestf "%A")
                ...
                let cancelled =
                    SvgAuthoring.cancelPreview gesture.Id previewed
                    |> Result.defaultWith (failtestf "%A")

                Expect.equal cancelled state "cancellation restores exact state"

                let committed =
                    SvgAuthoring.preview 4 gesture state
                    |> Result.bind (SvgAuthoring.commitPreview 4 gesture.Id)
                    |> Result.defaultWith (failtestf "%A")
                ...
                let newEdit =
                    SvgAuthoring.commit 7 { gesture with Id = "new-edit" } redone
                    |> Result.defaultWith (failtestf "%A")

                Expect.isEmpty newEdit.Redo "new edit clears redo"

                match SvgAuthoring.commit 4 gesture committed with
                | Error(SvgAuthoringError.StaleRevision(4, 5)) -> ()
                | value -> failtestf "stale completion changed state: %A" value
                ...
                match SvgAuthoring.commit 5 invalid committed with
                | Error(SvgAuthoringError.InvalidTransaction _) -> ()
                | value -> failtestf "invalid operation accepted: %A" value
                ...
                match
                    SvgAuthoring.commit
                        5
                        { gesture with
                            Schema = "fsgg.svg-authoring-transaction/99"
                        }
                        committed
                with
                | Error(SvgAuthoringError.InvalidTransaction issues) ->
                    Expect.equal issues.Head.Code "unknown-authoring-schema" "unknown transaction schema refuses"
                | value -> failtestf "unknown transaction schema committed: %A" value
                ...
                let cyclic =
                    {
                        ...
                        Operations =
                            [
                                SvgAuthoringOperation.UpsertAsset first
                                SvgAuthoringOperation.UpsertAsset second
                            ]
                    }

                match SvgAuthoring.commit 5 cyclic committed with
                | Error(SvgAuthoringError.InvalidTransaction issues) ->
                    Expect.exists
                        issues
                        (fun value -> value.Code = "cyclic-asset-reference")
                        "cycle refuses atomic commit"
                | value -> failtestf "cyclic catalog committed: %A" value

                let missingAsset =
                    asset "missing-owner" 1 document [ { AssetId = "absent"; Revision = 1 } ]

                let missing =
                    {
                        Schema = SvgAuthoring.transactionSchema
                        Id = "missing"
                        Operations = [ SvgAuthoringOperation.UpsertAsset missingAsset ]
                    }

                match SvgAuthoring.commit 5 missing committed with
                | Error(SvgAuthoringError.InvalidTransaction issues) ->
                    Expect.exists
                        issues
                        (fun value -> value.Code = "missing-asset-reference")
                        "missing reference refuses atomic commit"
                | value -> failtestf "missing reference committed: %A" value
            }
            ...
            test "play snapshot remains immutable across later edits" {
                ...
                let state =
                    SvgAuthoring.tryCreate 0 document (catalog []) []
                    |> Result.defaultWith (failtestf "%A")

                let snapshotted =
                    SvgAuthoring.takePlaySnapshot 0 state |> Result.defaultWith (failtestf "%A")
                ...
                let changed =
                    SvgAuthoring.commit
                        0
                        {
                            Schema = SvgAuthoring.transactionSchema
                            Id = "later"
                            Operations = [ SvgAuthoringOperation.ReplaceDocument replacement ]
                        }
                        snapshotted
                    |> Result.defaultWith (failtestf "%A")
            ...
            test "C03 paths paint definitions symbols and text import and export together" {
                ...
                let exported =
                    SvgDocument.exportSvg "studio-roundtrip" imported
                    |> Result.defaultWith (failtestf "%A")

                let reopened =
                    SvgImport.importXml (request "studio-reopen" SvgDocument.defaultLimits) exported
                    |> Result.defaultWith (failtestf "%A")
                ...
                match
                    SvgImport.importXml
                        (request "unsafe" SvgDocument.defaultLimits)
                        (xml.Replace("</defs>", "<style>@font-face{src:url(data:x)}</style></defs>"))
                with
                | Error issues ->
                    Expect.equal issues.Head.Code "active-content" "default import still refuses CSS/data resources"
            ...
            test "portable art edits preserve tool state and grouped history" {
                ...
                let path =
                    {
                        Commands =
                            [
                                ...
                                PathCommand.QuadTo({ X = 5.0; Y = 8.0 }, { X = 10.0; Y = 0.0 })
                                PathCommand.CubicTo({ X = 12.0; Y = 2.0 }, { X = 14.0; Y = 2.0 }, { X = 16.0; Y = 0.0 })
                                PathCommand.Close
                            ]
                ...
                let created =
                    SvgArt.create "curve" (SvgArtPrimitive.Path path) presentation rectangle
                    |> Result.defaultWith (failtestf "%A")

                let edited =
                    SvgArt.insertPathPoint "curve" 1 { X = 2.0; Y = 1.0 } created
                    |> Result.bind (SvgArt.removePathPoint "curve" 1)
                    |> Result.defaultWith (failtestf "%A")

                let grouped =
                    SvgArt.group "pair" [ "rect"; "curve" ] edited
                    |> Result.bind (SvgArt.ungroup "pair")
                    |> Result.defaultWith (failtestf "%A")

                let state =
                    SvgAuthoring.tryCreate 0 empty (catalog []) []
                    |> Result.defaultWith (failtestf "%A")
                ...
                let gesture =
                    {
                        Schema = SvgAuthoring.transactionSchema
                        Id = "pointer-1"
                        Operations = [ SvgAuthoringOperation.ReplaceDocument grouped ]
                    }

                let previewed =
                    SvgAuthoring.preview 0 gesture state
                    |> Result.bind (SvgAuthoring.preview 0 gesture)
                    |> Result.defaultWith (failtestf "%A")

                let committed =
                    SvgAuthoring.commitPreview 0 gesture.Id previewed
                    |> Result.defaultWith (failtestf "%A")

                Expect.equal committed.Undo.Length 1 "repeated gesture preview commits once"

                Expect.equal
                    SvgArt.initialState.Camera
                    SvgAffine.identity
                    "camera is separate from accepted document history"

                Expect.equal
                    (SvgArt.snapPoint SvgArt.initialState { X = 12.0; Y = (-4.0) })
                    (Ok { X = 16.0; Y = (-8.0) })
                    "grid snapping is deterministic"

                match SvgArt.translate [ "missing" ] 1.0 1.0 grouped with
                | Error _ -> ()
                | Ok _ -> failtest "invalid numeric selection changed content"
            }
            ...
            test "geometry preparation is bounded, adaptive and stale-safe" {
                let closed =
                    {
                        Commands =
                            [
                                PathCommand.MoveTo { X = 0.0; Y = 0.0 }
                                PathCommand.QuadTo({ X = 5.0; Y = 10.0 }, { X = 10.0; Y = 0.0 })
                ...
                let result =
                    {
                        OperationId = "union-1"
                        AcceptedRevision = 7
                        InputContentHash = prepared.Request.InputContentHash
                        Contours =
                            [
                                [
                                    { X = 0.0; Y = 0.0 }
                ...
                Expect.isOk
                    (SvgGeometry.transaction result prepared empty)
                    "matching result produces one atomic transaction"

                match SvgGeometry.transaction { result with AcceptedRevision = 8 } prepared empty with
                | Error(SvgArtError.InvalidInput issues) ->
                    Expect.equal issues.Head.Code "stale-geometry-result" "stale worker result discarded"
                | value -> failtestf "stale result accepted: %A" value

                Expect.isError
                    (SvgGeometry.prepare "bad" 0 PathOperation.Union [ closed ] [] 0.001)
                    "deviation below 0.01 refuses"
                ...
                Expect.isError
                    (SvgGeometry.prepare
                        ...
                        [
                            { closed with
                                Commands = closed.Commands |> List.filter ((<>) PathCommand.Close)
                            }
                        ]
                        []
                        0.25)
                    "open contours refuse"
                ...
                Expect.isError
                    (SvgGeometry.prepare "degenerate" 0 PathOperation.Union [ degenerate ] [] 0.25)
                    "degenerate contours refuse before worker dispatch"
            }
            ...
            test "asset revisions are immutable while identical reinsertion is harmless" {
                ...
                let state =
                    SvgAuthoring.tryCreate 0 document (catalog [ original ]) []
                    |> Result.defaultWith (failtestf "%A")
                ...
                let transaction value id =
                    {
                        Schema = SvgAuthoring.transactionSchema
                        Id = id
                        Operations = [ SvgAuthoringOperation.UpsertAsset value ]
                    }

                Expect.isOk
                    (SvgAuthoring.commit 0 (transaction original "same") state)
                    "identical revision reinsertion is accepted"

                let changed =
                    { original with
                        Rights = { original.Rights with License = "MIT" }
                    }

                match SvgAuthoring.commit 0 (transaction changed "changed") state with
                | Error(SvgAuthoringError.InvalidTransaction issues) ->
                    Expect.equal issues.Head.Code "immutable-asset-revision" "changed rights require a new revision"
            ...
            test "verified Noto resource round trips detached and rejects altered bytes and manifests" {
                    ...
                    File
                        .ReadAllText(
                            Path.Combine(
                                ...
                                "../../src/Scene.SvgBrowser/noto-sans-latin-400-normal.woff2.base64"
                            )
                        )
                        .Replace("\n", "")
                        .Replace("\r", "")
                ...
                let resource =
                    SvgResourceInterchange.notoSansLatin400 base64
                    |> Result.defaultWith (failtestf "%A")

                Expect.equal
                    resource.Sha256
                    "09aee8065d25508f23a4c3d92cd777ac869c52d93fd868a88f025d888a7937d6"
                    "approved exact bytes verified"

                let text =
                    {
                        Id = "text"
                        SemanticId = Some "text"
                        Visible = true
                ...
                let restored =
                    SvgResourceInterchange.importXml (request "offline" SvgDocument.defaultLimits) xml
                    |> Result.defaultWith (failtestf "%A")
                ...
                match SvgResourceInterchange.notoSansLatin400 altered with
                | Error issues -> Expect.equal issues.Head.Code "font-hash-mismatch" "altered bytes refuse"
                | Ok _ -> failtest "altered font accepted"
                ...
                match
                    SvgResourceInterchange.exportSvg
                        ...
                        {
                            Document = document
                            Fonts = [ { resource with License = "MIT" } ]
                        }
                with
                | Error issues -> Expect.equal issues.Head.Code "unapproved-font-manifest" "wrong rights refuse"
                | Ok _ -> failtest "wrong font rights accepted"

                let excessive = String.replicate 1398104 "A"

                match SvgResourceInterchange.notoSansLatin400 excessive with
                | Error issues ->
                    Expect.equal issues.Head.Code "font-byte-limit" "encoded size rejects before allocation"
                | Ok _ -> failtest "excessive font accepted"
            }
            ...
            test "scene envelope, grid placement and metadata history remain one atomic value" {
                ...
                let layer =
                    {
                        Id = "layer-main"
                        SemanticId = Some "layer-main"
                        Visible = true
                        Transform = SvgAffine.identity
                        ClipId = None
                        MaskId = None
                        Presentation = None
                        Content = SvgElementContent.Group document.Children
                    }
                            ...
                            [
                                {
                                    ...
                                    VisualElementId = Some visual
                                    PrefabInstanceId = None
                                    Properties =
                                        [
                                            {
                                                Key = "terrain"
                                                Value = SvgScenePropertyValue.Text "sand"
                                            }
                                            {
                                                Key = "cost"
                                                Value = SvgScenePropertyValue.Number 2.5
                                            }
                                        ]
                                }
                                {
                                    ...
                                    PrefabInstanceId = Some "prefab-a"
                                    Properties =
                                        [
                                            {
                                                Key = "solid"
                ...
                Expect.isEmpty
                    (SvgScene.validateDescriptors descriptors metadata)
                    "product descriptor accepts generic typed properties"

                Expect.equal
                    (SvgScenePlacement.grid metadata.Grid.Value { X = 14.2; Y = 5.1 })
                    (Ok { X = 11.0; Y = 4.0 })
                    "nonzero-origin integer grid snaps in document space"

                Expect.equal
                    (SvgScenePlacement.freeform { X = 1.25; Y = (-3.75) })
                    (Ok { X = 1.25; Y = (-3.75) })
                    "freeform coordinates retain fractions"
                ...
                Expect.equal
                    ...
                    (Ok envelope)
                    "scene v1 preserves document, catalog, entities and references"

                let initial =
                    SvgAuthoring.tryCreateScene 0 metadata sceneDocument envelope.Catalog instances
                    |> Result.defaultWith (failtestf "%A")
                ...
                let changed =
                    { metadata with
                        Entities =
                            [
                                { metadata.Entities.Head with
                                    Properties =
                                        [
                                            {
                                                Key = "terrain"
                                                Value = SvgScenePropertyValue.Text "water"
                                            }
                                            {
                                                Key = "cost"
                                                Value = SvgScenePropertyValue.Number 4.0
                                            }
                                        ]
                                }
                            ]
                ...
                let tx =
                    {
                        Schema = SvgAuthoring.transactionSchema
                        Id = "metadata-brush"
                        Operations = [ SvgAuthoringOperation.ReplaceSceneMetadata changed ]
                    }

                let committed =
                    SvgAuthoring.commit 0 tx initial |> Result.defaultWith (failtestf "%A")
                ...
                let snapshot =
                    SvgAuthoring.takePlaySnapshot 1 committed |> Result.defaultWith (failtestf "%A")
            ...
            test "scene import and metadata validation refuse without partial acceptance" {
                ...
                let migrated =
                    SvgScene.migrateLegacy document (catalog[]) [] []
                    |> Result.defaultWith (failtestf "%A")
                ...
                Expect.isError
                    (SvgScene.deserialize (wire.Replace(SvgScene.schema, "fsgg.svg-scene/9")))
                    "unknown serialized scene versions refuse"
                ...
                | Error issues ->
                    Expect.equal
                        issues.Head.Code
                        "scene-byte-limit"
                        "aggregate envelope limit is checked before parsing"
                | Ok _ -> failtest "over-budget scene envelope imported"

                let bad =
                    { migrated.Metadata with
                        Entities =
                            [
                                {
                                    EntityId = "bad"
                                    KindId = "sample.object"
                                    VisualElementId = Some "missing"
                                    PrefabInstanceId = None
                                    Properties =
                                        [
                                            {
                                                Key = "x"
                                                Value = SvgScenePropertyValue.Number nan
                                            }
                                        ]
                                }
                            ]
                ...
                let tx =
                    {
                        Schema = SvgAuthoring.transactionSchema
                        Id = "bad-metadata"
                        Operations = [ SvgAuthoringOperation.ReplaceSceneMetadata bad ]
                    }
