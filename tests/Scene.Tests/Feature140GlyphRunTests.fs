...
let private samples =
    [ "Stable"; "Cache 42"; "Symbols #@"; "mono"; "Fallback proof" ]
        ...
        [
            test "five deterministic samples produce stable fingerprints" {
                for sample in samples do
                    ...
                    Expect.equal
                        (Scene.glyphRunFingerprint a)
                        a.Fingerprint
                        "stored fingerprint matches recomputed fingerprint"
            }
            ...
            test "glyph-run proof is an explicit public Scene element and legacy text remains separate" {
                ...
                Expect.isFalse
                    (Scene.describe legacy |> List.contains GlyphRunElement)
                    "non-opt-in text does not become a glyph-run proof"
            }
