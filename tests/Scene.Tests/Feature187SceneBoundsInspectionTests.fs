...
let tests =
    testList
        "Feature187 deterministic scene bounds and hierarchy inspection"
        [
            test "MiniTank clipped authored text is an early structural red signal" {
                ...
                Expect.equal
                    correctedText.ViewportRelation
                    SceneViewportRelation.Inside
                    "policy correction clears the red signal"
            }
            ...
            test "translation, perspective, clip, shaped text, and typed unknowns are explicit" {
                ...
                let scene =
                    Scene.group
                        [
                            ...
                            Scene.line { X = 40.0; Y = 10.0 } { X = 40.0; Y = 50.0 } (Paint.fill Colors.white)
                            Scene.withPerspective
                                { affine with
                                    M13 = 0.0
                                    M23 = 0.0
                ...
                let glyph = rows |> List.find (fun row -> row.Kind = GlyphRunElement)
                ...
                let emptyPath = rows |> List.find (fun row -> row.Kind = PathElement)
                ...
                Expect.equal
                    emptyPath.ViewportRelation
                    SceneViewportRelation.Unknown
                    "unknown stays unknown at viewport layer"
                ...
                let horizonRectangle =
                    rows
                    |> List.find (fun row -> row.Kind = RectangleElement && row.Path.Contains("/perspective/"))
            ...
            test "paint effects and joins conservatively expose viewport bleed" {
                ...
                let mitered =
                    ...
                    |> Paint.withMiter 4.0
                    |> Scene.path (
                        Path.create Winding [ Path.moveTo 180.0 20.0; Path.lineTo 195.0 30.0; Path.lineTo 180.0 40.0 ]
                    )

                let discrete =
                    Paint.stroke Colors.white 2.0
                    |> Paint.withPathEffect (Discrete(4.0, 10.0))
                    |> Scene.path (Path.create Winding [ Path.moveTo 180.0 55.0; Path.lineTo 195.0 55.0 ])
                ...
                for kind, fragment in
                    ...
                    let row =
                        rows |> List.find (fun row -> row.Kind = kind && row.Path.Contains fragment)
            ...
            test "zero-width strokes preserve line hairlines and short-vertex marker footprints" {
                ...
                let line =
                    Scene.line { X = 40.0; Y = 10.0 } { X = 40.0; Y = 50.0 } zeroStroke
                    |> SceneInspection.inspect viewport
                    |> List.exactlyOne
                    |> knownBounds
                ...
                Expect.floatClose
                    ...
                    vertices.Width
                    14.0
                    "both marker circles contribute even with a zero stroke"
            ...
            test "chart bounds union only bars that the renderer actually draws" {
                ...
                let empty =
                    Scene.chart [ 0.0; -1.0 ] |> SceneInspection.inspect viewport |> List.exactlyOne
