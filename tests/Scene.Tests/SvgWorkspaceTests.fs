...
let tests =
    testList "SVG workspace" [
        test "responsive collapse preserves preferred layout and authoring owner state" {
            ...
            let art = { SvgArt.initialState with Selection = [] }
            ...
            Expect.equal (panel "timeline" narrow).Effective (SvgPanelPlacement.Docked(SvgWorkspaceDock.Bottom, 0)) "bottom dock remains"
            let (authorAfterWide, artAfterWide, wide), _ =
                updateWorkspace (SvgWorkspaceMessage.SetViewportWidth 1280.0) (authorAfterNarrow, artAfterNarrow, narrow)
            Expect.equal (panel "tools" wide).Effective (panel "tools" workspace).Preferred "preferred dock restores"
        ...
        test "layout codec is deterministic bounded and rejects malformed state" {
            let layout =
                { SvgWorkspace.defaultLayout with
                    Revision = 4
                    Panels =
                        [ { Id = "floating"; Preferred = SvgPanelPlacement.Floating(1.5, 2.5, 320.0, 200.0); Effective = SvgPanelPlacement.Collapsed }
                          { Id = "dock"; Preferred = SvgPanelPlacement.Docked(SvgWorkspaceDock.Right, 3); Effective = SvgPanelPlacement.Collapsed } ] }
            ...
            let duplicate = Encoding.UTF8.GetString bytes |> fun value -> value.Replace("end\n", "panel\tZG9jaw==\tc\nend\n") |> Encoding.UTF8.GetBytes
            Expect.isError (SvgWorkspace.decodeLayout duplicate) "duplicate panel is rejected before construction"
            let malformed = Encoding.UTF8.GetBytes "fsgg.svg-workspace-layout\t1\nschema\t!!!\nrevision\t0\npanel\teA==\tf,NaN,0,1,1\nend\n"
            Expect.isError (SvgWorkspace.decodeLayout malformed) "invalid base64 and non-finite placement refuse"
            Expect.equal (SvgWorkspace.decodeLayout (Array.zeroCreate 65537)) (Error [ "layout-byte-limit" ]) "byte budget is enforced"
        }
        ...
        test "invalid updates and imports preserve the previous workspace" {
            ...
            let invalidPlacement, _ = SvgWorkspace.update (SvgWorkspaceMessage.SetPanelPlacement("tools", SvgPanelPlacement.Floating(0.0, 0.0, -1.0, 20.0))) initial
            Expect.equal invalidPlacement initial "invalid placement is atomic"
            let imported, effects = SvgWorkspace.update (SvgWorkspaceMessage.ImportLayout(Encoding.UTF8.GetBytes "bad")) initial
        ...
        test "modes and overlays derive contexts and restore stable focus" {
            ...
            Expect.equal (SvgWorkspace.activeContexts initial) [ "workspace"; "workspace.play" ] "mode is projected into context"
            let opened, _ = SvgWorkspace.update (SvgWorkspaceMessage.OpenPalette "toolbar.play") initial
            Expect.equal (SvgWorkspace.activeContexts opened) [ "workspace"; "workspace.play"; "workspace.palette" ] "overlay context is innermost"
        ...
        test "accepted placement revisions persist while viewport changes remain preferences-free" {
            ...
            let moved, effects = SvgWorkspace.update (SvgWorkspaceMessage.SetPanelPlacement("tools", SvgPanelPlacement.Docked(SvgWorkspaceDock.Right, 2))) initial
            Expect.equal moved.Layout.Revision 1 "accepted preference advances revision"
            Expect.equal (panel "tools" moved).Preferred (SvgPanelPlacement.Docked(SvgWorkspaceDock.Right, 2)) "preferred edge changes"
            Expect.equal (panel "tools" moved).Effective SvgPanelPlacement.Collapsed "responsive effective placement remains collapsed"
            Expect.exists effects (function SvgWorkspaceEffect.PersistLayout bytes -> SvgWorkspace.decodeLayout bytes |> Result.isOk | _ -> false) "canonical preference bytes are persisted"
            let resized, _ = SvgWorkspace.update (SvgWorkspaceMessage.SetViewportWidth 1000.0) moved
            Expect.equal resized.Layout.Revision 1 "responsive projection is not a preference edit"
            Expect.equal (panel "tools" resized).Effective moved.Layout.Panels.Head.Preferred "wide projection restores the accepted dock"
        }
    ]
