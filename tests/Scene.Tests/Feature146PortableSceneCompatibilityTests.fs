...
let private packageBytes () =
    let package =
        SceneCodec.export (Scene.rectangle (0.0, 0.0, 10.0, 10.0) Colors.white)
...
let feature146PortableSceneCompatibilityTests =
    testList
        "Feature146 portable scene compatibility"
        [
            ...
            test "unknown optional tag is skipped and reported" {
                ...
                Expect.exists
                    report.Diagnostics
                    (fun d -> d.Severity = DiagnosticSeverity.Warning && d.Stage = Parse)
                    "optional tag warning is reported"
            }
        ]
