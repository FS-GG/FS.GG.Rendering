...
let feature146PackageCapabilityInspectionTests =
    testList
        "Feature146 package capability inspection"
        [
            test "required unsupported capability rejects" {
                ...
                Expect.equal
                    ...
                    "optional glyph-run can degrade when required capabilities are supported"

                Expect.exists
                    report.CapabilityVerdicts
                    (fun verdict -> verdict.Requirement.CapabilityId = "scene.glyph-run" && verdict.Degraded)
                    "glyph run is degraded"
            }
            ...
            test "blocking required capability names affected path" {
                ...
                Expect.exists
                    report.Diagnostics
                    (fun d -> d.CapabilityId = Some "scene.rectangle" && d.ScenePath.IsSome)
                    "affected scene path is diagnostic"
            }
        ]
