...
let feature146PortableSceneResourceTests =
    testList
        "Feature146 portable scene resource inspection"
        [
            test "required missing resource rejects package" {
                let package = resourceScenePackage ()

                let report =
                    SceneCodec.inspectWith SceneCodec.defaultInspectionOptions package.CanonicalBytes

                Expect.equal report.Status PackageRejected "missing required resource rejects"

                Expect.exists
                    report.Diagnostics
                    (fun d -> d.Stage = Resource && d.ResourceId = Some "image-0001")
                    "resource diagnostic names the missing resource"
            }
            ...
            test "resource hash mismatch rejects package" {
                ...
                Expect.exists
                    report.Diagnostics
                    (fun d -> d.Message.Contains("metadata does not match"))
                    "metadata mismatch is diagnostic"
            }
