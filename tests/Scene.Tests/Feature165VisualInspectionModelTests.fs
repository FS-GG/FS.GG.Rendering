    ...
    testList
        ...
        [
            test "status severity and fact tokens are stable lowercase contract strings" {
                Expect.equal
                    (VisualInspection.statusText VisualInspectionStatus.EnvironmentLimited)
                    "environment-limited"
                    "status token"

                Expect.equal
                    (VisualInspection.severityText VisualInspectionSeverity.Blocking)
                    "blocking"
                    "severity token"

                Expect.equal
                    (VisualInspection.measurementModeText VisualInspectionMeasurementMode.Approximate)
                    "approximate"
                    "measurement token"
                ...
                Expect.equal
                    (VisualInspection.nodeKindText (VisualInspectionNodeKind.Custom "Fancy Panel"))
                    "fancy-panel"
                    "custom kind token"

                Expect.equal
                    (VisualInspection.paintRoleText VisualInspectionPaintRole.Background)
                    "background"
                    "paint token"

                Expect.equal
                    (VisualInspection.surfaceRoleText VisualInspectionSurfaceRole.Popup)
                    "popup"
                    "surface token"

                Expect.equal
                    (VisualInspection.clipStatusText VisualInspectionClipStatus.Intentional)
                    "intentional"
                    "clip token"

                Expect.equal
                    (VisualInspection.coverageStatusText VisualInspectionCoverageStatus.Complete)
                    "complete"
                    "coverage token"
            }
            ...
            test "stable finding ids are deterministic across affected id order" {
                let first =
                    VisualInspection.stableFindingId "text-contained-in-owner" [ "body"; "title" ]

                let second =
                    VisualInspection.stableFindingId "text-contained-in-owner" [ "title"; "body" ]
            ...
            test "artifact diagnostics report duplicate ids invalid parents and unsupported facts" {
                ...
                Expect.exists
                    diagnostics
                    (fun d -> d.Contains("unsupported visual inspection fact"))
                    "unsupported fact disclosure"
            }
            ...
            test "normalizeArtifact orders nodes regions findings and unsupported facts deterministically" {
                ...
                let factA =
                    VisualInspection.unsupportedFact "zeta" None true "reason" "diagnostic" false

                let factB =
                    VisualInspection.unsupportedFact "alpha" None true "reason" "diagnostic" false
                ...
                Expect.equal
                    (normalized.Nodes |> List.map _.NodeId)
                    [ "first"; "second" ]
                    "nodes sorted by order and id"
            ...
            test "normalizeArtifact collapses duplicate visual finding ids keeping first occurrence (FR-006)" {
                ...
                let unique =
                    VisualInspection.finding "contrast" VisualInspectionSeverity.Info [ "b" ] [] "u" "expected" "actual"

                Expect.equal dupFirst.FindingId dupSecond.FindingId "precondition: the duplicates share a FindingId"

                let input =
                    { artifact [] [] with
                        Findings = [ dupFirst; dupSecond; unique ]
                    }

                let normalized = VisualInspection.normalizeArtifact input

                Expect.equal
                    (normalized.Findings |> List.map _.FindingId)
                    [ unique.FindingId; dupFirst.FindingId ]
                    "one per FindingId, sorted, unique preserved"

                Expect.equal
                    (normalized.Findings |> List.map _.Message)
                    [ "u"; "first" ]
                    "first occurrence of the duplicate is kept"
            }
            ...
            test "retained normalizeArtifact collapses duplicate finding ids uniformly (FR-006/SC-003)" {
                ...
                let normalized =
                    RetainedInspection.normalizeArtifact (retainedArtifact [ dupFirst; dupSecond; unique ])
                ...
                Expect.equal
                    (normalized.Findings |> List.map _.Message)
                    [ "u"; "first" ]
                    "first occurrence kept, unique preserved"
            }
        ]
