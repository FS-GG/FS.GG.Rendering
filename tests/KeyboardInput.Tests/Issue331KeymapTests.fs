...
let tests =
    testList
        "Issue 331 keymap value + rebind operations"
        [
            ...
            test "add is non-destructive on an already-bound key" {
                let keymap =
                    Keymap.empty |> Keymap.add "Enter" "Confirm" |> Keymap.add "Enter" "Cancel"
            ...
            test "rebind an already-bound key replaces its command (the headline op)" {
                ...
                Expect.equal
                    (Keymap.tryFind "ArrowLeft" rebound)
                    (Some "StrafeLeft")
                    "the key points at the new command"
            ...
            test "assignKey names the key-indexed upsert and preserves another key for the same command" {
                let keymap =
                    Keymap.empty
                    |> Keymap.assignKey "w" "MoveUp"
                    |> Keymap.assignKey "ArrowUp" "MoveUp"

                expectBindings
                    [ binding "w" "MoveUp"; binding "ArrowUp" "MoveUp" ]
                    keymap
                    "both command bindings remain"
            }

            test "replaceCommandBinding removes old command keys and displaces the intended key" {
                let before =
                    Keymap.ofBindings [ binding "w" "MoveUp"; binding "ArrowUp" "MoveUp"; binding "z" "Fire" ]
                ...
                expectBindings
                    [ binding "z" "MoveUp" ]
                    after
                    "the selected command has exactly one key and the prior owner is displaced"
            }
