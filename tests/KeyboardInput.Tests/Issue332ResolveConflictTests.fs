...
let tests =
    testList
        "Issue 332 resolve + conflict detection"
        [
            ...
            test "resolve agrees with tryFind (arguments flipped)" {
                ...
                Expect.equal
                    (Keymap.resolve keymap "ArrowUp")
                    (Keymap.tryFind "ArrowUp" keymap)
                    "resolve is tryFind flipped"
            }
            ...
            test "validate surfaces a shared-command conflict (many keys -> one command)" {
                let keymap =
                    Keymap.ofBindings [ binding "Space" "Confirm"; binding "Enter" "Confirm" ]
            ...
            test "validateBindings reports both kinds, duplicate-key first then shared-command" {
                ...
                let bindings =
                    [ binding "Space" "Fire"; binding "Space" "Jump"; binding "Enter" "Jump" ]

                let codes = Keymap.validateBindings bindings |> List.map (fun d -> d.Code)

                Expect.equal
                    codes
                    [ "DuplicateKeyBinding"; "SharedCommandBinding" ]
                    "duplicate-key precedes shared-command"
            }
            ...
            test "validateBindings is deterministic across binding order" {
                ...
                Expect.equal
                    (Keymap.validateBindings a)
                    (Keymap.validateBindings b)
                    "reordering the input does not reorder diagnostics"
            }
            ...
            test "validateBindings and validate are empty on clean input" {
                ...
                Expect.isEmpty
                    (Keymap.validate (Keymap.ofBindings bindings))
                    "no conflict in distinct keys/commands (keymap)"
            }
        ]
