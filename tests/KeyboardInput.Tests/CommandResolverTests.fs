...
let private key value = InputGesture.KeyChord(InputKeyIdentity.LogicalKey value, none)
let private step value = InputKeyIdentity.LogicalKey value, none
let private binding context command gesture = { Context = context; Command = command; Gesture = gesture }
...
let private catalog =
    { Contexts =
        [ { Id = "workspace"; Priority = 10; Exclusive = false; Overlaps = [ "editor" ] }
          { Id = "editor"; Priority = 20; Exclusive = false; Overlaps = [ "workspace" ] }
...
let private invocations effects =
    effects
    |> List.choose (function CommandResolverEffect.InvokeCommand value -> Some value.Command | _ -> None)

[<Tests>]
let tests =
    testList "SVG-INPUT-01 deterministic modal resolver" [
        test "terminal prefix waits for its injected deadline and fires once" {
            ...
            let next, deadlineEffects =
                CommandResolver.update catalog (CommandResolverObservation.DeadlineElapsed("sequence:e1", [ "prefix" ])) state
            Expect.equal (invocations deadlineEffects) [ "prefix" ] "deadline admits the terminal"
            let _, staleEffects =
                CommandResolver.update catalog (CommandResolverObservation.DeadlineElapsed("sequence:e1", [ "prefix" ])) next
        ...
        test "matching continuation cancels the old deadline and invokes the sequence" {
            ...
            let _, effects = CommandResolver.update catalog (press "e2" "keyboard" (key "C") [ "comment" ]) state
        ...
        test "nonmatching continuation retries exactly once at the root" {
            ...
            Expect.equal (effects |> List.filter ((=) (CommandResolverEffect.PreventDefault "e2")) |> List.length) 1 "one host event is claimed once"
        }

        test "deadline rechecks current availability" {
            let state, _ = run [ press "e1" "keyboard" (key "K") [ "prefix"; "comment" ] ]
            let _, effects = CommandResolver.update catalog (CommandResolverObservation.DeadlineElapsed("sequence:e1", [])) state
        ...
        test "profile replacement cancels pending input and applies immediately" {
            ...
            Expect.contains replacementEffects (CommandResolverEffect.CancelDeadline "sequence:e1") "old profile prefix is cancelled"
            Expect.equal replaced.Profile replacement "new profile is active in the same transition"
            let _, effects = CommandResolver.update catalog (press "e2" "keyboard" (key "H") [ "commit" ]) replaced
        ...
        test "continuous action remains held until its final owning source releases" {
            ...
            Expect.equal (pressed |> List.choose (function CommandResolverEffect.HeldActionChanged(command, isHeld) -> Some(command, isHeld) | _ -> None)) [ "move", true ] "first owner starts the aggregate"

            let oneOwner, firstRelease = CommandResolver.update catalog (release "u1" "keyboard-a" (key "ArrowRight")) state
            Expect.isEmpty (firstRelease |> List.choose (function CommandResolverEffect.HeldActionChanged(command, isHeld) -> Some(command, isHeld) | _ -> None)) "one owner remains"
            let _, finalRelease = CommandResolver.update catalog (release "u2" "keyboard-b" (key "ArrowRight")) oneOwner
            Expect.contains finalRelease (CommandResolverEffect.HeldActionChanged("move", false)) "last owner stops the aggregate"
        }
        ...
        test "context replacement neutralizes ownership before a late release" {
            ...
            let reset, effects = CommandResolver.update catalog (CommandResolverObservation.ContextsChanged [ "workspace" ]) state
            Expect.contains effects (CommandResolverEffect.HeldActionChanged("move", false)) "context change recovers held state"
            let _, late = CommandResolver.update catalog (release "u1" "keyboard" (key "ArrowRight")) reset
        ...
        test "release ownership survives modifier changes after the press" {
            let shifted = InputGesture.KeyChord(InputKeyIdentity.LogicalKey "ArrowRight", { none with Shift = true })
            let state, _ = run [ press "p1" "keyboard" (key "ArrowRight") [ "move" ] ]
            let _, effects = CommandResolver.update catalog (release "u1" "keyboard" shifted) state
            Expect.contains effects (CommandResolverEffect.HeldActionChanged("move", false)) "release matches the owned key identity"
        }
        ...
        test "capture receives an unbound gesture before command lookup and restores focus" {
            ...
            let next, effects = CommandResolver.update catalog (press "e1" "keyboard" (key "Z") []) state
            Expect.contains effects (CommandResolverEffect.CapturedGesture(key "Z")) "raw gesture is returned"
            Expect.contains effects (CommandResolverEffect.RequestFocus "binding-editor") "capture restores its focus owner"
        ...
        test "exclusive modal context blocks every lower context" {
            let state, _ = run [ CommandResolverObservation.PushModal { Context = "modal"; RestoreFocus = "canvas" } ]
            let _, blocked = CommandResolver.update catalog (press "e1" "keyboard" (key "H") [ "help" ]) state
            Expect.isEmpty (invocations blocked) "workspace does not receive modal input"
            let _, accepted = CommandResolver.update catalog (press "e2" "keyboard" (key "Enter") [ "commit"; "modal.accept" ]) state
            Expect.equal (invocations accepted) [ "modal.accept" ] "modal command resolves"
        }

        test "an unknown modal owner fails closed" {
            let state, _ = run [ CommandResolverObservation.PushModal { Context = "missing"; RestoreFocus = "canvas" } ]
            let _, effects = CommandResolver.update catalog (press "e1" "keyboard" (key "H") [ "help" ]) state
        ...
        test "Escape unwinds capture modal and prefix in innermost order without fallthrough" {
            ...
            let afterCapture, effects1 = CommandResolver.update catalog (CommandResolverObservation.Escape("esc1", "keyboard")) captured
            Expect.contains effects1 (CommandResolverEffect.RequestFocus "bindings") "capture closes first"

            let modal, _ = CommandResolver.update catalog (CommandResolverObservation.PushModal { Context = "modal"; RestoreFocus = "canvas" }) afterCapture
            let afterModal, effects2 = CommandResolver.update catalog (CommandResolverObservation.Escape("esc2", "keyboard")) modal
            Expect.contains effects2 (CommandResolverEffect.RequestFocus "canvas") "modal closes next"

            let pending, _ = CommandResolver.update catalog (press "e1" "keyboard" (key "K") [ "prefix"; "comment" ]) afterModal
            let cleared, effects3 = CommandResolver.update catalog (CommandResolverObservation.Escape("esc3", "keyboard")) pending
        ...
        test "composition focus takeover disconnect and disposal neutralize transient state" {
            let pending, _ = run [ press "e1" "keyboard" (key "K") [ "prefix"; "comment" ] ]
            let composing, compositionEffects = CommandResolver.update catalog CommandResolverObservation.CompositionStarted pending
            Expect.contains compositionEffects (CommandResolverEffect.CancelDeadline "sequence:e1") "composition cancels prefix"
            let _, ignored = CommandResolver.update catalog (press "e2" "keyboard" (key "Enter") [ "commit" ]) composing
            Expect.isEmpty ignored "composition owns key input"

            let held, _ = run [ press "p1" "pad" (key "ArrowRight") [ "move" ] ]
            let disconnected, disconnectEffects = CommandResolver.update catalog (CommandResolverObservation.SourceDisconnected "pad") held
            Expect.contains disconnectEffects (CommandResolverEffect.HeldActionChanged("move", false)) "disconnect recovers its held contribution"
            let taken, _ = CommandResolver.update catalog CommandResolverObservation.ModalTakenOver disconnected
            let disposed, _ = CommandResolver.update catalog CommandResolverObservation.Dispose taken
            let same, effects = CommandResolver.update catalog (press "e3" "keyboard" (key "H") [ "help" ]) disposed
