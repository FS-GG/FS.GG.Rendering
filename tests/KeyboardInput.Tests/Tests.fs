...
let recordFields<'T> =
    FSharpType.GetRecordFields(typeof<'T>) |> Array.map _.Name |> Set.ofArray
    ...
    testList
        ...
        [
            ...
            test "runtime model exposes layout modes pending sequence diagnostics and state display state" {
                ...
                |> List.iter (fun field -> Expect.isTrue (Set.contains field fields) $"KeyboardModel exposes {field}")
            }
            ...
            test "focus loss clears temporary state preserves persistent modes and reports diagnostics" {
                ...
                Expect.equal
                    recovered.PersistentModeState["layout"]
                    "qwerty"
                    "persistent mode state survives focus loss"

                Expect.exists
                    effects
                    (function
                    | ReportKeyboardDiagnostic diagnostic when diagnostic.Code = "FocusLostRecovered" -> true
                    | _ -> false)
                    "focus loss reports recovery diagnostic"
            }
            ...
            test "viewer keyboard events preserve down and up direction for interpreters" {
                ...
                let upKey, isStillDown =
                    ViewerKeyboard.normalizeEvent
                        {
                            RawKey = "Escape"
                            Direction = ViewerKeyDirection.KeyUp
                        }
            ...
            test "SVG intent mapping preserves composition and native editing" {
                Expect.equal
                    (ViewerKeyboard.tryMapSvgIntent "ArrowRight" true false false)
                    (Some SvgKeyboardIntent.FocusNext)
                    "forward arrow maps once"

                Expect.equal
                    (ViewerKeyboard.tryMapSvgIntent "ArrowLeft" true false false)
                    (Some SvgKeyboardIntent.FocusPrevious)
                    "back arrow maps once"

                Expect.equal
                    (ViewerKeyboard.tryMapSvgIntent "Enter" true false false)
                    (Some SvgKeyboardIntent.ActivateFocused)
                    "activation maps"

                Expect.equal
                    (ViewerKeyboard.tryMapSvgIntent "Escape" true false false)
                ...
                Expect.isNone
                    (ViewerKeyboard.tryMapSvgIntent "ArrowRight" true true false)
                    "IME composition is not consumed"

                Expect.isNone
                    (ViewerKeyboard.tryMapSvgIntent "ArrowRight" true false true)
                    "native editable controls keep their key"
...
let feature085NormalizeFamilies =
    ...
    testList
        "Feature 085 normalize key-name families (US3)"
        [
            ...
            test "the families are case-insensitive" {
                for spelling in [ "number5"; "DIGIT5"; "KeyPad5"; "kEy5" ] do
                    Expect.equal
                        (ViewerKeyboard.normalize spelling)
                        (Digit 5)
                        (sprintf "%s -> Digit 5 (case-insensitive)" spelling)
            }
            ...
            test "unrecognized names still normalize to Unknown raw (totality, FR-008)" {
                Expect.equal
                    (ViewerKeyboard.normalize "Totally-Unknown")
                    (Unknown "Totally-Unknown")
                    "unknown stays Unknown raw"

                Expect.equal (ViewerKeyboard.normalize "Number") (Unknown "Number") "prefix-only is not a digit family"

                Expect.equal
                    (ViewerKeyboard.normalize "KeyLong")
                    (Unknown "KeyLong")
                    "multi-char Key suffix is not a single key"
            }
