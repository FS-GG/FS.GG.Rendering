...
let tests =
    testList
        "Issue 334 keymap JSON codec (round-trip, versioned envelope)"
        [
            ...
            test "round-trips keys and commands that need JSON escaping" {
                let tricky =
                    Keymap.ofBindings [ binding "Quote\"Key" "cmd\\with\ttabs\nand\"quotes" ]
            ...
            test "decode rejects an unknown format discriminator" {
                let bytes =
                    Encoding.UTF8.GetBytes """{ "format": "other", "version": 1, "bindings": [] }"""
            ...
            test "decode rejects an unsupported version" {
                let bytes =
                    Encoding.UTF8.GetBytes """{ "format": "fsgg.keymap", "version": 99, "bindings": [] }"""
