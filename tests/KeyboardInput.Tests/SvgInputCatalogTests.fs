...
let private logical value =
    InputGesture.KeyChord(InputKeyIdentity.LogicalKey value, mods)

let private physical value =
    InputGesture.KeyChord(InputKeyIdentity.PhysicalCode value, mods)
...
let tests =
    testList
        "SVG-INPUT-01 portable command catalog"
        [
            test "ordered replace alias and unbind overrides stay distinct" {
                ...
                | Ok effective ->
                    Expect.equal
                        effective.Bindings.Length
                        3
                        "replace drops old command bindings, alias adds one, and unbind removes one"

                    Expect.contains
                        effective.Bindings
                        (binding "editor" "editor.commit" (physical "KeyK"))
                        "replacement retains physical identity"
                    ...
                    Expect.isFalse
                        (effective.Bindings |> List.exists (fun value -> value.Command = "editor.cancel"))
                        "explicit unbind remains unbound"
            }
            ...
            test "legacy keymap migration preserves logical key identity" {
                ...
                Expect.equal
                    migrated.Defaults
                    [ binding "editor" "editor.commit" (logical "Space") ]
                    "old key is not reinterpreted as a physical code"
            ...
            test "equal-priority overlap is ambiguous independent of insertion order" {
                ...
                let reverse =
                    CommandInput.compile catalog (profile (List.rev bindings) []) |> errorCodes
            ...
            test "codec round-trips physical logical AltGraph and ordered overrides byte-identically" {
                    ...
                    profile
                        [
                            binding
                                "editor"
                                "editor.commit"
                                (InputGesture.KeyChord(InputKeyIdentity.LogicalKey "é", altGraph))
                        ]
                        [
                            InputBindingOverride.ReplaceCommand(
                                "editor.commit",
                            ...
                            InputBindingOverride.AddAlias(
                                binding "editor" "editor.commit" (InputGesture.Touch "double-tap")
                            )
                            InputBindingOverride.UnbindCommand "editor.cancel"
                        ]
                ...
                Expect.stringStarts
                    ...
                    "fsgg.input-profile\t1\nschema\t"
                    "versioned envelope is explicit"
            ...
            test "codec rejects malformed and incomplete envelopes without throwing" {
                ...
                match
                    InputProfileCodec.decode (
                        Encoding.UTF8.GetBytes "fsgg.input-profile\t1\nschema\tZnNnZy5pbnB1dC1wcm9maWxlLzE=\nid\tYQ==\n"
                    )
                with
